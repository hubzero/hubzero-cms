	...
	switch ($this->task) {
		case 'update':
			...
			if (is_object($juser) && array_key_exists('auth_link_id', $juser) && is_numeric($juser->get('username')) && $juser->get('username') < 0) {
				...
				$html .= '<div id="link-existing">'."\n";
				$html .= '<p class="passed">You\'ve logged in successfully with your ' . ucfirst($hzad->authenticator) . ' account, but it doesn\'t seem to be linked to a current hub account. You can:</p>'."\n";

				// Option #1
				$html .= '<div id="option1-link-existing" class="options"><div class="clickable">Log in with an existing ' . $this->sitename . ' account, and link your ' . ucfirst($hzad->authenticator) . ' account to it</div>'."\n";
				$html .= '<div id="option1-inner" class="inner">'."\n";
				$conflict = false;
				...
				if(array_key_exists('email', $this->xregistration->_invalid) && !array_key_exists('email', $this->xregistration->_missing))
				{
					...
					$html .= '<p><span class="important">Is one of these you?</span> Based on your email address, we found the following accounts that may be yours.' .
						' Click one of them to login with that existing account ' .
						' and link it up with your ' . ucfirst($hzad->authenticator) . ' account.</p>';

					$html .= '<div id="account-suggestions">'."\n";

					foreach($username as $u)
					{
						$user_id  = JUserHelper::getUserId($u);
						$user     = JFactory::getUser($u);
						$pname    = (Hubzero_Auth_Link::find_by_user_id($user->id)) ? Hubzero_Auth_Link::find_by_user_id($user->id) : array("auth_link_name" => 'hubzero');

						foreach($pname as $p)
						{
							$html .= '<a href="' . JRoute::_('/logout?return=' . base64_encode(JRoute::_('/login?authenticator=' . $p['auth_domain_name'] . '&return=' .
								base64_encode(JRoute::_('/login?authenticator=' . $hzad->authenticator))))) .
								'"><div class="account-group"><div class="auth_link_icon" id="' . $p['auth_domain_name'] .
								'"></div><p><span class="user-icon">' . $user->name . '</span><br /><span class="email-icon">' . $user->email . '</span></p></div></a>'."\n";
						}
					}
					$html .= '</div><!-- / #account-suggestions -->'."\n";
				}

				$html .= '<div class="clear"></div>';

				$html .= '<div id="other-links">'."\n";
				$html .= '<p><span class="important">Have another account';
				$html .= ($conflict) ? ", but don't see it above" : '';
				$html .= '?</span> Choose one of the following that best matches your scenario:</p>'."\n";

				// Loop through the auth plugins
				foreach($plugins as $plugin) {
					$name = ($plugin->name == 'hubzero') ? 'local' : $plugin->name;
					if($plugin->name != $hzad->authenticator) {
						$html .= '<a href="' . JRoute::_('/logout?return=' .
				...
				$html .= '</div><!-- / #other-links -->'."\n";
				$html .= '</div><!-- / #option1-inner -->'."\n";
				$html .= '</div><!-- / #option1-link-existing -->'."\n";

				// Option #2
				$html .= '<div id="option2-create-new" class="options"><div class="clickable">Create a new account using your ' . ucfirst($hzad->authenticator) . ' identity</div></div>'."\n";

				$html .= '</div><!-- / #link-existing -->'."\n";
				$html .= '<div class="clear"></div>'."\n";

				// Start of main registration section (wrap the rest of the registration page in a div so we can hide it with javascript)
				$html .= '<div id="registration-main">'."\n";
			}
