...
<script type="text/javascript">
    ...
    function togglesecurity(a,show)
    {
        a.style.display = show? 'block' : 'none';
    }

    function CheckIncidentType()
    {
        var selectedItem = document.getElementById('security-incidenttype');
        var otherTextbox = document.getElementById('security-incidenttype-other');
        
        if(selectedItem.value == 'Other')
        {
            otherTextbox.disabled = false;
            otherTextbox.style.backgroundColor = "#ffffff";
        }
        else
        {
            otherTextbox.value = '';
            otherTextbox.disabled = true;
            otherTextbox.style.backgroundColor = "#eeeeee";
        }
    }
...
<script type="text/javascript">
        ...
        {
            //echo("alert('checked');");
            echo("function initSecurityIncident() { document.getElementById('isSecurityIncident').checked = true; togglesecurity(document.getElementById('securityfields'), true); }");
        }
        else
        {
            //echo("alert('nott checked');");
            echo("function initSecurityIncident() { document.getElementById('isSecurityIncident').checked = false; togglesecurity(document.getElementById('securityfields'), false); }");
        }
        ?>

    window.addEvent('domready', initSecurityIncident);
...
<?php } ?>
                ...
                <br/><input style="width:20px;" type="checkbox" id="isSecurityIncident" name="isSecurityIncident" onclick="togglesecurity(document.getElementById('securityfields'),this.checked)" value='1' <?php echo (isset($_POST['isSecurityIncident']) && $_POST['isSecurityIncident'] == '1') ? 'checked' : '';  ?>> Security related incident?
                            
		</fieldset>
		<div id="securityfields">
                <fieldset>

                    <h3>Security Incident Information</h3>

                    <label>
                        Site <span class="required"><?php echo JText::_('COM_FEEDBACK_REQUIRED'); ?></span>
                    </label>
                    <input type="text" name="security-site" maxlength="100" value = "<?php echo (isset($_POST['security-site'])) ? $_POST['security-site'] : ''; ?>"/>

                    <label>
                        POC Phone Number <span class="required"><?php echo JText::_('COM_FEEDBACK_REQUIRED'); ?></span>
                    </label>
                    <input type="text" name="security-poc-phone" maxlength="50" value = "<?php echo (isset($_POST['security-poc-phone'])) ? $_POST['security-poc-phone'] : ''; ?>"/>

                    <label>
                        Type of Incident <span class="required"><?php echo JText::_('COM_FEEDBACK_REQUIRED'); ?></span>
                    </label>

                    <select id="security-incidenttype" name="security-incidenttype" style="width:250px" onchange="CheckIncidentType();">
                        <option value="Denial of Service">Denial of Service </option>
                        <option value="Malware">Malware</option>
                        <option value="Site Compromise">Site Compromise</option>
                        <option value="Host Compromise">Host Compromise</option>
                        <option value="Probe/Scan">Probe/Scan</option>
                        <option value="Other">Other</option>
                    </select>

                    <input type="text" id="security-incidenttype-other" name="security-incidenttype-other" style="margin-left:20px; width:200px; background-color:#eeeeee; " value = "<?php echo (isset($_POST['security-incidenttype-other'])) ? $_POST['security-incidenttype-other'] : ''; ?>">
