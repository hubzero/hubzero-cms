...
<?php } ?>				
		...
		function addtagtxt(sometext) {
			var g = document.getElementById("maininput");
			if ( g != null) {
		...
		<fieldset>	
			...
			<p>Topics (You may open and choose as many options from this list as you like - they will be appended to the tags list at the top)</p>
				<select id="topic" multiple="none" onchange="addtagtext('topic');">
					<option value="" />None</option>
		...
		<p>Multimedia Type</p>
				<select id="type" multiple="none" onchange="addtagtext('type');">
		...
		if (window!=window.top) {
			...
			if (parent.parent.wargs.length > 0) {
		...
		window.onload = setTimeout("pulltags()", 1000);
