...
defined('_JEXEC') or die( 'Restricted access' );
	...
	if ($resource->access == 3 && !in_array($resource->group_owner, $usersgroups) && !$authorized) {
		...
		if($tab != 'play' && is_object($helper->firstChild) ) {
			$child_params =& new JParameter( $helper->firstChild->params );
			$link_action = $child_params->get( 'link_action', '' );
			$url = ResourcesHtml::processPath($option, $helper->firstChild, $resource->id, $link_action);
			$xtra = ' '. ResourcesHtml::getFileAttribs( $helper->firstChild->path );
			$html .= ResourcesHtml::primaryButton('download', $url, 'View Video', $xtra, $helper->firstChild->title, '');
		}
