...
switch ($this->level)
{
	case 1:
		...
		$html .= '<ul id="ultags">';
		if ((!$tg2) && (!isset($this->designator))) {
			$html .= '<li><a id="col1_all" class="';
			if ($tg == '') {
				$html .= 'open';
			}
			
			$html .= '" href="javascript:HUB.TagBrowser.nextLevel(\''.$type.'\',\'\',\'\',2,\'col1_all\',\''.$id.'\',\''.$terms.'\');">[ All ]</a></li>';
		}
		$lis = '';
		$i = 0;
		foreach ($tags as $tag)
		{
			...
			$descSep = ($tag->admin)? ': ' : ''; //separator for admin tag description
			$descTag = ($tag->admin)? strtoupper(stripslashes($tag->description)) : ''; //separator for admin tag description
			...
			$li .= '" href="javascript:HUB.TagBrowser.nextLevel(\''.$type.'\',\''.$tag->tag.'\',\''.$tg2.'\',2,\'col1_'.$tag->tag.'\',\''.$id.'\',\''.$terms.'\');">'.$descTag.$descSep.stripslashes($tag->raw_tag).' ('.$tag->ucount.')</a></li>';
