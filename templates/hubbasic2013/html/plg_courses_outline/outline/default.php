...
<?php } else { ?>
	...
	<?php /*if ($this->course->access('manage')) { ?>
		<div class="outline-toolbar">
			Some kind of manager or instruction text here? <a class="btn edit icon-edit" href="<?php echo JRoute::_($base . '&active=outline&action=build'); ?>">Build outline</a>
		</div>
	<?php }*/ ?>
									...
									<?php foreach ($agt->children() as $ag) { ?>
										<?php if ($ag->isAvailable() && $ag->isPublished()) : ?>
											<div class="asset-group">
												...
												$link = '<span class="asset-primary asset-unavailable">' . $title . '</span>';
												...
												if ($ag->assets()->total()) 
												{
													...
													foreach ($ag->assets() as $a)
													{
														if (($a->isAvailable() || $a->get('type') == 'form') && $a->isPublished())
														{
															...
															// ' . $a->get('subtype') . '
															$link = '<a class="asset-primary" href="' . $href . '"' . $target . '>' . ($title ? $title : $this->escape(stripslashes($a->get('title')))) . '</a>';
															break;
														}
													}
												} 
