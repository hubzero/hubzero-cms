	...
	<?php foreach ($this->sections as $k => $section) : ?>
		<?php
			...
			if ($section['name'] == 'overview' && count($this->pages) > 0)
			{
				...
				{
					$item  = "<li class=\"protected group-overview-tab\"><span class=\"overview\">Overview</span>";
				}
				else
				{
					$item  = "<li class=\"{$liClass} group-overview-tab\">";
					$item .= "<a class=\"overview\" title=\"{$this->group->get('description')}'s Overview Page\" href=\"{$link}\">Overview</a>";
				} 
			...
			else
			{
				...
				elseif($access == 'members' && !in_array($this->juser->get("id"), $this->group->get('members'))) 
				{
					...
					$item .= "<span class=\"{$class}\">{$title}</span>";
					$item .= "</li>";
				}
				...
				else
				{
					...
					$item .= "<a class=\"{$class}\" title=\"{$this->group->get('description')}'s {$title} Page\" href=\"{$link}\">{$title}</a>";
