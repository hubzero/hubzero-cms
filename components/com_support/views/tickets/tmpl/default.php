...
?>
					<tr class="<?php echo ($row->status == 2) ? 'closed' : $row->severity; ?>">
						...
						<td>
							<a href="<?php echo JRoute::_('index.php?option='.$this->option.'&task=ticket&id='.$row->id.'&find='.$fstring.'&limit='.$this->filters['limit'].'&limitstart='.$this->filters['start']); echo ($fstring != '') ? '?find='.$fstring : ''; ?>" title="<?php echo $row->report; ?>"><?php echo stripslashes($row->summary); ?></a>
