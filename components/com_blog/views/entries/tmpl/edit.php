...
<?php } ?>
			...
			<label for="entrycontent">
				...
				echo $editor->display('entry[content]', 'entrycontent', $this->escape(stripslashes($this->entry->content)), '', '50', '40');
				?>
...
<?php if ($this->entry->id) { ?>
			<a href="<?php echo JRoute::_('index.php?option='.$this->option.'&task='.JHTML::_('date',$this->entry->publish_up, '%Y', 0).'/'.JHTML::_('date',$this->entry->publish_up, '%m', 0).'/'.$this->entry->alias); ?>">[ Cancel ]</a>
<?php } ?>
		</p>
	</form>
</div><!-- / .section -->
