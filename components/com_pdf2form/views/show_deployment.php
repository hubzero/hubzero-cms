...
?>
<h2>Deployment: <?php echo htmlentities($title) ?></h2>
<p class="distribution-link">Link to distribute:
	<a href="<?php echo $dep->getLink() ?>"><?php echo $dep->getLink() ?></a>
	<span class="state <?php echo $dep->getState() ?>"><?php echo $dep->getState() ?></span>
</p>
<form action="/pdf2form" method="post" id="deployment">
	...
	<fieldset>
		...
		<input type="hidden" name="deploymentId" value="<?php echo $dep->getId() ?>" />
...
<table class="tablesorter">
	...
	<tbody>
		<?php foreach ($dep->getResults() as $row): ?>
			<tr>
				<td><?php echo htmlentities($row['name']); ?></td><td><?php echo htmlentities($row['email']) ?></td>
				<td><?php echo $row['version'] ?></td><td><?php echo number_format($row['score'], 1) ?></td>
				<td><?php echo date('Y-m-d H:i', strtotime($row['finished'])) ?></td>
			</tr>
		<?php endforeach; ?>
	</tbody>
</table>
