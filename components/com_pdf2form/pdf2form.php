...
try {
	switch ($task) {
		...
		case 'upload':
			...
			if ($pdf->hasErrors()) {
				...
				header('Location: /pdf2form?task=layout&formId='.$pdf->getId());
				exit();
			}
		break;
		case 'layout':
		...
		case 'updateDeployment':
			...
			if ($dep->hasErrors(NULL, TRUE)) {
				...
				header('Location: /pdf2form?task=showDeployment&id='.$dep->save($_POST['deploymentId']).'&formId='.$pdf->getId());
				exit();
			}
		break;
		case 'showDeployment':
		...
		case 'startWork':
			...
			header('Location: /pdf2form?task=complete&crumb='.$_POST['crumb'].(isset($_POST['tmpl']) ? '&tmpl='.$_POST['tmpl'] : ''));
		exit();
		case 'saveProgress':
	...
	header('Location: /login?return='.base64_encode($_SERVER['REQUEST_URI']));
	exit();
}
