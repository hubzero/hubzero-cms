...
defined('_JEXEC') or die('Restricted access');
	...
	{
		$comment = '<p class="warning">' . JText::_('COM_WIKI_COMMENT_REPORTED_AS_ABUSIVE') . '</p>';
	}
	else
	{
	...
	<li class="comment <?php echo $cls; ?>" id="c<?php echo $this->comment->get('id'); ?>">
		...
		<?php if ($this->depth < $this->config->get('comments_depth', 3)) { ?>
			<div class="addcomment comment-add<?php if (JRequest::getInt('reply', 0) != $this->comment->get('id')) { echo ' hide'; } ?>" id="comment-form<?php echo $this->comment->get('id'); ?>">
				...
				<p class="warning">
					<?php echo JText::_('COM_WIKI_WARNING_LOGIN_REQUIRED', '<a href="' . JRoute::_('index.php?option=com_users&view=login&return=' . base64_encode(JRoute::_($this->page->link('comments'), false, true))) . '">' . JText::_('COM_WIKI_LOGIN') . '</a>'); ?>
				</p>
				<?php } else { ?>
				<form id="cform<?php echo $this->comment->get('id'); ?>" action="<?php echo JRoute::_($this->page->link('comments')); ?>" method="post" enctype="multipart/form-data">
					<a name="commentform<?php echo $this->comment->get('id'); ?>"></a>
					<fieldset>
						<legend><span><?php echo JText::sprintf('COM_WIKI_REPLYING_TO', (!$this->comment->get('anonymous') ? $name : JText::_('COM_WIKI_ANONYMOUS'))); ?></span></legend>
						...
						<label id="comment-anonymous-label" for="comment-anonymous">
							...
							<?php echo JText::_('COM_WIKI_POST_COMMENT_ANONYMOUSLY'); ?>
						</label>
