...
<?php if ($this->config->get('access-edit-tag') || $this->config->get('access-delete-tag')) { ?>
	...
	<?php if ($this->config->get('access-delete-tag')) { ?>
								<a class="delete delete-tag" href="<?php echo JRoute::_('index.php?option='.$this->option.'&task=delete&id[]='.$row->id.'&search='.urlencode($this->filters['search']).'&sortby='.JRequest::getVar("sortby","").'&limit='.JRequest::getVar("limit",25).'&limitstart='.JRequest::getVar("limitstart",0)); ?>">
	...
	<?php if ($this->config->get('access-edit-tag')) { ?>
								<a class="edit" href="<?php echo JRoute::_('index.php?option='.$this->option.'&task=edit&id='.$row->id); ?>" title="<?php echo JText::_('COM_TAGS_EDIT_TAG'); ?> &quot;<?php echo stripslashes($row->raw_tag); ?>&quot;">
