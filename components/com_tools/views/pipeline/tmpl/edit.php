...
<?php if (!$this->id) { ?>
		<h3><?php echo JText::_('COM_TOOLS_SIDE_HOW_CONTRIBUTE'); ?></h3>
		<p><?php echo JText::_('COM_TOOLS_SIDE_EASY_PROCESS').' '.JText::_('COM_TOOLS_VIEW').' <a href="contribute/tools">'.JText::_('COM_TOOLS_RESOURCES').'</a> '.JText::_('COM_TOOLS_EXPLAINING_CONTRIBUTION').'.'; ?></p>
...
<?php if ($this->id) { ?>
					...
					<?php if (isset($this->defaults['published']) && $this->defaults['published']) { ?>
						<a href="<?php echo JRoute::_('index.php?option=' . $this->option . '&controller=' . $this->controller . '&task=versions&app=' . $this->defaults['toolname']); ?>"><?php echo JText::_('COM_TOOLS_ALL_VERSIONS'); ?></a>
					<?php } ?>
<?php } else { ?>
