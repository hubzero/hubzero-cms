...
class Threads extends SiteController
{
	...
	public function latestTask()
	{
		...
		if (count($rows) > 0)
		{
			foreach ($rows as $row)
			{
				...
				$item->date        = $date;
				$item->category    = ($row['scope_id'] == 0) ? \JText::_('COM_FORUM') : stripslashes($group->get('description'));
				$item->author      = $author;
	...
	public function editTask($post=null)
	{
		...
		if ($this->juser->get('guest'))
		{
			...
			$this->setRedirect(
				\JRoute::_('index.php?option=com_users&view=login&return=' . base64_encode($return)) . \JText::_('COM_FORUM_LOGIN_NOTICE'),
				'warning'
			);
			return;
		}
	...
	public function saveTask()
	{
		...
		{
			$this->setRedirect(
				\JRoute::_('index.php?option=com_users&view=login&return=' . base64_encode(\JRoute::_('index.php?option=' . $this->_option)))
			);
			return;
		}
		...
		if ($model->get('parent'))
		{
			...
			{
				$this->addComponentMessage(\JText::_('COM_FORUM_ERROR_THREAD_CLOSED'), 'error');
				$this->editTask($model);
				return;
			}
		}
	...
	public function downloadTask()
	{
		...
		if (!$this->database)
		{
			throw new Exception(\JText::_('COM_FORUM_DATABASE_NOT_FOUND'), 500);
		}
		...
		{
			throw new Exception(\JText::_('COM_FORUM_NOT_AUTH_FILE'), 403);
		}
		...
		{
			throw new Exception(\JText::_('COM_FORUM_FILE_NOT_FOUND') . ' ' . $filename, 404);
		}
		...
		if (!$xserver->serve())
		{
			...
			throw new Exception(\JText::_('COM_FORUM_SERVER_ERROR'), 500);
		}
		else
		{
			exit;
