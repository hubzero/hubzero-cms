		...
		<div id="content-header-extra">
			<ul id="useroptions">
				<?php if ($this->row->id && $this->permissions->can('edit.permissions')) : ?>
					<li>
						<?php $permRoute = JRoute::_('index.php?option=' . $this->option . '&controller=permissions&scope=Hub&scope_id=' . $this->row->id . '&tmpl=component'); ?>
				...
				<?php foreach ($this->row->contacts as $contact) : ?>
					<div class="grouping contact-grouping grid" id="contact-<?php echo $contact->id; ?>-group">
						...
						<div class="col span2 omega">
							<a href="<?php echo JRoute::_($this->base . '&task=deletecontact&id=' . $contact->id); ?>" class="btn btn-danger icon-delete delete_contact" title="Delete contact">Delete</a>
						</div>
					</div>
				<?php endforeach; ?>
