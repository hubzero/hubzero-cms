...
<?php if ($this->app['sess']) { ?>
	<ul id="app-options">
		...
		<li class="app-close"><a href="<?php echo JRoute::_('index.php?option='.$this->option.'&task=stop&sess='.$this->app['sess']); ?>" title="Terminate this session"><span>Close</span></a></li>
	</ul>
<?php } ?>
	...
	<p class="warning">
		...
		foreach ($this->shares as $row) 
		{
			if ($row->viewuser != $juser->get('username')) {
				?>&nbsp; <a href="<?php echo JRoute::_('index.php?option='.$this->option.'&task=unshare&sess='.$this->app['sess'].'&username='.$row->viewuser); ?>" title="Remove this user from sharing"><?php echo $row->viewuser; ?></a><?php
			}
		}
		?>
	</p>
