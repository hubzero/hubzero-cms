...
public function displayTask($posts = NULL)
{
	...
	$this->view->set('var', "tests");
	if(in_array($access_level,$authlevel) && JFactory::getUser()->id)
	{
		if(isset($posts))
		{
			echo "posts from another area";
			
			//var_dump($posts);
			//$this->view->kevin = TRUE;
		...
		else
		{
			...
			{
				$posts = $model->loadAllPosts($this->view->filters['limit'], $this->view->filters['start'],0);
			}
		...
		foreach ($posts as $post)
		{
			...
			$post->created =  $dt->format('m-d-y h:i A'); // output = 2012-08-15 00:00:00
	...
	else if(JFactory::getUser()->id == FALSE) // have person login
	{
			...
			JText::_('FEEDAGGREGATOR_LOGIN_NOTICE'),
			'warning'
			);
	}
}
