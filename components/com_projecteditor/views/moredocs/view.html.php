...
class ProjectEditorViewMoreDocs extends JView{
  ...
  function display($tpl = null){
    ...
    /* @var $oMoreModel ProjectEditorModelMoreDocs */
    ...
    if($oAuthorizer->canView($oProject)){
      ...
      $oDocumentDataFileArray = $oMoreModel->findDataFileDocumentsByDirectory("Documentation", $oHideExperimentArray, $oProject->getId(), $iLowerLimit, $iUpperLimit, $iExperimentId, $iTrialId, $iRepetitionId);
      $_REQUEST[DataFilePeer::TABLE_NAME] = serialize($oDocumentDataFileArray);

      $iResultsCount = $oMoreModel->findDataFileDocumentsByDirectoryCount("Documentation", $oHideExperimentArray, $oProject->getId(), $iExperimentId, $iTrialId, $iRepetitionId);
      ...
      $oDistinctExperimentArray = $oMoreModel->findDistinctExperimentsByDirectory("Documentation", $oProject->getId(), $oHideExperimentArray);
      ...
      if($iExperimentId){
        $oDistinctTrialArray = $oMoreModel->findDistinctTrialsByDirectory("Documentation", $oProject->getId(), $iExperimentId, $oHideExperimentArray);
      ...
      if($iTrialId){
        $oDistinctRepetitionArray = $oMoreModel->findDistinctRepetitionsByDirectory("Documentation", $oProject->getId(), $iExperimentId, $iTrialId, $oHideExperimentArray);
