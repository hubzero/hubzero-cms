...
<?php
  ...
  $strAction = "/warehouse/projecteditor/project/".$oProject->getId()."/documentation";
  if($oExperiment){
    $strAction = "/warehouse/projecteditor/project/".$oProject->getId()."/experiment/".$oExperiment->getId()."/documentation";
  }

  $iFileCheckboxId = $oProject->getId();
  if($oExperiment){
    $iFileCheckboxId = $oExperiment->getId();
  }
  ...
  <div id="warehouseWindow" style="padding-top:20px;">
    ...
    <div id="overview_section" class="main section" style="width:100%;float:left;">
      ...
      <div class="subject">
        ...
        <p class="experimentTitle"><?php if($oExperiment){ echo $oExperiment->getTitle(); } ?></p>
            ...
            <tr id="drawings">
              <td nowrap>
                <p class="editorParagraph">
                  <label for="actags" class="editorLabel">Documents:</label>
                  <a style="border-bottom:0px;" href="#" onclick="return false;"
                     class="Tips3" title="Documents :: Please provide documents related to the project or experiment.">
                     <img alt="" src="<?php echo $this->baseurl."/templates/fresh/images/icons/helptab.png" ?>" />
                  </a>
                </p>
                <p><a title="Upload documents file" class="modal" href="/warehouse/projecteditor/uploadform?format=ajax&projid=<?php echo $this->iProjectId; ?>&experimentId=<?php echo $this->iExperimentId; ?>&path=<?php echo $this->strPath; ?>&uploadType=<?php echo $this->uploadType; ?>">Upload File</a></p>
              </td>
              <td width="100%">
                <table cellpadding="1" cellspacing="1">
                    <thead>
                      <th width="1">
                        <input id="checkAll" type="checkbox" name="checkAll" onClick="setAllCheckBoxes('frmProject', 'dataFile[]', this.checked, <?php echo $iFileCheckboxId; ?>);setFilesToDelete('frmProject', 'dataFile[]', 'cbxDelete', <?php echo $iFileCheckboxId; ?>, 'fileDeleteLink', 112);"/>
                    ...
                    <?php
                      ...
                      foreach($oDocumentArray as $iDrawingIndex=>$oDataFile){
                        ...
                        $strDataFileTitle = (StringHelper::hasText($oDataFile->getTitle())) ? $oDataFile->getTitle() : $oDataFile->getName();
                        $strDataFileDescription = (StringHelper::hasText($oDataFile->getDescription())) ? $oDataFile->getDescription() : $oDataFile->getPath();
                        if(StringHelper::hasString($strDataFileDescription, "\/nees\/home\/")){
                          $strDataFileDescription = str_replace("/nees/home/", "", $strDataFileDescription);
                        }

                        if($oExperiment){
                          $strExperimentName =  $oExperiment->getName();
                          $strExperimentTitle = $oExperiment->getTitle();
                          $strDataFileDescription = str_replace($strExperimentName, $strExperimentTitle, $strDataFileDescription);
                        }
                      ?>
                        <tr class="<?php echo $strBgColor; ?>">
                          <td width="1"><input id="<?php echo $iFileCheckboxId; ?>" type="checkbox" name="dataFile[]" value="<?php echo $oDataFile->getId(); ?>" onClick="setFilesToDelete('frmProject', 'dataFile[]', 'cbxDelete', <?php echo $iFileCheckboxId; ?>, 'fileDeleteLink', 112);"/></td>
                          ...
                          <td nowrap>
                            [<a class="modal" href="/warehouse/projecteditor/editdocument?format=ajax&projectId=<?php echo $this->iProjectId; ?>&experimentId=<?php echo $this->iExperimentId; ?>&dataFileId=<?php echo $oDataFile->getId(); ?>&path=<?php echo get_friendlyPath($strPath); ?>&requestType=<?php echo $this->uploadType; ?>&return=<?php echo $this->strReturnUrl; ?>&index=<?php echo $this->iPageIndex; ?>&display=<?php echo $this->iDisplay; ?>">Edit</a>]&nbsp&nbsp;
                            <?php
                              if($oExperiment){
                            ...
                            <?php
                              ...
                              }elseif($oAuthorizer->canDelete($oProject)){
                            ?>
                                [<a class="modal" href="/warehouse/projecteditor/delete?path=<?php echo $oDataFile->getPath(); ?>&format=ajax&eid=<?php echo $oDataFile->getId(); ?>&etid=112&return=<?php echo $this->strReturnUrl; ?>" title="Remove <?php echo $oDataFile->getName(); ?>">Delete</a>]
                            <?php
                              }
                        ...
                        <div class="sectheaderbtn">
                          <?php
                            ...
                            if($oExperiment){
                              ...
                              $bCanDelete = $oAuthorizer->canDelete($oProject);
                            }
