...
defined('_JEXEC') or die( 'Restricted access' );
	...
	$html  = '';
	$html .= '<div class="main section upperpane">'.n;
	$html .= '<div class="aside rankarea">'.n;
	// Show resource ratings
		if (!$thistool) {
			...
			if (isset($resource->curversion) && $resource->curversion) {
				$txt_meta .= ' '.JText::_('Consult the latest published version').' <a href="'.JRoute::_('index.php?option=com_resources'.a.'id='.$resource->id).'?rev='.$curtool->revision.'">'.$resource->curversion.'</a> '.JText::_('for most current information.');
			}
			
			// show empty meta data box
			$html .= '<div class="metaplaceholder"><p>'.$txt_meta.'</p></div>'.n;
		}
		
	
	$html .= ' </div><!-- / .aside -->'.n;	
	$html .= '<div class="subject">'.n;	
	$html .= ' <div class="overviewcontainer">'.n;
	...
	if ($params->get('show_authors')) {
			...
			if ($helper->contributors && $helper->contributors != '<br />') {
				$html .= ' <div id="authorslist">'.n;
				$html .= $helper->contributors.n;
				$html .= '</div>'.n;
			}
	}
	...
	$introtext = $resource->introtext ? ResourcesHtml::shortenText($resource->introtext, 255, 0) : ResourcesHtml::shortenText($resource->fulltext, 255, 0);
	$html .= $introtext;
	//$html .= ' <a href="">'.JText::_('Learn more').' &rsaquo;</a>';
	$html .= '</p>'.n;
	$html .= ' </div><!-- / .overviewcontainer -->'.n;
	
	$html .= ' <div class="aside launcharea">'.n;
	...
	if ($revision && $thistool) {
		...
		if ($resource->revision!='dev') {
			$versiontext .=  '<br /> '.ucfirst(JText::_('PUBLISHED_ON')).' ';
	...
	if(!$thistool) {
		$html .= t.t.t.t.'<p class="curversion">'.JText::_('VERSION').' '.$versiontext.'</p>'.n;
	}
	else if($revision == 'dev') {
		$html .= t.t.t.t.'<p class="devversion">'.JText::_('VERSION').' '.$versiontext;
		$html .= $resource->toolpublished ? ' <span>'.JText::_('View').' <a href="'.JRoute::_('index.php?option=com_resources'.a.'id='.$resource->id).'?active=versions">'.JText::_('other versions').'</a></span>' : '';
		$html .='</p>'.n;
	}
	else {
		// Show archive message		
		$msg = '<strong>'.JText::_('ARCHIVE').'</strong> '.JText::_('VERSION').' '.$versiontext;
		if (isset($resource->curversion) && $resource->curversion) {
			$msg .= ' <br />'.JText::_('LATEST_VERSION').': <a href="'.JRoute::_('index.php?option=com_resources'.a.'id='.$resource->id).'?rev='.$curtool->revision.'">'.$resource->curversion.'</a>.';
		}
		$msg .= ' <a href="'.JRoute::_('index.php?option=com_resources'.a.'id='.$resource->id.a.'active=versions').'">'.JText::_('All versions').'</a>';
		$html .= ResourcesHtml::archive($msg).n;			
	}
	
	// doi message
	if($revision != 'dev' && $resource->doi) {
	$html .= t.t.'<p class="doi">'.JText::_('DOI').': '.$config->get('doi').'r'.$resource->id.'.'.$resource->doi.' <span><a href="'.JRoute::_('index.php?option=com_resources'.a.'id='.$resource->id.a.'active=about').'#citethis">'.JText::_('cite this').'</a></span></p>'.n;
	}
	
	// Open/closed source
	if (isset($resource->toolsource) && $resource->toolsource == 1 && isset($resource->tool)) { // open source
		$html .= '<p class="opensource_license">'.JText::_('Open source').': <a class="popup" href="index.php?option=com_resources'.a.'task=license'.a.'tool='.$resource->tool.a.'no_html=1">license</a> ';
		$html .= ($resource->taravailable) ? ' |  <a href="index.php/'.$resource->tarname.'?option='.$option.a.'task=sourcecode'.a.'tool='.$resource->tool.'">'.JText::_('download').'</a> '.n : ' | <span class="unavail">'.JText::_('code unavaialble').'</span>'.n;
		$html .= '</p>'.n;	
	} elseif (isset($resource->toolsource) && !$resource->toolsource) { // closed source, archive page
		$html .= '<p class="closedsource_license">'.JText::_('TOOL_IS_CLOSED_SOURCE').'</p>'.n;
	}
	...
	if (!$thistool) {
		...
		$html .= t.t.'<p class="supdocs">'.n;
		if($url) {			
			$html .= t.t.t.'<span><span class="guide"><a href="'.$url.'">'.JText::_('First-Time User Guide').'</a></span></span>'.n;			
		}
		$html .= t.t.t.'<span class="viewalldocs"><a href="'.JRoute::_('index.php?option=com_resources'.a.'id='.$resource->id.a.'active=supportingdocs').'">'.JText::_('View All Supporting Documents').'</a></span>'.n;
		$html .= t.t.'</p>'.n;
	}
	
	} // --- end else (if group check passed)
	
	$html .= ' </div><!-- / .aside launcharea -->'.n;	
	$html .= ' </div><!-- / .subject -->'.n;
	...
	if ($resource->access == 3 && (!in_array($resource->group_owner, $usersgroups) || $authorized=0)) {
		...
		$html .= '<div class="clear sep"></div>'.n;	
		$html .= '</div><!-- / .main section -->'.n;		
		$html .= '<div class="main section">'.n;
		$html .= ' <div class="aside extracontent">'.n;
		...
		$html .= ' <p class="viewalltypes">'.JText::_('View all').' <a href="'.JRoute::_('index.php?option='.$option.a.'type='.$typenorm).'">'.$resource->getTypeTitle().'</a></p>'.n;*/
		...
		$html .= ' </div><!-- / .aside -->'.n;		
		$html .= ' <div class="subject tabbed">'.n;
