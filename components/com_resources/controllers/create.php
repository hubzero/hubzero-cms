...
class ResourcesControllerCreate extends \Hubzero\Component\SiteController
{
	...
	public function step_compose_process()
	{
		...
		$oldPath = JPATH_ROOT . DS . trim($this->config->get('uploadpath', '/site/resources'), DS) . ResourcesHtml::build_path($created, $session->get('resources_temp_id') ,'');
		$newPath = JPATH_ROOT . DS . trim($this->config->get('uploadpath', '/site/resources'), DS) . ResourcesHtml::build_path($row->created, $row->id, '');
