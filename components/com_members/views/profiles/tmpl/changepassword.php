...
<header id="content-header">
	...
	<div id="content-header-extra">
		<p>
			<a class="member btn" href="<?php echo JRoute::_('index.php?option='.$this->option.'&id='.$this->profile->get('uidNumber')); ?>"><?php echo JText::_('COM_MEMBERS_MYACCOUNT'); ?></a></li>
		</p>
...
<section class="main section">
	<div class="section-inner">
		...
		<form action="<?php echo JRoute::_($this->profile->getLink() . '&task=changepassword', true, true); ?>" method="post" id="hubForm">
			<div class="explaination">
				<p><?php echo JText::_('COM_MEMBERS_CHANGEPASSWORD_EXPLANATION'); ?></p>
				<p><?php echo JText::sprintf('COM_MEMBERS_PASSWORD_IF_FORGOTTEN_RESET', JRoute::_('index.php?option=com_users&task=logout&return=' . base64_encode('/users/reset'))); ?></p>
			</div>
			<fieldset>
				...
				<div class="grid">
					<div class="col span6">
						<label<?php echo ($this->change && (!$this->newpass || $this->newpass != $this->newpass2)) ? ' class="fieldWithErrors"' : ''; ?>>
							<?php echo JText::_('COM_MEMBERS_FIELD_NEW_PASS'); ?>
							<input name="newpass" id="newpass" type="password" value="" />
							<?php
							if ($this->change && !$this->newpass)
							{
								echo '<span class="error">' . JText::_('COM_MEMBERS_PASS_BLANK') . '</span>';
							}
							?>
						</label>
					</div>
					<div class="col span6 omega">
						<label<?php echo ($this->change && (!$this->newpass2 || $this->newpass != $this->newpass2)) ? ' class="fieldWithErrors"' : ''; ?>>
							<?php echo JText::_('COM_MEMBERS_FIELD_PASS_CONFIRM'); ?>
							<input name="newpass2" id="newpass2" type="password" value="" />
							<?php
							if ($this->change && !$this->newpass2)
							{
								echo '<span class="error">' . JText::_('COM_MEMBERS_PASS_MUST_CONFIRM') . '</span>';
							}
							if ($this->change && $this->newpass && $this->newpass2 && ($this->newpass != $this->newpass2))
							{
								echo '<span class="error">' . JText::_('COM_MEMBERS_PASS_NEW_CONFIRMATION_MISMATCH') . '</span>';
							}
							?>
						</label>
					</div>
			...
			<p class="submit">
				...
				<input class="btn btn-success" name="submit" id="password-change-save" type="submit" value="<?php echo JText::_('COM_MEMBERS_CHANGEPASSWORD'); ?>" />
			</p>
		</form>
	</div>
