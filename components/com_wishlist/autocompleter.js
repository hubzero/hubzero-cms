...
// Tag Browser
//----------------------------------------------------------
HUB.CompleteResource = {
	initialize: function() {
		var el = $('acresource');
		
		if (el) {
			var completer3 = new Autocompleter.Ajax.Json(el, 'index.php?option=com_wishlist&no_html=1&task=autocomplete&which=resource', {
...
HUB.CompleteGroup = {
	initialize: function() {
		...
		if (el) {
			...
			var completer4 = new Autocompleter.Ajax.Json(el, 'index.php?option=com_wishlist&no_html=1&task=autocomplete&which=group', {
				'minLength': 1, // We wait for at least one character
				'overflow': true, // Overflow for more entries
				'wrapSelectionsWithSpacesInQuotes': false,
				'injectChoice': function(choice) {
...
window.addEvent('domready', HUB.CompleteResource.initialize);
window.addEvent('domready', HUB.CompleteGroup.initialize);