...
if ($this->reply->content)
{
	...
	$this->reply->content = $p->parse($this->reply->content, $wikiconfig, false);
}

//$commenttype = $this->reply->created_by == $this->wishauthor && $this->reply->anonymous != 1 ?  'submittercomment' : 'plaincomment';
...
<div class="comment-content">
	<p class="comment-title">
		...
		<a class="permalink" href="<?php echo JRoute::_('index.php?option=' . $this->option . '&task=wish&id=' . $this->listid . '#c' . $this->reply->id); ?>" title="<?php echo JText::_('COM_WISHLIST_PERMALINK'); ?>">
			<span class="comment-date-at">@</span> <span class="time"><time datetime="<?php echo $this->reply->created; ?>"><?php echo JHTML::_('date', $this->reply->created, JText::_('TIME_FORMAT_HZ1')); ?></time></span> 
			<span class="comment-date-on"><?php echo JText::_('COM_WISHLIST_ON'); ?></span> <span class="date"><time datetime="<?php echo $this->reply->created; ?>"><?php echo JHTML::_('date', $this->reply->created, JText::_('DATE_FORMAT_HZ1')); ?></time></span>
		</a>
	</p>
...
<?php if ($this->level < 3) { // Cannot reply at third level ?>
		<a class="icon-reply reply" href="<?php echo JRoute::_('index.php?option=' . $this->option . '&task=reply&cat=wish&id=' . $this->listid . '&refid=' . $this->reply->id . '&wishid=' . $this->wishid); ?>" id="rep_<?php echo $this->reply->id; ?>"><?php echo JText::_('COM_WISHLIST_REPLY'); ?></a>
<?php } ?>
<?php if ($this->abuse) { ?>
		<a class="icon-abuse abuse" href="<?php echo JRoute::_('index.php?option=com_support&task=reportabuse&category=itemcomment&id=' . $this->reply->id . '&parent=' . $this->wishid); ?>"><?php echo JText::_('COM_WISHLIST_REPORT_ABUSE'); ?></a>
<?php } ?>
<?php if ($this->juser->get('id') == $this->reply->created_by) { ?>
