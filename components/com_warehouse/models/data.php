...
//  	  $strExperimentNameArray = explode("-",$strExperimentName);
//  	  $strExperimentNameDisplay = (sizeof($strExperimentNameArray)==2) ? $strExperimentNameArray[1] : $strDataFile['E_NAME'];
...
//  	  $strRepetitionName = (sizeof($strRepetitionNameArray)==2) ? $strRepetitionNameArray[1] : $strDataFile['R_NAME'];
  	  ...
  	  $strReturn .= <<< ENDHTML
		  <tr class="$strRow">
			<td><a href="/collaborate/groups/curation" class="Tips3" title="$strDisplay :: $strDescription">$strDisplay</a></td>
...
ENDHTML;
  ...
  public function findDistinctExperimentsHTML($p_oExperimentArray, $p_iProjectId){
  	/*
  	$strReturn = "<select id=\"cboExperiment\" name=\"experiment\" onchange=\"getDataDropDownByEntity('cboExperiment', '/warehouse/trialdropdown/project/".$p_iProjectId."/experiment', 'trials');\">
  					<option selected value=0>-Select Experiment-</option>";
  	*/				
  	...
  	foreach($p_oExperimentArray as $strReturnArray){
  	  ...
  	  $strReturn .= <<< ENDHTML
		  <option value="$iExperimentId">$strExperimentName</option>			
ENDHTML;
  	}
  	$strReturn .= "</select>";
  	return $strReturn;
  ...
  public function findDistinctTrialsHTML($p_oTrialArray, $p_iProjectId, $p_iExperimentId){
//  	$strReturn = "<select id=\"cboTrial\" name=\"trial\" onchange=\"getDataDropDownByEntity('cboTrial', '/warehouse/repetitiondropdown/project/".$p_iProjectId."/experiment/".$p_iExperimentId."/trial', 'repetitions');\">
//  					<option selected value=0>-Select Trial-</option>";
  	...
  	foreach($p_oTrialArray as $strReturnArray){
  	  ...
  	  $strReturn .= <<< ENDHTML
		  <option value="$iTrialId">$strTrialName</option>			
ENDHTML;
  	}
  	$strReturn .= "</select>";
  	return $strReturn;
  ...
  public function findDistinctRepetitionsHTML($p_oRepetitionArray){
  	$strReturn = "<select id=\"cboRepetition\" name=\"repetition\">
  	...
  	foreach($p_oRepetitionArray as $strReturnArray){
  	  ...
  	  $strReturn .= <<< ENDHTML
		  <option value="$iRepetitionId">$strRepetitionName</option>			
ENDHTML;
  	}
  	$strReturn .= "</select>";
  	return $strReturn;
