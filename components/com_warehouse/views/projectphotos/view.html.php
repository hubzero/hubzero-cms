...
class WarehouseViewProjectPhotos extends JView {
    ...
    function display($tpl = null) {
        ...
        $oPhotoDataFileArray = $oPhotosModel->findDataFileByMimeType($iProjectId, 0, 0, 0, $iLowerLimit, $iUpperLimit);
        $_REQUEST[DataFilePeer::TABLE_NAME] = serialize($oPhotoDataFileArray);
        //echo "photo count=".sizeof($oPhotoDataFileArray)."<br>";

        $iResultsCount = $oPhotosModel->findDataFileByMimeTypeCount($iProjectId, 0);
        ...
        $strTabHtml = $oPhotosModel->getTabs("warehouse", $iProjectId, $strTabArray, $strTabViewArray, "project");
        $this->assignRef("strTabs", $strTabHtml);

        // update and get the page views
        $iEntityViews = $oPhotosModel->getEntityPageViews(1, $oProject->getId());
        $this->assignRef("iEntityActivityLogViews", $iEntityViews);

        // update and get the page views
        $iEntityDownloads = $oPhotosModel->getEntityDownloads(1, $oProject->getId());
