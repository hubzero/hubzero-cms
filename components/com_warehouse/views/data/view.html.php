...
class WarehouseViewData extends JView{
  ...
  function display($tpl = null){
	...
	//$strSubTabArray = array("Tools", "Drawings", "Videos", "Photos");
	...
	switch($strSubTab){
		...
		case "videos":
			 ...
			 */
			$strRepetitions = $this->displayTrialDropDowns($oDataModel, $strSubTab, $iProjectId, $iExperimentId, $strTool, $strUsageType);
			if($strRepetitions==""){
			  $this->displayRepetitionDropDowns($oDataModel, $strSubTab, $iProjectId, $iExperimentId, $iTrialId, $strTool, $strUsageType);
			}
		...
		default:
			...
			$iDataFileTotal = $oDataModel->findDataFileOpeningToolsCount( $strTool, $iProjectId, $iExperimentId, $iTrialId, $iRepetitionId );
			$strDataFileArray = $oDataModel->findDataFileOpeningTools( $strTool, $iLowerLimit, $iUpperLimit, $iProjectId, $iExperimentId, $iTrialId, $iRepetitionId );
	...
	$this->displayExperimentDropDowns($oDataModel, $strSubTab, $iProjectId, $strTool, $strUsageType);  //every subtab
  ...
  private function displayExperimentDropDowns($oDataModel, $strSubTab, $iProjectId, $strTool, $strUsageType){
  	$strExperimentArray = $oDataModel->findDistinctExperiments($iProjectId, $strTool, $strUsageType);
	$strExperiments = $oDataModel->findDistinctExperimentsHTML($strExperimentArray, $iProjectId);
	$this->assignRef('strExperimentDropDown', $strExperiments);
  }
  
  private function displayTrialDropDowns($oDataModel, $strSubTab, $iProjectId, $iExperimentId, $strTool, $strUsageType){
  	$strRepetitions = "";
  	
  	$strTrials = $oDataModel->findDistinctTrialsHTML(array(), $iProjectId, $iExperimentId);
	if($iExperimentId > 0){
	  $strTrialArray = $oDataModel->findDistinctTrials($iProjectId, $iExperimentId, $strTool, $strUsageType);
	  $strTrials = $oDataModel->findDistinctTrialsHTML($strTrialArray, $iProjectId, $iExperimentId);
	  if(sizeof($strTrialArray)==1){
	    ...
	    $strRepetitions = $oDataModel->findDistinctRepetitionsHTML($strRepetitionArray);
  ...
  private function displayRepetitionDropDowns($oDataModel, $strSubTab, $iProjectId, $iExperimentId, $iTrialId, $strTool, $strUsageType){
  	$strRepetitions = $oDataModel->findDistinctRepetitionsHTML(array(), $iProjectId, $iExperimentId, $iTrialId);
	if($iTrialId > 0){
	  $strRepetitionArray = $oDataModel->findDistinctRepetitions($iProjectId, $iExperimentId, $iTrialId, $strTool, $strUsageType);
	  $strRepetitions = $oDataModel->findDistinctRepetitionsHTML($strRepetitionArray);	  
	}
