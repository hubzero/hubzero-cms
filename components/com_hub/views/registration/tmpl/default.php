...
if (!defined("n")) {
	...
	if ($this->task == 'update')
	{
		$html = '<div id="content-header" class="full"><h2>'.JText::_('Update Account').'</h2></div><!-- / #content-header -->'.n;

		if (!empty($this->xregistration->_missing)) 
		{
			$html .= '<div class="help">'.n;
			$html .= $hubShortName.' requires additional registration information before your account can be used.';
	...
	{
		$html = '<div id="content-header" class="full"><h2>'.JText::_('Create New Account').'</h2></div><!-- / #content-header -->'.n;
	}
	else if ($this->task == 'edit')
	{
		$html = '<div id="content-header" class="full"><h2>'.JText::_('Change Personal Information').'</h2></div><!-- / #content-header -->'.n;
	
		if ($self) 
		{
			$html .= '<div class="help">'.n;
			$html .= t.'<h4>How do I change my password?</h4>'.n;
			$html .= t.'<p>Passwords can be changed with <a href="/password/change" title="Change password form">this form</a>.</p>'.n;
			$html .= '</div>'.n;
		}
	}
	else if ($this->task == 'proxy') 
	{
		$html = '<div id="content-header" class="full"><h2>'.JText::_('Proxy Create New Account').'</h2></div><!-- / #content-header -->'.n;
	}
	...
	if ($this->task == 'proxy') 
	{
		...
		$html .= '<a href="/whois">whois page</a>, under their confirmation email.</p>'.n;
		$html .= '</div>'.n;
	}
	...
	if ( ($this->task == 'create' || $this->task == 'proxy') && $emailusers) 
	{
		$html .= '<div class="error">'.n;
		$html .= t.'<p>The email address "' . htmlentities($this->registration['email'],ENT_COMPAT,'UTF-8') . '" is already registered. If you have lost or forgotten this ' . $hubShortName . ' login information, we can resend it to you at that email address now:</p>'.n;
		$html .= t.'<p class="submit"><input type="submit" name="resend" value="Email Existing Account Information" /></p>'.n;
		$html .= t.'<p>If you are aware you already have another account registered to this email address, and are requesting another account because you need more resources, ' . $hubShortName . ' would be happy to work with you to raise your resource limits instead:</p>'.n;
	...
	{
		// output $this->xregistration->_missing fields
	...
	//$html .= t.'<script type="text/javascript" src="components/com_myaccount/myaccount.js"></script>'.n; 
	$document =& JFactory::getDocument();
	if (is_file(JPATH_ROOT.DS.'components'.DS.'com_hub'.DS.'js'.DS.'register.js')) {
		$document->addScript('components'.DS.'com_hub'.DS.'js'.DS.'register.js');
	}
	...
	if ($this->registrationUsername != REG_HIDE || $this->registrationPassword != REG_HIDE) // login information
	{
		...
		if ($this->task == 'create' || $this->task == 'proxy')
		...
		{	
			$required = ($this->registrationUsername == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
		...
		{
			$required = ($this->registrationPassword == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
			...
			{
				$required = ($this->registrationConfirmPassword == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
	 ...
	 || $this->registrationPhone != REG_HIDE)
	{
		...
		{
			$required = ($this->registrationFullname == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
			$message = (!empty($this->xregistration->_invalid['name'])) ? '<span class="error">' . $this->xregistration->_invalid['name'] . '</span>' : '';
			$fieldclass = ($message) ? ' class="fieldWithErrors"' : '';
			
			/*$html .= t.t.t.'<label '.$fieldclass.'>Full Name: '.$required;
			$html .= t.t.t.t.'<input name="name" id="name" type="text" value="';
			$html .= htmlentities($this->registration['name'],ENT_COMPAT,'UTF-8') .'" />';
			$html .= t.t.t.t.$message;
			$html .= t.t.t.'</label>'.n;*/
		...
		if ($this->registrationEmail != REG_HIDE || $this->registrationConfirmEmail != REG_HIDE)
		{
			...
			{
				$required = ($this->registrationEmail == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
			...
			if ($this->registrationConfirmEmail != REG_HIDE) // confirm email
			{
				...
				$required = ($this->registrationConfirmEmail == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
				$fieldclass = ($message) ? ' class="fieldWithErrors"' : '';
			...
			{
				if ($this->task == 'proxy')
					$html .= t.t.registration_warning('Important! The user <strong>must</strong> confirm receipt of' .
						' confirmation e-mail in order to complete registration.');
				else if ($this->task == 'create')
					$html .= t.t.registration_warning('Important! You <strong>must</strong> confirm receipt of' .
						' confirmation e-mail in order to complete registration.');
				else
					$html .= t.t.registration_warning('Important! If you change youre E-Mail address ' .
						'you <strong>must</strong> confirm receipt of the ' .
						'confirmation e-mail in order to re-activate your account.');
			}
		}

		if ($this->registrationURL != REG_HIDE) // website
		{
			$required = ($this->registrationURL == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
		...
		{
			$required = ($this->registrationPhone == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
	...
	{
		$html .= t.'<div class="explaination">'.n;
		$html .= t.t.'<p>By providing this information 
				you are helping us target our efforts to 
				our users. We will <em>not</em> disclose your personal information to others unless required by law';
		...
		{
			$html .= t.t.'<p>We operate as a community service and
				are committed to serving a diverse population of users. This information helps us assess our 
				progress towards that goal.</p>'.n;
		}
		...
		{
			$required = ($this->registrationEmployment == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
			...
			if (empty($this->registration['orgtype']) || !empty($this->xregistration->_invalid['orgtype'])) {
				$html .= t.t.t.t.t.'<option value="" selected="selected">'.JText::_('(select from list)').'</option>'.n;
			}
		...
		{
			$required = ($this->registrationOrganization == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
		...
		{
			$required = ($this->registrationReason == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
			...
			if (!in_array($this->registration['reason'], $reasons)) {
				...
				$html .= t.t.t.t.'<option value="" selected="selected">'.JText::_('(select from list or enter below)').'</option>'.n;
			}
			...
			$html .= t.t.t.'</select>'.n;
			//$html .= t.t.t.'<textarea name="reason" id="reason" rows="4" cols="32">'.htmlentities($this->registration['reason'],ENT_COMPAT,'UTF-8').'</textarea>'.n;
			$html .= t.t.'</label>'.n;
		...
		{
			$required = ($this->registrationInterests == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
		...
		$this->registrationRace != REG_HIDE) 
	{
		...
		{
			$required = ($this->registrationCitizenship == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
			...
			if (!$this->registration['countryorigin'] || $this->registration['countryorigin'] == 'US')
				$html .= t.t.t.t.t.'<option value="">(select from list)</option>'.n;
		...
		{
			$required = ($this->registrationResidency == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
		...
		{
			$required = ($this->registrationSex == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
			$message = (!empty($this->xregistration->_invalid['sex'])) ? registration_error($this->xregistration->_invalid['sex']) : '';
			$fieldclass = ($message) ? ' class="fieldsWithErrors"' : '';

			$html .= t.t.'<fieldset'.$fieldclass.'>'.n;
			$html .= t.t.t.'<legend>'.JText::_('Sex').': '.$required.'</legend>'.n;
			$html .= ($message) ? t.t.t.$message.n : '';
			$html .= t.t.t.'<input type="hidden" name="sex" value="unspecified" />'.n;
			$html .= t.t.t.'<label>'.registration_radio('sex','male','option',$this->registration['sex']).' '.JText::_('Male').'</label>'.n;
			$html .= t.t.t.'<label>'.registration_radio('sex','female','option',$this->registration['sex']).' '.JText::_('Female').'</label>'.n;
			$html .= t.t.t.'<label>'.registration_radio('sex','refused','option',$this->registration['sex']).' '.JText::_('Do not wish to reveal').'</label>'.n;
			$html .= t.t.'</fieldset>'.n;
		}

		if ($this->registrationDisability != REG_HIDE) // disability
		{
			$required = ($this->registrationDisability == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
	...
	//{
		...
		{
			$required = ($this->registrationHispanic == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
		...
		{
			$required = ($this->registrationRace == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
	...
	if ($this->registrationOptIn != REG_HIDE)
	{
		...
		{
			$required = ($this->registrationOptIn == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
	...
	if ($this->registrationTOU != REG_HIDE)
	{
		...
		if ($this->registrationTOU != REG_HIDE)
		{
			$required = ($this->registrationTOU == REG_REQUIRED) ? '<span class="required">'.JText::_('required').'</span>' : '';
	...
	{
		case 'create':
			$html .= t.'<p class="submit"><input type="submit" name="create" value="'.JText::_('Create Account').'" /></p>'.n;
			$html .= t.'<input type="hidden" name="option" value="com_hub" />'.n;
			$html .= t.'<input type="hidden" name="view" value="registration" />'.n;
			$html .= t.'<input type="hidden" name="task" value="create" />'.n;
			$html .= t.'<input type="hidden" name="act" value="submit" />'.n;
		break;
		
		case 'proxy':
			$html .= t.'<p class="submit"><input type="submit" name="proxy" value="'.JText::_('Proxy Create Account').'" /></p>'.n;
			$html .= t.'<input type="hidden" name="option" value="com_hub" />'.n;
			$html .= t.'<input type="hidden" name="view" value="registration" />'.n;
			$html .= t.'<input type="hidden" name="task" value="proxycreate" />'.n;
			$html .= t.'<input type="hidden" name="act" value="submit" />'.n;
		break;
		
		case 'update':
			$html .= t.'<p class="submit"><input type="submit" name="update" value="'.JText::_('Update Account').'" /></p>'.n;
			$html .= t.'<input type="hidden" name="option" value="com_hub" />'.n;
			$html .= t.'<input type="hidden" name="view" value="registration" />'.n;
			$html .= t.'<input type="hidden" name="task" value="update" />'.n;
			$html .= t.'<input type="hidden" name="act" value="submit" />'.n;
		break;
		
		case 'edit':
			$html .= t.'<p class="submit"><input type="submit" name="edit" value="'.JText::_('Save').'" /></p>'.n;
			$html .= t.'<input type="hidden" name="option" value="com_hub" />'.n;
			$html .= t.'<input type="hidden" name="view" value="registration" />'.n;
			$html .= t.'<input type="hidden" name="task" value="edit" />'.n;
			$html .= t.'<input type="hidden" name="act" value="submit" />'.n;
		break;
	}
