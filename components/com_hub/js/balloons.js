...
NOTE:
	Some functionality removed to save on file size
*/
...
HUB.Balloons = {
	initialize: function() {
		HUB.Balloons.popboxes();
		HUB.Balloons.tooltips();
	},
	
	popboxes: function() {
		var cpLinks = document.getElements('.popbox');
		var relLinks = [];

		cpLinks.each(function(cpLink) {
			if ((!document.all) || (document.all && cpLink.parentNode.nodeName == 'LI')) {
				var testRel;
				if (testRel = cpLink.getAttribute('rel'))
					if (testRel.indexOf('contents')==0)
						relLinks.push(cpLink);
			}
		});

		relLinks.each(function(aLink) {
			var parSpan = document.createElement('DIV');
			parSpan.className = 'relProd';
			aLink.parentNode.insertBefore(parSpan,aLink);
			parSpan.appendChild(aLink.parentNode.removeChild(aLink));
		
			parSpan.onmouseover = function() {
				HUB.Balloons.showPopbox(this);
			}
			parSpan.onmouseout = function() {
				this.removeClass('show');
			}
		});
	},

	showPopbox: function(parSpan) {
		if (! parSpan.hasClass('populated')) {
			var prodLink = parSpan.getElementsByClassName('popbox')[0];
			var idRE     = /[0-9]+/;
			var prodID   = idRE.exec(prodLink.getAttribute('rel'));
			var contents = $('r'+prodID);
			if(contents) {
				var botDiv = document.createElement('DIV');
				botDiv.className = 'bottom';
				contents.appendChild(botDiv);
				contents.style.top = '-' + (contents.offsetHeight - 15) + 'px';
				parSpan.appendChild(contents);
			}
			parSpan.addClass('populated');
		}
		parSpan.addClass('show');
	},
