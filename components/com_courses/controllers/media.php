...
class CoursesControllerMedia extends Hubzero_Controller
{
	 ...
	 * @return     void
	 */
	public function assetuploadTask()
	{
		...
		{
			echo json_encode(array('error' => "No files given"));
			exit();
		}
		...
		{
			echo json_encode(array('error' => "Asset save failed"));
			exit();
		}

		// Create asset assoc object
		$assocObj = new CoursesTableAssetAssociation($this->database);
		...
		{
			echo json_encode(array('error' => "Asset association save failed"));
			exit();
		}
		...
		// Exit success
		echo json_encode(array('success'=>true, 'files'=>array($files)));
		exit();
	}
	...
	public function togglepublishedTask()
	{
		...
		if (!$assetObj->load($id))
		{
			echo json_encode(array('error' => "Loading asset $id failed"));
			exit();
		}
		...
		{
			echo json_encode(array('error' => "Saving asset $id state failed"));
			exit();
		}
