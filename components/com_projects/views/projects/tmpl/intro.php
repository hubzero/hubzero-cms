		...
		if (count($this->rows) > 0) { 	?>
			<ul class="flow">
				...
				<li <?php if ($setup) { echo 'class="s-dev"'; } else if ($row->state == 0) { echo 'class="s-inactive"'; } else if ($row->state == 5) { echo 'class="s-pending"'; } ?>>
					...
					<a href="<?php echo JRoute::_('index.php?option=' . $this->option . '&task=view&alias=' . $row->alias); ?>"><img src="<?php echo JRoute::_('index.php?option=' . $this->option . '&alias=' . $row->alias . '&task=media'); ?>" alt="" /><span class="block"><?php echo \Hubzero\Utility\String::truncate($this->escape($row->title), 30); ?></span></a><?php if ($setup) { ?><span class="s-complete"><?php echo JText::_('COM_PROJECTS_COMPLETE_SETUP'); ?></span><?php } else if ($row->state == 0) { ?><span class="s-suspended"><?php echo JText::_('COM_PROJECTS_STATUS_INACTIVE'); ?></span> <?php } else if ($row->state == 5) { ?><span class="s-suspended"><?php echo JText::_('COM_PROJECTS_STATUS_PENDING'); ?></span> <?php } ?>
