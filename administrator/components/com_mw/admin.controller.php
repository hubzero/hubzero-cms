...
{	
	...
	public function execute()
	{
		...
		/*
			case 'licenses': $this->licenses(); break;
			case 'addlicense': $this->addlicense(); break;
			case 'editlicense': $this->editlicense(); break;
			case 'savelicense': $this->savelicense(); break;
			case 'removelicense': $this->removelicense(); break; 
			case 'licenseassoc': $this->licenseassoc(); break;
			case 'addlicenseassoc': $this->addlicenseassoc(); break;
			case 'editlicenseassoc': $this->editlicenseassoc(); break;
			case 'savelicenseassoc': $this->savelicenseassoc(); break;
			case 'removelicenseassoc': $this->removelicenseassoc(); break;
		*/
	...
	protected function browse( ) 
	{
		...
		switch ($table) 
		{
			...
			default:
				...
				$html .= ' <li>'.MwHtml::admlink('app',$vars,'Modify app table', $this->_option).'</li>'.n;
			
				$vars['table'] = 'host';
				$html .= ' <li>'.MwHtml::admlink('host',$vars,'Modify host table', $this->_option).'</li>'.n;
			
				$vars['table'] = 'hosttype';
				$html .= ' <li>'.MwHtml::admlink('hosttype',$vars,'Modify hosttype table', $this->_option).'</li>'.n;
			
				//$vars['table'] = 'usertype';
				//$html .= ' <li>'.MwHtml::admlink('usertype',$vars,'Modify usertype table', $this->_option).'</li>'.n;
		...
		$html .= '<p>';
		$html .= MwHtml::admlink('Back to administrative menu',array(),'Back to administrative menu', $this->_option);
		$html .= '</p>'.n;
	...
	private function table( $rows, $headers, $middle, $trailer, &$tail_row ) 
	{
		...
		$html .= MwHtml::tableHeader($headers);
		$html .= ' <tbody>'.n; 
	...
	{
		$html  = '  <tr>'.n;
		$html .= MwHtml::td( $row->sessnum );
		$html .= MwHtml::td( $row->username );
		$html .= MwHtml::td( $row->appname );
		$html .= MwHtml::td( $row->start );
		$html .= MwHtml::td( $row->walltime );
		$html .= MwHtml::td( $row->viewtime );
		$html .= MwHtml::td( $row->cputime );
		$html .= MwHtml::td( $row->status );
		$html .= '  </tr>'.n;
		return $html;
	}
	...
	private function host_body($row) 
	{
		...
		$html  = '  <tr>'.n;
		$html .= MwHtml::td( MwHtml::admlink($hostname,$vars,'Edit '.$hostname, $this->_option) );
		...
		$html .= MwHtml::td( MwHtml::listedit($list, $hidden) );
		...
		$html .= MwHtml::td( MwHtml::admlink($status,$vars,'Change host status', $this->_option) );
		$html .= MwHtml::td( $uses );

		// A button to delete the whole thing.
		$html .= MwHtml::td( MwHtml::delete_button('hostname', 'host', $hostname, $this->_option) );
		$html .= '  </tr>'.n;
	...
	private function host_edit($row) 
	{
		...
		$html .= t.MwHtml::hInput('option',$this->_option);
		$html .= t.MwHtml::hInput('admin',1);
		$html .= t.MwHtml::hInput('table','host');
		$html .= t.MwHtml::hInput('op','update');
		$html .= t.MwHtml::hInput('filter_hostname',$row->hostname);
		$html .= t.MwHtml::hInput('status','check');
		...
		$html .= MwHtml::td( $row->status );
		$html .= MwHtml::td( 'uses' );
		$html .= MwHtml::td( MwHtml::sInput('insert','Update') );
		$html .= t.'<form>'.n;
		$html .= '  </tr>'.n;
		return $html;
	}
	...
	private function host_display()
	{
		...
		switch($op)
		{
			...
			case 'update':
				if ($hostname == '') {
					echo MwHtml::error('You must specify a valid hostname.');
				} else {
	...
	private function app_edit(&$row) 
	{
		...
		$html .= t.MwHtml::hInput('option',$this->_option);
		$html .= t.MwHtml::hInput('admin',1);
		$html .= t.MwHtml::hInput('table','app');
		$html .= t.MwHtml::hInput('op','update');
		$html .= t.MwHtml::hInput('filter_appname',$row->appname);
	...
	private function app_body(&$row) 
	{
		...
		$html  = MwHtml::td( MwHtml::admlink($appname,$vars,"Edit $appname", $this->_option) );
		$html .= MwHtml::td( MwHtml::admlink($geometry,$vars,"Edit $appname", $this->_option) );
		$html .= MwHtml::td( MwHtml::admlink($depth,$vars,"Edit $appname", $this->_option) );
		...
		$html .= MwHtml::td( MwHtml::listedit($list, $hidden) );
		$html .= MwHtml::td( MwHtml::admlink($timeout,$vars,"Edit $appname", $this->_option) );
		$html .= MwHtml::td( MwHtml::admlink($command,$vars,"Edit $appname", $this->_option) );
		$html .= MwHtml::td( MwHtml::admlink($description,$vars,"Edit $appname", $this->_option) );

		// And a button to delete the whole thing.
		$html .= MwHtml::td( MwHtml::delete_button('appname', 'app', $appname, $this->_option) );
		$html .= '  </tr>'."\n";
	...
	private function app_display() 
	{
		...
		switch($op) 
		{
			...
			case 'update':
				if ($appname == '') {
					echo MwHtml::error('You must specify an appname.');
				} else {
					$harr = array();
	...
	private function type_display() 
	{
		...
		switch($op) 
		{
			...
			case 'update':
				if ($name == '') {
					echo MwHtml::error('You must specify a valid name.');
				} else {
					if (($table == 'hosttype' && $filter_hosttype != '') ||
						...
						for($i=0; $i<count($rows); $i++) 
						{
							$row = $rows[$i];
							if ($row->name == $name) {
								echo MwHtml::error('"'.$name.'" already exists in the table.');
								$name = '';
							}
						}
						if ($name != '') {
	...
	private function hosttype_edit($row) 
	{
		...
		return MwHtml::updateform('hosttype', $bit, $refs, &$row, $this->_option);
	}
	...
	private function hosttype_body( $row ) 
	{
		...
		$html  = '  <tr>'."\n";
		$html .= MwHtml::td( MwHtml::admlink($row->name,$vars,"Edit $row->name", $this->_option) );
		$html .= MwHtml::td( $bit );
		$html .= MwHtml::td( MwHtml::admlink($row->description,$vars,"Edit $row->name", $this->_option) );
		$html .= MwHtml::td( $refs );
		$html .= '   <td>'.n;
		$html .= t.'<form name="'.$row->name.' users" method="get" action="index.php">'.n;
		$html .= t.MwHtml::hInput('option',$this->_option);
		$html .= t.MwHtml::hInput('admin',1);
		$html .= t.MwHtml::hInput('table','hosttype');
		$html .= t.MwHtml::hInput('item',$row->name);
		if ($refs > 0) {
			$html .= t.MwHtml::hInput('op','users');
			$html .= t.MwHtml::hInput('filter_hosttype',$row->name);
			$html .= t.MwHtml::sInput('users','Show Uses');
		} else {
			$html .= t.MwHtml::hInput('op','delete');
			$html .= t.MwHtml::sInput('delete','Delete');
		}
		$html .= t.'</form>'.n;
		$html .= '   </td>'.n;
		$html .= '  </tr>'.n;
	...
	private function usertype_edit( $row ) 
	{
		...
		return MwHtml::updateform('usertype', $bit, $refs, &$row, $this->_option);
	}
	...
	private function usertype_body( $row ) 
	{
		...
		$html .= t.MwHtml::hInput('option',$this->_option);
		$html .= t.MwHtml::hInput('admin',1);
		$html .= t.MwHtml::hInput('table','usertype');
		$html .= t.MwHtml::hInput('item',$row->name);
		if ($refs > 0) {
			$html .= t.MwHtml::hInput('op','users');
			$html .= t.MwHtml::hInput('filter_usertype',$row->name);
			$html .= t.MwHtml::sInput('users','Show Uses');
		} else {
			$html .= t.MwHtml::hInput('op','delete');
			$html .= t.MwHtml::sInput('delete','Delete');
	...
	protected function licenses()
	{
		...
		// Output HTML
		MwHtml::licenses( $rows, $pageNav, $this->_option, $filters );
	}
	...
	protected function editlicense($license=null) 
	{
		$database =& JFactory::getDBO();
		...
		if (!is_object($license)) {
			...
			$license = new License( $database );
			$license->load( $id );
		}
		
		// Ouput HTML
		MwHtml::editlicense( $license, $this->_option );
	}
	...
	protected function savelicense() 
	{
		...
		$row = new License( $database );
		if (!$row->bind( $_POST )) {
			$this->setError( $row->getError() );
			$this->editlicense($row);
			return;
		}
		...
		// Redirect
		$this->_redirect = 'index.php?option='.$this->_option.'&task=licenses';
		$this->_message = JText::_( 'LICENSE_SAVED' );
	}
	...
	protected function removelicense() 
	{
		...
		foreach ($ids as $id) 
		{
			// Remove references to the license
			if (!$lu->deleteAssociation( $id )) {
				$this->setError( $lu->getError() );
			}
			if (!$lt->deleteAssociation( $id )) {
				$this->setError( $lt->getError() );
			}
		...
		// Redirect
		$this->_redirect = 'index.php?option='.$this->_option.'&task=licenses';
		$this->_message = JText::_( 'LICENSE_REMOVED' );
	}
	...
	protected function cancellicense()
	{
		$this->_redirect = 'index.php?option='.$this->_option.'&task=licenses';
	}
	...
	protected function licenseassoc()
	{
		$app =& JFactory::getApplication();
		$database =& JFactory::getDBO();
		...
		// Get filters
		$filters = array();
		$filters['lid'] = JRequest::getInt( 'lid', 0 );
		$filters['tbl'] = JRequest::getVar( 'tbl', '' );
		$filters['search'] = urldecode($app->getUserStateFromRequest($this->_option.'.search', 'search', ''));
		$filters['sortby'] = $app->getUserStateFromRequest($this->_option.'.sortby', 'sortby', '');
		
		// Make sure we have a necessary var
		if (!$filters['tbl'] || !$filters['lid']) {
			$this->_redirect = 'index.php?option='.$this->_option.'&task=licenses';
			return;
		}
		...
		if ($filters['tbl'] == 'user') {
			$obj = new LicenseUser( $database );
		} else if ($filters['tbl'] == 'tool') {
			$obj = new LicenseTool( $database );
		}
		...
		// Output HTML
		MwHtml::licenseassoc( $rows, $pageNav, $this->_option, $filters, $database );
	}
	
	//-----------
	
	protected function addlicenseassoc() 
	{
		$this->editlicenseassoc();
	}
	
	//-----------

	protected function editlicenseassoc() 
	{
		$database =& JFactory::getDBO();
		...
		$filters = array();
		$filters['limit'] = 'all';
		$licenses = $obj->getRecords( $filters );
		
		// Ouput HTML
		MwHtml::editlicenseassoc( $licenses, $this->_option, $tbl, $lid, $id );
	}
	
	//-----------
	
	protected function savelicenseassoc() 
	{
		$database =& JFactory::getDBO();
		...
		// Load the tag object and bind the incoming data to it
		if ($tbl == 'user') {
			$row = new LicenseUser( $database );
		} else if ($tbl == 'tool') {
			$row = new LicenseTool( $database );
		}
		
		if (!$row->bind( $_POST )) {
			$this->setError( $row->getError() );
			$this->editlicenseassoc($row);
			return;
		}
		...
		// Redirect
		$this->_redirect = 'index.php?option='.$this->_option.'&task=licenseassoc&lid='.$lid.'&tbl='.$tbl;
		$this->_message = JText::_( 'LICENSE_ASSOCIATION_SAVED' );
	}
	
	//-----------

	protected function removelicenseassoc() 
	{
		$database =& JFactory::getDBO();
		...
		// Load some needed objects
		if ($tbl == 'user') {
			$l = new LicenseUser( $database );
		} else if ($tbl == 'tool') {
			$l = new LicenseTool( $database );
		}
		...
		foreach ($ids as $id) 
		{
			// Remove references to the license
			if (!$l->delete( $lid, $id )) {
				$this->setError( $l->getError() );
			}
		}

		// Redirect
		$this->_redirect = 'index.php?option='.$this->_option.'&task=licenseassoc&lid='.$lid.'&tbl='.$tbl;
		$this->_message = JText::_( 'LICENSE_ASSOCIATION_REMOVED' );
	}
}
?>
