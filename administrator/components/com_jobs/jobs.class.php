...
class JobSeeker extends JTable
{
	...
	function countSeekers( $filters, $uid=0)
	{
		$filters['limit'] = 0;
		$filters['start'] = 0;
		
		$seekers = $this->getSeekers( $filters, $uid);
		
		// make sure we don't incude duplicates
		$array=array();
	...
	function getSeekers( $filters, $uid=0, $excludeme = 0, $admin = 0)
	{
		...
		//}
		//else {
		//$query.= "\n , (SELECT count(*) FROM #__jobs_shortlist AS W WHERE W.seeker=s.uid AND W.emp=".$admin." AND s.uid != '".$uid."' AND s.uid=r.uid AND W.category='resume') AS shortlisted ";
		//}
		...
		if($filters['search']) {
			...
			if(count($s) > 0 ) { 
				...
				{
					$tquery .= $i==0 ? "AND (" : " OR ";
					$tquery .= "( LOWER(s.tagline) LIKE '%$s[$i]%')";	
					$tquery .= $i==($n-1) ? ") " : "";
				}
			}
			else {
		...
		$query.= "\n , (SELECT count(*) FROM #__jobs_seekers AS s WHERE s.uid=r.uid ".$tquery.") AS keywords ";
		*/
		...
		$query.= "\n , (SELECT count(*) FROM #__jobs_seekers AS s WHERE s.uid=r.uid ".$catquery.") AS type ";
