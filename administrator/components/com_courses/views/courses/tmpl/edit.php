...
$gparams = new $paramsClass($this->row->params);
			...
			<table class="admintable">
				<tbody>
					<tr>
						...
						<td><input type="text" name="fields[alias]" id="field-alias" value="<?php echo $this->escape(stripslashes($this->row->alias)); ?>" size="50" /></td>
					</tr>
					<tr>
						<td class="key"><label for="field-title"><?php echo JText::_('COM_COURSES_TITLE'); ?>:</label></td>
						<td><input type="text" name="fields[title]" id="field-title" value="<?php echo $this->escape(stripslashes($this->row->title)); ?>" size="50" /></td>
					</tr>
		 			<tr>
						...
						<td>
							...
							<?php echo $editor->display('fields[description]', $this->escape(stripslashes($this->row->description)), '', '', '40', '15'); ?>
						</td>
					</tr>
				</tbody>
			</table>
...
<?php if ($this->row->id) { ?>
			<iframe width="100%" height="400" name="managers" id="managers" frameborder="0" src="index.php?option=<?php echo $this->option; ?>&amp;controller=managers&amp;tmpl=component&amp;id=<?php echo $this->row->id; ?>"></iframe>
<?php } else { ?>
		...
		<table class="meta" summary="<?php echo JText::_('COM_COURSES_META_SUMMARY'); ?>">
			<tbody>
				<tr>
					...
					<td><?php echo $this->escape($this->row->group_id); ?></td>
				</tr>
				<tr>
...
<?php if ($this->row->created) { ?>
				<tr>
					...
					<td><?php echo $this->escape($this->row->created); ?></td>
				</tr>
<?php } ?>
					...
					<tr>
						...
						<td>
							<select name="fields[state]" id="field-state">
								<option value="0"<?php if ($this->row->state == 0) { echo ' selected="selected"'; } ?>><?php echo JText::_('Unpublished'); ?></option>
								<option value="1"<?php if ($this->row->state == 1) { echo ' selected="selected"'; } ?>><?php echo JText::_('Published'); ?></option>
								<option value="2"<?php if ($this->row->state == 2) { echo ' selected="selected"'; } ?>><?php echo JText::_('Deleted'); ?></option>
							</select>
						</td>
					</tr>
				</tbody>
