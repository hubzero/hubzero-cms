...
class PubInstall extends JObject {
	...
	public function installPublishing( ) 
	{
		...
		if (!in_array($prefix . 'publication_audience_levels', $this->tables)) 
		{
			...
			$queries[] = "INSERT INTO `jos_publication_audience_levels` (`id`,`label`,`title`,`description`) 
		...
		if ($iniSetup == 1)
		{
			...
			$queries[] = "INSERT INTO `jos_plugins`(`name`, `element`, `folder`, `access`, `ordering`, `published`, `iscore`, `client_id`, `checked_out`, `checked_out_time`, `params`) SELECT 'Publications - Share', 'share', 'publications', 0, 9, 1, 0, 0, 0, NULL, 'icons_limit=3\nshare_facebook=1\nshare_twitter=1\nshare_google=1\nshare_digg=1\nshare_technorati=1\nshare_delicious=0\nshare_reddit=1\nshare_email=0\nshare_print=0\n\n' FROM DUAL WHERE NOT EXISTS (SELECT `name` FROM `jos_plugins` WHERE name = 'Publications - Share')";
