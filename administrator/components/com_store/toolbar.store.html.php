...
class StoreToolbar
{
	function _ITEM($store_enabled ) 
	{
		$text = (!$store_enabled) ? ' <small><small style="color:red;">(store is disabled)</small></small>' : '';
	...
	function _ITEMS($store_enabled) 
	{
		$text = (!$store_enabled) ? ' <small><small style="color:red;">(store is disabled)</small></small>' : '';
		JToolBarHelper::title( JText::_( 'Store Manager' ).$text, 'addedit.png' );
		JToolBarHelper::addNew('newitem','New Store Item');
	...
	function _DEFAULT($store_enabled) 
	{	
