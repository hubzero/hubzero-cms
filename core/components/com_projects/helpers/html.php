...
class Html extends Obj
{
    ...
    public static function replaceUrls($string, $rel = 'nofollow')
    {
        ...
        // Replace url links
        $urlPattern = '#\b((?<!href=")(https?://www[.]|[\w-]+://?|(?<!://)www[.])'
            . '[^\s()<>]+(?:\([\w\d]+\)|([^[:punct:]\s]|/)))#';
        $string = preg_replace($urlPattern, "<a rel=\"{$rel}\" href=\"\$1\">\$1</a>", $string);
    ...
    public static function getSuggestions($model)
    {
        ...
        if (
            $model->access('owner') && $model->config()->get('grantinfo')
     ...
     * @return  string
     */
    public static function buildFileBrowserCrumbs(
        $dir = '',
        $url = '',
        &$parent = null,
        $linkit = true,
