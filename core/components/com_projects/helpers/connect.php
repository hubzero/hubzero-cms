...
class Connect extends Obj
{
     ...
     * @var array
     */
    private $services = array('google', 'dropbox');
     ...
     */
    public const GOOGLE_OAUTH2_REVOKE_URI = 'https://accounts.google.com/o/oauth2/revoke';

    /**
     * URL to refresh Google token
     */
    public const GOOGLE_OAUTH2_TOKEN_URI = 'https://accounts.google.com/o/oauth2/token';

    /**
     * Google auth URL
     */
    public const GOOGLE_OAUTH2_AUTH_URL = 'https://accounts.google.com/o/oauth2/auth';
    ...
    {
        if (!$param || !isset($this->connect[$service][$param])) {
            return null;
        }
        return $this->connect[$service][$param];
    }
    ...
    {
        if (!isset($this->connect[$service])) {
            $this->setError(Lang::txt('PLG_PROJECTS_FILES_SYNC_INVALID_SERVICE'));
            return false;
        }

        foreach ($this->connect[$service] as $key => $value) {
            if (!$value && $key != 'active') {
                // Return only active service?
                if ($active == true) {
                    return false;
                } else {
                    $this->connect[$service]['on'] = 0;
                }
            }
        }
        ...
        if ($service == 'google') {
            ...
            if ($code) {
                ...
                if (isset($token->refresh_token)) {
                    ...
                    if ($this->uid == $this->model->get('owned_by_user')) {
    ...
    public function getAPI($service = 'google', $uid = 0)
    {
        ...
        if (isset($this->_api[$service]) && $this->_api[$service] && $uid == $this->uid) {
    ...
    public function afterConnect($service = 'google', $uid = 0)
    {
        ...
        $exclude = array($this->model->get('owned_by_user'));
        $connected = $objO->getConnected($this->model->get('id'), $service, $exclude);
    ...
    public function getRemoteDirectory($service = 'google', $uid = 0, $shared = array())
    {
        ...
        if ($service == 'google') {
            ...
            if ($folderID == 1  && $owner) {
                ...
                if ($folderID) {
                    $obj = new \Components\Projects\Tables\Project($this->db);
    ...
    public function addRemoteFile(
        ...
        $objRFile = new \Components\Projects\Tables\RemoteFile($this->db);
        ...
        if ($service == 'google') {
            ...
            try {
                ...
                if ($check['labels']['trashed']) {
                    ...
                    $check
                    && isset($check['md5Checksum'])
                    && isset($local['md5Checksum'])
                    && $check['md5Checksum'] == $local['md5Checksum']
                ) {
                    $success = $remoteid;
                }
            } catch (Exception $e) {
     ...
     * @return     array
     */
    public function deleteRemoteItem(
        $projectid = null,
        $service = 'google',
        $uid = 0,
        $remoteid = 0,
     ...
     * @return  integer
     */
    public function prepRemoteParent(
        $projectid = null,
        $service = 'google',
        $uid = 0,
        $local = array(),
    ...
    public function getChangedItems(
        ...
        if ($service == 'google') {
            ...
            $changeID = Google::collectChanges(
                $apiService,
                $folderID,
                $remotes,
                $deletes,
    ...
    {
        if (!$service || !in_array($service, $this->services)) {
            $this->setError(Lang::txt('PLG_PROJECTS_FILES_SYNC_INVALID_SERVICE'));
            return false;
        }
    ...
    public function disconnect($service, $removeData = false)
    {
        ...
        // Project creator?
        $creator = $this->uid == $this->model->get('owned_by_user')  ? 1 : 0;

        // Clean up stored values for the disconnecting member
        $this->disconnectMember($service, $this->uid, $remoteid);
        ...
        if ($creator) {
            if ($removeData == true && $remoteid) {
                ...
                $this->deleteRemoteItem($this->model->get('id'), $service, $this->uid, $remoteid, $permanent = true);
