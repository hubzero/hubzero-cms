...
class Note extends \Components\Wiki\Models\Book
{
    ...
    public function page($id = null, $scope = '')
    {
        ...
        $this->_cache['page'] = \Components\Wiki\Models\Page::oneByPath($id, 'project', $this->projectId);
    ...
    public function getPublicStamp($id = 0, $register = false, $listed = null)
    {
        ...
        // Check valid stamp
        $objSt->checkStamp($this->projectId, json_encode($reference), 'notes');
        $list = ($listed !== null && $listed != $objSt->listed) ? true : false;

        if ($list == true) {
            return $objSt->registerStamp($this->projectId, json_encode($reference), 'notes', $listed);
        }

        // Register new stamp?
        if (!$objSt->id && $register == true) {
            $objSt->registerStamp($this->projectId, json_encode($reference), 'notes', $listed);
            $objSt->checkStamp($this->projectId, json_encode($reference), 'notes');
        }
    ...
    public function getFirstNote($prefix = '')
    {
        $query  = "SELECT p.pagename FROM `#__wiki_pages` AS p
				  WHERE p.scope='project' AND p.scope_id=" . $this->_db->quote($this->projectId) . " AND p.state!=2";
    ...
    public function getParentNotes($scope = '')
    {
        ...
        if ($remaining) {
            ...
            $query .= "WHERE p.scope='project' AND p.scope_id="
                . $this->_db->quote($this->projectId) . " AND p.state!=2 ";
            $k = 1;
            $where = '';
    ...
    public function getNotes($limit = 0, $orderby = 'p.path ASC, p.title ASC, p.times_rated ASC, p.id')
    {
        $query = "SELECT DISTINCT p.id, p.path, p.pagename, p.title, p.scope, p.times_rated
				  ...
				  WHERE p.scope='project' AND p.scope_id=" . $this->_db->quote($this->projectId) . "
    ...
    {
        $query = "UPDATE `#__wiki_pages` AS p SET p.path=replace(p.path, '/"
