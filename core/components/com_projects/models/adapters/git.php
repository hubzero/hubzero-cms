...
class Git extends Models\Adapter
{
    ...
    public function __construct($path = null, $remote = false)
    {
        ...
        // Git helper
        $this->_git = new Helpers\Git($this->path);
    ...
    {
        $cmd  = 'cd ' . escapeshellarg($this->path) . ' && ';
    ...
    public function filelist($params = [])
    {
        ...
        foreach ($files as $item) {
            ...
            $file = new Models\File($item, $this->path);
            ...
            if (isset($remotes[$file->get('localPath')])) {
                ...
                // Added this conditional so that the local repo date takes priority.
                // Otherwise the date will always show the day Google Drive was
                // connected which is not helpful.
                if (!$file->get('date')) {
    ...
    public function diff($params = array())
    {
        ...
        if ($mode == 'side-by-side') {
            $sideBySidePath = PATH_CORE . DS . 'plugins' . DS . 'projects' . DS . 'files'
                . DS . 'php-diff' . DS . 'Diff' . DS . 'Renderer' . DS . 'Html'
                . DS . 'hubSideBySide.php';
            include_once $sideBySidePath;

            // Generate a side by side diff
            $renderer = new \Diff_Renderer_Html_SideBySide();
            $diff = $objDiff->Render($renderer);
        } elseif ($mode == 'inline') {
            $inlinePath = PATH_CORE . DS . 'plugins' . DS . 'projects' . DS . 'files'
                . DS . 'php-diff' . DS . 'Diff' . DS . 'Renderer' . DS . 'Html'
                . DS . 'hubInline.php';
            include_once $inlinePath;
    ...
    public function getDiskUsage($params = array())
    {
        ...
        $path    = isset($params['path']) ? $params['path'] : $this->path;
    ...
    {
        if (!$this->path || !is_dir($this->path) || !is_dir($this->path . DS . '.git')) {
            return false;
        }
