...
class File extends Obj
{
    ...
    public function getPreview($model, $hash = '', $get = 'name', $render = '', $hashed = null)
    {
        ...
        if (!$hashed) {
            ...
            switch ($render) {
                ...
                default:
                    $hashed = $hash
                        ? Helpers\Html::createThumbName($this->get('name'), '-' . $hash . '-thumb', 'png')
                        : null;
                    $maxWidth  = 180;
                    $maxHeight = 180;
                    break;
        ...
        if ($get == 'localPath') {
            ...
            return Route::url(
                'index.php?option=com_projects&alias=' . $model->get('alias')
                . '&controller=media&media=' . urlencode(basename($image))
            );
        }
    ...
    public static function drawIcon($ext = '')
    {
        ...
        $class = 'file-type' . ($ext ? ' file-type-' . $ext : '');
        return '<img class="' . $class . '" src="' . $icon . '" alt="' . $ext . '" />';
    }
    ...
    public static function getIconImage($ext, $basename = false, $icon = '')
    {
        ...
        $icon = \Hubzero\Facades\Html::asset('image', 'assets/filetypes/' . $ext . '.svg', '', null, true, true);
        if (!$icon) {
            $icon = \Hubzero\Facades\Html::asset('image', 'assets/filetypes/file.svg', '', null, true, true);
        }
