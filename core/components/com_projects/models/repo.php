...
class Repo extends Obj
{
    ...
    private function initRepo()
    {
        ...
        if ($this->get('name') !== 'local') {
            ...
            if (!$this->tbl) {
                $this->tbl = new Tables\Repo($this->db);
            }
            $this->tbl->loadRepo($this->get('project')->get('id'), $this->get('name'));
        } else {
    ...
    public function call($call = null, $params = array())
    {
        ...
        // Perform request
        try {
            $result = $this->adapter->$call($params);
            if ($this->adapter->getError()) {
                $this->setError($this->adapter->getError());
            }
            return $result;
        } catch (Exception $e) {
            $this->setError($e);
    ...
    public function makeDirectory($params = array())
    {
        ...
        if ($this->dirExists($localDirPath)) {
            $this->setError(
                Lang::txt('COM_PROJECTS_FILES_ERROR_DIR_CREATE') . ' "' . $newDir . '". '
    ...
    public function versions($params = array(), &$versions = array(), &$timestamps = array())
    {
        ...
        $this->adapter->history($params, $versions, $timestamps);
    ...
    public function diff($params = array())
    {
        ...
        return $this->adapter->diff($params);
    }
    ...
    public function getLastRevision($params = array())
    {
        ...
        return $this->adapter->getLastRevision($params);
    }
    ...
    protected function _getVersionStatus($versions = array())
    {
        ...
        for ($k = (count($versions) - 1); $k >= 0; $k--) {
            ...
            $pattern = "/" . Lang::txt('PLG_PROJECTS_FILES_FILES_SHARE_EXPORTED') . "/";
            if (preg_match($pattern, $current['message']) && $next) {
