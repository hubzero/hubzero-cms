...
class Project extends Model
{
    ...
    public function check($name = '', $pid = 0, $ajax = 0)
    {
        ...
        if (!$name) {
            ...
            if (
                !$this->_tbl->checkUniqueName($name, $pid)
        ...
        if ($activity) {
            ...
            switch ($activity) {
                ...
                default:
                    ...
                    // Publications
                    if (
                        substr($activity, 0, strlen('started a new publication')) == 'started a new publication'
                    ...
                    if (
                        substr($activity, 0, strlen('published version')) == 'published version'
                    ...
                    if (
                        substr($activity, 0, strlen('submitted draft')) == 'submitted draft'
                    ...
                    $check = 'has been updated by administrator';
                    if (substr($activity, 0, strlen($check)) == $check) {
                        $action = 'updated';
                        $scope = 'publication';
                    }
                    // Notes
    ...
    public function recordFirstJoinActivity()
    {
        if ($this->isMemberConfirmed() && !$this->isProvisioned() && $this->isActive()) {
            if (!$this->member()->lastvisit) {
                $aid = $this->recordActivity(
                    Lang::txt('COM_PROJECTS_ACTIVITY_JOINED_THE_PROJECT'),
                    $this->get('id'),
                    '',
                    '',
                    'team',
    ...
    {
        $src  = '';
        $imagePath = trim($this->config()->get('imagepath', '/site/projects'), DS);
        $path = PATH_APP . DS . $imagePath . DS . $this->get('alias') . DS . 'images';
