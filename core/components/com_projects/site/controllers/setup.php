...
class Setup extends Base
{
    ...
    public function displayTask()
    {
        ...
        if ($this->model->exists() && !$this->model->access('owner')) {
            ...
            if (
                !$this->group->is_member_of('members', User::get('id'))
    ...
    public function saveTask()
    {
        ...
        // Check authorization
        $hasOwnerAccess = $this->model->access('owner');
        $hasManagerAccess = $this->model->access('manager');
        $hasContentEditAccess = $this->model->access('content')
            && $this->config->get('edit_description');
        if ($this->model->exists() && !($hasOwnerAccess || $hasManagerAccess || $hasContentEditAccess)) {
            throw new Exception(Lang::txt('ALERTNOTAUTH'), 403);
        } elseif (!$this->model->exists() && $this->_gid) {
            // Check group authorization to create a project
            if (
        ...
        $redirectUrl = 'index.php?option=' . $this->_option
            ...
            . '&active=' . $this->next;
        App::redirect(Route::url($redirectUrl) . $append);
        return;
    }
        ...
        if ($this->_setupComplete == 3) {
            ...
            if ($this->config->get('restricted_data', 0) == 1) {
                ...
                if ($restricted == 'yes') {
                    ...
                    if (
                        ($restrictions['ferpa_data'] == 'yes' && !$agree_ferpa)
        ...
        switch ($this->section) {
                ...
                if ($new && $this->section != 'info_custom') {
                    ...
                    if ($this->_gid) {
                        ...
                        foreach ($team as $user_id) {
                            ...
                            // They'll choose if they want to sync
                            // the entire group or not in the next step
                            $saved = $objO->saveOwners(
                                $this->model->get('id'),
                                $user_id,
                                $user_id,
                                $this->_gid,
                            ...
                            if (!$saved) {
                                $this->setError(
                                    Lang::txt('COM_PROJECTS_ERROR_SAVING_AUTHORS')
                                    . ': ' . $objO->getError()
                                );
                                return false;
                            }
                    ...
                    if (!empty($incoming)) {
                        foreach ($incoming as $key => $value) {
                            ...
                            if ($key == 'grant_status') {
                                ...
                                $dateStr = Date::of('now')->toLocal('M d, Y');
                                $meta = '<meta>' . $dateStr . ' - '
                                    . User::get('name') . '</meta>';

                                $cbase = $this->model->get('admin_notes');
                                $grantMsg = Lang::txt(
                                    'COM_PROJECTS_PROJECT_MANAGER_GRANT_INFO_UPDATE'
                                );
                                $cbase .= '<nb:sponsored>'
                                    . $grantMsg . $meta . '</nb:sponsored>';
                                ...
                                if (\Hubzero\User\Group::getInstance($admingroup)) {
                                    ...
                                    if (!empty($admins)) {
                                        Helpers\Html::sendHUBMessage(
                                            ...
                                            $admins,
                                            Lang::txt(
                                                'COM_PROJECTS_EMAIL_ADMIN_REVIEWER_NOTIFICATION'
                                            ),
                                            'projects_new_project_admin',
                                            'admin',
                                            $grantMsg,
            ...
            case 'settings':
                ...
                if (!empty($incoming)) {
                    ...
                    // Record activity
                    $activityMsg = Lang::txt('COM_PROJECTS_PROJECT_SETTINGS_UPDATED');
                    $this->model->recordActivity($activityMsg);
                }
                break;
        }
    }
    ...
    public function editTask()
    {
        ...
        $hasContentEditAccess = $this->model->access('content')
            && $this->config->get('edit_description');
        if (!$hasManagerAccess && !$hasContentEditAccess) {
            throw new Exception(Lang::txt('ALERTNOTAUTH'), 403);
        }
        ...
        $isCustomProfile = $this->config->get('custom_profile') == 'custom';
        $isInfoSection = $this->section == 'info' || $this->section == 'info_custom';
        if ($isCustomProfile && $isInfoSection) {
    ...
    public function getGrantAgencyTask()
    {
        ...
        if (!empty($verNum)) {
            ...
            $queryURL = $baseUrl
                . "?filter=types:funder&query.advanced=names.value:"
                . urlencode($term);
            ...
            foreach ($resultObj->items as $orgObj) {
                foreach ($orgObj->names as $nameObj) {
                    $isEnglish = $nameObj->lang == "en";
                    if ($isEnglish && !in_array($nameObj->value, $agencies)) {
