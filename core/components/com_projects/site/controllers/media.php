...
class Media extends Base
{
    ...
    public function doajaxuploadTask()
    {
        ...
        if ($size > $sizeLimit) {
            $formatted = \Hubzero\Utility\Number::formatBytes($sizeLimit);
            $max = preg_replace('/<abbr \w+=\\"\w+\\">(\w{1,3})<\\/abbr>/', '$1', $formatted);
            echo json_encode(array(
                'error' => Lang::txt('File is too large. Max file upload size is ') . $max
            ));
            return;
        }
        ...
        $isManager = $this->model->access('manager');
        $canEditContent = $this->model->access('content') && $this->config->get('edit_description');
        if (!$isManager && !$canEditContent) {
        ...
        if (!is_file($path . DS . $file)) {
            ...
            if (count($hi->getErrors()) == 0) {
                ...
                try {
                    ...
                    if (
                        strpos($e->getMessage(), 'Illegal IFD size') !== false
    ...
    public function deleteimgTask()
    {
        ...
        $isManager = $this->model->access('manager');
        $canEditContent = $this->model->access('content') && $this->config->get('edit_description');
        $hasAccess = $isManager || $canEditContent;
        ...
        if ($media == 'thumb' || $media == 'master') {
            ...
            if ($dir == 'tools') {
                ...
                if (!is_file($source)) {
                    ...
                    $defaultToolPic = $this->config->get(
                        'toolpic',
                        'plugins/projects/tools/images/default.gif'
                    );
                    $source = PATH_CORE . DS . trim($defaultToolPic, DS);
                }
            } else {
                ...
                $path     = trim($this->config->get('imagepath', '/site/projects'), DS);
                $source   = PATH_APP . DS . $path . DS . $this->model->get('alias')
                    . DS . $dir . DS . urldecode($media);
                $redirect = true;
            }
        }
