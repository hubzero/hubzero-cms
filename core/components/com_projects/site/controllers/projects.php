...
class Projects extends Base
{
    ...
    public function requestAccessTask()
    {
        ...
        $return = Route::url(
            'index.php?option=com_projects&task=requestaccess&alias='
                . $project . '&' . Session::getFormToken() . '=1',
            false
        );
        if (User::isGuest()) {
        ...
        if (!$existingMember || $existingMember->status == 2) {
            if ($this->model->exists()) {
                ...
                if ($member->save()) {
                    ...
                    $message = Lang::txt(
                        'COM_PROJECTS_EMAIL_MEMBERSHIPREQUEST_MESSAGE',
                        $member->user->name,
                        $member->user->email
                    );
    ...
    public function browseTask()
    {
        ...
        $hasReviewerAccess = $this->model->reviewerAccess($reviewer);
        if (User::isGuest() || (!$isAdmin && !$hasReviewerAccess)) {
            $filters['active'] = true;
        }

        $validSortby = array(
            'title', 'id', 'myprojects', 'owner', 'created',
            'type', 'role', 'privacy', 'status', 'grant_status'
        );
        if (!in_array($filters['sortby'], $validSortby)) {
    ...
    public function viewTask()
    {
        ...
        $isMember = $this->model->access('member');
        $hasPublicAccess = AccessHelper::allowPublicAccess($subdir);
        $layout = ($isMember || $hasPublicAccess) ? 'internal' : 'external';
        //$layout = $isMember && $preview && $this->model->isPublic() ? 'external' : $layout;
        $layout = $isMember && $preview ? 'external' : $layout;
        ...
        if (
            !in_array($this->model->get('access'), User::getAuthorisedViewLevels())
            && $layout != 'invited'
    ...
    public function activateTask()
    {
        ...
        if ($confirm && !$verified) {
            $error = $this->model->getError()
                ? $this->model->getError()
                : Lang::txt('COM_PROJECTS_ERROR_NAME_INVALID_OR_EMPTY');
            $this->setError($error);
    ...
    public function authTask()
    {
        ...
        if ($code) {
            ...
            $return = Route::url(
                'index.php?option=' . $this->_option . '&alias='
    ...
    public function processTask()
    {
        ...
        if ($action == 'save' && !$this->getError()) {
            ...
            if ($reviewer == 'sensitive') {
                ...
                // New approval
                if (
                    trim($params->get('grant_approval', '')) == '' && trim($grant_approval) != ''
                    ...
                    // Bump up quota
                    $quotaSize = floatval($this->config->get('premiumQuota', '30'));
                    $premiumQuota = Helpers\Html::convertSize($quotaSize, 'GB', 'b');
                    $this->model->saveParam('quota', $premiumQuota);

                    // Bump up publication quota
                    $pubQuotaSize = floatval($this->config->get('premiumPubQuota', '10'));
                    $premiumPubQuota = Helpers\Html::convertSize($pubQuotaSize, 'GB', 'b');
            ...
            if ($approve) {
                ...
                if ($reviewer == 'sponsored') {
                    if ($approve == 1) {
                        $approvalCode = ucfirst(Lang::txt('COM_PROJECTS_APPROVAL_CODE'));
                        $approvedText = Lang::txt('COM_PROJECTS_PROJECT_APPROVED_SPS');
                        $cbase .= '<nb:' . $reviewer . '>' . $approvedText
                            . ' ' . $approvalCode . ': ' . $grant_approval;
                        $cbase  .= (trim($comment) != '') ? '. ' . $comment : '';
                        $cbase  .= $meta . '</nb:' . $reviewer . '>';
                    } elseif ($approve == 2) {
            ...
            $browseUrl = Route::url(
                'index.php?option=' . $this->_option . '&task=browse&reviewer='
                    . $reviewer . '&filterby=' . $filterby
            );
            App::redirect($browseUrl);
            return;
        } else {
