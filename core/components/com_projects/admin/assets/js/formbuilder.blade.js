...
(function() {
  ...
  FormbuilderModel = (function(_super) {
    ...
    FormbuilderModel.prototype.indexInDOM = function() {
      ...
      for (var i = 0; i < wrappers.length; i++) {
        if (wrappers[i].getAttribute('data-cid') === _this.cid || wrappers[i]._cid === _this.cid) {
          matched = wrappers[i];
          matchedIndex = i;
          break;
        }
  ...
  ViewFieldView = (function(_super) {
    ...
    ViewFieldView.prototype.render = function() {
      this.el.className = this.className + ' response-field-' + this.model.get(Formbuilder.options.mappings.FIELD_TYPE);
  ...
  EditFieldView = (function(_super) {
    ...
    EditFieldView.prototype.addOption = function(e) {
      ...
      var i = -1;
      if (closestOption) {
        i = Array.prototype.indexOf.call(optionEls, closestOption);
      }
      var options = this.model.get(Formbuilder.options.mappings.OPTIONS) || [];
      var newOption = {
        label: "",
        checked: false
      };
    ...
    EditFieldView.prototype.removeOption = function(e) {
      var el = e.currentTarget;
      var removeButtons = this.el.querySelectorAll('.js-remove-option');
      var index = Array.prototype.indexOf.call(removeButtons, el);
      var options = this.model.get(Formbuilder.options.mappings.OPTIONS);
      ...
      if (this.model.get(Formbuilder.options.mappings.FIELD_TYPE) !== 'checkboxes') {
        ...
        for (var i = 0; i < defaults.length; i++) {
          if (defaults[i] !== el) {
            ...
            defaults[i].dispatchEvent(new Event('change'));
          }
        }
      }
      return this.forceRender();
  ...
  BuilderView = (function(_super) {
    ...
    BuilderView.prototype.bindSaveEvent = function() {
      ...
      return window.addEventListener('beforeunload', function(e) {
        if (_this.formSaved) {
          ...
          e.preventDefault();
          e.returnValue = Formbuilder.options.dict.UNSAVED_CHANGES;
    ...
    BuilderView.prototype.render = function() {
      ...
      this._responseFieldsEl = this.el.querySelector('.fb-response-fields');
    ...
    BuilderView.prototype._responseFields = function() {
      return this._responseFieldsEl || this.el.querySelector('.fb-response-fields');
    };
    ...
    BuilderView.prototype.bindWindowScrollEvent = function() {
      ...
      return window.addEventListener('scroll', function() {
        ...
        if (_this._fbLeft && _this._fbLeft._locked === true) {
          return;
        }
        var elRect = _this.el.getBoundingClientRect();
        var scrollTop = window.pageYOffset || document.documentElement.scrollTop;
        var elTop = elRect.top + scrollTop;
        newMargin = Math.max(0, scrollTop - elTop);
        var rfEl = _this._responseFields();
        maxMargin = rfEl ? rfEl.offsetHeight : 0;
        if (_this._fbLeft) {
    ...
    BuilderView.prototype.showTab = function(e) {
      ...
      if (targetEl) {
        var tabPanes = targetEl.parentNode.querySelectorAll('.fb-tab-pane');
    ...
    BuilderView.prototype.addOne = function(responseField, _, options) {
      ...
      var rfEl = this._responseFields();
      if (options.$replaceEl != null) {
        return options.$replaceEl.replaceWith(view.render().el);
      } else if ((options.position == null) || options.position === -1) {
        return rfEl.appendChild(view.render().el);
      } else if (options.position === 0) {
        return rfEl.insertBefore(view.render().el, rfEl.firstChild);
      } else {
        var wrappers = rfEl.querySelectorAll('.fb-field-wrapper');
        if (wrappers[options.position]) {
          return rfEl.insertBefore(view.render().el, wrappers[options.position]);
        } else {
          return rfEl.appendChild(view.render().el);
        }
      }
    ...
    BuilderView.prototype.setDraggable = function() {
      ...
      if (typeof jQuery !== 'undefined' && jQuery.fn.draggable) {
        var rfEl = this._responseFields();
        jQuery(addFieldButtons).draggable({
          connectToSortable: jQuery(rfEl),
          helper: function() {
            var helper = document.createElement('div');
            helper.className = 'response-field-draggable-helper';
            helper.style.width = rfEl.offsetWidth + 'px';
            helper.style.height = '80px';
            return helper;
          }
        });
      }
    ...
    BuilderView.prototype.addField = function(e) {
      var field_type = e.currentTarget.getAttribute('data-field-type');
    ...
    BuilderView.prototype.createAndShowEditView = function(model) {
      ...
      var responseFieldEl = null;
      for (var i = 0; i < wrappers.length; i++) {
        if (wrappers[i]._cid === model.cid || wrappers[i].getAttribute('data-cid') === model.cid) {
          responseFieldEl = wrappers[i];
          wrappers[i].classList.add('editing');
        } else {
          wrappers[i].classList.remove('editing');
        }
      }
      if (this.editView) {
      ...
      var newEditEl = this.editView.render().el;
    ...
    BuilderView.prototype.ensureEditViewScrolled = function() {
      ...
      var editing = document.querySelector('.fb-field-wrapper.editing');
      return this.scrollLeftWrapper(editing);
    };
    ...
    BuilderView.prototype.scrollLeftWrapper = function(responseFieldEl) {
      ...
      var elRect = _this.el.getBoundingClientRect();
      var rfRect = _this._responseFields().getBoundingClientRect();
    ...
    BuilderView.prototype.doAjaxSave = function(payload) {
      var _this = this;
      fetch(Formbuilder.options.HTTP_ENDPOINT, {
