...
class Projects extends AdminController
{
    ...
    public function displayTask()
    {
        ...
        // Get quotas
        $defaultQuotaVal = floatval($this->config->get('defaultQuota', 1));
        $premiumQuotaVal = floatval($this->config->get('premiumQuota', 30));
        $this->view->defaultQuota = Helpers\Html::convertSize($defaultQuotaVal, 'GB', 'b');
        $this->view->premiumQuota = Helpers\Html::convertSize($premiumQuotaVal, 'GB', 'b');
        ...
        if ($this->view->filters['quota'] != 'all' && $this->view->rows) {
            ...
            for ($i = 0, $n = count($rows); $i < $n; $i++) {
                ...
                if (
                    ($this->view->filters['quota'] == 'premium' && $quota < $this->view->premiumQuota)
        ...
        if ($this->config->get('offroot') && !is_dir($this->config->get('webpath'))) {
            $errorMsg = Lang::txt('Master directory does not exist. Administrator must fix this! ')
                . $this->config->get('webpath');
            $this->view->setError($errorMsg);
        }
    ...
    public function saveTask($redirect = false)
    {
        ...
        if ($this->config->get('messaging', 0) && $sendmail && count($managers) > 0) {
            ...
            Event::trigger('xmessage.onSendMessage', $messageParams);
        }
    ...
    public function eraseTask()
    {
        ...
        // Redirect
        $message = Lang::txt('COM_PROJECTS_PROJECT') . ' #' . $id
            . ' (' . $alias . ') ' . Lang::txt('COM_PROJECTS_PROJECT_ERASED');
    ...
    public function saveDescriptionTask()
    {
        ...
        foreach ($description->fields as $i => $element) {
            ...
            $fieldOpts = $element->field_options;
            $description = isset($fieldOpts->description) ? (string) $fieldOpts->description : '';
            $optionOther = isset($fieldOpts->include_other_option)
                ? (int) $fieldOpts->include_other_option
                : '';
            $optionBlank = isset($fieldOpts->include_blank_option)
                ? (int) $fieldOpts->include_blank_option
                : '';
            $field->set(array(
    ...
    public function getGrantAgencyTask()
    {
        ...
        if (!empty($verNum)) {
            $queryURL = "https://api.ror.org/$verNum/organizations"
                . "?filter=types:funder&query.advanced=names.value:" . urlencode($term);
    ...
    public function getGrantAgencyId($grantAgency)
    {
        ...
        if (!empty($verNum)) {
            $queryURL = "https://api.ror.org/$verNum/organizations"
                . "?filter=types:funder&query.advanced=names.value:" . urlencode($agencyQry);
