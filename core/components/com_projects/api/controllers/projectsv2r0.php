...
class Projectsv2r0 extends ApiController
{
    /**
     * Execute a task
     *
        ...
        if ($response->total) {
            ...
            foreach ($query->rows() as $i => $entry) {
                if (isset($searchable)) {
                    ...
                    $obj->thumbUrl      = str_replace(
                        '/api',
                        '',
                        $base . '/' . ltrim(Route::url($entry->link('thumb')), '/')
                    );
    ...
    public function createTask()
    {
        ...
        if ($row->config('restricted_data', 0) == 1) {
            ...
            if ($restricted == 'yes') {
                ...
                if ($selected == 0) {
                    throw new Exception(
                        Lang::txt('COM_PROJECTS_ERROR_SETUP_TERMS_SPECIFY_DATA'),
                        409
                    );
                }
                ...
                if (
                    ...
                    throw new Exception(
                        Lang::txt('COM_PROJECTS_ERROR_SETUP_TERMS_RESTRICTED_DATA_AGREE_REQUIRED'),
                        409
                    );
                }
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope'       => 'project',
                'scope_id'    => $row->get('id'),
                'description' => Lang::txt('COM_PROJECTS_ACTIVITY_ENTRY_CREATED', $link),
                'details'     => $fields
            ],
            'recipients' => [
    ...
    public function updateTask()
    {
        ...
        if ($row->config('grantinfo', 0)) {
            ...
            if (
                $row->params->get('grant_budget') != $budget
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope'       => 'project',
                'scope_id'    => $row->get('id'),
                'description' => Lang::txt('COM_PROJECTS_ACTIVITY_ENTRY_UPDATED', $link),
                'details'     => array(
                    'title' => $row->get('title'),
                    'url'   => $url
                )
        ...
        foreach ($ids as $id) {
            ...
            Event::trigger('system.logActivity', [
                'activity' => [
                    ...
                    'scope'       => 'project',
                    'scope_id'    => $id,
                    'description' => Lang::txt('COM_PROJECTS_ACTIVITY_ENTRY_DELETED', $link),
                    'details'     => $data
                ],
                'recipients' => [
                    $row->get('created_by_user'),
