...
class Owner extends Table
{
    ...
    public function isOwner($uid = null, $projectid = null, $status = 1)
    {
        ...
        $typequery = $status == 'active'
            ? " AND o.status != 2 "
            : " AND o.status=" . $this->_db->quote(intval($status));
    ...
    public function countOwners($projectid = null, $filters = array())
    {
        ...
        $query  = "SELECT COUNT(*) ";
        $query .= " FROM (SELECT DISTINCT projectid, userid, invited_name, invited_email FROM $this->_tbl";
    ...
    public function getInfo($projectid = null, $ids = array(), $groups = array())
    {
        ...
        if (count($ids) > 0) {
            $query  = "SELECT DISTINCT o.*, x.name, x.username, x.picture, ";
            $query .= "g.cn as groupname ";
    ...
    public function getOwnerId($projectid = null, $uid = 0, $name = null)
    {
        ...
        if ($name && intval($uid) == 0) {
            $query = "SELECT id FROM $this->_tbl WHERE projectid="
                . $this->_db->quote($projectid)
                . " AND invited_name=" . $this->_db->quote($name);
        } elseif (intval($uid)) {
            $query = "SELECT id FROM $this->_tbl WHERE projectid="
                . $this->_db->quote($projectid)
                . " AND userid=" . $this->_db->quote($uid);
        } else {
            return false;
        }
        $this->_db->setQuery($query);
        ...
        if ($result) {
            ...
            foreach ($result as $entry) {
                ...
                if ($limit && $i == $limit && $i != count($result)) {
                    $remaining = count($result) - $limit;
                    $names .= ', ' . Lang::txt('COM_PROJECTS_AND') . ' ' . $remaining;
                    $names .= ' ' . Lang::txt('COM_PROJECTS_MORE') . ' ';
                    $names .= $remaining == 1
                        ? Lang::txt('COM_PROJECTS_ACTIVITY_PERSON')
    ...
    public function getOwners($projectid = null, $filters = array())
    {
        ...
        $native     = isset($filters['native'])    ? $filters['native'] : '-';
        $pubVersionId = isset($filters['pub_versionid']) ? intval($filters['pub_versionid']) : 0;
        $pub        = $pubVersionId ? $filters['pub_versionid'] : '';
        $connected  = isset($filters['connected']) ? $filters['connected'] : 0;

        $query   =  "SELECT DISTINCT ";
        if (!$select) {
            $query .= " o.*, x.name, x.username, x.organization, x.picture, ";
            $query .= "g.cn as groupname, g.description as groupdesc, p.created_by_user ";
            $query .= ", if (o.userid = 0, o.invited_name, x.name) as fullname ";
            if ($pub) {
                $query .= " , pa.organization as a_organization, ";
                $query .= "pa.name as a_name, pa.credit ";
            }
        } else {
        ...
        $query .= " JOIN #__projects as p ON o.projectid=p.id";
        if ($pub) {
            $query .= " LEFT JOIN #__publication_authors as pa ON o.id=pa.project_owner_id";
            $query .= " AND pa.publication_version_id=" . $this->_db->quote($pub);
        }
        ...
        if ($groups && count($groups) > 0) {
            ...
            if ($synced != 1) {
                ...
                if (!empty($where_groups)) {
                    $query  = "SELECT DISTINCT m.uidNumber, m.gidNumber "
                        ...
                        . "WHERE m.gidNumber IN (" . implode(',', $where_groups) . ")";
                    $this->_db->setQuery($query);
                    $members = $this->_db->loadObjectList();
                }
                foreach ($members as $m) {
            ...
            if (!empty($owners_to_add)) {
                foreach ($owners_to_add as $newcomer) {
                    $memberGroup = $array_member_groups[$newcomer];
                    $isNative = $array_groups_native[$memberGroup];
                    $added = $this->saveOwners(
                        $projectid,
                        0,
                        $newcomer,
                        $memberGroup,
        ...
        if (!empty($users)) {
            foreach ($users as $user) {
                if ($remove == 1) {
                    ...
                    $query .= !$byownerid
                        ? " AND userid = " . $this->_db->quote($user)
                        : " AND id = " . $this->_db->quote($user);
                } else {
                    $query  = "UPDATE $this->_tbl SET status = " . $this->_db->quote($status)
                        . ", lastvisit = null, params = null, num_visits = 0, groupid = 0 "
                        . "WHERE projectid = " . $this->_db->quote($projectid);
                    $query .= !$byownerid
                        ? " AND userid = " . $this->_db->quote($user)
                        : " AND id = " . $this->_db->quote($user);
    ...
    public function reassignRole($projectid = null, $users = array(), $byownerid = 0, $role = 0)
    {
        ...
        if (!empty($users)) {
            foreach ($users as $user) {
                $query  = "UPDATE $this->_tbl SET role=" . $this->_db->quote($role)
                    . " WHERE projectid = " . $this->_db->quote($projectid);
                $query .= !$byownerid
                    ? " AND userid =" . $this->_db->quote($user)
                    : " AND id = " . $this->_db->quote($user);
    ...
    public function checkInvited($projectid = null, $email = '')
    {
        ...
        $query  = "SELECT id FROM $this->_tbl WHERE invited_email="
            . $this->_db->quote($email)
            . " AND projectid=" . $this->_db->quote($projectid) . " LIMIT 1";
        $this->_db->setQuery($query);
        return $this->_db->loadResult();
    }
    ...
    public function checkInvitedByName($projectid = null, $name = '')
    {
        ...
        $query  = "SELECT id FROM $this->_tbl WHERE invited_name="
            . $this->_db->quote($name)
            . " AND projectid=" . $this->_db->quote($projectid) . " LIMIT 1";
        $this->_db->setQuery($query);
        return $this->_db->loadResult();
        ...
        if ($userid && is_numeric($userid)) {
            ...
            if ($found === null) {
                ...
                $query  = "INSERT INTO $this->_tbl "
                    . "(`projectid`, `userid`, `groupid`, `added`, `status`, "
                    . "`native`, `role`, `invited_email`) "
                    . "SELECT $projectid, $userid, $groupid, '$now', $status, "
                    . "$native, $role, '$invited_email' FROM DUAL "
                    . "WHERE NOT EXISTS (SELECT `id` FROM $this->_tbl "
                ...
                $query = "UPDATE $this->_tbl SET added = '$now', status = 1, "
                    . "role = " . $this->_db->quote($role) . ", "
                ...
                foreach ($owners as $owner) {
                    ...
                    if ($found === null) {
                        ...
                        $query  = "INSERT INTO $this->_tbl "
                            . "(`projectid`, `userid`, `groupid`, `added`, `status`, "
                            . "`native`, `role`, `invited_email`) "
                            . "SELECT $projectid, $owner, $gidNumber, '$now', $status, "
                            . "$native, $role, '$invited_email' FROM DUAL "
                            . "WHERE NOT EXISTS (SELECT `id` FROM $this->_tbl "
                            . "WHERE `projectid` = $projectid AND `userid` = $userid LIMIT 1)";
                        $this->_db->setQuery($query);
                        ...
                        $query = "UPDATE $this->_tbl SET "
                            . "added = " . $this->_db->quote($now) . ", status = 1, "
                            . "groupid = " . $this->_db->quote($gidNumber) . ", "
                            . "role = " . $this->_db->quote($role)
                            . " WHERE projectid = " . $this->_db->quote($projectid)
                            . " AND userid = " . $this->_db->quote($owner);
                        $this->_db->setQuery($query);
                        if ($this->_db->query()) {
                            $added[] = $owner;
                        }
    ...
    public function loadOwner($projectid = null, $owner = 0)
    {
        ...
        $query = "SELECT * FROM $this->_tbl WHERE projectid="
            . $this->_db->quote($projectid)
            . " AND userid=" . $this->_db->quote($owner) . " LIMIT 1";
        $this->_db->setQuery($query);
    ...
    public function matchInvite($identifier = null, $code = '', $email = '')
    {
        ...
        $query .= " LEFT JOIN $this->_tbl AS o ON o.projectid=p.id"
            . " AND o.userid=0 AND o.status != 2"
            . " AND LOWER(o.invited_email)=" . $this->_db->quote(strtolower($email))
