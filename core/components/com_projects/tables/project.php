...
class Project extends Table
{
    ...
    public function buildQuery($filters = array(), $admin = false, $uid = 0, $showall = 0, $setup_complete = 3)
    {
        ...
        if ($reviewer == 'sensitive') {
            ...
            if ($mine) {
                if ($filterby == 'archived') {
                    ...
                    if ($uid) {
                        $quotedUid = $this->_db->quote($uid);
                        $quotedSetup = $this->_db->quote($setup_complete);
                        $query .= " WHERE (o.userid=" . $quotedUid . " AND o.status!=2"
                            . " AND p.state NOT IN (2, 3)"
                            . " AND ((p.setup_stage >= " . $quotedSetup . ")"
                            . " OR (o.role = 1 AND p.owned_by_user=" . $quotedUid . "))) ";
                    } else {
                        $query .= " WHERE 1=2";
                    }
                } else {
                ...
                if ($filterby == 'archived') {
                    ...
                    if ($uid) {
                        $quotedUid = $this->_db->quote($uid);
                        $quotedSetup = $this->_db->quote($setup_complete);
                        $query .= " WHERE ((p.state = 1 " . $privacy . ")"
                            . " OR (o.userid=" . $quotedUid . " AND o.status!=2"
                            . " AND ((p.state = 1 AND p.setup_stage >= " . $quotedSetup . ")"
                            . " OR (o.role = 1 AND p.owned_by_user=" . $quotedUid
                            . " AND p.state != 3)))) ";
                    } else {
                        $query .= " WHERE p.state = 1 " . $privacy;
        ...
        if ($search) {
            $searchTerm = $this->_db->quote('%' . $search . '%');
            $query .= " AND (p.title LIKE " . $searchTerm
                . " OR p.alias LIKE " . $searchTerm . ") ";
        }
    ...
    public function getGroupProjects(
        ...
        $query  = "SELECT DISTINCT p.*, IFNULL(o.role, 0) as role, o.id as owner, ";
        $query .= "o.added as since, o.status as confirmed ";
        $query .= ", x.name as authorname, g.cn as groupcn, g.description as groupname ";
        $query .= ", (SELECT COUNT(*) FROM `#__activity_recipients` AS pa "
            . "WHERE pa.scope='project' AND pa.scope_id=p.id "
        ...
        $query .= " LEFT JOIN #__project_owners AS o ON o.projectid=p.id "
            . "AND o.userid=" . $quotedUid . " AND o.userid != 0 AND p.state!= 2 ";
        ...
        if (isset($filters['filterby']) && in_array($filters['filterby'], array('active', 'archived'))) {
            if ($filters['filterby'] == 'archived') {
                $query .= " WHERE p.id=po.projectid AND p.state=3 "
                    . "AND po.status=1 AND po.groupid=" . $groupid;
            } elseif ($filters['filterby'] == 'active') {
                $query .= " WHERE p.id=po.projectid AND p.state NOT IN (2, 3) "
                    . "AND po.status=1 AND po.groupid=" . $groupid;
            }
        } else {
            $query .= " WHERE p.id=po.projectid AND p.state !=2 "
                . "AND po.status=1 AND po.groupid=" . $groupid;
        }
        ...
        if (isset($filters['count']) && $filters['count'] == 1) {
            $this->_db->setQuery($query);
            return $this->_db->loadResult();
        } elseif (isset($filters['limit']) && $filters['limit'] != 'all' && $filters['limit'] != 0) {
            $filters['start'] = isset($filters['start']) ? $filters['start'] : 0;
            $query .= " LIMIT " . $filters['start'] . "," . $filters['limit'];
        }
    ...
    public function getUserProjectIds($uid = 0, $active = 1, $include_provisioned = 0)
    {
        ...
        if ($uid) {
            ...
            if ($active == 1) {
                $quotedUid = $this->_db->quote($uid);
                $query .= "AND (p.state=1 OR (o.role = 1 "
                    . "AND p.owned_by_user=" . $quotedUid . " AND p.state !=2)) ";
            } else {
                $query .= "AND p.state !=2 ";
            }
    ...
    public function getGroupProjectIds($groupid = 0, $uid = 0, $active = 1)
    {
        ...
        if ($groupid) {
            ...
            $query .= $active == 1
                ? " AND (p.state=1 OR (o.role = 1 "
                    . "AND p.owned_by_user=" . $this->_db->quote($uid) . " AND p.state !=2)) "
                : " AND p.state !=2 ";
            $query .= " AND p.provisioned=0";
            */
    ...
    public function getUpdateCount($projects = array(), $uid = 0)
    {
        if (!empty($projects) && $uid != 0) {
            ...
            $query = "SELECT COUNT(*) "
                ...
                . "INNER JOIN `#__project_owners` as o "
                . "ON o.projectid=pa.scope_id AND o.userid=" . $quotedUid . " "
    ...
    public function loadProvisionedProject($pid = null)
    {
        ...
        $query = "SELECT p.* FROM `#__publications` AS pu "
            . "JOIN $this->_tbl AS p ON pu.project_id=p.id "
            . "WHERE pu.id=" . $this->_db->quote($pid) . " LIMIT 1";
        $this->_db->setQuery($query);
        if ($result = $this->_db->loadAssoc()) {
            return $this->bind($result);
        }
