...
class Posts extends SiteController
{
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt(
                    'COM_COLLECTIONS_ACTIVITY_COMMENT_' . ($comment['id'] ? 'UPDATED' : 'CREATED'),
                    $row->get('id'),
                    $item
                ),
                'details'     => array(
    ...
    public function voteTask()
    {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt(
                    'COM_COLLECTIONS_ACTIVITY_VOTED',
                    $post->get('item_id'),
                    '<a href="' . Route::url($url) . '">' . $title . '</a>'
                ),
                'details'     => array(
                    'collection_id' => $post->get('collection_id'),
                    'post_id' => $post->get('id'),
    ...
    public function collectTask()
    {
        ...
        // Display updated item stats if called via AJAX
        if ($no_html) {
            $count = $post->getCount(array('item_id' => $post->get('item_id'), 'original' => 0));
            echo Lang::txt('COM_COLLECTIONS_NUM_REPOSTS', $count);
            exit;
        }
