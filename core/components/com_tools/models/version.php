...
class Version
{
    ...
    public function getTool_VersionNames($tool = null)
    {
        ...
        } else { // object method call
            if (is_numeric($tool)) {
                $where_clause = "toolid=" . $db->Quote($tool);
            } elseif (!empty($tool)) {
                $where_clause = "toolname=" . $db->Quote($tool);
            } elseif (empty($this->toolname)) {
                $where_clause = "toolid=" . $db->Quote($this->id);
            } elseif (empty($this->toolid)) {
    ...
    public function read($instance = null)
    {
        if (is_null($instance)) {
            ...
            if (!empty($instance) && !is_string($instance) && !is_numeric($instance)) {
                $this->_error(
                    __FUNCTION__ . ": invalid tool version instance defined",
                    E_USER_ERROR
                );
                die();
            }
        ...
        if (!empty($instance) && !is_string($instance) && !is_numeric($instance)) {
            $this->_error(
                __FUNCTION__ . ": Argument #1 is not a valid string and not numeric",
                E_USER_ERROR
            );
            die();
        }
    ...
    private function _sql_update($all = false)
    {
        ...
        foreach ($this->_list_keys as $property) {
            ...
            } elseif ($property == 'member' || $property == 'owner') {
    ...
    private function _sql_delete()
    {
        ...
        if ($this->state == 3) {
            $db->setQuery(
                "DELETE FROM #__trac_user_action as a,#__trac as t " .
                "WHERE a.trac_id=t.id AND t.scope='tool' AND t.name=" . $db->Quote($this->toolname)
            );
            $db->query();

            $db->setQuery(
                "DELETE FROM #__trac_group_action as a,#__trac as t " .
                "WHERE a.trac_id=t.id AND t.scope='tool' AND t.name=" . $db->Quote($this->toolname)
            );
            $db->query();
    ...
    public function __set($property = null, $value = null)
    {
        ...
        } elseif (in_array($property, $this->_list_keys)) {
    ...
    public function getDevelopmentGroup($byid = false)
    {
        ...
        if ($byid == false) {
            $query = "SELECT cn FROM #__tool_groups AS tg " .
                "WHERE tg.role='1' AND tg.toolid=" . $db->Quote($this->toolid) . " LIMIT 1";
        } else {
            $query = "SELECT gidNumber FROM #__xgroups AS xg, #_tool_groups AS tg " .
                "WHERE tg.cn=xg.cn AND tg.role='1' AND tg.toolid=" .
                $db->Quote($this->toolid) . " LIMIT 1";
        }
        $db->setQuery($query);
        $result = $db->loadResult();
        return $result;
    ...
    public static function getVersionInfo($id, $version = null, $toolname = null, $instance = null)
    {
        ...
        $query .= "FROM #__tool_version as v LEFT JOIN #__doi_mapping as d " .
            "ON d.alias = v.toolname AND d.local_revision=v.revision ";
        if ($id) {
            $query .= "WHERE v.id = '" . $id . "' ";
        } elseif ($version && $toolname) {
