...
class Preferences extends Table
{
    ...
    {
        if (!$this->user_id) {
            $this->user_id = \Hubzero\Facades\User::get('id');
        }
        ...
        if (!$this->id) {
            ...
            if ($test->id) {
                $this->setError(\Hubzero\Facades\Lang::txt('User with ID of %s already has a record.', $this->user_id));
            }
        }
    ...
    protected function buildQuery($filters = array())
    {
        ...
        if (isset($filters['search']) && isset($filters['search_field'])) {
            $where[] = $this->_db->quoteName($filters['search_field'])
                . ' LIKE ' . $this->_db->quote('%' . $filters['search'] . '%');
        }
        if (
            isset($filters['class_alias'])
            && is_string($filters['class_alias'])
            && strlen($filters['class_alias']) > 0
        ) {
