...
class Group extends Table
{
    ...
    {
        $query = "INSERT INTO $this->_tbl (cn, toolid, role) VALUES ("
            . $this->_db->quote($cn) . ","
            . $this->_db->quote($toolid) . ","
            . $this->_db->quote($role) . ")";
    ...
    public function saveGroup($toolid, $devgroup, $members, $exist)
    {
        ...
        if (\Hubzero\User\Group::exists($devgroup)) {
            $group->read($devgroup);
            $existing_members = \Components\Tools\Helpers\Utils::transform(
                Tool::getToolDevelopers($toolid),
                'uidNumber'
            );
    ...
    public function saveMemberGroups($toolid, $newgroups, $editversion = 'dev', $membergroups = array())
    {
        ...
        if (count($to_delete) > 0 && $editversion != 'current') {
            foreach ($to_delete as $del) {
                $query = "DELETE FROM $this->_tbl WHERE cn=" . $this->_db->quote($del)
                    . " AND toolid=" . $this->_db->quote($toolid) . " AND role=0";
                $this->_db->setQuery($query);
                $this->_db->query();
            }
        }
