...
class Sessionsv1r0 extends ApiController
{
    ...
    public function infoTask()
    {
        ...
        if ($version !== 0) {
            if ($version === null) {
                $sql = "SELECT 0 AS id, tv.toolname AS alias, tv.toolname, tv.title,
                    tv.description, tv.toolaccess as access, tv.mw, tv.instance, tv.revision,
                    tv.fulltxt as abstract, '0000-00-00 00:00:00' AS created, tv.toolid, tv.id
                    FROM `#__tool_version` as tv
                    WHERE tv.toolname='{$tool}'
                    AND tv.revision IS NULL";
            } else {
                $sql = "SELECT r.id, r.alias, tv.toolname, tv.title, tv.description,
                    tv.toolaccess as access, tv.mw, tv.instance, tv.revision,
                    r.fulltxt as abstract, r.created, tv.toolid, tv.id
            ...
            $sql = "SELECT 0 AS id, t.toolname AS alias, t.toolname, t.title, t.description,
                t.toolaccess as access, t.mw, CONCAT(t.toolname,'_dev') AS instance,
                'dev' AS revision, t.fulltxt as abstract, '0000-00-00 00:00:00' AS created,
                t.id AS toolid, 0 AS id
                FROM `#__tool` as t
        ...
        if (!isset($status['hostreq'])) {
            ...
            $hostreq = array_map(function ($item) {
                return trim($item);
            }, $hostreq);
        }
    ...
    public function screenshotTask()
    {
        ...
        if (!$sess) {
            if ($notFound) {
                $screenshot = dirname(dirname(__DIR__)) . DS . 'site' . DS . 'assets'
                    . DS . 'img' . DS . 'screenshot-notfound.png';
            } else {
        ...
        if (!$screenshot) {
            ...
            $home_directory = DS . 'webdav' . DS . 'home' . DS . strtolower($sess->username)
                . DS . 'data' . DS . 'sessions';
    ...
    public function invokeTask()
    {
        ...
        $command = "start user=" . $result->get('username') . " ip=" . $app->ip
        ...
        if (!isset($output->session)) {
            throw new Exception(
                Lang::txt('There was a issue while trying to start the tool session. Please try again later.'),
                500
            );
        }
    ...
    public function runTask()
    {
        ...
        $command = 'start user=' . $profile->get('username')
            . " ip={$app->ip} app={$app->name} version={$app->version}" . $encoded;
        $status  = \Components\Tools\Helpers\Utils::middleware($command, $output);
    ...
    public function statusTask()
    {
        ...
        $dir = DS . 'webdav' . DS . 'home' . DS . $profile->get('username')
            . DS . 'data' . DS . 'sessions' . DS . $session;
        ...
        if ($finished) {
            $count = count($parsed);
            $pattern = '/\[status\] output saved in [\-_0-9a-zA-Z\.\/]*\/(run[0-9]*\.xml)/';
            preg_match($pattern, $parsed[($count - 2)], $matches);
    ...
    public function fileshareTask()
    {
        ...
        $command = "/usr/bin/sudo /usr/bin/hzappstream --remote 128.46.19.124 fileshare add "
            . "$username $sessionid $public_ip $private_ip --ipsec-use-default-psk";
