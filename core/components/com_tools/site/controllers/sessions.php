...
class Sessions extends SiteController
{
    ...
    protected function _buildPathway($session = null)
    {
        ...
        if (isset($this->app) && $this->app->name) {
            ...
            Pathway::append(
                ...
                'index.php?option=' . $this->_option . '&task=' . $this->_task . '&app=' . $appname
    ...
    public function quotaexceededTask()
    {
        ...
        if ($this->config->get('access-manage-session')) {
            $this->view->allsessions = $ms->getRecords(
                User::get('username'),
                '',
                $this->config->get('access-manage-session')
            );
        }
    ...
    public function invokeTask()
    {
        ...
        if (!empty($params)) {
            ...
            while ($line !== false) {
                ...
                } elseif (!empty($line)) { // Fail if unrecognized non-empty parameter line
                    break;
                }
        ...
        // Log::debug(
        //     "mw::invoke " . $app->name . " by " . User::get('username')
        //     . " from " . $app->ip . " _getToolAccess " . $status2
        // );
        ...
        if ($this->config->get('zones')) {
            ...
            $this->database->setQuery(
                "SELECT zone_id FROM `#__tool_version_zone` WHERE tool_version_id="
                . $this->database->quote($tv->id)
            );
        ...
        $startCmd = "start user=" . User::get('username') . " ip=" . $app->ip
            . " app=" . $app->name . " version=" . $app->version . $toolparams;
        $status = $this->middleware($startCmd, $output);
        if ($this->getError()) {
        ...
        $url = 'index.php?option=' . $this->_option . '&controller=' . $this->_controller
            . '&app=' . $app->toolname . '&task=session&sess=' . $app->sess . '&return=' . $rtrn
            . (Request::getWord('viewer') ? '&viewer=' . Request::getWord('viewer') : '');

        // Log activity
        $sessionLink = '<a href="' . Route::url($url) . '">' . $app->caption . '</a>';
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt(
                    'COM_TOOLS_ACTIVITY_SESSION_CREATED',
                    $app->sess,
                    $sessionLink
                ),
                'details'     => array(
    ...
    public function reinvokeTask()
    {
        ...
        // $session = \Components\Tools\Models\Middleware\Session::getInstance(
        //     $id,
        //     $this->config->get('access-manage-session')
        // );
        ...
        $status = $this->middleware(
            "start user=" . User::get('username') . " ip=" . Request::ip()
            . " app=" . $session->app() . " version=" . $session->app('version') . $toolparams,
            $output
        );
        if ($this->getError()) {
            App::redirect(
        ...
        // $reinvoked = \Components\Tools\Models\Middleware\Session::getInstance(
        //     $new_id,
        //     $this->config->get('access-manage-session')
        // );
        $reinvoked = $middleware->session(
            $new_id,
            $this->config->get('access-manage-session')
        );
    ...
    public function unshareTask()
    {
        ...
        // Drop through and re-view the session...
        //$this->viewTask();
        $sessionUrl = 'index.php?option=' . $this->_option
            . '&controller=' . $this->_controller
            . '&app=' . $app
            . '&task=session&sess=' . $sess;
        App::redirect(
            Route::url($sessionUrl)
        );
    }
    ...
    public function viewTask()
    {
        ...
        // Ensure we found an active session
        if (!$row->sesstoken) {
            $errorMsg = Lang::txt('MW_ERROR_SESSION_NOT_FOUND') . ': ' . $app->sess
                . '. ' . Lang::txt('MW_SESSION_NOT_FOUND_EXPLANATION');
            App::abort(404, $errorMsg);
            return;
        }
        ...
        if ($this->_task) {
            ...
            $pathwayUrl = 'index.php?option=' . $this->_option
                . '&controller=' . $this->controller
                . '&app=' . $toolname
                . '&task=session&sess=' . $app->sess;
            Pathway::append(
    ...
    private function _getToolAccess($tool, $login = '')
    {
        ...
        if ($tisDev) {
            if ($indevgroup) {
                // Log::debug(
                //     "mw::_getToolAccess($tool,$login): "
                //     . "DEV TOOL ACCESS GRANTED (USER IN DEVELOPMENT GROUP)"
                // );
                return true;
            } elseif ($admin) {
                // Log::debug(
                //     "mw::_getToolAccess($tool,$login): "
                //     . "DEV TOOL ACCESS GRANTED (USER IN ADMIN GROUP)"
                // );
                return true;
            } else {
                Log::debug(
                    "mw::_getToolAccess($tool,$login): "
                    . "DEV TOOL ACCESS DENIED (USER NOT IN DEVELOPMENT OR ADMIN GROUPS)"
                );
            ...
            if ($tisGroupControlled) {
                if ($ingroup) {
                    // Log::debug(
                    //     "mw::_getToolAccess($tool,$login): "
                    //     . "PUBLISHED TOOL ACCESS GRANTED (USER IN ACCESS GROUP)"
                    // );
                    return true;
                } elseif ($admin) {
                    // Log::debug(
                    //     "mw::_getToolAccess($tool,$login): "
                    //     . "PUBLISHED TOOL ACCESS GRANTED (USER IN ADMIN GROUP)"
                    // );
                    return true;
                } else {
                    Log::debug(
                        "mw::_getToolAccess($tool,$login): "
                        . "PUBLISHED TOOL ACCESS DENIED (USER NOT IN ACCESS OR ADMIN GROUPS)"
                    );
                ...
                if (!$exportAllowed) {
                    Log::debug(
                        "mw::_getToolAccess($tool,$login): PUBLISHED TOOL ACCESS DENIED (EXPORT DENIED)"
                    );
                    return false;
                } elseif ($admin) {
                    // Log::debug(
                    ...
                    // Log::debug(
                    //     "mw::_getToolAccess($tool,$login): "
                    //     . "PUBLISHED TOOL ACCESS GRANTED (USER IN DEVELOPMENT GROUP)"
                    // );
                    return true;
                } else {
                    // Log::debug(
                    //     "mw::_getToolAccess($tool,$login): PUBLISHED TOOL ACCESS GRANTED"
                    // );
                    return true;
                }
            }
            ...
            Log::debug(
                "mw::_getToolAccess($tool,$login): UNPUBLISHED TOOL ACCESS DENIED (TOOL NOT PUBLISHED)"
            );
