...
class Hosts extends AdminController
{
    ...
    public function displayTask()
    {
        ...
        $filters['start'] = ($filters['limit'] != 0
            ? (floor($filters['start'] / $filters['limit']) * $filters['limit'])
            : 0);
    ...
    public function editTask($row = null)
    {
        ...
        if ($row->hostname != '') {
            ...
            $sql = "SELECT appname, count(*) as count from session where exechost="
                . $mwdb->quote($row->hostname) . " group by appname";
            $mwdb->setQuery($sql);
            $toolCounts = $mwdb->loadObjectList();

            //get status counts
            $sql = "SELECT status, count(*) as count from display where hostname="
                . $mwdb->quote($row->hostname) . " group by status";
            $mwdb->setQuery($sql);
