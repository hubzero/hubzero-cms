                    ...
                    <fieldset class="adminform">
                        ...
                        <div class="input-wrap">
                            ...
                            <select name="fields[type]" id="field-type">
                                <option value="local"<?php if ($this->row->get('type') == 'local') {
                                    echo ' selected="selected"';
                                                     } ?>><?php echo Lang::txt('COM_TOOLS_FIELD_TYPE_LOCAL'); ?></option>
                                <option value="remote"<?php if ($this->row->get('type') == 'remote') {
                                    echo ' selected="selected"';
                                                      } ?>><?php echo Lang::txt('COM_TOOLS_FIELD_TYPE_REMOTE'); ?></option>
                            </select>
                        </div>

                        <div class="input-wrap">
                            <?php echo Lang::txt('COM_TOOLS_FIELD_STATE'); ?>:<br />
                            <label for="field-state-up"><input class="option" type="radio" name="fields[state]" id="field-state-up" size="30" value="up"<?php if ($this->row->get('state') == 'up') {
                                echo ' checked="checked"';
                                                                                                                                                        } ?> /> <?php echo Lang::txt('COM_TOOLS_FIELD_STATE_UP'); ?></label>
                            <label for="field-state-down"><input class="option" type="radio" name="fields[state]" id="field-state-down" size="30" value="down"<?php if ($this->row->get('state') == 'down') {
                                echo ' checked="checked"';
                                                                                                                                                              } ?> /> <?php echo Lang::txt('COM_TOOLS_FIELD_STATE_DOWN'); ?></label>
                        </div>
                    </fieldset>
                    <fieldset class="adminform">
                        ...
                        <div class="input-wrap">
                            ...
                            <input type="checkbox" name="zoneparams[websocket_enable]" id="field-zone-params-websocket-enable" value="1"<?php if ($this->row->params->get('websocket_enable')) {
                                echo ' checked="checked"';
                                                                                                                                        } ?> />
                        </div>
                        <div class="input-wrap">
                            <label for="field-zone-params-websocket-server" class="websocket<?php if (!$websocketEnabled) {
                                echo ' opaque';
                                                                                            } ?>"><?php echo Lang::txt('COM_TOOLS_FIELD_ZONE_WEBSOCKET_PROXY_SERVER'); ?>:</label>
                            <input type="text" name="zoneparams[websocket_server]" class="websocket<?php if (!$websocketEnabled) {
                                echo ' opaque';
                                                                                                   } ?>" id="field-zone-params-websocket-server" maxlength="255" value="<?php echo $this->escape(stripslashes($this->row->params->get('websocket_server', 'ws://' . $hn . ':8080'))); ?>"<?php if (!$websocketEnabled) {
                                                                                                   echo ' disabled="disabled"';
                                                                                                   } ?> />
                        </div>
                        <div class="input-wrap">
                            <label for="field-zone-params-websocket-secure-server" class="websocket<?php if (!$websocketEnabled) {
                                echo ' opaque';
                                                                                                   } ?>"><?php echo Lang::txt('COM_TOOLS_FIELD_ZONE_WEBSOCKET_PROXY_SECURE_SERVER'); ?>:</label>
                            <input type="text" name="zoneparams[websocket_secure_server]" class="websocket<?php if (!$websocketEnabled) {
                                echo ' opaque';
                                                                                                          } ?>" id="field-zone-params-websocket-secure-server" maxlength="255" value="<?php echo $this->escape(stripslashes($this->row->params->get('websocket_secure_server', 'wss://' . $hn . ':8443'))); ?>"<?php if (!$websocketEnabled) {
                                                                                                          echo ' disabled="disabled"';
                                                                                                          } ?> />
                        </div>
                        <div class="input-wrap">
                            ...
                            <input type="checkbox" name="zoneparams[vnc_enable]" id="field-zone-params-vnc-enable"  value="1"<?php if ($this->row->params->get('vnc_enable')) {
                                echo ' checked="checked"';
                                                                                                                             } ?> />
                        </div>
                        <div class="input-wrap">
                            <label for="field-zone-params-vnc-server" class="vnc<?php if (!$vncEnabled) {
                                echo ' opaque';
                                                                                } ?>"><?php echo Lang::txt('COM_TOOLS_FIELD_ZONE_VNC_PROXY_SERVER'); ?>:</label>
                            <input type="text" name="zoneparams[vnc_server]" class="vnc<?php if (!$vncEnabled) {
                                echo ' opaque';
                                                                                       } ?>" id="field-zone-params-vnc-server" maxlength="255" value="<?php echo $this->escape(stripslashes($this->row->params->get('vnc_server', 'http://' . $hn . ':80'))); ?>"<?php if (!$vncEnabled) {
                                                                                       echo ' disabled="disabled"';
                                                                                       } ?> />
                        </div>
                        <div class="input-wrap">
                            <label for="field-zone-params-vnc-secure-server" class="vnc<?php if (!$vncEnabled) {
                                echo ' opaque';
                                                                                       } ?>"><?php echo Lang::txt('COM_TOOLS_FIELD_ZONE_VNC_PROXY_SECURE_SERVER'); ?>:</label>
                            <input type="text" name="zoneparams[vnc_secure_server]" class="vnc<?php if (!$vncEnabled) {
                                echo ' opaque';
                                                                                              } ?>" id="field-zone-params-vnc-secure-server" maxlength="255" value="<?php echo $this->escape(stripslashes($this->row->params->get('vnc_secure_server', 'https://' . $hn . ':80'))); ?>"<?php if (!$vncEnabled) {
                                                                                              echo ' disabled="disabled"';
                                                                                              } ?> />
                        </div>
                    </fieldset>
