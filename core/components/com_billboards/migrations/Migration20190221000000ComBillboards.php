...
class Migration20190221000000ComBillboards extends Base
{
    ...
    {
        if (
            $this->db->tableExists('#__billboards_billboards')
            && $this->db->tableHasField('#__billboards_billboards', 'checked_out_time')
        ) {
            $query = "ALTER TABLE `#__billboards_billboards` CHANGE `checked_out_time`"
                . " `checked_out_time` DATETIME  NULL  DEFAULT NULL";

            $this->db->setQuery($query);
            $this->db->query();

            $query = "UPDATE `#__billboards_billboards` SET `checked_out_time`=NULL"
