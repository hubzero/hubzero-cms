...
class BillBoards extends AdminController
{
    ...
    public function saveTask()
    {
        ...
        if (isset($billboard_image['name']) && $billboard_image['name']) {
            ...
            $defaultLocation = 'app' . DS . 'site' . DS . 'media' . DS . 'images' . DS . 'billboards';
            $image_location  = $this->config->get('image_location', $defaultLocation);
            ...
            // Scan for viruses
            if (!\Hubzero\Facades\Filesystem::isSafe($billboard_image['tmp_name'])) {
            ...
            if (!move_uploaded_file($billboard_image['tmp_name'], $uploadDirectory . $billboard_image['name'])) {
                ...
                if ($old = $billboard->get('background_img')) {
                    if (file_exists($uploadDirectory . $old)) {
                        \Hubzero\Facades\Filesystem::delete($uploadDirectory . $old);
                    }
                }
