...
class Pages extends SiteController
{
    ...
    public function execute()
    {
        if (!$this->book->pages()->total()) {
            ...
            // App::get('config')->get('debug') || App::get('config')->get('profile')
            //     ? App::get('profiler')->mark('afterWikiSetup') : null;
        }

        $this->page = $this->book->page();

        if (in_array($this->page->getNamespace(), array('image', 'file'))) {
            $url = 'index.php?option=' . $this->_option . '&controller=media&scope='
                . $this->page->get('scope') . '&pagename=' . $this->page->get('pagename')
                . '&task=download';
    ...
    public function displayTask()
    {
        ...
        // App::get('config')->get('debug') || App::get('config')->get('profile')
        //     ? App::get('profiler')->mark('afterWikiParse') : null;
    ...
    public function editTask($revision = null)
    {
        ...
        if (
            (!$this->page->access('edit') || ($this->page->created_by && $this->page->created_by != User::get('id')))
        ...
        if (!$this->page->exists()) {
            ...
            if ($ischild && $this->page->get('pagename')) {
                ...
                $newPath = $this->page->get('path')
                    . ($this->page->get('path') ? '/' : '')
                    . $this->page->get('pagename');
                $this->page->set('path', $newPath);
        ...
        if ($items) {
            foreach ($items as $k => $branch) {
                ...
                $fullPagename = ($branch->get('path') ? $branch->get('path') . '/' : '')
                    . $branch->get('pagename');
                $branch->set('pagename', $fullPagename);
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt(
                    'COM_WIKI_ACTIVITY_PAGE_' . ($page['id'] ? 'UPDATED' : 'CREATED'),
                    '<a href="' . Route::url($this->page->link()) . '">'
                        . $this->page->title . '</a>'
                ),
                'details'     => array(
    ...
    public function deleteTask()
    {
        ...
        if (
            (!$this->page->access('delete') || ($this->page->created_by && $this->page->created_by != User::get('id')))
            && !$this->page->access('manage')
        ) {
            App::redirect(
                Route::url($this->page->link()),
            ...
            case 1:
                ...
                Event::trigger('system.logActivity', [
                    'activity' => [
                        ...
                        'scope_id'    => $this->page->get('id'),
                        'description' => Lang::txt(
                            'COM_WIKI_ACTIVITY_PAGE_DELETED',
                            '<a href="' . Route::url($this->page->link()) . '">'
                                . $this->page->title . '</a>'
                        ),
                        'details'     => array(
    ...
    public function saverenameTask()
    {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope_id'    => $this->page->get('id'),
                'description' => Lang::txt(
                    'COM_WIKI_ACTIVITY_PAGE_RENAMED',
                    '<a href="' . Route::url($this->page->link()) . '">'
                        . $this->page->get('title') . '</a>'
                ),
                'details'     => array(
    ...
    public function pdfTask()
    {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope_id'    => $this->page->get('id'),
                'description' => Lang::txt(
                    'COM_WIKI_ACTIVITY_PAGE_DOWNLOADED',
                    '<a href="' . Route::url($this->page->link()) . '">'
                        . $this->page->title . '</a>'
                ),
                'details'     => array(
