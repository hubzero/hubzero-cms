...
Toolbar::title(Lang::txt('COM_STORY_TOPICS'), 'story.png');
...
<form action="<?php echo Route::url('index.php?option=' . $this->option . '&controller=topics'); ?>" method="post" name="adminForm" id="adminForm">
...
<?php if ($canDo->get('core.edit')) { ?>
					<a href="<?php echo Route::url('index.php?option=' . $this->option . '&controller=topics&task=edit&id[]=' . $row->get('id')); ?>"><?php echo $this->escape($row->get('title')); ?></a>
<?php } else { ?>
...
<?php } ?>
				...
				<td>
					<a class="state <?php echo ($row->get('state') ? 'publish' : 'unpublish'); ?>" href="<?php echo Route::url('index.php?option=' . $this->option . '&controller=topics&task=' . ($row->get('state') ? 'unpublish' : 'publish') . '&id[]=' . $row->get('id') . '&' . Session::getFormToken() . '=1'); ?>">
