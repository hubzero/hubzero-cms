    ...
    public static function getRacialIdentification($resp_id)
    {
        ...
        if (is_array($resp_id)) {
            $dbh->setQuery(
                ...
                . 'coalesce(concat(\'(\', tribal_affiliation, \')\'), \'\')) separator \', \') '
                . 'AS identification FROM #__events_respondent_race_rel '
                . 'WHERE respondent_id IN (' . implode(', ', array_map('intval', $resp_id)) . ') '
                . 'GROUP BY respondent_id'
            );
            return $dbh->loadAssocList('respondent_id');
        } else {
            $dbh->setQuery(
                'SELECT group_concat(concat(race, '
                . 'coalesce(concat(\'(\', tribal_affiliation, \')\'), \'\')) separator \', \') '
                . 'FROM #__events_respondent_race_rel '
                . 'WHERE respondent_id = ' . intval($resp_id) . ' GROUP BY respondent_id'
            );
    ...
    public function __construct($filters)
    {
        ...
        if (array_key_exists('sortby', $filters)) {
            if (preg_match('/(registered|name|special|id)(?:\ (ASC|DESC))?/', $filters['sortby'], $match)) {
                ...
                } elseif ($match[1] == 'special') {
                    $this->order_desc = 'special ' . $match[2];
                    $this->order = ' ORDER BY CASE WHEN disability_needs OR dietary_needs IS NOT null'
                        . ' THEN 1 WHEN comment IS NOT null THEN 2 ELSE 3 END ' . $match[2];
                } else {
