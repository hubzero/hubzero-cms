...
class Event extends Table
{
        ...
        switch ($period) {
            case 'month':
                ...
                $sql = "SELECT $this->_tbl.*
					...
					AND (
					(
							(publish_up >= " . $this->_db->quote($select_date . '%') . "
								AND publish_up <= " . $this->_db->quote($select_date_fin . '%') . "
								AND publish_down IS NOT NULL
								AND publish_down != '0000-00-00 00:00:00')
						OR (publish_down IS NOT NULL
								AND publish_down !='0000-00-00 00:00:00'
								AND publish_down >= " . $this->_db->quote($select_date . '%') . "
								AND publish_down <= " . $this->_db->quote($select_date_fin . '%') . ")
						OR (publish_up >= " . $this->_db->quote($select_date . '%') . "
								AND publish_down IS NOT NULL
								AND publish_down !='0000-00-00 00:00:00'
								AND publish_down <= " . $this->_db->quote($select_date_fin . '%') . ")
						OR (publish_up <= " . $this->_db->quote($select_date . '%') . "
                ...
                if (isset($filters['scope']) && $filters['scope'] != '') {
                    if ($filters['scope'] == 'event') {
                        $sql .= " AND ({$this->_tbl}.scope IS NULL OR {$this->_tbl}.scope="
                            . $this->_db->quote($filters['scope']) . ")";
                    } else {
            ...
            case 'year':
                ...
                $sql = "SELECT $this->_tbl.* FROM #__categories AS b, $this->_tbl
					WHERE $this->_tbl.catid = b.id
					AND publish_up LIKE " . $this->_db->quote($year . '%') . "
					AND (publish_down >= " . $this->_db->quote($year . '%') . "
						OR publish_down = '0000-00-00 00:00:00' OR publish_down IS NULL)
					AND $this->_tbl.state = '1'";
            ...
            case 'week':
                ...
                $sql = "SELECT * FROM $this->_tbl
					WHERE ((publish_up >= " . $this->_db->quote($startdate . '%') . "
							AND publish_up <= " . $this->_db->quote($enddate . '%') . ")
						OR (publish_down >= " . $this->_db->quote($startdate . '%') . "
							AND publish_down <= " . $this->_db->quote($enddate . '%') . ")
						OR (publish_up >= " . $this->_db->quote($startdate . '%') . "
							AND publish_down <= " . $this->_db->quote($enddate . '%') . ")
						OR (publish_down >= " . $this->_db->quote($enddate . '%') . "
							AND publish_up <= " . $this->_db->quote($startdate . '%') . "))
					AND state = '1'";
            ...
            case 'day':
                ...
                $sql = "SELECT $this->_tbl.* FROM #__categories AS b, $this->_tbl
					WHERE $this->_tbl.catid = b.id AND
						((publish_up >= " . $this->_db->quote($select_date) . "
							AND publish_up <= " . $this->_db->quote($select_date_fin) . ")
						OR (publish_down >= " . $this->_db->quote($select_date) . "
							AND publish_down <= " . $this->_db->quote($select_date_fin) . ")
						OR (publish_up <= " . $this->_db->quote($select_date) . "
							AND publish_down >= " . $this->_db->quote($select_date_fin) . ")
						OR (publish_up >= " . $this->_db->quote($select_date) . "
    ...
    private function buildQuery($filters = array())
    {
        ...
        if (isset($filters['publish_up']) && isset($filters['publish_down'])) {
            ...
            $q .= " OR (publish_down IS NOT NULL AND publish_down != '0000-00-00 00:00:00'"
                . " AND publish_down <=" . $this->_db->quote($filters['publish_down']) . ")";
            $q .= " OR (publish_up IS NOT NULL"
                . " AND publish_up <= " . $this->_db->quote($filters['publish_up'])
                . " AND publish_down >=" . $this->_db->quote($filters['publish_down']) . "))";
            $where[] = $q;
        }
