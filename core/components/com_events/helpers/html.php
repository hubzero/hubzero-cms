...
class Html
{
    ...
    public static function buildRadioOption($arr, $tag_name, $tag_attribs, $key, $text, $selected)
    {
        ...
        for ($i = 0, $n = count($arr); $i < $n; $i++) {
            ...
            $html .= '<label class="option"><input class="option" name="' . $tag_name . '" id="'
                . $tag_name . $i . '" type="radio" value="' . $k . '"' . $sel . ' ' . $tag_attribs
                . '/>' . $t . '</label>' . "\n";
        }
        return $html;
    }
    ...
    public static function buildWeekDaysCheck($reccurweekdays, $args)
    {
        ...
        for ($a = 0; $a < 7; $a++) {
            ...
            $tosend .= '<label class="option"><input type="checkbox" id="cb_wd' . $a
                . '" name="reccurweekdays" value="' . $a . '" ' . $args . ' ' . $checked
                . '/>&nbsp;' . $day_name[$a] . '</label>' . "\n";
        }
        return $tosend;
    }
    ...
    public static function buildWeeksCheck($reccurweeks, $args)
    {
        ...
        for ($a = 1; $a < 6; $a++) {
            ...
            $tosend .= '<label class="option"><input class="option" type="checkbox" id="cb_wn' . $a
                . '" name="reccurweeks" value="' . $a . '" ' . $args . ' ' . $checked
                . '/>&nbsp;' . $week_name[$a] . '</label><br />' . "\n";
        }
        return $tosend;
    }
    ...
    public static function getMonthName($month)
    {
        ...
        switch ($month) {
            case '01':
                $monthname = Lang::txt('EVENTS_CAL_LANG_JANUARY');
                break;
            case '02':
                $monthname = Lang::txt('EVENTS_CAL_LANG_FEBRUARY');
                break;
            case '03':
                $monthname = Lang::txt('EVENTS_CAL_LANG_MARCH');
                break;
            case '04':
                $monthname = Lang::txt('EVENTS_CAL_LANG_APRIL');
                break;
            case '05':
                $monthname = Lang::txt('EVENTS_CAL_LANG_MAY');
                break;
            case '06':
                $monthname = Lang::txt('EVENTS_CAL_LANG_JUNE');
                break;
            case '07':
                $monthname = Lang::txt('EVENTS_CAL_LANG_JULY');
                break;
            case '08':
                $monthname = Lang::txt('EVENTS_CAL_LANG_AUGUST');
                break;
            case '09':
                $monthname = Lang::txt('EVENTS_CAL_LANG_SEPTEMBER');
                break;
            case '10':
                $monthname = Lang::txt('EVENTS_CAL_LANG_OCTOBER');
                break;
            case '11':
                $monthname = Lang::txt('EVENTS_CAL_LANG_NOVEMBER');
                break;
            case '12':
                $monthname = Lang::txt('EVENTS_CAL_LANG_DECEMBER');
                break;
        }
        return $monthname;
    }
    ...
    public static function getLongDayName($daynb)
    {
        ...
        switch ($daynb) {
            case '0':
                $dayname = Lang::txt('EVENTS_CAL_LANG_SUNDAY');
                break;
            case '1':
                $dayname = Lang::txt('EVENTS_CAL_LANG_MONDAY');
                break;
            case '2':
                $dayname = Lang::txt('EVENTS_CAL_LANG_TUESDAY');
                break;
            case '3':
                $dayname = Lang::txt('EVENTS_CAL_LANG_WEDNESDAY');
                break;
            case '4':
                $dayname = Lang::txt('EVENTS_CAL_LANG_THURSDAY');
                break;
            case '5':
                $dayname = Lang::txt('EVENTS_CAL_LANG_FRIDAY');
                break;
            case '6':
                $dayname = Lang::txt('EVENTS_CAL_LANG_SATURDAY');
                break;
        }
        return $dayname;
    }
        ...
        switch ($type) {
            case '0':
                if ($format_type == 0) {
                    ...
                    $newdate = self::getLongDayName($numero_jour) . '&nbsp;' . $jour_fr
                        . '&nbsp;' . self::getMonthName($mois_0) . '&nbsp;' . $annee;
                } elseif ($format_type == 1) {
                    // Us style : Monday, July 23 2003
                    $newdate = self::getLongDayName($numero_jour) . ',&nbsp;'
                        . self::getMonthName($mois_0) . '&nbsp;' . $jour_fr . '&nbsp;' . $annee;
                } else {
