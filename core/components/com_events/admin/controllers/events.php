...
class Events extends AdminController
{
    ...
    public function displayTask()
    {
        ...
        $this->database->setQuery(
            "SELECT id AS value, title AS text FROM `#__categories` WHERE extension='$this->_option'"
        );
    ...
    public function editTask()
    {
        ...
        if (!empty($this->view->fields)) {
            for ($i = 0, $n = count($this->view->fields); $i < $n; $i++) {
                ...
                array_push(
                    $this->view->fields[$i],
                    $this->parseTag($this->view->row->content, $this->view->fields[$i][0])
                );
                // clean the original text of any matches
                $this->view->row->content = str_replace(
                    '<ef:' . $this->view->fields[$i][0] . '>' . end($this->view->fields[$i])
                        . '</ef:' . $this->view->fields[$i][0] . '>',
                    '',
                    $this->view->row->content
                );
            }
    ...
    public function saveTask()
    {
        ...
        if ($row->extra_info != null) {
            if (
                (substr($row->extra_info, 0, 7) != 'http://')
