<?php

/**
 * @package    hubzero-cms
 * @copyright  Copyright (c) 2005-2020 The Regents of the University of California.
 * @license    http://opensource.org/licenses/MIT MIT
 */

// No direct access
defined('_HZEXEC_') or die();

Toolbar::title(Lang::txt('COM_EVENTS_MANAGER') . ': ' . Lang::txt('COM_EVENTS_CONFIGURATION'), 'event');
Toolbar::save();
Toolbar::cancel();
?>

<form action="<?php echo Route::url('index.php?option=' . $this->option); ?>" method="post" name="adminForm">
    <fieldset class="adminform">
        <legend><span><?php echo Lang::txt('COM_EVENTS_CAL_LANG_CONFIG'); ?></span></legend>

        <table class="admintable">
            <tbody>
                <tr>
                    <th scope="row" class="key"><?php echo Lang::txt('COM_EVENTS_CAL_LANG_CONFIG_ADMINMAIL'); ?></th>
                    <td>
                        <input type="text"
                            name="config[adminmail]"
                            size="30"
                            maxlength="50"
                            value="<?php echo $this->config->adminmail; ?>"
                        />
                    </td>
                </tr>
                <tr>
                    <th scope="row" class="key"><?php echo Lang::txt('COM_EVENTS_CAL_LANG_CONFIG_FIRSTDAY'); ?></th>
                    <td><?php
                    $sundayTxt = Lang::txt('COM_EVENTS_CAL_LANG_CONFIG_SUNDAY_FIRST');
                    $mondayTxt = Lang::txt('COM_EVENTS_CAL_LANG_CONFIG_MONDAY_FIRST');
                    $first[] = Html::select('option', '0', $sundayTxt, 'value', 'text');
                    $first[] = Html::select('option', '1', $mondayTxt, 'value', 'text');
                    echo Html::select(
                        'genericlist',
                        $first,
                        'config[starday]',
                        '',
                        'value',
                        'text',
                        $this->config->starday,
                        false,
                        false
                    );
                    ?></td>
                </tr>
                <tr>
                    <th scope="row" class="key"><?php echo Lang::txt('COM_EVENTS_CAL_LANG_CONFIG_VIEWMAIL'); ?></th>
                    <td><?php
                    $viewm[] = Html::select('option', 'YES', Lang::txt('JYES'), 'value', 'text');
                    $viewm[] = Html::select('option', 'NO', Lang::txt('JNO'), 'value', 'text');
                    echo Html::select(
                        'genericlist',
                        $viewm,
                        'config[mailview]',
                        '',
                        'value',
                        'text',
                        $this->config->mailview,
                        false,
                        false
                    );
                    ?></td>
                </tr>
                <tr>
                    <th scope="row" class="key"><?php echo Lang::txt('COM_EVENTS_CAL_LANG_CONFIG_VIEWBY'); ?></th>
                    <td><?php
                    $viewb[] = Html::select('option', 'YES', Lang::txt('YES'), 'value', 'text');
                    $viewb[] = Html::select('option', 'NO', Lang::txt('JNO'), 'value', 'text');
                    echo Html::select(
                        'genericlist',
                        $viewb,
                        'config[byview]',
                        '',
                        'value',
                        'text',
                        $this->config->byview,
                        false,
                        false
                    );
                    ?></td>
                </tr>
                <tr>
                    <th scope="row" class="key"><?php echo Lang::txt('COM_EVENTS_CAL_LANG_CONFIG_VIEWHITS'); ?></th>
                    <td><?php
                    $viewh[] = Html::select('option', 'YES', Lang::txt('YES'), 'value', 'text');
                    $viewh[] = Html::select('option', 'NO', Lang::txt('JNO'), 'value', 'text');
                    echo Html::select(
                        'genericlist',
                        $viewh,
                        'config[hitsview]',
                        '',
                        'value',
                        'text',
                        $this->config->hitsview,
                        false,
                        false
                    );
                    ?></td>
                </tr>
                <tr>
                    <th scope="row" class="key"><?php echo Lang::txt('COM_EVENTS_CAL_LANG_CONFIG_DATEFORMAT'); ?></th>
                    <td><?php
                    $frenchTxt = Lang::txt('COM_EVENTS_CAL_LANG_CONFIG_FRENCH_ENGLISH');
                    $deutschTxt = Lang::txt('COM_EVENTS_CAL_LANG_CONFIG_DEUTSCH');
                    $datef[] = Html::select('option', '0', $frenchTxt, 'value', 'text');
                    $datef[] = Html::select('option', '1', Lang::txt('COM_EVENTS_CAL_LANG_CONFIG_US'), 'value', 'text');
                    $datef[] = Html::select('option', '2', $deutschTxt, 'value', 'text');
                    echo Html::select(
                        'genericlist',
                        $datef,
                        'config[dateformat]',
                        '',
                        'value',
                        'text',
                        $this->config->dateformat,
                        false,
                        false
                    );
                    ?></td>
                </tr>
                <tr>
                    <th scope="row" class="key"><?php echo Lang::txt('COM_EVENTS_CAL_LANG_CONFIG_TIMEFORMAT'); ?></th>
                    <td><?php
                    $stdTime[] = Html::select('option', 'YES', Lang::txt('YES'), 'value', 'text');
                    $stdTime[] = Html::select('option', 'NO', Lang::txt('JNO'), 'value', 'text');
                    echo Html::select(
                        'genericlist',
                        $stdTime,
                        'config[calUseStdTime]',
                        '',
                        'value',
                        'text',
                        $this->config->calUseStdTime,
                        false,
                        false
                    );
                    ?></td>
                </tr>
                <tr>
                    <th scope="row" class="key"><?php echo Lang::txt('COM_EVENTS_CAL_LANG_CONFIG_STARTPAGE'); ?></th>
                    <td><?php
                    $dayTxt = Lang::txt('COM_EVENTS_CAL_LANG_REP_DAY');
                    $weekTxt = Lang::txt('COM_EVENTS_CAL_LANG_REP_WEEK');
                    $monthTxt = Lang::txt('COM_EVENTS_CAL_LANG_REP_MONTH');
                    $yearTxt = Lang::txt('COM_EVENTS_CAL_LANG_REP_YEAR');
                    $catTxt = Lang::txt('COM_EVENTS_CAL_LANG_EVENT_CATEGORIES');
                    $startpg[] = Html::select('option', 'day', $dayTxt, 'value', 'text');
                    $startpg[] = Html::select('option', 'week', $weekTxt, 'value', 'text');
                    $startpg[] = Html::select('option', 'month', $monthTxt, 'value', 'text');
                    $startpg[] = Html::select('option', 'year', $yearTxt, 'value', 'text');
                    $startpg[] = Html::select('option', 'categories', $catTxt, 'value', 'text');
                    echo Html::select(
                        'genericlist',
                        $startpg,
                        'config[startview]',
                        '',
                        'value',
                        'text',
                        $this->config->startview,
                        false,
                        false
                    );
                    ?></td>
                </tr>
                <tr>
                    <th scope="row" class="key"><?php echo Lang::txt('COM_EVENTS_CAL_LANG_CONFIG_NUMEVENTS'); ?></th>
                    <td>
                        <input type="text"
                            size="3"
                            name="config[calEventListRowsPpg]"
                            value="<?php echo $this->config->calEventListRowsPpg; ?>"
                        />
                    </td>
                </tr>
            </tbody>
        </table>
    </fieldset>
    <fieldset class="adminform">
        <legend><span><?php echo Lang::txt('COM_EVENTS_CAL_LANG_CUSTOM_FIELDS'); ?></span></legend>

        <table class="admintable">
            <thead>
                <tr>
                    <th scope="col"><?php echo Lang::txt('COM_EVENTS_CAL_LANG_FIELD'); ?></th>
                    <th scope="col"><?php echo Lang::txt('COM_EVENTS_CAL_LANG_TYPE'); ?></th>
                    <th scope="col"><?php echo Lang::txt('COM_EVENTS_CAL_LANG_REQUIRED'); ?></th>
                    <th scope="col"><?php echo Lang::txt('COM_EVENTS_CAL_LANG_SHOW'); ?></th>
                </tr>
            </thead>
            <tbody>
            <?php
            $fields = $this->config->fields;
            $r = count($fields);
            if ($r > 10) {
                $n = $r;
            } else {
                $n = 10;
            }
            for ($i = 0; $i < $n; $i++) {
                if ($r == 0 || !isset($fields[$i])) {
                    $fields[$i] = array();
                    $fields[$i][0] = null;
                    $fields[$i][1] = null;
                    $fields[$i][2] = null;
                    $fields[$i][3] = null;
                    $fields[$i][4] = null;
                }
                ?>
                <tr>
                    <td>
                        <input type="text"
                            name="fields[<?php echo $i; ?>][title]"
                            value="<?php echo $fields[$i][1]; ?>"
                            maxlength="255"
                        />
                    </td>
                    <td><select name="fields[<?php echo $i; ?>][type]">
                        <?php
                        $textSel = ($fields[$i][2] == 'text') ? ' selected="selected"' : '';
                        $cbSel = ($fields[$i][2] == 'checkbox') ? ' selected="selected"' : '';
                        $textTxt = Lang::txt('COM_EVENTS_CAL_LANG_TEXT');
                        $cbTxt = Lang::txt('COM_EVENTS_CAL_LANG_CHECKBOX');
                        ?>
                        <option value="text"<?php echo $textSel; ?>><?php echo $textTxt; ?></option>
                        <option value="checkbox"<?php echo $cbSel; ?>><?php echo $cbTxt; ?></option>
                    </select></td>
                    <?php $reqChk = ($fields[$i][3]) ? ' checked="checked"' : ''; ?>
                    <?php $showChk = ($fields[$i][4]) ? ' checked="checked"' : ''; ?>
                    <td>
                        <input type="checkbox"
                            name="fields[<?php echo $i; ?>][required]"
                            value="1"<?php echo $reqChk; ?>
                        />
                    </td>
                    <td>
                        <input type="checkbox"
                            name="fields[<?php echo $i; ?>][show]"
                            value="1"<?php echo $showChk; ?>
                        />
                    </td>
                </tr>
                <?php
            }
            ?>
            </tbody>
        </table>
    </fieldset>

    <input type="hidden" name="task" value="save" />
    <input type="hidden" name="option" value="<?php echo $this->option; ?>" />
    <input type="hidden" name="controller" value="<?php echo $this->controller; ?>" />

    <?php echo Html::input('token'); ?>
</form>
