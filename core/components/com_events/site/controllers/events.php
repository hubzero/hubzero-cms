...
class Events extends SiteController
{
    ...
    public function monthTask()
    {
        ...
        $select_date_fin = $year . '-' . $month . '-'
            . date("t", mktime(0, 0, 0, ($month + 1), 0, (int) $year)) . ' 23:59:59';
    ...
    public function weekTask()
    {
        ...
        $sdt = Date::of(
            $this_date->year . '-' . $this_date->month . '-' . $this_date->day . ' 00:00:00'
        )->toLocal($this->dateFormatShort);
        $edt = Date::of(
            $this_enddate->year . '-' . $this_enddate->month . '-' . $this_enddate->day . ' 00:00:00'
        )->toLocal($this->dateFormatShort);
    ...
    public function detailsTask()
    {
            ...
            if (is_object($group)) {
                ...
                $redirect = Route::url(
                    'index.php?option=com_groups&cn=' . $group->get('cn')
                        . '&active=calendar&action=details&event_id=' . $row->id,
                    false
                );
        ...
        // Adresse
        $row->adresse_info = preg_replace(
            "/(mailto:\/\/)?((-|$alphadigit|\.)+)@((-|$alphadigit|\.)+)(\.$alphadigit+)/i",
            "<a href=\"mailto:$2@$5$8\">$2@$5$8</a>",
            $row->adresse_info
        );
        $row->adresse_info = preg_replace(
            "/(http:\/\/|https:\/\/)((-|$alphadigit|\.)+)(\.$alphadigit+)/i",
            "<a href=\"$1$2$5$8\">$1$2$5$8</a>",
            $row->adresse_info == null ? '' : $row->adresse_info
        );

        // Contact
        $row->contact_info = stripslashes(strip_tags($row->contact_info));
        if (substr($row->contact_info, 0, strlen('mailto:')) == 'mailto:') {
            $row->contact_info = '<a href="mailto:'
                . $this->obfuscate(substr($row->contact_info, strlen('mailto:'))) . '">'
                . $this->obfuscate(substr($row->contact_info, strlen('mailto:'))) . '</a>';
        } elseif (preg_match("/^[_\.\%0-9a-zA-Z-]+@([0-9a-zA-Z-]+\.)+[a-zA-Z]{2,6}$/i", $row->contact_info)) {
            $row->contact_info = '<a href="mailto:' . $this->obfuscate($row->contact_info) . '">'
                . $this->obfuscate($row->contact_info) . '</a>';
        }
        //$row->contact_info = preg_replace(
        //    "/(mailto:\/\/)?((-|$alphadigit|\.)+)@((-|$alphadigit|\.)+)(\.$alphadigit+)/i",
        //    "<a href=\"mailto:$2@$5$8\">$2@$5$8</a>",
        //    $row->contact_info
        //);
        ...
        if (!empty($fields)) {
            for ($i = 0, $n = count($fields); $i < $n; $i++) {
                ...
                array_push(
                    $fields[$i],
                    self::parseTag($row->content == null ? '' : $row->content, $fields[$i][0])
                );
    ...
    public function eventregisterTask()
    {
        ...
        // Set the page title
        Document::setTitle(
            Lang::txt(strtoupper($this->_name)) . ': ' . Lang::txt('EVENTS_REGISTER')
    ...
    public function processTask()
    {
        ...
        if ($register) {
            ...
            $validEmail = $this->validEmail($register['email']);
        }
        if ($arrival) {
        ...
        if ($register['firstname'] && $register['lastname'] && $validEmail) {
            ...
            if (date('Y-m-d', strtotime($start)) === date('Y-m-d', strtotime($end))) {
                ...
                if (empty($tz)) {
                    $tz = \Hubzero\Facades\Config::get('offset');
                }
            ...
            if ($email != "" || $email != null || !isset($email)) {
                ...
                $this->sendEmail($hub, $email, $subject, $message);
            }

            // one for the attendee
            $this->sendEmail($hub, $register['email'], $subject, $message);
    ...
    private function log($reg)
    {
        $this->database->setQuery(
			...
			VALUES (' .
                $this->event->id . ', ' .
                $this->getValueString($this->database, $reg, array(
                    'firstname', 'lastname', 'affiliation', 'title', 'city', 'state',
                    'postalcode', 'country', 'telephone', 'fax', 'email', 'website',
                    'position', 'degree', 'gender', 'arrival', 'departure', 'disability',
        ...
        if (!is_null($races) && (!isset($races['refused']) || !$races['refused'])) {
            ...
            foreach (array('nativeamerican', 'asian', 'black', 'hawaiian', 'white', 'hispanic') as $race) {
                if (array_key_exists($race, $races) && $races[$race]) {
                    $tribalVal = $race == 'nativeamerican'
                        ? $this->database->quote($races['nativetribe'])
                        : 'null';
                    $this->database->execute(
                        'INSERT INTO `#__events_respondent_race_rel`(respondent_id, race, tribal_affiliation)
						VALUES (' . $resp_id . ', \'' . $race . '\', ' . $tribalVal . ')'
                    );
                }
            }
        }
     ...
     */
    private function getValueString($database, $reg, $values)
    {
        $rv = array();
        foreach ($values as $val) {
            switch ($val) {
                case 'position':
                    $hasPosition = (isset($reg['position']) || isset($reg['position_other']))
                        && ($reg['position'] || $reg['position_other']);
                    $rv[] = $hasPosition
                        ? $this->database->quote(
                ...
                case 'disability':
                    ...
                    break;
                default:
                    $rv[] = array_key_exists($val, $reg) && isset($reg[$val])
                        ? $this->database->quote($reg[$val])
                        : 'null';
                    break;
            }
        }
    ...
    public function editTask($row = null)
    {
        ...
        if ($row->id) {
            ...
            if (
                !$this->_authorize($row->created_by)
            ...
            $registerby_date = Date::of(
                $row->registerby == null ? '' : $row->registerby,
                $time_zone
            )->format('Y-m-d', true);
            $registerby_time = Date::of(
                $row->registerby == null ? '' : $row->registerby,
                $time_zone
            )->format('H:i', true);

            $arr = array(
                \Hubzero\Facades\Html::select('option', 0, strtolower(Lang::txt('EVENTS_NO')), 'value', 'text'),
                \Hubzero\Facades\Html::select('option', 1, strtolower(Lang::txt('EVENTS_YES')), 'value', 'text'),
            );

            $lists['state'] = \Hubzero\Facades\Html::select(
                'genericlist',
                $arr,
                'state',
                '',
            ...
            if ($row->publish_up && $row->publish_up != '0000-00-00 00:00:00') {
                ...
                $registerby_date = sprintf(
                    "%4d-%02d-%02d",
                    $event_registerby->year,
                    $event_registerby->month,
                    $event_registerby->day
                );
    ...
    public function deleteTask()
    {
        ...
        if (
            !$this->_authorize($event->created_by)
        ...
        */
        ...
        // E-mail subject line
        $subject  = '[' . Config::get('sitename') . ' ' . Lang::txt('EVENTS') . '] - '
            . Lang::txt('EVENTS_EVENT_DELETED');
        ...
        // Send the e-mail
        $this->sendMail(Config::get('sitename'), Config::get('mailfrom'), $subject, $message);
    ...
    public function saveTask()
    {
        ...
        // Build the message to be e-mailed
        if ($state == 'add') {
            $subject  = '[' . Config::get('sitename') . ' ' . Lang::txt('EVENTS_CAL_LANG_CAL_TITLE')
                . '] - ' . Lang::txt('EVENTS_CAL_LANG_MAIL_ADDED');
     ...
     */
    private function sendEmail(&$hub, $email, $subject, $message)
    {
        if ($hub) {
    ...
    {
        $sql = "SELECT * FROM `#__categories` WHERE extension='" . $this->_option
            . "' AND published = '1' ORDER BY lft ASC";
