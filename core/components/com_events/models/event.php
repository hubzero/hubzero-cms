...
class Event extends Model
{
    ...
    public function link()
    {
        ...
        return Route::url(
            'index.php?option=com_groups&cn=' . $group->get('cn')
                . '&active=calendar&action=details&event_id=' . $this->get('id')
        );
    }
    ...
    public function humanReadableRepeatingRule()
    {
        ...
        if ($rule['interval'] > 1) {
            ...
            $readable .= Lang::txt(
                'COM_EVENTS_REPEATS_EVERY',
                $rule['interval'],
                ucfirst(str_replace('ly', 's', $rule['freq']))
            );
        } else {
    ...
    public function export()
    {
        ...
        $created  = gmdate('Ymd', strtotime($this->get('created'))) . 'T'
            . gmdate('His', strtotime($this->get('created'))) . 'Z';
        $modified = gmdate('Ymd', strtotime($this->get('modified'))) . 'T'
            . gmdate('His', strtotime($this->get('modified'))) . 'Z';
        ...
        header(
            'Content-Disposition: attachment; filename='
                . str_replace(' ', '_', strtolower($title)) . '_export.ics'
        );
        echo $output;
        exit();
    }
