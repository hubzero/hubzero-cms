...
class joomlaInstallerScript
{
	...
	protected function updateDatabase()
	{
		...
		if (substr($db->name, 0, 5) == 'mysql')
		{
			...
			if ($db->getErrorNum())
			{
				echo Lang::txt('JLIB_DATABASE_ERROR_FUNCTION_FAILED', $db->getErrorNum(), $db->getErrorMsg()).'<br />';
				return;
			}
	...
	protected function updateManifestCaches()
	{
		...
		foreach ($extensions as $extension) {
			if (!$installer->refreshManifestCache($extension->extension_id)) {
				echo Lang::txt('FILES_JOOMLA_ERROR_MANIFEST', $extension->type, $extension->element, $extension->name, $extension->client_id).'<br />';
			}
		}
	}
	public function deleteUnexistingFiles()
	{
		$files = array(
			...
			'/components/com_users/admin/controllers/config.php',
