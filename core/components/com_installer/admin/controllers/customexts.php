...
class Customexts extends AdminController
{
    ...
    public function displayTask()
    {
        ...
        if ($filters['sort'] == 'name' || (!empty($filters['search']) && stripos($filters['search'], 'id:') !== 0)) {
            ...
            $sortDir = $filters['sort_Dir'] == 'desc' ? -1 : 1;
            \Hubzero\Utility\Arr::sortObjects(
                $result,
                $filters['sort'],
                $sortDir,
                true,
    ...
    public function saveTask()
    {
        ...
        } elseif ($model->type == "non-standard") {
        ...
        foreach ($ids as $id) {
            ...
            if (!is_dir($extension->path . DS . '.git')) {
                if ($extension->get('apikey')) {
                    $urlHost = parse_url($extension->get('url'), PHP_URL_HOST);
                    $urlPath = parse_url($extension->get('url'), PHP_URL_PATH);
                    $newURL = "https://oauth2:" . $extension->get('apikey') . "@" . $urlHost . $urlPath;
                    $museCmd = 'cloneRepo repoPath=' . $extension->path . ' sourceUrl=' . $newURL;
                } else {
                    $extUrl = $extension->get('url');
                    $museCmd = 'cloneRepo repoPath=' . $extension->path . ' sourceUrl=' . $extUrl;
                }
            ...
            if (isset($clone_response)) {
                ...
                if (preg_grep("/cloned an empty repository.../uis", $clone_response)) {
                ...
                } elseif (preg_grep("/could not read Username.../uis", $clone_response)) {
                ...
                } elseif (preg_grep("/fatal: Authentication failed.../uis", $clone_response)) {
                    ...
                    // Try and determine a successful clone.
                    // The string "cloning into" is always in the output message.
                    $clonePattern = "/Cloning into '" . preg_quote($extension->path, '/') . "'.../uis";
                    if (!preg_grep($clonePattern, $clone_response)) {
                        // Try and catch other errors.
                        // If other errors are found, it's best to add them above.
                        $failed[] = array(
                            'ext_id' => $id,
                            'extension' => $extName,
                    ...
                    // Run migrations
                    $migrations_response = Cli::migration(
                        false,
                        true,
                        null,
                        'up',
                ...
                // If we have a cloned repo, let's check for incoming form changes.
                ...
                $museCmd = 'updateGitURLconf repoPath=' . $extension->path;
                $museCmd .= ' newsourceUrl=' . $newURL;
                $updateGitURLconf_response = Cli::call($museCmd, $task = 'repository');
                $respJson = $updateGitURLconf_response == null ? '' : $updateGitURLconf_response;
                $updateGitURLconf_response = json_decode($respJson);
                ...
                if ($updateGitURLconf_response === null) {
                    ...
                    $failed[] = array(
                        'ext_id' => $id,
                        'extension' => $extName,
                        'message' => array("MUSE updateGitURLconf returned NULL", $museCmd)
                    );
                } elseif ($updateGitURLconf_response[0] != '' || json_last_error() != JSON_ERROR_NONE) {
                    // add failed message
                    $failed[] = array(
                        'ext_id' => $id,
                        'extension' => $extName,
                        'message' => $updateGitURLconf_response
                    );
                } else {
                    // Check if specified branch is being used. If not checkout specified branch
                    $gitBranch = $extension->get('git_branch');
                    $branchArg = (!empty($gitBranch)) ? ' git_branch=' . $gitBranch : '';
                    $museCmd = 'checkoutRepoBranch repoPath=' . $extension->path . $branchArg;
                    ...
                    if (preg_grep("/command not found/uis", $checkoutRepoBranch_response)) {
                        ...
                        $failed[] = array(
                            'ext_id' => $id,
                            'extension' => $extName,
                            'message' => $checkoutRepoBranch_response
                        );
                    } else {
                        // Check for updates in remote branch
                        $sourceArg = (!empty($gitBranch)) ? ' source=' . $gitBranch : '';
                        $museCmd = 'update -r=' . $extension->path . $sourceArg;

                        $fetch_response = Cli::call($museCmd, $task = 'repository');
                        $respJson = $fetch_response == null ? '' : $fetch_response;
                        $fetch_response = json_decode($respJson);

                        // check for failure scenarios, otherwise consider the muse update successful
                        $cmdNotFound = "/command not found/uis";
                        $unknownRev = "/unknown revision or path not in the working tree/uis";
                        if (!is_null($fetch_response) && preg_grep($cmdNotFound, $fetch_response)) {
                        ...
                        } elseif (!is_null($fetch_response) && preg_grep($unknownRev, $fetch_response)) {
                            ...
                            // if the response from the fetch was empty no changes needed
                            $upToDate = Lang::txt('COM_INSTALLER_CUSTOMEXTS_FETCH_CODE_UP_TO_DATE');
                            $output = ($fetch_response == '') ? array($upToDate) : $fetch_response;
    ...
    public function removeTask()
    {
        ...
        foreach ($ids as $id) {
            ...
            } elseif (preg_grep("/Updating the repository.../uis", $remove_response)) {
                // did we succeed
                // add success message
                $success[] = array(
                    'extension'   => $extension->get('name'),
