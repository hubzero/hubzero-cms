...
class View
{
    ...
    public static function buildRecursivePageMenu($group, $pageArray)
    {
        ...
        if (count($pageArray) > 0) {
            ...
            foreach ($pageArray as $key => $page) {
                ...
                if (
                    ($pageAccess == 'registered' && $isGuest) ||
                    ($pageAccess == 'members' && !$isMember)
                ) {
                    $out .= '<li class="protected"><span class="page">'
                        . $page->get('title') . '</span></li>';
                } else {
                    $out .= '<li class="' . $cls . '">';
                    $out .= '<a class="page" title="' . $page->get('title') . '" href="'
                        . $page->url() . '">' . $page->get('title') . '</a>';
                }
    ...
    public static function displaySectionsContent($group, $overviewSection = null)
    {
        ...
        if ($tab == 'overview' && $trueTab != 'login') {
            ...
            if (!$userHasAccess) {
                ...
                $noAccessMsg = Lang::txt('You do not have the permissions to access this group page.');
                $content = '<p class="info">' . $noAccessMsg . '</p>';
            }
        }
    ...
    public static function getPageCss($group)
    {
        ...
        $baseUrl = rtrim(str_replace('administrator', '', Request::base()), '/');
        $url = $baseUrl . '/groups/' . $group->get('cn');
    ...
    {
        if ($stylesheets = \Hubzero\Facades\Cache::get('groups.' . $url)) {
            return $stylesheets;
        }
        ...
        \Hubzero\Facades\Cache::put('groups.' . $url, $stylesheets, 15);
    ...
    {
        $lerror = new \Hubzero\Error\Exception\LegacyException(
            $error->getMessage(),
            $error->getCode(),
            null,
            null,
        ...
        // get error template
        // must wrap in output buffer to capture contents since returning content
        // through output method returns to the method that called handleSuperGroupError
        // with call_user_func
        ob_start();
        $template = new Template();
        $groupInstance = \Hubzero\User\Group::getInstance(Request::getString('cn', ''));
        $template->set('group', $groupInstance)
        ...
        $debugCss = '/core/plugins/system/debug/assets/css/debug.css';
        $groupsCss = '/core/components/com_groups/site/assets/css/groups.css';
        $document->addStylesheet($debugCss);
        $document->addStylesheet($groupsCss);
        $bufferOptions = array('type' => 'component', 'name' => '');
        $document->setBuffer($errorTemplate, $bufferOptions);
    ...
    public static function superGroupComponents($group, $tab = '')
    {
        ...
        $templateComponentFolder = PATH_APP . DS . trim($uploadPath, DS)
            . DS . $group->get('gidNumber') . DS . 'components' . DS . 'com_' . $tab;
    ...
    public static function superGroupPhpPages($group)
    {
        ...
        $templatePhpPagePath = PATH_APP . DS . trim($uploadPath, DS)
            . DS . $group->get('gidNumber') . DS . 'pages' . DS . $path . '.php';
