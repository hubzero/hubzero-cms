...
class Manage extends AdminController
{
    ...
    public function displayTask()
    {
        ...
        if (!$canDo->get('core.admin')) {
            if (
                $this->view->filters['type'][0] == 'system'
                || $this->view->filters['type'][0] == "0"
                || $this->view->filters['type'][0] == null
            ) {
    ...
    public function saveTask()
    {
        ...
        $autosubscribe = isset($g['discussion_email_autosubscribe'])
            ? $g['discussion_email_autosubscribe']
            : '';
        $group->set('discussion_email_autosubscribe', $autosubscribe);
        ...
        if ($group->isSuperGroup()) {
            $this->handleSuperGroup($group);

            // git lab stuff
            $this->handleSuperGroupGitlab($group);
        }
        ...
        if ($this->getTask() == 'apply') {
            $url = 'index.php?option=' . $this->_option
                ...
                . '&task=edit&id=' . $group->get('gidNumber');
            App::redirect(
                Route::url($url, false)
            );
            return;
     ...
     * @return  void
     */
    private function handleSuperGroup($group)
    {
        // get the upload path for groups
        $uploadPath = PATH_APP . DS
        ...
        $fileOwner = escapeshellcmd($this->config->get('super_group_file_owner', 'access-content'));
        shell_exec("chgrp -R " . $fileOwner . " " . $uploadPath . " 2>&1");
     ...
     * @return  void
     */
    private function handleSuperGroupGitlab($group)
    {
        $client = new Gitlab();
        if (!$client->validate()) {
            return;
        ...
        } elseif (count($gitLabGroup) > 1) {  // If search returns more than one match, return with error.
        ...
        if (array_key_exists('message', $gitLabProject)) {
            ...
            // Search result must match hub group name to gitlab project name
            // exactly or create new gitlab project
        ...
        $authorInfo = '"' . Config::get('sitename')
            . ' Groups <groups@' . $_SERVER['HTTP_HOST'] . '>"';
        ...
        $scriptPath = dirname(dirname(__DIR__)) . DS . 'admin' . DS . 'assets'
            . DS . 'scripts' . DS . 'gitlab_setup.sh';
        $cmd  = 'sh ' . $scriptPath . ' ';
        ...
        foreach ($ids as $id) {
            ...
            if (!is_dir($uploadPath . DS . '.git')) {
                ...
                if (substr($environment, 0, 7) != 'staging') {
                    $failed[] = array(
                        'group' => $group->get('cn'),
                        'message' => Lang::txt('COM_GROUPS_GITLAB_NOT_MANAGED_BY_GIT')
                    );
                    continue;
                }
                ...
                $scriptPath = dirname(dirname(__DIR__)) . DS . 'admin'
                    . DS . 'assets' . DS . 'scripts' . DS . 'gitlab_setup_stage.sh';
                $basePath = str_replace('/' . $group->get('gidNumber'), '', $uploadPath);
                $cmd  = 'sh ' . $scriptPath . ' ';
                $cmd .= $basePath . ' ' . $group->get('gidNumber') . ' '
                    . $group->get('cn') . ' ' . $gitLabUrl . ' 2>&1';
            ...
            if ($url_bits["pass"] !== $gitLabKey) {
                $gitLabUrl = $url_bits["scheme"] . '://' . $url_bits["user"]
                    . ':' . $gitLabKey . '@' . $url_bits["host"] . $url_bits["path"];
                $resetScriptPath = dirname(dirname(__DIR__)) . DS . 'admin'
                    . DS . 'assets' . DS . 'scripts' . DS . 'gitlab_reset_remote.sh';
                $rcmd  = 'sh ' . $resetScriptPath . ' ';
            ...
            $cmd .= $sudo . PATH_CORE . '/bin/muse' . ' ' . $task
                . ' ' . $museCmd . ' --format=json';
            ...
            if ($output == '' || json_last_error() == JSON_ERROR_NONE) {
                // code is up to date
                if ($output == '') {
                    $output = array(Lang::txt('COM_GROUPS_FETCH_CODE_UP_TO_DATE'));
                }
    ...
    public function deleteTask()
    {
        ...
        if (!empty($ids)) {
            foreach ($ids as $id) {
                ...
                $log .= Lang::txt('COM_GROUPS_PUBLIC_TEXT') . ': '
                    . stripslashes($group->get('public_desc')) . "\n";
                $log .= Lang::txt('COM_GROUPS_PRIVATE_TEXT') . ': '
                    . stripslashes($group->get('private_desc')) . "\n";
                $log .= Lang::txt('COM_GROUPS_RESTRICTED_MESSAGE') . ': '
    ...
    protected function _authorize($task = null, $group = null)
    {
        ...
        $isSystemGroupEdit = !$canDo->get('core.admin')
            && $task == 'edit'
            && $group->get('type') == 0;
        if (!$canDo->get($taskName) || $isSystemGroupEdit) {
            // No access
            return false;
        }
