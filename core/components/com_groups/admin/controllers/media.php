...
class Media extends AdminController
{
    ...
    public function execute()
    {
        ...
        $uploadPath = trim($this->config->get('uploadpath', '/site/groups'), DS);
    ...
    public function displayTask()
    {
        ...
        if ($dirs) {
            foreach ($dirs as $d) {
                $folders[] = \Hubzero\Facades\Html::select('option', substr($d, strlen($this->path)));
            }
        }
        sort($folders);

        // Create folder <select> list
        $dataPathUrl = Route::url(
