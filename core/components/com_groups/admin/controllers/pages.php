...
class Pages extends AdminController
{
    ...
    public function saveTask()
    {
        ...
        if (!$this->page->get('id') || $this->page->get('parent') != $parent_id) {
            ...
            $sql = "UPDATE `#__xgroups_pages` SET rgt=rgt+2 WHERE rgt>=" . $start
                . " AND gidNumber=" . $this->group->get('gidNumber');
            $this->database->setQuery($sql);
            $this->database->query();

            // update current lefts
            $sql2 = "UPDATE `#__xgroups_pages` SET lft=lft+2 WHERE lft>" . ($start - 1)
                . " AND gidNumber=" . $this->group->get('gidNumber');
        ...
        if (
            strpos($this->version->get('content'), '<?') !== false ||
    ...
    public function scanAgainTask()
    {
        ...
        $url = 'index.php?option=' . $this->_option . '&controller=' . $this->_controller
            . '&gid=' . $this->gid . '&task=scan&id=' . $groupPage->get('id');
        App::redirect(
            Route::url($url, false)
        );
    }
    ...
    public function errorsCheckAgainTask()
    {
        ...
        $url = 'index.php?option=' . $this->_option . '&controller=' . $this->_controller
            . '&gid=' . $this->gid . '&task=errors&id=' . $groupPage->get('id');
        App::redirect(
            Route::url($url, false)
        );
    }
