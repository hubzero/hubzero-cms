...
// set title
$title  = ($this->hook->get('id')) ? Lang::txt('COM_GROUPS_IMPORTHOOK_TITLE_EDIT') : Lang::txt('COM_GROUPS_IMPORTHOOK_TITLE_ADD');
    ...
    <div class="grid">
        <div class="col span8">
            <fieldset class="adminform">
                <legend><?php echo Lang::txt('COM_GROUPS_IMPORTHOOK_EDIT_FIELDSET_DETAILS'); ?></legend>

                <div class="input-wrap">
                    <label for="field-event">
                        <?php echo Lang::txt('COM_GROUPS_IMPORTHOOK_EDIT_FIELD_TYPE'); ?>
                    </label>
                    <select name="hook[event]" id="field-event">
                        <option value="postparse">
                            <?php echo Lang::txt('COM_GROUPS_IMPORTHOOK_EDIT_FIELD_TYPE_POSTPARSE'); ?>
                        </option>
                        <option <?php if ($this->hook->get('event') == 'postmap') {
                            echo 'selected="selected"';
                                } ?> value="postmap">
                            <?php echo Lang::txt('COM_GROUPS_IMPORTHOOK_EDIT_FIELD_TYPE_POSTMAP'); ?>
                        </option>
                        <option <?php if ($this->hook->get('event') == 'postconvert') {
                            echo 'selected="selected"';
                                } ?> value="postconvert">
                            <?php echo Lang::txt('COM_GROUPS_IMPORTHOOK_EDIT_FIELD_TYPE_POSTCONVERT'); ?>
                        </option>
                    </select>
                </div>
                <div class="input-wrap">
                    <label for="field-name">
                        <?php echo Lang::txt('COM_GROUPS_IMPORTHOOK_EDIT_FIELD_NAME'); ?>
                    </label>
                    <input type="text" name="hook[name]" id="field-name" value="<?php echo $this->escape($this->hook->get('name')); ?>" />
                </div>
                <div class="input-wrap">
                    <label for="field-notes">
                        <?php echo Lang::txt('COM_GROUPS_IMPORTHOOK_EDIT_FIELD_NOTES'); ?>
                    </label>
            ...
            <fieldset class="adminform">
                <legend><?php echo Lang::txt('COM_GROUPS_IMPORTHOOK_EDIT_FIELDSET_FILE'); ?></legend>

                <div class="input-wrap">
                    <label for="field-script">
                        <?php echo Lang::txt('COM_GROUPS_IMPORTHOOK_EDIT_FIELD_SCRIPT'); ?>
                    </label>
                    <?php
                    if ($this->hook->get('file')) {
                        echo Lang::txt('COM_GROUPS_IMPORTHOOK_EDIT_FIELD_SCRIPT_CURRENT', $this->hook->get('file'));
                        echo ' &mdash; <a rel="noopener noreferrer" target="_blank" href="' . Route::url('index.php?option=' . $this->option . '&controller=' . $this->controller . '&task=raw&id=' . $this->hook->get('id')) . '">' . Lang::txt('COM_GROUPS_IMPORTHOOK_EDIT_FIELD_SCRIPT_VIEWRAW') . '</a><br />';
                    }
                    ?>
