...
Toolbar::title(
    $group->get('description') . ': ' . $module->get('title') . ' - ' . Lang::txt('COM_GROUPS_MODULES_SCAN'),
    'groups'
);
...
<form
    ...
    data-confirm="{{ Lang::txt('COM_GROUPS_MODULES_MARK_SCANNED_CONFIRM') }}"
    ...
    @if (count($severe) > 0)
        <p class="error">
            {!! Lang::txt('COM_GROUPS_MODULE_SCAN_SEVERE', implode('<br />', $severe)) !!}
        </p>
    @endif

    @if (count($elevated) > 0)
        <p class="warning">
            {!! Lang::txt('COM_GROUPS_MODULE_SCAN_ELEVATED', implode('<br />', $elevated)) !!}
        </p>
    @endif

    @if (count($minor) > 0)
        <p class="info">
            {!! Lang::txt('COM_GROUPS_MODULE_SCAN_MINOR', implode('<br />', $minor)) !!}
        </p>
    @endif
