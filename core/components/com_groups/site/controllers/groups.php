...
class Groups extends Base
{
    ...
    public function displayTask()
    {
        ...
        // Get featured groups
        $featuredList = $this->config->get('intro_featuredgroups_list', '');
        $this->view->featuredgroups = Group\Helper::getFeaturedGroups($featuredList);
    ...
    public function viewTask()
    {
        ...
        if (!$this->validateCn($this->cn, true)) {
        ...
        if (
            !is_object($this->view->group)
            || !$this->view->group->get('gidNumber')
            || !$this->view->group->get('cn')
        ) {
    ...
    public function editTask()
    {
        ...
        $asset_path = PATH_APP . DS
            . trim($this->config->get('uploadpath', '/site/groups'), DS)
            . DS . $this->view->lid . DS . 'uploads';
        ...
        if (is_dir($asset_path)) {
            ...
            $logoPattern = '.jpg|.jpeg|.png|.gif|.svg|.PNG|.JPG|.JPEG|.GIF|.SVG';
            $this->view->logos = Filesystem::files($asset_path, $logoPattern, true, true);
        }
    ...
    public function saveTask()
    {
        ...
        if (
            (!$g_gidNumber && !User::authorise('core.create', $this->_option))
        ...
        $descriptionRaw    = Request::getString('description', Lang::txt('NONE'), 'post');
        $g_description     = preg_replace('/\s+/', ' ', trim($descriptionRaw));
        ...
        if (!$g_description) {
            $this->setNotification(
                Lang::txt('COM_GROUPS_SAVE_ERROR_MISSING_INFORMATION')
                    . ': ' . Lang::txt('COM_GROUPS_DETAILS_FIELD_DESCRIPTION'),
                'error'
            );
        }
        ...
        if (isset($customization['logo'])) {
            $logo_path = PATH_APP . DS
                . trim($this->config->get('uploadpath', '/site/groups'), DS)
                . DS . $group->get('gidNumber') . DS . 'uploads' . DS;
        ...
        $activityAction = ($this->_task == 'new' ? 'created' : 'updated');
        $activityKey = ($this->_task == 'new' ? 'CREATED' : 'UPDATED');
        $activityDesc = Lang::txt(
            'COM_GROUPS_ACTIVITY_GROUP_' . $activityKey,
            '<a href="' . $url . '">' . $group->get('description') . '</a>'
        );
    ...
    public function doDeleteTask()
    {
        ...
        $log .= Lang::txt('COM_GROUPS_GROUP_PUBLIC_TEXT') . ': '
            . stripslashes($group->get('public_desc'))  . "\n";
        $log .= Lang::txt('COM_GROUPS_GROUP_PRIVATE_TEXT') . ': '
            . stripslashes($group->get('private_desc'))  . "\n";
        $log .= Lang::txt('COM_GROUPS_GROUP_RESTRICTED_MESSAGE') . ': '
            . stripslashes($group->get('restrict_msg')) . "\n";
        ...
        $deletedGroupUrl = Route::url(
            'index.php?option=' . $this->_option . '&cn=' . $deletedgroup->get('cn')
        );
        $deletedDesc = Lang::txt(
            'COM_GROUPS_ACTIVITY_GROUP_DELETED',
            '<a href="' . $deletedGroupUrl . '">' . $deletedgroup->get('description') . '</a>'
        );
        Event::trigger('system.logActivity', [
            'activity' => [
                'action'      => 'deleted',
    ...
    public function suggestNonExistingGroupTask()
    {
        ...
        // Set notification
        $suggestUrl = 'index.php?option=' . $this->_option . '&task=new'
            . (is_numeric($this->cn) ? '' : '&suggested_cn=' . $this->cn);
        $this->setNotification(
            Lang::txt('COM_GROUPS_CREATE_SUGGEST', Route::url($suggestUrl)),
            'warning'
        );
    ...
    public function autocompleteTask()
    {
        ...
        $searchQuoted = $this->database->quote('%' . $filters['search'] . '%');
        $query = "SELECT t.gidNumber, t.cn, t.description
					FROM `#__xgroups` AS t
					WHERE (t.type=1 OR t.type=3)
					AND (LOWER(t.cn) LIKE " . $searchQuoted
                    . " OR LOWER(t.description) LIKE " . $searchQuoted . ")
    ...
    public function memberslistTask()
    {
        ...
        if ($filters['cn']) {
            $cnQuoted = $this->database->quote($filters['cn']);
            $query = "SELECT u.username, u.name
						FROM `#__users` AS u, `#__xgroups_members` AS m, `#__xgroups` AS g
						WHERE g.cn=" . $cnQuoted . " AND g.gidNumber=m.gidNumber
						AND m.uidNumber=u.id AND u.block = '0'
        ...
        if (count($rows) > 0) {
            foreach ($rows as $row) {
                $escapedName = htmlentities(stripslashes($row->name), ENT_COMPAT, 'UTF-8');
                $json[] = '{"username":"' . $row->username . '","name":"' . $escapedName . '"}';
            }
        }
    ...
    public function groupavailabilityTask($group = null)
    {
        ...
        // Ensure the data passed is valid
        $isReserved = ($group == 'new' || $group == 'browse');
        $isInvalidCn = !$this->validateCn($group);
        $groupExists = Group::exists($group, true);
        if ($isReserved || $isInvalidCn || $groupExists) {
            $availability = false;
        } else {
            $availability = true;
        }
    ...
    public function downloadTask($filename = '')
    {
        ...
        if ($this->active == 'wiki') {
            ...
            $isRestricted = $page->get('access') == 1
     ...
     */
    private function validateCn($cn, $allowDashes = false)
    {
