...
class Pages extends Base
{
    ...
    public function saveTask($apply = false)
    {
        ...
        $sql = "UPDATE `#__xgroups_pages` SET rgt=rgt+2"
            . " WHERE rgt>" . ($start - 1)
            . " AND gidNumber=" . $this->group->get('gidNumber');
        $this->database->setQuery($sql);
        $this->database->query();

        // update current lefts
        $sql2 = "UPDATE `#__xgroups_pages` SET lft=lft+2"
            . " WHERE lft>" . ($start - 1)
            . " AND gidNumber=" . $this->group->get('gidNumber');
        ...
        if ($apply) {
            ...
            $redirect = Route::url(
                ...
                . '&controller=pages&task=edit&pageid=' . $this->page->get('id')
            );
        } else {
        ...
        $pagesUrl = Route::url(
            'index.php?option=com_groups&cn=' . $this->group->get('cn') . '&controller=pages'
        );
        $groupLink = '<a href="' . $pagesUrl . '">' . $this->group->get('description') . '</a>';
        $pageActionKey = ($page['id'] ? 'UPDATED' : 'CREATED');
        $pageActionDesc = Lang::txt(
            'COM_GROUPS_ACTIVITY_PAGE_' . $pageActionKey,
            $this->page->get('title'),
            $groupLink
        );
    ...
    public function setStateTask($state = 1, $status = 'published')
    {
        ...
        $stateAction = ($state == 2 ? 'DELETED' : ($state == 1 ? 'PUBLISHED' : 'UNPUBLISHED'));
        $groupLink = '<a href="' . $url . '">' . $this->group->get('description') . '</a>';
        $stateDesc = Lang::txt(
            'COM_GROUPS_ACTIVITY_PAGE_' . $stateAction,
            $page->get('title'),
            $groupLink
        );
        Event::trigger('system.logActivity', [
    ...
    public function setHomeTask()
    {
        ...
        if ($version === null) {
            $errorMsg = Lang::txt('COM_GROUPS_PAGES_PAGE_HOME_ERROR', $page->get('title'));
            $this->setNotification($errorMsg, 'error');
            $pagesUrl = Route::url(
                'index.php?option=' . $this->_option
                . '&cn=' . $this->group->get('cn')
                . '&controller=pages'
            );
            App::redirect($pagesUrl);
        }
        ...
        // inform user
        $successMsg = Lang::txt('COM_GROUPS_PAGES_PAGE_HOME_SET', $page->get('title'));
        $this->setNotification($successMsg, 'passed');
    ...
    public function restoreTask()
    {
        ...
        $versionDate = Date::of($pageVersion->get('created'))->format('M d, Y @ g:ia');
        $restoreMsg = Lang::txt(
            'COM_GROUPS_PAGES_PAGE_VERSION_RESTORED',
            $page->get('title'),
            $version,
            $versionDate
        );
