...
class Membership extends Base
{
    ...
    public function doinviteTask()
    {
        ...
        foreach ($la as $l) {
            ...
            if (is_numeric($l)) {
                ...
                if (\Components\Members\Helpers\Utility::validemail($l)) {
                    ...
                    $emailQuoted = $this->database->quote($l);
                    $emailLikeQuoted = $this->database->quote($l . '%');
                    $query = "SELECT u.id FROM `#__users` AS u"
                        . " WHERE LOWER(u.email)=" . $emailQuoted
                        . " OR LOWER(u.email) LIKE " . $emailLikeQuoted . " LIMIT 1;";
        ...
        $inviteDesc = Lang::txt(
            'COM_GROUPS_ACTIVITY_GROUP_USER_INVITED',
            '<a href="' . $url . '">' . $this->view->group->get('description') . '</a>'
        );
        foreach ($invitees as $invitee) {
        ...
        $invitesDesc = Lang::txt(
            'COM_GROUPS_ACTIVITY_GROUP_USERS_INVITED',
            implode(', ', $activity),
            '<a href="' . $url . '">' . $this->view->group->get('description') . '</a>'
        );
        Event::trigger('system.logActivity', [
    ...
    public function acceptTask()
    {
        ...
        if ($this->view->group->get('type') == 2) {
            $this->errorHandler(403, Lang::txt('COM_GROUPS_ERROR_FORBIDDEN'));
            return;
        }
        ...
        $gidNumber = $this->view->group->get('gidNumber');
        $inviteemails_with_token = $group_inviteemails->getInviteEmails($gidNumber, false);
        ...
        if ($token) {
            ...
            $this->view->group->add('members', array(User::get('id')));
            $this->view->group->update();
            $emailQuoted = $this->database->quote(strtolower(User::get('email')));
            $gidQuoted = $this->database->quote($this->view->group->get('gidNumber'));
            $sql = "DELETE FROM `#__xgroups_inviteemails`"
                . " WHERE LOWER(email)=" . $emailQuoted
                . " AND gidNumber=" . $gidQuoted;
            $this->database->setQuery($sql);
            $this->database->query();
            ...
            $this->errorHandler(404, Lang::txt('COM_GROUPS_ERROR_UNABLE_TO_JOIN'));
        }
        ...
        $acceptedDesc = Lang::txt(
            'COM_GROUPS_ACTIVITY_GROUP_USER_ACCEPTED',
            '<a href="' . $url . '">' . $this->view->group->get('description') . '</a>'
        );
        Event::trigger('system.logActivity', [
            'activity' => [
    ...
    public function cancelTask()
    {
        ...
        $msgArgs = array('groups_cancelled_me', $subject, $message, $from, $managers, $this->_option);
        if (!Event::trigger('xmessage.onSendMessage', $msgArgs)) {
        ...
        $cancelledDesc = Lang::txt(
            'COM_GROUPS_ACTIVITY_GROUP_USER_CANCELLED',
            '<a href="' . $url . '">' . $this->view->group->get('description') . '</a>'
        );
        Event::trigger('system.logActivity', [
            'activity' => [
    ...
    public function joinTask()
    {
        ...
        $joinPolicy = $this->view->group->get('join_policy');
        if ($joinPolicy == 3 || $joinPolicy == 2) {
        ...
        if ($this->view->group->get('join_policy') == 0) {
            ...
            $joinedDesc = Lang::txt(
                'COM_GROUPS_ACTIVITY_GROUP_USER_JOINED',
                '<a href="' . $url . '">' . $this->view->group->get('description') . '</a>'
            );
            Event::trigger('system.logActivity', [
                'activity' => [
    ...
    public function dorequestTask()
    {
        ...
        $requestedDesc = Lang::txt(
            'COM_GROUPS_ACTIVITY_GROUP_USER_REQUESTED',
            '<a href="' . $url . '">' . $this->view->group->get('description') . '</a>'
        );
        Event::trigger('system.logActivity', [
            'activity' => [
     ...
     */
    private function generateRandomString($strLength = 25)
    {
        $str = '';
