...
class Media extends Base
{
    ...
    public function filebrowserTask()
    {
        ...
        $this->view->folderTree = $this->buildFolderTreeHtml($folderTree);
        $this->view->folderList = $this->buildFolderTreeSelect($folderTree);

        // add base path if super group
        /*if ($this->group->isSuperGroup())
        {
            $rootHtml = '<ul><li><a class="tree-folder-toggle" href="javascript:void(0);"></a>'
                . '<a data-folder="/" href="javascript:void(0);" class="tree-folder">/ (root)</a>'
                . $this->view->folderTree . '</li></ul>';
            $this->view->folderTree = $rootHtml;
        }*/
    ...
    private function buildFolderTreeHtml($tree)
    {
        ...
        foreach ($tree as $treeLevel) {
            $basePath = $base . '/site/groups/' . $hubzeroGroup->get('gidNumber');
            $folder       = str_replace($basePath, '', $treeLevel['relname']);
            ...
            $html .= '<li>';
            $html .= $nodeToggle
                . '<a data-folder="' . $folder . '" href="javascript:void(0);" class="tree-folder">'
                . $treeLevel['name'] . '</a>';
            $html .= $childrenHtml;
            $html .= '</li>';
        }
    ...
    private function buildFolderTreeSelectOptionList($tree)
    {
        ...
        foreach ($tree as $treeLevel) {
            $basePath = $base . '/site/groups/' . $hubzeroGroup->get('gidNumber');
            $value = str_replace($basePath, '', $treeLevel['relname']);
    ...
    public function listfilesTask()
    {
        ...
        if (is_dir($this->path)) {
            ...
            if (isset($this->view->type) && $this->view->type != '') {
                $imageExtensions = array('jpg', 'jpeg', 'png', 'gif', 'bmp', 'tiff');
                foreach ($files as $k => $file) {
                    $fileInfo = pathinfo($file);
                    $ext      = isset($fileInfo['extension']) ? strtolower($fileInfo['extension']) : '';
                    if ($this->view->type == 'images' && !in_array($ext, $imageExtensions)) {
                        unset($files[$k]);
                    } elseif ($this->view->type == 'files' && in_array($ext, $imageExtensions)) {
                        unset($files[$k]);
                    }
                }
            }
    ...
    public function ckeditorUploadTask()
    {
        ...
        if (!$upload->error) {
            ...
            $url .= $_SERVER['HTTP_HOST'] . DS . 'groups'
                . DS . $this->group->get('cn')
                . DS . 'File:' . $fileInfo['basename'];
        } else {
            $message = $upload->message;
        }
    ...
    public function doUpload()
    {
        ...
        if ($file['size'] > $sizeLimit) {
            $maxFormatted = \Hubzero\Utility\Number::formatBytes($sizeLimit);
            $abbrPattern = '/<abbr \w+=\\"\w+\\">(\w{1,3})<\\/abbr>/';
            $max = preg_replace($abbrPattern, '$1', $maxFormatted);
            $returnObj->error   = true;
    ...
    public function ajaxuploadTask()
    {
        ...
        $uploadDirectory = PATH_APP . DS
            . trim($this->config->get('uploadpath', '/site/groups'), DS)
            . DS . $this->group->get('gidNumber') . DS . ltrim($folder, DS);
        ...
        if ($size > $sizeLimit) {
            $maxFormatted = \Hubzero\Utility\Number::formatBytes($sizeLimit);
            $max = preg_replace('/<abbr \w+=\\"\w+\\">(\w{1,3})<\\/abbr>/', '$1', $maxFormatted);
    ...
    public function moveFileTask()
    {
        ...
        $folderTree = $this->buildFolderTree($folders);
        $folderList = $this->buildFolderTreeSelect($folderTree);
