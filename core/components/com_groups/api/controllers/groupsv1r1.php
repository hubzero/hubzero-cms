 ...
 *
 */
class Groupsv1r1 extends ApiController
{
    /**
     ...
     *      "allowedValues": "gidNumber, cn, description, published, approved, type,
     *          public_desc, join_policy, discoverability, logo, params, created, created_by"
     * }
     * @return  void
     */
    public function listTask()
    {
        ...
        foreach ($response as $i => $r) {
            ...
            $response[$i]->url  = str_replace('/api', '', $base . '/' . ltrim($groupUrl, '/'));
     ...
     * @apiParameter {
     *      "name":          "cn",
     *      "description":   "Group alias that appears in the url for group.
    ...
    public function createTask()
    {
        ...
        if (!in_array($joinPolicy, $policies)) {
            $allowedPolicies = implode(', ', $policies);
            $errors[] = array(
                'field'   => 'join_policy',
                'message' => Lang::txt(
                    'Group "join_policy" value must be one of the following: %s',
                    $allowedPolicies
                )
            );
        }
        ...
        if (!in_array($discoverability, $discoverabilities)) {
            $allowedDiscoverabilities = implode(', ', $discoverabilities);
            $errors[] = array(
                'field'   => 'discoverability',
                'message' => Lang::txt(
                    'Group "discoverability" value must be one of the following: %s',
                    $allowedDiscoverabilities
                )
            );
        }
    ...
    public function readTask()
    {
        ...
        $fields = Request::getString('fields', 'gidNumber,description,public_desc,logo,created,created_by,plugins');
            ...
            if (property_exists($record, $field)) {
                switch ($field) {
                    case 'logo':
                        ...
                        $group[$field] = str_replace('/api', '', $baseUrl . ltrim($record->getLogo(), '/'));
                        break;
                    case 'link':
                    ...
                    case 'plugins':
                        ...
                        if ($group[$field] == null) {
                            $events = Event::trigger('groups.onGroupAreas');
                            $plugins = array();
                            foreach ($events as $event) {
                                array_push($plugins, $event['name']);
                            }
                            $group[$field] = $plugins;
                        }
                        break;
        ...
        $base = rtrim(Request::base(), '/');
        $groupUrl = Route::url('index.php?option=com_groups&cn=' . $record->get('cn'));
        $group['url'] = str_replace('/api', '', $base . '/' . ltrim($groupUrl, '/'));
        ...
        if ($plugins) {
            ...
            foreach ($plugins as $section) {
                ...
                $sectionUrl = Route::url(
                    'index.php?option=com_groups&cn=' . $record->get('cn')
                    . '&active=' . $section['name']
                );
                $section['url'] = str_replace('/api', '', $base . '/' . ltrim($sectionUrl, '/'));

                $sections[] = $section;
            }
        }

        $overviewUrl = Route::url(
            'index.php?option=com_groups&cn=' . $record->get('cn') . '&active=overview'
        );
        array_unshift($sections, array(
            ...
            'url'              => str_replace('/api', '', $base . '/' . ltrim($overviewUrl, '/'))
        ));

        $group['sections'] = $sections;

        // Format for search indexing
        $searchFormat = Request::getWord('searchable', 'false');
        if ($searchFormat == 'true') {
            $group['title'] = $group['description'];
            $group['owner_type'] = 'group';
            $group['owner'] = $group['gidNumber'];
            $group['access_level'] = 'public';
            $combinedDesc = $group['description'] . ' ' . $group['public_desc'];
            $group['description'] = \Hubzero\Utility\Sanitize::stripAll($combinedDesc);
            $group['id'] = 'group-' . $group['gidNumber'];
            $group['url'] = '/groups/' . $record->get('cn');
            $group['hubtype'] = 'group';
        }
    ...
    public function updateTask()
    {
        ...
        // make sure we have a valid discoverability
        if (
            !in_array($discoverability, $discoverabilities)
