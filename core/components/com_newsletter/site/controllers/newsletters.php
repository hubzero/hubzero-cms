...
class Newsletters extends SiteController
{
    ...
    public function outputTask()
    {
        ...
        $newsletterUrl = 'https://' . $_SERVER['HTTP_HOST'] . DS . 'newsletter'
            . DS . $newsletter->alias . '?no_html=1';
        ...
        if (file_exists('/usr/bin/wkhtmltopdf') && file_exists('/usr/bin/xvfb-run')) {
            ...
            $cmd = '/usr/bin/xvfb-run -a -s "-screen 0 640x480x16" wkhtmltopdf '
                . $newsletterUrl . ' ' . $newsletterPdf;
        } elseif (file_exists('/usr/local/bin/wkhtmltopdf') && file_exists('/usr/local/bin/xvfb-run')) {
            //$cmd = '/usr/local/bin/wkhtmltopdf ' . $newsletterUrl . ' ' . $newsletterPdf;
            $cmd = '/usr/local/bin/xvfb-run -a -s "-screen 0 640x480x16" wkhtmltopdf '
                . $newsletterUrl . ' ' . $newsletterPdf;
        } elseif (file_exists('/usr/local/bin/wkhtmltopdf')) {
            $cmd = '/usr/local/bin/wkhtmltopdf ' . $newsletterUrl . ' ' . $newsletterPdf;
        } elseif (file_exists('/usr/bin/phantomjs')) {
            $rasterizeFile = dirname(__DIR__) . DS . 'assets' . DS . 'js' . DS . 'rasterize.js';
            $fallback = '/usr/bin/phantomjs --ssl-protocol=any --ignore-ssl-errors=yes '
                . '--web-security=false ' . $rasterizeFile . ' ' . $newsletterUrl
                . ' ' . $newsletterPdf . ' 8.5in*11in';
            if (!$cmd) {
                $cmd = $fallback;
            }
        }
