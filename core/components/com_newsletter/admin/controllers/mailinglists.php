...
class Mailinglists extends AdminController
{
    ...
    public function manageTask()
    {
        ...
        if (
            $filters['sort'] !== 'email' && $filters['sort'] !== 'date_added' &&
    ...
    public function addEmailTask()
    {
        ...
        // get request vars
        $mailinglistId = Request::getVar('id', array(Request::getInt('mid', 0)));
    ...
    public function doAddEmailTask()
    {
        ...
        if (isset($this->emailFile['size']) && $this->emailFile['size'] > 0) {
            ...
            // parse emails
            $emailFileEmails = $this->parseEmails($emailFileContents);
        }
        ...
        if ($this->emailBox != '') {
            // parse emails
            $emailBoxEmails = $this->parseEmails($this->emailBox);
        ...
        foreach ($emails as $k => $email) {
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                $badEmails[] = $email;
                unset($emails[$k]);
            } elseif (in_array($email, $currentEmails)) {
        ...
        if ($inserts > 0) {
            ...
            $emailList = implode('<br />&mdash; ', $emails);
            $message = Lang::txt(
                'COM_NEWSLETTER_MAILINGLSIT_MANAGE_ADD_SUCCESS',
                count($emails),
                $emailList
            );
        ...
        if (count($duplicateEmails) > 0) {
            $duplicateList = implode('<br />&mdash; ', $duplicateEmails);
            $message = Lang::txt(
                'COM_NEWSLETTER_MAILINGLSIT_MANAGE_ADD_DUPLICATE',
        ...
        if (count($badEmails) > 0) {
            $badList = implode('<br />&mdash; ', $badEmails);
            $message = Lang::txt(
                'COM_NEWSLETTER_MAILINGLSIT_MANAGE_ADD_FAILED',
                count($badEmails),
                $badList
            );
    ...
    public function exportTask()
    {
        ...
        // file name
        $dateString = Date::of('now')->format('m-d-Y');
        $filename = Lang::txt(
            'COM_NEWSLETTER_MAILINGLIST_EXPORT_FILENAME',
            $mailinglist->name,
            $dateString
        );
        $filename .= '.csv';
