...
class Newsletters extends AdminController
{
    ...
    private function dependencyCheck()
    {
        ...
        $sql = "SELECT * FROM `#__cron_jobs` WHERE `plugin`="
            . $database->quote('newsletter')
            . " AND `event`="
            . $database->quote('processMailings');
    ...
    public function stateTask()
    {
        ...
        if ($success) {
            // Set success message
            $msgKey = $publish
                ? 'COM_NEWSLETTER_PUBLISHED_SUCCESS'
                : 'COM_NEWSLETTER_UNPUBLISHED_SUCCESS';

            Notify::success(Lang::txt($msgKey));
        }
    ...
    public function previewTask()
    {
        ...
        // Generate output
        if (!$no_html) {
            $previewTitle = Lang::txt('COM_NEWSLETTER_PREVIEW');
            $content  = '<h2 class="modal-title">' . $previewTitle . '</h2><br /><br />';
            $iframeSrc = Route::url(
                'index.php?option=' . $this->_option
                . '&task=preview&id=' . $id . '&no_html=1'
            );
    ...
    public function doSendTestTask()
    {
        ...
        if (count($goodEmails)) {
            $emailList = implode('<br />', $goodEmails);
            Notify::success(
                Lang::txt('COM_NEWSLETTER_TEST_SEND_SUCCESS', $newsletter->name, $emailList)
            );
        }

        // Do we have any bad emails to tell user about
        if (count($badEmails)) {
            $badEmailList = implode('<br />', $badEmails);
            Notify::error(
                Lang::txt('COM_NEWSLETTER_TEST_SEND_FAIL', $newsletter->name, $badEmailList)
            );
        }
    ...
    public function doSendNewsletterTask()
    {
        ...
        if (!$newsletter->save()) {
            ...
            Notify::success(
                Lang::txt('COM_NEWSLETTER_NEWSLETTER_SEND_TO', $newsletter->name, $formattedCount)
            );
        }
    ...
    private function send(
        ...
        if ($sendingTest) {
            foreach ($newsletterContacts as $contact) {
                ...
                $unsubscribeLink       = 'https://' . $_SERVER['SERVER_NAME']
                    . '/newsletter/unsubscribe?e=' . urlencode($contact)
                    . '&t=' . $emailToken;
        ...
        // create recipients
        $this->sendTo($mailing, $newsletterContacts);
     ...
     * @return  void
     */
    private function sendTo($mailing, $emails)
    {
