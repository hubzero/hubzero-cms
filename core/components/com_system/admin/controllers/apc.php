...
class Apc extends AdminController
{
    ...
    public function hostTask()
    {
        ...
        $this->view->bmem_avail       = $this->bsize($mem['avail_mem']);
        $this->view->mem_used         = $this->view->mem_size - $this->view->mem_avail;
        $this->view->bmem_used        = $this->bsize($this->view->mem_size - $this->view->mem_avail);
        $this->view->seg_size         = $this->bsize($mem['seg_size']);

        // Calculate duration since cache start
        $duration = $time - $cache['start_time'];
        $durationUser = $time - $cache_user['start_time'];

        $this->view->req_rate         = round(($cache['num_hits'] + $cache['num_misses']) / $duration, 2);
        $this->view->hit_rate         = round(($cache['num_hits']) / $duration, 2);
        $this->view->miss_rate        = round(($cache['num_misses']) / $duration, 2);
        $this->view->insert_rate      = round(($cache['num_inserts']) / $duration, 2);
        $this->view->req_rate_user    = round(($cache_user['num_hits'] + $cache_user['num_misses']) / $durationUser, 2);
        $this->view->hit_rate_user    = round(($cache_user['num_hits']) / $durationUser, 2);
        $this->view->miss_rate_user   = round(($cache_user['num_misses']) / $durationUser, 2);
        $this->view->insert_rate_user = round(($cache_user['num_inserts']) / $durationUser, 2);
        $this->view->apcversion       = phpversion('apc');
        $this->view->phpversion       = phpversion();
        $this->view->number_files     = $cache['num_entries'];
        $this->view->size_files       = $this->bsize($cache['mem_size']);
        $this->view->number_vars      = $cache_user['num_entries'];
        $this->view->size_vars        = $this->bsize($cache_user['mem_size']);
        $this->view->host             = $this->host;
        $this->view->duration         = $this->duration($cache['start_time']);
    ...
    public function mkimageTask()
    {
        ...
        function fill_arc(
            ...
            if (function_exists("imagefilledarc")) {
                ...
                $startX = $centerX + cos(deg2rad($start)) * $r;
                $startY = $centerY + sin(deg2rad($start)) * $r;
                imageline($im, $centerX, $centerY, $startX, $startY, $color2);
                $startX1 = $centerX + cos(deg2rad($start + 1)) * $r;
                $startY1 = $centerY + sin(deg2rad($start)) * $r;
                imageline($im, $centerX, $centerY, $startX1, $startY1, $color2);
                $endX1 = $centerX + cos(deg2rad($end - 1)) * $r;
                $endY1 = $centerY + sin(deg2rad($end)) * $r;
                imageline($im, $centerX, $centerY, $endX1, $endY1, $color2);
                $endX = $centerX + cos(deg2rad($end)) * $r;
                $endY = $centerY + sin(deg2rad($end)) * $r;
                imageline($im, $centerX, $centerY, $endX, $endY, $color2);
                imagefill($im, $centerX + $r * cos($w) / 2, $centerY + $r * sin($w) / 2, $color2);
            }
            if ($text) {
                $textX = $centerX + $r * cos($w) / 2;
                $textY = $centerY + $r * sin($w) / 2;
                if ($placeindex > 0) {
                    imageline($im, $textX, $textY, $diameter, $placeindex * 12, $color1);
                    imagestring($im, 4, $diameter, $placeindex * 12, $text, $color1);
                } else {
                    imagestring($im, 4, $textX, $textY, $text, $color1);
                }
            }
        }
        ...
        switch ($MYREQUEST['IMG']) {
            case 1:
                ...
                for ($i = 0; $i < $mem['num_seg']; $i++) {
                    ...
                    if ($ptr < $mem['seg_size']) {
                        ...
                        $startDeg = $angle_from * 360;
                        $endDeg = $angle_to * 360;
                        fill_arc($image, $x, $y, $size, $startDeg, $endDeg, $col_black, $col_red);
                        if (($angle_to - $angle_from) > 0.05) {
                            array_push($string_placement, array($angle_from, $angle_to));
                        }
                    }
                ...
                foreach ($string_placement as $angle) {
                    $startDeg = $angle[0] * 360;
                    $endDeg = $angle[1] * 360;
                    $sizeText = $this->bsize($s * ($angle[1] - $angle[0]));
                    text_arc($image, $x, $y, $size, $startDeg, $endDeg, $col_black, $sizeText);
                }
                break;
            ...
            case 2:
                ...
                $hitHeight = -$a * ($size - 21) / $s;
                $hitPct = sprintf("%.1f%%", $cache['num_hits'] * 100 / $s);
                fill_box($image, 30, $size, 50, $hitHeight, $col_black, $col_green, $hitPct);

                $missHeight = -max(4, ($s - $a) * ($size - 21) / $s);
                $missPct = sprintf("%.1f%%", $cache['num_misses'] * 100 / $s);
                fill_box($image, 130, $size, 50, $missHeight, $col_black, $col_red, $missPct);
                break;
                ...
                for ($i = 0; $i < $mem['num_seg']; $i++) {
                    ...
                    foreach ($free as $block) {
                        if ($block['offset'] != $ptr) {
                            ...
                            if ($h > 0) {
                                $j++;
                                $blockSize = $this->bsize($block['offset'] - $ptr);
                                if ($j < 75) {
                                    fill_box(
                                        $image,
                                        $x,
                        ...
                        if ($h > 0) {
                            $j++;
                            $blockSize = $this->bsize($block['size']);
                            if ($j < 75) {
                                fill_box(
                                    $image,
                                    $x,
                    ...
                    if ($ptr < $mem['seg_size']) {
                        ...
                        if ($h > 0) {
                            $remainingSize = $this->bsize($mem['seg_size'] - $ptr);
                            fill_box(
                                $image,
                                $x,
                                $y,
