...
foreach ($this->cache[$scope] as $i => $entry) {
    ...
    $entryField = isset($entry[$fieldname])
        ? $entry[$fieldname]
        : 'huh';
    $list[$k . $entryField] = $entry;
}
...
if ($list) {
    ...
    foreach ($list as $k => $entry) {
        ...
        if ($searchMatch) {
            ...
            $field_value = htmlentities(
                strip_tags($entry[$fieldname], ''),
                ENT_QUOTES,
                'UTF-8'
            );
            $detailUrl = $this->MY_SELF . '&amp;OB='
                . $obParam . '&amp;SH='
                . md5($entry[$fieldkey]);
            $accessDate = date(
                DATE_FORMAT,
            ...
            if (
                ...
                $delUrl = $this->MY_SELF . '&OB=' . $obParam
                    . '&DU='
