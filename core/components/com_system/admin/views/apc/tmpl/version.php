...
if (defined('PROXY')) {
    $ctxt = stream_context_create(
        array('http' => array('proxy' => PROXY, 'request_fulluri' => true))
    );
...
if (!$rss) {
    ...
    if (version_compare($apcversion, $match[1], '>=')) {
        echo '<p class="message">You are running the latest'
            . ' version of APC (' . $apcversion . ')</p>';
        $i = 3;
    } else {
        $peclUrl = 'http://pecl.php.net/package/APC/' . $match[1];
        echo '<p class="warning">You are running an older'
            . ' version of APC (' . $apcversion . '),'
            . ' newer version ' . $match[1]
            . ' is available at <a href="' . $peclUrl . '">'
            . $peclUrl . '</a></p>';
        $i = -1;
    }
    ...
    while ($v = current($match[2])) {
        ...
        $peclVerUrl = "http://pecl.php.net/package/APC/$ver";
        echo '<b><a href="' . $peclVerUrl . '">'
            . htmlspecialchars($v)
            . '</a></b><br /><blockquote>';
