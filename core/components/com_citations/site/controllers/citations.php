...
class Citations extends SiteController
{
    ...
    public function browseTask()
    {
        ...
        $this->view->filters = array(
            ...
            'reftype'         => Request::getSimpleArray('reftype', array(
                'research' => 1, 'education' => 1, 'eduresearch' => 1, 'cyberinfrastructure' => 1
            )),
            'geo'             => Request::getSimpleArray('geo', array(
                'us' => 1, 'na' => 1, 'eu' => 1, 'as' => 1
            )),
            'aff'             => Request::getSimpleArray('aff', array(
                'university' => 1, 'industry' => 1, 'government' => 1
            )),
        ...
        if (
            !is_string($this->view->filters['startuploaddate'])
            || !preg_match(
        ...
        if (
            $this->view->filters['startuploaddate'] == '0000-00-00'
            || $this->view->filters['startuploaddate'] == '0000-00-00 00:00:00'
            || $this->view->filters['startuploaddate'] == ''
        ) {
            $this->view->filters['startuploaddate'] = '0000-00-00 00:00:00';
        } else {
            $this->view->filters['startuploaddate'] = Date::of($this->view->filters['startuploaddate'])
                ->format('Y-m-d 00:00:00');
        }

        if (
            !is_string($this->view->filters['enduploaddate'])
            || !preg_match(
                "/^([0-9]{4})-([0-9]{2})-([0-9]{2})[ ]([0-9]{2}):([0-9]{2}):([0-9]{2})$/",
        ...
        if (
            $this->view->filters['enduploaddate'] == '0000-00-00'
            ...
            $this->view->filters['enduploaddate'] = Date::of($this->view->filters['enduploaddate'])
                ->format('Y-m-d 00:00:00');
        }
...
/*
    ...
    public function viewTask()
    {
        ...
        $this->view->associations = $this->view->citation->resources()
        ...
        $title = $this->view->citation->title;
        $maxLen = $this->view->maxTitleLength;
        $this->view->shortenedTitle = (strlen($title) > $maxLen)
            ? substr($title, 0, $maxLen) . '&hellip;'
            : $title;
        ...
        Pathway::append(
            $this->view->shortenedTitle,
            'index.php?option=' . $this->_option . '&task=view&id=' . $this->view->citation->id
        );
        ...
        $componentTypeOverride = Component::path('com_citations')
            . DS . 'views' . DS . 'citations' . DS . 'tmpl' . DS . $typeAlias . '.php';
        $tempalteTypeOverride  = App::get('template')->path
            . DS . 'html' . DS . 'com_citations' . DS . 'citations' . DS . $typeAlias . '.php';
    ...
    public function loginTask()
    {
        App::redirect(
            Route::url(
                ...
                . base64_encode(Route::url('index.php?option=' . $this->_option . '&task=' . $this->_task, false, true))
            ),
    ...
    public function editTask($citation = null)
    {
        ...
        if ($id && $id != 0) {
            Pathway::append(
                $shortenedTitle,
                'index.php?option=' . $this->_option . '&task=view&id=' . $this->view->row->id
            );
        }
        Pathway::append(
            Lang::txt('JACTION_EDIT'),
            'index.php?option=' . $this->_option . '&task=edit&id=' . $this->view->row->id
        );
