...
class Format
{
    ...
    protected $_default_format = array(
        'apa'  => "{AUTHORS}, {EDITORS} ({YEAR}), {TITLE/CHAPTER}, <i>{JOURNAL}</i>, "
        ...
        'ieee' => "{AUTHORS}, {EDITORS} ({YEAR}), {TITLE/CHAPTER}, <i>{JOURNAL}</i>, "
                . "<i>{BOOK TITLE}</i>, {EDITION}, {CHAPTER}, {SERIES}, {PUBLISHER}, {ADDRESS}, "
                . "<b>{VOLUME}</b>, <b>{ISSUE/NUMBER}</b>: {PAGES}, {ORGANIZATION}, {INSTITUTION}, "
                . "{SCHOOL}, {LOCATION}, {MONTH}, {ISBN/ISSN}, (DOI: {DOI})"
    );
        ...
        if (count($assocs) > 0) {
            if (count($assocs) > 1) {
                ...
                foreach ($assocs as $rid) {
                    if ($rid->tbl == 'resource') {
                        $query = "SELECT published FROM `#__resources` WHERE id=" . $rid->oid;
                        ...
                        if ($state == 1) {
                            ...
                            if ($internally_cited_image) {
                                ...
                                $rrs[] = '<a class="internally-cited" href="' . $resUrl . '">[' . $k . ']</a>';
                            }
                        }
                    }
                }
                ...
                if ($assocs->first()->tbl == 'resource') {
                    $query = "SELECT published FROM `#__resources` WHERE id=" . $assocs->first()->oid;
                    $database->setQuery($query);
                    $state = $database->loadResult();
                    if ($state == 1) {
    ...
    public static function formatReference(&$row, $link = 'none', $highlight = '')
    {
        ...
        if (self::keyExistsOrIsNotEmpty('title', $row)) {
            if (!$row->url) {
                ...
                $cleanUrl = self::cleanUrl($row->url);
                $titleText = Str::highlight(stripslashes($row->title), $highlight);
                $html .= ', "<a href="' . $cleanUrl . '">' . $titleText . '</a>';
            }
        }
        if (
            self::keyExistsOrIsNotEmpty('journal', $row)
