...
class Citation extends Relational implements \Hubzero\Search\Searchable
{
     ...
     *
     * @param   mixed  $term   string containing search term or an associative
     *                         array containing multiple search terms for multiple columns.
     * @param   int    $depth  value indicating if the produced query string should be
     *                         nested in parenthesis with other querys of the same depth.
    ...
    public function filterBySearch(
        $term,
        $depth = 0,
        $searchableFields = array('title', 'isbn', 'doi', 'abstract', 'author', 'publisher', 'keywords')
    ) {
        if (is_array($term)) {
        ...
        foreach ($template_keys as $k => $v) {
            if (!$this->keyExistsOrIsNotEmpty($k, $this) && $k != 'author') {
                ...
                if ($k == 'doi' && $this->$k) {
                    ...
                    $replace_values[$v] = '<a rel="external" href="https://doi.org/' . $this->$k . '">'
                        . $this->$k . '</a>';
                }
                ...
                if ($k == 'author') {
                    ...
                    // There were mismatch of author names to a citation, reason being
                    // it was using the id to match it with cid in table.
                    ...
                    if ($authorCount > 0) {
                        foreach ($authors as $author) {
                            ...
                            if (is_string($author)) {
                                ...
                                // add author coins
                                $authorClean = trim(preg_replace('/\{\{\d+\}\}/', '', trim($author)));
                                $coins_data[] = 'rft.au=' . $authorClean;
                            } elseif (is_object($author)) { // legacy string -> new ORM method
                                if ($author->uidNumber > 0) {
                                    $url = Route::url('index.php?option=com_members&id=' . $author->uidNumber);
                                    $a[] = '<a rel="external" href="' . $url . '">' . $author->author . '</a>';
                                } else {
                                    $a[] = $author->author;
                                }
                            } else { // fallback
                ...
                if ($k == 'title') {
                    ...
                    $singleCitationView = isset($config['citation_single_view'])
                        ? $config['citation_single_view']
                        : 0;

                    if ($singleCitationView && isset($this->id)) {
                        $url = Route::url('index.php?option=com_citations&task=view&id=' . $this->id);
    ...
    public function formattedResourceLinks()
    {
        if (!$this->resources) {
            ...
            if ($resourceCount > 0) {
                ...
                if ($multiple && !empty($internallyCitedImageMultiple)) {
                    $imageSrc = $internallyCitedImageMultiple;
                } else {
            ...
            foreach ($tags as $tag) {
                if ($tag->tbl == 'citations' && $tag->label == '') {
                    ...
                    if (!$tag->tag->admin || ($tag->tag->admin && $isAdmin)) {
                        $liClass = $tag->tag->admin ? ' class="admin"' : '';
                        $aClass = 'tag' . ($tag->tag->admin ? ' admin' : '');
                        $tagUrl = Route::url('index.php?option=com_tags&tag=' . $tag->tag->tag);
                        $tagName = stripslashes($tag->tag->raw_tag);
                        $html .= '<li' . $liClass . '><a class="' . $aClass . '" href="' . $tagUrl . '">'
                            . $tagName . '</a></li>';
                    }
                }
            }
    ...
    public static function citationOpenUrl($openurl, $citation)
    {
        ...
        if (count($query) > 0) {
            ...
            if ($icon != '') {
                $imgSrc = 'index.php?option=com_citations&controller=citations&task=downloadimage&image=' . $icon;
                $link_text = '<img alt="' . $text . '" src="' . $imgSrc . '" />';
            } else {
                $link_text = $text;
            }
