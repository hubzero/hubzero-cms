...
abstract class Cart
{
    ...
    public static function getAllTransactions($filters = array(), $completedOnly = true)
    {
        ...
        // Get info
        $sql = "SELECT DISTINCT ";
        $hasUserInfo = !empty($filters['userInfo']) && $filters['userInfo'];
        $hasSearch = isset($filters['search']) && $filters['search'];
        if ($hasUserInfo || $hasSearch) {
    ...
    protected function getCartItems()
    {
        ...
        $sql = "SELECT `sId`, `crtiQty`, `crtiPrice`, `crtiOldPrice` FROM `#__cart_cart_items` crti "
            . "WHERE crti.`crtId` = {$this->crtId}";
        $this->_db->setQuery($sql);
        // Get all info
    ...
    protected function doItem($sId, $mode = 'add', $qty = 1, $retainOldValue = false)
    {
        ...
        if ($mode != 'sync') {
            ...
            if (!$skuInfo->pAllowMultiple) {
                ...
                if ((!empty($skuCartInfo->crtiQty) && $skuCartInfo->crtiQty > 0) || ($qty > 1)) {
                    ...
                    throw new \Exception(
                        $skuInfo->pName . " is already in the cart and cannot be added multiple times"
                    );
                }
                    ...
                    foreach ($allSkus as $skuId) {
                        ...
                        if ($skuId != $sId) {
                            ...
                            if (!empty($otherSkuInfo->crtiQty) && $otherSkuInfo->crtiQty > 0) {
                                ...
                                throw new \Exception(
                                    $skuInfo->pName
                                    . " is already in the cart and cannot be added multiple times"
                                );
                            }
                        }
                    }
            ...
            $hasQtyInCart = !empty($skuCartInfo->crtiQty) && $skuCartInfo->crtiQty > 0;
            if (!$skuInfo->sAllowMultiple && ($hasQtyInCart || ($qty > 1))) {
            ...
            if ($skuInfo->sTrackInventory) {
                ...
                if ($qty > $skuInfo->sInventory) {
                    ...
                    throw new \Exception(
                        'You are trying to add too many products to your cart. '
        ...
        $quotedSkuName = $this->_db->quote($skuName);
        $sql = "INSERT INTO `#__cart_cart_items`
				(`crtId`, `sId`, `crtiQty`, `crtiOldQty`, `crtiPrice`, `crtiOldPrice`, `crtiName`)
				VALUES
				({$this->crtId}, '{$sId}', {$qty}, NULL, {$skuInfo->sPrice}, NULL, " . $quotedSkuName . ")
				ON DUPLICATE KEY UPDATE `crtiOldPrice` = `crtiPrice`, `crtiPrice` = {$skuInfo->sPrice},
				`crtiName` = " . $quotedSkuName;
    ...
    /**
     * Update the existing items in the transaction (#__cart_transaction_items).
     * Update can be partial retaining the rest of the info.
     *
     * @param   int    $tId  transaction ID
    ...
    public static function updateTransactionItems($tId, $tiInfo, $returnChanges = true)
    {
        ...
        foreach ($tiInfo as $sId => $sInfo) {
            ...
            foreach ($sInfo as $key => $val) {
                ...
                if ($key != 'meta') {
                    ...
                    if ($returnChanges && $transactionItems[$sId]->$key != $val) {
                        $transactionItemsChanges[] = array(
                            'object' => 'cart_transaction_item',
                            'sId' => $sId,
                            'key' => $key,
                            'old' => $transactionItems[$sId]->$key,
                            'new' => $val
                        );
                    }
                } else {
            ...
            $sql = "UPDATE `#__cart_transaction_items` SET " . $setSql
                . " WHERE `tId` = " . $db->quote($tId) . " AND `sId` = " . $db->quote($sId);
            $db->setQuery($sql);
            $db->query();
        }
    ...
    public static function updateTransactionInfo($tId, $tInfo, $returnChanges = true)
    {
        ...
        foreach ($tInfo as $key => $val) {
            ...
            if ($returnChanges && $currerntTransactionInfo->$key != $val) {
                $transactionChanges[] = array(
                    'object' => 'cart_transaction_info',
                    'tId' => $tId,
                    'key' => $key,
                    'old' => $currerntTransactionInfo->$key,
                    'new' => $val
                );
            }
        }
        ...
        $tiTotal = $subtotal + $currerntTransactionInfo->tiTax + $currerntTransactionInfo->tiShipping
            - $currerntTransactionInfo->tiShippingDiscount - $currerntTransactionInfo->tiDiscounts;
    ...
    public static function killExpiredTransactions()
    {
        ...
        $sql = "SELECT t.tId
				...
				WHERE t.`tStatus` = 'pending' OR t.`tStatus` = 'released'
				AND TIMESTAMPDIFF(MINUTE, t.`tLastUpdated`, NOW()) > {$transactionTTL}";
