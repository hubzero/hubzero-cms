...
class CurrentCart extends Cart
{
        ...
        if ($cart) {
            ...
            if (!$user->get('guest')) {
                if (empty($this->cart->linked) || !$this->cart->linked) {
                    // link carts if not linked
                    // (this should only happen when user logs in with a cart created while not logged in)
    ...
    private function getCartChanges()
    {
        ...
        foreach ($items as $item) {
            ...
            if (!empty($item->crtiOldQty) && $item->crtiOldQty > $item->crtiQty) {
                if ($item->crtiQty) {
                    $changes[] = array(
                        $itemName . ' inventory reduced from ' . $item->crtiOldQty . ' to ' . $item->crtiQty,
                        'info'
                    );
                } else {
            ...
            if (!empty($item->crtiOldPrice) && $item->crtiOldPrice != $item->crtiPrice) {
                $changes[] = array(
                    $itemName . ' price changed from ' . $item->crtiOldPrice . ' to ' . $item->crtiPrice,
                    'info'
                );
            }
        }
    ...
    public function liftTransaction($resetStatus = false)
    {
        ...
        if (!empty($transaction->items)) {
            ...
            foreach ($transaction->items as $transactionItem) {
                $tiInfo = $transactionItem['transactionInfo'];
                $transactionTotalAmount += ($tiInfo->tiPrice * $tiInfo->qty);
            }
    ...
    public function setTransactionShippingInfo()
    {
        ...
        $requiredFields = array(
            'shippingToFirst', 'shippingToLast', 'shippingAddress',
    ...
    public function finalizeTransaction()
    {
        ...
        $tiTotal = $this->tInfo->tiSubtotal + $this->tInfo->tiShipping
     ...
     *
     * @param   string Step
     * @param   mixed Meta key to match the step among multiple of the same type
     *                (eg transaction can have multiple EULA steps for each SKU)
    ...
    public function applyCoupon($cnId)
    {
        ...
        foreach ($coupon->objects as $couponObject) {
            foreach ($cartItems as $sId => $cartItem) {
                if (
                    ($coupon->info->cnObject == 'sku' && $sId == $couponObject->cnoObjectId) ||
        ...
        if (count($coupon->objects) == 1) {
            ...
            if ($coupon->info->cnObject == 'sku') {
                ...
                // See if the product has only one SKU, then add this SKU to cart
                // (There is no way do decide what SKU to add if there are several of them)
    ...
    public function getCouponPerks()
    {
        ...
        */
        ...
        /*
            ...
            The coupons are ordered by time applied, so if there is more than one coupon for
            one SKU, we need to apply the most recently applied only. As we iterate through
            coupons we only want to record the recent one. Same for non item coupons --
            there is only one coupon per object type is allowed (shipping, total order
            discount, etc...)
        */
        ...
        */
            ...
            if ($itemCoupon) {
                ...
                foreach ($coupon->objects as $couponObject) {
                    foreach ($cartItems as $sId => $cartItem) {
                        // try to find a match
                        if (
                            ($couponObjectType == 'sku' && $sId == $couponObject->cnoObjectId) ||
                            ...
                            if ($couponAction == 'discount') {
                                ...
                                */
                                ...
                                if ($couponDiscountUnit == 'absolute') {
                                    ...
                                    $itemPrice = $cartItem['info']->sPrice;
                                    $discountAmount = ($itemPrice * $applyPerkToQty) *  ($couponDiscount / 100);
                                }
                                ...
                                throw new \Exception(
                                    Lang::txt('Invalid coupon. Only discounts are available for SKUs and products')
                                );
                            }
                            ...
                            // no break for products -- keep going maybe there are
                            // several SKUs of the same product in the cart
                        }
                        // No match
                    }
                }

                // calculate total item discounts, the final correct value will be
                // calculated on the last iteration of the parent loop
                ...
                // calculate total generic discounts, the final correct value will be
                // calculated on the last iteration of the parent loop
                foreach ($perks['generic'] as $perkDiscount) {
                    $genericDiscountsTotal += $perkDiscount->discount;
                }
            } elseif ($couponObjectType == 'shipping') {
                // Coupon is for shipping -- handled differently to be applied
                // later after the shipping cost is calculated
                // Initialize the perk
                unset($perk);
                $perk = new \stdClass();
                $perk->name = $cn->cnDescription;
        ...
        /*
            At this point $couponMappings has all the coupons that need to be applied to
            cart -- all other unused coupons have to be released
        */
    ...
    public function getMembershipInfo()
    {
        ...
        foreach ($cartItems as $sId => $item) {
            if (in_array($item['info']->ptId, $membershipTypes) && !empty($item['meta']['ttl'])) {
                ...
                $subscription = \Components\Storefront\Models\Memberships::getSubscriptionObject(
                    $type,
                    $itemInfo->pId,
                    $user->id
                );
    ...
    /**
     * Get all items in the cart if cart exists, if cart doesn't exist reset session data and
     * create a new cart. This is primarily for cases when session cart doesn't exist anymore.
     * In most cases cart will be there.
     *
     * @param void
     * @return array of items in the cart
     */
    ...
    /**
     * Update cart based on current availability (and pricing) and return all items in the cart,
    ...
    private function populateTransaction()
    {
        ...
        foreach ($cartItems as $sId => $skuInfo) {
            ...
            if ($productInfo->ptModel == 'software') {
                ...
                // If EULA is needed, add step, note the EULA required for each SKU, so for
                // multiple SKUs of the same product, multiple EULA will be required
                if ($eulaRequired) {
                    $step = new \stdClass();
                    $step->name = 'eula';
                    $step->meta = $sId;
    ...
    {
        // Go through each item in the transaction and see if it is available
        // in quantity needed and if price has not changed
