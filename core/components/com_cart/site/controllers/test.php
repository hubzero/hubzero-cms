...
class Test extends ComponentController
{
    ...
    public function testgroundTask()
    {
        ...
        if (0) {
            ...
            try {
                ...
                // second parameter [optional, unlimited by default]: max quantity of products
                // coupon will be applied to (if buying multiple)
        ...
        if (0) {
            ...
            // Existing course ID (pID returned with $course->add() when the course was created).
            // Must be set to be able to update.
            $course->setId(1023);
            try {
                $info = $course->update();
                //print_r($info);
    ...
    public function payTask()
    {
        if (!empty($_POST['dummypay'])) {
            ...
            // OMG!!! Ok, there was no way I could cURL post to the hub, so I post to the
            // external server that in turn posts back to the hub. I KNOW!!!!!
