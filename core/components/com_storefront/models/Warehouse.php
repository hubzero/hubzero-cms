...
class Warehouse extends \Hubzero\Base\Obj
{
    ...
    {
        return $this->fetchCollections('category', $filters);
    }
    ...
    public function getCategories($rtrn = 'list', $filters = array())
    {
        ...
        return $this->fetchAllCollections($rtrn, $filters);
    }
    ...
    public function checkProduct($product, $showInactive = false)
    {
        ...
        if (\Hubzero\Facades\Component::params('com_storefront')->get('productAccess')) {
    ...
    {
        $useAccessGroups = \Hubzero\Facades\Component::params('com_storefront')->get('productAccess');
        ...
        $sql = "SELECT DISTINCT p.*, pt.ptName, i.imgName FROM `#__storefront_products` p
				...
				LEFT JOIN `#__storefront_images` i
				ON (p.`pId` = i.`imgObjectId` AND i.`imgObject` = 'product' AND i.`imgPrimary` = 1)";
        /*if ($useAccessGroups)
        {
        ...
        if ($useAccessGroups) {
            if ($this->accessGroupsScope) {
                ...
                $accessGroupsList = implode(',', $this->accessGroupsScope);
                $sql .= " AND (
					p.`pId` IN (SELECT ag.pId FROM `#__storefront_product_access_groups` ag
						WHERE ag.`exclude`=0 and ag.agId IN(" . $accessGroupsList . ")) AND
					p.`pId` NOT IN (SELECT ag2.pId FROM `#__storefront_product_access_groups` ag2
						WHERE ag2.`exclude`=1 and ag2.agId IN(" . $accessGroupsList . "))
				)";
            }
        } else {
            if ($this->accessLevelsScope) {
        ...
        // WhiteList logic: run the expensive query twice, find the whitlisted SKUs for each
        // product, white list the corresponding product, keep the white listed products,
        // kill the non-authorized products (not very efficient)
        if ($this->userScope && ($useAccessGroups || $this->accessLevelsScope)) {
            ...
            foreach ($allProducts as $pId => $product) {
                if (!array_key_exists($pId, $products)) {
                    ...
                    $whitelistedSkus = \Components\Storefront\Admin\Helpers\RestrictionsHelper
                        ::checkWhitelistedSkusUser($this->userScope, $allProductSkus);
    ...
    public function getProductOptions($pId)
    {
        ...
        if ($this->userScope) {
            ...
            $sql .= " AND pr.scope = 'sku' AND pr.uId = '{$this->userScope}')";
        }

        $sql .= "	WHERE s.`pId` = {$pId} AND s.`sActive` = 1";
        $sql .= " 	AND (s.`publish_up` IS NULL OR s.`publish_up` <= NOW())";
        $sql .= " 	AND (s.`publish_down` IS NULL OR s.`publish_down` = '0000-00-00 00:00:00'";
        $sql .= " OR s.`publish_down` > NOW())";
    ...
    public function mapSku($pId, $options, $throwExceptionOnNomatch = true)
    {
        ...
        $sql = "SELECT COUNT(pog.`ogId`) AS cnt FROM `#__storefront_product_option_groups` pog";
        $sql .= " WHERE pog.`pId` = '{$pId}'";
    ...
    public function getSkusInfo($skus, $showInactive = false, $filters = false)
    {
        ...
        if (!$showInactive) {
            ...
            $sql .= " AND (p.`publish_down` IS NULL OR p.`publish_down` = '0000-00-00 00:00:00'";
            $sql .= " OR p.`publish_down` > NOW())";
        ...
        /*
            Parse the result and organize it by SKU (since same SKU can be returned several
            times, depending on the number of options):
    ...
    public function getSkuInfo($sId, $showInactive = true)
    {
        ...
        if ($this->accessLevelsScope) {
            if (!in_array($sInfo['info']->access, $this->accessLevelsScope)) {
                $errorMsg = Lang::txt('COM_STOREFRONT_PRODUCT_ACCESS_NOT_AUTHORIZED');
                $errorMsg .= ': ' . $sInfo['info']->pName . ', ' . $sInfo['info']->oName;
    ...
    {
        $this->doCoupon($coupon, 'add');
    }

    public function updateCoupon($coupon)
    {
        $this->doCoupon($coupon, 'update');
    }
     ...
     */
    private function doCoupon($coupon, $action)
    {
        $allowedActions = array('add', 'update');
        ...
        $deleteSql .= ')';
        $sql = "DELETE FROM `#__storefront_coupon_objects` WHERE `cnId` = ";
        $sql .= $this->_db->quote($cnId) . " AND `cnoObjectId` NOT IN {$deleteSql}";
        $this->_db->setQuery($sql);
        $this->_db->query();
     ...
     * @return array
     */
    private function fetchCollections($collectionType = 'category', $filters = false)
    {
        $sql = "SELECT DISTINCT c.`cId`, c.`cAlias`, c.`cName`, i.imgName
				...
				LEFT JOIN `#__storefront_images` i
				ON (c.`cId` = i.`imgObjectId` AND i.`imgObject` = 'collection' AND i.`imgPrimary` = 1)
     ...
     * @return array|int
     */
    private function fetchAllCollections($return = 'list', $filters = array())
    {
        $sql = "SELECT c.*
				FROM `#__storefront_collections` c
