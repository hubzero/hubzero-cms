...
class Sku
{
    ...
    public function verify()
    {
        ...
        // At this point option groups options set must be the same
        // as the product options groups, throw exception if not
    ...
    public function save()
    {
        ...
        $sql = "DELETE FROM `#__storefront_sku_meta`";
        $sql .= " WHERE `sId` = " . $db->quote($sId);
        $sql .= " AND `smId` NOT IN {$deleteSql}";
        $db->setQuery($sql);
        $db->query();
    ...
    */
    ...
    // Update all SKUs' references for a given product
    // Called when the product is updated to bring the SKUs up to speed
