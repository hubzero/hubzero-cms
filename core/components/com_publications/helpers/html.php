...
class Html
{
    ...
    public static function getThumb($pid = 0, $versionid = 0, $config = null, $force = false, $cat = '')
    {
        ...
        // Get default picture
        $toolPicDefault = 'components/com_publications/site/assets/img/tool_thumb.gif';
        $resourcePicDefault = 'components/com_publications/site/assets/img/resource_thumb.gif';
        $default = $cat == 'tools'
                ? trim($config->get('toolpic', $toolPicDefault), DS)
                : trim($config->get('defaultpic', $resourcePicDefault), DS);
     ...
     * @return  string
     */
    public static function showContributors(
        $contributors = '',
        $showorgs = false,
        $showaslist = false,
        $incSubmitter = false,
    ...
    public static function citation($cite, $pub, $citations)
    {
        ...
        $template = "{AUTHORS} ({YEAR}). <b>{TITLE/CHAPTER}</b>. <i>{JOURNAL}</i>, <i>{BOOK TITLE}</i>, "
            . "{EDITION}, {CHAPTER}, {SERIES}, {ADDRESS}, <b>{VOLUME}</b>, <b>{ISSUE/NUMBER}</b> {PAGES}, "
            . "{ORGANIZATION}, {INSTITUTION}, {SCHOOL}, {LOCATION}, {MONTH}, {ISBN/ISSN}. {VERSION}. "
            . "{PUBLISHER}. doi:{DOI}";
        ...
        if ($cite) {
            ...
            $html .= $formatted;
            if (!$pub->isDev()) {
                $bibtexUrl = Route::url($pub->link('citation') . '&task=citation&type=bibtex&no_html=1');
                $endnoteUrl = Route::url($pub->link('citation') . '&task=citation&type=endnote&no_html=1');
    ...
    public static function showVersionInfo($publication)
    {
        ...
        if ($publication->version->get('doi')) {
            $citeUrl = Route::url($publication->link() . '&active=about') . '#citethis';
            $text .= "\t\t" . '<span class="doi">' . 'doi:' . $publication->version->get('doi');
            $text .= ' - <span><a href="' . $citeUrl . '">'
                . Lang::txt('cite this') . '</a></span></span>' . "\n";
        }
        ...
        if ($publication->config('repository')) {
            if ($publication->version->get('doi') && $publication->archived()) {
                ...
                if ($archiveDate) {
                    $archiveDateFormatted = Date::of($archiveDate)->toLocal(Lang::txt('DATE_FORMAT_HZ1'));
                    $text .= "\t\t" . '<span class="archival-notice unarchived">'
                        . Lang::txt('COM_PUBLICATIONS_VERSION_TO_BE_ARCHIVED') . ' '
        ...
        if ($lastPubRelease && $lastPubRelease->id != $publication->version->get('id')) {
            ...
            . ' <a href="' . Route::url(
                $publication->link() . '&v=' . $lastPubRelease->version_number
            ) . '">' . $lastPubRelease->version_label . '</a></span>';
        }
    ...
    public static function showAccessMessage($publication)
    {
        ...
        if (!$publication->access('view-all')) {
            ...
            switch ($publication->version->get('state')) {
                case 1:
                    $m  = $publication->isDown()
                        ? Lang::txt('COM_PUBLICATIONS_STATUS_MSG_UNPUBLISHED')
                        : Lang::txt('COM_PUBLICATIONS_STATUS_MSG_PUBLISHED');
                    $m .= ' ';
                    ...
                    if ($publication->isEmbargoed()) {
                        $embargoDate = Date::of($publication->published_up)->toLocal('m d, Y');
                        $m = Lang::txt('COM_PUBLICATIONS_STATUS_MSG_PUBLISHED_EMBARGO') . ' ' . $embargoDate;
                    }
            ...
            if ($publication->access('owner')) {
                ...
                $editUrl = Route::url($publication->link('editversion'));
                $manageText = Lang::txt('COM_PUBLICATIONS_STATUS_MSG_MANAGE_PUBLICATION');
                $msg .= ' <a href="' . $editUrl . '">' . $manageText . '</a>.';
            }
        }
    ...
    public static function drawLauncher(
        ...
        if ($disabled) {
            ...
            $archiveUrl = Route::url(
                ...
                . '&task=serve&v=' . $pub->version_number . '&render=archive'
            );
    ...
    public static function showPubTitle($pub, $tabtitle = '', $append = '')
    {
        ...
        $editbaseUrl = Route::url($pub->link('editbase'));
        $truncatedTitle = \Hubzero\Utility\Str::truncate($pub->get('title'), 65);

        // Build the title link content
        $titleContent = '"';
        if ($append) {
            $titleContent .= '<a href="' . $pubUrl . '" >';
        }
        $titleContent .= $truncatedTitle . '"';
        if ($append) {
            $titleContent .= '</a>';
        }
        ?>
        <div id="plg-header">
            <h3 class="publications c-header">
                <a href="<?php echo $editbaseUrl; ?>" title="<?php echo $tabtitle; ?>">
                    <?php echo $tabtitle; ?>
                </a>
                &raquo;
                <span class="restype indlist"><?php echo $typetitle; ?></span>
                <span class="indlist"><?php echo $titleContent; ?></span>
                <?php if ($append) {
                    echo $append;
                } ?>
            </h3>
    ...
    {
        $editbaseUrl = Route::url($pub->link('editbase'));
        $submissionsText = ucfirst(Lang::txt('PLG_PROJECTS_PUBLICATIONS_MY_SUBMISSIONS'));
        $truncatedTitle = \Hubzero\Utility\Str::truncate($pub->get('title'), 65);
        ?>
        <h3 class="prov-header">
            <a href="<?php echo $editbaseUrl; ?>">
                <?php echo $submissionsText; ?>
            </a>
            &raquo; "<?php echo $truncatedTitle; ?>"
            <?php if ($append) {
                echo $append;
            } ?>
        </h3>
    ...
    public static function getMimeTypes(
        ...
        if ($pubType == 1) {
            self::getMIMEtypesOfPrimarySupportFiles(
                $publication_id,
                $publication_version_id,
                $secret,
                1,
        ...
        if (is_array($attachments) && array_key_exists(3, $attachments)) {
            self::getMIMEtypesOfGalleryFile(
                $publication_id,
                $publication_version_id,
                $mimeTypes,
                $fast
     ...
     */
    public static function getMIMEtypesOfPrimarySupportFiles(
        $publication_id,
        $publication_version_id,
        $secret,
        $role,
     ...
     * @return  null or false
     */
    public static function getMIMEtypesOfGalleryFile(
        $publication_id,
        $publication_version_id,
        &$mimeTypes,
        $fast = false
