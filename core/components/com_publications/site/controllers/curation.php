...
class Curation extends SiteController
{
    ...
    {
        if (!$this->_title) {
            $optionKey = strtoupper($this->_option);
            $controllerKey = strtoupper($this->_option . '_' . $this->_controller);
            $this->_title = Lang::txt($optionKey) . ': ' . Lang::txt($controllerKey);
        }
        ...
        if ($confirm) {
            ...
            if (!$this->getError()) {
                ...
                if ($owner && $owner != $previousOwner) {
                    ...
                    $baseUrl = rtrim(Request::base(), DS) . DS;
                    $curateUrl = $baseUrl . trim(Route::url($this->_pub->link('curate')), DS);
                    $previewUrl = $baseUrl . trim(Route::url($this->_pub->link('version')), DS);
                    $message = Lang::txt('COM_PUBLICATIONS_CURATION_EMAIL_ASSIGNED') . ' ' . $item . "\n\n";
                    $message .= Lang::txt('COM_PUBLICATIONS_CURATION_EMAIL_ASSIGNED_CURATE')
                        . ' ' . $curateUrl . "\n\n";
                    $message .= Lang::txt('COM_PUBLICATIONS_CURATION_EMAIL_ASSIGNED_PREVIEW')
                        . ' ' . $previewUrl;
                ...
                if (!$this->getError() && $owner != $previousOwner) {
                    ...
                    if (!empty($ownerProfile)) {
                        $changelog = '<p>Curation assigned to ' . $ownerProfile->get('name')
                            . ' (' . $ownerProfile->get('username') . ')</p>';
                    } else {
    ...
    public function approveTask()
    {
        ...
        if (
            !$this->getError() && !$this->config->get('graceperiod', 0)
        ...
        $paddedPid = \Hubzero\Utility\Str::pad($pid);
        $paddedVid = \Hubzero\Utility\Str::pad($vid);
        $doiFileName = str_replace(['.', '/'], '_', $this->_pub->version->doi) . ".zip";
        $zipPath = PATH_APP . DS . $webpath . DS . $paddedPid . DS . $paddedVid . DS . $doiFileName;
        ...
        $updateDoiMetadata = false;
        if ($this->_pub->version->doi) {
            $shoulder = $doiService->configs()->shoulder;
            $updateDoiMetadata = preg_match("/" . $shoulder . "/", $this->_pub->version->doi);
        }
        ...
        if ($updateDoiMetadata) {
            ...
            if ($doiService->getError()) {
                $errorMsg = Lang::txt('COM_PUBLICATIONS_CURATION_ERROR_UPDATE_METADATA')
                    . ' ' . $doiService->getError();
            ...
            if ($doiService->getError()) {
                $errorMsg = Lang::txt('COM_PUBLICATIONS_CURATION_ERROR_REGISTER_URL')
                    . ' ' . $doiService->getError();
                throw new Exception($errorMsg, 403);
            }
        }
    ...
    public function onAfterStatusChange()
    {
        ...
        // Create SFTP accessible link for package
        ...
        if ($status == 1) {
            $message = Lang::txt('COM_PUBLICATIONS_CURATION_EMAIL_CURATOR_APPROVED', $link);
        } else {
            $message = Lang::txt('COM_PUBLICATIONS_CURATION_EMAIL_CURATOR_KICKED_BACK');
        }
