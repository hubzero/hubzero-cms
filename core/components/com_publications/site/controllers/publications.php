...
class Publications extends SiteController
{
    ...
    protected function _buildPathway()
    {
        ...
        if (
            !empty($this->model) && $this->_identifier && ($this->_task == 'view'
    ...
    {
        $defaultUrl = Route::url('index.php?option=' . $this->_option . '&task=' . $this->_task);
    ...
    public function pageTask()
    {
        ...
        if (
            $this->model->masterExists() && !$this->model->exists()
        ...
        if ($this->model->_category->alias == 'series' && $this->config->get('show_ranking')) {
            $defaultsort = 'ranking';
        }
    ...
    public function serveTask()
    {
        ...
        if (
            !isset($this->model->_attachments['elements'][$elementId])
    ...
    public function licenseTask()
    {
        ...
        $this->view
            ...
            ->set(
                'title',
                stripslashes($this->model->version->get('title')) . ': ' . Lang::txt('COM_PUBLICATIONS_LICENSE')
            )
            ->setErrors($this->getErrors())
            ->display();
    }
    ...
    public function forkTask()
    {
        ...
        if (!$version->license->get('derivatives')) {
            ...
            App::redirect(
                Route::url(
                    'index.php?option=com_publications&id=' . $version->get('publication_id')
                        . '&v=' . $version->get('id'),
                    false
                )
            );
        ...
        foreach ($attachments as $attachment) {
            ...
            if ($attachment->get('type') == 'file') {
                ...
                $attachBasename = basename($attachment->get('path'));
                $file3 = \Components\Projects\Helpers\Html::fixFileName($attachBasename, '-' . $oldid);
                $file3new = \Components\Projects\Helpers\Html::fixFileName(
                    $attachBasename,
                    '-' . $attachment->get('id')
                );
                ...
                if (!file_exists($from . $file)) {
                    ...
                    if (file_exists($from . $file2)) {
                        ...
                        $galleryDir = $galleryParams['directory'];
                        $pathSuffix = $path ? $path . '/' : '';
                        $from  = dirname($pubfilespace) . '/' . $galleryDir . '/' . $pathSuffix;
                        $toPub = dirname($newpubfilespace) . '/' . $galleryDir . '/' . $pathSuffix;

                        if (!file_exists($from . $file)) {
                            // Let's try an alternate file name
                            if (!file_exists($from . $file2)) {
                                $galleryFile3Path = dirname($pubfilespace) . DS . $galleryDir . DS . $file3;
                                if (!file_exists($galleryFile3Path)) {
                                    Notify::error(Lang::txt('File does not exist: %s', $from . $file2));
                                    continue;
                                } else {
                                    $from = dirname($pubfilespace) . DS . $galleryDir . DS;
                                    $file = $file3;
                                    $filenew = $file3new;
                                    $toPub  = $newpubfilespace . DS . $galleryDir . DS;
                                }
                            } else {
                                // Found it
                ...
                foreach ($source as $type => $filename) {
                    ...
                    if ($type == 'main') {
                        ...
                        // Note: gallery items' filenames will have a suffix with attachment record ID
                        // but the database entry will point to the un-suffixed filename.
                        // DB: foo.jpg (in project space) = foo-123.jpg (in publication space)
                        $srcFile = $from . $filename;
                        $destFile = $toProj . $orig;
                        if (!Filesystem::copy($srcFile, $destFile)) {
                            App::abort(500, Lang::txt('Failed to copy file "%s" to "%s"', $srcFile, $destFile));
                        }
        ...
        $pubUrl = Route::url('index.php?option=com_publications&id=' . $pub_id . '&v=' . $vnum);
        $pubLink = '<a href="' . $pubUrl . '">' . $version->get('title') . '</a>';
        Event::trigger('system.logActivity', [
            'activity' => [
                'action'      => 'forked',
                'scope'       => 'publication',
                'scope_id'    => $pub_id,
                'description' => Lang::txt('COM_PUBLICATIONS_ACTIVITY_ENTRY_FORKED', $pubLink),
                'details'     => array(
        ...
        if ($pid) {
            $redirectUrl = $project->link('publications')
                . '&pid=' . $version->get('publication_id')
        ...
        $submitUrl = 'index.php?option=' . $this->_option
            . '&task=submit&pid=' . $version->get('publication_id')
            . '&version=' . $version->get('version_number');
        App::redirect(
            Route::url($submitUrl, false)
        );
    }
    ...
    public function compareTask()
    {
        ...
        if (
            !$lversion->get('id') || $lversion->isDeleted()
            || !$lpublica->get('id')
        ) {
            Notify::error(Lang::txt('COM_PUBLICATIONS_RESOURCE_NOT_FOUND'));
