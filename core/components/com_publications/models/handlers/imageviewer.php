...
class ImageViewer extends Base
{
    ...
    public function getConfig($savedConfig = array())
    {
        ...
        $parsedConfig = $this->_parent->parseConfig($this->_name, $configs, $savedConfig);
        $this->_config = json_decode(json_encode($parsedConfig), false);
        return $this->_config;
    }
    ...
    public function showImageBand($pub)
    {
        ...
        foreach ($attachments as $attach) {
            ...
            if (is_file(PATH_APP . DS . $fpath) && is_file(PATH_APP . DS . $thumbPath)) {
                ...
                $imgSrc = Route::url($pub->link('versionid')) . '/Image:' . $thumbName;
                $html .= '<img src="' . $imgSrc . '" alt="' . $title . '" class="thumbima" /></a>';
    ...
    public function drawList($attachments, $attConfigs, $pub, $authorized)
    {
        ...
        foreach ($attachments as $attach) {
            ...
            $html .= '<li>';
            $basePath = '/publications' . DS . $pub->id . DS . $pub->version_id;
            $html .= ' <a rel="lightbox" href="' . $basePath . '/Image:' . basename($fpath) . '">';
            $html .= '<span class="item-image';
            $html .= $params->get('pubThumb', null) && $authorized == 'administrator' ? ' starred' : '';
            $imgSrc = $basePath . '/Image:' . $thumbName;
            $html .= '"><img src="' . $imgSrc . '" alt="' . $title . '" class="thumbima" /></span>';
    ...
    public function getFilePath($path, $id, $configs = null, $params = null, $suffix = null)
    {
        ...
        if ($configs->dirHierarchy == 1) {
            ...
            $name = $suffix
                ? \Components\Projects\Helpers\Html::fixFileName($baseName, ' (' . $suffix . ')')
                : $baseName;
