...
class Curation extends Obj
{
    ...
    protected $readmeFileName = 'hubREADME.txt';
    ...
    private function _setBlocks($manifest = null, $masterManifest = null)
    {
        ...
        foreach ($manifest->blocks as $blockId => $b) {
            ...
            if ($masterManifest) {
                $masterBlock = $masterManifest->blocks->$blockId ?? null;
                $hasBParam = isset($b->params->published_editing);
                $hasMasterParam = isset($masterBlock->params->published_editing);
                if ($hasBParam && $hasMasterParam) {
                    $b->params->published_editing = $masterBlock->params->published_editing;
                }

                // Elements settings
                if (!empty($b->elements) && !empty($masterBlock->elements)) {
                    foreach ($b->elements as $elementId => $el) {
                        $masterElement = $masterBlock->elements->$elementId ?? null;
                        $hasElParams = isset($el->params);
                        $hasMasterElParam = isset($masterElement->params->published_editing);
                        if ($hasElParams && $hasMasterElParam) {
                            $el->params->published_editing = $masterElement->params->published_editing;
                        }
                    }
                }
            }
    ...
    */
    ...
    public function setProgress()
    {
        ...
        foreach ($this->_blocks as $blockId => $block) {
            ...
            $autoStatusPositive = $autoStatus->status > 0;
            $reviewStatusNotZero = $reviewStatus->status != 0;
            $reviewStatusOne = $reviewStatus->status == 1;
            if (($autoStatusPositive && $reviewStatusNotZero) || $reviewStatusOne || $reviewStatus->lastupdate) {
                $i++;
            }
            ...
            if ($reviewStatus) {
                if ($block->elements) {
                    foreach ($block->elements as $elementId => $element) {
                        ...
                        $autoElStatus = $autoStatus->elements->$elementId->status;
                        $reviewElStatus = $reviewStatus->elements->$elementId->status;
                        if ($autoElStatus == 0 && $reviewElStatus == 2) {
                            $i--;
    ...
    public function getCurationStatus($pub, $step, $elId = 0, $viewer = 'author')
    {
        ...
        if ($status->updated && isset($reviewStatus->updated_by) && $reviewStatus->updated_by) {
            ...
            if ($status->status != 3) {
                $updatedText = Lang::txt('COM_PUBLICATIONS_CURATION_UPDATED');
                $dateFormatted = Date::of($status->updated)->format('M d, Y @ H:i');
                $status->updatenotice = $updatedText . ' ' . $dateFormatted . $by;
            } else {
        ...
        <div class="status-notice">
            ...
            if ($viewer == 'curator') {
                ...
                if ($curatorStatus->authornotice && $curatorStatus->updated) {
                    ?>
                    <?php
                    $deleteText = Lang::txt('COM_PUBLICATIONS_CURATION_DISPUTE_DELETE');
                    $disputeNotice = Lang::txt('COM_PUBLICATIONS_CURATION_DISPUTE_NOTICE');
                    ?>
                    <span class="dispute-notice">
                        <span class="remove-notice" id="<?php echo $props; ?>">
                            [<a href="#<?php echo $elName; ?>"><?php echo $deleteText; ?></a>]
                        </span>
                        <?php echo $disputeNotice; ?>
            ...
            <?php if ($viewer == 'author' && $curatorStatus->curatornotice && !$curatorStatus->updated) {
                $disputeThis = Lang::txt('PLG_PROJECTS_PUBLICATIONS_CURATION_DISPUTE_THIS');
                ?>
                <span class="disputeit" id="<?php echo $props; ?>">
                    [<a href="#<?php echo $elName; ?>"><?php echo $disputeThis; ?></a>]
                </span>
            <?php } ?>
            <?php
            $failNoticeText = $viewer == 'curator'
                ? Lang::txt('COM_PUBLICATIONS_CURATION_NOTICE_TO_AUTHORS')
                : Lang::txt('COM_PUBLICATIONS_CURATION_CHANGE_REQUEST');
            ?>
            <span class="fail-notice"><?php echo $failNoticeText; ?></span>
    ...
    public function drawChecker($props, $reviewStatus, $url, $title = '')
    {
        ...
        $passClass = ($status == 1) ? 'picked' : '';
        $passClass .= $updated ? ' updated' : '';
        $failClass = ($status == 0) ? 'picked' : '';
        $failClass .= $updated ? ' updated' : '';
        $approveTitle = Lang::txt('COM_PUBLICATIONS_CURATION_APPROVE');
        $notApproveTitle = Lang::txt('COM_PUBLICATIONS_CURATION_NOT_APPROVE');
        ?>
        <div class="block-checker" id="<?php echo $props; ?>" rel="<?php echo $title; ?>">
            <span class="checker-pass <?php echo $passClass; ?>">
                <a href="<?php echo $url; ?>" title="<?php echo $approveTitle; ?>"></a>
            </span>
            <span class="checker-fail <?php echo $failClass; ?>">
                <a href="#addnotice" title="<?php echo $notApproveTitle; ?>"></a>
            </span>
        </div>
        <?php
    }
    ...
    public function getChangeLog($oldStatus = 0, $newStatus = 0, $curator = 0)
    {
        ...
        if (!empty($this->_progress) && ($newStatus == 7 || $oldStatus == 7)) {
            ...
            foreach ($this->_progress->blocks as $blockId => $block) {
                $changesRequested = $newStatus == 7 && $block->review->status == 0;
                $hasUpdates = $oldStatus == 7 && $block->review->lastupdate;
                if ($block->review && ($changesRequested || $hasUpdates)) {
                    $changelog .= '<li>';
    ...
    public function showPackageContents()
    {
        ...
        if (file_exists($bundle)) {
            $bundleContentLabel = Lang::txt('COM_PUBLICATIONS_BUNDLE_CONTENT');
            $bundleSize = \Hubzero\Utility\Number::formatBytes(filesize($bundle));
            $bundleMd5 = md5_file($bundle);
            $checksumLabel = Lang::txt('COM_PUBLICATIONS_BUNDLE_CHECKSUM');
            $contents .= '<div class="grid bundle-meta">
				<div class="col span4">
					<ul class="bundle-info">
						<li>' . $bundleContentLabel . '</li>
						<li><span class="bundle-size">' . $bundleSize . '</span></li>
					</ul>
				</div>
				<div class="col span8 omega">
					<div class="bundle-checksum">
						<span class="bundle-checksum-value">md5:' . $bundleMd5 . '</span>
						<span class="bundle-checksum-help icon-help tooltips" title="'
                            . $checksumLabel . '">' . $checksumLabel . '</span>
					</div>
				</div>
			</div>';
        }
        ...
        if ($element && $shots) {
            ...
            if (is_dir($galleryPath)) {
                foreach ($shots as $shot) {
                    ...
                    $fileExists = is_file($galleryPath . DS . $shot->srcfile);
                    $attachmentParams = array('path' => $shot->filename);
                    $elementRole = $element->manifest->params->role;
                    $attachmentLoaded = $objPA->loadElementAttachment(
                        $pub->version_id,
                        $attachmentParams,
                        $element->id,
                        'file',
                    ...
                    if ($fileExists && !$attachmentLoaded) {
                        $objPA = new Tables\Attachment($this->_db);
