...
class Metadata extends Base
{
  ...
  */
    ...
    public function getStatus($manifest, $pub = null)
    {
        ...
        $data = array();
        $metadataStr = $pub->metadata == null ? '' : $pub->metadata;
        preg_match_all("#<nb:(.*?)>(.*?)</nb:(.*?)>#s", $metadataStr, $matches, PREG_SET_ORDER);
        if (count($matches) > 0) {
