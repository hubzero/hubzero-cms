...
class Data extends Base
{
    ...
    public function getConfigs($element, $elementId, $pub, $blockParams)
    {
        ...
        // Old: Route::url('index.php?option=com_publications&id=' . $pub->id .
        //       '&task=serve&v=' . $pub->version_number);
        $configs->servePath = Route::url($pub->link('data'));
        // Old: Route::url('index.php?option=com_publications&id=' . $pub->id) .
        //       '/?vid=' . $pub->version_id . '&amp;task=serve';
    ...
    public function drawList($attachments, $element, $elementId, $pub, $blockParams, $authorized)
    {
        ...
        $serveLink .= '&v=' . $pub->version_number . '&el=' . $elementId;
        $url = Route::url($serveLink);
        ...
        if ($attachments) {
            ...
            foreach ($attachments as $attach) {
                ...
                $html .= '<a href="' . $itemUrl . '" title="' . $pop . '" ';
                $html .= 'rel="noopener" target="_blank" class="data-type">' . $title . '</a>';
                $html .= '</li>';
            }
        }
    ...
    public function drawLauncher($element, $elementId, $pub, $blockParams, $elements, $authorized)
    {
        ...
        if ($role == 1) {
            ...
            if (file_exists($pub->bundlePath()) && $pub->base == 'databases') {
                ...
                $show->href = Route::url($archiveLink);
    ...
    public function transferData($elementparams, $elementId, $pub, $blockParams, $attachments, $oldVersion, $newVersion)
    {
        ...
        $serveBase = Route::url('index.php?option=com_publications&id=' . $pub->id);
        $newConfigs->servePath = $serveBase . '/?vid=' . $newVersion->id . '&amp;task=serve';
        ...
        foreach ($attachments as $att) {
            ...
            $result = Event::trigger(
                'projects.clone_database',
                array($pAttach->object_name, $pub->_project, $newConfigs->servePath)
            );
            ...
            $dataPath = 'dataviewer' . DS . 'view' . DS . 'publication:dsl' . DS;
            $pAttach->path = $dataPath . $pAttach->object_name . DS . '?v=' . $dbVersion;
            $pAttach->store();
        }
    ...
    public function addAttachment($database_name, $pub, $configs, $uid, $elementId, $element, $ordering = 1)
    {
        ...
        if (
            $objPA->loadElementAttachment(
                $pub->version_id,
                array('object_name' => $database_name),
                $elementId,
                $this->_name,
                $element->role
            )
        ...
        if ($new) {
            $result = Event::trigger(
                'projects.clone_database',
                array($database_name, $pub->_project, $configs->servePath)
            );
            $dbVersion = $result && isset($result[0]) ? $result[0] : null;
        } else {
        ...
        // Failed to clone
        if (!$dbVersion) {
            $errMsg = Lang::txt('PLG_PROJECTS_PUBLICATIONS_ERROR_FAILED_DB_CLONE');
            $this->_parent->setError($errMsg);
            return false;
        }
        ...
        // Build link path
        $dataPath = 'dataviewer' . DS . 'view' . DS . 'publication:dsl' . DS;
        $objPA->path = $dataPath . $database_name . DS . '?v=' . $dbVersion;
    ...
    public function removeAttachment($row, $element, $elementId, $pub, $blockParams)
    {
        ...
        if (!$this->getError()) {
            $result = Event::trigger(
                'projects.remove_database',
                array($row->object_name, $pub->_project, $row->object_revision)
            );
    ...
    public function publishDataFiles($objPD, $configs)
    {
        ...
        if (!empty($files)) {
            foreach ($files as $file) {
                ...
                $srcFile = $repoPath . DS . $file;
                $destFile = $configs->dataPath . DS . $file;
                if (Filesystem::copy($srcFile, $destFile)) {
                    ...
                    if (stripos(Filesystem::mimetype($file), 'image') !== false) {
                        // Generate thumbnail
                        $thumb = \Components\Publications\Helpers\Html::createThumbName(
                            $file,
                            '_tn',
                            $extension = 'gif'
                        );
                        Filesystem::copy($srcFile, $configs->dataPath . DS . $thumb);

                        $thumbPath = $configs->dataPath . DS . $thumb;
                        ...
                        $med = \Components\Publications\Helpers\Html::createThumbName(
                            $file,
                            '_medium',
                            $extension = 'gif'
                        );
                        Filesystem::copy($srcFile, $configs->dataPath . DS . $med);
