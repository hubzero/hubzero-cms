...
class BundleBuilder
{
    ...
    protected function linkInto($target, $link, $log = null)
    {
        ...
        if (
            is_file($link) && @fileinode($link) !== false
        ...
        if ($r['primary_count'] == 0) {
            ...
            if ($carrier !== null) { // else *_unreadable already fired
                ...
                $has = function ($name, $size, $crc) use ($carrier, $byBase) {
                    if (
                        isset($carrier[$name]) && $carrier[$name]['size'] === $size
    ...
    protected function auditNonFile($versionId, $version, $base, $content, $name, $outer, &$r, $deep)
    {
        ...
        $has = function ($rel, $size, $crc) use ($entries, $byBase) {
            if (
                isset($entries[$rel]) && $entries[$rel]['size'] === $size
    ...
    protected function buildDatabase($versionId, $version, $base, $content, $force, $level, $log)
    {
        ...
        $log(sprintf(
            'Building Databases outer %s: %d datastore(s), %d entr(ies)...',
            basename($outerPath),
            count($dataAtts),
            count($entries)
        ));
    ...
    protected function buildMetadataOnly($versionId, $version, $base, $content, $force, $level, $log)
    {
        ...
        $log(sprintf(
            'Building metadata-only outer %s (README + %d supporting + %d gallery image(s))...',
            basename($outerPath),
            count($supporting),
            count($gallery)
        ));
