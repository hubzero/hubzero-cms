...
class Doi extends Obj
{
     ...
     * @const
     */
    public const SWITCH_OPTION_NONE = 0;
    public const SWITCH_OPTION_EZID = 1;
    public const SWITCH_OPTION_DATACITE = 2;
     ...
     * @const
     */
    public const STATE_FROM_PUBLISHED_TO_DRAFTREADY = 0;
    public const STATE_FROM_DRAFTREADY_TO_PUBLISHED = 1;
    ...
    */
    ...
    public function configs()
    {
        if (empty($this->_configs)) {
            ...
            $defaultSchema = 'http://schema.datacite.org/meta/kernel-4/metadata.xsd';
            $configs->xmlSchema = trim($params->get('doi_xmlschema', $defaultSchema), DS);
    ...
    public function on()
    {
        ...
        // Check required
        $hasDatacite = $this->_configs->dataciteServiceURL && $this->_configs->dataciteUserPW;
        $hasEzid = $this->_configs->prefix
            && $this->_configs->ezidServiceURL
            && $this->_configs->ezidUserPW;
        if ($this->_configs->dataciteEZIDSwitch && $this->_configs->shoulder && ($hasDatacite || $hasEzid)) {
            return true;
        }
    ...
    public function mapPublication($pub = null)
    {
        ...
        $pubUrl = $this->_configs->livesite . DS . 'publications' . DS
            . $pub->id . DS . $pub->version->version_number;
        $this->set('url', $pubUrl);
    ...
    public function setDefaults()
    {
        ...
        $pubIdentifier = $this->_configs->publisherIdentifier ?? '';
        $this->set('publisherIdentifier', htmlspecialchars($pubIdentifier));
        $pubIdentifierScheme = $this->_configs->publisherIdentifierScheme ?? '';
        $this->set('publisherIdentifierScheme', htmlspecialchars($pubIdentifierScheme));
        $pubIdentifierSchemeURI = $this->_configs->publisherIdentifierSchemeURI ?? '';
        $this->set('publisherIdentifierSchemeURI', htmlspecialchars($pubIdentifierSchemeURI));
    ...
    public function buildXml($doi = null)
    {
        ...
        if (!empty($authors) && is_array($authors)) {
            foreach ($authors as $author) {
                ...
                if (isset($author->orcid) && !empty($author->orcid)) {
                    $xmlfile .= '	<nameIdentifier nameIdentifierScheme="ORCID" '
                        . 'schemeURI="https://orcid.org/">' . $author->orcid . '</nameIdentifier>';
                }

                if ($author->organization) {
                    $orgidAtr = "";
                    if (isset($author->orgid) && !empty($author->orgid)) {
                        $orgidAtr = ' affiliationIdentifier="' . $author->orgid . '"'
            ...
            if ($this->get('creatorOrcid')) {
                $xmlfile .= '	<nameIdentifier nameIdentifierScheme="ORCID" '
                    . 'schemeURI="https://orcid.org/">' .  $this->get('creatorOrcid') . '</nameIdentifier>';
            }
            $xmlfile .= '</creator>';
        }
        $xmlfile .= '</creators>';
        ...
        if ($this->get('publisher')) {
            $publisherIdentifier = "";
            if ($this->get('publisherIdentifier')) {
                $publisherIdentifier = ' publisherIdentifier="' . $this->get('publisherIdentifier') . '"';
            }
            $publisherIdentifierScheme = "";
            if ($this->get('publisherIdentifierScheme')) {
                $publisherIdentifierScheme = ' publisherIdentifierScheme="'
                    . $this->get('publisherIdentifierScheme') . '"';
            }
            $publisherIdentifierSchemeURI = "";
            if ($this->get('publisherIdentifierSchemeURI')) {
                $publisherIdentifierSchemeURI = ' schemeURI="' . $this->get('publisherIdentifierSchemeURI') . '"';
            }
            $xmlfile .= ' <publisher xml:lang="en"'
                . $publisherIdentifier
                . $publisherIdentifierScheme
                . $publisherIdentifierSchemeURI
                . '>' . $this->get('publisher') . '</publisher>';
        }
        ...
        if ($this->get('contactPerson')) {
            ...
            foreach ($this->get('contactPerson') as $contactPerson) {
                ...
                $xmlfile .= '		<contributorName nameType="Personal">'
                    . $contactPerson->name . '</contributorName>';
                ...
                if (isset($contactPerson->orcid) && !empty($contactPerson->orcid)) {
                    $xmlfile .= '		<nameIdentifier nameIdentifierScheme="ORCID" '
                        . 'schemeURI="https://orcid.org/">' .  $contactPerson->orcid . '</nameIdentifier>';
                }

                if ($contactPerson->organization) {
                    $orgidAtr = "";
                    if (isset($contactPerson->orgid) && !empty($contactPerson->orgid)) {
                        $orgidAtr = ' affiliationIdentifier="' . $contactPerson->orgid . '"'
                            . ' affiliationIdentifierScheme="ROR" schemeURI="https://ror.org"';
                    }
                    $xmlfile .= '		<affiliation' .  $orgidAtr . '>' . $contactPerson->organization
                        . '</affiliation>';
                }
		...
		<resourceType resourceTypeGeneral="' . $this->get('resourceType') . '">'
            . $this->get('resourceTypeTitle') . '</resourceType>';
        ...
        if ($doiOfPrevPub || $doiOfNextPub || $citations) {
            ...
            if ($doiOfPrevPub) {
                $xmlfile .= '	<relatedIdentifier relatedIdentifierType="DOI" relationType="IsNewVersionOf">'
                    . $doiOfPrevPub . '</relatedIdentifier>';
            }

            if ($doiOfNextPub) {
                $xmlfile .= '	<relatedIdentifier relatedIdentifierType="DOI" '
                    . 'relationType="IsPreviousVersionOf">' . $doiOfNextPub . '</relatedIdentifier>';
            }
            ...
            if ($citations) {
                foreach ($citations as $citation) {
                    ...
                    if ($pos == 0) {
                        $citation->resourceTypeGeneral = substr(
                            trim($citation->resourceTypeGeneral),
                            $pos + strlen(self::DATACITE)
                        );
                    }

                    $isReferences = preg_match("/" . self::REFERENCES . "/i", $citation->relationType);
                    $isReferencedBy = preg_match("/" . self::REFERENCEDBY . "/i", $citation->relationType);
                    if (!empty($citation->relationType) && ($isReferences || $isReferencedBy)) {
                        if (preg_match("/" . self::REFERENCES . "/i", $citation->relationType)) {
                            $citation->relationType = ucfirst(self::REFERENCES);
                        } elseif (preg_match("/" . self::REFERENCEDBY . "/i", $citation->relationType)) {
                            $citation->relationType = self::ISREFERENCEDBY;
                        }

                        if (!empty($citation->resourceTypeGeneral)) {
                            $relType = $citation->relationType;
                            $resType = $citation->resourceTypeGeneral;
                            if (!empty($citation->relatedIdentifier)) {
                                $xmlfile .= '<relatedIdentifier relatedIdentifierType="DOI" '
                                    . 'relationType="' . $relType . '" '
                                    . 'resourceTypeGeneral="' . $resType . '">'
                                    . trim($citation->relatedIdentifier) . '</relatedIdentifier>';
                            } else {
                                if (!empty($citation->url)) {
                                    if (preg_match("/purl/i", $citation->url)) {
                                        $xmlfile .= '<relatedIdentifier relatedIdentifierType="PURL" '
                                            . 'relationType="' . $relType . '" '
                                            . 'resourceTypeGeneral="' . $resType . '">'
                                            . trim($citation->url) . '</relatedIdentifier>';
                                        ...
                                        $xmlfile .= '<relatedIdentifier relatedIdentifierType="Handle" '
                                            . 'relationType="' . $relType . '" '
                                            . 'resourceTypeGeneral="' . $resType . '">'
                                            . $val . '</relatedIdentifier>';
                                    } elseif (preg_match("/ark:/i", $citation->url) && !empty($citation->eprint)) {
                                        $xmlfile .= '<relatedIdentifier relatedIdentifierType="ARK" '
                                            . 'relationType="' . $relType . '" '
                                            . 'resourceTypeGeneral="' . $resType . '">'
                                            . $citation->eprint . '</relatedIdentifier>';
                                    } elseif (preg_match("/arxiv/i", $citation->url) && !empty($citation->eprint)) {
                                        $xmlfile .= '<relatedIdentifier relatedIdentifierType="arXiv" '
                                            . 'relationType="' . $relType . '" '
                                            . 'resourceTypeGeneral="' . $resType . '">'
                                            . $citation->eprint . '</relatedIdentifier>';
                                    } elseif (preg_match("/urn:/i", $citation->url) && !empty($citation->eprint)) {
                                        $xmlfile .= '<relatedIdentifier relatedIdentifierType="URN" '
                                            . 'relationType="' . $relType . '" '
                                            . 'resourceTypeGeneral="' . $resType . '">'
                                            . $citation->eprint . '</relatedIdentifier>';
                                    } else {
                                        $xmlfile .= '<relatedIdentifier relatedIdentifierType="URL" '
                                            . 'relationType="' . $relType . '" '
                                            . 'resourceTypeGeneral="' . $resType . '">'
                                            . $citation->url . '</relatedIdentifier>';
                                    }
                                }
                            }
                        }
        ...
        if ($this->get('license')) {
            ...
            $xmlfile .= '<rightsList><rights ' . $rightsURI . '>'
                . htmlspecialchars($this->get('license')) . '</rights></rightsList>';
        }
        if ($this->get('grantInfo')) {
            ...
            if (array_key_exists('grant_agency', $grantInfo)) {
                ...
                if (array_key_exists('grant_agency_id', $grantInfo)) {
                    $xmlfile .= '	<funderIdentifier funderIdentifierType="ROR">'
                        . $grantInfo['grant_agency_id'] . '</funderIdentifier>';
                }
        ...
        if ($this->get('size')) {
            ...
            if ($this->get('fileCount')) {
                ...
                if ($pubType == 5) {
                    $unitLabel = ($fileCount == 1) ? " publication" : " publications";
                } else {
                    $unitLabel = ($fileCount == 1) ? " file" : " files";
                }
                $xmlfile .= '	<size>' . $fileCount . $unitLabel . '</size>';
            }
            if ($this->get('size')) {
        ...
        if ($this->get('regTags')) {
            foreach ($this->get('regTags') as $regTag) {
                if (!empty($regTag->description) && preg_match("/^lcsh::/i", trim($regTag->description))) {
                    ...
                    $xmlfile .= '<subject subjectScheme="Library of Congress Subject Headings (LCSH)" '
                        . 'schemeURI="https://id.loc.gov/authorities/" valueURI="' . $url . '">'
                        . $regTag->raw_tag . '</subject>';
                } else {
        ...
        if (!empty($fosTagObj)) {
            ...
            if (!empty($valArr) && preg_match("/^fos::/i", trim($valArr[0])) && !empty($valArr[1])) {
                ...
                $xmlfile .= '<subject subjectScheme="Fields of Science and Technology (FOS)" '
                    . 'schemeURI="https://web-archive.oecd.org/2012-06-15/138575-38235147.pdf" '
                    . 'classificationCode="' . $classificationCode . '">' . $fosTagVal . '</subject>';
            }
        }
        $xmlfile .= '</subjects>';
        ...
        if ($this->get('synopsis')) {
            $xmlfile .= '<description descriptionType="Other">'
                . stripslashes($this->get('synopsis')) . '</description>';
        }
        $xmlfile .= '
			</descriptions>
		</resource>';
    ...
    public function runCurl($url, $postvals = null)
    {
        ...
        $options = array(
            ...
            CURLOPT_HTTPHEADER      => array(
                ...
                'Content-Length: ' . strlen($postvals)
            )
        );
    ...
    public function registerEZID($sendXml = false, $status = 'public')
    {
        ...
        if (($status == 'public') && ($sendXml == true)) {
            ...
            // Append XML
            $encodedXml = strtr($xml, array(":" => "%3A", "%" => "%25", "\n" => "%0A", "\r" => "%0D"));
            $input .= 'datacite: ' . $encodedXml . "\n";
     ...
     *
     * @param   string   $doi
     * @param   string   $stateSwitch  0 - from published to draft, 1 - from draft to published
     * @return  boolean
     */
     ...
     *
     * @param   string   $doi
     * @param   integer  $stateSwitch  0 - from published to draftready, 1 - vice versa
     *
     * @return  boolean
     */
     ...
     *          When a publication is reverted from published to draft, the DOI state
     *          is supposed to change from findable/public to registered/unavailable.
     *          When such publication is submitted, the DOI state is set to findable/public.
     *
     * @param   string    $doi
     * @param   integer   $stateSwitch  0 - from published to draft, 1 - from draft to published
     *
     * @return
     */
    public function revert($doi, $stateSwitch)
    {
        ...
        if (
            ($this->_configs->dataciteEZIDSwitch == self::SWITCH_OPTION_DATACITE)
