...
class Publication extends Obj
{
    ...
    public const STATE_DRAFT       = 3;
    public const STATE_POSTED      = 4;  // Posted for review
    public const STATE_PENDING     = 5;  // Pending publishing
    public const STATE_ARCHIVED    = 6;
    public const STATE_WORKED      = 7;  // Pending author changes
                ...
                if ($blockId && isset($this->_curationModel->_progress->blocks->$blockId)) {
                    if ($blockProperty) {
                        switch ($blockProperty) {
                            ...
                            case 'elementStatus':
                                $blockStatus = $this->_curationModel->_progress->blocks->$blockId->status;
                                if ($elementId && isset($blockStatus->elements->$elementId)) {
                                    return $blockStatus->elements->$elementId->status;
                                }
                                break;

                            case 'elements':
                                if ($elementId) {
                                    $blockManifest = $this->_curationModel->_progress->blocks->$blockId->manifest;
                                    if (isset($blockManifest->elements->$elementId)) {
                                        return $blockManifest->elements->$elementId;
                                    } else {
                                        $first = $this->curation('blocks', $blockId, 'firstElement');
                                        return $blockManifest->elements->$first;
                                    }
                                }
                            ...
                            case 'required':
                                $params = $this->_curationModel->_progress->blocks->$blockId->manifest->params;
                                return isset($params->required) ? $params->required : 0;
                                break;

                            case 'hasElements':
                                $elements = $this->_curationModel->_progress->blocks->$blockId->manifest->elements;
                                return empty($elements) ? false : true;
                                break;
    ...
    private function _authorize()
    {
        ...
        if (
            $this->params->get('access-admin-publication')
    ...
    public function bundlePath()
    {
        ...
        if (!isset($this->_bundlePath)) {
            ...
            $pubPath = Helpers\Html::buildPubPath(
                $this->get('id'),
                $this->get('version_id'),
                '',
                '',
            ...
            $this->_bundlePath = $pubPath . DS . $tarname;
        }
    ...
    public function getWikiPage($pageid = null, $masterscope = null, $versionid = null)
    {
        ...
        $query .= " FROM #__wiki_pages as p WHERE p.scope LIKE "
            . $this->_db->quote($masterscope . '%') . " ";
        if (is_numeric($pageid)) {
            $query .= " AND p.id=" . $this->_db->quote($pageid) . " ";
        } else {
            $query .= " AND p.pagename=" . $this->_db->quote($pageid) . " ";
        }
        $query .= " LIMIT 1";
    ...
    public function path($type = '', $root = false)
    {
        if (!isset($this->_basePath)) {
            $idPadded = \Hubzero\Utility\Str::pad($this->get('id'));
            $versionIdPadded = \Hubzero\Utility\Str::pad($this->get('version_id'));
            $this->_basePath = DS . trim($this->config('webpath'), DS)
                . DS . $idPadded . DS . $versionIdPadded;
        }
        switch (strtolower($type)) {
            case 'relative':
                $idPadded = \Hubzero\Utility\Str::pad($this->get('id'));
                $versionIdPadded = \Hubzero\Utility\Str::pad($this->get('version_id'));
                $path = '..' . DS . $idPadded . DS . $versionIdPadded;
                break;
            ...
            if (!is_file($source)) {
                ...
                if (is_dir(PATH_APP . DS . $path . DS . 'gallery')) {
                    ...
                    foreach ($file_list as $file) {
                        $filePath = PATH_APP . DS . $path . DS . 'gallery' . DS . $file;
    ...
    public function link($type = '')
    {
        ...
        switch (strtolower($type)) {
            ...
            case 'thumb':
                // Old approach:
                // $link = 'index.php?option=com_publications&id=' . $this->get('id')
                //     . '&v=' . $this->get('version_id') . '&media=Image:thumb';
            ...
            case 'masterimage':
                $link = 'index.php?option=com_publications&id=' . $this->get('id')
                    . '&v=' . $this->get('version_id') . '&media=Image:master';
                break;
    ...
    public function getWorkData($putCode = null)
    {
        ...
        foreach ($authors as $author) {
            ...
            $email = !empty($author->p_email) ? $author->p_email : '';
            $email = !empty($email) ? $email : (!empty($author->invited_email) ? $author->invited_email : '');
            if (!empty($email)) {
        ...
        // url
        $url = trim(Request::root(), DS) . DS . 'publications' . DS
            . $this->version->publication_id . DS . $this->version->version_number;
    ...
    public function updateWorkInORCID($orcidID, $accessToken, $putCode)
    {
        $memberConfig = Component::params('com_members');
        $srv = $memberConfig->get('orcid_service', 'members');
        $serviceURL = Request::scheme() . '://' . $this->orcidServices[$srv]
            . '/v3.0/' . $orcidID . '/work/' . $putCode;
        $header = [
