...
class Description extends Base
{
    ...
    public function save($manifest = null, $blockId = 0, $pub = null, $actor = 0, $elementId = 0)
    {
        ...
        // Parse metadata
        $data = array();
        preg_match_all("#<nb:(.*?)>(.*?)</nb:(.*?)>#s", $pub->metadata ? $pub->metadata : '', $matches, PREG_SET_ORDER);
        if (count($matches) > 0) {
        ...
        foreach ($this->_manifest->elements as $id => $element) {
            ...
            if ($field == 'metadata') {
                ...
                if (!$value && $required) {
                    ...
                    $isNewValue = $value && !isset($data[$aliasmap]);
                    $isChanged = isset($data[$aliasmap]) && $data[$aliasmap] != $value;
            ...
            $manifest = array(
                ...
                'elements'  => array(
                    1 => array (
                        ...
                        'about'     => '<p>Pick a descriptive yet concise publication title that will '
                            . 'quickly tell users about its content.</p>',
                        'adminTips' => '',
                        'params'    => array (
                            'required'      => 1,
                            'aliasmap'      => 'title',
