...
class File extends Base
{
    ...
    public function addToBundle($zip, $attachments, $element, $elementId, $pub, $blockParams, &$readme, $bundleDir)
    {
        ...
        if ($configs->multiZip == 1 && $attachments && count($attachments) > 1) {
            ...
            if (is_file($filePath)) {
                ...
                $bundleDirLower = strtolower($configs->bundleDirectory ?? '');
                $directoryLower = strtolower($configs->directory ?? '');
                if ($configs->directory && $bundleDirLower != $directoryLower) {
            ...
            foreach ($attachments as $attach) {
                ...
                $bundleDirLower = strtolower($configs->bundleDirectory ?? '');
                if ($configs->directory && $bundleDirLower != strtolower($configs->directory)) {
                    $where .= $configs->directory != $pub->secret ? DS . $configs->directory : '';
                }
    ...
    public function drawPackageList($attachments, $element, $elementId, $pub, $blockParams, $authorized)
    {
        ...
        foreach ($attachments as $attach) {
            ...
            if (file_exists($filePath)) {
                ...
                $list .= '<span class="item-title" id="file-' . $attach->id . '">';
                $list .= trim($where, DS) . '</span>';
                //$list .= '<span class="item-details">' . $attach->path . '</span>';
                $fileSize = trim(\Hubzero\Utility\Number::formatBytes(filesize($filePath)));
                $list .= '<span class="item-details">' . $fileSize . '</span>';
                $list .= '</li>';
            }
        }
    ...
    public function drawList($attachments, $element, $elementId, $pub, $blockParams, $authorized)
    {
        ...
        if ($configs->multiZip && $attachments && count($attachments) > 1) {
            ...
            // Get file icon
            $icon = '<img height="16" src="' . $file->getIcon() . '" alt="';
            $icon .= $file->get('ext') . '" />';

            // Serve as bundle
            $html .= '<li>';
            if ($file->exists() && $authorized) {
                $serveUrl = Route::url($pub->link('serve') . '&el=' . $elementId);
                $html .= '<a href="' . $serveUrl . '" title="' . $pop . '">';
                $html .= $icon . ' ' . $title . '</a>';
            } else {
                $html .= $icon . ' ' . $title . $notice;
            }
            ...
            if ($authorized === 'administrator') {
                ...
                $editUrl = 'index.php?option=com_publications&controller=items&task=editcontent';
                $editUrl .= '&id=' . $pub->get('id') . '&el=' . $elementId;
                $editUrl .= '&v=' . $pub->get('version_number');
                $html .= '<a href="' . $editUrl . '">';
            ...
            foreach ($attachments as $attach) {
                ...
                if ($file->exists() && $authorized) {
                    $serveUrl = Route::url(
                        $pub->link('serve') . '&el=' . $elementId
                        . '&a=' . $attach->id . '&download=1'
                    );
                    $html .= '<a href="' . $serveUrl . '" title="' . $pop . '">';
                    $html .= $icon . ' ' . $title . '</a>';
                } else {
                ...
                if ($authorized === 'administrator') {
                    $html .= ' <span class="edititem">';
                    $editUrl = 'index.php?option=com_publications&controller=items';
                    $editUrl .= '&task=editcontent&id=' . $pub->id;
                    $editUrl .= '&el=' . $elementId . '&v=' . $pub->version_number;
                    $html .= '<a href="' . $editUrl . '">';
                    $html .= Lang::txt('COM_PUBLICATIONS_EDIT');
                    $html .= '</a>';
                    $html .= '</span>';
    ...
    public function drawLauncher($element, $elementId, $pub, $blockParams, $elements, $authorized)
    {
        ...
        $serveLink = 'index.php?option=com_publications&task=serve&id=' . $pub->id;
        $serveLink .= '&v=' . $pub->version_number;
        $url = Route::url($serveLink) . '?el=' . $elementId;
        ...
        if ($role == 1) {
            ...
            if ($configs->fancyLauncher) {
                $html = \Components\Publications\Helpers\Html::drawLauncher(
                    'ic-download',
                    $pub,
                    $url,
                    $title,
                ...
                if ($showArchive == 1 || ($showArchive == 2 && count($attachments) > 1)) {
                    $archiveLink = 'index.php?option=com_publications&id=' . $pub->id;
                    $archiveLink .= '&task=serve&v=' . $pub->version_number . '&render=archive';
                    $url = Route::url($archiveLink);
                    ...
                    if ($path && file_exists($path)) {
                        $size = filesize($path);
                        $sizeFormatted = \Hubzero\Utility\Number::formatBytes($size);
                        $label .= ' (' . $sizeFormatted . ')';
                        $title .= ' (' . $sizeFormatted . ')';
                        ...
                        $sftpPath = $pub->config()->get('sftppath');
                        $sftpSize = intval($pub->config()->get('sftpsize', 5000));
                        if ($sftpPath && $size >= $sftpSize) {
                            $showDoc = true;
                    ...
                    $show = new \stdClass();
                    $showLink = 'index.php?option=com_publications&id=' . $pub->id;
                    $showLink .= '&task=serve&v=' . $pub->version_number . '&render=showcontents';
                    $show->href = Route::url($showLink);
                ...
                $html = \Components\Publications\Helpers\Html::primaryButton(
                    $class,
                    $url,
                    $label,
                    null,
    ...
    public function getFilePath($path, $id, $configs = null, $params = null, $suffix = null)
    {
        ...
        if ($configs->dirHierarchy == 1) {
            $path  = trim($path, DS);
            $name = $suffix
                ? \Components\Projects\Helpers\Html::fixFileName($path, ' (' . $suffix . ')')
                : $path;
            ...
            $name = $suffix
                ? \Components\Projects\Helpers\Html::fixFileName(basename($path), ' (' . $suffix . ')')
                : basename($path);
            $fpath = $configs->pubPath . DS . $name;
        } else {
    ...
    public function addAttachment($filePath, $pub, $configs, $uid, $elementId, $element, $ordering = 1)
    {
        ...
        $loaded = $objPA->loadElementAttachment(
            $pub->version_id,
            array('path' => $filePath),
            $elementId,
            $this->_name,
    ...
    public function buildDataObject($att, $view, $i = 1)
    {
        ...
        $defaultTitle   = $view->manifest->params->title
                        ? str_replace(
                            '{pubtitle}',
                            $view->pub->title,
                            $view->manifest->params->title
                        ) : null;
        $defaultTitle   = $view->manifest->params->title
                        ? str_replace(
                            '{pubversion}',
                            $view->pub->version_label,
                            $defaultTitle
                        ) : null;
        ...
        $props = $view->master->block . '-' . $view->master->blockId . '-' . $view->elementId;
        $data->set('props', $props);
        $downloadLink = $view->pub->link('serve') . '&el=' . $view->elementId;
        $downloadLink .= '&a=' . $att->id . '&download=1';
        $data->set('downloadUrl', Route::url($downloadLink));
