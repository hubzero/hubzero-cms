...
class Publication extends Table
{
    ...
    public function buildQuery($filters = array(), $admin = false)
    {
        ...
        if ($dev) {
            ...
            $query .= " AND V.id = (SELECT MAX(wv2.id) FROM `#__publication_versions` AS wv2"
                . " WHERE wv2.publication_id = C.id)";
        } else {
            if (!isset($filters['all_versions']) || !$filters['all_versions']) {
        ...
        if (!$dev) {
            $query .= " AND (V.published_up IS NULL OR V.published_up = '0000-00-00 00:00:00'"
                . " OR V.published_up <= '" . $now . "') ";
            $query .= " AND (V.published_down IS NULL OR V.published_down = '0000-00-00 00:00:00'"
                . " OR V.published_down >= '" . $now . "') ";
        }
    ...
    public function getCount($filters = array(), $admin = false)
    {
        ...
        $sql  = "SELECT C.id ";
        if (isset($filters['tag']) && $filters['tag'] != '') {
            $sql .= ", TA.tag, COUNT(DISTINCT TA.tag) AS uniques " . $query;
        } else {
            $sql .= $query;
        }
    ...
    public function getRecords($filters = array(), $admin = false)
    {
        ...
        $sql .= ", (SELECT vv.version_label FROM `#__publication_versions` as vv"
            . " WHERE vv.publication_id=C.id AND vv.state=3 ORDER BY ID DESC LIMIT 1) AS dev_version_label ";
        $sql .= ", (SELECT COUNT(*) FROM `#__publication_versions`"
            . " WHERE publication_id=C.id AND state!=3) AS versions ";

        $sortby  = isset($filters['sortby']) ? $filters['sortby'] : 'title';

        if ($sortby == 'popularity') {
            $sql .= ", (SELECT S.users FROM `#__publication_stats` AS S"
                . " WHERE S.publication_id=C.id AND S.period=14 ORDER BY S.datetime DESC LIMIT 1) as stat ";
        }

        if (isset($filters['tag']) && $filters['tag'] != '') {
            $sql .= ", TA.tag, COUNT(DISTINCT TA.tag) AS uniques ";
        } else {
            $sql .= " ";
        }
    ...
    public function getPublication($pid = null, $version = 'default', $project_id = null, $alias = null)
    {
        ...
        $sql .= ",(SELECT vv.state FROM #__publication_versions as vv"
            . " WHERE vv.publication_id=C.id AND vv.main=1 ORDER BY vv.created DESC LIMIT 1)
				AS default_version_status ";
        $sql .= ",(SELECT COUNT(*) FROM #__publication_versions"
            . " WHERE publication_id=C.id AND state!=3 ) AS versions ";
        $sql .= " FROM #__publication_versions as V, #__projects AS PP,"
    ...
    {
        $query = "SELECT rating FROM `#__publication_ratings` WHERE publication_id="
    ...
    {
        $query = "UPDATE $this->_tbl SET rating=" . $this->_db->quote($this->rating)
            . ", times_rated=" . $this->_db->quote($this->times_rated)
    ...
    public function getPubStats($validProjects = array(), $get = 'total', $when = null)
    {
        ...
        if ($get == 'advanced' || $get == 'released') {
            if ($get == 'advanced') {
                $query .= " AND (V.state = 4 OR V.state = 6 OR V.state = 5)  ";
            } else {
                $query .= " AND V.state = 1 AND V.main = 1";
            }
