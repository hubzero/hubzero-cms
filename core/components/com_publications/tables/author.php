...
class Author extends Table
{
    ...
    public function getAuthorOrder($uids = null, $vid = null)
    {
        ...
        $uids = "'" . implode("','", $uids) . "'";
        $query  = "SELECT id FROM $this->_tbl WHERE publication_version_id="
            . $this->_db->quote($vid) . " AND id IN (" . $uids . ")";
    ...
    public function getSubmitter($vid = null, $by = 0)
    {
        ...
        $query = "SELECT * FROM $this->_tbl WHERE role='submitter'"
            . " AND publication_version_id=" . $this->_db->quote($vid) . " LIMIT 1";
        $this->_db->setQuery($query);
        ...
        $query .= " COALESCE(A.name , x.name) as name, x.username, x.orcid,"
    ...
    public function saveSubmitter($vid = null, $by = 0, $projectid = 0)
    {
        ...
        $query = "SELECT * FROM $this->_tbl WHERE role='submitter'"
            . " AND publication_version_id=" . $this->_db->quote($vid);
        $this->_db->setQuery($query);
        if ($result = $this->_db->loadAssoc()) {
            $this->bind($result);
    ...
    public function deleteAssociation($uid = null, $vid = null, $delete = 0)
    {
        ...
        if ($delete == 1) {
            $query  = "DELETE FROM $this->_tbl WHERE publication_version_id="
                . $this->_db->quote($vid) . " AND user_id=" . $this->_db->quote($uid);
            $query .= " AND (role IS NULL OR role != 'submitter')  ";
        } else {
            $query  = "UPDATE $this->_tbl SET status=0 WHERE publication_version_id="
                . $this->_db->quote($vid) . " AND user_id=" . $this->_db->quote($uid);
            $query .= " AND (role IS NULL OR role != 'submitter')  ";
        }
    ...
    public function getCount($vid = null)
    {
        ...
        $query = "SELECT count(*) FROM $this->_tbl WHERE publication_version_id="
            . $this->_db->quote($vid) . " AND status=1 ";
        $this->_db->setQuery($query);
    ...
    public function getNeighbor($move)
    {
        switch ($move) {
            case 'orderup':
                $sql = "SELECT * FROM $this->_tbl WHERE publication_version_id="
                    . $this->_db->quote($this->publication_version_id)
                    . " AND ordering < " . $this->_db->quote($this->ordering)
                    . " ORDER BY ordering DESC LIMIT 1";
                break;
