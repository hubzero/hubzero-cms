...
class Items extends AdminController
{
    ...
    public function editTask($isnew = 0)
    {
        ...
        if (
            $this->view->model->get('checked_out')
    ...
    public function saveauthororderTask()
    {
        ...
        $url = Route::url(
            ...
            . '&task=edit' . '&id[]=' . $id . '&version=' . $version,
            false
        );
    ...
    public function saveTask($redirect = false)
    {
        ...
        $shoulderPattern = "/" . $doiService->_configs->shoulder . "/";
        if (
            $doi
            && (!$this->model->version->doi || !preg_match($shoulderPattern, $this->model->version->doi))
        ) {
            ...
            switch ($action) {
                ...
                case 'republish':
                    ...
                    if ($doiService->on()) {
                        if (
                            ...
                            if (!$doi) {
                                App::redirect(
                                    $url,
                                    Lang::txt('COM_PUBLICATIONS_ERROR_DOI')
                                    . ' ' . $doiService->getError(),
                                    'error'
                                );
                                return;
                    ...
                    if ($action == 'publish') {
                        ...
                        $acceptedDate = $this->model->version->accepted;
                        $nullDate = '0000-00-00 00:00:00';
                        if ($allowArchive && $acceptedDate && $acceptedDate != $nullDate) {
                            $monthFrom = Date::of($acceptedDate . '+1 month')->toSql();
                        ...
                        $notArchived = !$this->model->version->archived
                            || $this->model->version->archived == $nullDate;
                        if (
                            !$this->getError()
                            && $this->model->version->doi
                            && $allowArchive == true
                    ...
                    foreach ($authors as $author) {
                        if (!empty($author->user_id)) {
                            ...
                            // Add pub to orcid record when both orcid and access_token are not null.
                            // Otherwise, send email that includes ORCID management permission link
                            if (!empty($collaborator)) {
                                $orcidID = $collaborator->orcid;
                                $accessToken = $collaborator->access_token;
                                ...
                                if (!empty($author->invited_email)) {
                                    $permKey = 'COM_PUBLICATIONS_GRANT_ORCID_MANAGEMENT_PERMISSION';
                                    $subjectForPermissionEmail = Lang::txt($permKey);
                                    $msgKey = 'COM_PUBLICATIONS_GRANT_ORCID_EMAIL_MESSAGE';
                                    $messageForPermissionEmail = Lang::txt($msgKey);
                                    ...
                                    if (!empty($srv) && !empty($clientID) && !empty($redirectURI)) {
                                        ...
                                        $permissionURL .= 'orcid.org/oauth/authorize?client_id=' . $clientID
                                        ...
                                        $from['name']  = $siteName . ' ' . Lang::txt('COM_PUBLICATIONS');
                                        ...
                                        $permTxt = Lang::txt('COM_PUBLICATIONS_GRANT_ORCID_MANAGEMENT_PERMISSION');
                                        $eview->permissionTxt = $permTxt;
                ...
                case 'unpublish':
                    ...
                    $versionNum = $this->model->version->version_number;
                    if ($selectedReason == 0) {
                        $otherReason = Request::getString('reason', '');
                        $verTblObj->saveUnPubReason($id, $versionNum, $selectedReason, $otherReason);
                    } else {
                        if ($selectedReason == 1) {
                            $reason = Lang::txt('COM_PUBLICATIONS_UNPUBLISHED_NOT_AVAILABLE');
                            $verTblObj->saveUnPubReason($id, $versionNum, $reason);
                        } elseif ($selectedReason == 2) {
                            $reason = Lang::txt('COM_PUBLICATIONS_UNPUBLISHED_ERROR');
                            $verTblObj->saveUnPubReason($id, $versionNum, $reason);
                        }
                    }

                    // Update DOI metadata, and set resource URL to tombstone page url.
                    $doiShoulder = "/" . $doiService->_configs->shoulder . "/";
                    if ($doiService->on()) {
                        if (
                            $this->model->version->doi
                            && preg_match($doiShoulder, $this->model->version->doi)
                        ) {
                            $doiService->set('authors', $authors);
                            $doiService->update($this->model->version->doi, true);

                            $resURL = $doiService->_configs->livesite . DS
                                . 'index.php?option=' . $this->_option
                                . '&controller=' . $this->_controller
                                . '&task=tombstone' . '&id=' . $id
                                . '&v=' . $this->model->version->version_number;
        ...
        if (!$this->getError()) {
            ...
            if (!$this->model->version->store()) {
                ...
                // Build return url
                $link = '/projects/' . $project->get('alias') . '/publications/' . $id
                    . '/?version=' . $this->model->version->version_number;
        ...
        if ($redirect) {
            $editUrl = 'index.php?option=' . $this->_option . '&controller=' . $this->_controller
                . '&task=edit' . '&id[]=' . $id . '&version=' . $this->model->get('version_number');
            App::redirect(
                Route::url($editUrl, false)
            );
        } else {
            $listUrl = 'index.php?option=' . $this->_option . '&controller=' . $this->_controller;
            App::redirect(
                Route::url($listUrl, false)
            );
        }
    }
    ...
    private function _emailContributors($subject = '', $message = '', $authors = array(), $action = 'publish')
    {
        ...
        if ($authors && count($authors) > 0) {
            ...
            // Send message
            if (
                !Event::trigger('xmessage.onSendMessage', array(
        ...
        foreach ($ids as $id) {
            ...
            if ($version != 'all' && count($versions) > 1) {
                ...
                if ($i == count($versions)) {
                    ...
                    $webpath = trim($this->config->get('webpath'), DS);
                    $path = PATH_APP . DS . $webpath . DS . \Hubzero\Utility\Str::pad($id);
    ...
    public function resetratingTask()
    {
        ...
        // Redirect
        $redirectUrl = 'index.php?option=' . $this->_option . '&controller=' . $this->_controller
            . '&task=edit&id[]=' . $id;
        App::redirect(
            Route::url($redirectUrl, false),
            $this->_message
        );
    }
    ...
    public function archiveTask()
    {
        ...
        $url = Route::url(
            'index.php?option=' . $this->_option . '&controller=' . $this->_controller
            . '&task=edit' . '&id[]=' . $pid . '&version=' . $version,
            false
        );
