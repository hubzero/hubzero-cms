...
class Migration20180316000000ComPublications extends Base
{
    ...
    public function up()
    {
        if ($this->db->tableExists('#__publication_versions')) {
            $query = "SELECT id, published_up FROM `#__publication_versions` "
                . "WHERE state=1 AND (accepted IS NULL or accepted='0000-00-00 00:00:00');";
            ...
            if ($pubs) {
                foreach ($pubs as $pub) {
                    $query = "UPDATE `#__publication_versions` SET accepted="
                        . $this->db->quote($pub->published_up)
                        . " WHERE id=" . $this->db->quote($pub->id);
