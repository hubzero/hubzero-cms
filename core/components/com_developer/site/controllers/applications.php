...
class Applications extends SiteController
{
    ...
    public function displayTask()
    {
        ...
        if (User::isGuest()) {
            ...
            $return = Route::url($url, false, true);
            App::redirect(
    ...
    public function editTask($application = null)
    {
        // must be logged in
        if (User::isGuest()) {
            $url = 'index.php?option=' . $this->_option . '&controller=' . $this->_controller . '&task=edit';
            $return = Route::url($url, false, true);
            App::redirect(
                Route::url('index.php?option=com_users&view=login&return=' . base64_encode($return))
            );
        ...
        // or we can create
        if (
            !$this->config->get('access-edit-application', 0)
