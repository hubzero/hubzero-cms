		...
		<div class="subject">
			...
			<?php foreach (isset($endpoints[$activeVersion]) ? $endpoints[$activeVersion] : array() as $endpoint): ?>
				...
				<div class="doc-section endpoint" id="<?php echo $key; ?>">
					<h3 class="endpoint-header collapsed" role="button" tabindex="0" aria-expanded="false">
						<span class="http-method <?php echo $methodClass; ?>"><?php echo $httpMethod; ?></span>
						<?php echo htmlspecialchars($endpoint['name']) . $inherited; ?>
					</h3>
					...
					<div class="endpoint-content collapsed">
						...
						foreach ($endpoint as $k => $v):
							...
							?>
							<p><strong><?php echo htmlspecialchars($k); ?>:</strong> <?php echo htmlspecialchars($v); ?></p>
							<?php
						endforeach;
						?>
							...
							<table>
								...
								<tbody>
									<?php foreach ($endpoint['parameters'] as $param): ?>
										<tr>
											<td><?php echo (isset($param['name'])) ? htmlspecialchars($param['name']) : ' '; ?></td>
											<td><?php echo (isset($param['type'])) ? htmlspecialchars($param['type']) : ' '; ?></td>
											<td>
												<?php echo (isset($param['required']) && $param['required']) ? '<span class="required">' . Lang::txt('JREQUIRED') . '</span>.' : ''; ?>
												<?php echo (isset($param['description']) && $param['description']) ? htmlspecialchars($param['description']) : ''; ?>
											</td>
											<td>
												<code
													class="nohighlight"><?php echo (isset($param['default'])) ? htmlspecialchars($param['default']) : 'null'; ?></code>
											</td>
											<td>
												<?php if (isset($param['allowedValues'])): ?>
													<code class="nohighlight"><?php echo htmlspecialchars($param['allowedValues']); ?></code>
												<?php endif; ?>
											</td>
										</tr>
									<?php endforeach; ?>
						...
						<div class="try-it-out" data-endpoint="<?php echo htmlspecialchars($fullEndpoint); ?>"
							data-method="<?php echo $httpMethod; ?>" data-token="<?php echo htmlspecialchars($token); ?>">
