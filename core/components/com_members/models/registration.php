...
class Registration
{
    ...
    public function check($task = 'create', $id = 0, $field_to_check = array())
    {
        ...
        if ($registrationUsername != REG_HIDE) {
            ...
            if (!empty($login) && !Helpers\Utility::validlogin($login, $allowNumericFirstCharacter)) {
                $this->_invalid['login'] = 'Invalid login name. Please type at least 2 characters '
                    . 'and use only lowercase alphanumeric characters.';
            }
        }
        ...
        if (!empty($login) && ($task == 'create' || $task == 'proxycreate' || $task == 'update')) {
            ...
            if ($uid && $uid != $id) {
                $this->_invalid['login'] = 'The user login "' . htmlentities($login)
        ...
        if ($registrationEmail != REG_HIDE) {
            if (empty($email)) {
                ...
                if ($xid && ($allow_duplicate_emails == 0 || $allow_duplicate_emails == 2)) {
                    ...
                    } elseif ($allow_duplicate_emails == 2) {
    ...
    public function checkusername($username)
    {
        ...
        if (!Helpers\Utility::validlogin($username)) {
            $ret['message'] = 'Invalid login name. Please type between 2 and 32 characters '
                . 'and use only lowercase alphanumeric characters.';
            return $ret;
        }
    ...
    public function generateUsername($email)
    {
        ...
        // Generate username by appending a sequential number to the local part of
        // an address until there is an available username
        for ($i = 1; true; $i++) {
            // Make sure login username is no longer than max allowed by DB
            $numberLen = strlen($i);
