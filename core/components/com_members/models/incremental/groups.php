...
class Groups
{
    ...
    {
        $uid = (int)$uid;
        $sql = 'SELECT irl.* FROM `#__incremental_registration_groups` irg '
            . 'LEFT JOIN `#__incremental_registration_group_label_rel` irglr ON irglr.group_id = irg.id '
            . 'INNER JOIN `#__incremental_registration_labels` irl ON irl.id = irglr.label_id '
            . 'WHERE irg.hours <= (SELECT (unix_timestamp(current_timestamp) - unix_timestamp(registerDate))'
            . '/60/60 AS hours FROM `#__users` WHERE id = ' . $uid . ')';
        ...
        $sql = 'SELECT coalesce((SELECT opted_out FROM `#__profile_completion_awards` '
            . 'WHERE user_id = ' . $uid . '), 0)';
        self::$dbh->setQuery($sql);
        $times = self::$dbh->loadResult();
        if ($times > 0) {
            $sql = 'SELECT (unix_timestamp() - unix_timestamp(last_bothered))/60/60 > '
                . '(SELECT coalesce((SELECT hours FROM `#__incremental_registration_popover_recurrence` '
                . 'WHERE idx = ' . ($times - 1) . '), '
                . '(SELECT hours FROM `#__incremental_registration_popover_recurrence` ORDER BY idx DESC LIMIT 1))) '
        ...
        $sql = 'SELECT profile_key, profile_value FROM `#__user_profiles` '
            . 'WHERE user_id = ' . $uid . ' AND profile_key IN (' . implode(', ', $colNames) . ')';
        self::$dbh->setQuery($sql);
