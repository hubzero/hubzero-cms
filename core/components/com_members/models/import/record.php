...
class Record extends \Hubzero\Content\Import\Model\Record
{
    ...
    public function check()
    {
        ...
        $form->bind(new \Hubzero\Config\Registry($this->profile));

        if (!$form->validate($this->profile)) {
    ...
    private function mapEntryData()
    {
        ...
        } elseif (isset($this->raw->id) && $this->raw->id > 1) {
            $this->record->entry = Member::oneOrNew($this->raw->id);
        } elseif (isset($this->raw->username) && $this->raw->username) {
        ...
        foreach (array('disability', 'race', 'hispanic') as $key) {
            if (isset($this->raw->$key)) {
                // In PATCH mode, skip fields with no values
                if ($this->_mode == 'PATCH' && (!isset($this->profile[$key]) || !$this->profile[$key])) {
                    continue;
                }

                $this->profile[$key] = $this->multiValueField($this->profile[$key]);
            }
        }

        // If we have a name but no individual parts...
        $noGiven = !$this->record->entry->get('givenName');
        $noSurname = !$this->record->entry->get('surame');
        $hasName = $this->record->entry->get('name');
        if ($noGiven && $noSurname && $hasName) {
        ...
        $hasGiven = $this->record->entry->get('givenName');
        $hasSurname = $this->record->entry->get('surame');
        if (($hasGiven || $hasSurname) && !$hasName) {
            $name = array(
        ...
        if ($this->record->entry->get('id')) {
            ...
            if ($username && isset($this->raw->username) && $username != $this->raw->username) {
                ...
                $msg = Lang::txt('Usernames for existing members cannot be changed at this time.');
                array_push($this->record->notices, $msg);
            }
        } elseif (isset($this->raw->username) && $this->raw->username) {
     ...
     * @return  void
     */
    private function saveEntryData()
    {
        ...
        if ($isNew) {
            if (!$this->record->entry->get('username')) {
                ...
                if (!$valid) {
                    for ($i = 0; $i <= 99; $i++) {
                        $name = strtolower($this->record->entry->get('name'));
                        $username = preg_replace('/[^a-z0-9_]/i', '', $name) . $i;
                        if (Validate::username($username)) {
                            if ($this->usernameExists($username)) {
                                $valid = true;
                                break;
        ...
        if (!empty($this->profile)) {
            if (!$this->record->entry->saveProfile($this->profile)) {
