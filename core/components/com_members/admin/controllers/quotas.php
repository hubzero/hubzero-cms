...
class Quotas extends AdminController
{
    ...
    public function editTask($row = null)
    {
        ...
        $dataHref = Route::url(
            'index.php?option=' . $this->_option . '&controller=quotas&task=getClassValues&class_id=',
            false
        );
        $classes = Html::select(
            'genericlist',
    ...
    public function getQuotaUsageTask($returnType = 'json', $id = null)
    {
        ...
        if ($username && $host) {
            ...
            if (!$fp) {
                ...
                $used = (isset($info['softspace']) && $info['softspace'] != 0)
                    ? bcdiv($info['space'], $info['softspace'], 6)
                    : 0;
