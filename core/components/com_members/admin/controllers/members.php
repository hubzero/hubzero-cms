...
class Members extends AdminController
{
    ...
    public function runSelectQuery($query)
    {
        ...
        // json_encode: returns a string containing the JSON representation from mySQL
    ...
    public function deidentifyTask()
    {
        ...
        // Run through main CMS tables, then run through client specific database
        // tables that pertains to jobs, sessions, views with same trigger name
        foreach ($ids as $id) {
            ...
            // Can't rely on any order the plugins run in. Setting the deletion
            // profile key in controller before calling the plugins
            $insert_UserProfileWithStatus_Query = "INSERT INTO `#__user_profiles` "
                . "(`user_id`, `profile_key`, `profile_value`) values (?, 'deletion', 'marked')";
            ...
            if ($result) {
                ...
                $update_UsersById_Query = "UPDATE `#__users` set name=" . $db->quote($anonUserNameSpace)
                    . ", givenName=" . $db->quote($anonUserName)
                    . ", middleName='', surname='anonSurName', username=" . $db->quote($anonUserName)
                    . ", password=" .  $db->quote($anonPassword)
                    . ", block='1', registerIP='', params='', homeDirectory='', email="
                    . $db->quote($anonUserName . "@example.com") . " where id =" . $db->quote($id);
                $this->runUpdateOrDeleteQuery($update_UsersById_Query);

                $update_UserProfiles_Query = "UPDATE `#__user_profiles` SET profile_value='sanitized' "
                    . "WHERE user_id=" . $db->quote($id) . " AND profile_key='deletion'";
    ...
    public function blockTask()
    {
        ...
        foreach ($ids as $id) {
            ...
            if ($state == 1) {
                // Set user Shadow Expiration to a past date. This disables the account in LDAP.
                // "10957" is the number of days to January 1, 2000 from epoch start
    ...
    public function saveprofileTask()
    {
        ...
        foreach ($profile->fields as $i => $element) {
            ...
            $field->set(array(
                ...
                'option_other'  => (isset($opts->include_other_option) ? (int) $opts->include_other_option : ''),
                'option_blank'  => (isset($opts->include_blank_option) ? (int) $opts->include_blank_option : ''),
                ...
                'default_value' => (isset($opts->value) ? (string) $opts->value : ''),
