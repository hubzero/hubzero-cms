...
class Points extends AdminController
{
    ...
    public function displayTask()
    {
        ...
        $this->view->stats[] = array(
            ...
            'class'         => 'royaltytotal',
            'alltimepts'    => Transaction::getTotals('', 'deposit', '', $royalty, '', '', 1, ''),
            'thismonthpts'  => Transaction::getTotals('', 'deposit', '', $royalty, '', '', 1, $thismonth),
            'lastmonthpts'  => Transaction::getTotals('', 'deposit', '', $royalty, '', '', 1, $lastmonth),
            'alltimetran'   => Transaction::getTotals('', 'deposit', '', $royalty, '', '', 1, '', $calc),
            'thismonthtran' => Transaction::getTotals('', 'deposit', '', $royalty, '', '', 1, $thismonth, $calc),
            'lastmonthtran' => Transaction::getTotals('', 'deposit', '', $royalty, '', '', 1, $lastmonth, $calc),
            'avg'           => round(Transaction::getTotals('', 'deposit', '', $royalty, '', '', 1, '', 1))
        );
        ...
        $this->view->stats[] = array(
            ...
            'class'         => 'royalty',
            'alltimepts'    => Transaction::getTotals('answers', 'deposit', '', $royalty, '', '', 1, ''),
            'thismonthpts'  => Transaction::getTotals('answers', 'deposit', '', $royalty, '', '', 1, $thismonth),
            'lastmonthpts'  => Transaction::getTotals('answers', 'deposit', '', $royalty, '', '', 1, $lastmonth),
            'alltimetran'   => Transaction::getTotals('answers', 'deposit', '', $royalty, '', '', 1, '', $calc),
            'thismonthtran' => Transaction::getTotals('answers', 'deposit', '', $royalty, '', '', 1, $thismonth, $calc),
            'lastmonthtran' => Transaction::getTotals('answers', 'deposit', '', $royalty, '', '', 1, $lastmonth, $calc),
            'avg'           => round(Transaction::getTotals('answers', 'deposit', '', $royalty, '', '', 1, '', 1))
        );
        ...
        $this->view->stats[] = array(
            ...
            'class'         => 'royalty',
            'alltimepts'    => Transaction::getTotals('review', 'deposit', '', $royalty, '', '', 1, ''),
            'thismonthpts'  => Transaction::getTotals('review', 'deposit', '', $royalty, '', '', 1, $thismonth),
            'lastmonthpts'  => Transaction::getTotals('review', 'deposit', '', $royalty, '', '', 1, $lastmonth),
            'alltimetran'   => Transaction::getTotals('review', 'deposit', '', $royalty, '', '', 1, '', $calc),
            'thismonthtran' => Transaction::getTotals('review', 'deposit', '', $royalty, '', '', 1, $thismonth, $calc),
            'lastmonthtran' => Transaction::getTotals('review', 'deposit', '', $royalty, '', '', 1, $lastmonth, $calc),
            'avg'           => round(Transaction::getTotals('review', 'deposit', '', $royalty, '', '', 1, '', 1))
        );
        ...
        $this->view->stats[] = array(
            ...
            'class'         => 'royalty',
            'alltimepts'    => Transaction::getTotals($cat, 'deposit', '', $royalty, '', '', 1, ''),
            'thismonthpts'  => Transaction::getTotals($cat, 'deposit', '', $royalty, '', '', 1, $thismonth),
            'lastmonthpts'  => Transaction::getTotals($cat, 'deposit', '', $royalty, '', '', 1, $lastmonth),
            'alltimetran'   => Transaction::getTotals($cat, 'deposit', '', $royalty, '', '', 1, '', $calc),
            'thismonthtran' => Transaction::getTotals($cat, 'deposit', '', $royalty, '', '', 1, $thismonth, $calc),
            'lastmonthtran' => Transaction::getTotals($cat, 'deposit', '', $royalty, '', '', 1, $lastmonth, $calc),
            'avg'           => round(Transaction::getTotals($cat, 'deposit', '', $royalty, '', '', 1, '', 1))
        );
    ...
    public function editTask()
    {
        if ($uid = Request::getInt('uid', 0)) {
            ...
            $query = "SELECT * FROM `#__users_transactions` WHERE uid=" . $uid
    ...
    public function saveTask()
    {
        ...
        App::redirect(Route::url($url, false), Lang::txt('User info saved'));
    }
    ...
    public function saveconfigTask()
    {
        ...
        for ($i = 0; $i < count($points); $i++) {
            ...
            if ($point) {
                ...
                $query = "INSERT INTO `#__users_points_config` "
                    . "(`id`,`description`,`alias`,`points`) "
                    . "VALUES ($id,'$description','$alias', '$point')";
                $this->database->setQuery($query);
                $this->database->query();
            }
        }
        ...
        $url = 'index.php?option=' . $this->_option . '&controller=' . $this->_controller
            . '&task=config';
        App::redirect(Route::url($url, false), Lang::txt('Config Saved'));
    }
    ...
    public function process_batchTask()
    {
        ...
        $duplicate = MarketHistory::getRecord(
            intval($log['ref']),
            $log['action'],
            $log['category'],
            '',
        ...
        if ($data['amount'] && $data['description'] && $data['users']) {
            if (!$duplicate) { // run only once
                ...
                $msg = 'This batch transaction was already processed earlier. '
                    . 'Use a different identifier if you need to run it again.';
                Notify::warning(Lang::txt($msg));
            }
        } else {
