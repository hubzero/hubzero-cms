...
(function() {
  ...
  FormbuilderModel = (function(_super) {
    ...
    FormbuilderModel.prototype.indexInDOM = function() {
      var _this = this;
      var wrappers = document.querySelectorAll('.fb-field-wrapper');
      var matchEl = null;
      var idx = -1;
      for (var i = 0; i < wrappers.length; i++) {
        if (wrappers[i]._cid === _this.cid) {
          matchEl = wrappers[i];
          idx = i;
          break;
        }
  ...
  ViewFieldView = (function(_super) {
    ...
    ViewFieldView.prototype.render = function() {
      this.el.classList.add('response-field-' + this.model.get(Formbuilder.options.mappings.FIELD_TYPE));
      this.el._cid = this.model.cid;
      this.el.innerHTML = Formbuilder.templates["view/base" + (!this.model.is_input() ? '_non_input' : '')]({
        rf: this.model
      });
      return this;
    };
  ...
  EditFieldView = (function(_super) {
    ...
    EditFieldView.prototype.render = function() {
      this.el.innerHTML = Formbuilder.templates["edit/base" + (!this.model.is_input() ? '_non_input' : '')]({
        rf: this.model
      });
      rivets.bind(this.el, {
        model: this.model
    ...
    EditFieldView.prototype.remove = function() {
      ...
      var addFieldTab = this.parentView.el.querySelector('[data-target="#addField"]');
      if (addFieldTab) { addFieldTab.click(); }
    ...
    EditFieldView.prototype.addOption = function(e) {
      ...
      el = e.currentTarget;
      var optionEls = Array.prototype.slice.call(this.el.querySelectorAll('.option'));
      var closestOption = el.closest('.option');
      i = closestOption ? optionEls.indexOf(closestOption) : -1;
    ...
    EditFieldView.prototype.removeOption = function(e) {
      ...
      el = e.currentTarget;
      var removeButtons = Array.prototype.slice.call(this.el.querySelectorAll('.js-remove-option'));
      index = removeButtons.indexOf(el);
      options = this.model.get(Formbuilder.options.mappings.OPTIONS);
    ...
    EditFieldView.prototype.defaultUpdated = function(e) {
      ...
      if (this.model.get(Formbuilder.options.mappings.FIELD_TYPE) !== 'checkboxes') {
        var defaults = this.el.querySelectorAll('.js-default-updated');
        for (var d = 0; d < defaults.length; d++) {
          if (defaults[d] !== el) {
            defaults[d].checked = false;
            defaults[d].dispatchEvent(new Event('change'));
          }
        }
      }
      return this.forceRender();
  ...
  BuilderView = (function(_super) {
    ...
    BuilderView.prototype.bindSaveEvent = function() {
      var _this = this;
      this.formSaved = true;
      this.saveFormButton = this.el.querySelector('.js-save-form');
      if (this.saveFormButton) {
        this.saveFormButton.setAttribute('disabled', true);
        this.saveFormButton.textContent = Formbuilder.options.dict.ALL_CHANGES_SAVED;
      }
    ...
    BuilderView.prototype.reset = function() {
      this._responseFields().innerHTML = '';
      return this.addAll();
    };

    BuilderView.prototype.render = function() {
      var subview, _i, _len, _ref5;
      this.el.innerHTML = Formbuilder.templates['page']();
      this._fbLeft = this.el.querySelector('.fb-left');
      this._fbLeft._locked = false;
    ...
    BuilderView.prototype._responseFields = function() {
      return this.el.querySelector('.fb-response-fields');
    };
    ...
    BuilderView.prototype.bindWindowScrollEvent = function() {
      ...
      return window.addEventListener('scroll', function() {
        ...
        if (_this._fbLeft._locked === true) {
          return;
        }
        var elRect = _this.el.getBoundingClientRect();
        var elTop = elRect.top + window.pageYOffset;
        newMargin = Math.max(0, window.pageYOffset - elTop);
        maxMargin = _this._responseFields().offsetHeight;
        _this._fbLeft.style.marginTop = Math.min(maxMargin, newMargin) + 'px';
      });
    };
    ...
    BuilderView.prototype.showTab = function(e) {
      ...
      var targetPane = this.el.querySelector(target);
      if (targetPane) {
        var paneSiblings = targetPane.parentNode.querySelectorAll('.fb-tab-pane');
        for (var p = 0; p < paneSiblings.length; p++) {
          paneSiblings[p].classList.remove('active');
        }
    ...
    BuilderView.prototype.addOne = function(responseField, _, options) {
      ...
      var rf = this._responseFields();
      if (options.$replaceEl != null) {
        options.$replaceEl.parentNode.replaceChild(view.render().el, options.$replaceEl);
      } else if ((options.position == null) || options.position === -1) {
        rf.appendChild(view.render().el);
      } else if (options.position === 0) {
        rf.insertBefore(view.render().el, rf.firstChild);
      } else {
        var wrappers = rf.querySelectorAll('.fb-field-wrapper');
        var atPos = wrappers[options.position];
        if (atPos) {
          rf.insertBefore(view.render().el, atPos);
        } else {
          rf.appendChild(view.render().el);
        }
      }
    ...
    BuilderView.prototype.setSortable = function() {
      ...
      // Simple drag-and-drop reordering using HTML5 drag events
      var dragSrcEl = null;
      ...
      function handleDrop(e) {
        ...
        if (dragSrcEl !== this) {
          ...
          rf.insertBefore(dragSrcEl, this);
        }
      ...
      function handleDragEnd() {
        ...
        var items = rf.querySelectorAll('.fb-field-wrapper');
        for (var i = 0; i < items.length; i++) {
          items[i].classList.remove('sortable-placeholder');
        }
        _this.ensureEditViewScrolled();
      }
      ...
      // Use MutationObserver to rebind drag events when items are added
      var observer = new MutationObserver(function() {
        bindDragEvents();
      });
      observer.observe(rf, { childList: true });
      bindDragEvents();
    ...
    BuilderView.prototype.setDraggable = function() {
      var _this = this;
      var addFieldButtons = this.el.querySelectorAll('[data-field-type]');
      // Field-type buttons act as click-to-add (drag from sidebar not critical)
      // Drag from sidebar into the sortable area is handled by addField click event
    };
    ...
    BuilderView.prototype.hideShowNoResponseFields = function() {
      var noFields = this.el.querySelector('.fb-no-response-fields');
      if (noFields) {
        noFields.style.display = this.collection.length > 0 ? 'none' : '';
      }
    };

    BuilderView.prototype.addField = function(e) {
      var field_type;
      field_type = e.currentTarget.getAttribute('data-field-type');
    ...
    BuilderView.prototype.createAndShowEditView = function(model) {
      ...
      var wrappers = this.el.querySelectorAll('.fb-field-wrapper');
      var matchEl = null;
      for (var w = 0; w < wrappers.length; w++) {
        if (wrappers[w]._cid === model.cid) {
          matchEl = wrappers[w];
        }
        wrappers[w].classList.remove('editing');
      }
      if (matchEl) {
        matchEl.classList.add('editing');
      }
      if (this.editView) {
        if (this.editView.model.cid === model.cid) {
          var editTab = this.el.querySelector('.fb-tabs a[data-target="#editField"]');
          if (editTab) { editTab.click(); }
          this.scrollLeftWrapper(matchEl);
          return;
        }
      ...
      var editWrapper = this.el.querySelector('.fb-edit-field-wrapper');
      if (editWrapper) {
        editWrapper.innerHTML = '';
        editWrapper.appendChild($newEditEl);
      }
      var editTabLink = this.el.querySelector('.fb-tabs a[data-target="#editField"]');
    ...
    BuilderView.prototype.ensureEditViewScrolled = function() {
      ...
      return this.scrollLeftWrapper(document.querySelector('.fb-field-wrapper.editing'));
    };
    ...
    BuilderView.prototype.scrollLeftWrapper = function(responseFieldEl) {
      ...
      var fieldRect = responseFieldEl.getBoundingClientRect();
      var rfRect = this._responseFields().getBoundingClientRect();
      var scrollTarget = (elRect.top + window.pageYOffset + fieldRect.top - rfRect.top);
      window.scrollTo({
        top: scrollTarget,
        behavior: 'smooth'
      });
      setTimeout(function() {
        _this.lockLeftWrapper();
      }, 200);
    };

    BuilderView.prototype.lockLeftWrapper = function() {
      this._fbLeft._locked = true;
    };
    ...
    BuilderView.prototype.handleFormUpdate = function() {
      ...
      if (this.saveFormButton) {
        this.saveFormButton.removeAttribute('disabled');
        this.saveFormButton.textContent = Formbuilder.options.dict.SAVE_FORM;
      }
    };
    ...
    BuilderView.prototype.doAjaxSave = function(payload) {
      var _this = this;
      return fetch(Formbuilder.options.HTTP_ENDPOINT, {
        method: Formbuilder.options.HTTP_METHOD,
  ...
  Formbuilder = (function() {
    Formbuilder.helpers = {
      defaultFieldAttrs: function(field_type) {
        ...
        attrs[Formbuilder.options.mappings.NAME] = '';
        attrs[Formbuilder.options.mappings.FIELD_TYPE] = field_type;
        attrs[Formbuilder.options.mappings.FIELD_ID] = 0;
...
(function() {
  Formbuilder.registerField('calendar', {
    order: 20,
    view: "<input type='text' class='calendar rf-size-<%= rf.get(Formbuilder.options.mappings.SIZE) %>' placeholder='YYYY-MM-DD HH:mm:ss' />",
    edit: "",
    addButton: "<span class=\"symbol\"><span class=\"fa fa-calendar\"></span></span> Date/Time"
  });
}).call(this);
...
(function() {
  Formbuilder.registerField('dropdown', {
    order: 25,
    view: "<select>\n  <% if (rf.get(Formbuilder.options.mappings.INCLUDE_BLANK)) { %>\n    <option value=''>- Select -</option>\n  <% } %>\n\n  <% for (i in (rf.get(Formbuilder.options.mappings.OPTIONS) || [])) { %>\n    <option <%= rf.get(Formbuilder.options.mappings.OPTIONS)[i].checked && 'selected' %>>\n      <%= rf.get(Formbuilder.options.mappings.OPTIONS)[i].label %>\n    </option>\n  <% } %>\n</select><% if (rf.get(Formbuilder.options.mappings.INCLUDE_OTHER)) { %>\n  <div class='dropdown-other-option'>\n    <input type='text' placeholder='Other...' />\n  </div>\n<% } %>",
    edit: "<%= Formbuilder.templates['edit/options']({ includeBlank: true, includeOther: true }) %>",
...
(function() {
  Formbuilder.registerField('country', {
    order: 30,
    view: "<select>\n  <% if (rf.get(Formbuilder.options.mappings.INCLUDE_BLANK)) { %>\n    <option value=''>- Select -</option>\n  <% } %>\n\n  <option>- List of countries is auto-populated -</option>\n  </select>",
    edit: "<%= Formbuilder.templates['edit/options']({ includeBlank: true }) %>",
    addButton: "<span class=\"symbol\"><span class=\"fa fa-country\"></span></span> Country",
    defaultAttributes: function(attrs) {
      attrs.field_options.include_blank_option = false;
      return attrs;
    }
...
(function() {
  Formbuilder.registerField('email', {
    order: 40,
    view: "<input type='text' class='rf-size-<%= rf.get(Formbuilder.options.mappings.SIZE) %>' placeholder='you@somesite.com' />",
    edit: "",
...
(function() {
  Formbuilder.registerField('hidden', {
    order: 40,
    view: "<input type='text' class='rf-size-<%= rf.get(Formbuilder.options.mappings.SIZE) %>' placeholder='Hidden value' />",
    edit: "",
    addButton: "<span class=\"symbol\"><span class=\"fa fa-hidden\"></span></span> Hidden"
  });
}).call(this);

(function() {
  Formbuilder.registerField('orcid', {
    order: 40,
    view: "<input type='text' class='rf-size-<%= rf.get(Formbuilder.options.mappings.SIZE) %>' placeholder='####-####-####-####' />",
    edit: "",
    addButton: "<span class=\"symbol\"><span class=\"fa fa-orcid\"></span></span> ORCID"
  });
}).call(this);

(function() {
  Formbuilder.registerField('tags', {
    order: 40,
    view: "<input type='text' class='rf-size-<%= rf.get(Formbuilder.options.mappings.SIZE) %>' placeholder='Keyword or phrases separated by a comma or semicolon' />",
    edit: "",
    addButton: "<span class=\"symbol\"><span class=\"fa fa-tags\"></span></span> Tags"
  });
}).call(this);
