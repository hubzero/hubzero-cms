...
<?php if ($this->getError() && $this->getError() == 'login mismatch') : ?>
	<p class="warning">
		You are currently logged in as <strong><?php echo $this->escape($this->login); ?></strong>. If you're trying to activate a different account,
		you may do so by <a href="<?php echo $this->escape($this->redirect); ?>">confirming a different email address</a>.
	</p>
...
<?php elseif (!empty($this->justConfirmed)) : ?>
	<p class="passed">Thank you! Your email address "<?php echo $this->escape($this->email); ?>" has been confirmed and your account is now active. Please <a href="<?php echo $this->escape($this->loginUrl); ?>">log in</a> to start using <?php echo $this->escape($this->sitename); ?>.</p>
<?php else : ?>
