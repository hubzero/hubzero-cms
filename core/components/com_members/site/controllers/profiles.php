...
class Profiles extends SiteController
{
    ...
    public function autocompleteTask()
    {
        ...
        if (preg_match('/\d{4}-\d{4}-\d{4}-\d{4}/', $filters['search'])) {
            $orcid = $this->database->quote($filters['search']);
            $query = "SELECT u.id, u.name, u.username, u.access
					FROM `#__users` AS u
					WHERE u.block = 0 AND orcid= " . $orcid . " AND u.activation>0 $restrict
					ORDER BY u.name ASC
            ...
            // Example: MATCH(u.givenName,u.middleName,u.surname)
            //          AGAINST($filter['search'] IN BOOLEAN MODE)
            $searchLower = strtolower($filters['search']);
            $namePattern = $this->database->quote('%' . $searchLower . '%');

            if (strstr($filters['search'], ' ')) {
                $parts = explode(' ', $filters['search']);
                $part0Pattern = $this->database->quote('%' . strtolower($parts[0]) . '%');
                $part1Pattern = $this->database->quote('%' . strtolower($parts[1]) . '%');

                // Name with space could be "first middle last", "first last", or "first middle"
                $match = "(LOWER(u.name) LIKE " . $namePattern . "
					OR (LOWER(u.givenName) LIKE " . $part0Pattern . "
					AND (LOWER(u.middleName) LIKE " . $part1Pattern . "
					OR LOWER(u.surname) LIKE " . $part1Pattern . ")))";
            } else {
                $match = "(LOWER(u.name) LIKE " . $namePattern . "
					OR LOWER(u.givenName) LIKE " . $namePattern . "
					OR LOWER(u.surname) LIKE " . $namePattern . ")";
            }
        ...
        if (count($rows) > 0) {
            foreach ($rows as $row) {
                ...
                $hasAccess = in_array($user->get('access'), User::getAuthorisedViewLevels());
                $obj['org']      = $hasAccess ? $user->get('organization', '') : '';
    ...
    public function browseTask()
    {
        ...
        if ($filters['q']) {
            foreach ($filters['q'] as $q) {
                    ...
                    if ($q['o'] == 'LIKE') {
                        ...
                        foreach ($search as $v => $term) {
                            ...
                            foreach ($searches as $z => $searching) {
                                $alias = 's' . ($v . '_' . $z);
                                $cond = $alias . '.user_id=' . $a . '.id AND '
                                    . $alias . '.profile_key=' . $db->quote($searching);
                                $entries->joinRaw($b . ' AS ' . $alias, $cond, 'left');
                ...
                $alias = 't' . $i;
                $fieldKey = $db->quote($q['field']);
                $baseJoin = $alias . '.user_id=' . $a . '.id AND ' . $alias . '.profile_key=' . $fieldKey;

                if (in_array($q['o'], array('>', '>=', '<', '<=')) && is_numeric($q['value'])) {
                    $cond = $baseJoin . ' AND ' . $alias . '.profile_value '
                        . $q['o'] . ' ' . (int)$q['value'];
                    $entries->joinRaw($b . ' AS ' . $alias, $cond, 'inner');
                } elseif (is_array($q['value'])) {
                    foreach ($q['value'] as $key => $val) {
                        $q['value'][$key] = $db->quote($val);
                    }
                    $cond = $baseJoin . ' AND ' . $alias . '.profile_value IN ('
                        . implode(', ', $q['value']) . ')';
                    $entries->joinRaw($b . ' AS ' . $alias, $cond, 'inner');
                } else {
                    $cond = $baseJoin . ' AND ' . $alias . '.profile_value '
                        . $q['o'] . ' ' . $db->quote($q['value']);
                    $entries->joinRaw($b . ' AS ' . $alias, $cond, 'inner');
                }
        ...
        if (!$sortFound) {
            $alias = 't' . $i;
            $cond = $alias . '.user_id=' . $a . '.id AND '
                . $alias . '.profile_key=' . $db->quote($filters['sort']);
            $entries->joinRaw($b . ' AS ' . $alias, $cond, 'inner');
            $filters['sqlsort'] = $alias . '.profile_value';
        }
    ...
    {
        if (User::isGuest()) {
            $return = Route::url('index.php?option=' . $this->_option . '&task=myaccount', false, true);
            $loginUrl = 'index.php?option=com_users&view=login&return=' . base64_encode($return);
            App::redirect(
                Route::url($loginUrl, false),
    ...
    public function viewTask()
    {
        ...
        // Removed the !$profile->get('approved') check from conditional since
        // the Unapproved System plugin will handle this check.
        ...
        $isOwner = User::get('id') == $profile->get('id');
        $hasViewAccess = in_array($profile->get('access'), User::getAuthorisedViewLevels());
        ...
        $isUnconfirmed = $profile->get('activation') < 1;
        $isViewingOwnProfile = $loggedInUserId == $profile->get('id');
        $canManage = !User::isGuest() && User::authorise('core.manage', $this->_option);

        if ($isUnconfirmed && !$isViewingOwnProfile && !$canManage) {
            App::abort(404, Lang::txt('COM_MEMBERS_NOT_FOUND'));
        ...
        // Get the sections
        $eventParams = array(User::getInstance(), $profile, $this->_option, array($tab));
        $sections = Event::trigger('members.onMembers', $eventParams);
    ...
    public function changepasswordTask()
    {
        ...
        if (User::isGuest()) {
            $default = Route::url(
                'index.php?option=' . $this->_controller . '&task=changepassword',
                false,
                true
            );
            $rtrn = Request::getString('REQUEST_URI', $default, 'server');
        ...
        $hasExistingPass = $hzup && !empty($hzup->passhash);
        $passwordMatches = \Hubzero\User\Password::passwordMatches(
            $profile->get('id'),
            $oldpass,
            true
        );

        if ($hasExistingPass && !$passwordMatches) {
        ...
        } elseif ($hasExistingPass && $oldpass == $newpass) {
            ...
            $errorMsg = 'Password does not meet site password requirements. '
                . 'Please choose a password meeting all the requirements listed below.';
            $this->setError(Lang::txt($errorMsg));
    ...
    public function raiselimitTask()
    {
        ...
        if (User::isGuest()) {
            $default = Route::url(
                'index.php?option=' . $this->_controller . '&task=raiselimit',
                false,
                true
            );
            $rtrn = Request::getString('REQUEST_URI', $default, 'server');
            ...
            switch ($k) {
                case 'sessions':
                    ...
                    if ($this->view->authorized == 'admin') {
                        ...
                        $preferences->store();
                        $resourcemessage = 'The session limit for [' . $profile->get('username')
                            . '] has been raised from ' . $oldlimit . ' to ' . $newlimit . ' sessions.';
                    } elseif ($request === null) {
                        $this->view
                            ->set('title', $title)
                            ->set('resource', $k)
                ...
                case 'storage':
                    ...
                    if (User::authorise('core.manage', $this->_option)) {
                        $resourcemessage = 'The storage limit for [' . $profile->get('username')
                            . '] has been raised from ' . $oldlimit . ' to ' . $newlimit . '.';
                    } else {
                        $this->view
                            ->set('title', $title)
                            ->set('resource', $k)
                ...
                case 'meetings':
                    ...
                    $resourcemessage = ' meeting limit has been raised from '
                    ...
                    if (User::authorise('core.manage', $this->_option)) {
                        ...
                        $resourcemessage = 'The meeting limit for [' . $profile->get('username')
                            . '] has been raised from ' . $oldlimit . ' to ' . $newlimit . '.';
                    } else {
                        $this->view
                            ->set('title', $title)
        ...
        if ($request !== null && !empty($resourcemessage)) {
            ...
            $url = 'index.php?option=' . $this->_option . '&id=' . $profile->get('id')
                . '&task=' . $this->_task;
            $sef = Route::url($url);
            ...
            $this->view
                ...
                ->display();
            return;
        } elseif (User::authorise('core.manage', $this->_option) && !empty($resourcemessage)) {
            // Output the view
            $this->view
                ->set('resourcemessage', $resourcemessage)
                ->setLayout('success')
    ...
    public function editTask($profile = null)
    {
        ...
        if (User::isGuest()) {
            $default = Route::url(
                'index.php?option=' . $this->_controller . '&task=activity',
                false,
                true
            );
            $rtrn = Request::getString('REQUEST_URI', $default, 'server');
    ...
    public function saveTask()
    {
        ...
        if ($email != $oldemail) {
            ...
            $safeEmail = htmlentities($email, ENT_COMPAT, 'UTF-8');

            if ($result) {
                $msg = 'A confirmation email has been sent to "' . $safeEmail
                    . '". You must click the link in that email to re-activate your account.';
                Notify::success($msg);
            } else {
                $msg = 'An error occurred emailing "' . $safeEmail . '" your confirmation.';
                Notify::error($msg);
            }
     ...
     * @return  boolean
     */
    private function sendConfirmationCode($login, $email, $confirm, $name, $registerDate)
    {
        // Email subject
        ...
        $safeEmail = htmlentities($email, ENT_COMPAT, 'UTF-8');
        if ($msg->send()) {
            $message = 'A confirmation email has been sent to "' . $safeEmail
                . '". You must click the link in that email to re-activate your account.';
            Notify::success($message);
            $result = true;
        } else {
            $message = 'An error occurred emailing "' . $safeEmail . '" your confirmation.';
