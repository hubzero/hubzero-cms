...
class Credentials extends SiteController
{
    ...
    public function remindingTask()
    {
        ...
        $subject = Lang::txt('COM_MEMBERS_CREDENTIALS_EMAIL_REMIND_SUBJECT', Config::get('sitename'));
    ...
    public function resettingTask()
    {
        ...
        // Send mail
        if (!$message->send()) {
            $logMsg = 'Members password reset email failed[2]: '
                . Lang::txt('Failed to mail %s', $user->get('email'));
            Log::error($logMsg);
