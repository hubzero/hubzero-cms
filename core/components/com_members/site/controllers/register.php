...
class Register extends SiteController
{
    ...
    public function editTask()
    {
        if (User::isGuest()) {
            ...
            $returnUrl = Route::url(
                'index.php?option=' . $this->_option . '&controller='
                    . $this->_controller . 'task=' . $this->_task,
                false,
                true
            );
            $loginUrl = 'index.php?option=com_users&view=login&return=' . base64_encode($returnUrl);
            App::redirect(Route::url($loginUrl, false));
        ...
        if (Request::getString('edit', '', 'post')) {
            ...
            return $this->showRegistrationForm($xregistration, 'edit');
        }
        ...
        if ($self) {
            ...
            if ($updateEmail) {
                ...
                if (!$msg->send()) {
                    $errKey = 'COM_MEMBERS_REGISTER_ERROR_EMAILING_CONFIRMATION';
                    $this->setError(Lang::txt($errKey /*, $hubMonitorEmail*/));
    ...
    public function updateTask()
    {
        ...
        if (!$force && $check && $method == 'GET') {
            ...
            if (
                $_SERVER['REQUEST_URI'] == rtrim(Request::base(true), '/') . '/register/update'
        ...
        if (!$force && $check && $method == 'POST') {
            ...
            $keys = array(
                'email', 'name', 'surname', 'givenName', 'middleName',
                'usageAgreement', 'sendEmail', 'password'
            );
            foreach ($keys as $key) {
            ...
            if ($xprofile->save()) {
                ...
                if (!$member->saveProfile($profile, $access)) {
                    \Hubzero\Facades\Notify::error($member->getError());
        ...
        return $this->showRegistrationForm($xregistration, 'update');
    }
    ...
    public function createTask()
    {
        ...
        if (Request::getMethod() == 'POST') {
            ...
            if ($result) {
                ...
                if ($user->save()) {
                    ...
                    \Hubzero\Facades\Notify::error($user->getError());
                    $result = false;
                }
                ...
                if ($result) {
                    ...
                    if ($password) {
                        $newPass = $xregistration->get('password');
        ...
        return $this->showRegistrationForm($xregistration, 'create');
    }
    ...
    public function passwordstrengthTask()
    {
        ...
        // Build the HTML
        $html = '<span id="passwd-meter" style="width:' . $score . '%;" class="' . $cls . '">'
            . '<span>' . Lang::txt($txt) . '</span></span>';
            ...
            if ($pemail && \Components\Members\Helpers\Utility::validemail($pemail) /*&& ($newemail != $email)*/) {
                ...
                if ($pemail == $email) {
                    ...
                    if (!$this->getError()) {
                        ...
                        $subject  = $sitename . ' ' . Lang::txt('COM_MEMBERS_REGISTER_EMAIL_CONFIRMATION');
    ...
    public function confirmTask()
    {
        ...
        if (User::isGuest()) {
            ...
            if ($email != false && \Plugin::isEnabled('authentication', 'emailtoken')) {
                ...
                $confirmUrl = 'index.php?option=' . $this->_option . '&controller=' . $this->_controller
                    . '&task=' . $this->_task . '&confirm=' . $code . '&return=' . $return;
                $return = base64_encode(Route::url($confirmUrl, false, true));
        ...
        if ($xprofile->get('id') != $user->get('id')) {
            ...
            $confirmUrl = 'index.php?option=' . $this->option . '&controller='
                . $this->_controller . '&task=' . $this->_task . '&confirm=' . $code;
            $login_return  = base64_encode(Route::url($confirmUrl));
            $loginUrl = 'index.php?option=com_users&view=login&return=' . $login_return;
            $logout_return = base64_encode(Route::url($loginUrl));
    ...
    protected function _buildTitle()
    {
        ...
        \Hubzero\Facades\Document::setTitle($title);
    }
