...
class Media extends SiteController
{
    ...
    public function execute()
    {
        ...
        if (
            substr(strtolower($file), 0, 5) == 'image'
    ...
    public function uploadTask()
    {
        ...
        if ($file['size'] > $sizeLimit) {
            $maxSize = \Hubzero\Utility\Number::formatBytes($sizeLimit);
            $this->setError(Lang::txt('COM_MEMBERS_ERROR_UPLOADING_FILE_TOO_BIG', $maxSize));
    ...
    public function downloadTask()
    {
        ...
        if (Request::getString('active', 'profile') == 'blog') {
            ...
            $paddedId = \Hubzero\Utility\Str::pad($member->get('id'), 5);
            $base_path = str_replace('{{uid}}', $paddedId, $blog_params->get('uploadpath'));
        }
