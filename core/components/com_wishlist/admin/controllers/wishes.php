...
class Wishes extends AdminController
{
            ...
            foreach ($lists as $k => $list) {
                ...
                if (count($owners['individuals']) > 0) {
                    $query = "SELECT a.id, a.name FROM `#__users` AS a "
                        . "WHERE a.block = '0' AND a.id IN (" . implode(',', $owners['individuals']) . ") "
                        . "ORDER BY a.name";
    ...
    public function saveTask()
    {
        ...
        $oldText = rtrim(stripslashes($old->get('pagetext')));
        $newText = rtrim(stripslashes($page->get('pagetext')));
        if ($create_revision && $oldText != $newText) {
    ...
    public function stateTask()
    {
        ...
        if (count($ids) < 1) {
            $msg = $state
                ? Lang::txt('COM_WISHLIST_SELECT_PUBLISH')
                : Lang::txt('COM_WISHLIST_SELECT_UNPUBLISH');
            Notify::warning($msg);
            return $this->cancelTask();
        }
    ...
    public function cancelTask()
    {
        ...
        $url = 'index.php?option=' . $this->_option
            . '&controller=' . $this->_controller . '&wishlist=' . $wishlist;
