...
class Wishlist extends Relational
{
     ...
     * @var integer
     */
    public const WISHLIST_STATE_PRIVATE = 0;
     ...
     * @var integer
     */
    public const WISHLIST_STATE_PUBLIC  = 1;
    ...
    public function stage()
    {
        ...
        if (!$this->adapter()->exists()) {
            $msg = Lang::txt(
                'Item of category "%s" and ID of "%s" could not be found.',
                $this->get('category'),
        ...
        $this->set('description', $this->adapter()->title());
        $this->set('public', 1);
    ...
    {
        return $this->adapter()->pathway($title, $pathway);
    }
        ...
        if (!$this->adapter) {
            ...
            foreach ($files as $file) {
                ...
                if ($adapter->handles($scope)) {
                    ...
                    $this->adapter->set('wishlist', $this->get('id'));
                    break;
                }
            }
    ...
    public function removeOwner($what, $data)
    {
        ...
        foreach ($data as $result) {
            switch ($what) {
                ...
                case 'groups':
                    $group_id = (int) $this->groupId($result);
     ...
     * @return  integer
     */
    private function groupId($group)
    {
        if (is_numeric($group)) {
            return $group;
        }
    ...
    public function access($action = 'view', $assetType = 'list', $assetId = null)
    {
        if (!$this->config()->get('access-check-list-done', false)) {
            ...
            if (!User::isGuest()) {
                ...
                $this->config()->set($editStateKey, User::authorise('core.edit.state' . $at, $asset));
                ...
                if ($item->rankings->count() > 0) {
                    ...
                    foreach ($item->rankings as $vote) {
                        if (in_array($vote->get('userid'), $voters)) {
                            ...
                            if ($vote->get('effort') != 6) { // ignore "don't know" selection
                                $eff += $vote->get('effort');
                            } else {
                                $skipped++;
                            }
