...
class Wishlists extends SiteController
{
    ...
    protected function _buildPathway($wishlist)
    {
        ...
        if ($this->_task) {
            switch ($this->_task) {
                ...
                default:
                    // XSS fix, passing raw user supplied/maniuplatable data
                    // to function that creates link. See ticket 1420
                    Pathway::append(
    ...
    {
        if (User::isGuest()) {
            $url = 'index.php?option=' . $this->_option . '&task=' . $this->_task;
            $return = base64_encode(Request::getString('REQUEST_URI', Route::url($url, false, true), 'server'));
            App::redirect(
    ...
    public function wishlistTask()
    {
        ...
        // Set page title
        $showTitle = $model->isPublic() || (!$model->isPublic() && $this->_admin == 2);
        $this->_list_title = $showTitle ? $model->get('title') : '';
        $this->_subtitle   = $showTitle ? $model->get('title') : '';
        $this->_buildTitle();
        ...
        $voteTable = $vote->getTableName();
        $entryTable = $entries->getTableName();
        $positiveSubquery = "(SELECT COUNT(*) FROM `" . $voteTable . "` AS v "
            . "WHERE v.helpful='yes' AND v.category='wish' AND v.referenceid=" . $entryTable . ".id)";
        $negativeSubquery = "(SELECT COUNT(*) FROM `" . $voteTable . "` AS v "
            . "WHERE v.helpful='no' AND v.category='wish' AND v.referenceid=" . $entryTable . ".id)";

        $entries
            ->select($entryTable . '.*')
            ->select($positiveSubquery, 'positive')
            ->select($negativeSubquery, 'negative');
        ...
        if ($filters['sortby']) {
            switch ($filters['sortby']) {
                ...
                case 'ranking':
                    ...
                    break;
                case 'bonus':
                    $bonusSubquery = "(SELECT SUM(amount) FROM `#__users_transactions` "
                        . "WHERE category='wish' AND type='hold' AND referenceid=" . $entryTable . ".id)";
                    $entries
                        ->select($bonusSubquery, 'bonus')
                        ->order('status', 'asc')
                        ->order('bonus', 'desc')
                        ->order('positive', 'desc')
                        ->order('proposed', 'desc');
        ...
        $model->set('granted_count', $model->wishes('count', $sp_filters, true));
        $grantedCount = $model->get('granted_count');
        $percentage = ($total > 0 && $grantedCount > 0 ? round(($grantedCount / $total) * 100, 0) : 0);
        $model->set('granted_percentage', $percentage);
        */

        // Some extras
        $model->set('banking', ($this->banking ? $this->banking : 0));
        // do not allow points for individual wish lists
        $model->set('banking', ($model->get('category') == 'user' ? 0 : $this->banking));
    ...
    public function wishTask()
    {
        ...
        if ($votes->count() > 0) {
            ...
            foreach ($votes as $vote) {
                $isAdvisory = count($owners['advisory']) > 0
                    && $this->config->get('votesplit', 0)
                    && in_array($vote->get('userid'), $owners['advisory']);
                if ($isAdvisory) {
    ...
    public function saveplanTask()
    {
        ...
        if ($new_assignee) {
            ...
            // to wish assignee
            $subject = Lang::txt(strtoupper($this->_option)) . ', '
                . Lang::txt('COM_WISHLIST_WISH') . ' #' . $wishid . ' '
                . Lang::txt('COM_WISHLIST_MSG_HAS_BEEN_ASSIGNED_TO_YOU');
            ...
            // Send message
            $recipients = array($objWish->get('assigned'));
            $msgParams = array('wishlist_wish_assigned', $subject, $message, $from, $recipients, $this->_option);
            if (!Event::trigger('xmessage.onSendMessage', $msgParams)) {
    ...
    public function addwishTask()
    {
        ...
        // Build page title
        $canSeeTitle = $wishlist->isPublic() || (!$wishlist->isPublic() && $wishlist->access('manage'));
        $this->_list_title = $canSeeTitle ? $wishlist->get('title') : '';
        $this->_buildTitle();

        // Set the pathway
        $addUrl = 'index.php?option=' . $this->_option . '&task=add&category=' . $category . '&rid=' . $refid;
        $this->_taskpath = $wish->get('id') ? $wish->link('edit') : $addUrl;
    ...
    public function savewishTask()
    {
        ...
        if ($this->getError()) {
            // Set the pathway
            $cat = $wishlist->get('category');
            $rid = $wishlist->get('referenceid');
            $editUrl = 'index.php?option=' . $this->_option . '&task=editwish&category='
                . $cat . '&rid=' . $rid . '&wishid=' . $wishid;
            $addUrl = 'index.php?option=' . $this->_option . '&task=add&category=' . $cat . '&rid=' . $rid;
            $this->_taskpath = $wishid ? $editUrl : $addUrl;
        ...
        if (!$wishid) {
            ...
            $subject = Lang::txt(strtoupper($this->_option)) . ', '
                . Lang::txt('COM_WISHLIST_NEW_WISH') . ' '
                . Lang::txt('COM_WISHLIST_FOR') . ' ' . $this->_list_title . ' '
                . Lang::txt('from') . ' ' . $name;
            $from = array(
                'name'  => Config::get('sitename') . ' ' . Lang::txt(strtoupper($this->_option)),
                'email' => Config::get('mailfrom')
            ...
            $msgParams = array(
                'wishlist_new_wish', $subject, $message, $from, $owners['individuals'], $this->_option
            );
            if (!Event::trigger('xmessage.onSendMessage', $msgParams)) {
        ...
        if ($reward && $this->banking) {
            ...
            $BTL = new Teller(User::get('id'));
            $holdMsg = Lang::txt('COM_WISHLIST_BANKING_HOLD') . ' #' . $row->get('id') . ' '
                . Lang::txt('COM_WISHLIST_FOR') . ' ' . $this->_list_title;
            $BTL->hold($reward, $holdMsg, 'wish', $row->get('id'));
        }

        // Log activity
        $activityKey = 'COM_WISHLIST_ACTIVITY_WISH_' . ($wishid ? 'UPDATED' : 'CREATED');
        $activityLink = '<a href="' . Route::url($row->link('permalink')) . '">'
            . $row->get('subject') . '</a>';
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope'       => 'wishlist.wish',
                'scope_id'    => $row->get('id'),
                'description' => Lang::txt($activityKey, $activityLink),
                'details'     => array(
                    'subject'    => $row->get('subject'),
    ...
    public function editwishTask()
    {
        ...
        if (User::isGuest()) {
            // Set page title
            $canSeeTitle = $wishlist->isPublic() || (!$wishlist->isPublic() && $wishlist->get('admin') == 2);
            $this->_list_title = $canSeeTitle ? $wishlist->get('title') : '';
            $this->_buildTitle();
        ...
        if ($this->_task == 'editwish' && ($status = Request::getWord('status', ''))) {
            ...
            if ($changed) {
                ...
                // to wish author
                $subject1 = Lang::txt(strtoupper($this->_option)) . ', '
                    . Lang::txt('COM_WISHLIST_YOUR_WISH') . ' #' . $wish->get('id') . ' is ' . $status;

                // to wish assignee
                $subject2 = Lang::txt(strtoupper($this->_option)) . ', '
                    . Lang::txt('COM_WISHLIST_WISH') . ' #' . $wish->get('id') . ' '
                    . Lang::txt('COM_WISHLIST_HAS_BEEN') . ' '
                    . Lang::txt('COM_WISHLIST_MSG_ASSIGNED_TO_YOU');
        ...
        if ($changed) {
            ...
            if (!$wish->save()) {
                ...
                $msgParams = array(
                    'wishlist_status_changed', $subject1, $message, $from,
                    array($wish->get('proposed_by')), $this->_option
                );
                if (!Event::trigger('xmessage.onSendMessage', $msgParams)) {
                ...
                if (
                    ...
                    $msgParams2 = array(
                        'wishlist_wish_assigned', $subject2, $message, $from,
                        array($wish->get('assigned')), $this->_option
                    );
                    if (!Event::trigger('xmessage.onSendMessage', $msgParams2)) {
                        $this->setError(Lang::txt('COM_WISHLIST_ERROR_FAILED_MSG_ASSIGNEE'));
                    }
                }
            }
    ...
    public function movewishTask()
    {
        ...
        if ($category == 'question' or $category == 'ticket') {
            ...
            if ($listid != $newlist->get('id')) {
                ...
                $subject1 = Lang::txt(strtoupper($this->_option)) . ', '
                    . Lang::txt('COM_WISHLIST_NEW_WISH') . ' '
                    . Lang::txt('COM_WISHLIST_FOR') . ' ' . $newtitle . ' '
                    . Lang::txt('COM_WISHLIST_FROM') . ' ' . $name . ' - '
                    . Lang::txt('COM_WISHLIST_TRANSFERRED');
                $subject2 = Lang::txt(strtoupper($this->_option)) . ', '
                    . Lang::txt('COM_WISHLIST_YOUR_WISH') . ' #' . $wishid . ' '
                    . Lang::txt('COM_WISHLIST_WISH_TRANSFERRED_TO_DIFFERENT_LIST');
                ...
                $eventArgs = array(
                    'support_item_transferred',
                    $subject2,
                    $message,
                    $from,
                    array($wish->get('proposed_by')),
                    $this->_option
                );
                if (!Event::trigger('xmessage.onSendMessage', $eventArgs)) {
                    $this->setError(Lang::txt('COM_WISHLIST_ERROR_FAILED_MSG_AUTHOR'));
    ...
    public function addbonusTask()
    {
        ...
        // put the  amount on hold
        $BTL = new Teller(User::get('id'));
        $holdMsg = Lang::txt('COM_WISHLIST_BANKING_HOLD') . ' #' . $wish->get('id')
            . ' ' . Lang::txt('COM_WISHLIST_FOR') . ' ' . $wishlist->get('title');
        $BTL->hold(
            $amount,
            $holdMsg,
            'wish',
    ...
    public function deletewishTask()
    {
        ...
        // Log activity
        $wishUrl = Route::url($wish->link('permalink'));
        $wishLink = '<a href="' . $wishUrl . '">' . $wish->get('subject') . '</a>';
        Event::trigger('system.logActivity', [
            'activity' => [
                'action'      => 'deleted',
                'scope'       => 'wishlist.wish',
                'scope_id'    => $wish->get('id'),
                'description' => Lang::txt('COM_WISHLIST_ACTIVITY_WISH_DELETED', $wishLink),
                'details'     => array(
    ...
    public function savevoteTask()
    {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope'       => 'wishlist.wish',
                'scope_id'    => $wish->get('id'),
                'description' => Lang::txt('COM_WISHLIST_ACTIVITY_WISH_VOTED', $wishLink),
                'details'     => array(
                    'subject' => $wish->get('subject'),
                    'url'     => $wishUrl
                )
    ...
    public function savereplyTask()
    {
        ...
        if ($id && $category) {
            ...
            $subject = Lang::txt(strtoupper($this->_option)) . ', '
                . Lang::txt('COM_WISHLIST_MSG_COMENT_POSTED_YOUR_WISH') . ' #' . $wishid
                . ' ' . Lang::txt('BY') . ' ' . $name;
            ...
            // for the wish owner
            $subject1 = Lang::txt(strtoupper($this->_option)) . ', ' . $name . ' '
                . Lang::txt('COM_WISHLIST_MSG_COMMENTED_YOUR_WISH') . ' #' . $wishid;

            // for the person to whom wish is assigned
            $subject2 = Lang::txt(strtoupper($this->_option)) . ', ' . $name . ' '
                . Lang::txt('COM_WISHLIST_MSG_COMMENTED_ON_WISH') . ' #' . $wishid . ' '
                . Lang::txt('COM_WISHLIST_MSG_ASSIGNED_TO_YOU');

            // for original commentor
            $subject3 = Lang::txt(strtoupper($this->_option)) . ', ' . $name . ' '
                . Lang::txt('COM_WISHLIST_MSG_REPLIED_YOUR_COMMENT') . ' #' . $wishid;

            // for others included in the conversation thread.
            $subject4 = Lang::txt(strtoupper($this->_option)) . ', ' . $name . ' '
                . Lang::txt('COM_WISHLIST_MSG_COMMENTED_AFTER_YOU') . ' #' . $wishid;
            ...
            if ($objWish->get('proposed_by') != $row->get('created_by')) {
                ...
                $msgParams = array(
                    'wishlist_comment_posted', $subject1, $message, $from,
                    array($objWish->get('proposed_by')), $this->_option
                );
                if (!Event::trigger('xmessage.onSendMessage', $msgParams)) {
                    $this->setError(Lang::txt('COM_WISHLIST_ERROR_FAILED_MSG_AUTHOR'));
                }
            ...
            if (
                ...
                $msgParams = array(
                    'wishlist_comment_posted', $subject2, $message, $from,
                    array($objWish->get('assigned')), $this->_option
                );
                if (!Event::trigger('xmessage.onSendMessage', $msgParams)) {
                    $this->setError(Lang::txt('COM_WISHLIST_ERROR_FAILED_MSG_ASSIGNEE'));
                }
            ...
            if ($category == 'wishcomment') {
                ...
                if (
                    $parent->get('created_by') != $row->get('created_by')
                    && !in_array($parent->get('created_by'), $contacted)
                ) {
                    $contacted[] = $parent->get('created_by');
                    $msgParams = array(
                        'wishlist_comment_thread', $subject3, $message, $from,
                        array($parent->get('created_by')), $this->_option
                    );
                    if (!Event::trigger('xmessage.onSendMessage', $msgParams)) {
            ...
            if (count($comm) > 0) {
                $msgParams = array(
                    'wishlist_comment_thread', $subject4, $message, $from, $comm, $this->_option
                );
                if (!Event::trigger('xmessage.onSendMessage', $msgParams)) {
                    $this->setError(Lang::txt('COM_WISHLIST_ERROR_FAILED_MSG_COMMENTOR'));
                }
            ...
            // Log activity
            $wishUrl = Route::url($objWish->link());
            $wishLink = '<a href="' . $wishUrl . '">' . $objWish->get('subject') . '</a>';
            Event::trigger('system.logActivity', [
                'activity' => [
                    'action'      => 'created',
                    'scope'       => 'wishlist.comment',
                    'scope_id'    => $row->get('id'),
                    'description' => Lang::txt('COM_WISHLIST_ACTIVITY_COMMENT_CREATED', $row->get('id'), $wishLink),
                    'details'     => array(
                        'wish'    => $objWish->get('id'),
                        'url'     => $wishUrl
                    )
    ...
    public function deletereplyTask()
    {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope'       => 'wishlist.comment',
                'scope_id'    => $row->get('id'),
                'description' => Lang::txt('COM_WISHLIST_ACTIVITY_COMMENT_DELETED', $row->get('id'), $wishLink),
                'details'     => array(
                    'id'  => $row->get('id'),
                    'url' => $wishUrl
                )
    ...
    public function rateitemTask()
    {
        ...
        $filter = '&filterby=' . $filters['filterby'] . '&sortby=' . $filters['sortby']
            . '&limitstart=' . $filters['start'] . '&limit=' . $filters['limit'] . '&tags=' . $filters['tag'];
    ...
    public function getFilters($admin = 0)
    {
        ...
        $validFilters = array('all', 'open', 'accepted', 'rejected', 'granted', 'submitter', 'public', 'private');
        if (!in_array($filters['filterby'], $validFilters)) {
    ...
    public function userSelect($name, $ownerids, $active, $nouser = 0, $javascript = null, $order = 'a.name')
    {
        ...
        if ($nouser) {
            $users[] = \Hubzero\Facades\Html::select('option', '', 'No User', 'value', 'text');
            $users = array_merge($users, $database->loadObjectList());
        } else {
            $users = $database->loadObjectList();
        }

        $users = \Hubzero\Facades\Html::select('genericlist', $users, $name, ' ' . $javascript, 'value', 'text', $active, false, false);
    ...
    public function uploadTask($listdir)
    {
        ...
        if ($file['size'] > $sizeLimit) {
            ...
            $max = preg_replace('/<abbr \w+=\\"\w+\\">(\w{1,3})<\\/abbr>/', '$1', $formattedSize);
