...
class Db
{
	public static function get_db($db = false)
	{
		if (!$db) {
	...
	public static function get_results($sql, &$dd)
	{
		$link = isset($dd['db']) ? static::get_db($dd['db']) : static::get_db();
		$res = array(
			'data'  => array(),
			'total' => 0,
			'found' => 0,
		...
		if ($data !== null) {
			...
			if (isset($dd['total_records'])) {
				...
				$link->setQuery(static::query_gen_total($dd));
	...
	public static function query_gen(&$dd)
	{

		if (!isset($dd['cols']) && isset($dd['table'])) {
			$link = isset($dd['db']) ? static::get_db($dd['db']) : static::get_db();
			$link->setQuery("SELECT DB_column_name, Column_info FROM Columns_Info WHERE Table_name=" . $link->quote($dd['table']));
	...
	public static function query_gen_total($dd)
	{
