...
class Db
{
    public static function getDb($db = false)
    {
        if (!$db) {
            $db = DvConfig::$dv_conf['db'];
        }
        ...
        $link = \Hubzero\Database\Driver::getInstance([
            'driver'   => 'mysql',
            'host'     => $db['host'],
            'user'     => $db['user'],
        ...
        $link->exec("SET SESSION group_concat_max_len = 16384");

        return $link;
    }

    public static function getResults($sql, &$dd)
    {
        $link = isset($dd['db']) ? static::getDb($dd['db']) : static::getDb();
        $res = array(
            'data'  => array(),
            'total' => 0,
            'found' => 0,
        ...
        $link->setQuery($sql);
        $data = $link->loadAssocList();

        if ($data !== null) {
            $res['data'] = $data;

            $link->setQuery('SELECT FOUND_ROWS() AS found');
            $found = $link->loadAssoc();
            $res['found'] = $found ? $found['found'] : 0;

            if (isset($dd['total_records'])) {
                $res['total'] = $dd['total_records'];
            } else {
                $link->setQuery(static::queryGenTotal($dd));
    ...
    public static function queryGen(&$dd)
    {
        ...
        if (!isset($dd['cols']) && isset($dd['table'])) {
            ...
            $link->setQuery("SHOW COLUMNS FROM " . $link->quoteName($dd['table']));
            $result = $link->loadAssocList();
            ...
            foreach ($result as $rec) {
                if (isset($col_info[$rec['Field']])) {
                    ...
                    $dd['cols'][$dd['table'] . '.' . $rec['Field']] = array('label' => $label);
                }
            }
            ...
            $link = isset($dd['db']) ? static::getDb($dd['db']) : static::getDb();
            $tableList = implode(', ', array_map([$link, 'quote'], $tables));
            ...
            foreach ($dd['cols'] as $id => $prop) {
                if (isset($col_info[$id])) {
                    if (isset($col_info[$id]['label'])) {
                        $allowedTags = '<br><hr>';
                        $dd['cols'][$id]['label'] = nl2br(strip_tags($col_info[$id]['label'], $allowedTags));
                    }
        ...
        for ($i = 0; $i < count($cols_vis); $i++) {
            $col_id = $cols_vis[$i];
            $col = $cols[$col_id];
            $searchable = \Hubzero\Facades\Request::getString('bSearchable_' . $i, 'false');
            $fieldtype = \Hubzero\Facades\Request::getString('fieldtype_' . $i, 'string');
            $search_str = \Hubzero\Facades\Request::getString('sSearch_' . $i, '');

            if ($searchable === 'true' && $search_str !== '') {
                $colExpr = '`' . $col_id . '`';
                if ($col['aggr']) {
                    $having_filter[$col_id] = array(
                        'val' => $search_str,
                        'col' => $colExpr,
                        'fieldtype' => $fieldtype
                    );
                } elseif ($col['raw']) {
                    $where_filter[$col_id] = array(
                        'val' => $search_str,
                        'col' => $col['expr'],
                        'fieldtype' => $fieldtype
                    );
        ...
        // Filtered views
        $filters = \Hubzero\Facades\Request::getVar('filter', false);
        if ($filters !== false) {
        ...
        $search_str = \Hubzero\Facades\Request::getString('sSearch', '');
        if ($search_str != '') {
            for ($i = 0; $i < count($cols_vis); $i++) {
                $col_id = $cols_vis[$i];
        ...
        $sorting = \Hubzero\Facades\Request::getVar('iSortCol_0', false);
        if ($sorting !== false && count($cols_vis) > 0) {
            $sort_col_count = \Hubzero\Facades\Request::getInt('iSortingCols', 0);
            for ($i = 0; $i < $sort_col_count; $i++) {
                $idx = \Hubzero\Facades\Request::getInt('iSortCol_' . $i, null);
                $sortable = \Hubzero\Facades\Request::getString('bSortable_' . $idx, 'false');
                if ($sortable === 'true') {
                    $col_id = $cols_vis[$idx];
                    $sort_dir = \Hubzero\Facades\Request::getString('sSortDir_' . $i, 'asc');
                    if ($cols[$col_id]['aggr']) {
        ...
        // Limit
        $limit = '';
        $no_limit = \Hubzero\Facades\Request::getVar('nolimit', false);
        $limit_start = \Hubzero\Facades\Request::getVar('iDisplayStart', false);
        $limit_length = \Hubzero\Facades\Request::getVar('iDisplayLength', DvConfig::$dv_conf['settings']['limit']);
        if ($no_limit === false && $limit_start !== false && $limit_length != '-1') {
            ...
            $limit = " LIMIT 0, " . DvConfig::$dv_conf['settings']['limit'];
        }
    ...
    public static function queryGenTotal($dd)
    {
