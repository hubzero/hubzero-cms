...
class Json
{
                    ...
                    if ($val == null) {
                        ...
                        if (isset($dd['cols'][$key]['type_extra']) && $dd['cols'][$key]['type_extra'] == 'multi') {
                            ...
                            foreach ($list as $l) {
                                ...
                                $link = '/dataviewer/file/' . $db_id
                                    . '/?f=' . $repoPath . DS . $l;

                                $title = $l;
                                $missing = '';
                                if (isset($dd['cols'][$key]['file-verify'])) {
                                    $full_path = DvConfig::$dv_conf['base_path']
                                        . $dd['cols'][$key]['ds-repo-path'] . DS . $l;
                            ...
                            if (isset($dd['cols'][$key]['file-verify'])) {
                                $repoPath = $dd['cols'][$key]['ds-repo-path'];
                                $full_path = DvConfig::$dv_conf['base_path'] . $repoPath . DS . $val;
                                if (!file_exists($full_path)) {
                                    $missing = 'dv-file-missing';
                        ...
                        $isFullLink = isset($dd['cols'][$key]['url-display'])
                            && $dd['cols'][$key]['url-display'] == 'full_link';
                        if ($isFullLink) {
                        ...
                        if (!isset($dd['cols'][$key]['multi'])) {
                            ...
                            $isRepofiles = isset($dd['cols'][$key]['linktype'])
                                && $dd['cols'][$key]['linktype'] == 'repofiles';
                            if ($isRepofiles) {
                                $link = $dd['repo_base'];

                                $linkpath = isset($dd['cols'][$key]['linkpath'])
                                    ? trim($dd['cols'][$key]['linkpath']) : '';
                            ...
                            $val = self::dvToLink($rec, $key, $dd, $val, $preview);
                        } else {
                            ...
                            for ($i = 0; $i < count($multi_val); $i++) {
                                ...
                                $links[] = self::dvToLink($rec, $key, $dd, trim($multi_val[$i]), $preview);
                            }
    ...
    public static function dvToLink($rec, $key, $dd, $val, $preview)
    {
    ...
    public static function dvOpmodSetColor($param)
    {
        return "color: $param;";
    }
}
