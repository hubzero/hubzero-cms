...
class Spreadsheet
{
	public static function render($dd = false)
	{
		$name = $dd['dv_id'];
		...
		\Components\Dataviewer\Site\Lib\Html::dv_add_script('util.js');

		/* jQuery */
		\Components\Dataviewer\Site\Lib\Html::dv_add_script('jquery.js');

		/* Bootstrap */
		\Components\Dataviewer\Site\Lib\Html::dv_add_script('bootstrap/bootstrap.js');
		\Components\Dataviewer\Site\Lib\Html::dv_add_css('bootstrap/css/bootstrap.css');

		/* jQuery-UI */
		\Components\Dataviewer\Site\Lib\Html::dv_add_script('jquery-ui/jquery-ui.min.js');
		\Components\Dataviewer\Site\Lib\Html::dv_add_css('jquery-ui/smoothness/jquery-ui.min.css');

		\Components\Dataviewer\Site\Lib\Html::dv_add_css('font-awesome/css/font-awesome.css');

		\Components\Dataviewer\Site\Lib\Html::dv_add_script('excanvas.js');

		\Components\Dataviewer\Site\Lib\Html::dv_add_script('jquery-datatables/jquery.dataTables.min.js');
		\Components\Dataviewer\Site\Lib\Html::dv_add_css('jquery-datatables/css/jquery.dataTables_themeroller.css');
		\Components\Dataviewer\Site\Lib\Html::dv_add_css('jquery-datatables/css/jquery.dataTables_dv.css');

		\Components\Dataviewer\Site\Lib\Html::dv_add_script('datatables.plugins.js');

		\Components\Dataviewer\Site\Lib\Html::dv_add_script('jqplot/jquery.jqplot.min.js');
		\Components\Dataviewer\Site\Lib\Html::dv_add_script('jqplot/plugins.dev');
		\Components\Dataviewer\Site\Lib\Html::dv_add_css('jqplot/jquery.jqplot.css');

		\Components\Dataviewer\Site\Lib\Html::dv_add_script('spreadsheet.js');
		\Components\Dataviewer\Site\Lib\Html::dv_add_css('spreadsheet.css');

		\Components\Dataviewer\Site\Lib\Html::dv_add_script('dv-spreadsheet-charts.js');
		\Components\Dataviewer\Site\Lib\Html::dv_add_script('dv-spreadsheet-charts-dl.js');

		\Components\Dataviewer\Site\Lib\Html::dv_add_script('dv_custom_views.js');
		\Components\Dataviewer\Site\Lib\Html::dv_add_css('dv_custom_views.css');

		\Components\Dataviewer\Site\Lib\Html::dv_add_script('jquery.lazyload.min.js');

		if (isset($dd['show_maps'])) {
			\Components\Dataviewer\Site\Lib\Html::dv_add_script('/leaflet/leaflet.js');
			\Components\Dataviewer\Site\Lib\Html::dv_add_css('/leaflet/leaflet.css');
			\Components\Dataviewer\Site\Lib\Html::dv_add_script('dv_maps.js');
		}

		\Components\Dataviewer\Site\Lib\Html::dv_add_script('jquery.dv.js');


		DvConfig::$dv_conf['settings']['view']['id'] = $dd['dv_id'];
		DvConfig::$dv_conf['settings']['view']['type'] = 'spreadsheet';
		DvConfig::$dv_conf['settings']['data_url'] = "index.php?option=com_" . DvConfig::$com_name . "&task=data&db="
			. $dd['db_id']['id'] . '&dv=' . $dd['dv_id'];
		DvConfig::$dv_conf['settings']['view_url'] = "/dataviewer/view/{$dd['db_id']['id']}/{$dd['dv_id']}/";
		...
		if ($rec_ids != '') {
			...
			DvConfig::$dv_conf['settings']['view_url'] .= '?dv_first=1';
		}

		// Version
		$version = isset($dd['version']) ? "&v={$dd['version']}" : '';
		DvConfig::$dv_conf['settings']['data_url'] .= $version;
		DvConfig::$dv_conf['settings']['view_url'] .= $version;


		if ($dd) {
			DvConfig::$dv_conf['settings']['show_filter_options'] = isset($dd['filter_options']) ? $dd['filter_options'] : true;
			...
			// overrides
			DvConfig::$dv_conf['settings']['limit'] = (isset($dd['display_limit']))
				? $dd['display_limit']
				: DvConfig::$dv_conf['settings']['limit'];
			DvConfig::$dv_conf['settings']['hide_data'] = isset($dd['hide_data']);
			DvConfig::$dv_conf['settings']['serverside'] = (isset($dd['serverside']) && $dd['serverside']) ? true : false;

			if (!isset($dd['customizer']) && isset(DvConfig::$dv_conf['customizer'])) {
				$dd['customizer'] = DvConfig::$dv_conf['customizer'];
			}
			...
			$res = \Components\Dataviewer\Site\Lib\Db::get_results($sql, $dd);
				...
				if (isset($dd['group_by'])) {
					...
					foreach ($arr as $a) {
						...
						$group_by .= '<div class="dv_customizer_group_by_item_div"'
							...
							. ' class="dv_customizer_group_by_item" value="' . $a . '" />'
			...
			if (isset(DvConfig::$dv_conf['help_file_base_path'])) {
				$help_file = DvConfig::$dv_conf['help_file_base_path'] . $name . '/' . $name . '-help.html';
			...
			if (isset($dd['return']) && isset($dd['return']['raw'])) {
				...
				$return = '<span id="dv_return_link"'
					...
					. '<a href="' . $dd['return']['url'] . '"><strong>'
					. $dd['return']['label'] . '</strong></a></span>';
			}
			...
			$filted_view_str = '';
			if ($filter !== false) {
				DvConfig::$dv_conf['settings']['data_url'] .= '&filter=' . $filter;
				DvConfig::$dv_conf['settings']['filters']['fv_vals'] = $filter;
				$ff = explode('||', $filter);
				foreach ($ff as $f) {
					$f = explode('|', $f);
					$filted_view[$f[0]] = $f[1];
	...
	<div id="dv-spreadsheet">
		...
		<div id="dv-spreadsheet-toolbar" class="ui-corner-top">
			...
			<?php
			if (
				!isset($dd['top_menu']['fullscreen']['show'])
			...
			<div id="dv_customizer_content" style="height:100%; width:auto;"
				...
				<p id="dv_customizer_title_container" style="padding-left: 10px;">
					New Title: <input id="dv_customizer_view_title"
						value="<?php print $dd['title']; ?>" type="text" style="width: 550px;" />
					&nbsp;&nbsp;
					<input id="dv_customizer_group_by_btn"
						data-view-url="<?php print DvConfig::$dv_conf['settings']['view_url']; ?>"
		...
		<div id="dv_help_dialog" style="display: none;">
			<iframe src="<?php print $help_file; ?>" id="modalIframeId" width="100%" height="100%"
				marginWidth="0" marginHeight="0" frameBorder="0" scrolling="auto" title="Help">
		...
		foreach ($dd['cols'] as $id => $conf) {
			if (!isset($conf['hide'])) {
				...
				$title = '';
				$filter_msg = '';
				if (DvConfig::$dv_conf['settings']['serverside']) {
				...
				if (isset($conf['type']) && $conf['type'] == 'image') {
					print '<th><input title="' . $title . '" type="text" placeholder="' . $label
						. '" disabled=disabled /></th>';
				} elseif (isset($filted_view[$id])) {
					print '<th><input type="text" placeholder="' . $filted_view[$id]
						. '" disabled=disabled style="background: yellow;" /></th>';
				} else {
					print '<th><input title="' . $title . '" type="text" placeholder="' . $label
						. '" class="search_init" style="background: #FFF;" />'
						. '<span class="dv-col-clear-filter"'
						. ' style="margin-left: -15px; color: #FFF; cursor: pointer;">'
						. '<i class="icon-remove-sign"></i></span></th>';
				}
			}
		...
		<form style="display: none;" id="dv-spreadsheet-dl" method="POST"
			action="<?php
			print DvConfig::$dv_conf['settings']['data_url'];
			?>&nolimit=true<?php
