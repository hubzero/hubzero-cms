...
class Tag extends Relational
{
    ...
    public function normalize($tag)
    {
        ...
        $tagValue = $tag == null ? '' : $tag;
        $tag = str_replace(array_keys($transliterationTable), array_values($transliterationTable), $tagValue);
    ...
    {
        return $this->datetime($as, 'created');
    }
    ...
    public function modified($as = '')
    {
        ...
        return $this->datetime($as, 'modified');
    }
     ...
     * @return  string
     */
    private function datetime($as = '', $key = 'created')
    {
        $as = strtolower($as);
        $dt = $this->get($key);
    ...
    public function copyTo($tag_id)
    {
        ...
        if (!Objct::copyTo($this->get('id'), $tag_id)) {
            $this->addError('Failed to copy tag associations.');
            return false;
        }
