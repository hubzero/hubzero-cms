...
class Relationships extends AdminController
{
    ...
    public function suggestTask()
    {
        ...
        if (isset($_GET['term'])) {
            $term = $this->database->quote('%' . $_GET['term'] . '%');
            $this->database->setQuery("SELECT raw_tag FROM `#__tags` WHERE raw_tag LIKE " . $term);
            $later = array();
    ...
    public function updateTask()
    {
        if (isset($_POST['tag']) && ($tid = (int)$_POST['tag'])) {
            $desc = $this->database->quote($_POST['description']);
            $this->database->setQuery('UPDATE `#__tags` SET description = ' . $desc . ' WHERE id = ' . $tid);
            ...
            $insertLabel = "INSERT INTO `#__tags_object` (tbl, label, tagid, objectid) "
                . "VALUES ('tags', 'label', %d, %d)";
            $deleteLabel = "DELETE FROM `#__tags_object` "
                . "WHERE tbl = 'tags' AND label = 'label' AND tagid = %d AND objectid = %d";
            $insertLabelRev = "INSERT INTO `#__tags_object` (tbl, label, objectid, tagid) "
                . "VALUES ('tags', 'label', %d, %d)";
            $deleteLabelRev = "DELETE FROM `#__tags_object` "
                . "WHERE tbl = 'tags' AND label = 'label' AND objectid = %d AND tagid = %d";
            $insertParent = "INSERT INTO `#__tags_object` (tbl, label, objectid, tagid) "
                . "VALUES ('tags', 'parent', %d, %d)";
            $deleteParent = "DELETE FROM `#__tags_object` "
                . "WHERE tbl = 'tags' AND label = 'parent' AND objectid = %d AND tagid = %d";
            $insertChild = "INSERT INTO `#__tags_object` (tbl, label, tagid, objectid) "
                . "VALUES ('tags', 'parent', %d, %d)";
            $deleteChild = "DELETE FROM `#__tags_object` "
                . "WHERE tbl = 'tags' AND label = 'parent' AND tagid = %d AND objectid = %d";

            // reconcile post data with what we already know about a tag's relationships
            $relationships = array(
                'labels'   => array($insertLabel, $deleteLabel),
                'labeled'  => array($insertLabelRev, $deleteLabelRev),
                'parents'  => array($insertParent, $deleteParent),
                'children' => array($insertChild, $deleteChild)
            );
            foreach ($relationships as $type => $sql) {
                $ex = array_flip(array_map($normalize, $tag[$type]));
                if (isset($_POST[$type])) {
                    foreach ($_POST[$type] as $n_tag) {
                        $norm_n_tag = preg_replace('/[^a-zA-Z0-9]/', '', strtolower($n_tag));

                        // co-occurring tags neither need to be added nor deleted,
                        // just remove them from the pool and carry on
                ...
                // any tags that have not been unset were deleted on the form,
                // so we need to reflect that in the database
        ...
        if (isset($_POST['tag']) && ($tid = (int)$_POST['tag'])) {
            ...
            if (isset($_POST['really']) && $_POST['really'] === 'on') {
                if (isset($_POST['do_merge']) && $_POST['do_merge'] === 'on') {
                    ...
                    $mergeTag = preg_replace('/[^a-zA-Z0-9]/', '', strtolower(trim($_POST['merge_tag'])));
                    $this->database->setQuery("SELECT id FROM `#__tags` WHERE tag = '" . $mergeTag . "'");
                    if (!($merge_id = $this->database->loadResult())) {
                        throw new Exception('Merge target not found', 404);
                    }
                    $query = 'UPDATE `#__tags_object` SET tagid = ' . $merge_id . ' WHERE tagid = ' . $tid;
    ...
    public function updatefocusareasTask()
    {
        ...
        foreach ($existing as $id => $fa) {
            ...
            $mandatoryVal = $_POST['mandatory-' . $id];
            $multipleVal = $_POST['multiple-' . $id];
            $mandatoryDepth = $mandatoryVal === 'mandatory' ? 1
                : ($mandatoryVal === 'depth' ? (int)$_POST['mandatory-depth-' . $id] : 'NULL');
            $multipleDepth = $multipleVal === 'multiple' ? 1
                : ($multipleVal === 'depth' ? (int)$_POST['multiple-depth-' . $id] : 'NULL');

            $query = 'UPDATE `#__focus_areas` SET mandatory_depth = ' . $mandatoryDepth
                . ', multiple_depth = ' . $multipleDepth
                . ', tag_id = ' . $new_tag['id']
                . ' WHERE id = ' . $id;
            $this->database->setQuery($query);
            $this->database->execute();
            if (isset($_POST['types-' . $id])) {
                foreach ($_POST['types-' . $id] as $type_id) {
                    $query = 'INSERT INTO `#__focus_area_resource_type_rel` '
                        . '(focus_area_id, resource_type_id) VALUES (' . $id . ', ' . ((int)$type_id) . ')';
                    $this->database->setQuery($query);
                    $this->database->execute();
                }
            }
        ...
        for ($idx = 1; isset($_POST['name-new-' . $idx]); ++$idx) {
            ...
            $mandatoryVal = $_POST['mandatory-new-' . $idx];
            $multipleVal = $_POST['multiple-new-' . $idx];
            $mandatoryDepth = $mandatoryVal === 'mandatory' ? 1
                : ($mandatoryVal === 'depth' ? (int)$_POST['mandatory-depth-new-' . $idx] : 'NULL');
            $multipleDepth = $multipleVal === 'multiple' ? 1
                : ($multipleVal === 'depth' ? (int)$_POST['multiple-depth-new-' . $idx] : 'NULL');

            $query = 'INSERT INTO `#__focus_areas` (mandatory_depth, multiple_depth, tag_id) '
                . 'VALUES (' . $mandatoryDepth . ', ' . $multipleDepth . ', ' . $tag['id'] . ')';
            $this->database->setQuery($query);
            $this->database->execute();
    ...
    {
        $baseQuery = 'SELECT DISTINCT t.id, tag, raw_tag, description, COUNT(to1.id) AS count '
            . 'FROM `#__tags` t LEFT JOIN `#__tags_object` to1 ON to1.tagid = t.id ';
        $groupBy = ' GROUP BY t.id, tag, raw_tag, description';

        if (is_int($tag_str)) {
            $query = $baseQuery . 'WHERE t.id = ' . $tag_str . $groupBy;
        } else {
            $quotedTag = $this->database->quote($tag_str);
            $query = $baseQuery . 'WHERE tag = ' . $quotedTag . ' OR raw_tag = ' . $quotedTag . $groupBy;
        }
        $this->database->setQuery($query);
        ...
        $quotedNormTag = $this->database->quote($norm_tag);
        $quotedTagStr = $this->database->quote($tag_str);
        $query = 'INSERT INTO `#__tags` (tag, raw_tag) VALUES (' . $quotedNormTag . ', ' . $quotedTagStr . ')';
        $this->database->setQuery($query);
        $this->database->execute();
        $id = $this->database->insertid();
