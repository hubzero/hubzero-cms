<?php

/**
 * @package    hubzero-cms
 * @copyright  Copyright (c) 2005-2020 The Regents of the University of California.
 * @license    http://opensource.org/licenses/MIT MIT
 */

namespace Components\Tags\Migrations;

use Hubzero\Content\Migration\Base;

/**
 * Migration script for changing DATETIME fields default to NULL for com_tags
 **/
class Migration20190221000000ComTags extends Base
{
    /**
     * List of tables and their datetime fields
     *
     * @var  array
     **/
    public static $tables = array(
        '#__tags' => array(
            'created',
            'modified'
        ),
        '#__tags_log' => array(
            'timestamp'
        ),
        '#__tags_object' => array(
            'taggedon'
        ),
        '#__tags_substitute' => array(
            'created'
        )
    );

    /**
     * Up
     **/
    public function up()
    {
        foreach (self::$tables as $table => $fields) {
            foreach ($fields as $field) {
                if (
                    $this->db->tableExists($table)
                    && $this->db->tableHasField($table, $field)
                ) {
                    $query = "ALTER TABLE `$table` CHANGE `$field` `$field` DATETIME  NULL  DEFAULT NULL";

                    $this->db->setQuery($query);
                    $this->db->query();

                    $query = "UPDATE `$table` SET `$field`=NULL WHERE `$field`='0000-00-00 00:00:00'";

                    $this->db->setQuery($query);
                    $this->db->query();
                }
            }
        }
    }

    /**
     * Down
     **/
    public function down()
    {
        foreach (self::$tables as $table => $fields) {
            foreach ($fields as $field) {
                if (
                    $this->db->tableExists($table)
                    && $this->db->tableHasField($table, $field)
                ) {
                    $default = '0000-00-00 00:00:00';
                    $query = "ALTER TABLE `$table` CHANGE `$field` `$field` DATETIME NOT NULL DEFAULT '$default'";

                    $this->db->setQuery($query);
                    $this->db->query();
                }
            }
        }
    }
}
