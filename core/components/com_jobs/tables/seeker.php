...
class JobSeeker extends Table
{
    ...
    public function getSeekers($filters, $uid = 0, $excludeme = 0, $admin = 0, $count = 0)
    {
        ...
        if ($uid && !$count) {
            // shortlisted users
            $query .= "\n , (SELECT count(*) FROM `#__jobs_shortlist` AS W ";
            $query .= "WHERE W.seeker=s.uid AND W.emp=" . $this->_db->quote($uid);
            $query .= " AND s.uid != " . $this->_db->quote($uid);
            $query .= " AND s.uid=r.uid AND W.category='resume') AS shortlisted ";
            // is this my profile?
            $query .= "\n , (SELECT count(*) FROM `#__jobs_seekers` AS s ";
            $query .= "WHERE s.uid=" . $this->_db->quote($uid) . " AND s.uid=r.uid) AS mine ";
        }
        ...
        $query .= "JOIN `#__jobs_seekers` AS s ON s.uid=x.uidNumber ";
        $query .= "JOIN `#__jobs_resumes` AS r ON r.uid=s.uid  ";

        // Get shortlisted only
        if ($filters['filterby'] == 'shortlisted') {
            $query .= " JOIN `#__jobs_shortlist` AS W ON W.seeker=s.uid ";
            $query .= "AND W.emp=" . $this->_db->quote($uid);
            $query .= " AND s.uid != " . $this->_db->quote($uid);
            $query .= " AND s.uid=r.uid AND W.category='resume' ";
        }

        // Get applied only
        if ($filters['filterby'] == 'applied') {
            $query .= " JOIN `#__jobs_openings` AS J ON J.employerid=";
            $query .= $this->_db->quote($empid);
            $query .= " JOIN `#__jobs_applications` AS A ON A.jid=J.id ";
            $query .= "AND A.uid=s.uid AND A.status=1  ";
        }
    ...
    public function getSeeker($uid, $eid = 0, $admin = 0)
    {
        ...
        $query  = "SELECT DISTINCT x.name, x.countryresident, r.title, r.filename, r.created, ";
        $query .= "s.uid, s.lookingfor, s.tagline, s.sought_cid, s.sought_type, ";
        $query .= "s.updated, s.linkedin, s.url ";
        if ($eid) {
            $query .= "\n , (SELECT count(*) FROM `#__jobs_shortlist` AS W ";
            $query .= "WHERE W.seeker=s.uid AND W.emp=" . $this->_db->quote($eid);
            $query .= " AND s.uid=r.uid AND s.uid=" . $this->_db->quote($uid);
            $query .= " AND s.uid != " . $this->_db->quote($eid);
            $query .= " AND W.category='resume') AS shortlisted ";
        }
        $query .= "\n , (SELECT count(*) FROM `#__jobs_seekers` AS s ";
        $query .= "WHERE s.uid=" . $this->_db->quote($uid) . " AND s.uid=r.uid ";
        $query .= "AND s.uid = " . $this->_db->quote(User::get('id')) . ") AS mine ";
        $query .= "FROM #__xprofiles AS x ";
        $query .= "JOIN `#__jobs_seekers` AS s ON s.uid=x.uidNumber ";
        $query .= "JOIN `#__jobs_resumes` AS r ON r.uid=s.uid  ";
