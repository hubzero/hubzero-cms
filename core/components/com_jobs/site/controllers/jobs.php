...
class Jobs extends SiteController
{
    ...
    public function resumesTask()
    {
        ...
        if ($this->_admin or $this->_emp) {
            ...
            $this->view->setName('resumes')
                        ->setLayout('default')
                        ->display();
        } elseif ($this->_allowSubscriptions) {
    ...
    public function confirmTask()
    {
        ...
        if ($units) {
            ...
            if ($subid) {
                ...
                if ($serviceid != $subscription->serviceid) {
                    $prevunitcost = Service::oneOrNew($subscription->serviceid)->get('unitprice', 0);
                } else {
                    $prevunitcost = $newunitcost;
                }
                $unitsleft    = 0;
                $refund       = 0;
                ...
                if ($serviceid != $subscription->serviceid or $subscription->status == 2) {
                    if ($prevunitcost > 0 && $subscription->status != 2) {
                        $unitsleft = $subscription->getRemaining(
                            'unit',
                            $service->maxunits,
                            $service->unitsize
                        );
                        $needsRefund = $subscription->totalpaid > 0
                            && ($subscription->totalpaid - $unitsleft * $prevunitcost) < 0;
                    ...
                    $subscription = Subscription::blank();
                    $new = 1;
                } elseif ($subscription->expires > $now) {
                    // extending?
                    ...
                    if ($autoapprove && !$total) {
                        $subscription->units = $subscription->units + $units;
                        $subscription->pendingunits = 0;
                        $subscription->pendingpayment = 0;
                    } else {
                        $subscription->pendingunits = $units;
                        $subscription->pendingpayment = $units * $newunitcost;
                    }
                    $monthsToAdd = '+' . $subscription->units * $service->unitsize . ' month';
                    $newexprire = Date::of(strtotime($monthsToAdd))->toSql();
        ...
        if ($new) {
            ...
            if ($credit) {
                $subscription->pendingpayment = $subscription->pendingpayment < $credit;
            }
        ...
        if ($subid) {
            $this->_msg = Lang::txt('COM_JOBS_MSG_SUBSCRIPTION_PROCESSED');
        } else {
            $this->_msg = Lang::txt('COM_JOBS_MSG_SUBSCRIPTION_ACCEPTED');
        }
        if ($units) {
            if ($autoapprove && !$total) {
                $this->_msg .= ' ' . Lang::txt('COM_JOBS_MSG_SUBSCRIPTION_YOU_HAVE_ACCESS');
                $this->_msg .= ' ' . $subscription->units . ' ' . Lang::txt('COM_JOBS_MONTHS');
            } else {
                $this->_msg .= ' ' . Lang::txt('COM_JOBS_MSG_SUBSCRIPTION_WE_WILL_CONTACT');
            }
        }
    ...
    public function savejobTask()
    {
        ...
        } elseif ($job->status == 4 && $this->_task == 'confirmjob') {
    ...
    public function editjobTask()
    {
        ...
        if (!$employer->loadEmployer($uid) && !$this->_admin) {
            App::abort(404, Lang::txt('COM_JOBS_ERROR_EMPLOYER_NOT_FOUND'));
        } elseif (!$employer->id && $this->_admin) {
            $employer->uid = 1;
    ...
    protected function _getFilters($admin = 0, $emp = 0, $checkstored = 1, $jobs = 0)
    {
        ...
        $storedType = $this->getProperty('type') && $checkstored;
        $filters['type'] = $storedType ? $this->getProperty('type') : Request::getInt('type', 0);
        $storedSearch = $this->getProperty('search') && $checkstored;
        $filters['search'] = $storedSearch
            ? $this->getProperty('search')
            : trim(Request::getString('q', ''));
        $filters['filterby'] = trim(Request::getString('filterby', 'all'));
        $filters['sortdir']  = Request::getString('sortdir', 'ASC');

        // did we get stored prefs?
        $storedMatch = $this->getProperty('match') && $checkstored;
        $filters['match'] = $storedMatch
            ? $this->getProperty("match")
            : Request::getInt('match', 0);
    ...
    public function batchTask()
    {
        ...
        if ($archive) {
            ...
            App::redirect(
                Route::url('index.php?option=com_jobs&task=dashboard'),
                Lang::txt('COM_JOBS_ERROR_ARCHIVE_FAILED'),
                'error'
            );
        }
    }
