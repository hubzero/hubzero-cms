...
class IdP
{
    ...
    {
        return $this->trustedSigningcert[$entityId];
    }
    ...
    {
        return $this->trustedAcs[$entityId];
    }
    ...
    {
        return $this->trustedSlo[$entityId];
    }
    ...
    public function createSAMLResponse($user, $issuer, $acsUrl, $id, $session_id)
    {
        ...
        $signature = new \LightSaml\Model\XmlDSig\SignatureWriter(
            $this->getCertificate(),
            $this->getPrivateKey()
        );

        if (!$destination) {
            $statusCode = new \LightSaml\Model\Protocol\StatusCode(
                \LightSaml\SamlConstants::STATUS_RESPONDER
            );
            $response = (new \LightSaml\Model\Protocol\Response())
                ->setStatus(new \LightSaml\Model\Protocol\Status($statusCode))
     ...
     * @param $status
     */
    public function createSAMLLogoutResponse($sloUrl, $requestId, $status)
    {
        $statusCode = new \LightSaml\Model\Protocol\StatusCode($status);
        $signature = new \LightSaml\Model\XmlDSig\SignatureWriter(
            $this->getCertificate(),
            $this->getPrivateKey()
        );
