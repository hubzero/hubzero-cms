...
class Jobs extends Base implements CommandInterface
{
        ...
        if ($jobs->count()) {
            ...
            foreach ($jobs as $job) {
                ...
                try {
                    ...
                    $msg = Lang::txt('[%s] Event "%s" generated an error. Skipping...', $now, $job->get('event'));
                    $this->output->addLine($msg, 'error');
                }
            }
    ...
    public function deactivate()
    {
        ...
        if (!$id) {
            $msg = Lang::txt('A job ID must be provided. Example: `muse cron:job deactivate --job=1`');
            $this->output->addLine($msg, 'warning');
            return;
        }
    ...
    public function unpublish()
    {
        ...
        if (!$id) {
            $msg = Lang::txt('A job ID must be provided. Example: `muse cron:job unpublish --job=1`');
            $this->output->addLine($msg, 'warning');
            return;
        }
