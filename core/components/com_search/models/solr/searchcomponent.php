...
class SearchComponent extends Relational
{
     ...
     * State constants
     */
    public const STATE_NOTINDEXED = 0;
     ...
     */
    public function formatWithCounts(
        $counts,
        $activeType = null,
        $terms = null,
        $childTerms = null,
        ...
        if ($count > 0) {
            ...
            if ($activeType && $this->filters->count() > 0) {
                foreach ($this->filters()->order('ordering', 'ASC') as $filter) {
                    $filterField = $filter->get('field');
                    $filterSelectedOptions = isset($selectedOptions[$filterField])
                        ? $selectedOptions[$filterField] : array();
