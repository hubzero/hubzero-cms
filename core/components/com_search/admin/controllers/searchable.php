...
class Searchable extends AdminController
{
    ...
    public function displayTask()
    {
        ...
        $components = SearchComponent::all()
            ->where('state', '!=', SearchComponent::STATE_TRASHED)
            ->orWhere('state', 'IS', null)
            ->rows();
    ...
    public function activateIndexTask()
    {
        ...
        foreach ($components as $component) {
            ...
            if (!$recordsIndexed) {
                ...
                $linkUrl = 'index.php?option=com_search&controller=' . $this->_controller
                    . '&task=documentListing&facet=hubtype:' . $component->getSearchNamespace();
                $componentLink = Route::url($linkUrl);
                $recordsIndexed['state'] = SearchComponent::STATE_INDEXED; //1;
                $linkHtml = '<a href="' . $componentLink . '">' . $component->getSearchCount() . '</a>';
                $recordsIndexed['total'] = $linkHtml;
                $deleteUrl = 'index.php?option=' . $this->_option
                    . '&controller=searchable&task=deleteIndex&id=' . $component->get('id');
                $recordsIndexed['link'] = Route::url($deleteUrl, false);
    ...
    public function editTask($obj = null)
    {
        ...
        foreach ($searchComponent->filters()->order('ordering', 'ASC') as $filter) {
            ...
            if ($filter->options->count() > 0) {
                $options = $filter->options()->order('ordering', 'ASC')->rows();
                $filters[$filter->field]['options'] = $options->fieldsByKey('value');
            }
        }
    ...
    public function discoverTask()
    {
        ...
        if ($components->count() > 0) {
            if ($components->save()) {
                \Hubzero\Facades\Notify::success('New Searchable Components found');
            }
        } else {
            \Hubzero\Facades\Notify::warning('No new components found.');
        }
