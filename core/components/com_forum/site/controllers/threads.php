...
class Threads extends SiteController
{
    ...
    protected function buildPathway($section = null, $category = null, $thread = null)
    {
        ...
        if (isset($thread) && $thread->get('id')) {
            $threadTitle = '#' . $thread->get('id') . ' - '
                . Str::truncate(stripslashes($thread->get('title')), 100, array('exact' => true));
            $threadUrl = 'index.php?option=' . $this->_option . '&section=' . $section->get('alias')
                . '&category=' . $category->get('alias') . '&thread=' . $thread->get('id');
            Pathway::append($threadTitle, $threadUrl);
        }
    }
    ...
    public function editTask($post = null)
    {
        ...
        if (User::isGuest()) {
            $return = Route::url(
                'index.php?option=' . $this->_option . '&section=' . $section
                    . '&category=' . $category . '&task=new'
            );
            if ($id) {
                $return = Route::url(
                    'index.php?option=' . $this->_option . '&section=' . $section
                        . '&category=' . $category . '&thread=' . $id . '&task=edit'
                );
            }
            App::redirect(
    ...
    public function saveTask()
    {
        ...
        if (
            !$this->config->get('access-edit-' . $assetType)
        ...
        if (!$fields['id']) {
            ...
            $message = ($post->get('modified_by'))
                ? Lang::txt('COM_FORUM_POST_EDITED')
                : Lang::txt('COM_FORUM_POST_ADDED');
        }

        $url = 'index.php?option=' . $this->_option . '&section=' . $section
            . '&category=' . $category->get('alias') . '&thread=' . $post->get('thread')
            . '#c' . $post->get('id');
        ...
        if ($mentionEmailList) {
            ...
            $urlExt = 'forum/' . $section . '/' . $category->get('alias') . '/'
                . $post->get('thread') . '#c' . $post->get('id');
    ...
    public function downloadTask()
    {
        ...
        if (User::isGuest() && !in_array($post->get('access'), User::getAuthorisedViewLevels())) {
            $return = base64_encode(Route::url(
                'index.php?option=' . $this->_option . '&controller=' . $this->_controller
                    . '&section=' . $section . '&category=' . $category . '&thread=' . $thread_id
                    . '&post=' . $post_id . '&file=' . $file
            ));
            App::redirect(
                Route::url('index.php?option=com_users&view=login&return=' . $return)
            );
