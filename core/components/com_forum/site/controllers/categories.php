...
class Categories extends SiteController
{
    ...
    public function searchTask()
    {
        ...
        $filters = array(
            'scope'      => $this->forum->get('scope'),
            'scope_id'   => $this->forum->get('scope_id'),
            'orderBy'    => Request::getWord('orderBy', 'created'),
            'orderDir'   => Request::getWord('orderDir', 'DESC'),
            'state'      => Category::STATE_PUBLISHED,
            'access'     => User::getAuthorisedViewLevels()
        );
    ...
    public function saveTask()
    {
        ...
        // Log activity
        $activityKey = 'COM_FORUM_ACTIVITY_CATEGORY_' . ($fields['id'] ? 'UPDATED' : 'CREATED');
        $activityLink = '<a href="' . Route::url($url) . '">' . $category->get('title') . '</a>';
    ...
    public function deleteTask()
    {
        ...
        if (User::isGuest()) {
            $returnUrl = base64_encode(Route::url($url, false, true));
            App::redirect(
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope'       => 'forum.category',
                'scope_id'    => $category->get('id'),
                'description' => Lang::txt('COM_FORUM_ACTIVITY_CATEGORY_DELETED', $activityLink),
                'details'     => array(
                    'title' => $category->get('title'),
                    'url'   => Route::url($url)
                )
