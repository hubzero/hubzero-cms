...
class Sections extends SiteController
{
    ...
    public function saveTask()
    {
        ...
        if (User::isGuest()) {
            $returnUrl = base64_encode(Route::url('index.php?option=' . $this->_option, false, true));
            App::redirect(
                Route::url('index.php?option=com_users&view=login&return=' . $returnUrl),
        ...
        // Log activity
        $activityKey = 'COM_FORUM_ACTIVITY_SECTION_' . ($fields['id'] ? 'UPDATED' : 'CREATED');
        $activityUrl = Route::url('index.php?option=' . $this->_option);
        $activityLink = '<a href="' . $activityUrl . '">' . $section->get('title') . '</a>';
    ...
    public function deleteTask()
    {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope'       => 'forum.section',
                'scope_id'    => $section->get('id'),
                'description' => Lang::txt('PLG_GROUPS_FORUM_ACTIVITY_SECTION_DELETED', $activityLink),
                'details'     => array(
                    'title' => $section->get('title'),
                    'url'   => Route::url('index.php?option=' . $this->_option)
                )
