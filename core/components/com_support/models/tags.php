...
class Tags extends Cloud
{
    ...
    public function render($rtrn = 'html', $filters = array(), $clear = false)
    {
        if (strtolower($rtrn) == 'linkedlist') {
            ...
            foreach ($this->tags('list', $filters, $clear) as $tag) {
                $class = ($tag->get('admin') ? ' class="admin"' : '');
                $url = Route::url('index.php?option=com_support&task=tickets&find=tag:'
                    . $tag->get('tag'));
                $bits[] = '<a' . $class . ' href="' . $url . '">'
                    . stripslashes($tag->get('raw_tag')) . '</a>';
            }
            return implode(', ', $bits);
        }
    ...
    public function checkTags($id, $tagger_id = 0, $strength = 0, $admin = 0)
    {
        ...
        $sql = "SELECT t.tag, t.raw_tag, t.description, t.admin, rt.id, rt.objectid
            FROM `#__tags_object` AS rt
            JOIN `#__tags` AS t ON t.id=rt.tagid WHERE ";
