...
class Ticket extends Relational
{
    ...
    public function access($action = 'view', $item = 'tickets')
    {
        if (!$this->get('_access-check-done', false)) {
            ...
            if (
                $this->get('login') == User::get('username')
    ...
    public static function countWithQuery($query, $filters = array())
    {
        ...
        $having = '';
        if (
            preg_match('/GROUP BY f.id HAVING uniques=\'\d\'/i', $query, $matches)
    ...
    public static function allWithQuery($query, $filters = array())
    {
        ...
        $sql = "SELECT DISTINCT f.`id`, f.`summary`, f.`report`, f.`category`, f.`open`,
            f.`status`, f.`severity`, f.`resolved`, f.`group_id`, f.`owner`,
            f.`created`, f.`login`, f.`name`, f.`email`, f.`target_date` ";
        if ($having) {
    ...
    public static function parseFind($filters)
    {
        ...
        if (isset($filters['tag']) && $filters['tag'] != '') {
            $filter .= " AND st.objectid=f.id AND st.tbl='support' AND st.tagid=t.id"
                . " AND t.tag=" . $db->quote($filters['tag']);
        }
