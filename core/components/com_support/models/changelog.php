...
class Changelog extends Obj
{
    ...
    public function __construct($data = null)
    {
        if ($data) {
            ...
            if (substr($data, 0, 1) == '{') {
                $this->log = json_decode($data, true);
            } else {
                $this->format = 'html';
        ...
        if (!isset($this->log['changes'])) {
            $this->log['changes'] = array();
        }
        if (!isset($this->log['notifications'])) {
            $this->log['notifications'] = array();
        }
        if (!isset($this->log['cc'])) {
            $this->log['cc'] = array();
        }
    }
    ...
    public function cced($val)
    {
        ...
        foreach ($val as $acc) {
            ...
            if (!strstr($acc, '@')) {
                ...
                if (is_object($user)) {
                    $this->log['cc'][] = $user->get('username');
                } else {
                    // Move on - nothing else we can do here
                    continue;
                }
            } elseif (Validate::email($acc)) {
    ...
    {
        if (!isset($this->log[$from])) {
            $message = Lang::txt('COM_SUPPORT_ERROR_CHANGELOG_UNKNOWN_CATEGORY', (string) $from);
            throw new InvalidArgumentException($message);
        }

        foreach ($this->log[$from] as $key => $item) {
            if ($item->field == $field) {
                unset($this->log[$from][$key]);
            }
        }
    ...
    public function diff($before, $after)
    {
        ...
        if (
            intval($after->get('status')) != intval($before->get('status'))
