...
class Query extends Relational
{
    ...
    public function toSql($condition = null)
    {
        ...
        for ($i = 0; $i < count($condition->expressions); $i++) {
            ...
            if (strstr($expr->opval, '$1')) {
                if (strtolower($expr->fldval) == 'tag') {
                    // Tag field is handled separately
                } else {
                    $e[] = $prfx . '.' . $db->quoteName($expr->fldval) . ' '
                        . str_replace('$1', $expr->val, $expr->opval);
                }
            } else {
                $uid = 'username';
                if (strtolower($expr->fldval) == 'owner') {
                ...
                } elseif (strtolower($expr->fldval) == 'owner') {
                ...
                if (strtolower($expr->fldval) == 'status' && $expr->val == '-1') {
                    ...
                    $e[] = '(' . $prfx . '.' . $db->quoteName($expr->fldval) . ' '
                        . $expr->opval . ' ' . $db->quote($expr->val) . ' AND '
                        . $prfx . '.' . $db->quoteName('open') . ' = ' . $db->quote('0') . ')';
                } else {
                    $e[] = $prfx . '.' . $db->quoteName($expr->fldval) . ' '
                        . $expr->opval . ' ' . $db->quote($expr->val);
                }
            }
        }
        ...
        if (count($tags) > 0) {
            ...
            $notTagList = "'" . implode("','", $nottags) . "'";

            if (implode("','", $tags) == implode("','", $nottags)) {
                $tagIn = str_replace('$1', $tagList, 'IN ($1)');
                $e[] = 'f.' . $db->quoteName('id') . ' NOT IN (
							SELECT st.' . $db->quoteName('objectid') . '
							FROM #__tags_object AS st
							LEFT JOIN #__tags AS t
							ON st.' . $db->quoteName('tagid') . '=t.' . $db->quoteName('id') . '
							WHERE st.' . $db->quoteName('tbl') . '=\'support\'
							AND (t.' . $db->quoteName('tag') . ' ' . $tagIn . '
							OR t.' . $db->quoteName('raw_tag') . ' ' . $tagIn . '))';
                $having = " GROUP BY f.id ";
            } elseif (count($tags) && count($nottags)) {
                $tagIn = str_replace('$1', $tagList, 'IN ($1)');
                $notTagIn = str_replace('$1', $notTagList, 'IN ($1)');
                $e[] = '(t.' . $db->quoteName('tag') . ' ' . $tagIn . ' OR t.'
                    . $db->quoteName('raw_tag') . ' ' . $tagIn . ')';
                $e[] = 'f.' . $db->quoteName('id') . ' NOT IN (
							SELECT jto.' . $db->quoteName('objectid') . '
							FROM #__tags_object AS jto
							JOIN #__tags AS jt
							ON jto.' . $db->quoteName('tagid') . '=jt.' . $db->quoteName('id') . '
							WHERE jto.' . $db->quoteName('tbl') . '=\'support\'
							AND (jt.' . $db->quoteName('tag') . ' ' . $notTagIn . '
							OR jt.' . $db->quoteName('raw_tag') . ' ' . $notTagIn . '))';
                $having = " GROUP BY f.id ";
            } else {
                $tagIn = str_replace('$1', $tagList, 'IN ($1)');
                $e[] = '(t.' . $db->quoteName('tag') . ' ' . $tagIn . ' OR t.'
    ...
    public static function populateDefaults($type = 'common', $folder_id = 0)
    {
        ...
        switch (strtolower(trim($type))) {
            case 'common':
                $method = 'allCommon';
                $openExpr = '{"fldval":"open","flddisp":"Open/Closed","opval":"=","opdisp":"is","val":"1"}';
                $closedExpr = '{"fldval":"open","flddisp":"Open/Closed","opval":"=","opdisp":"is","val":"0"}';
                $typeExpr = '{"fldval":"type","flddisp":"Type","opval":"=","opdisp":"is","val":"0"}';
                $statusExpr = '{"fldval":"status","flddisp":"Status","opval":"=","opdisp":"is","val":"0"}';
                $ownerEmptyExpr = '{"fldval":"owner","flddisp":"Owner","opval":"=","opdisp":"is","val":""}';
                $ownerNullExpr = '{"fldval":"owner","flddisp":"Owner","opval":"=","opdisp":"is","val":"NULL"}';
                $data = array(
                    array(
                        'title'      => 'Open tickets',
                        'conditions' => '{"operator":"AND","expressions":['
                            . $openExpr . ',' . $typeExpr . '],"nestedexpressions":[]}',
                        'query'      => '',
                        'user_id'    => 0,
                        'sort'       => 'created',
                        'sort_dir'   => 'desc',
                    ...
                    array(
                        ...
                        'conditions' => '{"operator":"AND","expressions":['
                            . $openExpr . ',' . $typeExpr
                            . '],"nestedexpressions":[{"operator":"OR","expressions":['
                            . $ownerEmptyExpr . ',' . $ownerNullExpr
                            . '],"nestedexpressions":[]}]}',
                        'query'      => '',
                        'user_id'    => 0,
                        'sort'       => 'created',
                    ...
                    array(
                        ...
                        'conditions' => '{"operator":"AND","expressions":['
                            . $closedExpr . ',' . $typeExpr . '],"nestedexpressions":[]}',
                        'query'      => '',
                        'user_id'    => 0,
                        'sort'       => 'created',
                        'sort_dir'   => 'desc',
            ...
            case 'commonnotacl':
                ...
                $typeExpr = '{"fldval":"type","flddisp":"Type","opval":"=","opdisp":"is","val":"0"}';
                $statusExpr = '{"fldval":"status","flddisp":"Status","opval":"=","opdisp":"is","val":"0"}';
                $groupExpr = '{"fldval":"group","flddisp":"Group","opval":"=","opdisp":"is","val":"*"}';
                $ownerMeExpr = '{"fldval":"owner","flddisp":"Owner","opval":"=","opdisp":"is","val":"' . $me . '"}';
                $loginMeExpr = '{"fldval":"login","flddisp":"Submitter","opval":"=","opdisp":"is","val":"' . $me . '"}';
                $ownerEmptyExpr = '{"fldval":"owner","flddisp":"Owner","opval":"=","opdisp":"is","val":""}';
                $ownerNullExpr = '{"fldval":"owner","flddisp":"Owner","opval":"=","opdisp":"is","val":"NULL"}';
                $nestedAcl = '{"operator":"OR","expressions":['
                    . $groupExpr . ',' . $ownerMeExpr . ',' . $loginMeExpr
                    . '],"nestedexpressions":[]}';
                $data = array(
                    array(
                        'title'      => 'Open tickets',
                ...
                $data = array(
                    ...
                    array(
                        ...
                        'conditions' => '{"operator":"AND","expressions":['
                            . $closedExpr . ',' . $typeExpr
                            . '],"nestedexpressions":[{"operator":"OR","expressions":['
                            . $ownerMeExpr . ',' . $loginMeExpr . '],"nestedexpressions":[]}]}',
                        'query'      => '',
                        'user_id'    => 0,
                        'sort'       => 'created',
                        'sort_dir'   => 'desc',
