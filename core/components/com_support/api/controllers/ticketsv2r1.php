 ...
 */
class Ticketsv2r1 extends ApiController
{
    /**
     * Execute a request
     *
     ...
     * @apiParameter {
     *      "name":          "created",
     *      "description":   "A timestamp (YYYY-MM-DD HH:mm:ss) for items created on or after
     *                       the specified date. A time window can be specified adding
     *                       a second timestamp, separated by a comma.
     ...
     * @apiParameter {
     *      "name":          "closed",
     *      "description":   "A timestamp (YYYY-MM-DD HH:mm:ss) for items closed on or after
     *                       the specified date. A time window can be specified adding
     *                       a second timestamp, separated by a comma.
     *                       Example: 2018-01-01,2018-12-31",
     *      "type":          "string|integer",
    ...
    public function listTask()
    {
        ...
        $allowedSorts = array(
            'id', 'type', 'created', 'closed', 'status', 'open',
            'owner', 'severity', 'target_date', 'name', 'login', 'email'
        );
    ...
    private function toTimestamp($val = null)
    {
        if ($val) {
            ...
            } elseif (preg_match("/([0-9]{4})-([0-9]{2})-([0-9]{2})/", $val, $regs)) {
                $val .= ' 00:00:00';
            } elseif (preg_match("/([0-9]{4})-([0-9]{2})/", $val, $regs)) {
                $val .= '-01 00:00:00';
            } else {
    ...
    public function readTask()
    {
        ...
        $props = array(
            'status', 'created', 'severity', 'os', 'browser',
            'ip', 'hostname', 'uas', 'referrer', 'open', 'closed'
        );
        ...
        $ticketRoute = Route::url(
            'index.php?option=com_support&controller=tickets&task=tickets&id=' . $response->id
        );
        $response->url = str_replace('/api', '', rtrim(Request::base(), '/')
            . '/' . ltrim($ticketRoute, '/'));
