...
class Commentsv2r1 extends ApiController
{
     ...
     *                        the specified date. A time window can be specified adding a
     *                        second timestamp, separated by a comma. Example: 2018-01-01,2018-12-31",
     *      "type":          "string|integer",
     *      "required":      false,
     *      "default":       null,
    ...
    public function createTask()
    {
        ...
        if (Request::get('email_owner'))
        {
            ...
            $notifData = array(
                'role' => 'Ticket owner',
                'address' => $ticket->get_owner()->get('email'),
                'name' => $ticket->get_owner()->get('name')
            );
            $changelog->notifications[] = json_encode($notifData);
        }
