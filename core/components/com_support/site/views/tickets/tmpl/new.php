...
<section class="main section">
    ...
    $formAction = Route::url(
        ...
        . '&controller=' . $this->controller . '&task=new' . $tmpl
    );
    ?>
    <form
    ...
    >
        ...
        <input
            type="hidden"
            name="verified"
            value="<?php echo $this->escape($this->row->get('verified')); ?>"
        />

        <input
            type="hidden"
            name="problem[referer]"
            value="<?php echo $this->escape($this->row->get('referrer')); ?>"
        />
        <input
            type="hidden"
            name="problem[tool]"
            value="<?php echo $this->escape($this->row->get('tool')); ?>"
        />
        <input
            type="hidden"
            name="problem[short]"
            value="<?php echo $this->escape($this->row->get('short')); ?>"
        />
        ...
        <fieldset>
            ...
            <div class="form-group">
                <label for="reporter_login">
                    ...
                    <?php
                    $loginVal = $this->escape(
                        $this->row->get('login', $this->row->submitter->get('username'))
                    );
                    ?>
                    <input
                        type="text"
            ...
            <div class="form-group">
                <?php
                $nameErrClass = ($this->getError() && !$this->row->get('name'))
                    ? ' class="fieldWithErrors"' : '';
                $nameVal = $this->escape(
                    $this->row->get('name', $this->row->submitter->get('name'))
                );
                ?>
                <label for="reporter_name"<?php echo $nameErrClass; ?>>
                    <?php echo Lang::txt('COM_SUPPORT_NAME'); ?>
                    <span class="required"><?php echo Lang::txt('COM_SUPPORT_REQUIRED'); ?></span>
                    <input
                        type="text"
                        name="reporter[name]"
                        class="form-control"
            ...
            <div class="form-group">
                <?php
                $emailErrClass = ($this->getError() && !$this->row->get('email'))
                    ? ' class="fieldWithErrors"' : '';
                $emailVal = $this->escape(
                    $this->row->get('email', $this->row->submitter->get('email'))
                );
                ?>
                <label for="reporter_email"<?php echo $emailErrClass; ?>>
                    <?php echo Lang::txt('COM_SUPPORT_EMAIL'); ?>
                    <span class="required"><?php echo Lang::txt('COM_SUPPORT_REQUIRED'); ?></span>
                    <input
                        type="email"
                        name="reporter[email]"
            ...
            <?php
            $orgVal = $this->escape(
                $this->row->get('organization', $this->row->submitter->get('organization'))
            );
            ?>
            <input type="hidden" name="reporter[org]" value="<?php echo $orgVal; ?>" id="reporter_org" />
        </fieldset><div class="clear"></div>
    <?php else : ?>
        <?php
        $loginVal = $this->escape($this->row->get('login', $this->row->submitter->get('username')));
        $nameVal  = $this->escape($this->row->get('name', $this->row->submitter->get('name')));
        $emailVal = $this->escape($this->row->get('email', $this->row->submitter->get('email')));
        ?>
        <input type="hidden" name="reporter[login]" value="<?php echo $loginVal; ?>" id="reporter_login" />
        <input type="hidden" name="reporter[name]" value="<?php echo $nameVal; ?>" id="reporter_name" />
        <input type="hidden" name="reporter[email]" value="<?php echo $emailVal; ?>" id="reporter_email" />
    <?php endif; ?>
        ...
        <fieldset>
            ...
            <div class="form-group">
                <?php
                $reportErrClass = ($this->getError() && !$this->row->get('report'))
                    ? ' class="fieldWithErrors"' : '';
                ?>
                <label for="problem_long"<?php echo $reportErrClass; ?>>
                    <?php echo Lang::txt('COM_SUPPORT_TROUBLE_DESCRIPTION'); ?>
                    ...
                    <textarea
                        ...
                        class="form-control"
                        id="problem_long"
                    ><?php echo $this->row->get('report'); ?></textarea>
                </label>
            ...
            <fieldset>
                ...
                $uploadInstructions = Lang::txt('COM_SUPPORT_CLICK_OR_DROP_FILE');
            ...
            <fieldset>
                            ...
                            <label for="ticket-field-severity">
                                ...
                                >
                                    <?php foreach ($this->lists['severities'] as $severity) : ?>
                                        <?php
                                        $sevSel   = ($severity == 'normal') ? ' selected="selected"' : '';
                                        $sevLabel = Lang::txt(
                                ...
                                >
                                    ...
                                    ?>">
                                        <option value="0" selected="selected">
                                            <?php echo Lang::txt('COM_SUPPORT_COMMENT_OPT_NEW'); ?>
                                        </option>
                                        <?php
                                        foreach (
                                    ...
                                    <optgroup label="<?php echo Lang::txt('COM_SUPPORT_CLOSED'); ?>">
                                        <option value="0">
                                            <?php echo Lang::txt('COM_SUPPORT_COMMENT_OPT_CLOSED'); ?>
                                        </option>
                                        <?php
                                        foreach (
                        ...
                        <label for="ticket-field-category">
                            ...
                            >
                                ...
                                foreach ($this->lists['categories'] as $category) :
                                    ...
                                    ?>
                                    <option value="<?php echo $catAlias; ?>">
                                        <?php echo $catTitle; ?>
                                    </option>
                                    <?php
                                endforeach;
                ...
                <div class="form-group">
                    <label for="acmembers">
                        ...
                        $mc = Event::trigger(
                            'hubzero.onGetMultiEntry',
                            array(array('members', 'cc', 'acmembers', '', ''))
                        );
                        if (count($mc) > 0) :
                            ...
                            <span class="hint">
                                <?php echo Lang::txt('COM_SUPPORT_COMMENT_SEND_EMAIL_CC_INSTRUCTIONS'); ?>
                            </span>
                            <input
                                type="text"
                                name="cc"
        ...
        <?php if (!$this->row->get('verified')) : ?>
            ...
            <fieldset>
                ...
                <label id="fbBotcheck-label" for="fbBotcheck">
                    <?php echo Lang::txt('COM_SUPPORT_LEAVE_FIELD_BLANK'); ?>
                    <span class="required"><?php echo Lang::txt('JREQUIRED'); ?></span>
        ...
        <p class="submit">
            <input
                ...
                type="submit"
                name="submit"
                value="<?php echo Lang::txt('COM_SUPPORT_SUBMIT'); ?>"
            />
        </p>
    </form>
