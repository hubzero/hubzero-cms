...
class Tickets extends SiteController
{
    ...
    public function statsTask()
    {
        // Check authorization
        if (User::isGuest()) {
            $requestUri = 'index.php?option=' . $this->_option
                . '&controller=' . $this->_controller . '&task=' . $this->_task;
            $return = base64_encode(
                Request::getString('REQUEST_URI', Route::url($requestUri, false, true), 'server')
            );
            App::redirect(
        ...
        if (!$this->acl->check('read', 'tickets')) {
            $this->_return = Route::url(
                'index.php?option=' . $this->_option
                . '&controller=' . $this->_controller . '&task=tickets'
            );
        }
        ...
        if ($this->view->start && $end) {
            $sql .= " AND created >= " . $this->database->quote($this->view->start . '-01 00:00:00')
                . " AND created < " . $this->database->quote($end . '-01 00:00:00');
        }
        ...
        // Closed tickets
        $sql = "SELECT t.id AS ticket, t.owner AS created_by, t.closed AS created,
				YEAR(t.closed) AS `year`, MONTH(t.closed) AS `month`,
				UNIX_TIMESTAMP(t.created) AS opened, UNIX_TIMESTAMP(t.closed) AS closed
        ...
        if ($this->view->start && $end) {
            $sql .= " AND t.closed >= " . $this->database->quote($this->view->start . '-01 00:00:00')
                . " AND t.closed < " . $this->database->quote($end . '-01 00:00:00');
        }
        ...
        for ($k = $startyear, $n = $endyear; $k < $n; $k++) {
            ...
            for ($i; $i <= 12; $i++) {
                if ($k == $year && $i > $endmonth) {
                    ...
                    $closedVal = (isset($closed[$k]) && isset($closed[$k][$i])) ? $closed[$k][$i] : 0;
                    $this->view->closedmonths[$k][$i] = $closedVal;
                    $openedVal = (isset($open[$k]) && isset($open[$k][$i])) ? $open[$k][$i] : 0;
                    $this->view->openedmonths[$k][$i] = $openedVal;
                }
    ...
    public function newTask($row = null)
    {
        ...
        if ($row->get('verified') && $this->acl->check('update', 'tickets') > 0) {
            if (trim($this->config->get('group', ''))) {
                $lists['owner'] = $this->userSelectGroup(
                    'problem[owner]',
                    '',
                    1,
                    '',
    ...
    public function saveTask()
    {
        ...
        if (!$verified) {
            // Quick spam filter
            $spam = $this->detectSpam($problem['long'], $ip);
            if ($spam) {
        ...
        $timeSinceLastSubmit = time() - strtotime($prevSubmission->get('created'));
        $isSameReport = $prevSubmission->get('report') == $row->get('report');
        if ($isSameReport && ($timeSinceLastSubmit <= 15)) {
        ...
        if ($defs) {
            ...
            $emailSubject = Config::get('sitename') . ' '
                . Lang::txt('COM_SUPPORT_EMAIL_SUBJECT_NEW_TICKET', $row->get('id'));
            ...
            Event::trigger('system.logActivity', [
                'activity' => [
                    ...
                    'description' => Lang::txt(
                        'COM_SUPPORT_ACTIVITY_TICKET_CREATED',
                        '<a href="' . Route::url($row->link()) . '">#'
                            . $row->get('id') . ' - ' . $row->get('summary') . '</a>'
                    ),
                    'details'     => array(
                        'id'      => $row->get('id'),
        ...
        if (!User::isGuest() && $this->acl->check('update', 'tickets') > 0) {
            ...
            if (count($rowc->changelog()->get('changes')) > 0 || count($rowc->changelog()->get('cc')) > 0) {
                ...
                if (
                    count($rowc->changelog()->get('notifications')) > 0
                ...
                $ticketLink = '<a href="' . Route::url($row->link()) . '">#'
                    . $row->get('id') . ' - ' . $row->get('summary') . '</a>';
                $desc = Lang::txt('COM_SUPPORT_ACTIVITY_TICKET_UPDATED', $ticketLink);
                if ($rowc->get('comment')) {
                    $desc = Lang::txt(
                        'COM_SUPPORT_ACTIVITY_COMMENT_CREATED',
                        $rowc->get('id'),
                        $ticketLink
                    );
                }
     ...
     * @return  boolean
     */
    private function detectSpam($text, $ip)
    {
        // Spammer IPs (banned)
    ...
    public function ticketTask($comment = null)
    {
        ...
        if (\Hubzero\Facades\Notify::any('support')) {
            foreach (\Hubzero\Facades\Notify::messages('support') as $error) {
                if ($error['type'] == 'error') {
    ...
    public function updateTask()
    {
        ...
        if (User::isGuest()) {
            $requestUri = 'index.php?option=' . $this->_option
                . '&controller=' . $this->_controller . '&task=' . $this->_task;
            $return = base64_encode(Route::url($requestUri, false, true));
            App::redirect(
                Route::url('index.php?option=com_users&view=login&return=' . $return, false)
            );
            return;
        ...
        if ($lastcomment && $lastcomment->get('created') > $started) {
            ...
            $errorMsg = Lang::txt(
                'Changes were made to this ticket in the time since you began '
                . 'commenting/making changes. Please review your changes before submitting.'
            ) . $lastcomment->get('created') . ' > ' . $started;
        ...
        // Incoming comment
        if ($comment) {
            // If a comment was posted always change status to open.
            // This is the typical expected behavior. Preventing new comments
            // from re-opening a ticket can result in comments that are never responded to.
            // If a comment was posted by the ticket submitter to a
            // "waiting user response" ticket, change status.
            $notOpenButSameState = (!$row->isOpen())
                && $row->get('open') == $old->get('open');
            $isWaitingAndSameUser = $row->isWaiting()
                && User::get('username') == $row->get('login');
            if ($notOpenButSameState || $isWaitingAndSameUser) {
                $row->open();
            }
        }
        ...
        if (
            ...
            if (Request::getInt('email_submitter', 0) == 1) {
                ...
                if (!$rowc->isPrivate()) {
                    # Users can submit a ticket while not logged in and without
                # a username, that has to be handled differently
        ...
        $redirectUrl = 'index.php?option=' . $this->_option
            . '&controller=' . $this->_controller . '&task=ticket&id=' . $id;
        App::redirect(
    ...
    public function deleteTask()
    {
        ...
        $ticketLink = '<a href="' . Route::url($ticket->link()) . '">#'
            . $ticket->get('id') . ' - ' . $ticket->get('summary') . '</a>';
        $description = Lang::txt('COM_SUPPORT_ACTIVITY_TICKET_DELETED', $ticketLink);
     ...
     *   uas      (optional)
     *   referrer (optional)
     *   cookies  (optional) default: 1
     *            (since it's coming from rappture we assume they're already
     *            logged in and thus have cookies enabled)
     *   section  (optional)
     *   upload   (optional)
     *
     * @return  void
    ...
    public function createTask()
    {
        ...
        if (strstr($summary, '"') || strstr($summary, "'")) {
            ...
            $summary = str_replace('\"', '\\\\\\\\\"', $summary);
            $query = "SELECT id FROM `#__support_tickets` WHERE LOWER(summary) LIKE "
                . $this->database->quote('%' . strtolower($summary) . '%')
                . " AND type=1 LIMIT 1";
        }
        $query = "SELECT id FROM `#__support_tickets` WHERE LOWER(summary) LIKE "
            . $this->database->quote('%' . strtolower($summary) . '%')
        ...
        if ($ticket = $this->database->loadResult()) {
            ...
            $defaultCategory = Lang::txt('COM_SUPPORT_CATEGORY_TOOLS');
            $row->set('category', ($row->get('category') ? $row->get('category') : $defaultCategory));
            $row->set('resolved', '');
            $defaultEmail = (!isset($this->_data['supportemail']) ? '' : $this->_data['supportemail']);
            $row->set('email', ($row->get('email') ? $row->get('email') : $defaultEmail));
            $defaultName = Lang::txt('COM_SUPPORT_AUTOMATED_REPORT');
            $row->set('name', ($row->get('name') ? $row->get('name') : $defaultName));
    ...
    public function uploadTask($listdir, $comment_id = 0)
    {
        ...
        foreach ($file['name'] as $i => $name) {
            ...
            if ($file['size'][$i] > $sizeLimit) {
                $this->setError(
                    Lang::txt('File is too large. Max file upload size is %s', Number::formatBytes($sizeLimit))
                );
                continue;
            }
    ...
    private function userSelect($name, $active, $nouser = 0, $javascript = null, $order = 'a.name')
    {
        ...
        $this->database->setQuery($query);
        if ($nouser) {
            $users[] = \Hubzero\Facades\Html::select('option', '0', Lang::txt('COM_SUPPORT_NONE'), 'value', 'text');
        ...
        if ($results = $this->database->loadObjectList()) {
            ...
            foreach ($groups as $gname => $gusers) {
                $users[] = \Hubzero\Facades\Html::select('optgroup', Lang::txt('COM_SUPPORT_CHANGELOG_FIELD_GROUP') . ': ' . $gname);
    ...
    private function userSelectGroup($name, $active, $nouser = 0, $javascript = null, $group = '')
    {
            ...
            if (is_array($groups)) {
                foreach ($groups as $g) {
                    ...
                    if ($hzg->get('gidNumber')) {
                        ...
                        $users[] = \Hubzero\Facades\Html::select('optgroup', stripslashes($hzg->description));
                        ...
                        $users[] = \Hubzero\Facades\Html::select('option', '</OPTGROUP>');
                    }
                }
            }
        } else {
        ...
        if ($nouser) {
            array_unshift($users, \Hubzero\Facades\Html::select('option', '0', Lang::txt('COM_SUPPORT_NONE'), 'value', 'text'));
        }
