...
class Tickets extends AdminController
{
    ...
    public function editTask($comment = null)
    {
        ...
        if (trim($ticket->get('group_id') == null ? '' : $ticket->get('group_id'))) {
            $groupId = trim($ticket->get('group_id'));
            ...
            $configGroup = trim($this->config->get('group', ''));
            $lists['owner'] = $this->userSelectGroup(
                'ticket[owner]',
                $ticket->get('owner'),
                1,
            ...
            $lists['owner'] = $this->userSelect('ticket[owner]', $ticket->get('owner'), 1);
        }
    ...
    public function saveTask()
    {
        ...
        if ($isNew) {
            ...
            if ($defs) {
                ...
                $emailSubject = Config::get('sitename') . ' ' . Lang::txt('COM_SUPPORT') . ', '
                    . Lang::txt('COM_SUPPORT_TICKET_NUMBER', $ticket->get('id'));
        ...
        // Incoming comment
        if ($text) {
            // If a comment was posted always change status to open.  This is the typical
            // expected behavior. Preventing new comments from re-opening a ticket can
            // result in comments that are never responded to.
            $notOpenButSameState = (!$ticket->isOpen())
                && $ticket->get('open') == $old->get('open');
            $isWaitingAndSameUser = $ticket->isWaiting()
                && User::get('username') == $ticket->get('login');
        ...
        if (
            ...
            foreach ($ticket->watchers as $watcher) {
                ...
                $canReadPrivate = $comment->isPrivate()
                    && $this->acl->check('read', 'private_comments');
                if (!$comment->isPrivate() || $canReadPrivate) {
    ...
    public function batchTask()
    {
        ...
        $lists['owner'] = $this->userSelect('owner', '', 1);
    ...
    public function processTask()
    {
        ...
        $redirectUrl = 'index.php?option=' . $this->_option
            . '&controller=' . $this->_controller
            . (Request::getInt('no_html', 0) ? '&no_html=1' : '');
        App::redirect(
            Route::url($redirectUrl, false),
    ...
    private function userSelect($name, $active, $nouser = 0, $javascript = null, $order = 'a.name')
    {
        ...
        if ($nouser) {
            $users[] = \Hubzero\Facades\Html::select('option', '0', Lang::txt('COM_SUPPORT_NO_USER'), 'value', 'text');
        ...
        if ($results = $database->loadObjectList()) {
            ...
            foreach ($groups as $nme => $gusers) {
                $users[] = \Hubzero\Facades\Html::select('optgroup', Lang::txt('COM_SUPPORT_GROUP') . ' ' . $nme);
