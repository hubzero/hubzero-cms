...
$toolbarTitle = Lang::txt('COM_SUPPORT') . ': '
    . Lang::txt('COM_SUPPORT_TICKET') . ': ' . $text;
...
>
        ...
        <div class="col span8">
            <fieldset class="adminform">
                ...
                <input
                    ...
                    name="ticket[summary]"
                    id="field-summary"
                    value="<?php echo $this->escape($this->row->get('summary')); ?>"
                    size="50"
                />

                <div class="input-wrap">
                    <label for="field-login">
                        <?php echo Lang::txt('COM_SUPPORT_TICKET_FIELD_LOGIN'); ?>:
                    </label>
                    <input
                        type="text"
                        name="ticket[login]"
                        id="field-login"
                        value="<?php echo $this->escape(trim($this->row->get('login'))); ?>"
                        size="50"
                    />
                </div>
                <div class="input-wrap">
                    <label for="field-name">
                        <?php echo Lang::txt('COM_SUPPORT_TICKET_FIELD_NAME'); ?>:
                    </label>
                    <input
                        type="text"
                        name="ticket[name]"
                        id="field-name"
                        value="<?php echo $this->escape(trim($this->row->get('name'))); ?>"
                        size="50"
                    />
                </div>
                <div class="input-wrap">
                    <label for="field-email">
                        <?php echo Lang::txt('COM_SUPPORT_TICKET_FIELD_EMAIL'); ?>:
                    </label>
                    <input
                        type="email"
                        name="ticket[email]"
                        id="field-email"
                        value="<?php echo $this->escape($this->row->get('email')); ?>"
                        size="50"
                    />
                </div>
                <div class="input-wrap">
                    <label for="field-report">
                        <?php echo Lang::txt('COM_SUPPORT_TICKET_FIELD_DESCRIPTION'); ?>:
                    </label>
                    <?php
                    $reportVal = $this->escape(
                        trim($this->row->get('report') == null ? '' : $this->row->get('report'))
                    );
                    ?>
                    <textarea
                        name="ticket[report]"
                    ...
                    <div class="col span6">
                        <div class="input-wrap">
                            ...
                            $gc = Event::trigger(
                                ...
                                array(array('groups', 'ticket[group_id]', 'acgroup', '', '', '', 'owner'))
                            );
                            if (count($gc) > 0) {
                                echo $gc[0];
                            } else { ?>
                        ...
                        <div class="input-wrap">
                            ...
                            <select name="ticket[severity]" id="field-severity">
                                <?php foreach (\Components\Support\Helpers\Utilities::getSeverities() as $severity) { ?>
                                    <?php $sl = Lang::txt('COM_SUPPORT_TICKET_SEVERITY_' . strtoupper($severity)); ?>
                                    <option
                            ...
                            <select name="ticket[status]" id="field-status">
                                <optgroup label="<?php echo Lang::txt('COM_SUPPORT_TICKET_COMMENT_OPT_OPEN'); ?>">
                                    <option value="0" selected="selected">
                                        <?php echo Lang::txt('COM_SUPPORT_TICKET_COMMENT_OPT_NEW'); ?>
                                    </option>
    ...
    <?php $referer = Request::getString('HTTP_REFERER', null, 'server'); ?>
    <input type="hidden" name="ticket[referrer]" value="<?php echo $referer; ?>" />
    ...
    <?php $hostname = gethostbyaddr(Request::getString('REMOTE_ADDR', '', 'server')); ?>
    <input type="hidden" name="ticket[hostname]" value="<?php echo $hostname; ?>" />
    <?php $userAgent = Request::getString('HTTP_USER_AGENT', '', 'server'); ?>
    <input type="hidden" name="ticket[uas]" value="<?php echo $userAgent; ?>" />
