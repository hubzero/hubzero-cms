    ...
    <?php foreach ($this->folders as $folder) { ?>
        <li id="folder_<?php echo $this->escape($folder->id); ?>" class="open">
            ...
            <span class="folder-options">
                ...
                ><?php echo Lang::txt('COM_SUPPORT_DELETE'); ?></a>
                <a
                ...
                ><?php echo Lang::txt('COM_SUPPORT_EDIT'); ?></a>
            </span>
            <ul id="queries_<?php echo $this->escape($folder->id); ?>" class="queries">
                <?php foreach ($folder->queries()->order('ordering', 'asc')->rows() as $query) { ?>
                    <li
                        ...
                        <?php if ($this->show == $query->id) {
                            echo ' class="active"';
                        } ?>
                    >
                        <?php
                        $searchParam = (intval($this->show) != $query->id) ? '&search=' : '';
                        $queryUrl = Route::url(
                            'index.php?option=' . $this->option
                            . '&controller=tickets&show=' . $query->id . $searchParam
                        );
