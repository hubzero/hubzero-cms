...
if ($this->condition->expressions) {
    foreach ($this->condition->expressions as $expression) {
        ...
        <p class="conditions"><button class="remove" alt="<?php echo Lang::txt('COM_SUPPORT_QUERY_REMOVE'); ?>">&times;</button> <select class="fld">
                ...
                <option value="type"<?php if ($expression->fldval == 'type') {
                    echo ' selected="selected"';
            ...
            <select class="op">
                ...
                if ($operators) {
                    foreach ($operators as $operator) {
                        ?>
                        <option value="<?php echo $operator->val; ?>"<?php if ($expression->opval == $operator->val) {
                            echo ' selected="selected"';
                                       } ?>><?php echo $operator->label; ?></option>
                        <?php
                    }
                }
                ?>
        ...
        if (is_array($values)) {
            ...
            foreach ($values as $value) {
                ?>
                <option value="<?php echo $value->val; ?>"<?php if ($expression->val == $value->val) {
                    echo ' selected="selected"';
