...
$this->css('
.color-block {
	display:block;width:1em;height:1em;overflow:hidden;text-indent:5em;
	border:1px solid #999;background-color: attr(data-color);
}
');
?>
...
<form action="<?php echo $formAction; ?>" method="post" name="adminForm" id="adminForm">
    <fieldset id="filter-bar">
        ...
        <select name="open" id="filter-open" class="filter filter-submit">
            <option value="-1"<?php if ($this->filters['open'] < 0) {
                echo ' selected="selected"';
                              } ?>><?php echo Lang::txt('COM_SUPPORT_FOR_ALL'); ?></option>
            <option value="0"<?php if ($this->filters['open'] == 0) {
                echo ' selected="selected"';
                             } ?>><?php echo Lang::txt('COM_SUPPORT_FOR_CLOSED'); ?></option>
            <option value="1"<?php if ($this->filters['open'] == 1) {
                echo ' selected="selected"';
                             } ?>><?php echo Lang::txt('COM_SUPPORT_FOR_OPEN'); ?></option>
        </select>
    </fieldset>
    ...
    <table class="adminlist">
        ...
        foreach ($this->rows as $row) {
            if ($row->get('color')) {
                $colorBlock = '.color-block-' . $row->get('id')
                    . ' { background-color: #' . $row->get('color') . '; }';
                $this->css($colorBlock);
            }
            ?>
            <tr>
                ...
                <td class="priority-2">
                    <?php
                    if ($row->get('open')) {
                        echo Lang::txt('COM_SUPPORT_FOR_OPEN');
                    } else {
                        echo Lang::txt('COM_SUPPORT_FOR_CLOSED');
                    }
                    ?>
                </td>
                <td>
                ...
                <td class="priority-4">
                    <span class="color-block color-block-<?php echo $row->get('id'); ?>">
                        <?php echo $row->get('color'); ?>
                    </span>
                </td>
            </tr>
            <?php
