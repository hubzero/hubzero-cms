...
class Entriesv1r1 extends ApiController
{
    /**
     ...
     * @apiParameter {
     *         "name":          "recipients",
     *         "description":   "Filter by recipients (type:id). Example: user:1000,project:2413",
     *         "type":          "string",
     *         "required":      false,
     *         "default":       ""
     * }
    ...
    public function listTask()
    {
        ...
        'limit'      => Request::getInt('limit', \Hubzero\Facades\Config::get('list_limit')),
        ...
        if ($filters['start_date']) {
            if (
                preg_match($datetimePattern, $filters['start_date'])
                || preg_match($datePattern, $filters['start_date'])
            ) {
                $query->where(
                    $l . '.created',
                    '>=',
                    with(new Date($filters['start_date']))->toSql()
                );
            } else {
                throw new Exception(
                    Lang::txt('COM_ACTIVITY_ERROR_INVALID_INPUT', $filters['start_date']),
                    409
                );
            }
        }
        ...
        if ($filters['end_date']) {
            if (
                preg_match($datetimePattern, $filters['end_date'])
                || preg_match($datePattern, $filters['end_date'])
            ) {
                $query->where(
                    $l . '.created',
                    '<',
                    with(new Date($filters['end_date']))->toSql()
                );
            } else {
                throw new Exception(
                    Lang::txt('COM_ACTIVITY_ERROR_INVALID_INPUT', $filters['end_date']),
                    409
                );
            }
        }
        ...
        foreach ($rows as $row) {
            ...
            $dt = \Hubzero\Facades\Date::of($obj->created);
            $ct = \Hubzero\Facades\Date::of('now');
            ...
            $isoFormat = 'Y-m-d\TH:i:s\Z';
            foreach ($obj->recipients as $k => $recipient) {
                $created = $obj->recipients[$k]->created;
                $obj->recipients[$k]->created = with(new Date($created))->format($isoFormat);
                if ($obj->recipients[$k]->viewed) {
                    $viewed = $obj->recipients[$k]->viewed;
                    $obj->recipients[$k]->viewed = with(new Date($viewed))->format($isoFormat);
                }
            }
    ...
    public function createTask()
    {
        ...
        $isoFormat = 'Y-m-d\TH:i:s\Z';
        $row->set('created', with(new Date($row->get('created')))->format($isoFormat));
