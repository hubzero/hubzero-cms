...
class Item extends Nested
{
    ...
    public function save()
    {
        ...
        if ($isNew) {
            ...
            $query = $this->getQuery()
                ...
                ->where(
                    $reposition->right_where['col'],
                    $reposition->right_where['op'],
                    $reposition->right_where['val']
                );
    ...
    public function getForm()
    {
        ...
        if (
            !(($this->get('id') && User::authorise('core.edit.state', 'com_menus.item.' . (int) $this->get('id')))
    ...
    protected function preprocessForm(Form $form, $data, $group = 'content')
    {
        ...
        if ($type == 'component') {
            ...
            if (isset($args['view'])) {
                ...
                $layoutPath = $base . '/views/' . $view . '/tmpl/' . $layout . '.xml';
                $path = \Hubzero\Filesystem\Util::normalizePath($layoutPath);
                ...
                if (!$formFile && (strpos($layout, ':') > 0 )) {
                    $temp = explode(':', $layout);
                    $tplPath = PATH_APP . '/templates/' . $temp[0] . '/html/'
                        . $option . '/' . $view . '/' . $temp[1] . '.xml';
                    $templatePath = \Hubzero\Filesystem\Util::normalizePath($tplPath);
    ...
    public function moveByReference($referenceId, $position = 'after', $pk = 0)
    {
        ...
        if ($referenceId) {
            ...
            if (!$reference->get('id')) {
                $errorMsg = Lang::txt('JLIB_DATABASE_ERROR_MOVE_FAILED');
                $this->addError($errorMsg . ': Reference not found #' . $referenceId);
                return false;
            }

            // Get the reposition data for shifting the tree and re-inserting the node.
            $width = $node->rgt - $node->lft + 1;
            if (!$repositionData = $this->getTreeRepositionData($reference, $width, $position)) {
            ...
            $width = $node->rgt - $node->lft + 1;
            if (!$repositionData = $this->getTreeRepositionData($reference, $width, 'last-child')) {
                $this->addError(Lang::txt('JLIB_DATABASE_ERROR_MOVE_FAILED') . ': Reposition data');
                return false;
            }
        }
        ...
        $query = $this->getQuery()
            ...
            ->where(
                $repositionData->left_where['col'],
                $repositionData->left_where['op'],
                $repositionData->left_where['val']
            );
        ...
        $query = $this->getQuery()
            ...
            ->where(
                $repositionData->right_where['col'],
                $repositionData->right_where['op'],
                $repositionData->right_where['val']
            );
    ...
    protected function batchCopy($value, $pks, $contexts)
    {
        ...
        while (!empty($pks) && $count > 0) {
            ...
            $parentId = $model->get('parent_id');
            $modelAlias = $model->get('alias');
            $modelTitle = $model->get('title');
            list($title, $alias) = $this->generateNewTitle($parentId, $modelAlias, $modelTitle);
     ...
     *
     * Used to easily create distinct labels when copying objects.
     * The method has the following styles:
     *
     ...
     * @param   integer  $n       If supplied, this number is used for the copy,
     *                            otherwise it is the 'next' number.
