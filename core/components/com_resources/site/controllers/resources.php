...
class Resources extends SiteController
{
    ...
    public function browseTask()
    {
        ...
        $validSorts = [
            'date', 'date_published', 'date_created', 'date_modified',
    ...
    public function browserTask()
    {
        ...
        switch ($level) {
            case 1:
                ...
                $bits['tags'] = $rt->getTagsWithObjects($bits['id'], $bits['type'], $bits['tg2']);
            ...
            case 3:
                ...
                if ($firstChild || $model->isTool()) {
                    ...
                    $bits['primary_child'] = Html::primaryChild($this->_option, $model, $firstChild, $xact);
                }

                // Get the sections
                $bits['sections'] = Event::trigger(
                    'resources.onResources',
                    array($model, $this->_option, array('about'), 'metadata')
                );
    ...
    protected function selectPresentation()
    {
        ...
        App::redirect(Route::url($watchUrl));
    }
    ...
    protected function preWatch()
    {
        ...
        $path = $path ? $path : $activechild->relativepath();
        $path = $base . $path;
        ...
        if (!is_dir($media_path)) {
            ...
            foreach ($slide_video as $k => $v) {
                if (count($v) < 3) {
                    $formatList = implode(", {$k}.", array_keys($v));
                    $this->setError(Lang::txt(
                        'Video Slides must be Uploaded in the Three Standard Formats. '
                        . 'You currently only have ' . count($v) . " ({$k}." . $formatList . ').'
                    ));
                }

                $hasPng = file_exists($slide_path . DS . $k . '.png');
                $hasJpg = file_exists($slide_path . DS . $k . '.jpg');
                if (!$hasPng && !$hasJpg) {
                    $this->setError(Lang::txt(
                        'Slides containing video must have a still image of the slide '
                        . 'for mobile support. Please upload an image with the filename "'
                        . $k . '.png" or "' . $k . '.jpg".'
                    ));
                }
            }
    ...
    public function watchTask()
    {
        ...
        $shouldRedirect = is_object($tracking)
            && !$hasTime
    ...
    {
        // Incoming
        // Resource ID (primary method of identifying a resource)
        $id = Request::getInt('id', 0);
        // Alternate method of identifying a resource
        $alias = Request::getString('alias', '');
        // A parameter to see file size without formatting
        $fsize = Request::getString('fsize', '');

        // XSS fix. Revision gets pumped all over and dumped in URLs via plugins,
        // easier to fix at the input instead of risking missing an output.
        // See ticket 1416 - Get svk revision of a tool
        $revision = htmlentities(Request::getString('rev', ''));
        ...
        if (User::isGuest() && ($access == 1 || $access == 4)) {
            $defaultUri = Route::url($this->model->link(), false, true);
            $requestUri = Request::getString('REQUEST_URI', $defaultUri, 'server');
            $return = base64_encode($requestUri);
            App::redirect(
                Route::url('index.php?option=com_users&view=login&return=' . $return, false),
                Lang::txt('COM_RESOURCES_ALERTLOGIN_REQUIRED'),
                'warning'
        ...
        if ($this->model->inGroup()) {
            ...
            if ($group) {
                ...
                $typeAlias = $this->model->type->get('alias');
                $resourcesUrl = 'index.php?option=com_groups&cn=' . $group->get('cn')
                    . '&active=resources&area=' . $typeAlias;
                Pathway::append(
                    stripslashes($this->model->type->get('type')),
                    Route::url($resourcesUrl)
                );
            } else {
                Pathway::append(
                    stripslashes($this->model->type->get('type')),
        ...
        // Write title
        $optionTitle = Lang::txt(strtoupper($this->_option));
        Document::setTitle($optionTitle . ': ' . stripslashes($this->model->title));
        ...
        $templatePath = App::get('template')->path . DS . 'html' . DS . $this->_option
            . DS . 'view' . DS . $type_alias . '.php';
        $viewPath = dirname(__DIR__) . DS . 'views' . DS . 'view' . DS . 'tmpl'
            . DS . $type_alias . '.php';
        if (
            $type_alias
            && (is_file($templatePath) || is_file($viewPath))
        ) {
            $layout = $type_alias;
        }
        // Instantiate a new view
    ...
    public function feedTask()
    {
        ...
        $cleanedIntro = \Hubzero\Utility\Sanitize::clean(stripslashes($resource->introtext));
        $doc->description = htmlspecialchars(
            html_entity_decode($cleanedIntro),
            ENT_COMPAT,
            'UTF-8'
        );
        ...
        $cleanedText = \Hubzero\Utility\Sanitize::clean(stripslashes($resource->introtext));
        $doc->itunes_summary = html_entity_decode($cleanedText);
        ...
        if ($dimg) {
            ...
            $dimage->url = $dimg;
            $artworkTitle = $dtitle . ' ' . Lang::txt('COM_RESOURCES_RSS_ARTWORK');
            $dimage->title = trim(\Hubzero\Utility\Str::truncate(
                html_entity_decode($artworkTitle),
                250
            ));
            ...
            foreach ($rows as $row) {
                ...
                foreach ($queried_logical_types as $queried) {
                    ...
                    foreach ($all_logical_types as $logical_type) {
                        ...
                        if (
                            preg_match_all('/Podcast \(([^()]+)\)/', $logical_type->type, $matches) == 1
                ...
                if ($grandchildren && count($grandchildren) > 0) {
                    foreach ($grandchildren as $grandchild) {
                        $logicalType = (int)$grandchild->get('logical_type');
                        if (isset($relevant_logical_types_by_id[$logicalType])) {
                            if (stripslashes($grandchild->introtext) != '') {
                                ...
                                $pattern = "#<nb:(.*?)>(.*?)</nb:(.*?)>#s";
                                $gdescription = preg_replace(
                                    $pattern,
                                    '',
                                    stripslashes($gdescription)
                                );
                                $cleanedGdesc = \Hubzero\Utility\Sanitize::clean(
                                    $gdescription
                                );
                                $gdescription = html_entity_decode($cleanedGdesc);
                ...
                foreach ($podcasts as $podcast) {
                    ...
                    if ($podcast) {
                        $podcastp = $podcast;
                        $podcast = ltrim($this->fullPath($podcast), '/');
                        $uploadPath = $this->config->get('uploadpath');

                        if (substr($podcastp, 0, strlen($uploadPath)) == $uploadPath) {
                            // Do nothing
                        } else {
    ...
    {
        $path = Html::buildPath($created, $id, '');
    ...
    public function downloadTask()
    {
        ...
        $activeRestrict = $activeParams->get('restrict_direct_access');
        $typeRestrict = $typeParams->get('restrict_direct_access');
        $restrictDirectDownload = $activeRestrict ?: $typeRestrict;
        ...
        if (($token = Request::getString('token', '', 'get'))) {
            ...
            $key = App::hash($userAgent);
            $cipher = new \Hubzero\Encryption\Cipher\Simple();
            $encKey = new \Hubzero\Encryption\Key('simple', $key, $key);
            $crypter = new \Hubzero\Encryption\Encrypter($cipher, $encKey);
        ...
        $parentAccess = isset($parent) ? $parent->access : 0;
        $accessLevel = ($parentAccess > $resource->access) ? $parentAccess : $resource->access;

        if ($accessLevel == 1 && $user->isGuest()) {
            $defaultUrl = Route::url(
                'index.php?option=' . $this->_option . '&id=' . $this->id . '&d=' . $d,
                false,
                true
            );
            $requestUri = Request::getString('REQUEST_URI', $defaultUrl, 'server');
            $return = base64_encode($requestUri);
        ...
        if (
            $accessLevel == 4  // private
            || ($accessLevel == 3 && !$resource->get('standalone'))
        ) {
            // protected - allow images from 'protected' resource abstracts to come through
            if ($user->isGuest()) {
                $defaultUrl = Route::url(
                    'index.php?option=' . $this->_option . '&id=' . $this->id . '&d=' . $d,
                    false,
        ...
        } elseif (strpos($resource_path, '/') !== false) {
        ...
        $resource_file = str_replace($resource_base, '', $resource_path);
        $resource_file = ltrim($resource_file, '/');
        ...
        if (!file_exists($path)) {
            ...
            $path = $resource->basepath() . '/' . $resource_base
                . '/media/' . $request_file;
        ...
        $inlineTypes = [
            'tiff', 'tif', 'jpg', 'jpeg', 'jpe', 'gif', 'png', 'pdf',
            'htm', 'html', 'txt', 'json', 'xml', 'ogv', 'mp4', 'webm', 'ogg', 'mp3'
        ];
    ...
    public function licenseTask()
    {
        ...
        $title = $rowTitle . ': ' . Lang::txt('COM_RESOURCES_LICENSE');
    ...
    public function citationTask()
    {
        ...
        if ($revision) {
            ...
            $hasDoi = $rdoi->get('doi');
            $doiShoulder = $rdoi->get('doi_shoulder') ?: $tconfig->get('doi_shoulder');
            if ($hasDoi && $doiShoulder) {
                $handle = 'doi:' . $doiShoulder . '/' . strtoupper($rdoi->doi);
            }
        }
