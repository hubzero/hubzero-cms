...
class Attachments extends SiteController
{
    ...
    public function ajaxCreateTask()
    {
        ...
        if ($resource->isNew()) {
            ...
            $find = array('|', '\\', '{', '}', '^');
            $replace = array('%7C', '%5C', '%7B', '%7D', '%5E');
            $resource->path = str_replace($find, $replace, $resource->path);
    ...
    public function ajaxUploadTask()
    {
        ...
        $resource = Entry::blank()->set(array(
            ...
            'type'         => $this->getChildType($filename . '.' . $ext)
        ));
    ...
    public function saveTask()
    {
        ...
        $resource = Entry::blank()->set(array(
            ...
            'access'       => 0,
            'path'         => '', // make sure no path is specified just yet
            'type'         => $this->getChildType($file['name'])
        ));
        ...
        // Build the path
        $listdir = $this->buildPathFromDate($resource->get('created'), $resource->get('id'), '');
        ...
        if (!Filesystem::upload($file['tmp_name'], $path . DS . $file['name'])) {
            ...
            // File was uploaded
            // Check the file type
            $resource->set('type', $this->getChildType($file['name']));
        }
    ...
    protected function textifier($path, $id)
    {
        if (is_readable($path)) {
            ...
            if (!empty($hash)) {
                ...
                if (!$doc_id) {
                    $sql = "INSERT INTO `#__document_text_data` (hash) VALUES ("
                        . $database->quote($hash) . ")";
                    $database->execute($sql);
                    $doc_id = $database->insertId();
                }

                $sql = "INSERT IGNORE INTO `#__document_resource_rel` (document_id, resource_id) "
                    . "VALUES (" . (int)$doc_id . ", " . (int)$id . ")";
                $database->execute($sql);
     ...
     * @return  integer
     */
    private function getChildType($filename)
    {
     ...
     * @return  string
     */
    private function buildPathFromDate($date, $id, $base = '')
    {
