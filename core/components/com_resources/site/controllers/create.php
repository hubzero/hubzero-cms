...
class Create extends SiteController
{
    ...
    public function _buildPathway($row)
    {
        ...
        if ($this->step) {
            $stepTitle = Lang::txt('COM_CONTRIBUTE_STEP_NUMBER', $this->step) . ': '
                . Lang::txt('COM_CONTRIBUTE_STEP_' . strtoupper($this->steps[$this->step]));
            Pathway::append(
                $stepTitle,
    ...
    public function _buildTitle($row)
    {
        ...
        if ($this->step && isset($this->steps[$this->step])) {
            $this->_title .= ': ' . Lang::txt('COM_CONTRIBUTE_STEP_NUMBER', $this->step) . ': '
                . Lang::txt('COM_CONTRIBUTE_STEP_' . strtoupper($this->steps[$this->step]));
        }
    ...
    private function loadFocusAreas($type, $labels = null, $parent_id = null, $parent_label = null)
    {
        ...
        // Build the query based on whether we have a parent_id
        if ($parent_id) {
            // Get tags labeled focus area and parented by the tag identified by $parent_id
            $sql = 'SELECT DISTINCT t.raw_tag AS label, t2.id, t2.tag, t2.raw_tag, t2.description
                FROM `#__tags` t
                INNER JOIN `#__tags_object` to1
                    ON to1.tbl = \'tags\' AND to1.tagid = t.id AND to1.label = \'label\'
                INNER JOIN `#__tags_object` to2
                    ON to2.tbl = \'tags\' AND to2.label = \'parent\'
                    AND to2.objectid = to1.objectid AND to2.tagid = ' . $parent_id . '
            ...
            // Get tags labeled as focus areas that are not also a parent of another
            // tag labeled as a focus area
            $sql = 'SELECT DISTINCT t.raw_tag AS label, t2.id, t2.tag, t2.raw_tag, t2.description
                FROM `#__tags` t
                LEFT JOIN `#__tags_object` to1
                    ON to1.tagid = t.id AND to1.label = \'label\' AND to1.tbl = \'tags\'
                INNER JOIN `#__tags` t2 ON t2.id = to1.objectid
                WHERE t.tag IN (' . $labels . ') AND (
                    SELECT COUNT(*)
                    FROM `#__tags_object` to2
                    INNER JOIN `#__tags_object` to3
                        ON to3.tbl = \'tags\' AND to3.label = \'label\'
                        AND to3.objectid = to2.tagid
                    INNER JOIN `#__tags` t3
                        ON t3.id = to3.tagid AND t3.tag IN (' . $labels . ')
                    WHERE to2.tbl = \'tags\' AND to2.label = \'parent\'
                        AND to2.objectid = t2.id
                    LIMIT 1
                ) = 0
                ORDER BY t.tag,
                    CASE WHEN t2.raw_tag LIKE \'other%\' THEN 1 ELSE 0 END,
                    t2.raw_tag';
        }
        $this->database->setQuery($sql);
        $fas = $this->database->loadAssocList('raw_tag');
        foreach ($fas as &$fa) {
            $fa['children'] = $this->loadFocusAreas($type, $labels, $fa['id'], $fa['label']);
        }
        return $fas;
    }
    ...
    public function step_tags($existing = array())
    {
        ...
        $fas = $this->loadFocusAreas($this->database->loadResult());
        $focusareas = array();
    ...
    public function step_compose_process()
    {
        ...
        $publishUp = $row->get('publish_up');
        if (!$publishUp || $publishUp == '0000-00-00 00:00:00') {
            $publishUp = Date::toSql();
        }
        $row->set('publish_up', $publishUp);
        ...
        $row->set('title', $this->txtClean($row->get('title')));
        ...
        if (trim($row->get('fulltxt'))) {
            ...
            $row->set('fulltxt', $this->txtClean($row->get('fulltxt')));
            $row->set('footertext', $this->txtClean($row->get('footertext')));
        }
        ...
        $oldPath = $row->basepath() . Html::buildPath($created, $session->get('resources_temp_id'), '');
        $newPath = $row->filespace();

        // if we have a temp dir, move it to permanent location
        if (is_dir($oldPath)) {
            \Hubzero\Facades\Filesystem::move($oldPath, $newPath);
        ...
        $resourceUrl = Route::url('index.php?option=com_resources&id=' . $row->get('id'));
        $resourceLink = '<a href="' . $resourceUrl . '">' . $row->get('title') . '</a>';
        $activityKey = 'COM_RESOURCES_ACTIVITY_ENTRY_' . (!$isNew ? 'UPDATED' : 'CREATED');
        Event::trigger('system.logActivity', [
            'activity' => [
                'action'      => ($isNew ? 'updated' : 'created'),
                'scope'       => 'resource',
                'scope_id'    => $row->get('id'),
                'description' => Lang::txt($activityKey, $resourceLink),
                'details'     => array(
                    'title' => $row->get('title'),
                    'url'   => $resourceUrl
                )
    ...
    public function step_authors_process()
    {
        ...
        if ((!$prev && $row->get('group_owner')) || ($prev && !$row->get('group_owner'))) {
            ...
            $groupLink = '<a href="' . $groupUrl . '">' . $group->get('description') . '</a>';
            Event::trigger('system.logActivity', [
                'activity' => [
                    'action'      => 'updated',
                    'scope'       => 'resource',
    ...
    public function step_tags_process()
    {
        ...
        $userId = User::get('id');
        $sql = 'SELECT 1 FROM `#__author_assoc`
            WHERE authorid = ' . $userId . ' AND subtable = \'resources\' AND subid = ' . $id . '
            UNION
            SELECT 1 FROM `#__resources`
            WHERE id = ' . $id . ' AND (created_by = ' . $userId . ' OR modified_by = ' . $userId . ')
            UNION
            SELECT 1 FROM `#__users` u
            INNER JOIN `#__user_usergroup_map` cagam ON cagam.user_id = u.id
            INNER JOIN `#__usergroups` caag ON caag.id = cagam.group_id
                AND (caag.title = \'Super Administrator\'
                    OR caag.title = \'Super Users\'
                    OR caag.title = \'Administrator\')
            WHERE u.id = ' . $userId;
        $this->database->setQuery($sql);
        ...
        foreach ($push as $idx => $tag) {
            $this->database->setQuery(
                ...
                INNER JOIN `#__tags_object` to2
                    ON to2.tagid = ' . $tag[4] . '
                    AND to2.tbl = \'tags\'
                    AND to2.objectid = to1.tagid
                WHERE to1.objectid = ' . $tag[3] . '
                    AND to1.tbl = \'tags\'
                    AND to1.label = \'parent\''
            );
            $any_match = false;
            $parent = array();
            ...
            if (!$possible_parents || $any_match) {
                ...
                $fasTag = $fas[$tag[2]] ?? null;
                if ($fasTag && $fasTag['actual_depth'] < $fasTag['minimum_depth']) {
                    // count depth if necessary to determine whether focus area
                    // constraints are satisified
                    $depth = $parent ? 2 : 1;
                    while (
                        $parent_id
                        && $fas[$tag[2]]['actual_depth'] < $fas[$tag[2]]['minimum_depth']
                        && $depth < $fas[$tag[2]]['minimum_depth']
                    ) {
                        $this->database->setQuery(
                            'SELECT t.id
                            FROM `#__tags_object` to1
                            ...
                            INNER JOIN `#__tags_object` to2
                                ...
                                AND to2.tbl = \'tags\'
                                AND to2.objectid = to1.tagid
                            WHERE to1.objectid IN (' . implode(',', $parent_id) . ')
                                AND to1.tbl = \'tags\'
                                AND to1.label = \'parent\''
                        );
    ...
    public function submitTask()
    {
        ...
        if (!$published) {
            ...
            if (!empty($defs)) {
                ...
                $subjectTxt = Lang::txt(
                    'COM_RESOURCES_EMAIL_SUBJECT_NEW_SUBMISSION',
                    $resource->id
                );
                $message->setSubject($sitename . ' ' . $subjectTxt);
                $message->addFrom(
                    Config::get('mailfrom'),
            ...
            $resourceUrl = Route::url($resource->link());
            $resourceLink = '<a href="' . $resourceUrl . '">'
                . $resource->get('title') . '</a>';
            $activityKey = 'COM_RESOURCES_ACTIVITY_ENTRY_' . strtoupper($activity);
            Event::trigger('system.logActivity', [
                'activity' => [
                    'action'      => $activity,
                    'scope'       => 'resource',
        ...
        // If the resource gets published (auto-publish or the user is in the
        // auto-approved list), trigger the onAfterContentSubmission event
            ...
            case 2:
                ...
                Event::trigger('system.logActivity', [
                    'activity' => [
                        ...
                        'description' => Lang::txt(
                            'COM_RESOURCES_ACTIVITY_ENTRY_DELETED',
                            $resourceLink
                        ),
                        'details'     => array(
                            'title' => $resource->get('title'),
     ...
     * @return  string
     */
    private function txtClean($text)
    {
