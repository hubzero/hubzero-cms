...
HUB.Video = {
	...
	controlBar: function()
	{
		...
		$jQ(document).on('click', '#video-container .control', function(e) {
			...
			$jQ('#video-container .control.open').not(btn).removeClass('open').attr('aria-expanded', 'false');
			if (isOpen) {
		...
		// clicking inside a popup keeps it open
		$jQ(document).on('click', '#video-container .control-container', function(e) {
			e.stopPropagation();
		});
		...
		$jQ('#video-stage').on('mouseleave', function() {
	...
	{	
		...
		// start syncing subtitles (also called by external plugins like NanoBud)
		if (!syncInterval)
		{
			var capturedSubs = sub_titles;
			syncInterval = setInterval(function() {
				HUB.Video.syncSubtitles( capturedSubs );
			}, 100);
		}
	},
	...
	transcriptSearch: function()
	{
		$jQ('#transcript-search').on('input', function() {
			var term = $jQ(this).val().trim();

			// clear previous state
			$jQ('.transcript-line-text').removeHighlight();
			$jQ('.transcript-line').removeClass('search-hidden');
			$jQ('#transcript-search-clear').toggle(term.length > 0);

			if (!term) {
				$jQ('#transcript-search-count').hide().text('');
				return;
			}
			...
			var count = 0,
				$firstMatch = null;
			$jQ('.transcript-line').each(function() {
				var text = $jQ(this).find('.transcript-line-text').text();
				if (text.toUpperCase().indexOf(term.toUpperCase()) === -1) {
					$jQ(this).addClass('search-hidden');
				} else {
					count++;
					if (!$firstMatch) { $firstMatch = $jQ(this); }
				}
			});

			// highlight matching text within visible lines
			$jQ('.transcript-line:not(.search-hidden) .transcript-line-text').highlight(term);

			// show result count
			$jQ('#transcript-search-count')
				.text(count > 0 ? count + ' result' + (count !== 1 ? 's' : '') : 'No results')
				.show();

			// scroll to first match
			if ($firstMatch) {
				var containerTop = $jQ('#transcripts').offset().top,
					matchTop = $firstMatch.offset().top,
					scrollTarget = $jQ('#transcripts').scrollTop() + (matchTop - containerTop) - 20;
				$jQ('#transcripts').animate({ scrollTop: scrollTarget }, 200);
			}
		});

		$jQ('#transcript-search-clear').on('click', function() {
			$jQ('#transcript-search').val('').trigger('input').trigger('focus');
		});
	},
