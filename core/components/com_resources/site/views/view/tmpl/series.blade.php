{{--
  Resource series view — upper pane with overview, podcast feeds, and child
              ...
              <p>
                @if($audio)
                  <a id="resource-audio-feed"
                     href="{{ $feedBase . 'audio' }}">{{ Lang::txt('COM_RESOURCES_AUDIO_PODCAST') }}</a><br />
                @endif
                @if($video)
                  <a id="resource-video-feed"
                     href="{{ $feedBase . 'video' }}">{{ Lang::txt('COM_RESOURCES_VIDEO_PODCAST') }}</a><br />
                @endif
                @if($notes)
                  <a id="resource-slides-feed"
                     href="{{ $feedBase . 'slides' }}">{{ Lang::txt('COM_RESOURCES_SLIDES_PODCAST') }}</a>
                @endif
              </p>
            @endif
...
@if($model->access('view-all'))
  ...
  {{-- Child resource listing (series items) --}}
  @if($tab == 'about' && $ccount > 0)
    @php
      ...
      $defaultsort = $model->params->get('sort_children', 'ordering');
      $defaultsort = $model->params->get('show_ranking') ? 'ranking' : $defaultsort;
    ...
    <form method="get" id="series" action="{{ Route::url($model->link()) }}">
      <section class="section">
        <div class="section-inner hz-layout-with-aside">
          <div class="subject">
            <h3>{{ Lang::txt('COM_RESOURCES_IN_THIS_SERIES') }}</h3>

            @if($children->count())
              <ul class="list bg-base-100 rounded-box shadow-sm"
                  aria-label="{{ Lang::txt('COM_RESOURCES_IN_THIS_SERIES') }}">
