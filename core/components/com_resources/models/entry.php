...
class Entry extends Relational implements \Hubzero\Search\Searchable
{
     ...
     * State constants
     **/
    public const STATE_ARCHIVED = -1;
    public const STATE_DRAFT    = 2;
    public const STATE_PENDING  = 3;
    public const STATE_TRASHED  = 4;
    public const STATE_DRAFT_INTERNAL = 5;
    ...
    public function transformGroups()
    {
        ...
        $sql = "SELECT g.cn FROM #__resource_acl_group AS a, #__xgroups AS g "
            . "WHERE resource_id = ? AND a.group_id = g.gidNumber";
            ...
            if ($contributor->get('authorid') > 0) {
                if (!$solr) {
                    $profileUrl = Route::url(
                        'index.php?option=com_members&id=' . $contributor->get('authorid')
                    );
                    $name = '<a href="' . $profileUrl . '" data-rel="contributor" '
                        . 'class="resource-contributor" title="View the profile of '
                        . $name . '">' . $name . '</a>';
                }
            }
    ...
    public function contributors($idx = null)
    {
        ...
        if ($idx == 'tool') {
            if ($this->isTool() && $this->revision == 'dev') {
                ...
                $db = App::get('db');
                $sql = "SELECT n.id, t.name AS name, n.name AS xname, NULL AS xorg, "
                     . "n.givenName, n.givenName AS firstname, n.middleName, "
                     . "n.middleName AS middlename, n.surname, n.surname AS lastname, "
                     . "t.organization, t.*, a.role"
                     . " FROM `#__tool_authors` AS t "
                     . "LEFT JOIN `#__users` AS n ON n.id=t.uid "
                     . "JOIN `#__tool_version` AS v ON v.id=t.version_id"
                     . " LEFT JOIN `#__author_assoc` AS a ON a.authorid=t.uid "
                     . "AND a.subtable='resources' AND a.subid=" . $db->quote($this->get('id'))
                     . " WHERE t.toolname=" . $db->quote($this->get('alias'))
                     . " AND v.state<>3"
    ...
    public function isPublished()
    {
        ...
        $unpublishedStates = array(
            self::STATE_UNPUBLISHED,
            self::STATE_PENDING,
            self::STATE_DRAFT,
            self::STATE_TRASHED,
            self::STATE_DRAFT_INTERNAL
        );
        if (in_array($this->get('published'), $unpublishedStates)) {
            return false;
        }
    ...
    public function relativepath()
    {
        ...
        if ($date && preg_match($datePattern, $date, $regs)) {
        ...
        if ($date) {
            ...
            if (
                !is_dir($this->basepath() . DS . $dir_year . DS . $dir_month . DS . Str::pad($this->get('id')))
    ...
    {
        $param = $this->get('alias')
            ? 'alias=' . $this->get('alias')
            : 'id=' . $this->get('id');
        return 'index.php?option=com_resources&' . $param;
    }
        ...
        if ($this->isTool()) {
            ...
            if ($revisions) {
                foreach ($revisions as $tool) {
                    ...
                    if (
                        ($revision && $tool->revision == $revision && $revision != 'dev')
                        or ($revision == 'dev' and $tool->state == 3)
                    ) {
                        $this->thistool = $tool;
                    }
                    // Current version
                    $isSingleRev = count($revisions) == 1;
                    $isMultiRevMatch = count($revisions) > 1
                        && $revisions[1]->version == $tool->version;
                    if ($tool->state == 1 && ($isSingleRev || $isMultiRevMatch)) {
                        $this->curtool = $tool;
    ...
    public function searchResult()
    {
        ...
        $publicationDate = !empty($fields['yearofpublication'])
            ? $fields['yearofpublication']
            : '';
        if (empty($publicationDate) && !empty($fields['publicationdate'])) {
