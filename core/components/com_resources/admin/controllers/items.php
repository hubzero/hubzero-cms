...
class Items extends AdminController
{
    ...
    public function editTask($isnew = 0)
    {
        ...
        $configPath = dirname(dirname(__DIR__)) . DS . 'config' . DS . 'entry.xml';
        $params = new \Hubzero\Html\Parameter($row->get('params'), $configPath);

        // Build selects of various types
        if ($row->standalone != 1) {
            $typeRows = Type::all()->whereEquals('category', 30)->order('type', 'asc')->rows();
            $lists['type'] = Html::selectType(
                $typeRows,
                'fields[type]',
                $row->get('type'),
                'type'
            );
            $logicalTypeRows = Type::all()->whereEquals('category', 28)->order('type', 'asc')->rows();
            $lists['logical_type'] = Html::selectType(
                $logicalTypeRows,
                'fields[logical_type]',
            ...
            $lists['sub_type'] = Html::selectType(
                $typeRows,
                'fields[logical_type]',
                $row->get('logical_type'),
                'sub_type',
            ...
            $typeRows = Type::all()->whereEquals('category', 27)->order('type', 'asc')->rows();
            $lists['type'] = Html::selectType(
                $typeRows,
                'fields[type]',
                $row->get('type'),
            ...
            $logicalTypeRows = Type::all()
                ->whereEquals('category', 21)
                ->order('type', 'asc')
                ->rows();
            $lists['logical_type'] = Html::selectType(
                $logicalTypeRows,
        ...
        if ($row->standalone == 1) {
            ...
            $authorslist->set('option', $this->_option);
     ...
     * @return  bool
     */
    private function emailContributors($row)
    {
        $contributors = array();
        ...
        foreach ($ids as $id) {
            ...
            if (!$resource->get('checked_out') || $resource->get('checked_out') == User::get('id')) {
                ...
                if ($resource->get('standalone') == 1 && $this->config->get('email_when_approved')) {
                    // If the state went from pending to published
                    if (
                        $resource->get('published') == Entry::STATE_PUBLISHED
                        && $old == Entry::STATE_PENDING
                    ) {
                        $this->emailContributors($resource);
    ...
    public function resethitsTask()
    {
        ...
        // Redirect
        $editUrl = 'index.php?option=' . $this->_option . '&controller='
            . $this->_controller . '&task=edit&id=' . $id;
        App::redirect(Route::url($editUrl, false));
    }
