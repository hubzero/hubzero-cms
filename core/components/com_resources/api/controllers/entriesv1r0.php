...
class Entriesv1r0 extends ApiController
{
    ...
    public function renderlatexTask()
    {
        ...
        if (!is_dir($dir)) {
            \Hubzero\Facades\Filesystem::makeDirectory($dir);
        }
        ...
        try {
            ...
            $command = 'cd ' . $dir . '; /usr/bin/latex ' . $filename
                . '.tex < /dev/null |grep ^!|grep -v Emergency > '
                . $dir . DS . $filename . '.error 2> /dev/null 2>&1';
            exec($command, $output_lines, $exit_status);

            // execute dvi2png to build png
            $command = "/usr/bin/dvipng -bg 'transparent' -q -T tight -D 100 -o "
                . $dir . DS . $filename . '.png ' . $dir . DS . $filename . '.dvi 2>&1';
            exec($command, $output_lines, $exit_status);
