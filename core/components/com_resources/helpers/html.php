...
class Html
{
    ...
    {
        $fieldId = 'field-' . str_replace(array('[', ']'), '', $name);
        $html  = '<select class="' . $class . '" name="fields[' . $name . ']" id="' . $fieldId . '"';
        if (!$groups) {
    ...
    public static function dateToPath($date)
    {
        ...
        if ($date && preg_match($pattern, $date, $regs)) {
     ...
     * @return  string
     */
    public static function buildUrl($id, $base)
    {
     ...
     * @return  string
     */
    public static function buildPath($date, $id, $base)
    {
        $dir_id = self::niceidformat($id);
        ...
        if ($date) {
            ...
            $dirPath = $base . DS . $dir_year . DS . $dir_month . DS . $dir_id;
            if (!is_dir($dirPath) && intval($dir_year) <= 2013 && intval($dir_month) <= 11) {
    ...
    public static function sortSupportingDocs($publication, $option, $children)
    {
        ...
        if ($children) {
            foreach ($children as $child) {
                ...
                $toShow = array(
                    'User Guide', 'Syllabus', 'iTunes', 'iTunes U',
                    'Audio', 'Video', 'Slides', 'YouTube', 'Vimeo'
                );
                ...
                foreach ($toShow as $item) {
                    if (
                        strtolower($doctitle) !=  preg_replace('/' . strtolower($item) . '/', '', strtolower($doctitle))
                        && !in_array($item, $shown)
                    ) {
                        ...
                        if ($linkAction == 1) {
                            $supli[] = ' <li><a class="' . $class . '" rel="external" href="' . $url
                                . '" title="' . $child->title . '"' . $extra . '>' . $item
                                . '</a></li>' . "\n";
                        } elseif ($linkAction == 2) {
                            $class .= ' play';
                            $class .= ' ' . $width . 'x' . $height;
                            $supli[] = ' <li><a class="' . $class . '" href="' . $url
                                . '" title="' . $child->title . '"' . $extra . '>' . $item
                                . '</a></li>' . "\n";
                        } else {
                            $supli[] = ' <li><a class="' . $class . '" href="' . $url
        ...
        if ($docs > 0 && $otherdocs > 0) {
            $docsUrl = Route::url(
                'index.php?option=' . $option . '&id=' . $publication->id
                . '&active=supportingdocs'
            );
            $viewAllTxt = Lang::txt('COM_RESOURCES_VIEW_ALL');
            $suppDocsTxt = Lang::txt('COM_RESOURCES_SUPPORTING_DOCUMENTS');
            $moreTxt = Lang::txt('COM_RESOURCES_MORE');
            $supln .= ' <li class="otherdocs"><a href="' . $docsUrl
                . '" title="' . $viewAllTxt . ' ' . $docs . ' ' . $suppDocsTxt . ' ">'
                . $otherdocs . ' ' . $moreTxt . '</a></li>' . "\n";
        }
    ...
    public static function license($license)
    {
        ...
        if ($rl->id) {
            if (substr($rl->name, 0, 6) != 'custom') {
                ...
                if ($rl->url) {
                    $html .= ' under <a rel="license" href="' . $rl->url . '" title="'
                        . $rl->title . '">' . $rl->title . '</a>';
                } else {
                    $html .= ' under ' . $rl->title;
                }
                ...
                $licenseUrl = Route::url(
                    ...
                    . substr($rl->name, 6) . '&no_html=1'
                );
                $html = '<p class="' . $rl->name . ' license">Licensed under '
                    . '<a rel="license" class="popup" href="' . $licenseUrl . '">'
                    . $rl->title . '</a>.</p>';
            }
        }
        return $html;
        ...
        foreach ($sections as $section) {
            if ($section['html'] != '') {
                ...
                */
                ...
                $html .= '<div class="' . $cls . 'section" id="'
                    . $section['area'] . '-section">' . $section['html'] . '</div>';
            }
            $k++;
        }
    ...
    public static function tabs($option, $id, $cats, $active = 'about', $alias = '')
    {
        ...
        foreach ($cats as $cat) {
            ...
            if ($name != '') {
                ...
                $html .= '><a class="tab" data-rel="' . $name . '" href="' . $url . '">'
    ...
    public static function citation($option, $cite, $id, $citations, $type, $rev = '')
    {
        ...
        if ($cite) {
            ...
            if (is_numeric($rev) || (is_string($rev) && $rev != 'dev')) {
                ...
                $bibtexUrl = 'index.php?option=' . $option . '&task=citation&id=' . $id
                    . '&citationFormat=bibtex&no_html=1&rev=' . $rev;
                $html .= "\t\t\t" . '<a href="' . $bibtexUrl . '" title="'
                    . Lang::txt('COM_RESOURCES_DOWNLOAD_BIBTEX_FORMAT')
                    . '">BibTex</a> <span>|</span> ' . "\n";
                $endnoteUrl = 'index.php?option=' . $option . '&task=citation&id=' . $id
                    . '&citationFormat=endnote&no_html=1&rev=' . $rev;
                $html .= "\t\t\t" . '<a href="' . $endnoteUrl . '" title="'
                    . Lang::txt('COM_RESOURCES_DOWNLOAD_ENDNOTE_FORMAT')
                    . '">EndNote</a>' . "\n";
                $html .= "\t\t" . '</p>' . "\n";
            }
            $html .= "\t" . '</li>' . "\n";
    ...
    public static function processPath($option, $item, $pid = 0, $action = 0)
    {
        if ($item->standalone == 1) {
            ...
            switch ($type) {
                ...
                case 'video':
                    $url = Route::url(
                        'index.php?option=' . $option . '&id=' . $pid
                        . '&resid=' . $item->id . '&task=video'
                    );
                    break;
                ...
                default:
                    if ($action == 2) {
                        ...
                        $isExternal = strstr($item->path, 'http')
                        ...
                        if ($isExternal) {
                            ...
                            $url = Route::url(
                                'index.php?option=' . $option . '&id=' . $item->id
                                . '&task=download&file=' . basename($item->path)
                            );
                        }
                    }
                    break;
        ...
        switch ($resource->get('type')) {
            ...
            case 7:
                ...
                if (
                    ...
                    $pop   = '<p class="warning">'
                        . Lang::txt('COM_RESOURCES_TOOL_SESSION_INVOKE_DISABLED') . '</p>';
                    $html .= self::primaryButton(
                        'link_disabled',
                        '',
                ...
                if (in_array($table, $tables)) {
                    if (isset($resource->revision) && $resource->toolpublished) {
                        ...
                        $isDev = !isset($resource->revision) || $resource->revision == 'dev';
                        $v = $isDev ? 'test' : $resource->revision;
                ...
                if ($hztv) {
                    ...
                    // @FIXME: this only seems to fail on hubbub VMs where
                    // workspace resource is incomplete/incorrect (bad data in DB?)
                    if ($ht) {
                        $toolAlias = $resource->get('tool', $resource->get('alias'));
                        $toolgroups = $ht->getToolGroupsRestriction($hztv->toolid, $toolAlias);
                    }
                }
                ...
                if (!User::isGuest() && !$ingroup && $toolgroups) {
                    $pop = '<p class="warning">'
                        . Lang::txt('COM_RESOURCES_TOOL_IS_RESTRICTED') . '</p>';
                    $html .= self::primaryButton(
                        'link_disabled',
                        '',
                        Lang::txt('COM_RESOURCES_LAUNCH_TOOL'),
                    ...
                    (isset($resource->revision) && $resource->toolpublished)
                    || !isset($resource->revision)
                ) {
                    // Dev or published tool
                    $pop = (User::isGuest())
                        ? '<p class="warning">'
                            . Lang::txt('COM_RESOURCES_TOOL_LOGIN_REQUIRED_TO_RUN') . '</p>'
                        : '';
                    $pop = ($resource->revision == 'dev')
                        ? '<p class="warning">'
                            . Lang::txt('COM_RESOURCES_TOOL_VERSION_UNDER_DEVELOPMENT') . '</p>'
                        : $pop;
                    $html .= self::primaryButton(
                        'launchtool',
                        $lurl,
                    ...
                    // Tool unpublished
                    $pop = '<p class="warning">'
                        . Lang::txt('COM_RESOURCES_TOOL_VERSION_UNPUBLISHED') . '</p>';
                    $html .= self::primaryButton(
                        'link_disabled',
                        '',
                        Lang::txt('COM_RESOURCES_LAUNCH_TOOL'),
            ...
            case 4:
                ...
                $playUrl = Route::url($resource->link() . '&task=play');
                $html .= self::primaryButton('', $playUrl, 'Start learning module');
                break;

            case 6:
                $mesg = Lang::txt('COM_RESOURCES_VIEW') . ' Course Lectures';
                $linkUrl = Route::url($resource->link()) . '#courselecture';
                $html .= self::primaryButton('download', $linkUrl, $mesg, '', $mesg, '');
                break;

            case 31:
                $mesg = Lang::txt('COM_RESOURCES_VIEW') . ' Series';
                $linkUrl = Route::url($resource->link()) . '#series';
                $html .= self::primaryButton('download', $linkUrl, $mesg, '', $mesg, '');
                break;

            case 2:
                $mesg = Lang::txt('COM_RESOURCES_VIEW') . ' Workshop ';
                $linkUrl = Route::url($resource->link()) . '#workshop';
                $html .= self::primaryButton('download', $linkUrl, $mesg, '', $mesg, '');
                break;
            ...
            default:
                ...
                switch ($rtLinkAction) {
                    ...
                    default:
                        ...
                        if (substr($firstChild->path, 0, 16) == 'https://doi.org/') {
                            ...
                            substr($firstChild->path, 0, 7) == 'http://'
                        ...
                        } elseif (substr($firstChild->path, 0, 1) == '/') {
                            $mesg  = Lang::txt('COM_RESOURCES_VIEW_LINK');
                        }
                        break;
                }
                ...
                if ($firstChild->access == 1 && User::isGuest()) {
                    ...
                    $loginMsg = Lang::txt('COM_RESOURCES_LOGIN_REQUIRED_TO_DOWNLOAD');
                    $pop = '<p class="warning">' . $loginMsg . '</p>' . "\n";
                    $loginUrl = Route::url('index.php?option=com_login&return=')
                        . base64_encode(Request::current());
                    $html .= self::primaryButton(
                        $class . ' disabled',
                        $loginUrl,
                        $mesg,
                    ...
                    if ($firstChild->access == 3) {
                        ...
                        $isGroupMember = in_array($resource->get('group_owner'), $usersgroups);
                        if (!$isGroupMember || count($common) <= 0) {
                            ...
                            if (User::isGuest()) {
                                ...
                                $groupListTxt = Lang::txt(
                                    'COM_RESOURCES_ERROR_MUST_BE_PART_OF_GROUP'
                                );
                                $html .= $groupListTxt . ' ' . implode(', ', $ghtml);
                            }
                            $html .= '</p>';
     ...
     * @return  string
     */
    public static function primaryButton(
        $class,
        $href,
        $msg,
        $xtra = '',
    ...
    public static function getFileAttribs($path, $base_path = '', $fsize = 0)
    {
        ...
        if (substr($path, 0, 16) == 'https://doi.org/') {
            ...
            substr($path, 0, 18) == 'https://vimeo.com/'
            || substr($path, 0, 25) == 'https://player.vimeo.com/'
        ) {
            $type = 'Vimeo';
            $fs = '';
        } elseif (
            substr($path, 0, 20) == 'https://youtube.com/'
            || substr($path, 0, 24) == 'https://www.youtube.com/'
            || substr($path, 0, 17) == 'https://youtu.be/'
        ) {
            $type = 'YouTube';
            $fs = '';
        } elseif (preg_match("/(?:https?:|mailto:|ftp:|gopher:|news:|file:)/", $path)) {
            $type = 'HTM';
            $fs = '';
