...
class Usage
{
    ...
    public function fetch($disp)
    {
        ...
        $sql = "SELECT * FROM `#__resource_stats` WHERE resid=" . $this->_resid
     ...
     * @return  string  HTML
     */
    public function displaySubstats()
    {
        ...
        $ratingClass = $this->getRatingClass($this->rating);
        $html .= '   <td><span class="avgrating' . $ratingClass . '"><span>'
            . $this->rating . ' out of 5 stars</span></span></td>' . "\n";
        $html .= '  </tr>' . "\n";
        $html .= '  <tr>' . "\n";
    ...
    public function valfmt($val)
    {
        if ($val != 'unavailable') {
            if ($val <= 60) {
                $val = ceil($val) . ' secs';
            } elseif ($val > 60 && $val <= 3600) {
                $val = ceil($val / 60) . ' mins';
            } elseif ($val > 3600 && $val <= 86400) {
