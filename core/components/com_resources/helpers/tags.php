...
class Tags extends Cloud
{
    ...
    public function getTagsWithObjects($id = 0, $type = 0, $tag = '')
    {
        ...
        $this->_db->setQuery(
            ...
            "WHERE o.tagid=t.id AND t.tag=" . $this->_db->quote($tag) .
            " AND o.tbl=" . $this->_db->quote($this->_scope)
        );
        ...
        $sql = "SELECT t.id, t.tag, t.raw_tag, r.id AS rid, 0 AS ucount, NULL AS rids
				...
				AND r.published=1
				AND r.standalone=1
				AND (r.publish_up IS NULL OR r.publish_up = '0000-00-00 00:00:00'
					OR r.publish_up <= " . $this->_db->quote($now) . ")
				AND (r.publish_down IS NULL OR r.publish_down = '0000-00-00 00:00:00'
					OR r.publish_down >= " . $this->_db->quote($now) . ") ";
        if ($type) {
        ...
        if (!\User::isGuest()) {
            ...
            if ($xgroups != '') {
                ...
                $sql .= "AND (r.access=0 OR r.access=1 OR r.access=3 OR (r.access=4 AND ";
                $sql .= "(r.group_owner IN ('" . $groups . "') ";
    ...
    public function getObjectsOnTag($tag = '', $id = 0, $type = 0, $sortby = 'title', $tag2 = '', $filterby = array())
    {
        ...
        if ($tag || $tag2) {
            ...
            switch ($sortby) {
                case 'users':
                    $query .= ", (SELECT rs.users FROM #__resource_stats AS rs ";
                    $query .= "WHERE rs.resid=C.id AND rs.period=14 ";
                    $query .= "ORDER BY rs.datetime DESC LIMIT 1) AS users ";
                    break;
                case 'jobs':
                    $query .= ", (SELECT rs.jobs FROM #__resource_stats AS rs ";
                    $query .= "WHERE rs.resid=C.id AND rs.period=14 ";
                    $query .= "ORDER BY rs.datetime DESC LIMIT 1) AS jobs ";
                    break;
            }
            $query .= "FROM #__resources AS C ";
            if ($id) {
                $query .= "INNER JOIN #__resource_assoc AS RA ON ";
                $query .= "(RA.child_id = C.id AND RA.parent_id=" . $id . ")";
            }
            if ($type == 7) {
                if (!empty($filterby)) {
        ...
        if ($type == 7) {
            $query .= " AND TV.toolname=C.alias AND TV.state=1 AND TV.revision = ";
            $query .= "(SELECT MAX(revision) FROM #__tool_version as TV ";
            $query .= "WHERE TV.toolname=C.alias AND TV.state=1 GROUP BY TV.toolid) ";
        }
        ...
        $query .= "AND (C.publish_up IS NULL OR C.publish_up = '0000-00-00 00:00:00' ";
        $query .= "OR C.publish_up <= " . $this->_db->quote($now) . ") ";
        ...
        if (!\User::isGuest()) {
            ...
            if ($xgroups != '') {
                ...
                $query .= "(C.access=0 OR C.access=1 OR C.access=3 OR (C.access=4 AND ";
                $query .= "(C.group_owner IN ('" . $groups . "') ";
                foreach ($usersgroups as $group) {
        ...
        if ($tag || $tag2) {
            if ($tag && !$tag2) {
                $query .= "AND RTA.objectid=C.id AND RTA.tbl='$this->_scope' ";
                $query .= "AND (TA.tag IN (" . $this->_db->quote($tag) . "))";
                $query .= " GROUP BY C.id HAVING uniques=1";
            } elseif ($tag2 && !$tag) {
                $query .= "AND RTA.objectid=C.id AND RTA.tbl='$this->_scope' ";
                $query .= "AND (TA.tag IN (" . $this->_db->quote($tag2) . "))";
                $query .= " GROUP BY C.id HAVING uniques=1";
            } elseif ($tag && $tag2) {
                $query .= "AND RTA.objectid=C.id AND RTA.tbl='$this->_scope' ";
                $query .= "AND (TA.tag IN (" . $this->_db->quote($tag) . ",";
                $query .= $this->_db->quote($tag2) . "))";
        ...
        if ($tags && count($tags) > 0) {
            ...
            foreach ($tags as $tag) {
                ...
                if ($showsizes == 1) {
                    ...
                    $tagUrl = Route::url(
                        'index.php?option=com_resources&task=browse&tag=' . implode(',', $lsst)
                    );
                    $tll[$tag->tag] = "\t" . '<li' . $class . '>'
                        . '<span data-size="' . round($size, 1) . 'em">'
                        . '<a class="tag' . ($tag->admin ? ' admin' : '') . '" rel="nofollow" '
                        . 'href="' . $tagUrl . '">' . stripslashes($tag->raw_tag) . '</a>'
                        . '</li>' . "\n";
                } else {
                    $tagUrl = urldecode(Route::url(
                        'index.php?option=com_resources&task=browse&tag=' . implode(',', $lsst)
                    ));
                    $tll[$tag->tag] = "\t" . '<li' . $class . '>'
                        . '<a class="tag' . ($tag->admin ? ' admin' : '') . '" rel="nofollow" '
