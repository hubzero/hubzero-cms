...
class DoiService extends Obj
{
    ...
    public function configs()
    {
        if (empty($this->_configs)) {
            ...
            $configs->xmlSchema = trim($params->get('doi_xmlschema', $defaultSchema), '/');
    ...
    public function on()
    {
        ...
        $hasServiceUrl = $this->_configs->serviceURL && $this->_configs->userPW;
        $isMode2 = $this->_configs->serviceSwitch == 2 && $hasServiceUrl;
        $isMode1 = $this->_configs->serviceSwitch == 1 && $hasServiceUrl && $this->_configs->prefix;
        if ($hasBase && ($isMode2 || $isMode1)) {
            return true;
        }
    ...
    public function mapData($pub = null)
    {
        ...
        $hasValidDate = isset($pub['datePublished'])
            && $pub['datePublished']
            && $pub['datePublished'] != '0000-00-00 00:00:00';
        $pubDate = $hasValidDate
    ...
    public function buildXml($doi = null)
    {
        ...
        $xmlfile .= '<resource xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" '
        ...
        if (!empty($authors) && is_array($authors)) {
            ...
            if ($this->get('creatorOrcid')) {
                $orcidUrl = 'http://orcid.org/' . $this->get('creatorOrcid');
                $xmlfile .= '	<nameIdentifier nameIdentifierScheme="ORCID">'
                    . $orcidUrl . '</nameIdentifier>';
            }
            $xmlfile .= '</creator>';
        }
        $xmlfile .= '</creators>';
        ...
        $xmlfile .= '<resourceType resourceTypeGeneral="' . $this->get('resourceType') . '">'
        ...
        if ($this->get('relatedDoi')) {
            ...
            $xmlfile .= '	<relatedIdentifier relatedIdentifierType="DOI" '
                . 'relationType="IsNewVersionOf">' . $this->get('relatedDoi')
                . '</relatedIdentifier>';
            $xmlfile .= '</relatedIdentifiers>';
        }
        if ($this->get('version')) {
    ...
    public function registerEZID($sendXml = false, $status = 'public')
    {
        ...
        if (($status == 'public') && ($sendXml == true)) {
            ...
            // Append XML
            $replacements = array(":" => "%3A", "%" => "%25", "\n" => "%0A", "\r" => "%0D");
            $input .= 'datacite: ' . strtr($xml, $replacements) . "\n";
