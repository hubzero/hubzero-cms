...
class Redirect extends SiteController
{
    private function extractBaseDomain($host)
    {
        ...
        } elseif (preg_match('/([a-z0-9\-]+)\.(?:[a-z]{2,63})$/i', $host, $matches)) {
            return $matches[1]; // e.g., google.com → google
        } elseif (preg_match('/([a-z0-9\-]+)\.[a-z0-9\-]+\.[a-z]{2,63}$/i', $host, $matches)) {
    ...
    public function displayTask()
    {
        ...
        if (!$host || !isset($host['host'])) {
            ...
            App::redirect($url);
        } elseif (strpos($host['host'], $_SERVER['HTTP_HOST']) !== false && strpos($host['host'], "proxy") === false) {
            App::redirect($url);
            ...
            if ($params) {
                $delayEnabled = isset($params["delay_enabled"]) && $params["delay_enabled"] == "ENABLED";
                if ($delayEnabled && isset($params["delay_seconds"])) {
