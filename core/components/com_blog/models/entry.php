...
class Entry extends Relational implements \Hubzero\Search\Searchable
{
    ...
    public function setup()
    {
        $this->addRule('publish_down', function ($data) {
            ...
            return $data['publish_down'] >= $data['publish_up']
                ? false
                : Lang::txt('The entry cannot end before it begins');
        });
    }
    ...
    public function visibility($as = 'text')
    {
        ...
        if ($as == 'text') {
            if (!isset($access)) {
                $access = \Hubzero\Facades\Html::access('assetgroups');
            }
            foreach ($access as $a) {
    ...
    public function transformContent()
    {
        ...
        if (!isset($this->$property)) {
            ...
            $this->$property = \Hubzero\Facades\Html::content('prepare', $this->get($field, ''), $params);
        }
    ...
    public function access($action = 'view')
    {
        ...
        if (!$this->params->get('access-check-done', false)) {
            ...
            if (User::isGuest()) {
                ...
                if (
                    !$this->params->get('access-admin-entry')
