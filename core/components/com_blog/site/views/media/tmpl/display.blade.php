...
<div id="attachments" class="space-y-3 pb-2">
  <form action="{{ $uploadUrl }}"
    ...
    <label id="upload-picker"
           class="flex flex-col items-center justify-center gap-1 px-3 py-3
                  ...
                  cursor-pointer hover:border-primary/40 hover:bg-muted
                  transition-colors">
      ...
      <span class="text-xs text-faint-foreground">File uploads automatically after selection</span>
  ...
  <div id="file-detail" class="hidden rounded-lg mt-4 overflow-hidden
    ...
    <div class="bg-muted px-3 py-3 border-b border-primary/15">
      <p class="text-sm font-semibold break-all leading-tight" id="detail-filename"></p>
      <div class="flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-subtle-foreground mt-1.5">
    ...
    <div class="grid grid-cols-3 gap-1 px-2 py-2 bg-base-100">
      <button type="button" id="btn-insert"
              class="btn btn-xs btn-ghost border-default">
        <span>Insert</span>
      </button>
      ...
      <button type="button" id="btn-delete"
              class="btn btn-xs btn-ghost border-default text-error">
        <span>Delete</span>
      </button>
    </div>
    {{-- Macro preview --}}
    <div class="px-3 py-2 bg-muted border-t border-default">
      <p class="text-[0.65rem] text-faint-foreground mb-1">Inserts as:</p>
      <code id="detail-ref"
            class="block text-[0.65rem] bg-accent-surface rounded px-1.5 py-1
                   break-all leading-snug text-muted-foreground"></code>
    </div>
  </div>
  ...
  {{-- Usage hint --}}
  <div class="text-[0.7rem] text-faint-foreground mt-3 px-1 leading-snug space-y-1.5">
    ...
    <div class="mt-1">
      ...
      <code class="block bg-muted px-1 py-0.5 rounded">&#91;[Image(file.jpg)]]</code>
      <p class="mb-0.5 mt-1.5">or for other files:</p>
      <code class="block bg-muted px-1 py-0.5 rounded">&#91;[File(doc.pdf)]]</code>
    </div>
  </div>
</div>
