...
class Entries extends SiteController
{
    ...
    public function editTask($entry = null)
    {
        if (User::isGuest()) {
            $rtrn = Request::getString(
                'REQUEST_URI',
                Route::url('index.php?option=' . $this->_option . '&task=' . $this->_task, false, true),
                'server'
            );
            App::redirect(
        ...
        if (
            !$this->config->get('access-create-entry')
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt(
                    'COM_BLOG_ACTIVITY_ENTRY_' . ($fields['id'] ? 'UPDATED' : 'CREATED'),
                    '<a href="' . Route::url($row->link()) . '">' . $row->get('title') . '</a>'
                ),
                'details'     => array(
                    'title' => $row->get('title'),
    ...
    public function deleteTask()
    {
        if (User::isGuest()) {
            $rtrn = Request::getString(
                'REQUEST_URI',
                Route::url('index.php?option=' . $this->_option, false, true),
                'server'
            );
            App::redirect(
                Route::url('index.php?option=com_users&view=login&return=' . base64_encode($rtrn)),
        ...
        if (
            !$this->config->get('access-delete-entry')
            && !$this->config->get('access-manage-entry')
        ) {
            App::abort(403, Lang::txt('COM_BLOG_NOT_AUTH'));
        }
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt(
                    ...
                    '<a href="' . Route::url($entry->link()) . '">' . $entry->get('title') . '</a>'
                ),
                'details'     => array(
    ...
    public function savecommentTask()
    {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt(
                    'COM_BLOG_ACTIVITY_COMMENT_' . ($data['id'] ? 'UPDATED' : 'CREATED'),
                    $comment->get('id'),
                    '<a href="' . Route::url($entry->link() . '#c' . $comment->get('id')) . '">' .
                        $entry->get('title') . '</a>'
                ),
                'details'     => array(
                    'title'    => $entry->get('title'),
    ...
    public function deletecommentTask()
    {
        ...
        if (!$id) {
            $url = 'index.php?option=' . $this->_option . '&year=' . $year .
                '&month=' . $month . '&alias=' . $alias;
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope_id'    => $comment->get('id'),
                'description' => Lang::txt(
                    'COM_BLOG_ACTIVITY_COMMENT_DELETED',
                    $comment->get('id'),
                    '<a href="' . Route::url($entry->link()) . '">' . $entry->get('title') . '</a>'
                ),
                'details'     => array(
    ...
    public function commentsTask()
    {
        ...
        $doc->description = Lang::txt(
            'COM_BLOG_COMMENTS_RSS_DESCRIPTION',
            Config::get('sitename'),
            stripslashes($this->entry->get('title'))
        );
        $doc->copyright   = Lang::txt('COM_BLOG_RSS_COPYRIGHT', date("Y"), Config::get('sitename'));
    ...
    private function _comment(&$doc, $row)
    {
        // Load individual item creator class
        $item = new \Hubzero\Document\Type\Feed\Item();
        $item->title = Lang::txt('Comment #%s', $row->get('id')) . ' @ ' .
