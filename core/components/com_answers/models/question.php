...
class Question extends Relational
{
     ...
     * @var  integer
     */
    public const ANSWERS_STATE_OPEN   = 0;
     ...
     * @var  integer
     */
    public const ANSWERS_STATE_CLOSED = 1;
     ...
     * @var  integer
     */
    public const ANSWERS_STATE_DELETE = 2;
     ...
     * @var  integer
     */
    public const ANSWERS_STATE_REPORTED = 3;
    ...
    public function reward($val = 'reward')
    {
        ...
        if ($this->get('reward', -1) == 1) {
            ...
            $earnings = ($this->get('asker_earnings') + $this->get('reward', 0)) . ' &mdash; ' . $this->get('maxaward');
            $this->set('answer_earnings', $earnings);
        }
    ...
    {
        // Remove comments
        foreach ($this->responses()->rows() as $response) {
            if (!$response->destroy()) {
    ...
    {
        $config = \Hubzero\Facades\Component::params('com_answers');

        if ($key) {
            if ($key == 'banking' && $config->get('banking', -1) == -1) {
                $config->set('banking', \Hubzero\Facades\Component::params('com_members')->get('bankAccounts'));
            }
