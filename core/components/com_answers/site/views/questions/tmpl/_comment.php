    ...
    <li class="comment <?php echo $cls; ?>" id="<?php echo ($this->depth == 1 ? 'a' : 'c') . $this->comment->get('id'); ?>">
        ...
        <div class="comment-content">
            ...
            <p class="comment-title">
                <strong>
                    ...
                    if (!$this->comment->get('anonymous')) {
                        ...
                        if (in_array($this->comment->creator->get('access'), User::getAuthorisedViewLevels()) && !$this->comment->creator->get('block') && $this->comment->creator->get('approved')) {
            ...
            <div class="comment-body">
                ...
                if ($componentPath) {
                    ...
                    $comment = preg_replace('#<a\s[^>]*href="([^"]*)"[^>]*?>(.*?)</a>#is', "<a href='$1' rel='nofollow'>$2</a>", $comment);
                }
        ...
        <?php if ($this->depth < $this->config->get('comments_depth', 3)) { ?>
            <div class="addcomment comment-add<?php if (Request::getInt('reply', 0) != $this->comment->get('id')) {
                echo ' hide';
                                              } ?>" id="comment-form<?php echo $this->depth . $this->comment->get('item_type') . $this->comment->get('id'); ?>">
