...
class Questions extends SiteController
{
    ...
    {
        $this->config->set('banking', \Hubzero\Facades\Component::params('com_members')->get('bankAccounts'));
    ...
    public function savereplyTask()
    {
        ...
        $subject = Config::get('sitename') . ' ' . Lang::txt('COM_ANSWERS_ANSWERS') . ', '
            . Lang::txt('COM_ANSWERS_QUESTION') . ' #' . $question->get('id') . ' '
            . Lang::txt('COM_ANSWERS_RESPONSE');
        $message = array();
        ...
        foreach ($notify as $authorid) {
            if (!in_array($authorid, $receivers) && $question->get('email')) {
                $params = array(
                    'answers_reply_comment', $subject, $message, $from, array($authorid), $this->_option
                );
        ...
        // admin emails
        if (!empty($receivers)) {
            $params = array('new_answer_admin', $subject, $message, $from, $receivers, $this->_option);
            if (!Event::trigger('xmessage.onSendMessage', $params)) {
                $this->setError(Lang::txt('COM_ANSWERS_MESSAGE_FAILED'));
            }
        }
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt(
                    'COM_ANSWERS_ACTIVITY_COMMENT_' . ($comment['id'] ? 'UPDATED' : 'CREATED'),
                    '<a href="' . Route::url($question->link()) . '">' . $question->get('subject') . '</a>'
                ),
                'details'     => array(
        ...
        if (!$this->getError()) {
            ...
            Event::trigger('system.logActivity', [
                'activity' => [
                    ...
                    'scope_id'    => $id,
                    'description' => Lang::txt(
                        'COM_ANSWERS_ACTIVITY_VOTED_' . ($vote == 'yes' ? 'UP' : 'DOWN'),
                        '<a href="' . Route::url($row->link()) . '">' . $txt . '</a>'
                    ),
                    'details'     => array(
                        'title'       => $question->get('title'),
    ...
    public function newTask($question = null)
    {
        ...
        $r_u = \Hubzero\Facades\Component::params('com_answers')->get('restrict_users');
        if ($r_u == "active") {
            $r_d = \Hubzero\Facades\Component::params('com_answers')->get('restrict_days');
            $r_t = new \DateTime();
    ...
    public function saveqTask()
    {
        ...
        if (!empty($recipients)) {
            ...
            // Build the message subject
            $subject = Lang::txt('COM_ANSWERS_ANSWERS') . ', '
                . Lang::txt('new question about content you author or manage');
            ...
            $params = array(
                'new_question_admin', $subject, $message, $from, $recipients, $this->_option
            );
            if (!Event::trigger('xmessage.onSendMessage', $params)) {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'anonymous'   => $row->get('anonymous', 0),
                'description' => Lang::txt(
                    'COM_ANSWERS_ACTIVITY_QUESTION_' . ($fields['id'] ? 'UPDATED' : 'CREATED'),
                    '<a href="' . Route::url($row->link()) . '">' . $row->get('subject') . '</a>'
                ),
                'details'     => array(
                    'title' => $row->get('title'),
                    'url'   => $row->link()
    ...
    public function deleteqTask()
    {
        ...
        if ($reward && $this->config->get('banking')) {
            ...
            if ($responses->count())
            {
                ...
                // Build the message subject
                $subject = Config::get('sitename') . ' ' . Lang::txt('COM_ANSWERS_ANSWERS') . ', '
                    . Lang::txt('COM_ANSWERS_QUESTION') . ' #' . $id . ' '
                    . Lang::txt('COM_ANSWERS_WAS_REMOVED');
                ...
                // Send the message
                $params = array(
                    'answers_question_deleted', $subject, $message, $from, $users, $this->_option
                );
                if (!Event::trigger('xmessage.onSendMessage', $params)) {
                    $this->setError(Lang::txt('COM_ANSWERS_MESSAGE_FAILED'));
                }
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt(
                    'COM_ANSWERS_ACTIVITY_QUESTION_DELETED',
                    '<a href="' . Route::url($question->link()) . '">' . $question->get('subject') . '</a>'
                ),
                'details'     => array(
                    'title' => $question->get('title'),
    ...
    public function saveaTask()
    {
        ...
        if (!in_array($authorid, $receivers) && $question->get('email')) {
            ...
            $params = array($messageType, $subject, $message, $from, array($authorid), $this->_option);
            if (!Event::trigger('xmessage.onSendMessage', $params)) {
                $this->setError(Lang::txt('COM_ANSWERS_MESSAGE_FAILED'));
            }
        }
        ...
        $link = '<a href="' . Route::url($question->link() . '#a' . $row->get('id')) . '">'
            . $question->get('subject') . '</a>';
        Event::trigger('system.logActivity', [
            'activity' => [
                'action'      => ($response['id'] ? 'updated' : 'created'),
                'scope'       => 'question.answer',
                'scope_id'    => $row->get('id'),
                'description' => Lang::txt('COM_ANSWERS_ACTIVITY_ANSWER_SUBMITTED', $link),
                'details'     => array(
                    'title'       => $question->get('title'),
                    'question_id' => $question->get('id'),
                    'url'         => $question->link()
        ...
        if (!$this->getError()) {
            ...
            Event::trigger('system.logActivity', [
                'activity' => [
                    ...
                    'scope_id'    => $rid,
                    'description' => Lang::txt(
                        'COM_ANSWERS_ACTIVITY_ANSWER_ACCEPTED',
                        $rid,
                        '<a href="' . Route::url($question->link() . '#a' . $rid) . '">'
                            . $question->get('subject') . '</a>'
                    ),
                    'details'     => array(
                        'title'       => $question->get('title'),
    ...
    public function latestTask()
    {
        ...
        //get module params
        $params = \Hubzero\Facades\Module::params($mid);
        ...
        $doc->copyright   = Lang::txt(
            'COM_ANSWERS_LATEST_QUESTIONS_RSS_COPYRIGHT',
            gmdate("Y"),
            Config::get('sitename')
        );
