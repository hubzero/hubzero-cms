...
class Articles extends SiteController
{
    ...
    public function displayTask()
    {
        ...
        if ($access = $filters['access']) {
            ...
            if ($item->catid == 0 || $item->category_access === null) {
                ...
                $hasAccess = in_array($item->access, $groups)
                    && in_array($item->category_access, $groups);
                $item->params->set('access-view', $hasAccess);
            }
        }
        ...
        if ($active) {
            ...
            // If the current view is the active item and an article view for this article,
        ...
        $results = Event::trigger(
            ...
            array('com_content.article', &$item, &$params, $offset)
        );
        ...
        if (!$params->get('intro_only') && $offset == 0) {
            // [!] HUBZERO - (zooley) Removing hit counter as it can contribute to
            // performance issues. Need a better way of doing this.
            //$data->hit();
        }
        ...
        $isContentArticle = $menu && $menu->query['option'] == 'com_content'
            && $menu->query['view'] == 'article' && $id == $item->id;
        if ($menu && !$isContentArticle) {
            ...
            $notContentMenu = $menu->query['option'] != 'com_content';
            while (
                $category
                && ($notContentMenu || $menu->query['view'] == 'article' || $id != $category->id)
                && $category->id > 1
            ) {
                $path[] = array(
    ...
    public function archiveTask()
    {
        ...
        $stateKey = 'com_content.archive.list' . $itemid . '.limit';
        $limit  = Request::getState($stateKey, 'limit', $params->get('display_num'), 'uint');
        ...
        $case_when .= 'CONCAT_WS(\':\', ' . implode(', ', array('a.id', 'a.alias')) . ')';
        $case_when .= ' ELSE ';
        $case_when .= 'a.id END';
        ...
        $case_when .= ' THEN ';
        $case_when .= 'CONCAT_WS(\':\', ' . implode(', ', array('c.id', 'c.alias')) . ')';
        $case_when .= ' ELSE ';
        $case_when .= 'c.id END';
        ...
        foreach ($items as $item) {
            ...
            $item->catslug = ($item->category_alias)
                ? ($item->catid . ':' . $item->category_alias) : $item->catid;
            $item->parent_slug = ($item->parent_alias)
                ? ($item->parent_id . ':' . $item->parent_alias) : $item->parent_id;
        }
    ...
    public function featuredTask()
    {
        ...
        if (
            ...
            $filters['published'] = array(
                Article::STATE_UNPUBLISHED,
                Article::STATE_PUBLISHED,
                Article::STATE_ARCHIVED
            );
        }
        ...
        foreach ($items as $i => & $item) {
            ...
            $item->catslug = ($item->category_alias)
                ...
                : $item->catid;
            $item->parent_slug = ($item->parent_alias)
                ? ($item->parent_id . ':' . $item->parent_alias)
                : $item->parent_id;
            ...
            $eventArgs = array('com_content.featured', &$item, &$params, 0);
            $results = Event::trigger('content.onContentPrepare', $eventArgs);

            // Old plugins: Use processed text as introtext
            $item->introtext = $item->text;

            $eventArgs = array('com_content.featured', &$item, &$item->params, 0);
            $results = Event::trigger('content.onContentAfterTitle', $eventArgs);
            $item->event->afterDisplayTitle = trim(implode("\n", $results));

            $results = Event::trigger('content.onContentBeforeDisplay', $eventArgs);
            $item->event->beforeDisplayContent = trim(implode("\n", $results));

            $results = Event::trigger('content.onContentAfterDisplay', $eventArgs);
            $item->event->afterDisplayContent = trim(implode("\n", $results));
        }
    ...
    public function categoriesTask()
    {
        ...
        $options = array();
        $options['countItems'] = $mparams->get('show_cat_num_articles_cat', 1)
            || !$mparams->get('show_empty_categories_cat', 0);
    ...
    public function categoryTask()
    {
        ...
        $stateKey = 'com_content.category.list.' . $itemid . '.filter_order';
        $orderCol = Request::getState($stateKey, 'filter_order', '', 'string');
        $filter_fields = array(
            'id', 'a.id',
            'title', 'a.title',
            'alias', 'a.alias',
        ...
        $stateKey = 'com_content.category.list.' . $itemid . '.filter_order_Dir';
        $listOrder = Request::getState($stateKey, 'filter_order_Dir', '', 'cmd');
        if (!in_array(strtoupper($listOrder), array('ASC', 'DESC', ''))) {
            $listOrder = 'ASC';
        }
        $filters['direction'] = $listOrder;
        ...
        if (Request::getCmd('layout') == 'blog' || $params->get('layout_type') == 'blog') {
            ...
            $stateKey = 'com_content.category.list.' . $itemid . '.limit';
            $limit = Request::getState($stateKey, 'limit', $params->get('display_num'), 'uint');
        }
        ...
        if (count($children)) {
            if ($params->get('orderby_pri') == 'alpha' || $params->get('orderby_pri') == 'ralpha') {
                $sortDir = ($params->get('orderby_pri') == 'alpha') ? 1 : -1;
                \Hubzero\Utility\Arr::sortObjects($children, 'title', $sortDir);
            }
        }
        $parent = false;
        ...
        for ($i = 0, $n = count($items); $i < $n; $i++) {
            ...
            $eventArgs = array('com_content.category', &$item, &$item->params, 0);
            $results = Event::trigger('content.onContentPrepare', $eventArgs);
        ...
        $menuHasCategory = $menu && strpos($menu->link, 'view=category') !== false;
        $menuHasId = $menu && strpos($menu->link, '&id=' . (string) $category->id) !== false;

        if ((!$menu) || !$menuHasCategory || !$menuHasId) {
            ...
            // We need to set the layout from the query in case this is an alternative menu item
        ...
        $notContentMenu = !$menu || $menu->query['option'] != 'com_content';
        $isArticleView = $menu && $menu->query['view'] == 'article';
        if ($menu && ($notContentMenu || $isArticleView || $id != $category->id)) {
            $path = array(array(
                'title' => $category->title,
                'link'  => ''
            ));
            ...
            while (
                ($notContentMenu || $isArticleView || $id != $parent->id)
                && $parent->id > 1
            ) {
                $path[] = array(
                    'title' => $parent->title,
    ...
    public function editTask()
    {
        ...
        if ($id) {
            ...
            // New item.
            if ($catId) {
                $asset = 'com_content.category.' . $catId;
                $item->params->set('access-change', User::authorise('core.edit.state', $asset));
                $item->catid = $catId;
            } else {
        ...
        if (empty($item->id)) {
            ...
            $hasCategories = count(User::getAuthorisedCategories('com_content', 'core.create'));
            $authorised = $canCreate || $hasCategories;
        } else {
