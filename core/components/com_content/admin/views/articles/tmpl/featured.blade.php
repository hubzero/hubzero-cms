{{--
  Featured Articles — Admin list view
...
@php
  ...
  $canDo = \Components\Content\Admin\Helpers\Permissions::getActions($filters['category_id'] ?? 0);

  $sort    = $filters['sort'] ?? 'ordering';
  $sortDir = $filters['sort_Dir'] ?? 'ASC';
  $canOrder  = User::authorise('core.edit.state', 'com_content.article');
  $saveOrder = ($sort === 'fp.ordering' || $sort === 'ordering');
  $userId    = User::get('id');
    ...
    <table class="admin-table">
      <thead>
        <tr>
          ...
          <th>{!! Html::grid('sort', 'JSTATUS', 'a.state', $sortDir, $sort) !!}</th>
          <th class="priority-2">{!! Html::grid('sort', 'JCATEGORY', 'a.catid', $sortDir, $sort) !!}</th>
          <th class="priority-3">{!! Html::grid('sort', 'JGRID_HEADING_ORDERING', 'fp.ordering', $sortDir, $sort) !!}</th>
          <th class="priority-4">{!! Html::grid('sort', 'JGRID_HEADING_ACCESS', 'a.access', $sortDir, $sort) !!}</th>
          <th class="priority-5">{!! Html::grid('sort', 'JGRID_HEADING_CREATED_BY', 'a.created_by', $sortDir, $sort) !!}</th>
          <th class="priority-5">{!! Html::grid('sort', 'JDATE', 'a.created', $sortDir, $sort) !!}</th>
          <th class="priority-6 text-right">{!! Html::grid('sort', 'JGRID_HEADING_ID', 'a.id', $sortDir, $sort) !!}</th>
        </tr>
      </thead>
      <tfoot>
        <tr>
      ...
      <tbody>
        @foreach($items as $i => $item)
          @php
            ...
            $catTitle = $item->get('category_title') ?: Lang::txt('JNONE');
            $accessTitle = $item->get('access_level') ?: '';
          ...
          <tr>
            ...
            <td class="priority-2">{{ $catTitle }}</td>
            <td class="priority-3">
            ...
            <td class="priority-4">{{ $accessTitle }}</td>
            <td class="priority-5">
            ...
            <td class="priority-6 text-right">{{ (int) $item->id }}</td>
          </tr>
        @endforeach
      </tbody>
    </table>
