...
class Assets extends AdminController
{
    ...
    public function saveTask()
    {
        ...
        if ($lid != $row->get('id')) {
            ...
            $path = PATH_APP . DS . $uploadpath . DS . $fields['course_id'];
        ...
        {
            $uploadpath = trim($this->config->get('uploadpath', '/site/courses'), DS);
            $path = PATH_APP . DS . $uploadpath . DS . $fields['course_id'] . DS . $row->id;
    ...
    public function reorderTask()
    {
        ...
        $where = "scope=" . $this->database->Quote($scope)
            . " AND scope_id=" . $this->database->Quote(intval($scope_id));
        if (!$tbl->move($move, $where)) {
            echo $tbl->getError();
            return;
        }
