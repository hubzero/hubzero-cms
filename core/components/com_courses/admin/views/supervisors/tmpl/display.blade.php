{{--
  Courses — Supervisors iframe view (tmpl=component)
...
@php
  ...
  $managers  = $model->members(array(
      'student'     => 0,
      'course_id'   => $model->get('course_id'),
      'offering_id' => $model->get('id'),
      'section_id'  => $model->section()->get('id'),
  ), true);
...
<div id="groups" class="p-2">
  ...
  <form action="{{ $routeUrl }}" method="post" class="mb-4">
    <div class="flex items-end gap-2 flex-wrap">
      ...
      <div>
        <label for="add-role" class="sr-only">{{ Lang::txt('COM_COURSES_COL_ROLE') }}</label>
        <select name="role" id="add-role" class="select select-bordered select-sm">
      ...
      <div>
        ...
        <input type="hidden" name="section" value="{{ $model->section()->get('id') }}" />
        <input type="hidden" name="offering" value="{{ $model->get('id') }}" />
      ...
      <tbody>
        @foreach($managers as $i => $manager)
          @php
            ...
            $memberUrl = Route::url(
                'index.php?option=' . $option . '&controller=members&task=edit&id=' . $u->get('id'), false
            );
          @endphp
          <tr>
            <td class="w-8">
              ...
              <input type="hidden"
                     ...
                     value="{{ $model->get('course_id') }}" />
              <input type="hidden"
                     name="entries[{{ $i }}][offering_id]"
                     value="{{ $model->get('id') }}" />
              <input type="hidden"
                     name="entries[{{ $i }}][section_id]"
                     value="{{ $model->section()->get('id') }}" />
              <input type="hidden"
            ...
            <td>
              <select name="entries[{{ $i }}][role_id]"
                ...
                @foreach($offerings as $offering)
                  ...
                  @endphp
                  @if($oroles && count($oroles))
                    <optgroup label="{{ Lang::txt('COM_COURSES_OFFERING') }}: {{ $offering->get('title') }}">
                      @foreach($oroles as $role)
                        <option value="{{ $role->id }}"
                                @selected($manager->get('role_id') == $role->id)>
                          {{ $role->title }}
