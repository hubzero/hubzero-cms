...
class Section extends Base
{
     ...
     */
    public static function &getInstance($oid = null, $offering_id = null)
    {
        static $instances;
    ...
    {
        if (!$id) {
            $id = \Hubzero\Facades\User::get('id');
        }
        ...
        foreach ($data as $result) {
            ...
            if (!$model->exists()) {
                $this->setError(Lang::txt(
                    'Entry for user #%s, course #%s, offering #%s, section #%s not found.',
                    $user_id,
                    $this->get('course_id'),
                    $this->get('offering_id'),
                    $this->get('id')
  ...
  */
    ...
    public function logo($rtrn = '')
    {
        ...
        $path = $uploadPath . '/' . $this->get('course_id') . '/sections/' . $this->get('id');
