...
class Tool extends Content
{
     ...
     * Class info
     *
     * Action message - what the user will see if presented with multiple
    ...
    public function create()
    {
        ...
        if (isset($_FILES['files'])) {
            // @FIXME: should these come from the global settings,
            // or should they be courses specific
            // Get config
            ...
            for ($i = 0; $i < count($_FILES['files']['name']); $i++) {
                ...
                if ($size > $sizeLimit) {
                    ...
                    $max = preg_replace($pattern, '$1', $formatted);
                    return array(
                        'error' => "File is too large. Max file upload size is $max"
                    );
                }
                ...
                $move = move_uploaded_file($tmpFileName, $target_path);
                if (!$move) {
        ...
        foreach ($toAttach as $identifier) {
            ...
            if (is_file($filePath)) {
                ...
                if (Filesystem::copy($filePath, $targetPath)) {
                    $slashPos = strrpos($identifier, '/');
                    $lastPos = $slashPos ? $slashPos + 1 : 0;
    ...
    private function checkForRemoteConnection($identifier)
    {
        ...
        if (preg_match('/^([0-9]*):\/\//', $identifier, $matches)) {
            if (isset($matches[1])) {
                ...
                */
                // It doesn't have to be local (iData storage):
                // https://mygeohub.org/support/ticket/1536#c9499
                return $conFile;
            }
        }
        return false;
