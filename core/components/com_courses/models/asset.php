...
class Asset extends Base
{
    ...
    public function render($course = null, $option = 'com_courses')
    {
        ...
        $tmplPath = PATH_CORE . DS . 'components' . DS . 'com_courses' . DS . 'site'
            . DS . 'views' . DS . 'assets' . DS . 'tmpl' . DS;
        if (file_exists($tmplPath . $type . '_' . $subtype . '.php')) {
            $layout = $type . '_' . $subtype;
        } elseif (file_exists($tmplPath . $type . '.php')) {
            $layout = $type;
        }
    ...
    public function duplicate($courseId)
    {
        ...
        if ((($fileType == 'video') && ($fileSubtype == 'video')) || ($fileType == 'file')) {
            ...
            $basePath = PATH_APP . DS . trim($cconfig->get('uploadpath', '/site/courses'), DS) . DS;
            $originalDirectory = $basePath . $originalCourseId . DS . $originalId . DS;
