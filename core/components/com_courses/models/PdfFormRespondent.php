...
class PdfFormRespondent
{
    ...
    public function __construct($depId, $member_id, $attempt = 1)
    {
        ...
        if (($res = $dbh->loadAssoc())) {
            ...
            $dbh->setQuery(
                'INSERT INTO #__courses_form_respondents(deployment_id, member_id, attempt) '
                . 'VALUES ' . $insertValues
            );
            $dbh->query();
    ...
    public function saveAnswers($answers)
    {
        ...
        foreach ($questions as $question) {
            ...
            $dbh->setQuery(
                'INSERT INTO #__courses_form_responses(respondent_id, question_id, answer_id) '
                . 'VALUES (' . $this->id . ', ' . $question->id . ', ' . (int)$answer . ')'
            );
            $dbh->query();
        }
    ...
    public function getAnswers()
    {
        ...
        $dbh->setQuery(
            ...
            . 'INNER JOIN #__courses_form_questions pfq ON pfq.id = pfr.question_id '
            . 'INNER JOIN #__courses_form_answers pfa '
            . 'ON pfr.question_id = pfa.question_id AND pfa.correct '
            . 'WHERE pfr.respondent_id = ' . $this->id
        );
        ...
        $total = $rv['summary']['total'];
        $correct = $rv['summary']['correct'];
        $rv['summary']['score'] = ($total > 0) ? number_format($correct * 100 / $total, 1) : 0;
    ...
    public function saveProgress($qid, $aid)
    {
        ...
        $dbh->setQuery(
            ...
            . '(respondent_id, question_id, answer_id, submitted) VALUES ' . $insertValues
        );
        $dbh->query();
    ...
    public function getCompletedAttempts()
    {
        ...
        $query .= ' AND `member_id` = ' . $dbh->quote($this->member_id)
            . ' AND `finished` IS NOT NULL ORDER BY `attempt` ASC';
        $dbh->setQuery($query);
