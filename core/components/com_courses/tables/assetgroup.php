...
class AssetGroup extends Table
{
    ...
    {
        $sql = "SELECT ordering from $this->_tbl WHERE `unit_id`=" . $this->_db->quote(intval($unit_id))
            . " AND `parent`=" . $this->_db->quote(intval($parent)) . " ORDER BY ordering DESC LIMIT 1";
    ...
    public function count($filters = array())
    {
        ...
        $query .= $this->buildQuery($filters['w']);
    ...
    /*public function delete($oid=null)
    {
        ...
        // Delete sub groups
        $query = "DELETE FROM #__courses_offering_section_dates WHERE scope_id = "
            . $this->_db->quote($this->$k) . " ANd scope=" . $this->_db->quote('asset_group');
        $this->_db->setQuery($query);
        if (!$this->_db->query())
        {
