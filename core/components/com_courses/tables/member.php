...
class Member extends Table
{
    ...
    public function loadBySection($uid = null, $oid = null)
    {
        ...
        $query = "SELECT * FROM $this->_tbl WHERE `user_id`=" . $this->_db->quote((int) $uid)
            . " AND `section_id`=" . $this->_db->quote((int) $oid) . " LIMIT 1";
    ...
    public function loadByOffering($uid = null, $oid = null)
    {
        ...
        $query = "SELECT * FROM $this->_tbl WHERE `user_id`=" . $this->_db->quote((int) $uid)
            . " AND `offering_id`=" . $this->_db->quote((int) $oid) . " LIMIT 1";
    ...
    /*public function save()
    {
        $this->_db->setQuery("UPDATE $this->_tbl
            SET `role_id`=" . $this->_db->quote($this->role_id)
            . ", `permissions`=" . $this->_db->quote($this->permissions) . "
    ...
    private function buildQuery($filters = array())
    {
        ...
        if (isset($filters['search']) && $filters['search']) {
            $q  = "(LOWER(u.name) LIKE " . $this->_db->quote('%' . strtolower($filters['search']) . '%')
    ...
    {
        $query  = "SELECT m.*, u.name, u.email, u.username, r.alias AS role_alias, r.title AS role_title, "
            . "r.permissions AS role_permissions ";
        /*if (isset($filters['offering_id']))
        {
            $query .= "(SELECT cm.user_id FROM #__courses_managers AS cm "
                . "JOIN #__courses_offerings AS co ON cm.course_id=co.course_id "
                . "WHERE cm.user_id=m.user_id AND co.id="
                . $this->_db->quote(intval($filters['offering_id'])) . ")";
        }
        else
        {
            $query .= "null";
        ...
        $hasStart = isset($filters['start']) && is_numeric($filters['start']) && $filters['start'] >= 0;
        if ($hasStart && !empty($filters['limit'])) {
