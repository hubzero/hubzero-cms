...
class Media extends SiteController
{
    ...
    public function trackingTask()
    {
        ...
        if ($trackingInformation->isNew()) {
            ...
            if ($time_viewed < 10 && $time_viewed > 0) {
                $totalViewingTime = $trackingInformation->get('total_viewing_time') + $time_viewed;
                $trackingInformation->set('total_viewing_time', $totalViewingTime);
            }
        ...
        if ($event == 'start' || $trackingInformationDetailed->isNew()) {
            ...
            // Check to see if we need to set a new farthest position
            $currentPos = $trackingInformationDetailed->get('current_position');
            $farthestPos = $trackingInformationDetailed->get('farthest_position');
    ...
    private function ajaxuploadTask()
    {
        ...
        $uploadDirectory = PATH_APP . DS . $uploadPath . DS . $listdir . DS;
    ...
    public function deletefolderTask()
    {
        ...
        $uploadPath = trim($this->config->get('uploadpath', '/site/courses'), DS);
        $del_folder = DS . $uploadPath . DS . trim($listdir, DS) . DS . ltrim($folder, DS);
