...
class Course extends SiteController
{
    ...
    {
        $url = 'index.php?option=' . $this->_option . '&gid=' . $this->course->get('id')
            . '&task=' . $this->_task;
        $return = base64_encode(Route::url($url, false, true));
    ...
    public function savepageTask()
    {
        ...
        if (!$model->bind($page)) {
            // Redirect back to the course page
            $action = ($model->get('id') ? 'addpage' : 'editpage');
            $url = $course->link() . '&active=' . $model->get('url') . '&action=' . $action;
            App::redirect(
                Route::url($url),
                $model->getError(),
                'error'
    ...
    public function courseavailabilityTask($course = null)
    {
        ...
        if (($course == 'new' || $course == 'browse') || !$this->validCn($course) || $c->exists()) {
            $availability = false;
        } else {
            $availability = true;
        }
    ...
    public function downloadTask()
    {
        ...
        $filename = \Hubzero\Facades\Filesystem::clean($filename);
        $filename = str_replace(' ', '_', $filename);

        // Get the configured upload path
        $base_path = DS . trim($this->config->get('filepath', '/site/courses'), DS)
            . DS . $this->course->get('id') . DS . 'pagefiles';
