...
if (!$no_html && $tmpl != 'component') :
    ...
    if ($logo = $this->course->offering()->section()->logo('url')) {
        $src = $logo;
    } elseif ($logo = $this->course->offering()->logo('url')) {
        $src = $logo;
    }
    ?>
    <header id="content-header"<?php if ($this->course->get('logo')) {
        echo ' class="with-identity"';
                               } ?>>
        <h2>
...
<?php if (!$this->course->offering()->access('view') && !$sparams->get('preview', 0)) {
    ...
    echo $view;
} elseif ($this->course->offering()->section()->expired() && !$sparams->get('preview', 0)) { ?>
