{{--
  ...
  Variables from controller (copyTask):
    $course — Course model instance to copy
...
@php
  ...
  $pageTitle = Lang::txt('COM_COURSES_COPY_COURSE');
...
<x-page-container :title="$pageTitle">
  ...
  <form action="{{ $formUrl }}" method="post" class="max-w-2xl">
    <x-form-section :heading="Lang::txt('COM_COURSES_COPY_ENTRY')">
      <p class="text-sm text-base-content/70 mb-4">
        {{ Lang::txt('COM_COURSES_COPY_EXPLANATION') }}
      </p>
      ...
      <x-form-field
        ...
        <input type="text"
               ...
               value="{{ e($course->get('alias') . '_copy') }}"
               autocomplete="off" />
      </x-form-field>
      ...
      <x-form-field
        ...
        <input type="text"
               ...
               id="field-title"
               class="input input-bordered w-full"
               value="{{ e(stripslashes($course->get('title')) . ' Copy') }}" />
      </x-form-field>
    </x-form-section>
