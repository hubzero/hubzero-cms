...
<header id="content-header">
    ...
    <?php if ($this->config->get('access-create-course')) { ?>
        <div id="content-header-extra">
            <p>
                <?php $routeUrl = Route::url('index.php?option=' . $this->option . '&controller=course&task=new'); ?>
                <a
                    class="icon-add add btn"
                    href="<?php echo $routeUrl; ?>"><?php echo Lang::txt('COM_COURSES_CREATE_COURSE'); ?></a>
            </p>
...
<section class="main section">
    <?php
        ...
        ?>
    <form class="section-inner hz-layout-with-aside" action="<?php echo $routeUrl; ?>" method="get">
        <div class="subject">

            <div class="container data-entry">
                <?php $txt = Lang::txt('COM_COURSES_SEARCH'); ?>
                        ...
                        foreach ($tags as $tag) {
                            ...
                            <li>
                                <?php
                                    $routeUrl = Route::url(
                                        $url . '&tag='
                                        . implode(',', $this->model->parseTags($this->filters['tag'], $tag))
                                    );
                                ?>
                        ...
                        <p class="course-group-img">
                            ...
                            <a href="<?php echo $routeUrl; ?>">
                                <img
                                    ...
                                    <?php echo $atts; ?>
                                    alt="<?php echo $this->escape($group->get('description')); ?>" />
                            </a>
                        </p>
            ...
            <div class="container">
                <nav class="entries-filters" aria-label="<?php echo Lang::txt('JGLOBAL_FILTER_AND_SORT_RESULTS'); ?>">
                    ...
                    <ul class="entries-menu order-options">
                        <?php
                            ...
                            foreach ($sortOptions as $sortKey => $sortLang) {
                                $active = ($this->filters['sortby'] == $sortKey)
                                    ? ' class="active"' : '';
                                $sortUrl = Route::url(
                                    'index.php?option=' . $this->option
                                    . '&task=browse&sortby=' . $sortKey . $qs
                                );
                                $sortTitle = Lang::txt('COM_COURSES_SORT_BY_' . strtoupper($sortKey));
                                $sortLabel = Lang::txt($sortLang);
                                ?>
                        <li>
                            <a<?php echo $active; ?>
                                href="<?php echo $sortUrl; ?>"
                                title="<?php echo $sortTitle; ?>">
                                <?php echo $sortLabel; ?>
                            </a>
                        </li>
                            <?php } ?>
                ...
                <h3>
                    <?php
                        ...
                        $maxEnd = $this->filters['start'] + $this->filters['limit'];
                        $e = ($this->total > $maxEnd) ? $maxEnd : $this->total;

                    if ($this->filters['search'] != '') {
                        if ($this->filters['tag'] != '') {
                            echo Lang::txt(
                                'COM_COURSES_SEARCH_FOR_IN_WITH',
                ...
                if ($this->courses->total() > 0) {
                                    ...
                                    <a href="<?php echo Route::url($course->link()); ?>">
                                        <?php if ($logo = $course->logo('url')) { ?>
                                            ...
                                            <img
                                                ...
                                                alt="<?php echo $this->escape($course->get('title')); ?>" />
                                        <?php } else { ?>
                                            <span></span>
                                        <?php } ?>
                                    </a>
                                    ...
                                    <dd class="instructors">
                                        ...
                                        if (count($instructors) > 0) {
                                            ...
                                            foreach ($instructors as $i) {
                                                $instructor = \Components\Members\Models\Member::oneOrNew(
                                                    $i->get('user_id')
                                                );
                                                $name = $this->escape(
                                                    stripslashes($instructor->get('name'))
                                                );
                                                $viewLevels = User::getAuthorisedViewLevels();
                                                if (in_array($instructor->get('access'), $viewLevels)) {
                                                    $link = Route::url($instructor->link());
                                                    $names[] = '<a href="' . $link . '">'
                                                        . $name . '</a>';
                                                } else {
                                                    $names[] = $name;
                                            ...
                                            $txt = Lang::txt('COM_COURSES_COURSE_INSTRUCTORS');
                                            echo $txt;
                                            ?>: <span class="entry-instructors"><?php echo $nameList; ?></span><?php
                                        }
                                        ?>
                                    </dd>
                                </dl>
                    ...
                    <?php $txt = Lang::txt('COM_COURSES_NO_RESULTS_FOUND'); ?>
                    <li class="no-results"><p class="warning"><?php echo $txt; ?></p></li>
                <?php } ?>
                </ol>
