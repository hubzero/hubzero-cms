...
class Assetv1r0 extends base
{
    ...
    public function saveTask()
    {
        ...
        if ($progress = Request::getInt('progress_factors', false)) {
            $asset->set('progress_factors', array(
        ...
        if ($tool_param = Request::getInt('tool_param', false)) {
            ...
            if ($config->get('tool_path')) {
                ...
                $asset_path = DS . trim($config->get('uploadpath'), DS)
                    . DS . $this->course_id . DS . $asset->get('id');
            ...
            && Request::getInt('edit_tool_param', false)
        ) {
        ...
        // Build the asset url
        $url = Route::url(
            'index.php?option=com_courses&controller=offering&gid=' . $this->course->get('alias')
            . '&offering=' . $this->offering_alias . '&asset=' . $asset->get('id')
        );
    ...
    public function deleteTask()
    {
        ...
        $path    = DS . trim($params->get('uploadpath', '/site/courses'), DS)
            . DS . $this->course_id . DS . $assetObj->id;
    ...
    public function getformidTask()
    {
        ...
        $query = "SELECT `id` FROM `#__courses_form_deployments` WHERE `form_id` = "
            . $database->quote($formId);
