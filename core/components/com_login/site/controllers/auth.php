...
class Auth extends SiteController
{
	...
	{
		// An absent `return` gives getString() null, which this handed straight
		// to preg_match() -- a PHP 8 deprecation -- and then answered true for,
		// so the caller ran base64_decode(null) for a second one, both printed
		// ahead of the Location header. The caller's fallback is the same either
		// way, so refusing the empty string changes no live URL.
		if ($str === null || $str === '')
		{
			return false;
		}
	...
	public function loginTask()
	{
		...
		else
		{
			...
			{
				$return	= Route::url('index.php?option=com_login');
			}
	...
	public function logoutTask()
	{
				...
				if (class_exists($className))
				{
					if (method_exists($className, 'logout'))
					{
						...
						if ($authenticator == 'pucas')
						{
							...
							{
								App::redirect(Route::url('index.php?option=com_login&task=endsinglesignon&authenticator=' . $authenticator, false));
								return;
							}
							else
							{
		...
		//{
			//$options['clientid'] = 0; //$this->getClientId();
		//}
		$error = false;
		...
		if (!($error instanceof Exception))
		{
			...
			if (isset($user->auth_link_id) && $user->auth_link_id && empty($authenticator))
			{
				...
				if ($auth_domain_name == 'pucas')
				{
					...
					if ($auto_logoff)
					{
						App::redirect(Route::url('index.php?option=com_login&task=logout&authenticator=' . $auth_domain_name, false));
						return;
					}
					else
					{
						App::redirect(Route::url('index.php?option=com_login&task=endsinglesignon&authenticator=' . $auth_domain_name, false));
						return;
					}
				}
			}
