...
// phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
    ...
    public function onInit()
    {
        ...
        // Add ckeditor
        Document::addScript(
            str_replace('/administrator', '', Request::base(true)) . '/' . $this->_basePath . 'ckeditor.js?v='
            . filemtime(__DIR__ . '/assets/ckeditor.js')
        );
        Document::addScript(
            str_replace('/administrator', '', Request::base(true)) . '/' . $this->_basePath . 'adapters/jquery.js?v='
            . filemtime(__DIR__ . '/assets/adapters/jquery.js')
        );
    }
    ...
    public function onDisplay(
        ...
        $editor  = '<textarea name="' . $name . '" id="' . $id . '" ' . ($row ? 'rows="' . $row . '"' : '')
            . ' ' . ($col ? 'cols="' . $col . '"' : '') . ' ' . implode(' ', $atts) . '>' . $content
            . '</textarea>' . $script;
        if (App::isAdmin()) {
    ...
    private function buildConfig($params = array())
    {
        ...
        $config->extraPlugins                  = 'tableresize,iframedialog,hubzeroequation,hubzerogrid,'
            . 'hubzeromacro,hubzerohighlight,wordcount,mentions';
        ...
        $config->extraAllowedContent           = 'img(*)[*]; style(*)[*]; mark(*)[*]; span(*)[*]; map(*)[*]; '
            . 'area(*)[*]; *(*)[*]{*}';
        $config->specialChars                  = array(
            ...
            't', 'u', 'v', 'w', 'x', 'y', 'z', '{', '|', '}', '~', "&euro;", "&lsquo;", "&rsquo;", "&ldquo;",
            "&rdquo;", "&ndash;", "&mdash;", "&iexcl;", "&cent;", "&pound;", "&curren;", "&yen;", "&brvbar;",
            "&sect;", "&uml;", "&copy;", "&ordf;", "&laquo;", "&not;", "&reg;", "&macr;", "&deg;", "&sup2;",
            ...
            "&ucirc;", "&uuml;", "&yacute;", "&thorn;", "&yuml;", "&OElig;", "&oelig;", "&#372;", "&#374", "&#373",
            "&#375;", "&sbquo;", "&#8219;", "&bdquo;", "&hellip;", "&trade;", "&#9658;", "&bull;", "&rarr;",
            "&rArr;", "&hArr;", "&diams;", "&asymp;", "&Omega;"
        );
        ...
        $config->toolbar                       = array(
            ...
            array(
                ...
                'HubzeroEquation', 'HubzeroGrid', 'Templates'
            ),
            array('Link', 'Unlink', 'Anchor'),
            '/',
            array('Format', 'Bold', 'Italic', 'Underline', 'Strike', 'Subscript', 'Superscript'),
            array(
                'NumberedList', 'BulletedList', '-', 'Blockquote', 'CreateDiv', '-', 'JustifyLeft',
        ...
        if (in_array('minimal', $this->params->get('class')) && in_array('images', $this->params->get('class'))) {
            ...
            $config->toolbar = array_merge(
                array_splice($config->toolbar, 0, 1),
                array(array('Image')),
                $config->toolbar
            );
        }
