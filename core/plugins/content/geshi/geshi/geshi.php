...
class GeSHi
{
     ...
     */
    // Styles for important parts of the code
    public $important_styles = 'font-weight: bold; color: red;';
    ...
    public function set_header_type($type)
    {
        ...
        if (
            !in_array($type, array(GESHI_HEADER_NONE, GESHI_HEADER_DIV,
    ...
    {
        if (
            GESHI_NO_LINE_NUMBERS != $flag && GESHI_NORMAL_LINE_NUMBERS != $flag
    ...
    {
        if (in_array($case, array(GESHI_CAPS_NO_CHANGE, GESHI_CAPS_UPPER, GESHI_CAPS_LOWER))) {
    ...
    public function optimize_keyword_group($key)
    {
        ...
        if (isset($this->language_data['PARSER_CONTROL'])) {
            if (isset($this->language_data['PARSER_CONTROL']['KEYWORDS'])) {
                ...
                $keywordControl = $this->language_data['PARSER_CONTROL']['KEYWORDS'][$key];
                if (isset($keywordControl['SPACE_AS_WHITESPACE'])) {
                    if (isset($keywordControl['SPACE_AS_WHITESPACE'])) {
                        $space_as_whitespace = $keywordControl['SPACE_AS_WHITESPACE'];
                    }
                }
            }
        }
    ...
    public function build_parse_cache()
    {
        ...
        foreach (array_keys($this->language_data['KEYWORDS']) as $key) {
            if (
                !isset($this->lexic_permissions['KEYWORDS'][$key]) ||
            ...
            while ($i < $length) {
                ...
                if (is_array($delim_copy[$next_match_pointer['dk']])) {
                    ...
                    while (true) {
                        ...
                        if (
                            $code[$i] == $next_match_pointer['open'][0] && ($next_match_pointer['open_strlen'] == 1 ||
                ...
                for ($i = 0; $i < $length; ++$i) {
                    ...
                    if (isset($this->language_data['COMMENT_REGEXP']) && $next_comment_regexp_pos < $i) {
                        ...
                        foreach ($this->language_data['COMMENT_REGEXP'] as $comment_key => $regexp) {
                            $match_i = false;
                            if (
                                isset($comment_regexp_cache_per_key[$comment_key]) &&
                                ...
                                // This is to allow use of the offset parameter in preg_match
                                // and stay as compatible with older PHP versions as possible
                            } elseif (
                                (GESHI_PHP_PRE_433
                    ...
                    if (isset($is_string_starter[$char])) {
                        ...
                        if (is_array($is_string_starter[$char])) {
                            ...
                            foreach ($is_string_starter[$char] as $testchar) {
                                if (
                                    $testchar === substr($part, $i, strlen($testchar)) &&
                    ...
                    if ($string_started && ($i != $next_comment_regexp_pos)) {
                        ...
                        if (
                            !isset($this->language_data['STYLES']['STRINGS'][$string_key]) ||
                        ...
                        if (!$this->use_classes) {
                            $strStyle = $this->language_data['STYLES']['STRINGS'][$string_key];
                            $string_attributes = ' style="' . $strStyle . '"';
                        } else {
                        ...
                        do {
                            ...
                            if ($this->lexic_permissions['ESCAPE_CHAR']) {
                                ...
                                if (isset($this->language_data['ESCAPE_REGEXP']) && $next_escape_regexp_pos < $start) {
                                    ...
                                    foreach ($this->language_data['ESCAPE_REGEXP'] as $escape_key => $regexp) {
                                        $match_i = false;
                                        if (
                                            isset($escape_regexp_cache_per_key[$escape_key]) &&
                                            ...
                                            // This is to allow use of the offset parameter
                                            // in preg_match and stay as compatible with older
                                            // PHP versions as possible
                                        } elseif (
                                            (GESHI_PHP_PRE_433
                                                && preg_match(
                                                    $regexp,
                            ...
                            if (
                                $simple_escape < $next_escape_regexp_pos &&
                                $simple_escape < $length &&
                                ...
                                if (!$this->use_classes) {
                                    $escapeStyle = $this->language_data['STYLES']['ESCAPE_CHAR'][0];
                                    $escape_char_attributes = ' style="' . $escapeStyle . '"';
                                } else {
                                    $escape_char_attributes = ' class="es0"';
                                }

                                //Add the style for the escape char ...
                                $escChar = self::hsc($this->language_data['ESCAPE_CHAR']);
                                $string .= "<span$escape_char_attributes>" . $escChar;
                                ...
                                //Get the style for this escaped char ...
                                if (!$this->use_classes) {
                                    $escStyle = $this->language_data['STYLES']['ESCAPE_CHAR'][$escape_key];
                                    $escape_char_attributes = ' style="' . $escStyle . '"';
                                } else {
                                ...
                                $remainderLen = $close_pos - $start + $char_len;
                                $string .= $this->hsc(substr($part, $start, $remainderLen)) . '</span>';
                        ...
                        $this->lexic_permissions['STRINGS'] && $hq && $hq[0] == $char &&
                        substr($part, $i, $hq_strlen) == $hq &&
                        ($i != $next_comment_regexp_pos)
                    ) {
                        // The start of a hard quoted string
                        if (!$this->use_classes) {
                            $strHardStyle = $this->language_data['STYLES']['STRINGS']['HARD'];
                            $escHardStyle = $this->language_data['STYLES']['ESCAPE_CHAR']['HARD'];
                            $string_attributes = ' style="' . $strHardStyle . '"';
                            $escape_char_attributes = ' style="' . $escHardStyle . '"';
                        } else {
                        ...
                        while ($close_pos = strpos($part, $this->language_data['HARDQUOTE'][1], $start)) {
                            ...
                            $hardChar = $this->language_data['HARDCHAR'];
                            if (
                                $this->lexic_permissions['ESCAPE_CHAR'] &&
                                $part[$close_pos - 1] == $hardChar &&
                        ...
                        if ($this->lexic_permissions['ESCAPE_CHAR'] && $this->language_data['ESCAPE_CHAR']) {
                            ...
                            while ($es_pos = strpos($string, $this->language_data['ESCAPE_CHAR'], $start)) {
                                ...
                                $c = 0;
                                while (
                                    isset($string[$es_pos + $c]) && isset($string[$es_pos + $c + 1])
                        ...
                        if ($i == $next_comment_regexp_pos) {
                            ...
                            if ($this->lexic_permissions['COMMENTS']['MULTI']) {
                                if (!$this->use_classes) {
                                    $comStyle = $this->language_data['STYLES']['COMMENTS'][$comment['key']];
                                    $attributes = ' style="' . $comStyle . '"';
                                } else {
                        ...
                        if (!$COMMENT_MATCHED) {
                            ...
                            if (!empty($this->language_data['COMMENT_MULTI']) && $next_comment_multi_pos < $i) {
                                ...
                                foreach ($this->language_data['COMMENT_MULTI'] as $open => $close) {
                                    $match_i = false;
                                    if (
                                        isset($comment_multi_cache_per_key[$open]) &&
                            ...
                            if ($i == $next_comment_multi_pos) {
                                ...
                                if (
                                    ...
                                    if ($open != GESHI_START_IMPORTANT) {
                                        if (!$this->use_classes) {
                                            $multiStyle = $this->language_data['STYLES']['COMMENTS']['MULTI'];
                                            $attributes = ' style="' . $multiStyle . '"';
                                        } else {
                                ...
                                // Short-cut through all the multiline code
                                $commentLen = $close_pos - $i - $open_strlen + $close_strlen;
                                $rest_of_comment = $this->hsc(substr($part, $i + $open_strlen, $commentLen));
                                if (
                                    ($this->lexic_permissions['COMMENTS']['MULTI'] ||
                        ...
                        if (!$COMMENT_MATCHED) {
                            ...
                            if (!empty($this->language_data['COMMENT_SINGLE']) && $next_comment_single_pos < $i) {
                                ...
                                foreach ($this->language_data['COMMENT_SINGLE'] as $comment_key => $comment_mark) {
                                    $match_i = false;
                                    if (
                                        isset($comment_single_cache_per_key[$comment_key]) &&
                            ...
                            if ($next_comment_single_pos == $i) {
                                ...
                                if (
                                    (empty($sc_disallowed_before) || ($i == 0) ||
                                    ...
                                    if ($this->lexic_permissions['COMMENTS'][$comment_key]) {
                                        if (!$this->use_classes) {
                                            $comKeyStyle = $this->language_data['STYLES']['COMMENTS'][$comment_key];
                                            $attributes = ' style="' . $comKeyStyle . '"';
                                        } else {
                                            $attributes = ' class="co' . $comment_key . '"';
                                        }
                                        $markedComment = $this->hsc($this->change_case($comment_mark));
                                        $test_str = "<span$attributes>" . $markedComment;
                                    } else {
            ...
            for ($key = 0, $n = count($lines); $key < $n; $key++) {
                ...
                for ($i = 0; $i < $length; ++$i) {
                    ...
                    if ($IN_TAG) {
                        ...
                        // Moved out of the loop as it doesn't change within the loop
    ...
    public function handle_keyword_replace($match)
    {
        ...
        if ($this->keyword_links) {
            ...
            if (
                isset($this->language_data['URLS'][$k]) &&
    ...
    public function handle_regexps_callback($matches)
    {
        ...
        $regexpStyle = $this->language_data['STYLES']['REGEXPS'][$this->_rx_key];
        return ' style="' . call_user_func($regexpStyle, $matches[1]) . '"' . $matches[1] . '|>';
    }
    ...
    public function parse_non_string_part($stuff_to_parse)
    {
        ...
        foreach (array_keys($this->language_data['KEYWORDS']) as $k) {
            if (
                !isset($this->lexic_permissions['KEYWORDS'][$k]) ||
                ...
                // since we don't want PHP / PCRE to crash due to too large patterns
                // we split them into smaller chunks
                $cachedKwLists = $this->language_data['CACHED_KEYWORD_LISTS'][$k];
                for ($set = 0, $set_length = count($cachedKwLists); $set <  $set_length; ++$set) {
                    $keywordset =& $this->language_data['CACHED_KEYWORD_LISTS'][$k][$set];
                    // Might make a more unique string for putting the number in soon
                    // Basically, we don't put the styles in yet because then the styles
                    // themselves will get highlighted if the language has a CSS keyword
                    // in it (like CSS, for example ;))
                    $pattern = "/$disallowed_before_local({$keywordset})" .
                        "(?!\<DOT\>(?:htm|php|aspx?))$disallowed_after_local/$modifiers";
            ...
            if ($this->lexic_permissions['REGEXPS'][$key]) {
                if (is_array($regexp)) {
                    if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
                        ...
                        $searchPattern = '/' . $regexp[GESHI_SEARCH] . '/' . $regexp[GESHI_MODIFIERS];
                        $replacement = $regexp[GESHI_BEFORE] . '<|!REG3XP' . $key . '!>' .
                            $regexp[GESHI_REPLACE] . '|>' . $regexp[GESHI_AFTER];
                        $stuff_to_parse = preg_replace($searchPattern, $replacement, $stuff_to_parse);
                    }
                } else {
                    if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
                        // produce valid HTML when we match multiple lines
        ...
        $numbersPrecheck = $this->language_data['PARSER_CONTROL']['NUMBERS']['PRECHECK_RX'];
        if ($this->lexic_permissions['NUMBERS'] && preg_match($numbersPrecheck, $stuff_to_parse)) {
            $numbers_found = true;
        ...
        if ($this->lexic_permissions['METHODS'] && $this->language_data['OOLANG']) {
            ...
            foreach ($this->language_data['OBJECT_SPLITTERS'] as $key => $splitter) {
                if (false !== strpos($stuff_to_parse, $splitter)) {
                    if (!$this->use_classes) {
                        $methodStyle = $this->language_data['STYLES']['METHODS'][$key];
                        $attributes = ' style="' . $methodStyle . '"';
                    } else {
                        $attributes = ' class="me' . $key . '"';
                    }
                    $quotedSplitter = preg_quote(
                        $this->language_data['OBJECT_SPLITTERS'][$key],
                        '/'
                    );
                    $pattern = "/($oolang_before)($quotedSplitter)" .
                        "($oolang_spaces)($oolang_after)/";
                    $replacement = "\\1\\2\\3<|$attributes>\\4|>";
                    $stuff_to_parse = preg_replace($pattern, $replacement, $stuff_to_parse);
                }
            }
        }
        ...
        if ($this->lexic_permissions['SYMBOLS'] && !empty($this->language_data['SYMBOLS'])) {
            // Get all matches and throw away those within a block that is already
            // highlighted... (i.e. matched by a regexp)
            $symbolPattern = "/<\|(?:<DOT>|[^>])+>(?:(?!\|>).*?)\|>|<\/a>|(?:" .
                $this->language_data['SYMBOL_SEARCH'] . ")+(?![^<]+?>)/";
            $n_symbols = preg_match_all(
                $symbolPattern,
                $stuff_to_parse,
                $pot_symbols,
            ...
            for ($s_id = 0; $s_id < $n_symbols; ++$s_id) {
                ...
                if (strpos($symbol_match, '<') !== false || strpos($symbol_match, '>') !== false) {
                    ...
                    if (
                        strpos($symbol_match, '<SEMI>') === false &&
                ...
                if ($this->language_data['MULTIPLE_SYMBOL_GROUPS']) {
                    ...
                    $symbolSearch = $this->language_data['SYMBOL_SEARCH'];
                    preg_match_all(
                        "/" . $symbolSearch . "/",
                        $symbol_match,
                        $sym_match_syms,
                    ...
                    foreach ($sym_match_syms[0] as $sym_ms) {
                        ...
                        if (
                            isset($this->language_data['SYMBOL_DATA'][$sym_ms])
                            ...
                            if (!$this->use_classes) {
                                $symStyle = $this->language_data['STYLES']['SYMBOLS'][$old_sym];
                                $symbol_hl .= '<| style="' . $symStyle . '">';
                            } else {
                    ...
                    if (!$this->use_classes) {
                        $sym0Style = $this->language_data['STYLES']['SYMBOLS'][0];
                        $symbol_hl = '<| style="' . $sym0Style . '">';
                    } else {
                        $symbol_hl = '<| class="sy0">';
                    }
                ...
                $replaceOffset = $symbol_offset + $global_offset;
                $stuff_to_parse = substr_replace(
                    $stuff_to_parse,
                    $symbol_hl,
                    $replaceOffset,
                    $symbol_length
        ...
        if (isset($this->link_styles[GESHI_LINK])) {
            if ($this->use_classes) {
                $linkOpen = '<a' . $this->link_target . ' href=';
                $stuff_to_parse = str_replace('<|UR1|', $linkOpen, $stuff_to_parse);
            } else {
                $linkStyle = $this->link_styles[GESHI_LINK];
                $linkOpen = '<a' . $this->link_target . ' style="' . $linkStyle . '" href=';
                $stuff_to_parse = str_replace('<|UR1|', $linkOpen, $stuff_to_parse);
            }
        } else {
            $linkOpen = '<a' . $this->link_target . ' href=';
    ...
    public function merge_arrays()
    {
        ...
        for ($i = 0; $i < $narrays; $i++) {
            if (!is_array($arrays[$i])) {
                ...
                $errorMsg = 'Argument #' . ($i + 1) . ' is not an array - ' .
                    'trying to merge array with scalar! Returning false!';
        ...
        for ($i = 1; $i < $narrays; $i++) {
            foreach ($arrays[$i] as $key => $value) {
                if (is_array($value) && isset($ret[$key])) {
                    // if $ret[$key] is not an array you try to merge an scalar value
                    // with an array - the result is not defined (incompatible arrays)
    ...
    public function finalise(&$parsed_code)
    {
        ...
        if ($this->line_numbers != GESHI_NO_LINE_NUMBERS && $this->header_type != GESHI_HEADER_PRE_TABLE) {
            ...
            for ($i = 0, $n = count($code); $i < $n;) {
                ...
                if (
                    $this->line_numbers == GESHI_FANCY_LINE_NUMBERS &&
    ...
    public function header()
    {
        ...
        if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
            if ($this->header_type == GESHI_HEADER_PRE) {
                ...
                $this->header_type == GESHI_HEADER_DIV ||
    ...
    public function get_stylesheet($economy_mode = true)
    {
        ...
        foreach ($this->language_data['STYLES']['REGEXPS'] as $group => $styles) {
            if (
                ...
                if (
                    is_array($this->language_data['REGEXPS'][$group]) &&
        ...
        $extraCount = count($this->highlight_extra_lines);
        $extraStylesCount = count($this->highlight_extra_lines_styles);
        if (!$economy_mode || ($extraCount != $extraStylesCount)) {
            $extraStyle = $this->highlight_extra_lines_style;
            $stylesheet .= "{$selector}.ln-xtra, {$selector}li.ln-xtra, " .
                "{$selector}div.ln-xtra {{$extraStyle}}\n";
        }
        $stylesheet .= "{$selector}span.xtra { display:block; }\n";
        foreach ($this->highlight_extra_lines_styles as $lineid => $linestyle) {
            $stylesheet .= "{$selector}.lx$lineid, {$selector}li.lx$lineid, " .
                "{$selector}div.lx$lineid {{$linestyle}}\n";
        }
    ...
    public function optimize_regexp_list($list, $regexp_delimiter = '/')
    {
        ...
        for ($i = 0, $i_max = count($list); $i < $i_max; ++$i) {
            ...
            while (true) {
                ...
                if (isset($prev_keys[$level])) {
                    ...
                    $char = 0;
                    while (
                        isset($entry[$char]) && isset($prev_keys[$level][$char])
                    ...
                    if ($char > 0) {
                        ...
                        if ($char == strlen($prev_keys[$level])) {
                            ...
                            if (
                                in_array($new_key_part1[0], $regex_chars)
                                ...
                                $prevPointer = $pointer[$prev_keys[$level]];
                                $pointer[$new_key_part1] = array($new_key_part2 => $prevPointer);
                                unset($pointer[$prev_keys[$level]]);
                                $pointer = &$pointer[$new_key_part1];
                                // recreate key index
                                $newKeys = array($new_key_part1, $new_key_part2);
                                array_splice($prev_keys, $level, count($prev_keys), $newKeys);
                ...
                if ($level == 0 && !empty($tokens)) {
                    ...
                    $totalSubpatterns = $num_subpatterns + $new_subpatterns;
                    if (GESHI_MAX_PCRE_SUBPATTERNS && $totalSubpatterns > GESHI_MAX_PCRE_SUBPATTERNS) {
        ...
        $new_entry = $this->_optimize_regexp_list_tokens_to_string($tokens);
        $newSubpatterns = substr_count($new_entry, '(?:');
        $totalPatterns = $num_subpatterns + $newSubpatterns;
        if (GESHI_MAX_PCRE_SUBPATTERNS && $totalPatterns > GESHI_MAX_PCRE_SUBPATTERNS) {
            if (!empty($regexp_list[$list_key])) {
                ++$list_key;
            }
            $regexp_list[$list_key] = $new_entry;
    ...
    public function _optimize_regexp_list_tokens_to_string(&$tokens, $recursed = false)
    {
        ...
        foreach ($tokens as $token => $sub_tokens) {
            ...
            unset($sub_tokens['']);
            if (!empty($sub_tokens)) {
                $subList = $this->_optimize_regexp_list_tokens_to_string($sub_tokens, true);
                $list .= '(?:' . $subList . ')';
                if ($close_entry) {
        ...
        if (!$recursed) {
            ...
            //     create_function(
            //         '$matches',
            //         'return "(?:" . preg_replace("#" . preg_quote($matches[1], "#") .
            //         "(?=\||$)#", "", $matches[0]) . ")" . $matches[1];'
            //     ),
            //     $list
            // );
