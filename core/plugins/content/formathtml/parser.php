...
class Parser
{
    ...
    {
        return $this->_tokenPrefix . $this->randomString();
    }
    ...
    public function parse($text, $fullparse = true, $linestart = 0, $camelcase = 1)
    {
        ...
        if (trim($this->_data['input']) && !trim($this->_data['output'])) {
            $this->_data['output'] = '<p class="warning">Parsing error resulted in empty content. ' .
    ...
    {
        $text = preg_replace_callback('/<(pre)(.*?)>(.+?)<\/(pre)>/is', array(&$this, 'dataPush'), $text ?? '');

        $text = preg_replace_callback('/<(code)(.*?)>(.+)<\/(code)>/iU', array(&$this, 'dataPush'), $text ?? '');
    ...
    private function dataPull($matches)
    {
        ...
        if (isset($this->_tokens[$tag]) && isset($this->_tokens[$tag][$key])) {
            return $this->{'restore' . ucfirst($tag)}($this->_tokens[$tag][$key]);
        }
        if ($tag == 'macro') {
            return '';
        }
    ...
    private function unstrip($text, $html = true)
    {
        foreach ($this->_tokens as $tag => $vals) {
            $pattern = '/<(' . $tag . ') (.+?)><\/(\1) \2>/si';
            $text = preg_replace_callback($pattern, array(&$this, 'dataPull'), $text) ?? $text;
    ...
    private function macros($text)
    {
        ...
        return preg_replace_callback($pattern, array(&$this, 'getMacro'), $text);
    }
                ...
                foreach ($this->getMacroPaths() as $path) {
                    if (is_file($path . $file)) {
                        ...
                        if (is_file($path . $altfile)) {
                            ...
                            $macroname = __NAMESPACE__ . '\\Macros\\' .
