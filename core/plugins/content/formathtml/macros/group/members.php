...
class Members extends GroupMacro
{
    ...
    public function render()
    {
        ...
        $isMember = (in_array(\Hubzero\Facades\User::get('id'), $this->group->get('members'))) ? true : false;
    ...
    private function getGroupMembers($group, $filters = array())
    {
        ...
        // Get group params
        $params = \Hubzero\Facades\Component::params("com_groups");
    ...
    private function renderMembers($group, $members)
    {
        ...
        if (count($members) > 0) {
            ...
            foreach ($profiles as $profile) {
                $profileName = stripslashes($profile->get('name'));
                $content .= '<a href="' . Route::url($profile->link()) . '" class="member" ' .
                    'title="Go to ' . $profileName . '\'s Profile.">';
                $content .= '<img src="' . $profile->picture() . '" alt="' . $profileName . '" ' .
                    'class="member-border" width="50px" height="50px" />';
                $content .= '<span class="name">' . $profileName . '</span>';
