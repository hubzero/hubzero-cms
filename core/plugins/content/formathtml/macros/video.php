...
class Video extends Macro
{
    ...
    public function description()
    {
        ...
        $txt['html'] = '<p>Embeds a video into the Page. Accepts either full video URL ' .
            '(YouTube, Vimeo, Kaltura, Blip TV) or a file name or path.</p>
            <p><strong>Youtube URL:</strong> ' .
            'https://www.youtube.com/watch?v=<span class="highlight">FgfGOEpZEOw</span></p>
            <p>Examples:</p>
            <ul>
            ...
            <p>Size attributes may be given as single numeric values or with units (%, em, px). ' .
            'When an attribute name is given (e.g., width=600, height=338), order does not matter. ' .
            'Attribute values may be quoted but are not required to be. ' .
            'When a name attribute is not give (e.g., 600, 338), the first value will be set ' .
            'as width and the second value as height.</p>
            <p>Examples:</p>
            <ul>
                <li><code>[[Video(MyVideo.m4v, width="600", height="338")]]</code></li>
                <li><code>[[Video(https://www.youtube.com/watch?v=FgfGOEpZEOw, ' .
                'width=600px, height=338px)]]</code></li>
                <li><code>[[Video(https://blip.tv/play/hNNNg4uIDAI.x?p=1, 640, 380)]]</code> ' .
                '- width 640px, height 380px</li>
                <li><code>[[Video(https://player.vimeo.com/video/67115692, 100%)]]</code> ' .
                '- width of 100%</li>
            </ul>';
    ...
    public function render()
    {
        ...
        $width = (isset($this->attr['width']) && $this->attr['width'] != '')
            ? $this->attr['width']
            : $default_width;
        $height = (isset($this->attr['height']) && $this->attr['height'] != '')
            ? $this->attr['height']
            : $default_height;
        ...
        if (!strstr($url, 'http')) {
            ...
            if (strstr($url, '/')) {
                ...
                // Just a file name.
                $video_url = $this->path($url);

                if (!file_exists($video_url)) {
                    $video_url = $this->path($url, true);

                    if (!file_exists($video_url)) {
                        return '(video:' . $url . ' not found)' . $this->path($url);
                    }
                }
            }
        } else {
        ...
        if (stristr($video_url, 'youtube')) {
        ...
        } elseif (stristr($video_url, 'vimeo')) {
            // Vimeo
            $type = 'vimeo';
        } elseif (stristr($video_url, 'blip')) {
            // BlipTV
            $type = 'blip';
        } elseif (stristr($video_url, 'kaltura')) {
            // Kaltura
            $type = 'kaltura';
            ...
            if (!stristr($video_url, 'iframeembed') && stristr($video_url, 'kmc')) {
                ...
                $video_url = 'https://www.kaltura.com/p/' . $partner_id . '/sp/' . $partner_id .
                    '00/embedIframeJs/uiconf_id/' . $uiconf_id . '/partner_id/' . $partner_id .
                    '?iframeembed=true&playerId=movie' . rand(0, 1000) . '&entry_id=' . $entry_id .
        ...
        if ($type == 'local') {
            ...
            $movieId = rand(0, 1000);
            $html .= '<video id="movie' . $movieId . '" width="' . $width . '" height="' . $height . '" ' .
                'preload controls>';
            switch ($ext) {
                case 'mov':
                case 'mp4':
                case 'm4v':
                    $html .= '<source src="' . $this->link($url) . '" type="video/mp4" />';
                    break;

                case 'ogg':
                case 'ogv':
                    $html .= '<source src="' . $this->link($url) . '" type="video/ogg" />';
                    break;

                case 'webm':
                    $html .= '<source src="' . $this->link($url) . '" type="video/webm" />';
                    break;
            }
            $html .= '</video>';
            $html .= '</div>';
            ...
            // External
            $movieId = rand(0, 1000);
            $html = '<iframe sandbox="allow-scripts allow-same-origin" id="movie' . $movieId . '" ' .
                'src="' . $video_url . '" width="' . $width . '" height="' . $height . '" ' .
