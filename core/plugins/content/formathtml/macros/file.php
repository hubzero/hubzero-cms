...
class File extends Macro
{
    ...
    {
        $txt = array();
        $txt['wiki'] = 'This macro alternately generates a link to a file or embeds the file ' .
            'for display (only valid for CDF files and images). The first argument is the filename.';
        $txt['html'] = '<p>This macro alternately generates a link to a file or embeds the file ' .
            'for display (only valid for CDF files and images). ' .
            'The first argument is the filename.</p>
            <ul>
                <li>digits and unit are interpreted as the <code>width</code> attribute ' .
                '(ex. 120, 25%) for the file/image</li>
                <li><code>right</code>, <code>left</code>, <code>top</code> or <code>bottom</code> ' .
                'are interpreted as the alignment for the file (currently, only applies to images)</li>
                <li><code>link=some Link...</code> replaces the link to the file source by the one ' .
                'specified using <code>link</code>. If no value is specified, the link is simply removed.</li>
                <li><code>nolink</code> means without link to image source ' .
                '(deprecated, use <code>link=</code>)</li>
                <li><code>key=value</code> style are interpreted as HTML attributes or CSS style ' .
                'indications for the file. Valid keys are:<br />
                ...
                <li><code>details</code> indicates wether to display file information ' .
                '(size, etc.). Defaults to: on, Accepts: false, no, off, hide, hidden, none.<br />
                The single attribute <code>nodetails</code> is an alternative.</li>
            </ul>
            <p>Examples:</p>
            <ul>
                <li><code>[[File(mydoc.pdf)]]</code> # simplest</li>
                <li><code>[[File(mydoc.pdf, alt="My document")]]</code> # ALT text. For images, ' .
                'this is the "alt" attribute. For files, the "title" attribute.</li>
                <li><code>[[File(mydoc.pdf, desc="My document")]]</code> # Link text. ' .
                'If none is provided, the filename will be used.</li>
                <li><code>[[File(mydoc.pdf, details="off")]]</code> or ' .
                '<code>[[File(mydoc.pdf, nodetails)]]</code> to link a file and NOT display file metadata.</li>
            </ul>
            <p>Examples (CDF):</p>
            <ul>
                <li><code>[[File(prog.cdf, width=300, height=500)]]</code> # with embed width and height sizes</li>
                <li><code>[[File(prog.cdf, width=300, height=500, altimage=prog.png)]]</code> ' .
                '# CDF file with sizes and alternate image to show if CDF plugin isn\'t available.</li>
                <li><code>[[File(prog.cdf, width=300, height=500, alt="A nifty program")]]</code> ' .
                '# CDF file with sizes and alternate content to show if CDF plugin isn\'t available.</li>
            </ul>';
        return $txt['html'];
    }
    ...
    public function render()
    {
        ...
        $singlePattern = '/[, ](left|right|top|center|bottom|nodetails|[0-9]+(px|%|em)?)(?:[, ]|$)/i';
        ...
        $quotedPattern = '/[, ](alt|altimage|althref|desc|title|width|height|align|border|longdesc|' .
            'class|id|usemap|link|rel|details)=(?:["\'])([^"\']*)(?:["\'])/i';
        ...
        $unquotedPattern = '/[, ](alt|altimage|althref|desc|title|width|height|align|border|longdesc|' .
            'class|id|usemap|link|rel|details)=([^"\',]*)(?:[, ]|$)/i';
    ...
    public function parseAttributeValuePair($matches)
    {
        ...
        $size   = '/^[0-9]+(%|px|em)+$/';
        $attrs  = '/(alt|altimage|althref|desc|title|width|height|align|border|' .
            'longdesc|class|id|usemap|rel|details)=(.+)/';
        $quoted = "/(?:[\"'])(.*)(?:[\"'])$/";
    ...
    private function embed($file, $attr = array())
    {
        ...
        switch ($ext) {
            case 'unity3d':
                ...
                $attr['href'] = (isset($attr['href']) && $attr['href'] && $attr['href'] != 'none')
                    ? $attr['href']
                    : $this->link($file);
                ...
                if (!array_key_exists('alt', $attr)
                    ...
                    && $attr['altimage'] != ''
                    && file_exists($this->path($attr['altimage'])))
                {
                    $althref = (array_key_exists('althref', $attr) && $attr['althref'] != '')
                        ? $attr['althref']
                        : $attr['href'];
                    $descHtml = htmlentities($attr['desc'], ENT_COMPAT, 'UTF-8');
                    $attr['alt'] = '<a class="attachment" rel="internal" href="' .
                        $althref . '" title="' . $descHtml . '">';
                    $attr['alt'] .= '<img src="' . $this->link($attr['altimage']) .
                        '" alt="' . $descHtml . '" />';
                    $attr['alt'] .= '</a>';
                }
                else
                {
                    ...
                    $descHtml = htmlentities($attr['desc'], ENT_COMPAT, 'UTF-8');
                    $attr['alt'] .= '<a class="attachment" rel="internal" href="' .
                        $althref . '" title="' . $descHtml . '">' . $attr['desc'] . '</a>';
                }
                */
                    ...
                    'data-href="' . $attr['href'] . '">
                        <div class="missing">
                            <a href="http://unity3d.com/webplayer/" title="Unity Web Player. Install now!">
                                ...
                                'src="' . $scheme . 'webplayer.unity3d.com/installation/getunity.png" ' .
                                'width="193" height="63" />
                            </a>
                        </div>
                    </div>' . "\n";
            ...
            case 'cdf':
                ...
                if (
                    ...
                    $althref = (array_key_exists('althref', $attr) && $attr['althref'] != '')
                        ? $attr['althref']
                        : $attr['href'];
                    $attr['alt'] = '<a href="http://www.wolfram.com/cdf-player/" ' .
                        'title="CDF Web Player. Install now!">';
                    $descHtml = htmlentities($attr['desc'], ENT_COMPAT, 'UTF-8');
                    $attr['alt'] .= '<img src="' . $this->link($attr['altimage']) . '" alt="' .
                        $descHtml . '" />';
                    $attr['alt'] .= '</a>';
                } else {
                ...
                $heightInt = intval($attr['height']);
                $html = '<noscript>';
                $html .= '<div class="embedded-plugin" data-width="' . $widthInt . '" ' .
                    'data-height="' . $heightInt . '" data-href="' . $attr['href'] . '" ' .
                    'style="width: ' . $widthInt . 'px; height: ' . $heightInt . ';">';
                $html .= $attr['alt'];
                $html .= '</div>';
                $html .= '</noscript>' . "\n";
                break;
            ...
            default:
                ...
                if (in_array($ext, $this->imgs)) {
                    ...
                    } elseif (file_exists($this->path($file, true))) {
                        $size = filesize($this->path($file, true));
                    }
                    ...
                    $html = '<a class="attachment" rel="' . $attr['rel'] . '" href="' .
                        $attr['href'] . '" title="' . $attr['title'] . '">' . $attr['desc'] . '</a>';
