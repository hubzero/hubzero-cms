...
class Loadmodule extends Plugin
{
    ...
    public function onContentPrepare($context, &$article, &$params, $page = 0)
    {
        ...
        $hasLoadPosition = isset($article->text) && strpos($article->text, 'loadposition') !== false;
        $hasLoadModule = isset($article->text) && strpos($article->text, 'loadmodule') !== false;
        if (isset($article->text) && !$hasLoadPosition && !$hasLoadModule) {
            return true;
        ...
        if ($matches) {
            foreach ($matches as $match) {
                ...
                // We should replace only first occurrence in order to allow positions with
                // the same name to regenerate their content:
