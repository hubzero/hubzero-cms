...
if ($mark = $this->params->get('onCommentMark')) {
    ...
    $rtrn = $this->url ? $this->url : Request::getString('REQUEST_URI', 'index.php?option=' . $this->option . '&id=' .
    $this->obj_id . '&active=comments', 'server');
            ...
            <div class="comment-content">
                ...
                <p class="comment-title">
                    ...
                    $creatorLink = Route::url(
                        $this->comment->creator->link()
                    );
                    $creatorName = $this->escape(
                        stripslashes($this->comment->creator->get('name'))
                    );
                    ?>
                    <strong>
                    ...
                    <a class="permalink"
                        href="<?php echo $this->comment->link(); ?>"
                        title="<?php echo Lang::txt('PLG_HUBZERO_COMMENTS_PERMALINK'); ?>">
                        <?php
                        ...
                        <span class="comment-date-at">
                            <?php echo Lang::txt('PLG_HUBZERO_COMMENTS_AT'); ?>
                        </span>
                        <span class="time">
                        ...
                        <span class="comment-date-on">
                            <?php echo Lang::txt('PLG_HUBZERO_COMMENTS_ON'); ?>
                        </span>
                        <span class="date">
                            <time datetime="<?php echo $createdDatetime; ?>">
                ...
                <div class="comment-attachments">
                    ...
                    foreach ($this->comment->files()->rows() as $attachment) {
                        ...
                        if ($attachment->exists()) {
                            ...
                            if ($attachment->isImage()) {
                                if ($attachment->width() > 400) {
                                    $html = '<p><a href="'
                                        ...
                                        . Route::url($link)
                                        . '" alt="'
                                        . $this->escape($attachment->get('description'))
                                        . '" width="400" /></a></p>';
                                } else {
                                    $html = '<p><img src="'
                                        . Route::url($link)
                                ...
                                $html  = '<a class="attachment '
                                    . Filesystem::extension($attachment->get('filename'))
                                    . '" href="'
                                    . Route::url($link)
                                    . '" title="'
                                    . $this->escape($attachment->get('description'))
                            ...
                            $html .= '<p class="attachment-meta">';
                            $html .= '<span class="attachment-size">'
                                . $this->escape($attachment->get('filename'))
                                . '</span>';
                            $html .= '<span class="attachment-action">'
                ...
                <?php if (!$this->comment->isReported()) { ?>
                    <?php
                    $isOwner = $this->comment->get('created_by') == User::get('id');
                    $canManage = $this->params->get('access-manage-comment');
                    $canDelete = ($this->params->get('access-delete-comment') && $isOwner) || $canManage;
                    $canEdit = ($this->params->get('access-edit-comment') && $isOwner) || $canManage;
                    $canCreate = $this->params->get('access-create-comment');
                    $maxDepth = $this->params->get('comments_depth', 3);
                    $canReply = $canCreate && $this->depth < $maxDepth;
                    ...
                    <p class="comment-options">
                        <?php if ($canDelete) { ?>
                            <a class="icon-delete delete"
                                ...
                                data-txt-confirm="<?php echo Lang::txt('PLG_HUBZERO_COMMENTS_CONFIRM'); ?>"><!--
                        ...
                        <?php if ($canReply) { ?>
                            <?php
                            $cancelTxt = Lang::txt('PLG_HUBZERO_COMMENTS_CANCEL');
                            $replyTxt = Lang::txt('PLG_HUBZERO_COMMENTS_REPLY');
                        ...
                        <?php } ?>
                            <a class="icon-abuse abuse"
                                data-txt-flagged="<?php echo Lang::txt('PLG_HUBZERO_COMMENTS_REPORTED_AS_ABUSIVE'); ?>"
                ...
                <?php if ($canReply) { ?>
                    <div class="addcomment hide" id="comment-form<?php echo $this->comment->get('id'); ?>">
                        <form action="<?php echo Route::url($this->comment->link('base')); ?>"
                            method="post"
                            enctype="multipart/form-data">
                            <fieldset>
                                <legend>
                                    <?php
                                    $replyingToName = !$this->comment->get('anonymous')
                                        ? $this->comment->get('name')
                                        : Lang::txt('JANONYMOUS');
                                    $replyingToTxt = Lang::txt(
                                        'PLG_HUBZERO_COMMENTS_REPLYING_TO',
                                        $replyingToName
                                    );
                                    ?>
                                    <span><?php echo $replyingToTxt; ?></span>
                                </legend>

                                <input type="hidden" name="comment[id]" value="0" />
                                <input type="hidden"
                                    name="comment[item_id]"
                                    value="<?php echo $this->escape($this->comment->get('item_id')); ?>"/>
                                <input type="hidden"
                                    name="comment[item_type]"
                                    value="<?php echo $this->escape($this->comment->get('item_type')); ?>"/>
                                <input type="hidden"
                                    name="comment[parent]"
                                ...
                                <div class="form-group">
                                    ...
                                    $enterTxt = Lang::txt('PLG_HUBZERO_COMMENTS_ENTER_COMMENTS');
                                    ?>
                                ...
                                <div class="form-group">
                                    <?php
                                    $fileId = 'comment-' . $commentId . '-file';
                                    $attachTxt = Lang::txt('PLG_HUBZERO_COMMENTS_ATTACH_FILE');
                                    ?>
                                    <label
                                        class="form-check-label <?php echo $fileId; ?>"
                                        for="<?php echo $fileId; ?>">
                                        <span class="label-text">
                                            <?php echo $attachTxt; ?>:
                                        </span>
