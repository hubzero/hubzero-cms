...
// phpcs:ignoreFile PSR1.Classes.ClassDeclaration.MissingNamespace, PSR1.Classes.ClassDeclaration.MultipleClasses
...
class ActivitiesCollection extends Collection
{
    ...
    {
        $response =  $this->CTCTRequest->makeRequest(
            $this->uri,
            'POST',
            $postString,
...
class ContactsCollection extends Collection
{
    ...
    public function updateContact(Contact $Contact)
    {
        $contactXml = $Contact->createXml();
        $response = $this->CTCTRequest->makeRequest(
            $this->CTCTRequest->baseUri . $Contact->link,
            'PUT',
            $Contact->createXml()
        );
        return ($response['info']['http_code'] == 204) ? true : false;
    ...
    public function searchContactsByLastUpdate($List, $date)
    {
        ...
        try {
            ...
            if (in_array($List->name, $listTypes)) {
                $ext .= '&listtype=' . strtolower(str_replace(' ', '-', $List->name));
            } else {
                $ext .= '&listid=' . substr(strrchr($List->id, "/"), 1);
            }
...
class CampaignsCollection extends Collection
{
    ...
    {
        $response =  $this->CTCTRequest->makeRequest(
            $this->CTCTRequest->baseUri . $Campaign->link,
            'PUT',
            $Campaign->createXml()
        );
        return ($response['info']['http_code'] == 204) ? true : false;
    ...
    public function getCampaignsByStatus($status, $page = null)
    {
        ...
        try {
            if (!in_array($status, $statusList)) {
                throw new CTCTException("Campaign status '" . $status . "' is not a valid option");
            }
    ...
    public function scheduleCampaign(Campaign $Campaign, $time)
    {
        ...
        $response =  $this->CTCTRequest->makeRequest(
            $this->CTCTRequest->baseUri . $Campaign->link . '/schedules',
            'POST',
            $schedXml
        );
        $parsedResponse = simplexml_load_string($response['xml']);
    ...
    {
        $response = $this->CTCTRequest->makeRequest(
            $this->CTCTRequest->baseUri . $Campaign->link . '/schedules/1',
            'DELETE'
        );
...
class LibraryCollection extends Collection
{
    ...
    public function uploadImage($imageLocation, $folderUrl)
    {
        ...
        try {
            ...
            if (empty($file)) {
                throw new CTCTException('Unable to get file contents from: ' . $imageLocation);
            }
            if (empty($md5Hash)) {
                throw new CTCTException('Unable to create md5Hash from: ' . $imageLocation);
            }
        } catch (CTCTException $e) {
