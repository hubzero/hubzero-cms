...
class Hubzero extends Plugin
{
    ...
    public function onUserLogin($user, $options = array())
    {
        ...
        if (
            (App::get('config')->get('session_handler') != 'database' && (time() % 2 || $session->isNew()))
...
// phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    ...
    public function runSelectQuery($query)
    {
        ...
        // json_encode: returns a string containing the JSON representation from the mySQL
        // -> json_decode: Returns the value encoded in json in appropriate PHP type
    ...
    public function onUserDeidentify($user_id)
    {
        ...
        // NOTE: moved the insert (pre deletes) and updated (post deletes) profile key SQL
        // statement to controller com_members/admin/controllers/members.php
        $delete_UserProfile_Query = "DELETE from `#__user_profiles` where user_id =" . $db->quote($userId)
            . " AND 'profile_key' !='edulevel' AND profile_key !='gender' AND profile_key !='hispanic'"
            . " AND profile_key !='organization' AND profile_key !='orgtype' AND profile_key !='race'"
            . " AND profile_key !='reason'";
        $this->runUpdateOrDeleteQuery($delete_UserProfile_Query);

        $update_SupportTicketsByEmail_Query = "UPDATE `#__support_tickets` set login='',"
            . " ip='', email='', hostname='', name='' where email=" . $db->quote($userEmail);
        $this->runUpdateOrDeleteQuery($update_SupportTicketsByEmail_Query);

        $update_SupportTicketsByLogin_Query = "UPDATE `#__support_tickets` set login='',"
            . " ip='', email='', hostname='', name='' where login=" . $db->quote($userName);
        ;
        ...
        $delete_ProfileCompletionAward_Query = "DELETE from `#__profile_completion_awards`"
            . " where user_id=" . $db->quote($userId);
        $this->runUpdateOrDeleteQuery($delete_ProfileCompletionAward_Query);

        $update_NewsletterMailingRecipientActions_Query = "UPDATE `#__newsletter_mailing_recipient_actions`"
            . " set email='',ip='' where email=" . $db->quote($userEmail);
        $this->runUpdateOrDeleteQuery($update_NewsletterMailingRecipientActions_Query);

        $update_NewsletterMailingRecipients_Query = "UPDATE `#__newsletter_mailing_recipients`"
            . " set email='' where email=" . $db->quote($userEmail);
        $this->runUpdateOrDeleteQuery($update_NewsletterMailingRecipients_Query);

        $update_NewsletterMailingListEmails_Query = "UPDATE `#__newsletter_mailinglist_emails`"
            . " set email='' where email=" . $db->quote($userEmail);
        $this->runUpdateOrDeleteQuery($update_NewsletterMailingListEmails_Query);

        $update_NewsletterMailingListUnsubscribes_Query = "UPDATE `#__newsletter_mailinglist_unsubscribes`"
            . " set email='' where email=" . $db->quote($userEmail);
        ...
        $update_MediaTrackingDetailed_Query = "UPDATE `#__media_tracking_detailed`"
            . " set ip_address='' where user_id=" . $db->quote($userId);
        $this->runUpdateOrDeleteQuery($update_MediaTrackingDetailed_Query);

        $update_MediaTracking_Query = "UPDATE `#__media_tracking` set ip_address=''"
            . " where user_id=" . $db->quote($userId);
        ...
        $update_EventRegistration_Query = "UPDATE `#__event_registration` set name='',"
            . " email='', phone='', address='', city='', zip='', username=" . $db->quote($anonUserName)
            . " where username=" . $db->quote($userName);
        $this->runUpdateOrDeleteQuery($update_EventRegistration_Query);

        $delete_CartSavedAddresses_Query = "DELETE from `#__cart_saved_addresses`"
            . " where uidNumber=" . $db->quote($userId);
        ...
        $delete_XProfilesDisability_Query = "DELETE from `#__xprofiles_disability`"
            . " where uidNumber=" . $db->quote($userId);
        ...
        $delete_UsersQuotasLogByUserName_Query = "DELETE from `#__users_quotas_log`"
            . " where name=" . $db->quote($userName);
        $this->runUpdateOrDeleteQuery($delete_UsersQuotasLogByUserName_Query);

        $delete_UsersQuotasLogByActorId_Query = "DELETE from `#__users_quotas_log`"
            . " where actor_id=" . $db->quote($userId);
        ...
        $delete_UsersPasswordHistory_Query = "DELETE from `#__users_password_history`"
            . " where user_id=" . $db->quote($userId);
        $this->runUpdateOrDeleteQuery($delete_UsersPasswordHistory_Query);

        $update_UsersPointsSubscription_Query = "UPDATE `#__users_points_subscriptions`"
            . " set contact=" . $db->quote($anonUserName) . " where uid=" . $db->quote($userId);
        ...
        $update_XProfilesById_Query = "UPDATE `#__xprofiles` set name=" . $db->quote($anonUserNameSpace) . ","
            . " username=" . $db->quote($anonUserName) . ", userPassword=" . $db->quote($anonPassword) . ","
            . " url='', phone='', regHost='', regIP='', givenName=" . $db->quote($anonUserName) . ","
            . " middleName='', surname='anonSurName', picture='', public=0, params='', note='', orcid='',"
            . " homeDirectory='/home/anonymous', email=" . $db->quote($anonUserName . "@example.com")
            . " where uidNumber =" . $db->quote($userId);
