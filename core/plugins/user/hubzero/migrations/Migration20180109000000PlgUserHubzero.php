...
class Migration20180109000000PlgUserHubzero extends Base
{
    ...
    public function up()
    {
        ...
        if ($this->db->tableExists('#__extensions')) {
            $this->db->setQuery("SELECT extension_id FROM `#__extensions` "
                . "WHERE `type`='plugin' AND `folder`='user' AND `element`='joomla'");
            $id = $this->db->loadResult();
        }

        if ($id) {
            $this->db->setQuery("UPDATE `#__extensions` SET `element`='hubzero', "
                . "`name`='plg_user_hubzero' WHERE `extension_id`=" . $id);
            $this->db->query();
        } else {
