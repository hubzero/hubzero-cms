...
class Members extends Plugin
{
    ...
    public function onGroup($group, $option, $authorized, $limit, $limitstart, $action, $access, $areas = null)
    {
        ...
        if ($returnhtml) {
            ...
            //     .$this->group->description.': '.Lang::txt('PLG_GROUPS_MEMBERS')
            // );
            ...
            if (!$arr['html']) {
                ...
                $view->q = Request::getString('q', '');
                $view->filter = Request::getWord('filter', '');
                ...
                $view->current_inviteemails = $group_inviteemails->getInviteEmails($gidNumber, true);

                switch ($view->filter) {
                    case 'invitees':
                        $view->groupusers = $view->q
                            ? $group->search('invitees', $view->q)
                            : $group->get('invitees');
                    ...
                    case 'pending':
                        $view->groupusers = $view->q
                            ? $group->search('applicants', $view->q)
                            : $group->get('applicants');
                        $view->managers   = array();
                        break;
                    case 'managers':
                        $view->groupusers = $view->q
                            ? $group->search('managers', $view->q)
                            : $group->get('managers');
                        $view->groupusers = $view->role_filter
                            ? \Hubzero\User\Group\Helper::search_roles($group, $view->role_filter)
                            : $view->groupusers;
        ...
        if (count($pending) > 0 && in_array(User::get('id'), $group->get("managers"))) {
            $title = Lang::txt(
                'PLG_GROUPS_MEMBERS_GROUP_HAS_REQUESTS',
                $group->get('description'),
                count($pending)
            );
            $link = Route::url(
                'index.php?option=com_groups&cn=' . $this->group->get('cn') . '&active=members&filter=pending'
            );
            $alertHtml = '<a class="alrt" href="' . $link . '"><span><h5>';
            $alertHtml .= Lang::txt('PLG_GROUPS_MEMBERS_ALERT') . '</h5>' . $title . '</span></a>';
            $arr['metadata']['alert'] = $alertHtml;
        }
    ...
    private function approve()
    {
        ...
        // Note: we use two different lists to avoid situations where the user is already a
        // member but somehow an applicant too. Recording the list of applicants for removal
        // separate allows for removing the duplicate entry from the applicants list without
        // trying to add them to the members list (which they already belong to).
        $users = array();
        $applicants = array();
                ...
                Event::trigger('system.logActivity', [
                    'activity' => [
                        ...
                        'description' => Lang::txt(
                            ...
                            '<a href="' . Route::url('index.php?option=com_members&id=' . $uid) . '">'
                                . $targetuser->get('name') . '</a>',
                            '<a href="' . Route::url('index.php?option=com_groups&cn=' . $this->group->get('cn'))
    ...
    private function promote()
    {
        ...
        $redirectUrl = 'index.php?option=com_groups&cn=' . $this->group->get('cn');
        $redirectUrl .= '&active=members&filter=' . $filter . '&limit=' . $limit . '&limitstart=' . $start;
        App::redirect(Route::url($redirectUrl));
    }
                ...
                Event::trigger('system.logActivity', [
                    'activity' => [
                        ...
                        'description' => Lang::txt(
                            ...
                            '<a href="' . Route::url('index.php?option=com_members&id=' . $targetuser->get('id'))
                                . '">' . $targetuser->get('name') . '</a>',
                            '<a href="' . Route::url('index.php?option=com_groups&cn=' . $this->group->get('cn'))
                                . '">' . $this->group->get('description') . '</a>'
                        ),
    ...
    private function remove()
    {
        ...
        // Set the page title
        $pageTitle = Lang::txt(strtoupper($this->name)) . ': ' . $this->group->get('description');
        $pageTitle .= ': ' . Lang::txt(strtoupper($this->action));
    ...
    private function add()
    {
        ...
        $redirectUrl = 'index.php?option=com_groups&cn=' . $this->group->get('cn');
        $redirectUrl .= '&task=invite&return=members';
        App::redirect(Route::url($redirectUrl), '', 'message', true);
    }
    ...
    private function confirmcancel()
    {
        ...
        $redirectUrl = 'index.php?option=com_groups&cn=' . $this->group->get('cn');
        $redirectUrl .= '&active=members&filter=invitees';
        App::redirect(Route::url($redirectUrl), '', '', true);
    }
    ...
    private function submitrole()
    {
        ...
        $db = App::get('db');
        $sql = "INSERT INTO `#__xgroups_member_roles` (roleid,uidNumber) VALUES ("
            . $db->Quote($role) . "," . $db->Quote($uid) . ")";
        $db->setQuery($sql);
        $db->query();
    ...
    private function notifyUser($targetuser)
    {
        ...
        switch (strtolower($this->action)) {
            case 'approve':
                ...
                // Message
                $plain  = "Your request for membership in the " . $group->get('description');
                $plain .= " group has been approved.\r\n";
            ...
            case 'confirmdeny':
                // Incoming
                $reason = Request::getString('reason', '', 'post');
                ...
                // Message
                $plain  = "Your request for membership in the " . $group->get('description');
                $plain .= " group has been denied.\r\n\r\n";
                if ($reason) {
                    $plain .= stripslashes($reason) . "\r\n\r\n";
                }
                $plain .= "If you feel this is in error, you may try to join the group again, \r\n";
                $plain .= "this time better explaining your credentials and ";
                $plain .= "reasons why you should be accepted.\r\n\r\n";
            ...
            case 'confirmremove':
                ...
                // Message
                $plain  = "Your membership in the " . $group->get('description');
                $plain .= " group has been cancelled.\r\n\r\n";
                if ($reason) {
                    $plain .= stripslashes($reason) . "\r\n\r\n";
                }
                $plain .= "If you feel this is in error, you may try to join the group again ";
            ...
            case 'confirmcancel':
                ...
                // Message
                $plain  = "Your invitation for membership in the " . $group->get('description');
                $plain .= " group has been cancelled.\r\n\r\n";
                if ($reason) {
                    $plain .= stripslashes($reason) . "\r\n\r\n";
                }
                $plain .= "If you feel this is in error, you may try to join the group ";
                $plain .= "by going to:\r\n";
                $plain .= Request::base() . $sef . "\r\n";
                break;
        }
