...
class Usage extends Plugin
{
    ...
    public function onGroup($group, $option, $authorized, $limit, $limitstart, $action, $access, $areas = null)
    {
        ...
        if ($return == 'html') {
            ...
            //get the group pages
            $query = "SELECT id, title FROM `#__xgroups_pages` WHERE state=1 "
                . "AND gidNumber=" . $database->quote($group->get('gidNumber'));
    ...
    public static function getWikipageCount($gid, $authorized)
    {
        ...
        $database = App::get('db');
        $sql = "SELECT COUNT(*) FROM `#__wiki_pages` AS p "
            . "WHERE p.scope=" . $database->quote('group') . " "
            . "AND p.scope_id=" . $database->quote($gid);
    ...
    public static function getWikifileCount($gid, $authorized)
    {
        ...
        $database = App::get('db');
        $sql = "SELECT id FROM `#__wiki_pages` AS p "
            . "WHERE p.scope=" . $database->quote('group') . " "
            . "AND p.scope_id=" . $database->quote($gid);
        $database->setQuery($sql);
        ...
        if ($pageids) {
            ...
            $sql = "SELECT COUNT(*) FROM `#__wiki_attachments` "
    ...
    public function getGroupPageViews($gid, $pageid, $start, $end)
    {
        ...
        $query = "SELECT * FROM `#__xgroups_pages_hits` "
            . "WHERE `gidNumber`=" . $database->quote($gid) . " "
            . "AND `date` > " . $database->quote($start) . " "
            . "AND `date` < " . $database->quote($end);
        if ($pageid != '') {
            $query .= " AND `pageid`=" . $database->quote($pageid);
        }
        $query .= " GROUP BY ip ORDER BY `date` ASC";
        $database->setQuery($query);
    ...
    public static function getGroupBlogCommentCount($gid)
    {
        ...
        $query = "SELECT count(*) FROM `#__blog_entries` as be, `#__blog_comments` as bc "
            . "WHERE be.scope='group' AND be.scope_id=" . $database->quote($gid) . " "
            . "AND be.id=bc.entry_id";
        $database->setQuery($query);
    ...
    public static function getGroupCalendarCount($gid)
    {
        ...
        $query = "SELECT COUNT(*) FROM `#__events` "
            . "WHERE scope=" . $database->quote('group') . " "
            . "AND scope_id=" . $database->quote($gid) . " AND state=1";
        $database->setQuery($query);
        return $database->loadResult();
    }
}
