...
class Calendar extends Plugin
{
    ...
    public function onGroup($group, $option, $authorized, $limit, $limitstart, $action, $access, $areas = null)
    {
        ...
        if ($upcoming_events > 0) {
            ...
            $link = Route::url(
                'index.php?option=com_groups&cn=' . $this->group->get('cn') . '&active=calendar'
            );
            $arr['metadata']['alert'] = '<a class="alrt" href="' . $link . '">';
            $arr['metadata']['alert'] .= '<span><h5>Calendar Alert</h5>' . $title . '</span></a>';
        }
    ...
    private function eventSources()
    {
        ...
        $baseUrl = 'index.php?option=com_groups&cn=' . $this->group->get('cn');
    ...
    public function events()
    {
        ...
        foreach ($rawEvents as $rawEvent) {
            ...
            $event->start = ($event->allDay == 1)
                ? $up->setTimezone('UTC')->format($timeFormat, true)
                : $up->toTimezone($timezone, $timeFormat, $ignoreDst);
            $event->className = ($rawEvent->get('calendar_id'))
                ? 'calendar-' . $rawEvent->get('calendar_id')
                : 'calendar-0';
            if ($rawEvent->get('publish_down') && $rawEvent->get('publish_down') != '0000-00-00 00:00:00') {
                $event->end = ($event->allDay == 1)
                    ? $down->setTimezone('UTC')->format($timeFormat, true)
                    : $down->toLocal($timeFormat, $ignoreDst);
            }
    ...
    private function edit()
    {
        ...
        if (!in_array($this->user->get('id'), $this->group->get('members'))) {
            $calUrl = 'index.php?option=' . $this->option . '&cn=' . $this->group->get('cn');
            $calUrl .= '&active=calendar&year=' . $this->year . '&month=' . $this->month;
            App::redirect(
                Route::url($calUrl),
        ...
        if ($view->event->get('id')) {
            ...
            if ($eventCalendar->isSubscription()) {
                $detailsUrl = 'index.php?option=' . $this->option . '&cn=' . $this->group->get('cn');
                $detailsUrl .= '&active=calendar&action=details&event_id=' . $view->event->get('id');
                App::redirect(
                    Route::url($detailsUrl),
    ...
    private function save()
    {
        ...
        // repeating rule
        $event['repeating_rule'] = $this->buildRepeatingRule();
        ...
        $extraInfo = isset($event['extra_info']) && $event['extra_info'];
        if ($extraInfo && ! \Hubzero\Utility\Validate::url($event['extra_info'])) {
        ...
        $publishDown = isset($event['publish_down']) && $event['publish_down'];
        if ($publishDown && $event['publish_down'] != '0000-00-00 00:00:00') {
        ...
        $hasEmail = isset($event['email']) && $event['email'] != '';
        if ($registration && $hasEmail && !filter_var($event['email'], FILTER_VALIDATE_EMAIL)) {
        ...
        $detailsUrl = 'index.php?option=' . $this->option . '&cn=' . $this->group->get('cn');
        $detailsUrl .= '&active=calendar&action=details&event_id=' . $eventsModelEvent->get('id');
        App::redirect(
            Route::url($detailsUrl),
            $message,
            'passed'
    ...
    private function delete()
    {
        ...
        if ($this->user->get('id') != $eventsModelEvent->get('created_by') && $this->authorized != 'manager') {
            ...
            $calUrl = 'index.php?option=' . $this->option . '&cn=' . $this->group->get('cn');
            $calUrl .= '&active=calendar&year=' . $year . '&month=' . $month;
            App::redirect(
                Route::url($calUrl),
    ...
    private function subscribe()
    {
        ...
        if ($plugin_access == 'registered' || $plugin_access == 'members') {
            ...
            $notMember = !is_object($auth) && !in_array($auth->id, $this->group->get('members'));
            if ($plugin_access == 'members' && $notMember) {
        ...
        $eventsCalendarArchive = \Components\Events\Models\Calendar\Archive::getInstance();
        $subscriptionName = '[' . Config::get('sitename') . '] Group Calendar: ';
        $subscriptionName .= $this->group->get('description');
    ...
    {
        $realm = '[' . Config::get('sitename') . '] Group Calendar: ';
        $realm .= $this->group->get('description');
        ...
        $emptyUser = !isset($httpBasicUsername) || $httpBasicUsername == '';
        $emptyPass = !isset($httpBasicPassword) || $httpBasicPassword == '';
        ...
        if (!is_object($user) || $user->id == '' || $user->id == 0) {
            $logMsg = $httpBasicUsername . ' ' . $_SERVER['REMOTE_ADDR'];
            $logMsg .= ' invalid group calendar subscription auth for ' . $this->group->get('cn');
            App::get('log')->logger('auth')->info($logMsg);
    ...
    private function register()
    {
        ...
        if ($registerby >= $now) {
            ...
            $restricted = $view->event->get('restricted');
            $badPassword = $restricted != '' && $restricted != $password && !isset($this->register);
            if ($badPassword) {
    ...
    private function doRegister()
    {
        ...
        if ($params->get('show_email', 1) == 1) {
            $emailEmpty = !isset($register['email']) || $register['email'] == '';
            $emailInvalid = !filter_var($register['email'], FILTER_VALIDATE_EMAIL);
            ...
            $existingReg = \Components\Events\Tables\Respondent::checkUniqueEmailForEvent(
                $register['email'],
                $event_id
            );
            if ($existingReg > 0) {
        ...
        } elseif (isset($register['position'])) {
        ...
        $hasDisability = isset($disability) && strtolower($disability) == 'yes';
        $eventsRespondent->disability_needs = $hasDisability ? 1 : null;
        $hasDietaryNeeds = isset($dietary['needs']) && strtolower($dietary['needs']) == 'yes';
        $eventsRespondent->dietary_needs    = $hasDietaryNeeds ? $dietary['specific'] : null;
        ...
        $sql = "INSERT INTO `#__events_respondent_race_rel` (respondent_id, race, tribal_affiliation)
		        VALUES (" . $this->database->quote($eventsRespondent->id) . ", "
                . $this->database->quote(implode(',', $r)) . ", "
                . $this->database->quote($race['nativetribe']) . ")";
        $this->database->setQuery($sql);
        $this->database->query();
        ...
        if ($eventsEvent->email != '') {
            ...
            //declare subject
            $subject = '[' . $email->sitename . '] Group "' . $this->group->get('description');
            $subject .= '" Event Registration: ' . $eventsEvent->title;
            $subject = Lang::txt($subject);
            ...
            //send email
            $this->sendEmail($eventsEvent->email, $from, $subject, $message);
        }
        ...
        if ($params->get('show_email', 1) == 1) {
            ...
            $to      = User::get('email');
            $fromName = $email->sitename . ' Group Calendar: ' . $this->group->get('description');
            $from    = array('email' => 'groups@' . $_SERVER['HTTP_HOST'], 'name'  => $fromName);
        ...
        $detailsUrl = 'index.php?option=' . $this->option . '&cn=' . $this->group->get('cn');
        $detailsUrl .= '&active=calendar&action=details&event_id=' . $event_id;
        App::redirect(
            Route::url($detailsUrl),
    ...
    private function download()
    {
        ...
        //var to hold output
        $output = 'First Name,Last Name,Title,Affiliation,Email,Website,Telephone,Fax,City,State,Zip,';
        $output .= 'Country,Current Position,Highest Degree Earned,Gender,Race,Arrival Info,';
        $output .= "Departure Info,Disability Needs,Dietary Needs,Attending Dinner,Abstract,Comments,Register Date\n";
    ...
    private function saveCalendar()
    {
        ...
        //inform and redirect
        $calendarsUrl = 'index.php?option=' . $this->option . '&cn=' . $this->group->get('cn');
        $calendarsUrl .= '&active=calendar&action=calendars';
        App::redirect(
            Route::url($calendarsUrl),
    ...
    private function refreshCalendar()
    {
        ...
        if (!$calendar->refresh(true)) {
            $errorMsg = 'Unable to sync the group calendar "%s". ';
            $errorMsg .= 'Please verify the calendar subscription URL is valid.';
            $this->setError(Lang::txt($errorMsg, $calendar->getError()));
     ...
     * @return  string
     */
    private function buildRepeatingRule()
    {
        $rules = array();
    ...
    private function getAllFutureEvents()
    {
        ...
        $sql = "SELECT COUNT(*)
				...
				AND state=1
				AND (publish_up >=" . $db->quote(Date::of()->toSql()) . "
				OR publish_down >= " . $db->quote(Date::of()->toSql()) . ")";
        $db->setQuery($sql);
    ...
    {
        $db = App::get('db');
        $now = $db->quote(Date::of()->toSql());
        $endOfMonth = $db->quote(Date::of()->format("Y-m-t 23:59:59"));
        $sql = "SELECT COUNT(*)
				FROM `#__events`
				WHERE scope=" . $db->quote('group') . "
				AND scope_id=" . $this->group->get('gidNumber') . "
				AND state=1
				AND (publish_up >= " . $now . " OR publish_down >= " . $now . ")
				AND publish_up <= " . $endOfMonth;
        $db->setQuery($sql);
        return $db->loadResult();
    }
