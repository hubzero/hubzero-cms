...
class IcalParser
{
    ...
    public function __construct($file, $callback = null)
    {
        if (strpos($file, DS) === false) {
            $this->parseString($file, $callback);
        } else {
            $this->parseFile($file, $callback);
        }
    }
    ...
    public function parseString($string, $callback = null)
    {
        ...
        foreach (explode("\n", $string) as $row) {
            ...
            list($key, $middle, $value, $allDay) = $this->parseRow($row);
            ...
            if ($callback) {
                ...
                if ($section === 'VCALENDAR') {
                    ...
                    if ($key == 'DTSTART' || $key == 'DTEND') {
                        $this->data[$section][$counters[$section]]['ALLDAY'] = $allDay;
                    }
                }
            }
        }
    ...
    private function parseRow($row)
    {
        ...
        $dateKeys = array('DTSTAMP', 'LAST-MODIFIED', 'CREATED', 'DTSTART', 'DTEND');
        if (in_array($key, $dateKeys)) {
            ...
            // Checks if time contains the value Z, which indicates it is
            // not necessary to convert a timezone since it is already in UTC
            if (substr($value, -1) != 'Z') {
        ...
        $rrulePattern = '#(?<key>[^=;]+)=(?<value>[^;]+)#';
        if ($key === 'RRULE' && preg_match_all($rrulePattern, $value, $matches, PREG_SET_ORDER)) {
            $middle = null;
            $value = array();
            foreach ($matches as $match) {
        ...
        $text_properties = array(
            'CALSCALE', 'METHOD', 'PRODID', 'VERSION', 'CATEGORIES', 'CLASS',
            'COMMENT', 'DESCRIPTION', 'LOCATION', 'RESOURCES', 'STATUS', 'SUMMARY',
            'TRANSP', 'TZID', 'TZNAME', 'CONTACT', 'RELATED-TO', 'UID', 'ACTION',
            'REQUEST-STATUS'
        );
        ...
        return array($key, $middle, $value, $allDay);
    }
    ...
    /**
     * Return reverse sorted eventlist as array or false if calenar is empty
     *
     * @return array|boolean
     */
