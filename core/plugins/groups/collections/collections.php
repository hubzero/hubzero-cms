...
class Collections extends Plugin
{
    ...
    public function onGroupDelete($group)
    {
        ...
        $database = App::get('db');
        $sql = "SELECT id FROM `#__collections` WHERE `object_type`='group' AND `object_id`="
            . $database->quote($group->get('gidNumber'));
        ...
        if (count($entries) > 0) {
            ...
            $sql = "SELECT i.id FROM `#__collections_items` AS i "
                . "LEFT JOIN `#__collections_posts` AS p ON p.`item_id`=i.`id` "
                . "WHERE p.`original`=1 AND p.`collection_id` IN (" . implode(',', $entries) . ")";
            $database->setQuery($sql);
            $ids = $database->loadColumn();

            if ($ids && count($ids)) {
                // Mark all posts as "trashed"
                $sql = "UPDATE `#__collections_items` SET `state`=2 WHERE `id` IN ("
                    . implode(',', $ids) . ")";
                $database->setQuery($sql);
                $database->query();
            }
    ...
    public function onGroupDeleteCount($group)
    {
        ...
        $sql = "SELECT COUNT(*) FROM `#__collections` WHERE `object_type`="
            . $database->quote('group') . " AND `object_id`="
            . $database->quote($group->get('gidNumber'));
        $database->setQuery($sql);
    ...
    public function onGroup($group, $option, $authorized, $limit, $limitstart, $action, $access, $areas = null)
    {
        ...
        if ($return == 'html') {
            ...
            $editorOptions = array('class' => 'minimal no-footer');
            ...
            if (
                ...
                $groupUrl = 'index.php?option=com_groups&cn=' . $group->get('cn') . '&active=' . $active;
                $url = Route::url($groupUrl, false, true);
                App::redirect(
                        ...
                        case 'post':
                            ...
                            if (isset($bits[1])) {
                                if ($bits[1] == 'new' || $bits[1] == 'save') {
                                    ...
                                    if (isset($bits[2])) {
                                        $postActions = array(
                                            'post', 'vote', 'collect', 'remove',
                                            'move', 'comment', 'savecomment', 'deletecomment'
                                        );
                                        if (in_array($bits[2], $postActions)) {
                                            $this->action = $bits[2];
                                        } else {
            ...
            switch ($this->action) {
                ...
                case 'vote':
                    ...
                    break;
                case 'collect':
                    $arr['html'] = $this->repost();
                    break;
                case 'remove':
                    $arr['html'] = $this->remove();
                    break;
                case 'move':
                    $arr['html'] = $this->move();
                    break;
                ...
                case 'followcollection':
                    $arr['html'] = $this->follow('collection');
                    break;
                case 'unfollowcollection':
                    $arr['html'] = $this->unfollow('collection');
                    break;
                case 'collectcollection':
                    $arr['html'] = $this->repost();
                    break;
                case 'newcollection':
                    $arr['html'] = $this->newcollection();
                    break;
                case 'editcollection':
                    $arr['html'] = $this->editcollection();
                    break;
                case 'savecollection':
                    $arr['html'] = $this->savecollection();
                    break;
                case 'deletecollection':
                    $arr['html'] = $this->deletecollection();
                    break;
                case 'all':
                case 'collections':
                    $arr['html'] = $this->collections();
                    break;
    ...
    {
        $groupUrl = 'index.php?option=' . $this->option . '&cn=' . $this->group->get('cn');
        $groupUrl .= '&active=' . $this->_name;
        $route = Route::url($groupUrl, false, true);
     ...
     * @return  string
     */
    private function follow($what = 'collection')
    {
     ...
     * @return  string
     */
    private function unfollow($what = 'collection')
    {
        // Is the board restricted to logged-in users only?
        if (User::isGuest()) {
            return $this->login();
    ...
    private function save()
    {
        ...
        if ($tmp) {
            $db = App::get('db');
            $sql = "SELECT id FROM `#__collections_items` WHERE `title`=" . $db->quote($tmp)
                . " AND `id`!=" . $db->quote($item->get('id'));
            $db->setQuery($sql);
            $others = $db->loadColumn();

            if (count($others) > 0) {
                $sql = "SELECT * FROM `#__collections_assets` WHERE `item_id` in ("
                    . implode(",", $others) . ")";
                $db->setQuery($sql);
        ...
        if ($this->getError()) {
            ...
            return $this->edit($post->item()->bind($fields));
        }
        ...
        $url = 'index.php?option=' . $this->option . '&cn=' . $this->group->get('cn');
        $url .= '&active=' . $this->_name . '&scope=' . $collection->get('alias');
        ...
        $collectionLink = '<a href="' . Route::url($collection->link()) . '">'
            . $collection->get('title') . '</a>';

        $groupUrl = Route::url('index.php?option=com_groups&cn=' . $this->group->get('cn'), false);
        $groupTitle = '<a href="' . $groupUrl . '">' . $this->group->get('description') . '</a>';

        $activityKey = 'PLG_GROUPS_COLLECTIONS_ACTIVITY_POST_' . ($p['id'] ? 'UPDATED' : 'CREATED');
        Event::trigger('system.logActivity', [
            'activity' => [
                'action'      => ($p['id'] ? 'updated' : 'created'),
                'scope'       => 'collections.item',
                'scope_id'    => $item->get('id'),
                'description' => Lang::txt($activityKey, $itemTitle, $collectionLink, $groupTitle),
                'details'     => array(
    ...
    private function repost()
    {
        ...
        $activityDesc = Lang::txt(
            'PLG_GROUPS_COLLECTIONS_ACTIVITY_POST_CREATED',
            $itemTitle,
            $collectionLink,
            $groupTitle
        );
        ...
        if ($no_html) {
            $repostCount = $post->getCount(array('item_id' => $post->get('item_id'), 'original' => 0));
            echo Lang::txt('PLG_GROUPS_COLLECTIONS_POST_REPOSTS', $repostCount);
            exit;
        }
    ...
    private function remove()
    {
        ...
        $routeUrl = 'index.php?option=' . $this->option . '&cn=' . $this->group->get('cn');
        $routeUrl .= '&active=' . $this->_name . '&scope=' . $collection->get('alias');
        $route = Route::url($routeUrl);
        ...
        $collectionLink = '<a href="' . $route . '">' . $collection->get('title') . '</a>';
        Event::trigger('system.logActivity', [
            'activity' => [
                'action'      => 'deleted',
                'scope'       => 'collections.post',
                'scope_id'    => $post->get('id'),
                'description' => Lang::txt('PLG_GROUPS_COLLECTIONS_ACTIVITY_POST_DELETED', $collectionLink),
                'details'     => array(
    ...
    private function delete()
    {
        ...
        $itemLink = '<a href="' . $route . '">' . $item->get('title') . '</a>';
        Event::trigger('system.logActivity', [
            'activity' => [
                'action'      => 'deleted',
                'scope'       => 'collections.item',
                'scope_id'    => $item->get('id'),
                'description' => Lang::txt('PLG_GROUPS_COLLECTIONS_ACTIVITY_ITEM_DELETED', $itemLink),
                'details'     => array(
                    'collection_id' => $post->get('collection_id'),
                    'post_id'       => $post->get('id'),
                    'item_id'       => $item->get('id'),
    ...
    private function savecomment()
    {
        ...
        $url = Route::url(
            'index.php?option=com_collections&controller=posts&post=' . $post->get('id') . '&task=comment'
        );

        $activityKey = 'PLG_GROUPS_COLLECTIONS_ACTIVITY_COMMENT_' . ($data['id'] ? 'UPDATED' : 'CREATED');
        $titleLink = '<a href="' . $url . '#c' . $comment->get('id') . '">' . $title . '</a>';
        Event::trigger('system.logActivity', [
            'activity' => [
                'action'      => ($data['id'] ? 'updated' : 'created'),
                'scope'       => 'collections.comment',
                'scope_id'    => $comment->get('id'),
                'description' => Lang::txt($activityKey, $comment->get('id'), $titleLink),
                'details'     => array(
                    'collection_id' => $post->get('collection_id'),
                    'post_id'       => $post->get('id'),
                    'item_id'       => $post->get('item_id'),
    ...
    private function deletecomment()
    {
        ...
        $titleLink = '<a href="' . Route::url($entry->link()) . '">' . $title . '</a>';
        Event::trigger('system.logActivity', [
            'activity' => [
                'action'      => 'deleted',
                'scope'       => 'collections.comment',
                ...
                'description' => Lang::txt(
                    'PLG_GROUPS_COLLECTIONS_ACTIVITY_COMMENT_DELETED',
                    $comment->get('id'),
                    $titleLink
                ),
                'details'     => array(
    ...
    private function vote()
    {
        ...
        $urlPath = 'index.php?option=' . $this->option . '&cn=' . $this->group->get('cn');
        $urlPath .= '&active=' . $this->_name . '&scope=' . $collection->get('alias');
        $url = Route::url($urlPath);
        ...
        $collectionLink = '<a href="' . $url . '">' . $collection->get('title') . '</a>';
        Event::trigger('system.logActivity', [
            'activity' => [
                'action'      => 'voted',
                'scope'       => 'collections.item',
                'scope_id'    => $post->item()->get('id'),
                'description' => Lang::txt('PLG_GROUPS_COLLECTIONS_ACTIVITY_ITEM_VOTED', $collectionLink),
                'details'     => array(
                    'collection_id' => $collection->get('id'),
                    'post_id'       => $post->get('id'),
     ...
     * @return  string
     */
    private function editcollection($row = null)
    {
        // Login check
        if (User::isGuest()) {
            return $this->login();
        ...
        // Access check
        $canCreate = $this->params->get('access-create-collection');
        $canEdit = $this->params->get('access-edit-collection');
    ...
    private function savecollection()
    {
        ...
        $activityKey = 'PLG_GROUPS_COLLECTIONS_ACTIVITY_COLLECTION_'
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt($activityKey, $collectionLink),
                'details'     => array(
    ...
    private function deletecollection()
    {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope_id'    => $collection->get('id'),
                'description' => Lang::txt(
                    'PLG_GROUPS_COLLECTIONS_ACTIVITY_COLLECTION_DELETED',
                    $collectionLink
                ),
                'details'     => array(
                    'title' => $collection->get('title'),
