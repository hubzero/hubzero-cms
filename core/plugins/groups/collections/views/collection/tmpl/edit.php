        ...
        <div class="form-group">
            <label for="field-access">
                ...
                <select name="fields[access]" id="field-access" class="form-control">
                    <option value="0"<?php if ($this->entry->get('access', $default) == 0) {
                        echo ' selected="selected"';
                                     } ?>><?php echo Lang::txt('PLG_GROUPS_COLLECTIONS_FIELD_PRIVACY_PUBLIC'); ?></option>
                    <option value="1"<?php if ($this->entry->get('access', $default) == 1) {
                        echo ' selected="selected"';
                                     } ?>><?php echo Lang::txt('PLG_GROUPS_COLLECTIONS_FIELD_PRIVACY_REGISTERED'); ?></option>
                    <option value="4"<?php if ($this->entry->get('access', $default) == 4) {
                        echo ' selected="selected"';
                                     } ?>><?php echo Lang::txt('PLG_GROUPS_COLLECTIONS_FIELD_PRIVACY_PRIVATE'); ?></option>
                </select>
            </label>
        </div>
                    ...
                    <label for="field-layout" class="form-control<?php if ($this->task == 'save' && !$this->entry->get('layout')) {
                        ...
                        <select name="fields[layout]" id="field-layout">
                            <option value="grid"<?php if ($this->entry->get('layout') == 'grid') {
                                echo ' selected="selected"';
                                                } ?>><?php echo Lang::txt('PLG_GROUPS_COLLECTIONS_FIELD_LAYOUT_GRID'); ?></option>
                            <option value="list"<?php if ($this->entry->get('layout') == 'list') {
                                echo ' selected="selected"';
                                                } ?>><?php echo Lang::txt('PLG_GROUPS_COLLECTIONS_FIELD_LAYOUT_LIST'); ?></option>
                        </select>
                    </label>
                </div>
            </div>
                    ...
                    <label for="field-sort" class="form-control<?php if ($this->task == 'save' && !$this->entry->get('sort')) {
                        ...
                        <select name="fields[sort]" id="field-sort">
                            <option value="created"<?php if ($this->entry->get('sort') == 'created') {
                                echo ' selected="selected"';
                                                   } ?>><?php echo Lang::txt('PLG_GROUPS_COLLECTIONS_FIELD_SORT_CREATED'); ?></option>
                            <option value="ordering"<?php if ($this->entry->get('sort') == 'ordering') {
                                echo ' selected="selected"';
                                                    } ?>><?php echo Lang::txt('PLG_GROUPS_COLLECTIONS_FIELD_SORT_ORDERING'); ?></option>
                        </select>
                    </label>
                </div>
            </div>
