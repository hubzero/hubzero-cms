    ...
    public function onGroup($group, $option, $authorized, $limit, $limitstart, $action, $access, $areas = null)
    {
        ...
        if ($return == 'html') {
            ...
            if (
                ...
                $groupUrl .= '&active=' . $active . '&area=' . $area;
                $url = Route::url($groupUrl);
        ...
        $pluginPath = DS . 'plugins' . DS . $this->_type . DS . $this->_name;
                    ...
                    // Add extra variables to let the tab view know we need a different base url
                    Request::setVar('tab_active_key', 'tab_active');
                    $tabBaseUrl = 'index.php?option=' . $this->option;
                    $tabBaseUrl .= '&cn=' . $this->group->cn . '&active=resources';
                    Request::setVar('tab_base_url', $tabBaseUrl);
                    // Added a noview variable to indicate to the controller that
                    // we do not want it to try to display the view, simply build it
    ...
    public function getResources(
        ...
        if ($limit) {
            ...
            if ($rows) {
                ...
                $baseUrl = 'index.php?option=com_groups&cn=' . $this->group->cn . '&active=resources';
                foreach ($rows as $key => $row) {
                    if ($this->group->type == '3') {
                        if ($row->alias) {
                            $href = Route::url($baseUrl . '&alias=' . $row->alias);
                        } else {
                            $href = Route::url($baseUrl . '&id=' . $row->id);
                        }
                    } else {
            ...
            foreach ($ares as $area => $val) {
                if (is_array($val)) {
                    ...
                    foreach ($val as $a => $t) {
                        if ($limitstart == -1) {
                            if ($i == 0) {
                                $aclTable = 'jos_resource_acl_group';
                                $counts[] = self::allWithFilters($filters)
                                    ->join($aclTable, $aclTable . '.resource_id', 'jos_resources.id', 'left')
                                    ->total(true);
                            } else {
                                $counts[] = 0;
                            }
