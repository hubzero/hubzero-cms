...
class Citations extends Plugin
{
     ...
     * @var  string
     */
    public const PLUGIN_SCOPE = 'group';
    ...
    public function onGroup($group, $option, $authorized, $limit, $limitstart, $action, $access, $areas = null)
    {
        ...
        if ($returnhtml) {
            ...
            if ($this->group->isSuperGroup()) {
                $uploadPath = trim(Component::params('com_groups')->get('uploadpath', '/site/groups'), DS);
                $this->path = PATH_APP . DS . $uploadPath . DS . $this->group->get('gidNumber');
                $this->path .= DS . 'template' . DS . 'html' . DS . 'plg_' . $this->_type . '_' . $this->_name;
            }
    ...
    private function browse()
    {
        ...
        $obj = $this->filterHandler(Request::getArray('filters', array()), $this->group->get('gidNumber'));
        ...
        $view->filters['type'] = $view->filters['type'] ?? "";
        $view->filters['tag'] = $view->filters['tag'] ?? "";
        $view->filters['author'] = $view->filters['author'] ?? "";
        $view->filters['publishedin'] = $view->filters['publishedin'] ?? "";
        $view->filters['year_start'] = $view->filters['year_start'] ?? "";
        $view->filters['year_end'] = $view->filters['year_end'] ?? "";
        $view->filters['startuploaddate'] = $view->filters['startuploaddate'] ?? "";
        $view->filters['enduploaddate'] = $view->filters['enduploaddate'] ?? "";
        ...
        // OpenURL
        $view->openurl = $this->handleOpenURL();
    ...
    private function edit()
    {
        ...
        $upperName = strtoupper($this->_name);
        $view->title = Lang::txt($upperName) . ': ' . Lang::txt($upperName . '_' . strtoupper($this->action));
    ...
    private function save()
    {
        ...
        $customID = Request::getString('custom4', '');
        if ($customID != '') {
        ...
        if ($authors->count()) {
            foreach ($authors as $author) {
                $uidSuffix = $author->get('uidNumber') ? '{{' . $author->get('uidNumber') . '}}' : '';
                $auths[] = $author->get('author') . $uidSuffix;
            }
    ...
    private function publish()
    {
        ...
        if ($id != 0 && !$bulk) {
            ...
            // save the state
            if (
                $citation->save() && $citation->scope == self::PLUGIN_SCOPE
            ...
            foreach ($citationIDs as $id) {
                ...
                // save the state
                if (
                    $citation->scope == self::PLUGIN_SCOPE
    ...
    private function settings()
    {
        if ($_POST) {
            ...
            $params->display = Request::getString('display', '');
            $params->include_coins = Request::getString('include_coins', '');
            $params->coins_only = Request::getString('coins_only', '');
            $params->citations_show_tags = Request::getString('citations_show_tags', '');
            $params->citations_show_badges = Request::getString('citations_show_badges', '');
            ...
            $view->include_coins = $params->include_coins ?? "false";
            $view->coins_only = $params->coins_only ?? "false";
            $view->citations_show_tags = $params->citations_show_tags ?? "true";
            $view->citations_show_badges = $params->citations_show_badges ?? "true";
            $citationsFormat = $params->citationFormat ?? 1;
    ...
    private function upload()
    {
        ...
        // Build import redirect URL
        $importUrl = 'index.php?option=' . $this->_option . '&cn=' . $this->group->get('cn');
        $importUrl .= '&active=citations&action=import';
        ...
        $citations = Event::trigger('citation.onImport', array($file, 'group', $gidNumber));
    ...
    private function process()
    {
        ...
        if (isset($this->group) && $this->group->get('cn') != '') {
            ...
            // view variables
            $view->citations_require_attention = $citations_action_attention ?? null;
            $view->citation_require_no_attention = $citation_action_no_attention ?? null;
     ...
     * @return  void
     */
    private function loginTask()
    {
        $returnUrl = 'index.php?option=' . $this->option . DS . $this->group->get('cn');
        $returnUrl .= DS . $this->_name . '&action=' . $this->action;
        $loginUrl = 'index.php?option=com_users&view=login&return='
            . base64_encode(Route::url($returnUrl, false, true));
     ...
     */
    private function filterHandler($filters = array(), $scope_id = 0)
    {
        ...
        if ($filterCount > 0) {
            foreach ($filters as $filter => $value) {
                ...
                $skipFilters = array('search', 'sort', 'tag', 'filter');
                if (!in_array($filter, $skipFilters) && $value != "") {
                    switch ($filter) {
                        case 'author':
