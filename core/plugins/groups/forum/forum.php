    ...
    public function onGroup($group, $option, $authorized, $limit, $limitstart, $action, $access, $areas = null)
    {
        ...
        if ($return == 'html') {
            ...
            if (
                ...
                $return = base64_encode(Request::getString('REQUEST_URI', $defaultUrl, 'server'));
                App::redirect(
            ...
            if (
                ...
                $msg = Lang::txt('GROUPS_PLUGIN_REQUIRES_MEMBER', ucfirst($active_real));
                $arr['html'] = '<p class="warning">' . $msg . '</p>';
                return $arr;
            }
            ...
            $this->limit = $limit;
            $this->base = 'index.php?option=' . $this->option;
            $this->base .= '&cn=' . $this->group->get('cn') . '&active=' . $this->_name;
            ...
            if (strstr($path, '/')) {
                ...
                if (isset($bits[0]) && trim($bits[0])) {
                    if ($bits[0] == 'new') {
                        $action = 'newsection';
                    } elseif ($bits[0] == 'settings' || $bits[0] == 'savesettings') {
                        $action = $bits[0];
                    } elseif ($bits[0] == 'unsubscribe') {
                        $action = 'unsubscribe';
                    } else {
...
// phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
            ...
            Event::trigger('system.logActivity', [
                'activity' => [
                    ...
                    'description' => Lang::txt(
                        'PLG_GROUPS_FORUM_ACTIVITY_SECTION_' . ($fields['id'] ? 'UPDATED' : 'CREATED'),
                        '<a href="' . Route::url($this->base) . '">' . $section->get('title') . '</a>'
                    ),
                    'details'     => array(
        ...
        if (!$section->save()) {
            ...
            Event::trigger('system.logActivity', [
                'activity' => [
                    ...
                    'scope_id'    => $section->get('id'),
                    'description' => Lang::txt(
                        'PLG_GROUPS_FORUM_ACTIVITY_SECTION_DELETED',
                        '<a href="' . Route::url($this->base) . '">' . $section->get('title') . '</a>'
                    ),
                    'details'     => array(
                        'title' => $section->get('title'),
    ...
    public function savecategory()
    {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt(
                    'PLG_GROUPS_FORUM_ACTIVITY_CATEGORY_' . ($fields['id'] ? 'UPDATED' : 'CREATED'),
                    '<a href="' . Route::url($this->base) . '">' . $category->get('title') . '</a>'
                ),
                'details'     => array(
    ...
    public function deletecategory()
    {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope_id'    => $category->get('id'),
                'description' => Lang::txt(
                    'PLG_GROUPS_FORUM_ACTIVITY_CATEGORY_DELETED',
                    '<a href="' . Route::url($this->base) . '">' . $category->get('title') . '</a>'
                ),
                'details'     => array(
                    'title' => $category->get('title'),
    ...
    {
        // Incoming
        $threadId = Request::getInt('thread', 0);
        $filters = array(
            'limit'    => (int)Request::getState('groups.forum.thread', 'limit', Config::get('list_limit'), 'int'),
            'start'    => (int)Request::getState('groups.forum.thread.' . $threadId, 'limitstart', 0, 'int'),
            'section'  => Request::getString('section', ''),
            'category' => Request::getCmd('category', ''),
        ...
        if (
            $thread->get('scope_id') != $this->forum->get('scope_id')
    ...
    public function editthread($post = null)
    {
        ...
        /*if (User::isGuest())
        {
            ...
            {
                $return = Route::url(
                    $this->base . '&scope=' . $sectionAlias . '/' . $category . '/' . $id . '/edit',
                    false,
                    true
                );
            }
    ...
    public function savethread()
    {
        ...
        /*if (User::isGuest())
        {
            App::redirect(
                Route::url(
                    ...
                    . base64_encode(Route::url($this->base, false, true))
                )
            );
            return;
        }*/
        ...
        if (!$fields['id']) {
            ...
            $message = $post->get('modified_by')
                ? Lang::txt('PLG_GROUPS_FORUM_POST_EDITED')
        ...
        if ($params->get('email_forum_comments') && (isset($moving) && $moving == false)) {
            ...
            $creatorName = $post->creator->get('name', Lang::txt('PLG_GROUPS_FORUM_UNKNOWN'));
            $fromName = (!$post->get('anonymous') ? $creatorName : Lang::txt('JANONYMOUS'));
            ...
            foreach ($userIDsToEmail as $userID) {
                ...
                if ($allowEmailResponses) {
                    ...
                    $unsubscribeToken = $encryptor->buildEmailToken(
                        1,
                        3,
                        $userID,
                        $this->group->get('gidNumber')
                    );
                    $unsubscribeUrl = Route::url(
                        'index.php?option=com_groups&cn=' . $this->group->get('cn')
                        . '&active=forum&action=unsubscribe&t=' . $unsubscribeToken
                    );
                    $unsubscribeLink = rtrim(Request::base(), '/') . '/' . ltrim($unsubscribeUrl, DS);
                ...
                $subject = Lang::txt('PLG_GROUPS_FORUM') . ': ' . $this->group->get('cn');
                $subject .= ' - ' . $thread->get('title');
                ...
                $messageArgs = array(
                    ...
                    $this->group->get('gidNumber'),
                    false,
                    $post->get('anonymous', 0)
                );
                if (!Event::trigger('xmessage.onSendMessage', $messageArgs)) {
                    $this->setError(Lang::txt('GROUPS_ERROR_EMAIL_MEMBERS_FAILED'));
                }
            }
        }

        $url = $this->base . '&scope=' . $section->get('alias') . '/' . $category->get('alias');
        $url .= '/' . $thread->get('id');
        ...
        if ($mentionEmailList) {
            ...
            $urlExt = 'groups/' . $this->group->get('cn') . '/forum/';
            $urlExt .= $section->get('alias') . '/' . $category->get('alias');
            $urlExt .= '/' . $post->get('thread') . '#c' . $post->get('id');
            $host = $_SERVER['HTTP_HOST'];
            $externalUrl = 'https://' . $host . '/' . $urlExt;

            $this->emailToAllMentionedUsersInGroup(
                $mentionEmailList,
                $comment,
                $externalUrl,
                $createdUserName,
...
// phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _getEmailRecipientIds($category)
    {
        ...
        foreach ($users as $user) {
            ...
            $sendEmail = $this->_shouldUserReceiveEmail(
                $userId,
                $memberoptions,
                $categorySubscriptionsEnabled,
                $category
            );
...
// phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
        ...
        if (!$post->save()) {
            ...
            Event::trigger('system.logActivity', [
                'activity' => [
                    ...
                    'description' => Lang::txt(
                        'PLG_GROUPS_FORUM_THREAD_DELETED',
                        '<a href="' . Route::url($url) . '">' . $post->get('title') . '</a>'
                    ),
                    'details'     => array(
                        'thread' => $post->get('thread'),
    ...
    public function upload($thread_id, $post_id)
    {
        ...
        if ($file['size'] > $sizeLimit) {
            $max = \Hubzero\Utility\Number::formatBytes($sizeLimit);
            $max = preg_replace('/<abbr \w+=\\"\w+\\">(\w{1,3})<\\/abbr>/', '$1', $max);
    ...
    public function download()
    {
        ...
        {
            $return = Route::url(
                'index.php?option=' . $this->option . '&cn=' . $this->group->get('cn')
                . '&active=forum&scope=' . $section . '/' . $category . '/' . $thread
                . '/' . $post . '/' . $file
            );
            App::redirect(
                Route::url('index.php?option=com_users&view=login&return=' . base64_encode($return))
            ...
            foreach ($sections as $section) {
                ...
                if ($categories->count()) {
                    ...
                    foreach ($categories as $category) {
                        ...
                        $log .= '.category.' . $category->get('id') . '.post' . "\n";
    ...
    private function savesettings()
    {
        ...
        $settingsUrl .= '&active=' . $this->_name . '&action=settings';
        App::redirect(
        ...
        if (!$section->move($dir)) {
            ...
            Event::trigger('system.logActivity', [
                'activity' => [
                    ...
                    'scope_id'    => $section->get('id'),
                    'description' => Lang::txt(
                        'PLG_GROUPS_FORUM_ACTIVITY_SECTION_REORDERED',
                        '<a href="' . Route::url($this->base) . '">' . $section->get('title') . '</a>'
                    ),
                    'details'     => array(
                        'title' => $section->get('title'),
