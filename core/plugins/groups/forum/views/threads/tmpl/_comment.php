...
?>
    <li class="comment <?php echo $cls; ?><?php if (!$this->comment->get('parent')) {
        echo ' start';
                       } ?>" id="c<?php echo $this->comment->get('id'); ?>">
        ...
        <div class="comment-content">
            <p class="comment-title">
                ...
                <a class="permalink"
                    href="<?php echo Route::url($this->comment->link('anchor')); ?>"
                    title="<?php echo Lang::txt('PLG_GROUPS_FORUM_PERMALINK'); ?>">
                    ...
                    <?php if ($this->comment->wasModified()) { ?>
                        ...
                        ?>
                        &mdash;
                        <?php echo Lang::txt('PLG_GROUPS_FORUM_EDITED'); ?>
        ...
        <?php if ($this->group->published == 1) { ?>
            ...
            $this->config->get('access-manage-thread')
                ...
                ?>
                <?php if ($this->comment->get('parent') && ($canDeletePost || $isCreator)) { ?>
            ...
            <?php if (!$this->comment->isReported()) { ?>
                <?php
                $isThreadOpen = !$this->thread->get('closed');
                $isTreeThreading = $this->config->get('threading') == 'tree';
                $maxDepth = $this->config->get('threading_depth', 3);
                $withinDepth = $this->depth < $maxDepth;
                ...
                <?php if ($canReply) { ?>
                    ...
                    <a class="icon-reply reply active"
                        ...
                        data-txt-inactive="<?php echo Lang::txt('PLG_GROUPS_FORUM_REPLY'); ?>"
            ...
            <?php if ($canReply) { ?>
                ...
                ?>
                <div class="comment-add<?php echo $hideClass; ?>"
                    id="comment-form<?php echo $commentId; ?>">
            <form id="cform<?php echo $this->comment->get('id'); ?>"
                action="<?php echo Route::url($this->thread->link()); ?>"
                method="post"
                enctype="multipart/form-data">
                        <fieldset>
                            ...
                            $replyToName = !$this->comment->get('anonymous')
                                ...
                                : Lang::txt('JANONYMOUS');
                            $replyTxt = Lang::txt(
                                'PLG_GROUPS_FORUM_REPLYING_TO',
                                $replyToName
                            );
                            ?>
                            <legend>
                                <span><?php echo $replyTxt; ?></span>
                            </legend>
                            ...
                            <input type="hidden"
                                name="fields[access]"
                                value="<?php echo $this->thread->get('access', 0); ?>"/>
                            <input type="hidden"
                                name="fields[scope]"
                                value="<?php echo $this->thread->get('scope'); ?>"/>
                            <input type="hidden"
                                name="fields[category_id]"
                                value="<?php echo $this->thread->get('category_id'); ?>"/>
                            <input type="hidden"
                                name="fields[scope_id]"
                                value="<?php echo $this->thread->get('scope_id'); ?>"/>
                            <input type="hidden"
                                name="fields[scope_sub_id]"
                                value="<?php echo $this->thread->get('scope_sub_id'); ?>"/>
                            <input type="hidden"
                                name="fields[object_id]"
                                value="<?php echo $this->thread->get('object_id'); ?>"/>
                            <input type="hidden"
                                name="fields[parent]"
