...
class Resources extends Plugin
{
    ...
    public function updateResourceRanking(\Components\Cron\Models\Job $job)
    {
        ...
        $sql = "SELECT r.id, r.ranking, r.ranked
				...
				AND r.state=1
				AND (r.ranked IS NULL OR r.ranked = '0000-00-00 00:00:00' OR r.ranked < "
                . $database->quote($timestamp) . ")
    ...
    public function emailMemberResources(\Components\Cron\Models\Job $job)
    {
        ...
        $query = "SELECT DISTINCT(u.id)
				...
				AND u.`approved` > 0
				AND u.`sendEmail`=1
				AND (l.`created` IS NULL OR l.`created` = '0000-00-00 00:00:00' OR l.`created` <= "
                . $db->quote($previous) . ")
				LIMIT 0," . $batch;
        $db->setQuery($query);
        $users = $db->loadColumn();
            ...
            foreach ($users as $user) {
                ...
                if (!empty($tags)) {
                    $subquery = "
						...
						+
						(LN(LN(1 / (case when datediff(NOW(), " . $r . ".created) >= 1 then datediff(NOW(), "
                        . $r . ".created) else 1 end) + 1)) + 10)
					";
