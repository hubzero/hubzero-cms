...
class Newsletter extends Plugin
{
    ...
    public function processMailings(\Components\Cron\Models\Job $job)
    {
        ...
        $sql = "SELECT nmr.id AS mailing_recipientid, nm.id AS mailingid, nm.nid AS newsletterid,"
            . " nm.lid AS mailinglistid, "
            . "nmr.email, nm.subject, nm.html_body, nm.plain_body, nm.headers, nm.args, nm.tracking, nl.autogen, "
            . "nm.date AS sendDate"
        ...
        foreach ($queuedEmails as $queuedEmail) {
            ...
            if ($queuedEmail->tracking) {
                $queuedEmail->html_body = \Components\Newsletter\Helpers\Helper::addTrackingToEmailMessage(
                    $queuedEmail->html_body,
                    $emailToken
                );
            }
            ...
            $unsubscribeLink       = 'https://' . $_SERVER['SERVER_NAME']
                . '/newsletter/unsubscribe?e=' . urlencode($queuedEmail->email) . '&t=' . $emailToken;
            ...
            $queuedEmail->headers   = str_replace(
                "{{UNSUBSCRIBE_MAILTO_LINK}}",
                $unsubscribeMailtoLink,
                $queuedEmail->headers
            );
            ...
            if ($message->send()) {
                ...
                if ($queuedEmail->autogen == 0 || $queuedEmail->autogen == null) {
                    ...
                    $newsletterMailingRecipient = \Components\Newsletter\Models\Mailing\Recipient::one(
                        $queuedEmail->mailing_recipientid
                    );
                    ...
                    $sql = "SELECT *, max(date) AS maxDate FROM `#__newsletter_mailings` WHERE nid = "
                        . "{$queuedEmail->newsletterid} AND deleted=0;";
                    $database->setQuery($sql);
                    ...
                    if ($windowMax - $windowMin == 0) {
                        ...
                        foreach ($emails as $email) {
                            if ($email->status == 'active') {
                                $values[] = "(" . $database->quote($newMailing->id) . ","
                                    . $database->quote($email->email) . ",'queued', "
                                    . $database->quote(Date::of(time())->toLocal()) . ")";
                            }
                        }
                        ...
                        if (count($values) > 0) {
                            ...
                            $sql = "INSERT INTO `#__newsletter_mailing_recipients`"
                                . " (`mid`,`email`,`status`,`date_added`)"
                    ...
                    // load recipient object
                    $newsletterMailingRecipient = \Components\Newsletter\Models\Mailing\Recipient::oneOrNew(
                        $queuedEmail->mailing_recipientid
                    );
