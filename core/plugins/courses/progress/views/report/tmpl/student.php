...
if ($auxiliary instanceof \Traversable) {
    $auxiliary = iterator_to_array($auxiliary);
}

$assets = array_merge((array) $assets, (array) $auxiliary);
...
<div class="progress">
    <?php if ($this->course->access('manage')) : ?>
        <div class="extra">
            <a href="<?php echo Route::url($this->base . '&active=progress') ?>"
                class="back btn icon-back"><?php echo Lang::txt('Back to all students') ?></a>
        </div>
    <?php endif; ?>
    ...
    <?php if (
        $this->course->certificate()->exists()  // The course has a certificate
        && $this->course->offering()->section()->params('certificate')  // The section is allowing certificates
        && $this->course->offering()->gradebook()->isEligibleForRecognition($this->member->get('id'))
) :    // The user is eligible for a certificate ?>
        <div class="recognition certificate earned">
            <h3>Congratulations!</h3>
            <p>
                You've completed all of the requirements of
                <?php echo $this->escape(stripslashes($this->course->get('title'))); ?>, qualifying you to receive
                a certificate of completion.
            </p>
            <p>
                <a class="claim-item"
                    href="<?php echo Route::url($this->course->offering()->link() . '&controller=certificate'); ?>">
                            ...
                            <?php if (isset($details['forms'][$unit->get('id')])) :
                                ...
                                <?php foreach ($details['forms'][$unit->get('id')] as $form) : ?>
                                    ...
                                    <tr class="<?php echo $class ?>">
                                        <td class="grade-details-title"><a
                                                href="<?php echo $form['url'] ?>"><?php echo $form['title'] ?></a></td>
                                        <td class="grade-details-score">
                                            <?php echo $form['score'] . (is_numeric($form['score']) ? '%' : '') ?>
                                        </td>
                        ...
                        <tbody>
                            ...
                            <?php foreach ($details['aux'] as $aux) : ?>
                                ...
                                <tr class="<?php echo $class ?>">
                                    ...
                                    <td class="grade-details-score">
                                        <?php echo $aux['score'] . (is_numeric($aux['score']) ? '%' : '') ?>
                                    </td>
