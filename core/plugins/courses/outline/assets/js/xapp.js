...
jQuery(document).ready(function($) {
	Hubzero.initApi();
	$('.xapp-edit').on('submit', '#select-form', function(e){
		e.preventDefault();
	...
	.hover(function() {
		$(this).siblings('.xapp-files-upload').css('opacity', 1);
	}, function() {
		$(this).siblings('.xapp-files-upload').css('opacity', 0.7);
	});
	...
	$('.xapp-files-available').on('click', '.xapp-files-delete', function() {
		var t    = $(this),
			data = t.parents('.edit-form').serializeArray();

		data.push({name : "filename", value : t.prev('.xapp-files-filename').html()});
		...
		$.ajax({
			...
			success    : function ( data, textStatus, jqXHR ) {
				t.parents('.xapp-file').fadeOut(500, function() {
					$(this).remove();
				});
			}
		});
...
function addAssetItems(files, asset_id) {
	var html = '';
	if ($('.xapp-files-available ul').length) {
		$.each(files, function ( i, file ) {
			html += '<li class="xapp-file">';
			html += '<span class="xapp-files-filename">' + file.name + '</span>';
			html += '<div class="xapp-files-delete"></div>';
			html += '</li>';
		});

		$('.xapp-files-list').prepend(html);
	} else {
		html  = '<ul class="xapp-files-list">';
		$.each(files, function ( i, file ) {
			html += '<li class="xapp-file">';
			html += '<span class="xapp-files-filename">' + file.name + '</span>';
			html += '<div class="xapp-files-delete"></div>';
		...
		html += '</ul>';
		$('.xapp-files-available').html(html);
	}
