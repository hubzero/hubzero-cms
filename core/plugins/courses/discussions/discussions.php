...
class PlgCoursesDiscussions extends Plugin
{
    ...
    protected function _thread($post, $filters = array())
    {
        ...
        $view = $this->view('list', 'threads')
            ...
            ->set(
                'base',
                $this->base . '&category=' . $category->get('alias')
                . ($filters['search'] ? '&action=search&search=' . $filters['search'] : '')
            )
    ...
    public function panel()
    {
        ...
        foreach ($sects as $key => $section) {
            ...
            $section->set(
                'categories',
                isset($categories[$section->get('id')]) ? $categories[$section->get('id')] : array()
            );

            if (
                (!$section->get('categories') || !count($section->get('categories')))
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt(
                    'PLG_COURSES_DISCUSSIONS_ACTIVITY_SECTION_' . ($fields['id'] ? 'UPDATED' : 'CREATED'),
    ...
    public function deletesection()
    {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope_id' => $section->get('id'),
                'description' => Lang::txt(
                    'PLG_COURSES_DISCUSSIONS_ACTIVITY_SECTION_DELETED',
                    '<a href="' . Route::url($this->base) . '">' . $section->get('title') . '</a>'
                ),
                'details' => array(
                    'title' => $section->get('title'),
    ...
    public function categories()
    {
        ...
        $threads = $category->threads()
            ->select(
                "*, (CASE WHEN last_activity IS NOT NULL AND last_activity != '0000-00-00 00:00:00'"
                . " THEN last_activity ELSE created END)",
                'activity'
            )
            ->whereEquals('state', $filters['state'])
            ->whereIn('access', $filters['access'])
    ...
    public function savecategory()
    {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt(
                    'PLG_COURSES_DISCUSSIONS_ACTIVITY_Category_' . ($fields['id'] ? 'UPDATED' : 'CREATED'),
    ...
    public function deletecategory()
    {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope_id' => $category->get('id'),
                'description' => Lang::txt(
                    'PLG_COURSES_DISCUSSIONS_ACTIVITY_CATEGORY_DELETED',
                    '<a href="' . Route::url($this->base) . '">' . $category->get('title') . '</a>'
                ),
                'details' => array(
                    'title' => $category->get('title'),
    ...
    public function editthread($post = null)
    {
        ...
        if (User::isGuest()) {
            $return = Route::url(
                $this->offering->link() . '&active=' . $this->_name . '&unit=' . $section . '&b=' . $category . '&c=new'
            );
            if ($id) {
                $return = Route::url(
                    $this->offering->link() . '&active=' . $this->_name . '&unit=' . $section
                    . '&b=' . $category . '&c='
                    . $id . '/edit'
                );
            }
    ...
    public function savethread()
    {
        ...
        if (
            ($fields['id'] && !$this->params->get('access-edit-thread'))
    ...
    public function download()
    {
        ...
        if (User::isGuest()) {
            $return = Route::url(
                $this->offering->link() . '&active=' . $this->_name . '&unit=download&b=' . $thread . '&file=' . $file
            );
            App::redirect(
                Route::url('index.php?option=com_users&view=login&return=' . base64_encode($return))
            );
    ...
    public function reorder($dir = 1)
    {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'scope_id' => $section->get('id'),
                'description' => Lang::txt(
                    'PLG_COURSES_DISCUSSIONS_ACTIVITY_DELETED',
                    '<a href="' . Route::url($this->base . '&unit=manage') . '">'
                    . $section->get('title') . '</a>'
                ),
                'details' => array(
                    'title' => $section->get('title'),
