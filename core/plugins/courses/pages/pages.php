    ...
    public function _ajaxUpload()
    {
        ...
        if (!empty($allowed) && !in_array(strtolower($ext), $allowed)) {
            ...
            echo json_encode(array(
                'error' => Lang::txt('PLG_COURSES_PAGES_ERROR_UPLOADING_INVALID_FILE', implode(', ', $allowed))
            ));
            exit();
        }
...
// phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
     ...
     * @return  string
     */
    private function getPath($page = null)
    {
        ...
        if (is_object($page)) {
            ...
            if (!$this->view->offering->access('manage') && $this->view->offering->access('manage', 'section')) {
                $path .= $this->view->course->get('id') . DS . 'sections' . DS
                    . $this->view->offering->section()->get('id') . DS . 'pagefiles';
            } else {
...
// phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    ...
    public function fileDownload()
    {
        ...
        // Add PATH_CORE
        $filepath = $this->getPath($page) . DS . ltrim($filename, DS);
