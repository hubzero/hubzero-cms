...
<div class="section">
    ...
    <?php } ?>
    <form action="<?php echo Route::url($this->publication->link('questions')); ?>"
        method="post"
        id="hubForm"
        class="full">
        <fieldset id="ask">
            ...
            <label>
                ...
                $tf = Event::trigger(
                    ...
                    array(array('tags', 'tags', 'actags','', $this->row->get('tags', '')))
                );
                $tf = implode("\n", $tf);

                echo $tf ? $tf : '<textarea name="tags" id="actags" rows="6" cols="35">'
                    . $this->escape($this->row->get('tags', ''))
                    . '</textarea>';
                ?>
            </label>
            ...
            <label for="field-question">
                ...
                <?php
                $questionContent = $this->escape(
                    stripslashes($this->row->get('question'))
                );
                echo $this->editor(
                    'question[question]',
                    $questionContent,
            ...
            <?php if ($this->banking) { ?>
                <label for="field-reward">
                    ...
                    <input type="text"
                        ...
                        id="field-reward"
                        value=""
                        size="5" <?php if ((int) $this->funds <= 0) {
