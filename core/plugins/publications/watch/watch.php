 ...
 */
namespace Plugins\Publications\Watch;
...
// phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
    ...
    public function onPublicationSub($publication, $option, $miniview = 0)
    {
        ...
        $areas = array('watch');
        if (
            !array_intersect($areas, $this->onPublicationSubAreas($publication))
    ...
    private function subscribe()
    {
        ...
        if (User::isGuest() || !$this->publication->exists()) {
            $subscribeUrl = Route::url($this->publication->link('version') . '&active=watch&action=' .
                $this->action . '&confirm=' . $confirm, false);
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt(
                    'PLG_PUBLICATIONS_WATCH_' . strtoupper($this->action) . 'D',
                    '<a href="' . $url . '">' . $this->publication->get('title') . '</a>'
                ),
                'details'     => array(
    ...
    public function onWatch($publication, $activity = 'newversion')
    {
        ...
        switch ($activity) {
            case 'newversion':
                ...
                $subject = Lang::txt('PLG_PUBLICATIONS_WATCH_PUBLICATIONS') . ': ' .
                    Lang::txt('PLG_PUBLICATIONS_WATCH_SUBJECT_EMAIL');
        ...
        if (!empty($message) && count($subscribers) > 0) {
            foreach ($subscribers as $subscriber) {
                ...
                // Send message
                if ($subscriber->email) {
                    $this->sendEmail($subscriber, $message, $subject, $url);
                }
            }
        }
     ...
     * @return  bool
     */
    private function sendEmail($subscriber, $message, $subject, $url)
    {
        ...
        $eview->unsubscribeLink = Route::url($this->publication->link() .
            '&active=watch&action=unsubscribe&confirm=1&email=' . $email);
