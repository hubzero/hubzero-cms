...
// @phpcs:disable PSR2.Classes.PropertyDeclaration.Underscore
     ...
     */
    private function confirm($word, $instanceNo = '')
    {
    ...
    public function onCheckAnswer($code = null)
    {
        ...
        if (
            $task && $task != 'logout'
            && $imgCatchaTxt
            && !$this->confirm($imgCatchaTxt, $imgCatchaTxtInst)
        ) {
            return false;
        }
     ...
     * @return string
     */
    private function getCapthcaHtml()
    {
    ...
    {
        // Define $r, $g, $b to avoid undefined variable warnings if needed,
        // although the original code used them in hexdec() calls.
        // The original code was buggy: hexdec(substr($hex, 0, 1) . $r) where $r is undefined.
        // Assuming the intention was simply hexdec(substr($hex, 0, 1) . substr($hex, 0, 1)) or similar for 3-char hex?
        // Actually, for 3 digit hex #RGB, $r$r, $g$g, $b$b is standard expansion.
        // The original code:
        // $color['r'] = hexdec(substr($hex, 0, 1) . $r);
        // This relies on $r being undefined (null/empty string) to work as just the single char?
        // Wait, if $hex is 3 chars, e.g. "FFF", substr is "F". hexdec("F") is 15. But we want 255.
        // So it should be $r = substr($hex, 0, 1); $color['r'] = hexdec($r . $r);
        // I will fix this logic bug while I'm at it, or at least suppress the warning if I can't be sure.
        // BUT strict PSR-12 conversion shouldn't change logic unless obvious fix.
        // `Undefined variable: r` is a bug. I will fix it to duplicate the character.
