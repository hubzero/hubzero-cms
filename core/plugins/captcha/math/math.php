...
// @phpcs:disable PSR2.Classes.PropertyDeclaration.Underscore
    ...
    public function onDisplay($name = null, $id = 'image_captcha_1', $class = '')
    {
        ...
        $problem['key'] = $this->generateHash($problem['sum'], date('j'));

        // Build the fields
        $html = '<div class="form-group">' . "\n";
        $html .= '<label for="captcha-answer">' . "\n";
        $html .= Lang::txt('PLG_CAPTCHA_MATH_TROUBLE_MATH', $problem['operand1'], $problem['operand2']);
        $html .= "\t" . '<input type="text" name="captcha_answer" id="captcha-answer" value="" size="3" '
            . 'id="answer" class="option form-control" /> <span class="required">'
            . Lang::txt('JREQUIRED') . '</span>' . "\n";
        $html .= "\t" . '<input type="hidden" name="captcha_krhash" id="captcha-krhash" value="'
            . $problem['key'] . '" />' . "\n";
        $html .= '</label>' . "\n";
        $html .= '</div>' . "\n";
    ...
    public function onCheckAnswer($code = null)
    {
        ...
        $answer = $this->generateHash($answer, date('j'));
    ...
    private function generateHash($input, $day)
    {
        ...
        $enc = substr($enc, 26, 1) . substr($enc, 10, 1) . substr($enc, 23, 1)
            . substr($enc, 3, 1) . substr($enc, 19, 1);
