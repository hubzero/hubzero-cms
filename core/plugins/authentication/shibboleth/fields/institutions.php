...
class Institutions extends Field
{
    ...
    protected function getInput()
    {
        ...
        $html[] = '<div class="shibboleth" data-iconify="'
            . $a(preg_replace('#^' . preg_quote(PATH_CORE) . '#', '', __FILE__)) . '">';
        $html[] = '<p class="xml-source"><label>Shibboleth ID provider configuration file: <input type="text" '
            . 'name="xmlPath" value="' . $a($val['xmlPath']) . '" /></label></p>';
        ...
        $html[] = '<input type="hidden" class="serialized" name="' . $this->name . '" value="'
            . $a(json_encode($val)) . '" />';
        $html[] = '</div>';
    ...
    private static function getIdpList($val, $alwaysUpdate = true)
    {
        ...
        foreach ($xml->xpath('//shib:SSO') as $item) {
            ...
            if (!($idp = curl_exec($curl))) {
            ...
            } elseif (!($idp = simplexml_load_string($idp))) {
                ...
                if (($orgUrl = $idp->xpath('//saml:OrganizationURL'))) {
                    $item['host'] = preg_replace(
                        '/^.*[.]([^.]+[.][^.]+)$/',
                        '$1',
                        parse_url($orgUrl[0], \PHP_URL_HOST)
                    );
                }
