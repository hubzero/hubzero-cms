...
class plgTagsBlogs extends \Hubzero\Plugin\Plugin
{
    ...
    public function onTagView($tags, $limit = 0, $limitstart = 0, $sort = '', $areas = null)
    {
        ...
        $e_fields = "SELECT e.id, e.title, e.alias, NULL AS itext, e.content AS ftext,
                    e.state, e.created, e.created_by, NULL AS modified, e.publish_up, e.publish_down,
                    CONCAT('index.php?option=com_blog&task=view&id=', e.id) AS href,
                    'blogs' AS section, COUNT(DISTINCT t.tagid) AS uniques, e.params,
                    e.scope AS rcount, u.name AS data1, e.scope_id AS data2, NULL AS data3 ";
        ...
        if (User::isGuest()) {
            ...
            $e_where .= " AND ( (e.access IN (1,2)) OR (e.access IN (5) AND scope='member' "
                . "AND scope_id=" . User::get('id') . ") $gwhere )) ";
        }
