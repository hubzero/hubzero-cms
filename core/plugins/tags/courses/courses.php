...
class plgTagsCourses extends \Hubzero\Plugin\Plugin
{
    ...
    public function onTagView($tags, $limit = 0, $limitstart = 0, $sort = '', $areas = null)
    {
        ...
        $e_fields = "SELECT e.id, e.title, e.alias, e.blurb AS itext, e.description AS ftext, e.state, e.created,
                    e.created_by, NULL AS modified,
                         NULL AS publish_up, NULL AS publish_down,
                         CONCAT('index.php?option=com_courses&gid=', e.alias) AS href,
        ...
        $e_where  = " WHERE e.created_by=u.id AND e.state=1 AND t.objectid=e.id AND t.tbl='courses'"
            . " AND t.tagid IN ($ids)";
