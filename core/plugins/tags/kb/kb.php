...
// phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
    ...
    public function onTagView($tags, $limit = 0, $limitstart = 0, $sort = '', $areas = null)
    {
        ...
        $e_fields = "SELECT e.id,
             e.title, e.alias, e.fulltxt AS itext, e.fulltxt AS ftext, e.state, e.created, e.created_by, e.modified,
                    e.created AS publish_up, NULL AS publish_down,
                         CONCAT('index.php?option=com_kb&category=&alias=', e.alias) AS href, 'kb' AS section,
    ...
    {
        $row
            ->href = Route::url('index.php?option=com_kb&section=' . $row->data2 . '&category=' . $row->data1
                . '&alias=' . $row->alias);
        ...
        $html .= "\t\t" . '<p class="title"><a href="' . $row->href . '">' . stripslashes($row->title)
            . '</a></p>' . "\n";
        if ($row->ftext) {
            $html .= "\t\t" . '<p>' . \Hubzero\Utility\Str::truncate(
                \Hubzero\Utility\Sanitize::stripAll(stripslashes($row->ftext)),
                200
            ) . "</p>\n";
        }
