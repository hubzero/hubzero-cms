...
class plgResourcesUsage extends \Hubzero\Plugin\Plugin
{
    ...
    public function onResources($model, $option, $areas, $rtrn = 'all')
    {
        ...
        if (!in_array($table, $tables)) {
            ...
            $arr['metadata'] = '<p class="usage"><a href="' . $url . '">'
                . Lang::txt('PLG_RESOURCES_USAGE_DETAILED') . '</a></p>';
            return $arr;
        }
        ...
        if ($rtrn == 'all' || $rtrn == 'metadata') {
            ...
            if ($model->isTool()) {
                ...
                $arr['metadata'] = '<p class="usage"><a href="' . $url . '">'
                    . Lang::txt($langKey, $stats->users) . '</a></p>';
            } else {
                if ($statsUpdating) {
            ...
            if (isset($clusters->users) && $clusters->users && isset($clusters->classes) && $clusters->classes) {
                $arr['metadata'] .= '<p class="usage">' . Lang::txt(
                    'PLG_RESOURCES_USAGE_NUM_USERS_IN_CLASSES',
                    $clusters->users,
                    $clusters->classes
                ) . '</p>';
            }
    ...
    public static function getTid($id, $datetime, $period = 14)
    {
        ...
        $sql = "SELECT t.id FROM `#__resource_stats_tools` AS t WHERE t.resid = "
            . $database->quote($id) . " AND t.period = " . $database->quote($period)
            . " AND t.datetime = '" . $datetime . "-00 00:00:00' ORDER BY t.id LIMIT 1";
        $database->setQuery($sql);
    ...
    public function getValues($id, $period)
    {
        ...
        foreach ($results as $result) {
            // $point = new stdClass;
            $result->datetime = str_replace('-', '/', str_replace('-00 00:00:00', '-01', $result->datetime))
                . ' 00:00:00';
            // $point->users = $result->users;
            // $point->users = $result->users;

            // $data->users[]       = "[Date.parse('" . str_replace('-',
            //      '/', str_replace('-00 00:00:00', '-01', $result->datetime)) . " 00:00:00')," . $result->users . "]";
            // $interactive[] = "[Date.parse('" . str_replace('-',
            //      '/', str_replace('-00 00:00:00', '-01', $result->datetime))
            //      . " 00:00:00')," . $result->sessions . "]";
            // $sessions[]    = "[Date.parse('" . str_replace('-',
            //      '/', str_replace('-00 00:00:00', '-01', $result->datetime))
            //      . " 00:00:00')," . $result->simulations . "]";
            // $data->runs[]        = "[Date.parse('" . str_replace('-',
            //      '/', str_replace('-00 00:00:00', '-01', $result->datetime))
            //      . " 00:00:00')," . $result->jobs . "]";
            $data->points[] = $result;
        ...
        $results = $this->getTopValue($id, 1, $id, date('Y-m'), $period);
        $data->countries = $results;

        $results = $this->getTopValue($id, 2, $id, date('Y-m'), $period);
        $data->domains = $results;

        $results = $this->getTopValue($id, 3, $id, date('Y-m'), $period);
        $data->orgs = $results;
