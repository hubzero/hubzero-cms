...
// phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
    ...
    public function &onResourcesAreas($model)
    {
        ...
        if (
            $model->type->params->get('plg_questions')
     ...
     * @return  string
     */
    private function new($row = null)
    {
        // Login required
        if (User::isGuest()) {
            $rtrn = Request::getString(
                'REQUEST_URI',
                Route::url($this->model->link() . '&active=' . $this->_name, false, true),
                'server'
            );
        ...
        if (!empty($receivers)) {
            ...
            if (
                !Event::trigger(
                    ...
                    array('new_question_admin', $subject, $message, $from, $receivers, 'com_answers')
                )
            ) {
        ...
        Event::trigger('system.logActivity', [
            'activity' => [
                ...
                'description' => Lang::txt(
                    'PLG_RESOURCES_QUESTIONS_ACTIVITY_QUESTION_'
                    . (isset($fields['id']) && $fields['id'] ? 'UPDATED' : 'CREATED'),
