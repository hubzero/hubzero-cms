...
// phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
    ...
    public function onProject($model, $action = '', $areas = null, $tool = null)
    {
        ...
        if ($returnhtml) {
            ...
            Lang::load('com_wiki') || Lang::load(
                'com_wiki',
                Component::path('com_wiki') . DS . 'site'
            );
            ...
            if (
                substr(strtolower($this->_pagename), 0, strlen('image:')) == 'image:'
            ...
            if (
                !file_exists(
                    Component::path('com_wiki') . DS . 'site' . DS . 'controllers'
...
// @phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    ...
    public function fixupPathway()
    {
        ...
        if ($this->model->groupOwner()) {
            ...
            Pathway::append(
                Str::truncate($this->model->groupOwner('description'), 50),
        ...
        if ($this->_tool && $this->_tool->id) {
            ...
            Pathway::append(
                Str::truncate($this->_tool->title, 50),
                Route::url('index.php?option=' . $this->_option . '&alias=' . $this->model->get('alias')
                    . '&active=tools&tool=' . $this->_tool->id)
            );

            Pathway::append(
                ucfirst(Lang::txt('COM_PROJECTS_TOOLS_TAB_WIKI')),
                Route::url('index.php?option=' . $this->_option . '&alias=' . $this->model->get('alias')
                    . '&active=tools&tool=' . $this->_tool->id . '&action=wiki')
            );
        } else {
            Pathway::append(
