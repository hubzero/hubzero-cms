    ...
    public function onProject($model, $action = '', $areas = null)
    {
        ...
        if (in_array($this->_task, $tasks)) {
            ...
            switch ($this->_task) {
                ...
                case 'addcitation':
                    $html = $this->_addCitation();
                    break;

                case 'deletecitation':
                    $html = $this->_deleteCitation();
                    break;
...
// @phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _deleteCitation()
    {
        ...
        if (!$this->getError()) {
            // Unattach citation
            if ($this->_unattachCitation($vid, $cid)) {
        ...
        $url = Route::url(
            $route . '&pid=' . $pid . ($vid ? '&vid=' . $vid : '&version=' . $version) . '&section=citations',
            false
        );
...
// @phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _addCitation()
    {
        ...
        // Attach citation
        if ($this->_attachCitation($vid, $doi, $format, $this->_uid)) {
...
// @phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _attachCitation($vid = 0, $doi = null, $format = 'apa', $actor = 0, $returnStatus = false)
    {
        ...
        if (!$citationAssociation->isNew()) {
            ...
            // Get DOI preview
            $output = self::_parseUrl($doi, true, true, $format);
            ...
            if (isset($output->error) && $output->error) {
                ...
                $url = '';
                $data = self::_getDoiMetadata($doi, false, $url);
...
// @phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _editcite()
    {
        ...
        // Get publication
        $view->publication = $objP->getPublication(
            $view->version->publication_id,
            $view->version->version_number,
            $this->model->get('id')
        );
...
// @phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _parseDoi()
    {
        ...
        return $this->_parseUrl($doi, true, true, $format);
    }
...
// @phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _parseUrl($url = '', $citation = true, $incPreview = true, $format = 'apa')
    {
        ...
        if ($doi) {
            $output['rtype'] = 'doi';
            $data = self::_getDoiMetadata($doi, $citation, $url, $format);
        ...
        if ($data) {
            ...
            if ($content) {
                ...
                $out .= $title
                    ? stripslashes('<h5>' . addslashes($title) . '</h5>')
                ...
                $out .= $description
                    ? stripslashes(
                        '<p>' . \Hubzero\Utility\Str::truncate(addslashes($description), 200) . '</p>'
                    )
