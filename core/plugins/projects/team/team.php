...
class Team extends Plugin
{
    ...
    {
        $counts = array(
            'team' => count($model->team(array(), true))
        );
    ...
    protected function display($edit = 0)
    {
        ...
        if (!in_array($view->filters['sortby'], array('group', 'added', 'date', 'role', 'name', 'status'))) {
     ...
     * @return  string
     */
    protected function _choose() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
    ...
    protected function _select() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        // Get css
        if (!$ajax) {
            Assets::addPluginStylesheet('projects', 'team', 'selector');
        }
    ...
    protected function _save() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        if (count($uids) > 0) {
            $this->_msg = Lang::txt('PLG_PROJECTS_TEAM_SUCCESS_ADDED_OR_INVITED') . ' ' .
                count($uids) . ' ' . Lang::txt('PLG_PROJECTS_TEAM_NEW') . ' ' .
                Lang::txt('PLG_PROJECTS_TEAM_MEMBERS');
            ...
            if (!$setup) {
                ...
                for ($i = 0; $i < count($uids); $i++) {
                    ...
                    if ($i > 1) {
                        $left = count($uids) - 3;
                        if ($left) {
                            $note .= ' ' . Lang::txt('PLG_PROJECTS_TEAM_AND') . ' ' . $left . ' '
                                . Lang::txt('PLG_PROJECTS_TEAM_MORE') . ' ';
                            $note .= $left == 1
            ...
            $this->setError(Lang::txt('PLG_PROJECTS_TEAM_MEMBERS_INVALID_NAMES') . '<br />' .
                Lang::txt('PLG_PROJECTS_TEAM_MEMBERS_INVALID_NAMES_EXPLAIN'));
        }
     ...
     * @return mixed
     */
    protected function _approveMembership() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
     ...
     * @return mixed
     */
    protected function _denyMembership() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        Request::checkToken('get');
        $projectId = $this->model->get('id');
        $userId = User::getInstance()->get('id');
    ...
    private function sendMemberRequestEmail($email, $subject, $message)
    {
     ...
     * @return  mixed
     */
    protected function _changeOwner() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        // Incoming
        ...
        if ($confirm) {
            ...
            if (!$objO->id) {
                App::abort(404, Lang::txt('Error loading user'));
            }
            ...
            $view->groups = UserHelper::getGroups($this->model->get('owned_by_user'), 'members', 1);
            if ($this->model->groupOwner()) {
     ...
     * @return  mixed
     */
    protected function _quit() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        // Incoming
        $confirm = Request::getInt('confirm', 0, 'post');
        ...
        if ($confirm && !$onlymanager && !$this->model->access('owner')) {
            ...
            if ($deleted) {
                ...
                $aid = $this->model->recordActivity(
                    Lang::txt('PLG_PROJECTS_TEAM_PROJECT_QUIT'),
                    0,
                    '',
                    '',
                    'team',
     ...
     * @return  string
     */
    protected function _changeRole() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        // Incoming
        $checked = Request::getArray('owner', array());
        $groups = Request::getArray('group', array());
    ...
    public function sendInviteEmail($uid = 0, $email = '', $code = '', $role = 0, $model = '', $option = 'com_projects')
    {
        ...
        if (isset($uid) && $uid != 0 && $uid != false) {
            ...
            if (
                \Components\Projects\Helpers\Html::email(
                    $email,
