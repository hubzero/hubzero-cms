...
class Sync extends \Hubzero\Base\Obj
{
    ...
    public function __construct($connect = null)
    {
        ...
        $this->_logPath = \Components\Projects\Helpers\Html::getProjectRepoPath(
    ...
    public function sync($service = 'google', $queue = false, $auto = false)
    {
        ...
        $startTime = date('c');
        $passed = $synced != 1
            ? \Components\Projects\Helpers\Html::timeDifference(strtotime($startTime) - strtotime($synced))
            : 'N/A';
        ...
        if (count($locals) > 0) {
            ...
            foreach ($locals as $filename => $local) {
                ...
                if ($local['synced'] && ($local['synced'] > $local['modified'])) {
                    $output .= '## item in sync: ' . $filename
                        . ' local: ' . $local['modified'] . ' synced: ' . $local['synced'] . "\n";
                    continue;
                }

                // Record sync status
                $this->writeToFile(
                    Lang::txt('PLG_PROJECTS_FILES_SYNC_SYNCING') . ' '
                ...
                if ($local['status'] == 'W') {
                    if ($local['remoteid']) {
                        ...
                        $parentId = $this->_connect->prepRemoteParent(
                            $this->model->get('id'),
                            $service,
                            $projectOwner,
                            $local,
                        ...
                        if ($parentId != $local['rParent']) {
                            ...
                            $output .= '>> moved ' . $local['rename'] . ' to ' . $filename
                                . ' (new parent id ' . $parentId . ')' . "\n";

                            if ($local['type'] == 'folder') {
                                $this->_connect->fixConvertedItems(
                                    $service,
                                    $this->_uid,
                                    $local['rename'],
                                    'W',
        ...
        if (!empty($newRemotes) && $synced != 1) {
            ...
            foreach ($newRemotes as $filename => $nR) {
                ...
                if ($nR['thumb']) {
                    $this->writeToFile(
                        Lang::txt('PLG_PROJECTS_FILES_SYNC_GET_THUMB') . ' '
        ...
        if (count($remotes) > 0 && $synced != 1) {
            ...
            foreach ($remotes as $filename => $remote) {
                ...
                if ($match && (($match['modified'] > $remote['modified']) > 0)) {
                    // skip
                    $output .= '== local and remote change match, but remote is older, picking local: '
                        . $filename . "\n";
                    continue;
                }
                    ...
                    if ($remote['converted'] == 1) {
                        ...
                        if ($remote['type'] == 'file') {
                            ...
                            if ($remote['md5'] == $md5Checksum) {
                                ...
                                if (
                                    $this->_connect->downloadFileCurl(
                                        $service,
                                        $projectOwner,
                                        $remote['url'],
                                    ...
                                    $output .= ' ! versions differ: remote md5 ' . $remote['md5']
                                        . ', local md5' . $md5Checksum . "\n";
                                    $output .= '++ sent update from remote to local: ' . $filename . "\n";
                                    $updated = 1;
                                } else {
                                    // Error
                                    $output .= '[error] failed to update local file with remote change: '
                                        . $filename . "\n";
                                    continue;
                                }
                            }
                        ...
                        if ($remote['type'] == 'folder') {
                            ...
                            if ($checkAvail <= 0) {
                                // Error
                                $output .= '[error] not enough space for ' . $filename
                                    . ' (' . $remote['fileSize'] . ' bytes) avail space:' . $checkAvail . "\n";
                                continue;
                            } else {
                                $avail = $checkAvail;
