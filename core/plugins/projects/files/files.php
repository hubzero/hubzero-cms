...
class Files extends Plugin
{
     ...
     * @var  string
     */
    protected $_path = null; // phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
    ...
    public function &onProjectAreas($alias = null)
    {
        ...
        if ($this->params->get('default_action', 'browse') == 'connections') {
            ...
            $area['children'][] = array(
                'name' => 'default',
                'title' => sprintf(
                    $this->params->get('default_connection_name', '%s Master Repository'),
                    $model->get('title')
                ),
                'url' => UrlHelper::updatePerMembership(
                    $model->link('files') . '&action=browse',
                    $userIsMember
                ),
                'class' => 'filesystem default'
                    . (Request::getString('action') == 'browse' && !$active ? ' active' : ''),
                ...
                if ($connections->count()) {
                    foreach ($connections as $connection) {
                        $area['children'][] = array(
                            ...
                            'url' => UrlHelper::updatePerMembership(
                                $model->link('files') . '&action=browse&connection=' . $connection->id,
                                $userIsMember
                            ),
                            'class' => 'filesystem ' . $connection->provider->alias
                                . (!$connection->isShared() ? ' private' : '')
                                . ($active == $connection->id ? ' active' : ''),
                            //'image' => $img,
                            'icon' => 'f0a0'
                        );
                    }
    ...
    public function onProject($model, $action = '', $areas = null, $params = array())
    {
        ...
        if ($returnhtml) {
            ...
            $ctask = 'connections';
            if (
                ($connection = Request::getInt('connection', null))
    ...
    protected function _select() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        if ($this->model->isProvisioned()) {
            $route = 'index.php?option=com_publications&task=submit&active=files';
        } else {
            $route = $this->model->link('files');
        }
        $filterUrl = Route::url($route);
        $filterUrl .= '?action=filter&amp;p=' . $props . '&amp;ajax=1&amp;no_html=1';
        $elId = '';
        if ($pid !== 0) {
            ...
            $filterUrl .= '&amp;pid=' . $view->publication->get('id') .
                '&amp;vid=' . $view->publication->get('version_id');
        ...
        if ($this->model->get('id')) {
            ...
            $fileList = $this->fileList($fileParams, $allowed, $selected, $used);
            if (!empty($fileList)) {
                return $fileList;
            }
            ...
            if ($cid > 0) {
                ...
                $dir = Entity::fromPath('', $con->adapter());
        ...
        $view->showCons = ($this->params->get('default_action', 'browse') == 'connections')
            ? true
            : false;
        $view->min = $min;
        $view->max = $max;
     ...
     */
    private function fileList($params = array(), $allowed = array(), $selected = array(), $used = array())
    {
        $template = null;
        $directory = urldecode(Request::getString('directory', ''));
        if (!empty($directory)) {
            ...
            if ($cid > 0) {
                // Get directory that we're interested in
                $con = Connection::oneOrFail($cid);
                $dir = Entity::fromPath(($directory != '.' ? $directory : ''), $con->adapter());
        ...
        if (!empty($items)) {
            foreach ($items as $element) {
                ...
                if ($params['file']->get('converted')) {
                    ...
                    if (
                        $this->_connect->deleteRemoteItem(
                            $this->model->get('id'),
                            $this->_remoteService,
                            $this->model->get('owned_by_user'),
                            $params['file']->get('remoteId'),
                            false
                        )
                    ) {
    ...
    public function _serve($type = '', $projectid = 0, $query = '')
    {
        ...
        if ($hash) {
            ...
            $tempName = 'temp-' . \Components\Projects\Helpers\Html::generateCode(4, 4, 0, 1, 0);
    ...
    protected function _download() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        // File download
        if (count($items) > 1) {
            $archive = $this->archiveFiles($items);
            ...
            if (!empty($this->_remoteService) && $file->get('converted') && $this->_task == 'open') {
                ...
                if (!$connected) {
                    ...
                    // Redirect
                    App::redirect(
                        Route::url($this->model->link('files') . '&action=connect')
                    );
                }
                ...
                if (!$openLink) {
                    ...
                    App::abort(
                        404,
                        Lang::txt('PLG_PROJECTS_FILES_FILE_NOT_FOUND') . ' ' . $file->get('name')
                    );
                }
    ...
    protected function _compile() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        $outputDir = DS . $imagePath . DS . strtolower($this->model->get('alias'));
        $outputDir .= DS . 'compiled';
        ...
        if (!empty($contentFile) && file_exists(PATH_APP . $outputDir . DS . $contentFile)) {
            ...
            if ($cType == 'application/pdf') {
                ...
                if ($gspath && file_exists(DS . $gspath . DS . 'gs')) {
                    ...
                    $cmd = $gspath . "gs -dNOPAUSE -sDEVICE=jpeg -r300 -dFirstPage=1"
                        . " -dLastPage=1 -sOutputFile=$exportPath $pdfPath 2>&1";
                    exec($cmd, $out);
     ...
     */
    // @phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    public function _diskspace($model, $repoName = 'local', $by = '')
    {
        // Output HTML
        $view = new \Hubzero\Plugin\View(
            array(
        ...
        $view->quota = $model->params->get(
            'quota',
            \Components\Projects\Helpers\Html::convertSize(
                floatval($model->config()->get('defaultQuota', '1')),
                'GB',
                'b'
            )
        );
    ...
    protected function _iniSync() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        if (!$ajax) {
            ...
            $this->_rSync->set('output', $this->browse());
     ...
     * @return  mixed  array or false
     */
    private function archiveFiles($items)
    {
    ...
    public function getMembersPath()
    {
        ...
        $path = DS . trim($mconfig->get('webpath', '/site/members'), DS);
        $path .= DS . $dir . DS . 'files';
        ...
        if (!empty($failed) && !$uploaded && !$uploaded) {
            ...
            if ($updated) {
                ...
                if (count($updateParts) > 2) {
                    $message .= basename($updateParts[0]) . ' and ' .
                        (count($updateParts) - 1) . ' more files ';
                } else {
                    $u = 0;
