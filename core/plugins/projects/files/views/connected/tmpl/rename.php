	...
	<?php else : ?>
		<form id="hubForm-ajax" method="post" action="<?php echo Route::url($this->url); ?>">
			<fieldset>
				...
				<input type="hidden" name="oldname" value="<?php echo $this->escape($this->item->getPath()); ?>" />
				<h5><?php echo Lang::txt('PLG_PROJECTS_FILES_NEW_NAME'); ?></h5>
				<label>
					<input type="text" name="newname" maxlength="250" value="<?php echo $this->escape($this->item->getFilename()); ?>" />
				</label>
