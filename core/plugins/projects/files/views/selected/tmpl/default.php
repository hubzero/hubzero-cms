...
<li>
    <img class="file-type file-type-<?php echo $this->file->get('ext'); ?>"
        ...
        alt="<?php echo $this->escape($this->file->get('name')); ?>"/>

    <?php echo $this->escape($this->file->get('name')); ?>

    <?php if ($this->file->get('converted')) : ?>
        <?php
        $remoteLabel = $this->file->get('type') == 'folder'
            ? Lang::txt('PLG_PROJECTS_FILES_REMOTE_FOLDER')
            : Lang::txt('PLG_PROJECTS_FILES_REMOTE_FILE');
        ?>
        <span class="remote-file"><?php echo $remoteLabel; ?></span>
    <?php endif; ?>

    <?php if ($this->file->get('converted') && $this->file->get('originalPath')) : ?>
        <span class="remote-file faded">
            <?php echo Lang::txt('PLG_PROJECTS_FILES_CONVERTED_FROM_ORIGINAL')
                . ' '
                . basename($this->file->get('originalPath')); ?>
    ...
    <input type="hidden"
        name="<?php echo ($this->file->get('type') == 'folder' ? 'folder' : 'asset') . $multi; ?>"
        value="<?php echo $this->escape(urlencode($this->file->get('name'))); ?>"/>
