...
<div class="">
    <form action="<?php echo Route::url($this->model->link('files') . '&action=saveconnection'); ?>" method="post"
        ...
        <fieldset>
            ...
            <div class="input-wrap">
                ...
                <input type="text" name="connect[name]" id="param-name"
                    value="<?php echo $this->escape($this->connection->name); ?>" />
            </div>
            ...
            if (count($fieldSet)) :
                ...
                <fieldset class="panelform">
                    ...
                    <?php foreach ($fieldSet as $field) : ?>
                        <?php if (!$field->hidden) : ?>
                            <div class="input-wrap <?php if ($field->type == 'Spacer') {
                                echo ' input-spacer';
                                                   } ?>">
            ...
            <input type="hidden" name="connect[project_id]"
                value="<?php echo $this->escape($this->connection->get('project_id')); ?>" />
            <input type="hidden" name="connect[owner_id]"
                value="<?php echo $this->escape($this->connection->get('owner_id')); ?>" />
            <input type="hidden" name="connect[provider_id]"
                value="<?php echo $this->escape($this->connection->get('provider_id')); ?>" />
            <input type="hidden" name="connect[id]"
                value="<?php echo $this->escape($this->connection->get('id')); ?>" />
            ...
            <p class="submit">
                ...
                <a class="btn btn-secondary"
                    href="<?php echo Route::url($this->model->link('files')); ?>"><?php echo Lang::txt('Cancel'); ?></a>
            </p>
        </fieldset>
    </form>
</div>