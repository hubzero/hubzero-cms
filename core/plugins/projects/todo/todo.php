...
class Todo extends Plugin
{
    ...
    protected function _item() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        // Append breadcrumbs
        Pathway::append(
            stripslashes(Str::truncate($view->row->get('content', ''), 40)),
    ...
    protected function _save() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        if ($task == 'save' && $content != '') {
            ...
            $objTD->content = Sanitize::stripAll($objTD->content);

            // Save access under details
            if (strlen($objTD->content) > 255) {
                $objTD->details = $objTD->content;
            }
            $objTD->content = Str::truncate($objTD->content, 255);
                ...
                if (count($date) == 3) {
                    ...
                    if (intval($month) && intval($day) && intval($year)) {
                        ...
                        if (checkdate($month, $day, $year)) {
                            ...
                            $timezone = \Hubzero\Utility\Arr::getValue($userParams, 'timezone', $serverTimezone);
            ...
            // Get list name
            $objTD->todolist = $listcolor == 'none'
                ? null
                : $objTD->getListName($this->model->get('id'), $objTD->color);
                    ...
                    if ($state == 1) {
                        ...
                        $aid = $this->model->recordActivity(
                            ...
                            Route::url(
                                ...
                                . '&active=todo' . '&action=view&todoid=' . $objTD->id
                            ),
                            'todo',
                            1
                        );
        ...
        if ($newlist != '' && $newcolor != '') {
            $new = 0;
            $newlist = Sanitize::stripAll(trim($newlist));
    ...
    protected function _saveComment() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        $comment = Str::truncate($comment, 800);
