...
class Publications extends Plugin
{
    ...
    public function onProject($model, $action = '', $areas = null)
    {
        ...
        Assets::addPluginStylesheet('projects', 'publications');
        Assets::addPluginStylesheet('projects', 'publications', 'curation');
        Assets::addPluginScript('projects', 'publications', 'curation');
        ...
        switch ($this->_task) {
            ...
            case 'undispute':
            case 'saveitem':
                $arr['html'] = $this->_saveDraft();
                break;

            case 'orderup':
                $arr['html'] = $this->_reorder('up');
                break;
            case 'orderdown':
                $arr['html'] = $this->_reorder('down');
                break;

            // Individual items editing
            case 'edititem':
            case 'editauthor':
                $arr['html'] = $this->_editItem();
                break;
            ...
            case 'versions':
                $arr['html'] = $this->_versions();
                break;

            // Unpublish/delete
            case 'cancel':
                $arr['html'] = $this->_cancelDraft();
                break;

            // Contribute process outside of projects
            case 'contribute':
                $arr['html'] = $this->_contribute();
                break;
            ...
            case 'diskspace':
                $arr['html'] = $this->_pubDiskSpace($this->model);
                break;

            // Handlers
            case 'handler':
                $arr['html'] = $this->_handler();
                break;
        }
    ...
    protected function browse()
    {
        ...
        Assets::addPluginStylesheet('projects', 'files', 'diskspace');
        Assets::addPluginScript('projects', 'files', 'diskspace');

        // Need to calculate ALL publications
        $allRows = $view->pub->entries('list', array('project' => $this->model->get('id'), 'dev' => 1));

        // Get used space
        $view->dirsize = PubHtml::getDiskUsage($allRows, false);
    ...
    protected function _handler() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        $view->publication = new Publication($pid, null, $vid);
        ...
        // Load handler
        $modelHandler = new Handlers($this->_database);
    ...
    protected function _select() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        Assets::addPluginStylesheet('projects', 'publications', 'selector');
    ...
    protected function _saveParam() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        // Load publication
        $publication = new Publication($pid, null, $vid);
    ...
    protected function _saveDraft() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        // Load publication
        if ($vid) {
            $pub = new Publication($pid, null, $vid);
            $version = $pub->versionAlias();
        } else {
            $pub = new Publication($pid, $version);
        }
    ...
    public function onAfterSave($pub)
    {
        ...
        if ($this->_activity) {
            ...
            $aid = $this->model->recordActivity(
                ...
                $pubTitle,
                Route::url(
                    'index.php?option=' . $this->_option . '&alias=' . $this->model->get('alias')
                    . '&active=publications&pid=' . $pub->id . '&version=' . $pub->get('version_number')
                ),
                'publication',
                1
            );
    ...
    protected function createDraft()
    {
        ...
        // Models\Publication
        $pub = new Publication($this->_pid, 'dev');
    ...
    protected function editDraft()
    {
        ...
        if (!$pid) {
            ...
            // Redirect to first block
            App::redirect(htmlspecialchars_decode(Route::url(
                $pub->link('editversion') . '&move=continue&step=' . $blockId . '&section=' . $firstBlock
            )));
            return;
        }
    ...
    protected function _suggestLicense()
    {
        ...
        if ($this->_task == 'save_license') {
            ...
            if (!$l_title && !$l_url && !$l_text) {
                ...
                $group = Group::getInstance($admingroup);
    ...
    protected function _overQuota()
    {
        ...
        // Get all publications
        $rows = $pub_model->entries(
            'list',
            array('project' => $this->model->get('id'), 'dev' => 1, 'ignore_access' => 1)
        );
    ...
    protected function _publishDraft() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        if (
            $revertAllowed && $pub->version->state == 1
        ...
        if ($pub->config()->get('repository') && $this->_task != 'revert') {
            ...
            foreach ($contact as $key) {
                ...
                if (empty($owner->user_id) && empty($owner->invited_email)) {
                    Notify::error(
                        Lang::txt('PLG_PROJECTS_PUBLICATIONS_PUBLICATION_ERROR_CONTACT_EMAIL_ADDRESS_MISSING'),
                        'projects'
                    );
                    App::redirect(Route::url($pub->link('editversion') . '&action=' . $this->_task));
                    return;
        ...
        if (
            $state == 1 && (!$pub->version->get('accepted')
                || $pub->version->get('accepted') == '0000-00-00 00:00:00')
        ) {
        ...
        if ($requireDoi > 0 && $this->_task == 'publish' && !$pub->version->doi) {
            ...
            if ($doi) {
                ...
                $this->setError(Lang::txt('PLG_PROJECTS_PUBLICATIONS_ERROR_REGISTER_DOI')
                    . ' ' . $doiService->getError());
                $doiErr = true;
            }
        }

        $updateDoiMetadata = ($pub->version->doi
            ? preg_match("/" . $doiService->configs()->shoulder . "/", $pub->version->doi)
            : false
        );
        ...
        if (!$review && ($autoApprove || $this->_pubconfig->get('autoapprove') == 1) && $updateDoiMetadata) {
            ...
            if ($doiService->getError()) {
                $this->setError(Lang::txt('PLG_PROJECTS_PUBLICATIONS_ERROR_UPDATE_DOI')
                    . ' ' . $doiService->getError());
                $doiErr = true;
            } else {
                ...
                if ($doiService->getError()) {
                    $this->setError(Lang::txt('PLG_PROJECTS_PUBLICATIONS_ERROR_REGISTER_NAME_URL')
                        . ' ' . $doiService->getError());
                    $doiErr = true;
                }
            }
        ...
        if ($this->_task == 'revert' && $updateDoiMetadata && $originalStatus == 1) {
            ...
            if ($doiService->getError()) {
                $this->setError(Lang::txt('PLG_PROJECTS_PUBLICATIONS_ERROR_UNREGISTER_DOI')
                    . ' ' . $doiService->getError());
                $doiErr = true;
            }
        }
    ...
    public function onAfterChangeState($pub, $originalStatus = 3)
    {
        ...
        $this->_msg .= ' <a href="' . Route::url($pub->link('version')) . '">'
            . Lang::txt('PLG_PROJECTS_PUBLICATIONS_PUBLICATION_VIEWIT') . '</a>';

        $pubtitle = Str::truncate($pub->version->get('title'), 100);
        ...
        if (!$this->model->isProvisioned() && !$this->getError()) {
            $aid = $this->model->recordActivity(
                ...
                $pub->id,
                $pubtitle,
                Route::url($pub->link('editversion')),
                'publication',
                1
            );
        }
        ...
        if ($notify) {
            ...
            $curatorMessage = ($pub->version->get('state') == 5)
                ? $message . "\n" . "\n"
                . Lang::txt('PLG_PROJECTS_PUBLICATIONS_EMAIL_CURATORS_REVIEW') . ' ' . $curationPubLink
                : $message;
            ...
            foreach ($curatorgroups as $curatorgroup) {
                if (trim($curatorgroup) && $group = Group::getInstance($curatorgroup)) {
                    $members = $group->get('members');
                    $managers = $group->get('managers');
            ...
            if (!$this->getError()) {
                ...
                if ($pub->version->get('state') == 1) {
                    ...
                    if (!$pub->version->store()) {
                        ...
                        $action .= ' ' . strtolower(Lang::txt('version')) . ' ' . $pub->version->get('version_label');
                        $action .= ' ' . Lang::txt('PLG_PROJECTS_PUBLICATIONS_OF') . ' '
                            . strtolower(Lang::txt('publication')) . ' "' . $pubtitle . '" ';
                    ...
                    $path = PATH_APP . DS . trim($this->_pubconfig->get('webpath'), DS) . DS
                        . Str::pad($pid);

                    // Build version path
                    $vPath = $path . DS . Str::pad($vid);
    ...
    protected function _pubDiskSpace($model) // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        $view->quota = $view->params->get('pubQuota')
            ? $view->params->get('pubQuota')
            : \Components\Projects\Helpers\Html::convertSize(
                floatval($model->config()->get('pubQuota', '1')),
                'GB',
                'b'
            );
    ...
    public function serve($type = '', $projectid = 0, $query = '')
    {
        ...
        $pubPath = \Components\Publications\Helpers\Html::buildPubPath(
            $data->pid,
            $data->vid,
            $base_path,
            $folder,
