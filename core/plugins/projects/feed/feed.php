...
class Feed extends Plugin
{
    ...
    public function onProjectExtras($model, $area)
    {
        ...
        $html = ModuleHelper::renderModules('projectpage');
    ...
    protected function _save() // phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    {
        ...
        // Text clean-up
        $comment['description'] = Sanitize::stripScripts($comment['description']);
        $comment['description'] = Sanitize::stripImages($comment['description']);

        $row = Log::oneOrNew($comment['id'])->set($comment);
        ...
        if ($comment['description']) {
            ...
            if (!$row->save()) {
                ...
                $this->_msg = ($isNew
                    ? Lang::txt('PLG_PROJECTS_BLOG_NEW_BLOG_ENTRY_SAVED')
                    : Lang::txt('PLG_PROJECTS_BLOG_BLOG_ENTRY_SAVED'));
            }
            ...
            if ($row->get('id') && $isNew) {
                ...
                if ($row->get('parent')) {
                    ...
                    $currentRecipients = Recipient::all()
                        ->whereEquals('log_id', $row->get('parent'))
                        ->whereEquals('state', Recipient::STATE_PUBLISHED)
                        ->rows();
                ...
                Event::trigger('system.logActivity', [
                    'activity' => [
                        ...
                        'details' => array(
                            'url' => Route::url(
                                $this->model->link() . '&active=' . $this->_name . '#activity' . $row->get('id')
                            ),
    ...
    public function onSharedUpdate($model, $entry, $managers = 0, $posted_by = 0, $posted = null)
    {
        ...
        $entry = Sanitize::stripScripts((string) $entry);
        $entry = Sanitize::stripImages($entry);
