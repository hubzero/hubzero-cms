...
class WikiParser
{
        ...
        if (isset($matches[1]) && $matches[1] != '') {
            ...
            if (!isset($_macros[$matches[1]])) {
                ...
                if (class_exists($macroname)) {
                    ...
                    // No such macro: remember that, and leave $macro unset so
                    // the is_object() check below returns an empty string
                    $macro = false;
    ...
    private function _getTable($matches)
    {
        ...
        $hasTheadRow = false;
        if (!empty($rowMeta[0])) {
            $headerCount = count(array_filter($rowMeta[0], function ($t) {
                return $t === 'h';
            }));
        ...
        foreach ($rows as $i => $rowData) {
            ...
            foreach ($rowData['cells'] as $j => $cellData) {
                ...
                if ($ctyp === 'h') {
                    ...
                    } elseif ($allTh && strpos($catts, 'colspan') !== false) {
                ...
                } elseif ($ctyp === 'd' && $isFirstRow && $hasTheadRow) {
                    // Promote cells in header row to <th> when hasTheadRow
                    $ctyp = 'h';
                    $catts = ' scope="col"' . $catts;
                } elseif ($ctyp === 'd' && $j === 0 && !$isFirstRow && $hasTheadRow) {
        ...
        foreach ($textLines as $oLine) {
            ...
            if (0 == $prefixLength) {
                ...
                $closematch = preg_match(
                    ...
                    '<td|<th|<\\/?div|<hr|<\\/pre|<\\/p|' . $this->token() . '-pre|<\\/li|<\\/dl|<\\/ul|<\\/ol|<\\/?center)/iS',
                    $t
                );
