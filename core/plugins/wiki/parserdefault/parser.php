...
class WikiParser
{
    ...
    public function parse($text, $fullparse = true, $linestart = 0, $camelcase = 1)
    {
        ...
        //$text = preg_replace_callback('/MACRO' . $this->token . '/i', array(&$this, 'restore_macros'), $text);
        $text = preg_replace_callback('/<(macro) (.+?)>(.*)<\/(\1) \2>/si', array(&$this, 'dataPull'), $text);
        $this->tokens['macro'] = array();
        ...
        $pattern = '!<p>\s*(</?(?:table|tr|td|th|div|ul|ol|li|pre|select|form|blockquote|p|h[1-6])[^>]*>)!';
        $text = preg_replace($pattern, "$1", $text);
        $pattern = '!(</?(?:table|tr|td|th|div|ul|ol|li|pre|select|form|blockquote|p|h[1-6])[^>]*>)\s*</p>!';
        $text = preg_replace($pattern, "$1", $text);

        // Format headings and build a table of contents
        if (
            $this->get('fullparse')
            && strstr($text, '<p>MACRO' . $this->token() . '[[TableOfContents]]' . "\n" . '</p>')
        ) {
            $text = $this->toc($text);
        }
    ...
    public function links($text)
    {
        ...
        $char_regexes = array(
            ...
            'autourl'    => "(?<=[^=\"\'\[])\!?" .  // Make sure it's not preceeded by quotes and brackets
                //"(https?:|mailto:|ftp:|gopher:|news:|file:)" .  // protocol
                //"([^ |\\/\"\']*\\/)*([^ |\\t\\n\\/\"\'\<]*[A-Za-z0-9\\/?=&~_])",  // link
                "(?i)\b((?:(https?:|mailto:|ftp:|gopher:|news:|file:)\/\/|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)" .
                "([^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|" .
                "[^\s`!()\[\]{};:'\".,<>?«»“”‘’]))",
     ...
     * @return     string
     */
    private function restoreAnchor($txt)
    {
        return $txt;
    }
    ...
    public function linkAuto($matches)
    {
        ...
        if (
            !preg_match('/^(https?:|mailto:|ftp:|gopher:|news:|file)/i', $href)
            && substr($href, 0, strlen('://')) != '://'
        ) {
            $href = 'http://' . $href;
        }
        ...
        return $prfx . $this->dataPush(array(
            $matches[0],
            'anchor',
            $this->randomString(),
    ...
    public function linkInternal($matches)
    {
        ...
        if ($namespace == 'help') {
            ...
            $p = \Components\Wiki\Models\Page::oneByPath(
                ($scope ? $scope . '/' . $pagename : $pagename),
                $this->get('domain'),
                $this->get('domain_id')
            );
        }
        ...
        return $this->dataPush(array(
            ...
            '<a class="'
                . $cls
                . '" href="'
                . str_replace(array('\\', '"', "'"), '', $href)
                . '">'
                . $this->glyphs(trim($title))
                . '</a>'
        ));
    }
    ...
    private function strip($text)
    {
        ...
        foreach ($lines as $i => $txt) {
            ...
            if ('' == $line) {
                ...
                $output[] = '';
            } elseif ('{{{' == substr($line, 0, 3) || '<pre>{{{' == substr($line, 0, 8)) {
        ...
        $output = implode("\n", $output);
        $output = preg_replace_callback('/<(pre) (\w+?)>(.*)<\/(\1) \2>/si', array(&$this, 'dataPush'), $output);
     ...
     * @return     string
     */
    private function restorePre($txt)
    {
    ...
    private function code($text)
    {
        ...
        foreach ($rules as $start => $end) {
            $text = preg_replace_callback(
                '/(^|\s|[[({>"])' . preg_quote($start, '/') . '(.*?)' . preg_quote($end, '/') . '(\s|$|["\])}])?/ms',
                array(&$this, 'getCode'),
                $text
            );
        }
    ...
    private function getCode($m)
    {
        ...
        return $before . $this->dataPush(array(
            $whole,
            'code',
            $this->randomString(),
            $text
     ...
     * @return     string
     */
    private function restoreCode($txt)
    {
        if (!$this->get('wikitohtml', true)) {
    ...
    private function cleanXss($string)
    {
        ...
        $string = preg_replace(
            ...
            's[\x00-\x20]*c[\x00-\x20]*r[\x00-\x20]*i[\x00-\x20]*p[\x00-\x20]*t[\x00-\x20]*:#iUu',
            '$1=$2nojavascript...',
            $string
        );
        $string = preg_replace(
            '#([a-z]*)[\x00-\x20]*=([\'\"]*)[\x00-\x20]*v[\x00-\x20]*b[\x00-\x20]*' .
            's[\x00-\x20]*c[\x00-\x20]*r[\x00-\x20]*' .
            'i[\x00-\x20]*p[\x00-\x20]*t[\x00-\x20]*:#iUu',
            '$1=$2novbscript...',
            $string
        );
        ...
        // Only works in ie...
        $pattern = '#(<[^>]+)style[\x00-\x20]*=[\x00-\x20]*([\`\'\"]*).*expression[\x00-\x20]*\([^>]*>#iU';
        $string = preg_replace($pattern, "$1>", $string);
        $pattern = '#(<[^>]+)style[\x00-\x20]*=[\x00-\x20]*([\`\'\"]*).*behaviour[\x00-\x20]*\([^>]*>#iU';
        $string = preg_replace($pattern, "$1>", $string);
        $string = preg_replace(
            '#(<[^>]+)style[\x00-\x20]*=[\x00-\x20]*([\`\'\"]*).*s[\x00-\x20]*c[\x00-\x20]*r[\x00-\x20]*i[\x00-\x20]*'
            . 'p[\x00-\x20]*t[\x00-\x20]*:*[^>]*>#iUu',
            "$1>",
            $string
        );
        ...
        do {
            $oldstring = $string;
            $string = preg_replace(
                '#</*(applet|meta|xml|blink|link|head|body|style|script|embed|object|iframe|frame|frameset|' .
                'ilayer|layer|bgsound|title|base)[^>]*>#i',
                '',
                $string
            );
    ...
    private function admonitions($text)
    {
        ...
        return preg_replace_callback(
            '/\{admonition ([^\}]+)\}(.*?)\{\/admonition \1\}/s',
            array(&$this, 'getAdmonition'),
            $text
        ); //admonitionCallback
    }
     ...
     * @return  string  Parsed wiki content
     */
    private function getAdmonition($matches)
    {
        return $this->restorePre($matches[2]);
    }
        ...
        if (isset($matches[1]) && $matches[1] != '') {
            ...
            if (count($macro->linkLog) > 0) {
                foreach ($macro->linkLog as $linkLog) {
                    array_push($this->data['links'], $linkLog);
                }
            }
     ...
     * @return  string
     */
    private function restoreMacro($txt)
    {
        return $txt;
    }
    ...
    private function pba($in, $element = '', $include_id = 1)
    {
        ...
        if (!empty($in)) {
            ...
            if ($element == 'td' or $element == 'tr') {
                if (preg_match("/(" . self::$patterns['vlgn'] . ")/", $matched, $vert)) {
            ...
            if (preg_match("/(" . self::$patterns['hlgn'] . ")/", $matched, $horiz)) {
     ...
     * @return  string
     */
    private function getQuotes($text)
    {
    ...
    {
        $ac  = self::$patterns['algn'] . self::$patterns['clss'];
        $sac = self::$patterns['spns'] . self::$patterns['algn'] . self::$patterns['clss'];
     ...
     * @return  string
     */
    private function getTable($matches)
    {
        ...
        foreach (preg_split("/\|\|( *)$/m", $matches[2], -1, PREG_SPLIT_NO_EMPTY) as $row) {
            ...
            foreach (explode('||', $row) as $cell) {
                ...
                // Cell attributes
                if (
                    preg_match("/^(_?{$sac}\.)(.*)/", $cell, $cmtch)
                    || preg_match("/^(=?{$sac}\.)(.*)=?/", $cell, $cmtch)
                ) {
                    // Parse Block Attributes
                ...
                if (!$catts) {
                    ...
                    } elseif ($suffixLength > 0 && $prefixLength == 0) {
        ...
        foreach ($textLines as $oLine) {
            ...
            if (0 == $prefixLength) {
                ...
                $openmatch  = preg_match(
                    '/(?:<table|<blockquote|<h1|<h2|<h3|<h4|<h5|<h6|<pre|<tr|<p|<dl|<ul|<ol|<li|<\\/tr|<\\/td' .
                    '|<\\/th)/iS',
                    $t
                );
    ...
    public function toc($text)
    {
        ...
        foreach ($matches[3] as $headline) {
            ...
            if ($enoughToc && (!isset($maxTocLevel) || $toclevel < $maxTocLevel)) {
                ...
                $toc .= $this->tocLine($anchor, $_tocLine, $numbering, $toclevel);
            }
            ...
            {*/
                $editlink = '';
            //}
            $head[$headlineCount] = $this->makeHeadline(
                $level,
