...
class Resources extends WikiMacro
{
    ...
    public function description()
    {
        ...
        $txt['html'] = '<p>This macro will insert a linked title to a resource. It can be passed '
            . 'either an ID or alias.</p>';
        return $txt['html'];
    }
    ...
    public function render()
    {
        ...
        if ($r) {
            ...
            if ($nolink) {
                ...
                return '<a href="' . \Hubzero\Facades\Route::url($link) . '">' . stripslashes($r[1]) . '</a>';
            }
        }
    ...
    public function screenshots($alias, $num = 1)
    {
        ...
        if ($d) {
            while (false !== ($entry = $d->read())) {
                $img_file = $entry;
                if (
                    is_file(PATH_APP . $path . DS . $alias . DS . $img_file)
                    && substr($entry, 0, 1) != '.'
