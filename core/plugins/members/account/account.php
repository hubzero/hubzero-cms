...
// @phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
                ...
                if ($passinfo = $this->getPassInfo()) {
                    ...
                    if ($passinfo['diff'] <= $passinfo['warning'] && $passinfo['diff'] > 0) {
                        ...
                        $arr['metadata']['alert'] = '<a class="alrt" href="' . $link . '">' .
                            '<span><h5>Password Expiration</h5>' . $title . '</span></a>';
                    } elseif ($passinfo['diff'] < 0) {
    ...
    private function sendtoken()
    {
        ...
        if ($this->user->isGuest()) {
            Notify::warning(Lang::txt('You must be logged in to access this area.'));
            ...
            App::redirect(
                Route::url(
                    ...
                    . base64_encode(
                        Route::url(
                            'index.php?option=' . $this->option . '&task=myaccount&active=account&action=sendtoken',
                            false
                        )
                    ),
                    false
        ...
        Notify::warning(
            Lang::txt(
                'Please check the email associated with this account (' . $this->member->get('email')
                . ') for your confirmation token!'
            )
        );

        App::redirect(
            Route::url($this->member->link() . '&active=account&task=confirmtoken', false)
        );
    ...
    private function confirmtoken()
    {
        ...
        if ($this->user->isGuest()) {
            ...
            App::redirect(
                Route::url(
                    ...
                    base64_encode(
                        Route::url(
                            'index.php?option=' . $this->option . '&task=myaccount&active=account&action=confirmtoken'
                        )
                    ),
                    false
                )
        ...
        if (strlen($token) != 32)
        {
            ...
            $view->notifications = array(
                array(
                    'type' => 'error',
                    'message' => Lang::txt('Invalid token length, please re-input token')
                )
            );
            return $view->loadTemplate();
        }
        */
        ...
        if (!$row = $this->getToken()) {
            ...
            $view->notifications = array(
                array(
                    'type' => 'error',
                    'message' => Lang::txt('You must be logged in to access this area.')
                )
            );
        ...
        if (!($crypt == $testcrypt))
        {
            ...
            $view->notifications = array(
                array(
                    'type' => 'error',
                    'message' => Lang::txt('Invalid token.  Please try re-entering your token')
                )
            );
            return $view->loadTemplate();
        }
        */
        ...
        Notify::warning(Lang::txt('Please provide a new password'));

        App::redirect(
            Route::url($this->member->link() . '&active=account&task=setlocalpass', false)
        );
    ...
    private function setlocalpass()
    {
        ...
        if ($this->user->isGuest()) {
            ...
            App::redirect(
                Route::url(
                    ...
                    base64_encode(
                        Route::url(
                            'index.php?option=' . $this->option . '&task=myaccount&active=account&action=setlocalpass'
                        )
                    ),
                    false
                )
        ...
        if (is_null($token)) {
            ...
            Notify::error(Lang::txt('You must first verify your email address by inputting the token.'));

            App::redirect(
                Route::url(
                    'index.php?option=' . $this->option . '&id='
                    . $this->user->get('id') . '&active=account&task=sendtoken',
                    false
                )
            );
            return;
        ...
        if (!$password1 || !$password2) { // must enter password twice
            ...
            $this->setError(
                Lang::txt(
                    'Password does not meet site password requirements. ' .
                    'Please choose a password meeting all the requirements listed.'
                )
            );
            $passrules = true;
        }
        ...
        if (Request::getInt('no_html', 0)) {
            ...
            Notify::success(Lang::txt('Password reset successful'));

            App::redirect(
                Route::url($this->member->link() . '&active=account', false)
            );
        }
    ...
    private function unlink()
    {
        ...
        if (empty($hzup->passhash) && count(\Hubzero\Auth\Link::find_by_user_id($this->member->get('id'))) <= 1) {
            Notify::warning(Lang::txt('PLG_MEMBERS_ACCOUNT_CANT_REMOVE_ONLY_ACCESS'));
        ...
        // Set the redirect
        Notify::success(Lang::txt('PLG_MEMBERS_ACCOUNT_UNLINKED'));
    ...
    private function getPassInfo()
    {
        ...
        if ($diff > $hzup->shadowWarning) {
            $message_style = 'info';
        } elseif ($diff <= $hzup->shadowWarning && $diff > 0) {
    ...
    private function getToken()
    {
        ...
        $db = App::get('db');
        $db->setQuery(
            'SELECT id, params FROM `#__users` WHERE block = 0 AND username = '
            . $db->Quote($this->user->get('username'))
        );
