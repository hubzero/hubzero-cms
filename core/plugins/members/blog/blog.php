...
// @phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
    ...
    private function feed()
    {
        ...
        $doc->title       = Config::get('sitename') . ' - '
            . stripslashes($this->member->get('name')) . ': ' . Lang::txt('Blog');
        $doc->description = Lang::txt(
            'PLG_MEMBERS_BLOG_RSS_DESCRIPTION',
            Config::get('sitename'),
            stripslashes($this->member->get('name'))
        );
        $doc->copyright   = Lang::txt(
            'PLG_MEMBERS_BLOG_RSS_COPYRIGHT',
            date("Y"),
            Config::get('sitename')
        );
    ...
    private function entry()
    {
        if (isset($this->entry) && is_object($this->entry)) {
            ...
            $states = array(
                \Components\Blog\Models\Entry::STATE_UNPUBLISHED,
                \Components\Blog\Models\Entry::STATE_PUBLISHED
            );
    ...
    private function save()
    {
        ...
        $langKey = 'PLG_MEMBERS_BLOG_ACTIVITY_ENTRY_' . ($entry['id'] ? 'UPDATED' : 'CREATED');
    ...
    private function savecomment()
    {
        ...
        $langKey = 'PLG_MEMBERS_BLOG_ACTIVITY_COMMENT_' . ($data['id'] ? 'UPDATED' : 'CREATED');
        ...
        Event::trigger(
            ...
            'activity' => [
                ...
                'description' => Lang::txt(
                    'PLG_MEMBERS_BLOG_ACTIVITY_COMMENT_DELETED',
                    $comment->get('id'),
                    $link
                ),
                'details'     => array(
