...
// @phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _save($database, $option, $member, $task, $emp)
    {
        ...
        } elseif ($task == 'savetitle' && $author && $title) {
...
// @phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _upload($database, $option, $member)
    {
        ...
        // Incoming
        $title = Request::getString('title', '');
        $default_title = $member->get('firstname')
        ...
        $file['name'] = $member->get('firstname')
            ? $member->get('firstname') . ' ' . $member->get('lastname') . ' '
            . ucfirst(Lang::txt('PLG_MEMBERS_RESUME_RESUME'))
            : $member->get('name') . ' '
            . ucfirst(Lang::txt('PLG_MEMBERS_RESUME_RESUME'));
        ...
        if (!$row->loadResume($member->get('id'))) {
            ...
            $row->main = 1;
        } elseif (file_exists($path . DS . $row->filename)) { // remove prev file first
...
// @phpcs:ignore PSR2.Methods.MethodDeclaration.Underscore
    protected function _download($member)
    {
        ...
        $default_title = $member->get('givenName')
            ? $member->get('givenName') . ' ' . $member->get('surname') . ' ' . ucfirst(Lang::txt('Resume'))
            : $member->get('name') . ' ' . ucfirst(Lang::txt('Resume'));
