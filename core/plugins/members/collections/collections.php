...
// @phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
        ...
        if ($returnhtml) {
            ...
            switch ($this->action) {
                ...
                case 'following':
                    ...
                    break;
                case 'follow':
                    $arr['html'] = $this->follow('member');
                    break;
                case 'unfollow':
                    $arr['html'] = $this->unfollow('member');
                    break;
    ...
    {
        $route = Route::url(
            'index.php?option=' . $this->option . '&id=' . $this->member->get('id') . '&active=' . $this->_name
        );
    ...
    private function post()
    {
        ...
        if (
            $collection->get('access') == 4 // private collection
            && User::get('id') != $this->member->get('id')
        ) { // is user the collection owner?
        ...
        if (!$this->params->get('access-view-item')) {
            App::abort(
                403,
                Lang::txt('PLG_MEMBERS' . strtoupper($this->_name) . 'NOT_AUTH')
            );
            return;
        }
    ...
    private function save()
    {
        ...
        if ($tmp) {
            $db = App::get('db');
            $db->setQuery(
                "SELECT id FROM `#__collections_items` WHERE `title`=" . $db->quote($tmp)
            ...
            if (count($others) > 0) {
                $db->setQuery(
                    "SELECT * FROM `#__collections_assets` WHERE `item_id` in (" . implode(",", $others) . ")"
                );
        ...
        Event::trigger(
            ...
            'activity' => [
                ...
                'description' => Lang::txt(
                    'PLG_MEMBERS_COLLECTIONS_ACTIVITY_POST_' . ($p['id'] ? 'UPDATED' : 'CREATED'),
                    '<a href="' . Route::url($url) . '">' . $collection->get('title') . '</a>'
                ),
                'details' => array(
    ...
    private function repost()
    {
        ...
        Event::trigger(
            ...
            'activity' => [
                ...
                'scope_id' => $post->id,
                'description' => Lang::txt(
                    'PLG_MEMBERS_COLLECTIONS_ACTIVITY_POST_CREATED',
        ...
        if ($no_html) {
            echo Lang::txt(
                '%s reposts',
                $post->getCount(array('item_id' => $post->get('item_id'), 'original' => 0))
            );
            exit;
        }
    ...
    private function remove()
    {
        ...
        Event::trigger(
            ...
            'activity' => [
                ...
                'description' => Lang::txt(
                    'PLG_MEMBERS_COLLECTIONS_ACTIVITY_POST_DELETED',
                    '<a href="' . $route . '">' . $collection->get('title') . '</a>'
                ),
                'details' => array(
    ...
    private function delete()
    {
        ...
        Event::trigger(
            ...
            'activity' => [
                ...
                'scope_id' => $item->get('id'),
                'description' => Lang::txt(
                    'PLG_MEMBERS_COLLECTIONS_ACTIVITY_ITEM_DELETED',
                    '<a href="' . $route . '">' . $item->get('title') . '</a>'
                ),
                'details' => array(
                    'collection_id' => $post->get('collection_id'),
                    'post_id' => $post->get('id'),
    ...
    private function savecomment()
    {
        ...
        Event::trigger(
            ...
            'activity' => [
                ...
                'description' => Lang::txt(
                    'PLG_MEMBERS_COLLECTIONS_ACTIVITY_COMMENT_' . ($data['id'] ? 'UPDATED' : 'CREATED'),
                    $comment->get('id'),
                    '<a href="' . $url . '#c' . $comment->get('id') . '">' . $title . '</a>'
                ),
                'details' => array(
                    'collection_id' => $post->get('collection_id'),
                    'post_id' => $post->get('id'),
    ...
    private function vote()
    {
        ...
        Event::trigger(
            ...
            'activity' => [
                ...
                'description' => Lang::txt(
                    'PLG_MEMBERS_COLLECTIONS_ACTIVITY_ITEM_VOTED',
                    '<a href="' . $url . '">' . $collection->get('title') . '</a>'
                ),
                'details' => array(
                    'collection_id' => $collection->get('id'),
                    'post_id' => $post->get('id'),
    ...
    private function savecollection()
    {
        ...
        // Access check
        if (
            !$this->params->get('access-edit-collection')
            || !$this->params->get('access-create-collection')
        ) {
            $this->setError(Lang::txt('PLG_MEMBERS_' . strtoupper($this->_name) . '_NOT_AUTHORIZED'));
            return $this->collections();
        }
        ...
        if (
            $collection->get('access') != 0
            && $collection->get('access') != 1
            && $collection->get('access') != 4
        ) {
        ...
        Event::trigger(
            ...
            'activity' => [
                ...
                'description' => Lang::txt(
                    'PLG_MEMBERS_COLLECTIONS_ACTIVITY_COLLECTION_' . ($fields['id'] ? 'UPDATED' : 'CREATED'),
                    '<a href="' . $url . '">' . $collection->get('title') . '</a>'
                ),
                'details' => array(
    ...
    private function deletecollection()
    {
        ...
        Event::trigger(
            ...
            'activity' => [
                ...
                'scope_id' => $collection->get('id'),
                'description' => Lang::txt(
                    'PLG_MEMBERS_COLLECTIONS_ACTIVITY_COLLECTION_DELETED',
                    '<a href="' . Route::url($collection->link()) . '">' . $collection->get('title') . '</a>'
                ),
                'details' => array(
                    'title' => $collection->get('title'),
