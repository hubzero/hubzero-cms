...
if ($this->rows->total() > 0) {
    foreach ($this->rows as $row) {
        ...
        <div class="post <?php echo $type; ?>"
            ...
            <?php if (!User::isGuest()) { ?>
                ...
                $isCreatorOrEditor = $row->get('created_by') == User::get('id')
            ...
            <?php if ($row->original() || $item->get('created_by') != $this->member->get('id')) { ?>
                <?php
                $itemCreatorName = $this->escape(
                    stripslashes($item->creator()->get('name'))
                );
                $itemCreatorUrl = Route::url(
                    'index.php?option=com_members&id=' . $item->get('created_by')
                );
                $itemCreatorPic = $item->creator()->picture(0);
                $itemCreatedTime = Date::of('date', $item->get('created'))
                    ->toLocal(Lang::txt('TIME_FORMAT_HZ1'));
                $itemCreatedDate = Date::of($item->get('created'))
                    ->toLocal(Lang::txt('DATE_FORMAT_HZ1'));
                ?>
                <div class="convo attribution clearfix">
                    <a href="<?php echo $itemCreatorUrl; ?>"
                        title="<?php echo $itemCreatorName; ?>"
                        class="img-link">
                        <img src="<?php echo $itemCreatorPic; ?>"
                            alt="Profile picture of <?php echo $itemCreatorName; ?>" />
                    </a>
                    <p>
                        <a href="<?php echo $itemCreatorUrl; ?>">
                            <?php echo $itemCreatorName; ?>
                        </a>
                        posted
                        <br />
                        <span class="entry-date">
                            <span class="entry-date-at">@</span>
                            <span class="date"><?php echo $itemCreatedTime; ?></span>
                            <span class="entry-date-on">on</span>
                            <span class="time"><?php echo $itemCreatedDate; ?></span>
                        </span>
                    </p>
                </div><!-- / .attribution -->
            <?php } ?>
            <?php // if ($item->get('created_by') != $this->member->get('id')) ?>
            <?php if (!$row->original()) { ?>
                <?php
                $rowCreatorName = $this->escape(
                    stripslashes($row->creator()->get('name'))
                );
                $rowCreatorUrl = Route::url(
                    'index.php?option=com_members&id=' . $row->get('created_by')
                );
                $memberPic = $this->member->picture(0);
                $collectionSuffix = $this->collection->get('is_default')
                    ? ''
                ...
                <div class="convo attribution reposted clearfix">
                    <a href="<?php echo $rowCreatorUrl; ?>"
                        title="<?php echo $rowCreatorName; ?>"
                        class="img-link">
                        <img src="<?php echo $memberPic; ?>"
                            alt="Profile picture of <?php echo $rowCreatorName; ?>" />
                    </a>
                    <p>
                        <a href="<?php echo $rowCreatorUrl; ?>">
                            <?php echo $rowCreatorName; ?>
                        </a>
                        onto
                        ...
                        <span class="entry-date">
                            <span class="entry-date-at">@</span>
                            <span class="date"><?php echo $rowCreatedTime; ?></span>
                            <span class="entry-date-on">on</span>
                            <span class="time"><?php echo $rowCreatedDate; ?></span>
                        </span>
                    </p>
                </div><!-- / .attribution -->
            <?php } ?>
    ...
    <?php if ($this->params->get('access-create-item')) { ?>
            ...
            <!-- <div class="questions">
                ...
                <p>This is how you can add other content on the site to a collection. You can collect wiki pages,
                resources, and more. You can even collect other collections!<p>
            </div><!- / .post-type -->
    <?php } else { ?>
            <div class="instructions">
