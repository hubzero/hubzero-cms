...
// @phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
    ...
    private function browseAction()
    {
        ...
        if ($view->isAdmin) {
            ...
            $view->citations = $obj['citations']->paginated()->including(
                [
                'relatedAuthors',
                function ($author) {
        ...
        $view->filters['startuploaddate'] = isset($view->filters['startuploaddate'])
            ? $view->filters['startuploaddate']
            : "";
        $view->filters['enduploaddate'] = isset($view->filters['enduploaddate'])
    ...
    private function editAction($row = null)
    {
        ...
        $view->title = Lang::txt('PLG_MEMBERS_CITATIONS')
            . ': ' . Lang::txt('PLG_MEMBERS_CITATIONS_' . strtoupper($this->action));
        ...
        if (!$id) {
            ...
            if (
                ...
                && ($view->row->relatedAuthors->count() == 0 && $view->row->author != '')
            ) {
    ...
    private function saveAction()
    {
        ...
        if ($isNew) {
            ...
            foreach ($authorField as $key => $a) {
                // Create a new row
                $authorObj = \Components\Citations\Models\Author::blank()->set(
                    array(
                    'cid' => $citation->id,
                    'ordering' => $key,
                    'author' => $a
    ...
    private function deleteAction()
    {
        ...
        if ($id != 0 && !$bulk) {
            ...
            if (
                $citation->save() && $citation->scope == 'member'
    ...
    private function settingsAction()
    {
        if ($_POST) {
            ...
            $view->citations_show_badges = (isset($params->citations_show_badges)
                ? $params->citations_show_badges : "true");
    ...
    private function loginAction()
    {
        App::redirect(
            Route::url(
                ...
                . base64_encode(
                    Route::url(
                        $this->member->link() . '&active=' . $this->_name . '&action=' . $this->action,
                        false,
                        true
                    )
                )
