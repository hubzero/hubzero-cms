...
<form action="{{ Route::url($member->link() . '&active=messages&task=trash') }}" method="post">
  {{-- Filter row --}}
  ...
  <div class="flex flex-wrap items-center gap-2 mb-4">
    ...
    <select id="msg-action" class="select select-bordered select-sm" name="action">
      ...
      <option value="delete">{{ Lang::txt('PLG_MEMBERS_MESSAGES_MSG_DELETE') }}</option>
    </select>
  ...
  <div class="overflow-x-auto">
    <table class="table table-zebra w-full">
      ...
      <tbody>
        ...
        @else
          <tr>
            <td colspan="5">{{ Lang::txt('PLG_MEMBERS_MESSAGES_NONE') }}</td>
          </tr>
        @endif
      </tbody>
    </table>
