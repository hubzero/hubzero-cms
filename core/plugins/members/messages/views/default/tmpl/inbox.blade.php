...
<form action="{{ Route::url($member->link() . '&active=messages') }}" method="post">
  {{-- Filter row --}}
  ...
  <div class="flex flex-wrap items-center gap-2 mb-4">
    ...
    <select id="msg-action" class="select select-bordered select-sm" name="action">
      ...
      <option value="sendtoarchive">{{ Lang::txt('PLG_MEMBERS_MESSAGES_MSG_SEND_TO_ARCHIVE') }}</option>
            ...
            <tr>
              ...
              <td>
                <a class="btn btn-ghost btn-xs"
                   href="{{ $trashUrl }}"
                   aria-label="{{ Lang::txt('PLG_MEMBERS_MESSAGES_DELETE_TITLE') ?? Lang::txt('PLG_MEMBERS_MESSAGES_TRASH') }}">
