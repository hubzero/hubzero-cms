...
// @phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
    ...
    public function onMembersContributionsAreas()
    {
        ...
        if (!$this->cats) {
            // Get categories
            $this->cats = \Components\Resources\Models\Type::getMajorTypes();
        }
    ...
    {
        $query = "SELECT COUNT(R.id) FROM `#__resources` AS R, `#__author_assoc` AS AA WHERE AA.authorid=" . $user_id
            . " AND R.id = AA.subid AND AA.subtable = 'resources' AND R.published=1 AND R.standalone=1";
        return $query;
    }
    ...
    public function onMembersContributions($member, $option, $limit, $limitstart, $sort, $areas = null)
    {
        ...
        if ($limit) {
            ...
            if (isset($filters['sortby']) && ($filters['sortby'] == 'usage' || $filters['sortby'] == 'users')) {
                ...
                $query->select(
                    '(SELECT rs.users FROM ' . $s . ' AS rs WHERE rs.resid=' . $t
                    . '.id AND rs.period=14 ORDER BY rs.datetime DESC LIMIT 1)',
                    'users'
                );
            }
