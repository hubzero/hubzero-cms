...
class MarkdownExtra extends Markdown
{
    ...
    {
        return ($line[0] === ' ' || $line[0] === '[') && preg_match(
            '/^ {0,3}\[(.+?)\]:\s*([^\s]+?)(?:\s+[\'"](.+?)[\'"])?\s*(' . $this->specialAttributesRegex . ')?\s*$/',
            $line
        );
    }
    ...
    {
        while (
            isset($lines[$current]) && preg_match(
                '/^ {0,3}\[(.+?)\]:\s*(.+?)(?:\s+[\(\'"](.+?)[\)\'"])?\s*(' . $this->specialAttributesRegex . ')?\s*$/',
                $lines[$current],
                $matches
            )
        ) {
            ...
            if (isset($matches[3])) {
                ...
                if (
                    isset($lines[$current + 1]) && preg_match(
                        '/^\s+[\(\'"](.+?)[\)\'"]\s*$/',
                        $lines[$current + 1],
                        $matches
                    )
    ...
    protected function renderLink($block)
    {
        ...
        return '<a href="' . htmlspecialchars($block['url'], ENT_COMPAT | ENT_HTML401, 'UTF-8') . '"'
            . (empty($block['title']) ? '' : ' title="' . htmlspecialchars(
                $block['title'],
