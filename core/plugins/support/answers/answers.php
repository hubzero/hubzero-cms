...
// phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
                ...
                switch ($category) {
                    case 'answer':
                        $rows[$key]->href = ($parent)
                            ? Route::url('index.php?option=com_answers&task=question&id=' . $parent)
                            : '';
                        break;
                    case 'question':
                        $url = 'index.php?option=com_answers&task=question&id=' . $rows[$key]->id;
    ...
    public function deleteReportedItem($referenceid, $parentid, $category, $message)
    {
        ...
        switch ($category) {
            ...
            case 'question':
                ...
                $query = "SELECT r.id, r.created_by FROM `#__answers_responses` AS r "
                    . "WHERE r.question_id=" . $referenceid;
                $database->setQuery($query);
                ...
                if ($banking && $reward) {
                        ...
                        foreach ($responders as $r) {
                            ...
                            if (is_object($zuser)) {
                                ...
                                if ($validEmail && $email) {
                                    ...
                                    $mes .= '----------------------------' . "\r\n\r\n";
                                    $mes .= Lang::txt(
                                        'PLG_SUPPORT_ANSWERS_QUESTION',
                                        $referenceid
                                    ) . "\r\n";

                                    \Components\Support\Helpers\Utilities::sendEmail(
                                        $hub,
                                        $zuser->get('email'),
                                        $sub,
                                        $mes
                    ...
                    if ($asker) {
                        ...
                        if (isset($asker_id)) {
                            // Remove hold
                            $sql = "DELETE FROM `#__users_transactions` "
                                . "WHERE category='answers' AND type='hold' "
                                . "AND referenceid=" . $parentid . " AND uid='" . $asker_id . "'";
    ...
    public function getReward($id)
    {
        ...
        $sql = "SELECT amount FROM `#__users_transactions` "
            . "WHERE category='answers' AND type='hold' AND referenceid=" . $id;
        $database->setQuery($sql);
