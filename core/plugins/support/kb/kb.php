...
class plgSupportKb extends \Hubzero\Plugin\Plugin
{
    ...
    public function getReportedItem($refid, $category, $parent)
    {
        ...
        $query = "SELECT rc.id, rc.content as text, rc.created_by as author, rc.created, NULL as subject, "
            . "rc.anonymous as anon, 'kb' AS parent_category, "
            . "c.alias AS section, c.alias AS category, f.alias AS article "
            . "FROM `#__kb_comments` AS rc "
            . "LEFT JOIN `#__kb_articles` AS f ON f.id = rc.entry_id "
            . "LEFT JOIN `#__categories` AS c ON c.id = f.category "
            . "WHERE rc.id=" . $refid;
        ...
        if ($rows) {
            foreach ($rows as $key => $row) {
                ...
                $rows[$key]->href = Route::url(
                    'index.php?option=com_kb&category=' . $row->category . '&alias=' . $row->article
                );
            }
        }
        return $rows;
