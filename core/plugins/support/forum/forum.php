...
class Forum extends Plugin
{
    ...
    public function getReportedItem($refid, $category, $parent)
    {
        ...
        $query = "SELECT rc.id, rc.comment as `text`, rc.parent, rc.created_by as author, rc.created, "
            . "rc.title as subject, rc.anonymous as anon, 'forum' AS parent_category, "
        ...
        if ($rows) {
            ...
            foreach ($rows as $key => $row) {
                ...
                if ($row->parent)
                {
                    $thread = $this->getThread($row->parent);
                }*/
                ...
                switch ($row->scope) {
                    case 'course':
                        ...
                        $url = 'index.php?option=com_courses&gid=' . $course->get('alias')
                            . '&controller=offering&offering=' . $offering->get('alias')
                            . '&active=discussions&thread=' . $row->thread;
                        break;

                    case 'group':
                        $group = \Hubzero\User\Group::getInstance($row->scope_id);
                        $url = 'index.php?option=com_groups&cn=' . $group->get('cn')
                            . '&active=forum&scope=' . $row->section . '/' . $row->category . '/' . $parent;
                        break;

                    case 'site':
                    default:
                        $url = 'index.php?option=com_forum&section=' . $row->section
                            . '&category=' . $row->category . '&thread=' . $parent;
                        break;
                }
     ...
     * @return  array
     */
    private function getThread($parent = 0)
    {
