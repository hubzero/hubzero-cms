...
class Captcha extends Plugin
{
    ...
    public function onGetCaptcha($ext = 'com')
    {
        ...
        $type = 'generate' . ucfirst(strtolower($type)) . 'Captcha';
    ...
    public function onValidateCaptcha()
    {
        ...
        if (
            $task && $task != 'logout'
            && $this->confirm(trim($captcha['answer']), trim($captcha['instance']), trim($captcha['krhash']))
        ) {
            return true;
        }
    ...
    private function generateTextCaptcha()
    {
        ...
        $nqs = count($this->questions);
        $use = rand(0, $nqs - 1);
        $question = $this->questions[$use];

        $html = '<label for="captcha-answer">' . "\n";
        $html .= Lang::txt('PLG_SUPPORT_TEXTCAPTCHA_PLEASE_ANSWER') . ' <span class="required">'
            . Lang::txt('PLG_SUPPORT_TEXTCAPTCHA_REQUIRED') . '</span><br />' . "\n";
        switch ($question['type']) {
            ...
            case 'list':
                ...
                $numseq = count($this->sequence);
                $set = rand(0, $numseq - 1);

                $html .= "\t" . Lang::txt($question['question']) . "\n";
                $html .= "\t" . '<select name="captcha[answer]" id="captcha-answer">' . "\n";
                $html .= "\t\t" . '<option value="">'
                    . Lang::txt('PLG_SUPPORT_TEXTCAPTCHA_SELECT_ANSWER') . '</option>' . "\n";
                foreach ($this->sequence[$set] as $row) {
                    $html .= "\t\t" . '<option value="'
                        . Lang::txt($question['options'][$row]) . '">'
                        . Lang::txt($question['options'][$row]) . '</option>' . "\n";
                }
                $html .= "\t" . '</select>' . "\n";
                break;
        ...
        $html .= '</label>' . "\n";
        $html .= "\t" . '<input type="hidden" name="captcha[krhash]" id="captcha-krhash" value="'
            . $this->generateHash($key, date('j')) . '" />' . "\n";
        //$html .= "\t" . '<input type="hidden" name="captcha[type]" id="captcha-type" value="text" />' . "\n";

        App::get('session')->set(
            'securiy_code' . Request::getInt('instanceNo', $GLOBALS['totalCaptchas']),
            $this->generateHash($key, date('j'))
        );
        ...
        $html .= '<input type="hidden" name="captcha[instance]" id="captcha-instance" value="'
            . $GLOBALS['totalCaptchas'] . '" />' . "\n";
     ...
     * @return string
     */
    private function generateHash($input, $day)
    {
        // Add date:
        ...
        $enc = substr($enc, 26, 1) . substr($enc, 10, 1) . substr($enc, 23, 1) . substr($enc, 3, 1)
            . substr($enc, 19, 1);
    ...
    private function generateImageCaptcha()
    {
        ...
        return $this->getImageCaptchaHtml();
    }
    ...
    {
        $imageFunction = 'createImage' . $this->params->get('imageFunction');
        $imageFunction = (!method_exists($this, $imageFunction)) ? 'createImageAdv' : $imageFunction;
     ...
     */
    private function confirm($word, $instanceNo = '', $hash = '')
    {
        $word = $this->generateHash(strtolower($word), date('j'));

        $securiy_code = App::get('session')->get('securiy_code' . $instanceNo);

        if ($hash && $word == $hash) {
            return true;
        } elseif ($word == $securiy_code) {
            return true;
        } else {
            return false;
        }
     ...
     * @return  string
     */
    private function getImageCaptchaHtml()
    {
        if (!isset($GLOBALS['totalCaptchas'])) {
            $GLOBALS['totalCaptchas'] = -1;
        }
     ...
     * @return  void
     */
    private function createImageAdv()
    {
        if (!isset($GLOBALS['totalCaptchas'])) {
            $GLOBALS['totalCaptchas'] = -1;
        }
        ...
        $security_code = $this->generateHash($this->keystring, date('j'));
     ...
     * @return  void
     */
    private function createImagePlain()
    {
        ...
        $security_code = $this->generateHash($security_code, date('j'));
        ...
        for ($i = 0; $i < strlen($security_code); $i++) {
            ...
            @imagettftext(
                ...
                $y,
                $white,
                __DIR__ . DS . 'assets' . DS . 'fonts' . DS . 'adlibBT.TTF',
                $security_code[$i]
            );
            $x += ($size * 2);
        }
