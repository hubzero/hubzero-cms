...
// phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
    ...
    private function getTopList($database, $period, $dthis, $s_top, $table_header = 'Value')
    {
        ...
        $sql = "SELECT * FROM `#__stats_topvals` WHERE top = " . $database->quote($s_top)
             . " AND datetime = " . $database->quote($dthis . '-00')
             . " AND period = " . $database->quote($period) . " ORDER BY rank";
            ...
            foreach ($results as $row) {
                ...
                if ($row->rank == '0') {
                    ...
                    if ($s_top == "6" || $s_top == "7" || $s_top == "8") {
                        $value = $this->timeUnits($row->value);
                    } else {
                    ...
                    $html .= "\t\t" . '<tr class="' . $cls . '">' . "\n";
                    $html .= "\t\t\t" . '<td>' . $row->rank . '</td>' . "\n";
                    $html .= "\t\t\t" . '<td class="textual-data"><a href="'
                            . Route::url('index.php?option=com_resources&id=' . $name[0]
                            . '&active=usage') . '">' . $name[1] . '</a></td>' . "\n";
                    $html .= "\t\t\t" . '<td>' . $value . '</td>' . "\n";
                    if ($total) {
                        $vl = number_format((($row->value / $total) * 100), 2);
                    }
     ...
     */
    private function getTopRankTools($database)
    {
        $html = '';
        $count = 1;

        $sql = 'SELECT DISTINCT id, title, published, ranking'
        . ' FROM `#__resources`'
        . ' WHERE published = "1"'
        . ' AND standalone = "1"'
        . ' AND type = "7"'
        ...
        if ($results) {
            ...
            foreach ($results as $row) {
                ...
                if ($row->published == "1") {
                    ...
                    $html .= "\t\t\t" . '<td>' . $count . '</td>' . "\n";
                    $html .= "\t\t\t" . '<td class="textual-data"><a href="'
                            . Route::url('index.php?option=com_resources&id=' . $row->id
                            . '&active=usage') . '">' . $row->title . '</a></td>' . "\n";
                    $html .= "\t\t\t" . '<td>' . $ranking . '</td>' . "\n";
                    $html .= "\t\t" . '</tr>' . "\n";
                } else {
                    $html .= "\t\t" . '<tr class="' . $cls . '">' . "\n";
     ...
     * @return  string HTML
     */
    private function getTopCitedTools($database)
    {
        $html = '';
        ...
        if ($result) {
            $html .= "\t" . '<tfoot>' . "\n";
            $html .= "\t\t" . '<tr class="summary">' . "\n";
            $html .= "\t\t\t" . '<th colspan="2" class="numerical-data">'
                    . Lang::txt('Total Tools Citations') . '</th>' . "\n";
        ...
        $count = 1;
        $sql = 'SELECT DISTINCT r.id, r.title, r.published, COUNT(c.id) AS citations'
        . ' FROM `#__resources` r, `#__citations` c, `#__citations_assoc` ca'
        . ' WHERE r.id = ca.oid'
        . ' AND ca.cid = c.id'
        . ' AND ca.tbl = "resource"'
        ...
        if ($results) {
            ...
            foreach ($results as $row) {
                ...
                if ($row->published == "1") {
                    ...
                    $html .= "\t\t\t" . '<td class="textual-data"><a href="'
                            . Route::url('index.php?option=com_resources&id=' . $row->id
                            . '&active=usage') . '">' . stripslashes($row->title) . '</a></td>';
     ...
     * @return  string
     */
    private function timeUnits($time)
    {
        if ($time < 60) {
            $data = $time . ' seconds';
     ...
     * @return  string HTML
     */
    private function dropDownDates(&$db, $period, $s_top, $dthis)
    {
        $months = array(
        ...
        switch ($period) {
            case '3':
                ...
                foreach ($monthsReverse as $key => $month) {
                    $value = $cur_year . '-' . $key;
                    if (!$qtd_found && $this->checkForData($value, 3)) {
            ...
            case '12':
                ...
                for ($i = $cur_year; $i >= $year_data_start; $i--) {
                    foreach ($monthsReverse as $key => $month) {
                        ...
                        $value = $i . '-' . $key;
                        if ($this->checkForData($value, 12) && (intval($key) < $cur_month || $i < $cur_year)) {
                            $html .= '<option value="' . $value . '"';
                            if ($value == $dthis) {
                                $html .= ' selected="selected"';
                            }
            ...
            case '0':
                ...
                foreach ($monthsReverse as $key => $month) {
                    $value = $cur_year . '-' . $key;
                    if (!$ytd_found && $this->checkForData($value, 0)) {
                        $html .= '<option value="' . $value . '"';
                        if ($value == $dthis) {
                            $html .= ' selected="selected"';
                        }
     ...
     * @return  boolean
     */
    private function checkForData($yearmonth, $period)
    {
                $database = App::get('db');
        if (!isset($this->totals)) {
                $sql = "SELECT COUNT(datetime) AS total, DATE_FORMAT(datetime,'%Y-%m') as date"
                        . " FROM `#__stats_topvals` WHERE `period`="
                        . $database->quote($period) . " GROUP BY datetime";
    ...
    private function navLinks($period = '12', $top = '')
    {
        ...
        $html .= '><a href="' . Route::url('index.php?option=' . $this->_option . '&task='
                . $this->_task . '&period=12&top=' . $top) . '"><span>'
                . Lang::txt('PLG_USAGE_PERIOD_PRIOR12') . '</span></a></li>' . "\n";
        $html .= "\t\t" . '<li';
        if ($period == 'month' || $period == '1') {
            $html .= ' class="active"';
        }
        $html .= '><a href="' . Route::url('index.php?option=' . $this->_option . '&task='
                . $this->_task . '&period=1&top=' . $top) . '"><span>'
                . Lang::txt('PLG_USAGE_PERIOD_MONTH') . '</span></a></li>' . "\n";
        $html .= "\t\t" . '<li';
        if ($period == 'qtr' || $period == '3') {
            $html .= ' class="active"';
        }
        $html .= '><a href="' . Route::url('index.php?option=' . $this->_option . '&task='
                . $this->_task . '&period=3&top=' . $top) . '"><span>'
                . Lang::txt('PLG_USAGE_PERIOD_QTR') . '</span></a></li>' . "\n";
        $html .= "\t\t" . '<li';
        if ($period == 'year' || $period == '0') {
            $html .= ' class="active"';
        }
        $html .= '><a href="' . Route::url('index.php?option=' . $this->_option . '&task='
                . $this->_task . '&period=0&top=' . $top) . '"><span>'
                . Lang::txt('PLG_USAGE_PERIOD_YEAR') . '</span></a></li>' . "\n";
        $html .= "\t\t" . '<li';
        if ($period == 'fiscal' || $period == '13') {
            $html .= ' class="active"';
        }
        $html .= '><a href="' . Route::url('index.php?option=' . $this->_option . '&task='
                . $this->_task . '&period=13&top=' . $top) . '"><span>'
                . Lang::txt('PLG_USAGE_PERIOD_FISCAL') . '</span></a></li>' . "\n";
        $html .= "\t\t" . '<li';
        if ($period == '14') {
            $html .= ' class="active"';
        }
        $html .= '><a href="' . Route::url('index.php?option=' . $this->_option . '&task='
                . $this->_task . '&period=14&top=' . $top) . '"><span>'
                . Lang::txt('PLG_USAGE_PERIOD_OVERALL') . '</span></a></li>' . "\n";
        $html .= "\t" . '</ul>' . "\n";
        $html .= '</div>' . "\n";
    ...
    public function getDateWithData($period)
    {
        ...
        if (!$dthis) {
            ...
            for ($i = $cur_year; $i >= $year_data_start; $i--) {
                foreach ($monthsReverse as $key => $month) {
                    $value = $i . '-' . $key;
                    if ($this->checkForData($value, $period) && (intval($key) < $cur_month || $i < $cur_year)) {
                        $dthis = $value;
                        break;
                    }
                }
    ...
    public function onUsageDisplay($option, $task, $db, $months, $monthsReverse, $enddate)
    {
        ...
        $html  = '<form method="post" action="' . Route::url('index.php?option=' . $this->_option
                . '&task=' . $this->_task . '&period=' . $period) . '">' . "\n";
        ...
        if ($results) {
            foreach ($results as $row) {
                ...
                if ($s_top == $top) {
                    $html .= "\t\t\t\t" . '<option value="' . $data[$top]['id']
                            . '" selected="selected">' . htmlentities($data[$top]['name'])
                            . '</option>' . "\n";
                    ...
                    $html .= "\t\t\t\t" . '<option value="' . $data[$top]['id'] . '">'
                            . htmlentities($data[$top]['name']) . '</option>' . "\n";
                }
            }
        }
        ...
        $html .= "\t\t" . '</label> ' . "\n";
        if ($s_top < 9) {
            $html .= $this->dropDownDates($database, $period, $s_top, $dthis) . ' ';
        }
        ...
        if ($s_top) {
            ...
            if ($s_top == '9') {
                $html .= $this->getTopCitedTools($database);
            } elseif ($s_top == '1') {
                $html .= $this->getTopRankTools($database);
            } else {
