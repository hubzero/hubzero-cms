...
// phpcs:ignore PSR2.Classes.PropertyDeclaration.Underscore
    ...
    private function regionList(&$db, $region, $t = 0, $enddate = 0)
    {
        ...
        if ($regionname) {
            ...
            $twelvetext  = date("M", mktime(0, 0, 0, $dtmonthnext, 1, $dtyear)) . ' ' . $dtyearprior
                    . ' - ' . date("M", mktime(0, 0, 0, $dtmonth, 1, $dtyear)) . ' ' . $dtyear;
            $period = array(
            ...
            for ($pidx = 0; $pidx < count($period); $pidx++) {
                ...
                if ($results) {
                    foreach ($results as $row) {
                        ...
                        array_push($regionlistset, array(
                            $row->name, $row->value, $formattedval, sprintf("%0.1f%%", 100)
                        ));
                    }
                }
                    ...
                    foreach ($results as $row) {
                        if ($row->rank > 0 && (!$size || $row->rank <= $size)) {
                            ...
                            if ($regionlistset[0][1] > 0) {
                                array_push($regionlistset, array(
                                    $row->name, $row->value, $formattedval,
                                    sprintf("%0.1f%%", (100 * $row->value / $regionlistset[0][1]))
                                ));
                            } else {
