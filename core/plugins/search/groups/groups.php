...
class Groups extends Plugin
{
    ...
    public static function onSearch($request, &$results, $authz)
    {
        ...
        foreach ($terms['forbidden'] as $forb) {
            $addtl_where[] = "(g.cn NOT LIKE '%$forb%' AND g.description NOT LIKE '%$forb%' "
                . "AND g.public_desc NOT LIKE '%$forb%')";
        }
        ...
        $results->add(new \Components\Search\Models\Basic\Result\Sql(
			...
			WHERE
				(g.type = 1 OR g.type = 3) AND g.published=1 AND g.approved=1
				AND g.discoverability = 0 AND $weight > 0" .
