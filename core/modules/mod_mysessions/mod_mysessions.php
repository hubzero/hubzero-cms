 ...
 */
class Mysessions extends Module
{
    protected $database;
    protected $error;
    protected $sessions;
     ...
     * @return  void
     */
    private function setTimeout($sess)
    {
     ...
     * @return  string
     */
    private function getTimeout($sess)
    {
        $mwdb = \Components\Tools\Helpers\Utils::getMWDBO();
    ...
    public function display()
    {
        ...
        $this->error = false;
        $mwOn = $this->toolsConfig->get('mw_on');
        if (!$mwdb || !$mwdb->connected() || !$mwOn || ($mwOn > 1 && !$authorized)) {
            $this->error = true;
            return false;
        }
        ...
        if (
            $this->params->get('show_screenshots', 1)
            && strstr(User::get('email'), '@') != '@invalid'
        ) {
            $cmd = "/bin/sh " . Component::path('com_tools') . "/scripts/mw screenshot " .
                User::get('username') . " 2>&1 </dev/null";
