{{--
  Recent Questions module — daisyUI layout.
...
<div{!! $cssId ? ' id="' . $cssId . '"' : '' !!}{!! $cssClass ? ' class="' . $cssClass . '"' : '' !!}>
          ...
          @else
            <div class="min-w-0 grow">
              ...
              <div class="flex flex-wrap items-center gap-x-2 text-xs text-base-content/60 mt-0.5">
                <span>{{ Lang::txt('MOD_RECENTQUESTIONS_ASKED_BY', e($name)) }}</span>
  ...
  @else
    <p class="text-base-content/60">{{ Lang::txt('MOD_RECENTQUESTIONS_NO_RESULTS') }}</p>
  @endif
</div>
