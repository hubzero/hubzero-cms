...
if ($this->error) {
    ...
    <div<?php echo ($this->moduleclass) ? ' class="' . $this->moduleclass . '"' : ''; ?>>
        <ul class="module-nav">
            <li>
                <?php
                $pointsUrl = Route::url('index.php?option=com_members&id='
                    . User::get('id') . '&active=points');
                ?>
                <a class="icon-points" href="<?php echo $pointsUrl; ?>">
                    <?php echo Lang::txt('MOD_MYPOINTS_ALL_TRANSACTIONS'); ?>
                </a>
            </li>
        </ul>
        <p class="point-balance">
            <span><?php echo Lang::txt('MOD_MYPOINTS_YOU_HAVE'); ?> </span>
            <?php echo $this->escape($this->summary); ?><small>
            <?php echo strtolower(Lang::txt('MOD_MYPOINTS_POINTS')); ?></small>
        </p>
        <?php if (count($this->history) > 0) { ?>
            <table class="transactions">
                <caption><?php
                    echo Lang::txt('MOD_MYPOINTS_TRANSACTIONS_TBL_CAPTION', $this->escape($this->limit));
                ?></caption>
                <thead>
                    <tr>
                        ...
                        <th scope="col" class="numerical-data"><?php
                            echo Lang::txt('MOD_MYPOINTS_TRANSACTIONS_TBL_TH_AMOUNT');
                        ?></th>
                        <th scope="col" class="numerical-data"><?php
                            echo Lang::txt('MOD_MYPOINTS_TRANSACTIONS_TBL_TH_BALANCE');
                        ?></th>
                    </tr>
                </thead>
                <tbody>
                ...
                foreach ($this->history as $item) {
                    ...
                    <tr class="<?php echo $cls; ?>">
                        <td>
                            <time datetime="<?php echo $this->escape($item->created); ?>"><?php
                                echo Date::of($item->created)->toLocal(Lang::txt('DATE_FORMAT_HZ1'));
                            ?></time>
                        </td>
                        <td>
                            <?php echo $item->type; ?>
