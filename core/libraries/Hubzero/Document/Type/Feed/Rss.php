...
class Rss extends Renderer
{
    ...
    public function render($name = null, $params = null, $content = null)
    {
        ...
        $url = rtrim(\Hubzero\Facades\App::get('request')->root(), '/') . '/';
        ...
        if ($config->get('sitename_pagetitles', 0) == 1) {
            $data->title = $lang->txt('JPAGETITLE', $sitename, $data->title);
        } elseif ($config->get('sitename_pagetitles', 0) == 2) {
            $data->title = $lang->txt('JPAGETITLE', $data->title, $sitename);
        }
        ...
        for ($i = 0; $i < count($data->items); $i++) {
            if (
                (strpos(
                    $data->items[$i]->link,
                    'http://'
                ) === false) and (strpos($data->items[$i]->link, 'https://') === false)
            ) {
            ...
            $feed .= "			<description>" . $this->relToAbs($data->items[$i]->description) . "</description>\n";
            ...
            if ($data->items[$i]->itunes_summary != '') {
                $feed .= "			<itunes:summary>" .
                    $this->escape($data->items[$i]->itunes_summary) .
                    "</itunes:summary>\n";
            }
            if ($data->items[$i]->itunes_duration != '') {
                $feed .= "			<itunes:duration>" .
                    $this->escape($data->items[$i]->itunes_duration) .
                    "</itunes:duration>\n";
            }
            ...
            if ($data->items[$i]->itunes_keywords != '') {
                $feed .= "			<itunes:keywords>" .
                    $this->escape($data->items[$i]->itunes_keywords) .
                    "</itunes:keywords>\n";
            }
            if ($data->items[$i]->itunes_author != '') {
                $feed .= "			<itunes:author>" .
                    $this->escape($data->items[$i]->itunes_author) .
                    "</itunes:author>\n";
            }
            if ($data->items[$i]->itunes_category != '') {
                $feed .= "			<itunes:category text=\"" .
                    $this->escape($data->items[$i]->itunes_category) .
                    "\">\n";
            ...
            if ($data->items[$i]->itunes_image != null) {
                ...
                if ($data->items[$i]->itunes_image->description != '') {
                    $feed .= "			<description><![CDATA[" .
                        $data->items[$i]->itunes_image->description .
                        "]]></description>\n";
                }
                $feed .= "			</itunes:image>\n";
            }
    ...
    {
        $base = \Hubzero\Facades\App::get('request')->base();
