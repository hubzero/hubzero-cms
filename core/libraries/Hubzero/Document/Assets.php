...
class Assets
{
    ...
    {
        if (\Hubzero\Facades\App::has($key)) {
            return \Hubzero\Facades\App::get($key);
        }
        return null;
    }
    ...
    public static function addStylesheet($stylesheet)
    {
        ...
        if ($document = self::app('document')) {
            $document->addStyleSheet(rtrim(Request::base(true), '/') .
                $stylesheet .
                '?v=' .
                filemtime($root .
                $stylesheet));
    ...
    public static function addScript($script, $defer = false, $async = false)
    {
        ...
        if ($document = self::app('document')) {
            $document->addScript(rtrim(Request::base(true), '/') .
                $script .
                '?v=' .
                filemtime($root .
                $script), "text/javascript", $defer, $async);
        }
    }
    ...
    public static function getSystemStylesheet($elements = null)
    {
        ...
        try {
            ...
            if ($env == 'production' && file_exists($output)) {
                $output = rtrim(Request::root(true), '/') .
                    '/app/cache/' .
                    $client .
                    '/' .
                    $primary .
                ...
                */
                ...
                if (!is_array($cache) || $newCache['updated'] > $cache['updated']) {
                    ...
                    $newCache['compiled'] = str_replace(array("'/media/system/", "'/core/assets/"), "'" .
                        rtrim(Request::root(true), '/') .
                        '/core/assets/', $newCache['compiled']);
            ...
            if (!file_exists($cachedir . DS . $cachefile)) {
                ...
                $patterns = array(
                    ...
                    /* '/ ?([,:;{}]) ?/' remove space before and after , : ; { }
                       [!] apparently, IE 7 doesn't like this and won't process the stylesheet */
                );
                $replacements = array(
                    '',
                    ' ',
                ...
                $contents = str_replace(array("url('/media/system/", "url('/core/assets/"), "url('" .
                    rtrim(Request::root(true), '/') .
