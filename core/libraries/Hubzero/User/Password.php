...
class Password
{
    ...
    public function update()
    {
        ...
        if ($affected > 0) {
            \Hubzero\Facades\Event::trigger('user.onAfterStorePassword', array($this));
        }
    ...
    public function delete()
    {
        ...
        if ($affected > 0) {
            \Hubzero\Facades\Event::trigger('user.onAfterDeletePassword', array($this));
        }
    ...
    public static function changePasshash($user, $passhash)
    {
        ...
        $db->setQuery("UPDATE `#__xprofiles` SET userPassword=" .
            $db->quote($passhash) .
            " WHERE uidNumber=" .
            $db->quote($hzup->get('user_id')));
        $db->query();

        $db->setQuery("UPDATE `#__users` SET password=" .
            $db->quote($passhash) .
            " WHERE id=" .
            $db->quote($hzup->get('user_id')));
        $db->query();
     ...
     * @param   string   $salt          The salt to use to encrypt the password. If not present, a
     *                                  new salt will be generated.
     * @param   string   $encryption    The kind of password encryption to use. Defaults to md5-hex.
     * @param   boolean  $show_encrypt  Some password systems prepend the kind of encryption to the
     ...
     *
     * @param   string  $encryption  The kind of password encryption to use. Defaults to md5-hex.
     * @param   string  $seed        The seed to get the salt from (probably a previously generated
     *                               password). Defaults to generating a new seed.
    ...
    public static function passwordMatches($user, $password, $alltables = false)
    {
        ...
        if (is_object($hzup) && !empty($hzup->passhash)) {
            ...
            if (is_object($profile) && ($profile->get('userPassword') != '')) {
                ...
                $user = \Hubzero\Facades\User::getInstance($user);
