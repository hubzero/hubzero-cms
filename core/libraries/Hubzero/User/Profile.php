...
class Profile extends Obj
{
    ...
    private function sql_load($user)
    {
        ...
        $this->_registry = new Registry($result['params']);
        ...
        $this->_registry->merge($this->_registry);
     ...
     */
    // phpcs:ignore PSR1.Methods.CamelCapsMethodName.NotCamelCaps
    private function sql_author_load($authorid)
    {
     ...
     */
    // phpcs:ignore PSR1.Methods.CamelCapsMethodName.NotCamelCaps
    private function xregistration_load($registration)
    {
        static $_propertyregmap = array(
            'username' => 'login',
            'name' => 'name',
            'email' => 'email',
            'orcid' => 'orcid',
        ...
        $this->set(
            ...
            $this->get('mailPreferenceOption') ? $this->get('mailPreferenceOption') : '-1'
        );
    ...
    public function loadRegistration(&$registration)
    {
        ...
        $keys = array('email', 'name', 'orgtype', 'countryresident', 'countryorigin', 'disability', 'hispanic', 'race',
            'phone', 'reason', 'edulevel', 'role', 'surname', 'givenName', 'middleName', 'orcid');
        foreach ($keys as $key) {
        ...
        if ($registration->get('mailPreferenceOption') !== null) {
            $this->set(
                'mailPreferenceOption',
                $registration->get('mailPreferenceOption') ? $registration->get('mailPreferenceOption') : '-1'
            );
        }
    ...
    public function load($user, $storage = 'mysql')
    {
        ...
        if ($storage == 'mysql') {
            return $this->sql_load($user);
        }

        if ($storage == 'author') {
            return $this->sql_author_load($user);
        }

        if ($storage == 'xregistration') {
            return $this->xregistration_load($user);
        }
    ...
    public function update()
    {
        ...
        foreach ($classvars as $property => $value) {
            ...
            if ($property == 'params') {
                if (is_object($this->params)) {
                    $query .= "params='" . str_replace("", "", $this->params->toString()) . "'";
                } else {
                    $query .= "params=''";
                }
                continue;
        ...
        foreach ($classvars as $property => $value) {
            ...
            if (property_exists(__CLASS__, '_auxv_' . $property)) {
                ...
                $query = "DELETE FROM #__xprofiles_" .
                    $property .
                    " WHERE uidNumber=" .
                    $this->get('uidNumber') .
    ...
    public function get($property, $default = null)
    {
        ...
        if (!property_exists(__CLASS__, $property)) {
            ...
            } elseif (property_exists(__CLASS__, '_auxv_' . $property)) {
        ...
        if ($this->$property === false) {
            ...
            $query = "SELECT $property_name FROM `#__xprofiles` AS x, 
                `#__xprofiles_$property_name` AS xp WHERE x.uidNumber=xp.uidNumber AND xp.uidNumber=" .
                $db->quote($this->get('uidNumber')) .
            ...
            } elseif ('_auxs_' == substr($property, 0, 6)) {
                if (isset($result[0])) {
    ...
    public function &getParameters($loadsetupfile = false, $path = null)
    {
        ...
        if ($loadsetupfile)
        {
            ...
            $this->params->loadSetupFile($file);
        }
        */
