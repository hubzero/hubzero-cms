...
class Helper
{
    ...
    public function convertInviteEmails($email, $user_id)
    {
        ...
        $sql = 'DELETE FROM `#__xgroups_inviteemails` WHERE email=' .
            $db->quote($email) .
            ' AND gidNumber IN (' .
            $gids .
            ');';
    ...
    public static function search_roles($group, $role = '')
    {
        ...
        $query = "SELECT uidNumber FROM `#__xgroups_roles` as r, `#__xgroups_member_roles` as m WHERE r.id='" .
            $role .
            "' AND r.id=m.roleid AND r.gidNumber='" .
            $group->gidNumber .
            "'";
    ...
    public static function getPluginAccess($group, $get_plugin = '')
    {
        ...
        if ($active_group_plugins) {
            ...
            foreach ($hub_group_plugins as $hgp) {
                // if group defined access level is not an acceptable value or not set use default value that is set
                // perplugin
                //else use group defined access level
                if (
                    !isset($active_group_plugins[$hgp['name']]) ||
                    !in_array($active_group_plugins[$hgp['name']], $acceptable_levels)
                ) {
