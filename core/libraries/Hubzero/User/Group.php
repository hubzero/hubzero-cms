...
class Group extends Obj
{
    ...
    public static $_list_keys = array('members', 'managers', 'applicants', 'invitees');
    ...
    public function update()
    {
        ...
        foreach (self::$_list_keys as $property) {
            ...
            if (in_array($property, array('members', 'managers'))) {
                ...
                $aNewUserGroupEnrollments = array_merge($aNewUserGroupEnrollments, array_diff(
                    $list,
                    $aExistingUserMembership
                ));
            }
            ...
            if (!array_key_exists($property, get_object_vars($this))) {
                ...
                if (is_object($db)) {
                    $groups = array('applicants' => array(),
                        'invitees' => array(), 'members' => array(), 'managers' => array());
                    foreach ($groups as $key => $data) {
                        $this->__set($key, $data);
                    }

                    $query = "(select uidNumber, 'invitees' AS role from #__xgroups_invitees where gidNumber=" .
                        $db->quote($this->gidNumber) .
                        ")
						UNION
							(select uidNumber, 'applicants' AS role from #__xgroups_applicants where gidNumber=" .
                                $db->quote($this->gidNumber) .
                                ")
						UNION
							(select uidNumber, 'members' AS role from #__xgroups_members where gidNumber=" .
                                $db->quote($this->gidNumber) .
                                ")
						UNION
							(select uidNumber, 'managers' AS role from #__xgroups_managers where gidNumber=" .
                                $db->quote($this->gidNumber) .
                                ")";
    ...
    public function getEmails($tbl = 'managers')
    {
        ...
        $query = 'SELECT u.email FROM #__users AS u, #__xgroups_' .
            $tbl .
            ' AS gm WHERE gm.gidNumber=' .
            $db->quote($this->gidNumber) .
            ' AND u.id=gm.uidNumber;';
