...
class Paginator extends Obj
{
    ...
    protected function _buildDataObject()
    {
        ...
        $data->all = new Item(\Hubzero\Facades\Lang::txt('JLIB_HTML_VIEW_ALL'), $this->prefix);
        if (!$this->_viewall) {
            $data->all->base = '0';
            $data->all->link = \Hubzero\Facades\Route::url($params . '&' . $this->prefix . 'limitstart=');
        }

        // Set the start and previous data objects.
        $data->start    = new Item(\Hubzero\Facades\Lang::txt('JLIB_HTML_START'), $this->prefix);
        $data->previous = new Item(\Hubzero\Facades\Lang::txt('JPREV'), $this->prefix);
        ...
        if ($this->get('pages.current') > 1) {
            ...
            $data->start->base = '0';
            $data->start->link = \Hubzero\Facades\Route::url($params . '&' . $this->prefix . 'limitstart=0');

            $data->previous->base = $page;
            $data->previous->link = \Hubzero\Facades\Route::url($params . '&' . $this->prefix . 'limitstart=' . $page);
        }

        // Set the next and end data objects.
        $data->next = new Item(\Hubzero\Facades\Lang::txt('JNEXT'), $this->prefix);
        $data->end  = new Item(\Hubzero\Facades\Lang::txt('JLIB_HTML_END'), $this->prefix);

        if ($this->get('pages.current') < $this->get('pages.total')) {
            $next = $this->get('pages.current') * $this->limit;
            $data->next->base = $next;
            $data->next->link = \Hubzero\Facades\Route::url($params . '&' . $this->prefix . 'limitstart=' . $next);

            $end  = ($this->get('pages.total') - 1) * $this->limit;
            $data->end->base  = $end;
            $data->end->link  = \Hubzero\Facades\Route::url($params . '&' . $this->prefix . 'limitstart=' . $end);
        }
        ...
        for ($i = $this->get('pages.start'); $i <= $this->get('pages.stop'); $i++) {
            ...
            if ($i != $this->get('pages.current')) { // || $this->_viewall)
                ...
                $data->pages[$i]->link = \Hubzero\Facades\Route::url($params . '&' . $this->prefix . 'limitstart=' . $offset);
            }
        }
