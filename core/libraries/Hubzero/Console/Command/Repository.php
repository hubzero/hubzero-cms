...
class Repository extends Base implements CommandInterface
{
    ...
    public function execute()
    {
        ...
        } elseif ($this->arguments->getOpt('version')) {
    ...
    public function rollback()
    {
        ...
        if ($this->output->isInteractive()) {
            $proceed = $this->output->getResponse('Are you sure you want to rollback to the snapshot taken on ' .
                $date .
                '? [y|n]');
    ...
    public function clean()
    {
        if ($this->output->isInteractive()) {
            ...
            $proceed   = $this->
                output->
                getResponse('Do you want to purge all rollback points except the latest? [y|n]');
            if ($proceed == 'y' || $proceed == 'yes') {
    ...
    public function syntax()
    {
        // Get files
        $status = $this->mechanism->status();
        $files  = (isset($status['added']) ||
            isset($status['modified'])) ? array_merge($status['added'], $status['modified']) : array();
            ...
            foreach ($files as $file) {
                ...
                if (!$this->arguments->getOpt('no-sniffing')) {
                    ...
                    $cmd    = "php " .
                        ...
                        'bin' .
                        DS .
                        "phpcs --standard={$standards}{$branch}/ruleset.xml -n {$base}{$file}";
    ...
    public function updateGitURLconf()
    {
        ...
        if (preg_match("/fatal: Authentication failed.../uis", $lsremote_command_response)) {
            $output = array(Lang::txt("Authentication failure. Please check the specified Access Token has access to 
                this extension's repository."));
    ...
    public function checkoutRepoBranch()
    {
        ...
        if ($this->arguments->getOpt('git_branch')) {
            ...
            $default_remote_branch_cmd = "git remote show " .
                trim($default_remote) .
