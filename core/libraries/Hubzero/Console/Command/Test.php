...
class Test extends Base implements CommandInterface
{
    ...
    public function run()
    {
        ...
        switch ($parts[0]) {
            case 'tpl':
                unset($parts[0]);
                $path = ($path == 'app' ? PATH_APP : PATH_CORE) .
                    DS .
                    'templates' .
                    DS .
                    implode('_', $parts) .
            ...
            default:
                $this->output->error(
                    'Sorry, we were not able to find an extension by that name or ' .
                    'that extension type is not currently supported'
                );
                break;
        }
        ...
        $cmd = 'php ' .
            PATH_CORE .
            '/vendor/bin/phpunit --display-deprecations --bootstrap ' .
            PATH_CORE .
