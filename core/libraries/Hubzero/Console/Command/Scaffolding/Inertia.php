...
/**
 * Scaffolding class for Inertia component/view boilerplate.
 *
 * @museIgnoreHelp
 */
class Inertia extends Scaffolding
{
    /**
     * Build Inertia scaffold files.
     *
     * @return  void
     */
    public function construct()
    {
        ...
        $view = strtolower(trim((string) $this->arguments->getOpt('v', $this->arguments->getOpt('view', 'dashboard'))));
        ...
        $jsEntryPath = $componentPath . DS . 'site' . DS . 'assets' . DS . 'js' . DS . 'inertia-' . $view . '.js';
        $testPath = $componentPath . DS . 'tests' . DS . 'Inertia' . ucfirst($view) . 'Test.php';

        $this->ensureDirectory(dirname($controllerPath));
        $this->ensureDirectory(dirname($defaultViewPath));
        $this->ensureDirectory(dirname($jsEntryPath));
        if ($withTest) {
            $this->ensureDirectory(dirname($testPath));
        }

        $targets = array($controllerPath, $defaultViewPath, $jsEntryPath);
        if ($withTest) {
            $targets[] = $testPath;
        }
        ...
        $this->addTemplateFile('inertia.controller.tmpl', $controllerPath)
            ->addTemplateFile('inertia.default.tmpl', $defaultViewPath)
            ->addTemplateFile('inertia.app.tmpl', $jsEntryPath);

        if ($withTest) {
            $this->addTemplateFile('inertia.test.tmpl', $testPath);
        }

        $this->make();

        $this->output->addLine('Created Inertia scaffold for com_' . $component, 'success');
        $this->output->addLine('Controller: ' . $controllerPath);
        $this->output->addLine('View (root): ' . $defaultViewPath);
        $this->output->addLine('JS entry: ' . $jsEntryPath);
        if ($withTest) {
    ...
    {
        $this->output
            ->addOverview('Create Inertia boilerplate inside an existing component.')
            ->addArgument(
