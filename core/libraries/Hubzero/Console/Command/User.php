...
class User extends Base implements CommandInterface
{
    ...
    public function merge()
    {
        ...
        if ($directionalIndicator) {
            ...
            if ((!$sourceUser || !$destinationUser) && !$this->output->isInteractive()) {
                $this->output->error('Please provide a source and 
                    destination user in the format: muse user merge [sourceUserId] into [destUserId]');
            }
        } else {
            if ($this->output->isInteractive()) {
                $destinationUser = (int)$this->
                    output->
                    getResponse('What is the destination user ID (this is the user that will remain after the merge)?');
                $sourceUser      = (int)$this->
                    output->
                    getResponse('What is the source user ID (this is the user that will be deleted after the merge)?');
            } else {
                $this->output->error('Please provide a source and 
                    destination user in the format: muse user merge [sourceUserId] into [destUserId]');
            }
            ...
            foreach ($columns as $column => $type) {
                if (in_array($table . '.' . $column, $unames) || in_array($column, $fields)) {
                    ...
                    $query  = "SELECT * FROM `{$table}` WHERE `{$column}` = 
                        '{$sourceUser}' OR `{$column}` LIKE '%{$sUserName}%'";
                    $query .= ((isset($constraints[$table .
                        '.' .
                        $column])) ? ' ' .
                        $constraints[$table .
                        '.' .
                    ...
                    if ($results && count($results) > 0) {
                        ...
                        foreach ($results as $row) {
                            if (!$tablePK) {
                                $this->output->addLine(
                                    "Ignoring {$table}.{$column} due to lack of primary key",
                                    'warning'
                                );
                                continue 2;
                            }
                                ...
                                try {
                                    ...
                                    if (is_numeric($row->$column)) {
                                        $query = "UPDATE `{$table}` SET `{$column}` = 
                                            '{$destinationUser}' WHERE `{$tablePK}` = '{$row->$tablePK}'";
                                    } else {
                                        $numericUpdate = false;
                                        $query = "UPDATE `{$table}` SET `{$column}` = REPLACE({$column}, 
                                            '{$sUserName}', '{$dUserName}') WHERE `{$tablePK}` = '{$row->$tablePK}'";
                                    }
                                    ...
                                    if ($e->getPrevious()->getCode() == '23000') {
                                        $this->output->addLine(
                                            "Ignoring {$table}.{$column} due to integrity constraint violation",
                                            'warning'
                                        );
                                        continue 2;
                                    } else {
    ...
    public function unmerge()
    {
        ...
        if ($directionalIndicator) {
            ...
            if ((!$sourceUser || !$destinationUser) && !$this->output->isInteractive()) {
                $this->output->error('Please provide a source and 
                    destination user in the format: muse user unmerge [destUserId] from [sourceUserId]');
            }
        } else {
            if ($this->output->isInteractive()) {
                $destinationUser = (int)$this->output->getResponse(
                    'What is the destination user ID ' .
                    '(this is the user that was deleted during the initial merge)?'
                );
                $sourceUser = (int)$this->output->getResponse(
                    'What is the source user ID ' .
                    '(this is the user that was the recipient of the initially merged data)?'
                );
            } else {
                $this->output->error(
                    'Please provide a source and destination user in the format: ' .
                    'muse user unmerge [destUserId] from [sourceUserId]'
                );
            }
        }
