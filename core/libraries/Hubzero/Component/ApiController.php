...
class ApiController implements ControllerInterface
{
    ...
    public function __construct(Response $response, $config = array())
    {
        ...
        foreach ($r->getMethods(ReflectionMethod::IS_PUBLIC) as $method) {
            ...
            if (
                !in_array($name, array('registerTask', 'unregisterTask'))
    ...
    public function execute()
    {
        ...
        } elseif (isset($this->_taskMap[$this->_task])) {
    ...
    public function indexTask()
    {
        ...
        foreach ($classReflector->getMethods() as $method) {
            ...
            // skip constructor
            if (
                substr($method->getName(), -4) != 'Task' ||
                in_array($method->getName(), array('registerTask', 'unregisterTask'))
            ) {
                continue;
            }
            ...
            if ($this->isDynamic && $endpoint['name'] != 'index') {
                ...
                foreach ($properties as $prop => $type) {
                    ...
                    if (
                        $prop == $model->getPrimaryKey() &&
                        in_array($endpoint['name'], array('read', 'update', 'delete'))
                    ) {
        ...
        if (!$this->_model) {
            ...
            if ($this->_model && !strstr($this->_model, '\\')) {
                $this->_model = 'Components\\' .
                    ucfirst($this->_name) .
                    '\\Models\\' .
                    ucfirst(strtolower($this->_model));
            }
        }
