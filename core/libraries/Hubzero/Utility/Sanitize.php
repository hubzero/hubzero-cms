...
class Sanitize
{
    ...
    public static function clean($string)
    {
        ...
        $string = preg_replace(
            '#([a-z]*)[\x00-\x20]*=[\x00-\x20]*([\`\'\"]*)[\\x00-\x20]*j[\x00-\x20]*a[\x00-\x20]*' .
            'v[\x00-\x20]*a[\x00-\x20]*s[\x00-\x20]*c[\x00-\x20]*r[\x00-\x20]*i[\x00-\x20]*p[\x00-\x20]*' .
            't[\x00-\x20]*:#iUu',
            '$1=$2nojavascript...',
            $string
        );
        $string = preg_replace(
            ...
            'c[\x00-\x20]*r[\x00-\x20]*i[\x00-\x20]*p[\x00-\x20]*t[\x00-\x20]*:#iUu',
            '$1=$2novbscript...',
            $string
        );
        ...
        $string = preg_replace(
            '#(<[^>]+)style[\x00-\x20]*=[\x00-\x20]*([\`\'\"]*).*s[\x00-\x20]*c[\x00-\x20]*' .
            'r[\x00-\x20]*i[\x00-\x20]*p[\x00-\x20]*t[\x00-\x20]*:*[^>]*>#iUu',
            "$1>",
            $string
        );
    ...
    protected static function _buildHtmlPurifierConfig($options)
    {
        ...
        $root = str_replace(['http://', 'https://', '.'], ['', '', '\.'], \Hubzero\Facades\App::get('request')->root());
        $defaultSettings = [
            ...
            'URI.SafeIframeRegexp' =>
                "%^(https?:)?//(www\.youtube(?:-nocookie)?\.com/embed/|player\.vimeo\.com/video/|$root)%",
        ];
    ...
    {
        $client = \Hubzero\Facades\App::get('client');
        ...
        if (!is_dir($clientSerializerPath)) {
            \Hubzero\Facades\App::get('filesystem')->makeDirectory($clientSerializerPath);
        }
     ...
     * @param   string  $type    Return type for the variable (INT, UINT, FLOAT, BOOLEAN, WORD,
