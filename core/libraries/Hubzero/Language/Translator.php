...
class Translator extends Obj
{
     ...
     */
    public static function exists($lang, $basePath = PATH_APP, $client = null)
    {
        static $paths = array();
        ...
        $path = self::getLanguagePath($basePath, $lang, $client);
    ...
    public function load($extension = 'hubzero', $basePath = PATH_APP, $lang = null, $reload = false, $default = true)
    {
        ...
        $path = self::getLanguagePath($basePath, $lang, $this->client);
        ...
        if (isset($this->paths[$extension][$filename]) && !$reload) {
            ...
            if ($result === false && $default) {
                ...
                $path = self::getLanguagePath($basePath, $this->default, $this->client);
     ...
     */
    public static function getKnownLanguages($basePath = PATH_APP, $client = null)
    {
        $dir = self::getLanguagePath($basePath, null, $client);
     ...
     */
    public static function getLanguagePath($basePath = PATH_APP, $language = null, $client = null)
    {
        // The app and core bases hold one pack directory per client;
        // any other base is used as given
        if ($basePath == PATH_APP) {
            $dir = PATH_APP . DS . 'language' . DS . strtolower(self::clientName($client));
        } elseif ($basePath == PATH_CORE) {
            $dir = PATH_CORE . DS . 'bootstrap' . DS . ucfirst(strtolower(self::clientName($client))) . DS . 'language';
        } else {
    ...
    /**
     * Name of the client whose language files are wanted
     *
     * @param   string|null  $client  An explicit client name, or null for the current one
     * @return  string
     */
    protected static function clientName($client = null)
    {
        if ($client) {
            return $client;
        }
