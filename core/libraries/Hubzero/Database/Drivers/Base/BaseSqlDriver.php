...
use Hubzero\Database\Drivers\Base\BasePdoDriver as PdoDriver;
 ...
 *         └── Drivers\Base\BaseSqlDriver (this class)
 ...
 * - Connection handling (that's in Driver.php/BasePdoDriver.php)
 * - Database-specific syntax (that's in concrete Drivers/* classes)
 *
 */
abstract class BaseSqlDriver extends PdoDriver
{
    ...
    /**
     * Resolve and instantiate schema grammar from centralized registry metadata.
     *
     * @return \Hubzero\Database\Drivers\Base\BaseSchemaGrammar
     */
    protected function makeSchemaGrammarFromRegistry(): \Hubzero\Database\Drivers\Base\BaseSchemaGrammar
    {
        $driverName = (string) ($this->name ?? '');
        $class = \Hubzero\Database\BackendRegistry::resolveGrammarClassFor($driverName);

        if (!is_string($class) || $class === '') {
            throw new \RuntimeException(
                'Unsupported schema grammar for driver "' . $driverName . '".'
            );
        }

        if (!class_exists($class)) {
            throw new \RuntimeException(
                'Schema grammar class does not exist: ' . $class
            );
        }

        if (!is_subclass_of($class, '\\Hubzero\\Database\\Drivers\\Base\\BaseSchemaGrammar')) {
            throw new \RuntimeException(
                'Invalid schema grammar class "' . $class
