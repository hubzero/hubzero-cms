...
class Asset
{
    ...
    protected static function includeRelativeFiles($folder, $file, $relative, $detect_browser, $detect_debug)
    {
        ...
        if (strpos($file, 'http') === 0) {
            ...
            if ($detect_browser) {
                ...
                // Try to include files named filename.ext, filename_browser.ext, filename_browser_major.ext,
            ...
            // If not relative and http is not present in filename
                ...
                foreach ($potential as $strip) {
                    // Detect debug mode
                    if (
                        $detect_debug && App::get('config')->get('debug') && file_exists(PATH_ROOT .
                        "/$strip-uncompressed.$ext")
                    ) {
     ...
     * @return  mixed    nothing if $path_only is false, null, path or array of path if specific
     * css browser files were detected
     */
     ...
     * @param   boolean  $detect_debug    detect debug to search for compressed files if debug is on
     * @return  mixed  nothing if $path_only is false, null, path or array of path if specific js
