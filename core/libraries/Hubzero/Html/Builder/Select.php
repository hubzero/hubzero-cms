        ...
        foreach ($data as $dataKey => $group) {
            ...
            if ($noGroup) {
                ...
                $html .= '<optgroup' .
                    ...
                    '"') .
                    ' label="' .
                    ($options['group.label.toHtml'] ? self::escape($label) : $label) .
                    '">';
                $html .= self::options($subList, $options);
                $html .= '</optgroup>';
            }
    ...
    public static function options($arr, $optKey = 'value', $optText = 'text', $selected = null, $translate = false)
    {
        ...
        foreach ($arr as $elementKey => &$element) {
            ...
            if ($options['groups'] && $key == '<OPTGROUP>') {
                ...
                $html .= '<option value="' .
                    ($options['option.key.toHtml'] ? self::escape($key) : $key) .
                    '"' .
                    $extra .
                    '>';
                $html .= $options['option.text.toHtml'] ? self::escape(html_entity_decode(
                    $text,
                    ENT_COMPAT,
                    'UTF-8'
                )) : $text;
    ...
    public static function ordering($name, $query, $attribs = null, $selected = null, $neworder = null, $chop = null)
    {
        ...
        if (empty($neworder)) {
            ...
            if (empty($items)) {
                ...
                $options[] = self::option($items[$i - 1]->value + 1, ($items[$i - 1]->value + 1) .
                    ' ' .
                    Lang::txt('JOPTION_ORDER_LAST'));
            }
            $html = self::genericlist($options, $name, array('list.attr' => $attribs,
