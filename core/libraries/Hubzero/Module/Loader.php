...
class Loader
{
    ...
    public function render($module, $attribs = array())
    {
        ...
        if (file_exists($path)) {
            $this->app['language']->load($module->module, PATH_APP .
                ...
                'bootstrap' .
                DS .
                $this->app['client']->name, null, false, true) ||
     ...
     * @param   object  $cacheparams   Module cache parameters - id or url parameters, depending on
     *                                 the module cache mode
     * @return  string
     */
    public function cache($module, $moduleparams, $cacheparams)
    {
        ...
        if (
            $moduleparams->get(
                'owncache',
                null
            ) === '0' || $this->app['config']->get('caching') == 0 || $this->app['user']->getInstance()->get('id')
        ) {
        ...
        $wrkaroundoptions = array('nopathway' => 1,
            'nohead' => 0, 'nomodules' => 1, 'modulemode' => 1, 'mergehead' => 1);
        $wrkarounds = true;
