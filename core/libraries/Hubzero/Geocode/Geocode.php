...
class Geocode
{
    ...
    public static function getGeoDBO()
    {
        ...
        if (!is_object($instance)) {
            $geodb_params = \Hubzero\Facades\Component::params('com_system');
    ...
    public static function getCountriesByContinent($continent = '')
    {
        ...
        $gdb->setQuery("SELECT DISTINCT country FROM country_continent WHERE LOWER(continent) =" .
            $gdb->quote(strtolower($continent)));
    ...
    public static function getContinentByCountry($country = '')
    {
        ...
        $gdb->setQuery("SELECT DISTINCT continent FROM country_continent WHERE LOWER(country) ='" .
            strtolower($country) .
            "'");
        return $gdb->loadColumn();
    }
    ...
    public static function ipcountry($ip = '')
    {
        ...
        if ($ip) {
            ...
            $sql = "SELECT LOWER(countrySHORT) FROM ipcountry WHERE ipFROM <= " .
                 $gdb->quote($n_ip) .
                " AND ipTO >= " .
                $gdb->quote($n_ip);
            $gdb->setQuery($sql);
    ...
    public static function is_d1nation($country)
    {
        ...
        if ($country) {
            ...
            $gdb->setQuery("SELECT COUNT(*) FROM countrygroup WHERE LOWER(countrycode) = " .
                $gdb->quote(strtolower($country)) .
                " AND countrygroup = 'D1'");
            $c = $gdb->loadResult();
            if ($c > 0) {
                $d1nation = true;
    ...
    public static function is_iplocation($ip, $location)
    {
        ...
        if ($ip && $location) {
            ...
            $sql = "SELECT COUNT(*) FROM iplocation WHERE ipfrom <= " .
                 $gdb->quote($n_ip) .
                " AND ipto >= " .
                $gdb->quote($n_ip) .
                " AND LOWER(location) = " .
                 $gdb->quote(strtolower($location));
            $gdb->setQuery($sql);
            $c = $gdb->loadResult();
            if ($c > 0) {
                $iplocation = true;
