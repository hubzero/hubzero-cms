...
class TraceableDispatcher implements DispatcherInterface
{
    ...
    public function getNotCalledListeners()
    {
        try {
            ...
            if (null !== $this->logger) {
                $this->
                    logger->
                    info(sprintf('An exception was thrown while getting the uncalled listeners (%s)', $e->
                    getMessage()), array('exception' => $e));
            }
    ...
    private function preProcess(Event $event)
    {
        foreach ($this->dispatcher->getListeners($event) as $listener) {
            ...
            $this->dispatcher->getListeners()[$event->getName()]->add(new TraceableListener(
                $listener,
                $name,
                $this->stopwatch
            ), $priority);
            // $this->dispatcher->addListener(new TraceableListener($listener, $name, $this->stopwatch),
            // array($event->getName()=> $priority));
        }
    }
        ...
        foreach ($this->dispatcher->getListeners($event) as $listener) {
            ...
            if (null !== $this->logger && $skipped) {
                $this->logger->debug(sprintf(
                    'Listener "%s" was not called for event "%s".',
                    $info['pretty'],
                    $eventName
                ));
            }
            ...
            if ($listener->stoppedPropagation()) {
                if (null !== $this->logger) {
                    $this->logger->debug(sprintf(
                        'Listener "%s" stopped propagation of the event "%s".',
                        $info['pretty'],
                        $eventName
                    ));
                }
