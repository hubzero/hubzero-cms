...
class Request extends BaseRequest
{
     ...
     * @param   string   $hash     Where the public should come from (POST, GET, FILES, COOKIE, METHOD)
    ...
    public function getInt($key, $default = 0, $hash = 'input')
    {
        ...
        $str = is_array($str) ? self::flatten('', $str) : $str;
    ...
    public static function normalizeQueryString($qs)
    {
        ...
        foreach (explode('&', $qs) as $param) {
            if ('' === $param || '=' === $param[0]) {
                ...
                // Also ignore pairs with empty key, even if there was a value, e.g. "=value", as such nameless values
                // cannotbe retrieved anyway.
            ...
            // PHP also converts "+" to spaces when filling the global _GET or when using the function parse_str. This
            // iswhy we use urldecode and then normalize to
        ...
        if ($new_state !== null) {
            switch ($type) {
                case 'int':
                    ...
                    break;
                case 'word':
                    $new_state = (string) self::flatten('', $new_state);
     ...
     * @return  string
     */
    private function flatten($separator, $arrayvar)
    {
        $out = '';
        ...
        if (is_array($arrayvar)) {
            foreach ($arrayvar as $av) {
                if (is_array($av)) {
                    $out .= self::flatten($separator, $av); // Recursive Use of the Array
                } else {
                    $out .= $separator . $av;
                }
            }
