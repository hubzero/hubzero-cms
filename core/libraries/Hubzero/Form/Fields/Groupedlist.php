            ...
            switch ($element->getName()) {
                ...
                case 'option':
                    ...
                    $tmp = Dropdown::option(
                        ...
                        App::get('language')->
                            alt(trim((string) $element), preg_replace('/[^a-zA-Z0-9_\-]/', '_', $this->
                            fieldname)),
                        'value',
                        'text',
                ...
                case 'group':
                    ...
                    foreach ($element->children() as $option) {
                        ...
                        // Create a new option object based on the <option /> element.
                        $tmp = Dropdown::option(
                            ($option['value']) ? (string) $option['value'] : App::get('language')->txt(trim((string)
                                $option)),
                            App::get('language')->txt(trim((string) $option)),
                            'value',
                            'text',
    ...
    protected function getInput()
    {
        ...
        if ((string) $this->element['readonly'] == 'true') {
            $html[] = Dropdown::groupedlist(
                ...
                null,
                array(
                    'list.attr' => $attr, 'id' => $this->
                        id, 'list.select' => $this->
                        value, 'group.items' => null, 'option.key.toHtml' => false,
