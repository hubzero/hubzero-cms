 ...
 */
  ...
  */
  ...
  public function EncodeQ($str, $position = 'text') {
    ...
    switch (strtolower($position)) {
      case 'phrase':
        $encoded = preg_replace_callback("/([^A-Za-z0-9!*+\/ -])/",function($m) { return '='.sprintf('%02X', ord($m[1])); }, $encoded);
        break;
      case 'comment':
        $encoded = preg_replace_callback("/([\(\)\"])/", function($m) { return '='.sprintf('%02X', ord($m[1])); }, $encoded);
      case 'text':
      default:
