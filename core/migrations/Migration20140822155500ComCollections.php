    ...
    public function up()
    {
        if ($this->db->tableExists('#__collections_following')) {
            if (!$this->db->tableHasKey('#__collections_following', 'idx_follower_type_follower_id')) {
                $query = "ALTER TABLE `#__collections_following` "
                    . "ADD INDEX `idx_follower_type_follower_id` (`follower_type`, `follower_id`);";
            ...
            if (!$this->db->tableHasKey('#__collections_following', 'idx_following_type_following_id')) {
                $query = "ALTER TABLE `#__collections_following` "
                    . "ADD INDEX `idx_following_type_following_id` (`following_type`, `following_id`);";
                $this->db->setQuery($query);
                $this->db->query();
            }
        }
