 ...
 **/
class Migration20190318000000HgUpdateQueue extends Base
{
	/**
	 * Up
	 **/
	public function up()
	{
		...
		$triggers = $this->db->loadColumn('Trigger');
		...
		if (!empty($triggers))
		{
			...
			{
				if (substr($trigger, 0, strlen('hg_')) == 'hg_')
				{
					$hasTriggers = true;
				}
			}
		...
		if ($hasTriggers)
		{
			// Original attmpt found in Migration20170627153900ComHubgraph.php
			if (!$this->removeTriggers())
			{
				$this->setError('Failed to remove `hg_*` triggers. Cannot drop `hg_update_queue` table.', 'warning');
				return false;
			}
			else
			{
				// Double paranoid check
				$query = "SHOW TRIGGERS;";
				$this->db->setQuery($query);
				$triggers = $this->db->loadColumn('Trigger');
		...
		if (!$hasTriggers)
		{
			if ($this->db->tableExists('hg_update_queue'))
			{
				...
				$this->log('Dropped table `%s`', 'hg_update_queue');
			}
		}
		else
		{
	 ...
	 * @return  bool
	 **/
	private function removeTriggers()
	{
		if (!$this->runAsRoot())
		{
		...
		foreach ($triggers as $trigger)
		{
			...
			$this->log('Dropped trigger `%s`', $trigger);
		}
	...
	public function down()
	{
		if (!$this->db->tableExists('hg_update_queue'))
		{
			...
			$this->log('Created table `hg_update_queue`');
		}
	}
}
