    ...
    public function up()
    {
        if ($this->db->tableExists('#__contact_details')) {
            ...
            if ($this->db->tableHasField('#__contact_details', 'alias')) {
                $query = "ALTER TABLE `#__contact_details` CHANGE COLUMN `alias` `alias` VARCHAR(255) CHARACTER "
                    . "SET 'utf8' COLLATE 'utf8_bin' NOT NULL DEFAULT '';";
                $this->db->setQuery($query);
                $this->db->query();
            }
            if ($this->db->tableHasField('#__contact_details', 'checked_out')) {
                $query = "ALTER TABLE `#__contact_details` CHANGE COLUMN `checked_out` `checked_out` INT(10) "
                    . "UNSIGNED NOT NULL DEFAULT '0';";
                $this->db->setQuery($query);
                $this->db->query();
            }
            if ($this->db->tableHasField('#__contact_details', 'access')) {
                $query = "ALTER TABLE `#__contact_details` CHANGE COLUMN `access` `access` INT(10) UNSIGNED NOT "
                    . "NULL DEFAULT '0';";
                $this->db->setQuery($query);
                $this->db->query();
            }
            ...
            if (!$this->db->tableHasField('#__contact_details', 'created')) {
                $query = "ALTER TABLE `#__contact_details` ADD COLUMN  `created` datetime NOT NULL DEFAULT "
                    . "'0000-00-00 00:00:00';";
                $this->db->setQuery($query);
                $this->db->query();
            }
            if (!$this->db->tableHasField('#__contact_details', 'created_by')) {
                $query = "ALTER TABLE `#__contact_details` ADD COLUMN   `created_by` int(10) unsigned NOT NULL "
                    . "DEFAULT '0';";
                $this->db->setQuery($query);
                $this->db->query();
            }
            if (!$this->db->tableHasField('#__contact_details', 'created_by_alias')) {
                $query = "ALTER TABLE `#__contact_details` ADD COLUMN   `created_by_alias` varchar(255) NOT NULL "
                    . "DEFAULT '';";
                $this->db->setQuery($query);
                $this->db->query();
            }
            if (!$this->db->tableHasField('#__contact_details', 'modified')) {
                $query = "ALTER TABLE `#__contact_details` ADD COLUMN   `modified` datetime NOT NULL DEFAULT "
                    . "'0000-00-00 00:00:00';";
                $this->db->setQuery($query);
                $this->db->query();
            }
            if (!$this->db->tableHasField('#__contact_details', 'modified_by')) {
                $query = "ALTER TABLE `#__contact_details` ADD COLUMN   `modified_by` int(10) unsigned NOT NULL "
                    . "DEFAULT '0';";
                $this->db->setQuery($query);
                $this->db->query();
            }
            ...
            if (!$this->db->tableHasField('#__contact_details', 'featured')) {
                $query = "ALTER TABLE `#__contact_details` ADD COLUMN   `featured` tinyint(3) unsigned NOT NULL "
                    . "DEFAULT '0' COMMENT 'Set if article is featured.';";
                $this->db->setQuery($query);
                $this->db->query();
            }
            if (!$this->db->tableHasField('#__contact_details', 'xreference')) {
                $query = "ALTER TABLE `#__contact_details` ADD COLUMN   `xreference` varchar(50) NOT NULL COMMENT"
                    . "'A reference to enable linkages to external data sets.';";
                $this->db->setQuery($query);
                $this->db->query();
            }
            if (!$this->db->tableHasField('#__contact_details', 'publish_up')) {
                $query = "ALTER TABLE `#__contact_details` ADD COLUMN   `publish_up` datetime NOT NULL DEFAULT "
                    . "'0000-00-00 00:00:00';";
                $this->db->setQuery($query);
                $this->db->query();
            }
            if (!$this->db->tableHasField('#__contact_details', 'publish_down')) {
                $query = "ALTER TABLE `#__contact_details` ADD COLUMN   `publish_down` datetime NOT NULL DEFAULT "
                    . "'0000-00-00 00:00:00';";
                $this->db->setQuery($query);
                $this->db->query();
            }
            if ($this->db->tableHasField('#__contact_details', 'published')) {
                $query = "ALTER TABLE `#__contact_details` CHANGE `published` `published` tinyint(1) NOT NULL "
                    . "DEFAULT '0';";
                $this->db->setQuery($query);
                $this->db->query();
            }
