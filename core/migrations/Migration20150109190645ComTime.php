...
class Migration20150109190645ComTime extends Base
{
    ...
    public function up()
    {
        ...
        if ($this->db->tableExists('#__time_tasks')) {
            if (
                $this->db->tableHasField('#__time_tasks', 'liaison')
            ...
            if (
                $this->db->tableHasField('#__time_tasks', 'assignee')
        ...
        if (!$this->db->tableExists('#__time_liaisons')) {
            ...
            if ($liaisons && count($liaisons) > 0) {
                foreach ($liaisons as $liaison) {
                    $query = "INSERT INTO `#__time_liaisons` (`user_id`) VALUES ("
                        . $this->db->quote($liaison->user_id) . ")";
                    $this->db->setQuery($query);
                    $this->db->query();
                }
            }
        ...
        if (
            $this->db->tableExists('#__time_proxies')
