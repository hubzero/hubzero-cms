...
class Hubzero_Cart
{
	...
	public function setTransactionShippingInfo() 
	{
		...
		// Check values
		ximport('Hubzero_Cart_Helper');
		
		if (empty($errors) && !Hubzero_Cart_Helper::validZip(JRequest::getVar('shippingZip', false, 'post', 'string')))
		{
		...
		{
			// save shipping info
			$shippingToFirst = Hubzero_Cart_Helper::escapeDb(JRequest::getVar('shippingToFirst', false, 'post', 'string'));
			$shippingToLast = Hubzero_Cart_Helper::escapeDb(JRequest::getVar('shippingToLast', false, 'post', 'string'));
			$shippingAddress = Hubzero_Cart_Helper::escapeDb(JRequest::getVar('shippingAddress', false, 'post', 'string'));
			$shippingCity = Hubzero_Cart_Helper::escapeDb(JRequest::getVar('shippingCity', false, 'post', 'string'));
			$shippingState = Hubzero_Cart_Helper::escapeDb(JRequest::getVar('shippingState', false, 'post', 'string'));
			$shippingZip = Hubzero_Cart_Helper::escapeDb(JRequest::getVar('shippingZip', false, 'post', 'string'));
			...
			$saveAddress = Hubzero_Cart_Helper::escapeDb(JRequest::getVar('saveAddress', false, 'post', 'string'));
	...
	{		
		...
		ximport('Hubzero_Cart_ProductHandler');
		
		foreach ($transactionItems as $sId => $item)
		{
			$productHandler = new Hubzero_Cart_ProductHandler($item, $crtId);
	...
	public function getSavedShippingAddresses()
	{
		...
		$juser =& JFactory::getUser();
		$uId = $juser->id;
		$uId = Hubzero_Cart_Helper::escapeDb($uId);
	...
	{
		ximport('Hubzero_Storefront_Coupons');
		$coupon = Hubzero_Storefront_Coupons::getCouponInfo($cnId, true, true, true, true);
	...
	public function getCoupons() 
	{
		...
		// Get coupon types
		ximport('Hubzero_Storefront_Coupons');
		$coupons = Hubzero_Storefront_Coupons::getCouponsInfo($cnIds);
...
/* ----------------------------------------------------------------------------------------------------------------------------------------------- */
	...
	private function linkCarts()
	{
		...
		ximport('Hubzero_Storefront_Coupons');
		$storefrontCoupons = new Hubzero_Storefront_Coupons;
