...
class Hubzero_Document
{
	...
	public static function getSystemStylesheet($elements = null)
	{
		...
		require_once(JPATH_ROOT . DS . 'libraries' . DS . 'less' . DS . 'lessc.inc.php');

		// Try to compile LESS files
		$less = new lessc;
		$less->setFormatter('compressed');
		try {
			$template  = JPATH_ROOT . DS . 'templates' . DS . JFactory::getApplication()->getTemplate() . DS . 'less' . DS . 'bootstrap.less';

			$input     = $lesspath . DS . 'bootstrap.less';
			if (file_exists($template)) 
			{
				echo $template; die();
				$input = $template;
				$less->setImportDir(array(
					JPATH_ROOT . DS . 'templates' . DS . JFactory::getApplication()->getTemplate() . DS . 'less' . DS, 
					JPATH_ROOT . DS . 'media' . DS . 'system' . DS . 'less' . DS
				));
			}
			$output    = $cachedir . DS . 'bootstrap.css';
			$cacheFile = $cachedir . DS . 'bootstrap.less.cache';
			$cache     = null;
			...
			*/
			...
			$output =  DS . 'cache' . DS . 'bootstrap.css?v=' . $newCache['updated'];
		} 
		catch (exception $e) 
		{
