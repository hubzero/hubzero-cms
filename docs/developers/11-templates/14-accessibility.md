...
## Why read this before you start

Two practical reasons.

**Half of a page is not yours.** The pagination, the admin grids, the toolbar,
the form labels and the message block are generated by framework helpers. If
you assume the framework handles this and build on top, you inherit whatever
those helpers do — and this chapter is a list of what they do. Knowing which
parts arrive already built tells you where an
[output override](09-overrides.md) is the only lever you have.

**The shipped templates are not a safe pattern to copy.** `northgate` starts
from `kimera`, and `kimera` has no skip link and no visually-hidden utility
class. Copying it means inheriting both. Each section below says what to copy,
what to fix on the way, and what you cannot reach from a template at all.
...
- Live regions are close to absent, and none of them is the one every page has.
  A handful of component views declare `aria-live` — the upload list in
  [`com_support`'s new-ticket view](../../../core/components/com_support/site/views/tickets/tmpl/new.php)
  is one — and each is a one-off written by whoever wrote that view. Nothing in
  a template, a module, or the framework's own markup declares one, so there is
  no region for a template to write into and nothing to inherit.
...
`app/templates/northgate/html/{extension}/{view}/{layout}.php` replaces the
whole layout, so you can write the header cell yourself:

```php
<th scope="col" aria-sort="<?php echo $sorted ? $dir : 'none'; ?>">
	<a href="<?php echo $sortUrl; ?>"><?php echo Lang::txt('COM_BOOKINGS_INSTRUMENT'); ?></a>
</th>
```
