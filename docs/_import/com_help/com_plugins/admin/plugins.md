<!--
status: imported
source: core/components/com_plugins/admin/help/en-GB/plugins.phtml
imported: 2026-09-09
-->
# Plugins

The plug-in manager is accessed from the back-end administrator panel. It is used to edit an installed plug-in's details and options.

## How to Access

Select **Extensions → Plug-in Manager** from the drop-down menu of the ***Administrator Panel***.

## Description

The Plug-in Manager allows you to enable and disable plug-ins and to edit a plug-in's details and options. It is also useful for quickly enabling/disabling multiple plug-ins at once.
...
- **Element.** The plug-in directory name which contains the plug-in's files. This directory will be located in the directory which corresponds to its type. For example, the 'Authentication - Hubzero' plug-in is of type 'Authentication' and element 'hubzero'. So the website directory it is located in is 'plugins/authentication/hubzero'
...
- Configurable plug-in settings are referred to as 'options'. In HUBzero 1.2 and lower these configurable settings were referred to as 'parameters'. You may see the terms 'options' and 'parameters' used interchangeably in help documentation and tutorials you encounter.

## Related Information

- To change the Details or Options for a Plug-in: Plugin Manager - Edit
- To install Plug-ins: Extension Manager - Install
- To uninstall Plug-ins: Extension Manager - Manage
