 ...
 * for the DAQ channel, and include it as a <TimeSeries> XML element.
 */

require_once 'lib/render/SDORenderer.php';

class DAQChannelSDORenderer extends SDORenderer {
  ...
  function render( BaseObject $obj, $title = null ) {
    ...
    $objxml = $doc->getRootDataObject("central")->DAQChannel[0];

    // Figure out where the data file is, and which column we need to read out of it.
    $dataFiles = $obj->getOutputDataFiles();
    /*
