...
class Authorizer {
	...
	{	  	
  	...
  	else
  	{
    		...
    		if(is_null($person))
    		{
    			...
    			JError::raiseError( 500, 'Cannot find user information for:' . $userName);
    		}
   ...
   * retrienve an authorizer for use within the hub
   *
   * @return Authorizer
   */
  public static function getInstanceForUseOnHub($userName, $entitiyID, $entityTypeID) {
    if (empty(self::$instance))
      self::$instance = new Authorizer($userName, $entitiyID, $entityTypeID);
  ...
  private function canDo($entity, $action) {
  	...
  	// NEES/Joomla addition for super user access
