...
abstract class BaseTrial extends BaseObject  implements Persistent {
	...
	public function setCreatorId($v)
	{
		...
		if ($this->creator_id !== $v) {
			$this->creator_id = $v;
			$this->modifiedColumns[] = TrialPeer::CREATOR_ID;
		}
	...
	public function setCreatedDate($v)
	{
		...
		if ($this->created_date !== $ts) {
			...
			$this->modifiedColumns[] = TrialPeer::CREATED_DATE;
		}
	...
	public function setModifiedById($v)
	{
		...
		if ($this->modified_by_id !== $v) {
			...
			$this->modifiedColumns[] = TrialPeer::MODIFIED_BY_ID;
		}
	...
	public function setModifiedDate($v)
	{
		...
		if ($this->modified_date !== $ts) {
			...
			$this->modifiedColumns[] = TrialPeer::MODIFIED_DATE;
		}
	...
	public function setAppId($v)
	{
		...
		if ($this->app_id !== $v) {
			$this->app_id = $v;
			$this->modifiedColumns[] = TrialPeer::APP_ID;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->creator_id = $rs->getFloat($startcol + 19);

			$this->created_date = $rs->getDate($startcol + 20, null);

			$this->modified_by_id = $rs->getFloat($startcol + 21);

			$this->modified_date = $rs->getDate($startcol + 22, null);

			$this->app_id = $rs->getFloat($startcol + 23);
			...
			return $startcol + 24; // 24 = TrialPeer::NUM_COLUMNS - TrialPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		$result = array(
			...
			$keys[19] => $this->getCreatorId(),
			$keys[20] => $this->getCreatedDate(),
			$keys[21] => $this->getModifiedById(),
			$keys[22] => $this->getModifiedDate(),
			$keys[23] => $this->getAppId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[19], $arr)) $this->setCreatorId($arr[$keys[19]]);
		if (array_key_exists($keys[20], $arr)) $this->setCreatedDate($arr[$keys[20]]);
		if (array_key_exists($keys[21], $arr)) $this->setModifiedById($arr[$keys[21]]);
		if (array_key_exists($keys[22], $arr)) $this->setModifiedDate($arr[$keys[22]]);
		if (array_key_exists($keys[23], $arr)) $this->setAppId($arr[$keys[23]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(TrialPeer::CREATOR_ID)) $criteria->add(TrialPeer::CREATOR_ID, $this->creator_id);
		if ($this->isColumnModified(TrialPeer::CREATED_DATE)) $criteria->add(TrialPeer::CREATED_DATE, $this->created_date);
		if ($this->isColumnModified(TrialPeer::MODIFIED_BY_ID)) $criteria->add(TrialPeer::MODIFIED_BY_ID, $this->modified_by_id);
		if ($this->isColumnModified(TrialPeer::MODIFIED_DATE)) $criteria->add(TrialPeer::MODIFIED_DATE, $this->modified_date);
		if ($this->isColumnModified(TrialPeer::APP_ID)) $criteria->add(TrialPeer::APP_ID, $this->app_id);
	 ...
	 * actually need in Trial.
	 */
	public function getRepetitionsJoinPersonRelatedByCreatorId($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'lib/data/om/BaseRepetitionPeer.php';
		if ($criteria === null) {
		...
		if ($this->collRepetitions === null) {
			if ($this->isNew()) {
				...
				$this->collRepetitions = RepetitionPeer::doSelectJoinPersonRelatedByCreatorId($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * actually need in Trial.
	 */
	public function getRepetitionsJoinPersonRelatedByModifiedById($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'lib/data/om/BaseRepetitionPeer.php';
		if ($criteria === null) {
		...
		if ($this->collRepetitions === null) {
			if ($this->isNew()) {
				...
				$this->collRepetitions = RepetitionPeer::doSelectJoinPersonRelatedByModifiedById($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
