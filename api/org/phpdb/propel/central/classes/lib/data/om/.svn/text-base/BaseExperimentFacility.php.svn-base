...
include_once 'lib/data/ExperimentFacilityPeer.php';

/**
 * Base class that represents a row from the 'EXPERIMENT_FACILITY' table.
 *
 * 
 *
 * @package    lib.data.om
 */
abstract class BaseExperimentFacility extends BaseObject  implements Persistent {
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = ExperimentFacilityPeer::ID;
		}
	...
	public function setExperimentId($v)
	{
		...
		if ($this->expid !== $v) {
			$this->expid = $v;
			$this->modifiedColumns[] = ExperimentFacilityPeer::EXPID;
		}
	...
	public function setFacilityId($v)
	{
		...
		if ($this->facilityid !== $v) {
			$this->facilityid = $v;
			$this->modifiedColumns[] = ExperimentFacilityPeer::FACILITYID;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->facilityid = $rs->getFloat($startcol + 2);
			...
			return $startcol + 3; // 3 = ExperimentFacilityPeer::NUM_COLUMNS - ExperimentFacilityPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating ExperimentFacility object", $e);
		}
	}
	...
	public function delete($con = null)
	{
		...
		try {
			$con->begin();
			ExperimentFacilityPeer::doDelete($this, $con);
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = ExperimentFacilityPeer::doInsert($this, $con);
					...
					$affectedRows += ExperimentFacilityPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = ExperimentFacilityPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = ExperimentFacilityPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = ExperimentFacilityPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ExperimentFacilityPeer::ID)) $criteria->add(ExperimentFacilityPeer::ID, $this->id);
		if ($this->isColumnModified(ExperimentFacilityPeer::EXPID)) $criteria->add(ExperimentFacilityPeer::EXPID, $this->expid);
		if ($this->isColumnModified(ExperimentFacilityPeer::FACILITYID)) $criteria->add(ExperimentFacilityPeer::FACILITYID, $this->facilityid);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(ExperimentFacilityPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of ExperimentFacility (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     ExperimentFacility Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     ExperimentFacilityPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new ExperimentFacilityPeer();
		}
		return self::$peer;
	}
