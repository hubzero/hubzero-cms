 ...
 **/
class Migration20150918134424ComUsage extends Base
{
	/**
	 * Up
	 **/
	public function up()
	{
		...
		if (!$connected)
		{
			...
			$query = "SELECT Host, Db, User FROM mysql.db WHERE User = " . $this->db->quote($options['user']) . " AND Db = " . $this->db->quote($options['database']);
			$this->db->setQuery($query);
			$results = $this->db->loadObjectList();

			if (count($results) > 0)
			{
				$this->setError('Metrics database connection credentials are invalid.', 'warning');
				return false;
			}
			else
			{
			...
			{
				$this->setError('Metrics database connection failed.', 'warning');
				return false;
			}
		}
