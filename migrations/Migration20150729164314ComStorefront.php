...
class Migration20150729164314ComStorefront extends Base
{
	...
	public function up()
	{
		if ($this->db->tableExists('#__storefront_collections') && $this->db->tableHasField('#__storefront_collections', 'cParent')) {
			$query = "ALTER TABLE `#__storefront_collections` MODIFY `cParent` CHAR";
		...
		if ($this->db->tableExists('#__storefront_product_types')) {
			$query = "INSERT INTO `#__storefront_product_types` (`ptId`, `ptName`, `ptModel`) VALUES (30, 'Software Download', 'software')";
			$this->db->setQuery($query);
			$this->db->query();
		}
	}
	...
	{
		if ($this->db->tableExists('#__storefront_collections') && $this->db->tableHasField('#__storefront_collections', 'cParent')) {
			$query = "ALTER TABLE `#__storefront_collections` MODIFY `cParent` INT(16)";
			$this->db->setQuery($query);
			$this->db->query();
		}
		...
		if ($this->db->tableExists('#__storefront_product_types')) {
			$query = "DELETE FROM `#__storefront_product_types` WHERE ptId = 30 AND ptName = 'Software Download' AND  ptModel = 'software'";
			$this->db->setQuery($query);
			$this->db->query();
		}
	}
