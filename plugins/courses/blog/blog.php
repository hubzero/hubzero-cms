 ...
 */
class plgCoursesBlog extends JPlugin
{
	/**
	 * Constructor
	 * 
	...
	public function &onCourseAreas()
	{
		$area = array(
			'name' => 'blog',
			'title' => JText::_('PLG_COURSES_BLOG'),
	...
	public function onCourse($course, $option, $authorized, $limit=0, $limitstart=0, $action='', $access, $areas=null)
	{
		...
		if ($return == 'html') 
		{
			...
			$this->params = $p->getParams($course->gidNumber, 'courses', 'blog');

			//push the css to the doc
			ximport('Hubzero_Document');
			Hubzero_Document::addPluginStylesheet('courses', 'blog');
	...
	private function _browse()
	{
		...
		if (strstr($path, '/')) 
		{
			$path = str_replace('/courses/' . $this->course->get('cn') . '/blog', '', $path);
			$path = ltrim($path, DS);
			$bits = explode('/', $path);
		...
		$pagenavhtml = str_replace('courses/?', 'courses/' . $this->course->get('cn') . '/blog/?', $pagenavhtml);
		$pagenavhtml = str_replace('action=browse', '', $pagenavhtml);
		$pagenavhtml = str_replace('&amp;&amp;', '&amp;', $pagenavhtml);
		$pagenavhtml = str_replace('?&amp;', '?', $pagenavhtml);

		$path = $this->params->get('uploadpath');
		$view->path = str_replace('{{gid}}', $this->course->get('gidNumber'),$path);
	...
	private function _feed()
	{
		...
		$doc->link = JRoute::_('index.php?option=' . $this->option . '&gid='.$this->course->cn . '&active=blog');
		...
		$doc->title  = $jconfig->getValue('config.sitename').': '.JText::_('Courses').': '.stripslashes($this->course->description).': '.JText::_('Blog');
		//$doc->title .= ($filters['year']) ? ': '.$filters['year'] : '';
		//$doc->title .= ($filters['month']) ? ': '.sprintf("%02d",$filters['month']) : '';

		$doc->description = JText::sprintf('PLG_COURSES_BLOG_RSS_DESCRIPTION',$jconfig->getValue('config.sitename'));
		$doc->copyright = JText::sprintf('PLG_COURSES_BLOG_RSS_COPYRIGHT', date("Y"), $jconfig->getValue('config.sitename'));
		$doc->category = JText::_('PLG_COURSES_BLOG_RSS_CATEGORY');
		...
		if (count($rows) > 0) 
		{
			...
			$path = $this->params->get('uploadpath');
			$path = str_replace('{{gid}}', $this->course->get('gidNumber'), $path);
			...
			foreach ($rows as $row)
			{
				...
				// URL link to article
				$link = JRoute::_('index.php?option=' . $this->option . '&gid=' . $this->course->cn . '&active=blog&scope=' . JHTML::_('date', $row->publish_up, $this->yearFormat, 0) . '/' . JHTML::_('date', $row->publish_up, $this->monthFormat, 0) . '/' . $row->alias);
	...
	private function _entry()
	{
		...
		else 
		{
			...
			$view->row->loadAlias($alias, 'course', 0, $this->course->get('gidNumber'));
		}
		...
		{
			JError::raiseError(403, JText::_('PLG_COURSES_BLOG_NOT_AUTH'));
			return;
		}
		...
		if ($view->row->content) 
		{
			...
			$wikiconfig = array(
				'option'   => $this->option,
				'scope'    => $this->course->get('gidNumber') . DS . 'blog',
		...
		if (is_file(JPATH_ROOT . DS . 'plugins' . DS . 'courses' . DS . 'blog' . DS . 'blog.js')) {
			$document->addScript('plugins' . DS . 'courses' . DS . 'blog' . DS . 'blog.js');
		}*/
		$view->canpost = $this->_getPostingPermissions();
	...
	private function _edit()
	{
		$juser =& JFactory::getUser();
		$app =& JFactory::getApplication();
		$blog = JRoute::_('index.php?option=' . $this->option . '&gid=' . $this->course->get('cn') . '&active=blog');

		if ($juser->get('guest')) 
		{
			//$app->enqueueMessage(JText::_('COURSES_LOGIN_NOTICE'), 'warning');
		...
		if (!$view->entry->id) 
		{
			...
			$view->entry->state = 0;
			$view->entry->scope = 'course';
			$view->entry->created_by = $juser->get('id');
			$view->entry->course_id = $this->course->get('gidNumber');
		}
	...
	private function _save()
	{
		...
		if ($juser->get('guest')) 
		{
			$this->setError(JText::_('COURSES_LOGIN_NOTICE'));
			return $this->_login();
		}

		if (!$this->authorized) 
		{
			$this->setError(JText::_('PLG_COURSES_BLOG_NOT_AUTHORIZED'));
			return $this->_browse();
		}
		...
		//return $this->_entry();
		$app =& JFactory::getApplication();
		$app->redirect(JRoute::_('index.php?option=com_courses&gid=' . $this->course->get('cn') . '&active=blog&scope=' . JHTML::_('date', $row->publish_up, $this->yearFormat, 0) . '/' . JHTML::_('date', $row->publish_up, $this->monthFormat, 0) . '/' . $row->alias));
	}
	...
	private function _delete()
	{
		...
		if (!$process || !$confirmdel) 
		{
			if ($process && !$confirmdel) 
			{
				$this->setError(JText::_('PLG_COURSES_BLOG_ERROR_CONFIRM_DELETION'));
			}
	...
	private function _settings()
	{
		...
		$view->settings->loadPlugin($this->course->gidNumber, 'courses', 'blog');
		$view->authorized = $this->authorized;
	...
	private function _savesettings()
	{
		...
		$app->redirect(JRoute::_('index.php?option=com_courses&gid=' . $this->course->get('cn') . '&active=blog&task=settings'));
	}
