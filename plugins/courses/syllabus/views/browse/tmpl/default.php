...
<div class="main section">
			...
			<div class="container">
				...
				<?php
					if ($this->config->get('feeds_enabled')) {
						$path  = 'index.php?option='.$this->option.'&gid='.$this->course->cn.'&active=blog&scope=feed.rss';
			...
			<ol>
				<?php if ($this->firstentry) { ?>
					...
					<?php for ($i=$now, $n=$start; $i >= $n; $i--) { ?>
						<li>
							<a href="<?php echo JRoute::_('index.php?option=com_courses&gid='.$this->course->cn.'&active=blog&scope='.$i); ?>">
								<?php echo $i; ?>
							</a>
							<?php if (($this->year && $i == $this->year) || (!$this->year && $i == $now)) { ?>
								<ol>
									<?php
										$m = array(
											'PLG_COURSES_BLOG_JANUARY',
											'PLG_COURSES_BLOG_FEBRUARY',
											'PLG_COURSES_BLOG_MARCH',
											'PLG_COURSES_BLOG_APRIL',
											'PLG_COURSES_BLOG_MAY',
											'PLG_COURSES_BLOG_JUNE',
											'PLG_COURSES_BLOG_JULY',
											'PLG_COURSES_BLOG_AUGUST',
											'PLG_COURSES_BLOG_SEPTEMBER',
											'PLG_COURSES_BLOG_OCTOBER',
											'PLG_COURSES_BLOG_NOVEMBER',
											'PLG_COURSES_BLOG_DECEMBER'
										);
										if ($i == $now) {
											$months = date("m");
										} else {
									...
									<?php for ($k=0, $z=$months; $k < $z; $k++) { ?>
										<li>
											<a<?php if ($this->month && $this->month == ($k+1)) { echo ' class="active"'; } ?> href="<?php echo JRoute::_('index.php?option=com_courses&gid='.$this->course->get('cn').'&active=blog&scope='.$i.'/'.sprintf( "%02d",($k+1),1)); ?>">
			...
			<ol>
				<?php if ($this->popular) { ?>
					<?php foreach ($this->popular as $row) { ?>
						<li>
							<a href="<?php echo JRoute::_('index.php?option=com_courses&gid='.$this->course->cn.'&active=blog&scope='.JHTML::_('date',$row->publish_up, $this->yearFormat, $this->tz).'/'.JHTML::_('date',$row->publish_up, $this->monthFormat, $this->tz).'/'.$row->alias); ?>">
	...
	<div class="subject">
		<form action="<?php echo JRoute::_('index.php?option=com_courses&gid='.$this->course->cn.'&active=blog&task=browse'); ?>" method="get" id="blogentries">
...
<?php if ($this->juser->get('id') == $row->created_by || $this->authorized == 'manager' || $this->authorized == 'admin') { ?>
						<a class="edit" href="<?php echo JRoute::_('index.php?option=com_courses&gid='.$this->course->cn.'&active=blog&task=edit&entry='.$row->id); ?>" title="<?php echo JText::_('Edit'); ?>">
							<?php echo JText::_('Edit'); ?>
						</a>
						<a class="delete" href="<?php echo JRoute::_('index.php?option=com_courses&gid='.$this->course->cn.'&active=blog&task=delete&entry='.$row->id); ?>" title="<?php echo JText::_('Delete'); ?>">
...
<?php if ($row->allow_comments == 1) { ?>
						<dd class="comments">
							<a href="<?php echo JRoute::_('index.php?option=com_courses&gid='.$this->course->cn.'&active=blog&scope='.JHTML::_('date', $row->publish_up, $this->yearFormat, $this->tz) . '/' . JHTML::_('date', $row->publish_up, $this->monthFormat, $this->tz) . '/' . $row->alias . '#comments'); ?>">
								<?php echo JText::sprintf('PLG_COURSES_BLOG_NUM_COMMENTS', $row->comments); ?>
							</a>
						</dd>
<?php } else { ?>
						<dd class="comments">
...
<?php } ?>
					...
					<div class="entry-content">
						<p>
							<?php 
							$content = plgCoursesBlog::stripWiki($row->content);
