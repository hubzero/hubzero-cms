...
/**
 * Courses Plugin class for wiki
 */
class plgCoursesWiki extends JPlugin
{
	/**
	 * Constructor
	 * 
	 ...
	 * @return     void
	 */
	public function plgCoursesWiki(&$subject, $config)
	{
	...
	public function &onCourseAreas()
	{
		$area = array(
			'name' => 'wiki',
			'title' => JText::_('PLG_COURSES_WIKI'),
	...
	public function onCourse($course, $option, $authorized, $limit=0, $limitstart=0, $action='', $access, $areas=null)
	{
		...
		if ($return == 'html') 
		{
			 ...
			 && ($course_plugin_acl == 'registered' || $course_plugin_acl == 'members')) 
			{
				...
				$arr['html']  = '<p class="warning">' . JText::sprintf('COURSES_PLUGIN_REGISTERED', ucfirst($active)) . '</p>';
			...
			if (!$scope) 
			{
				JRequest::setVar('scope', $course->get('cn') . DS . $active);
			}
			...
			Hubzero_Document::addPluginStylesheet('courses', 'wiki');
	...
	/**
	 * Remove any associated resources when course is deleted
	 * 
	...
	{
		// Get all the IDs for pages associated with this course
		$ids = $this->getPageIDs($course->get('cn'));
		...
		// Start the log text
		$log = JText::_('PLG_COURSES_WIKI_LOG') . ': ';

		if (count($ids) > 0) 
		{
			// Loop through all the IDs for pages associated with this course
			foreach ($ids as $id)
			{
		...
		{
			$log .= JText::_('PLG_COURSES_WIKI_NO_RESULTS_FOUND')."\n";
		}
	...
	/**
	 * Return a count of items that will be removed when course is deleted
	 * 
	...
	{
		return JText::_('PLG_COURSES_WIKI_LOG') . ': ' . count($this->getPageIDs($course->get('cn')));
	}

	/**
	 * Get a list of page IDs associated with this course
	 * 
	 * @param      string $gid Course alias
	 * @return     array
	 */
	public function getPageIDs($gid=NULL)
	{
		...
		$database =& JFactory::getDBO();
		$database->setQuery("SELECT id FROM #__wiki_page AS p WHERE p.course_cn='" . $gid . "'");
