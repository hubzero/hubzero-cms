...
(function ($) {
  function init(plot) {
    ...
    function onMouseMove(e) {
      if (selection.active) {
        if (selection.navigate == true) {
          moveSelection(e);
          plot.getPlaceholder().trigger("plotnavigating", [getSelection()]);
        }
        else {
          updateSelection(e);
    ...
    function onMouseDown(e) {
      ...
      if (plot.getOptions().selection.navigate == true &&
          isInsideSelection(e)) {
        // don't redraw, navigate instead
        selection.navigate = true;
        setSelectionPos(selection.current, e);
      }
      else {
        // redraw
    ...
    function onMouseUp(e) {
      ...
      if (selectionIsSane()) {
        if (selection.navigate == true) {
          plot.getPlaceholder().trigger("plotnavigated", [getSelection()]);
        }
        else {
    ...
    function moveSelection(pos) {
      ...
      var start = { x: selection.current.x, y: selection.current.y };
      setSelectionPos(selection.current, pos);

      var xdiff = start.x - selection.current.x;
      var ydiff = start.y - selection.current.y;

		if ((selection.second.x - xdiff) >= plot.width() || (selection.first.x - xdiff) <= 0) {
			xdiff = 0;
		}
		selection.first.x -= xdiff;
		selection.second.x -= xdiff;
		if ((selection.second.y - ydiff) >= plot.height() || (selection.first.y - ydiff) <= 0) {
			ydiff = 0;
		}
      	selection.first.y -= ydiff;
		selection.second.y -= ydiff;
