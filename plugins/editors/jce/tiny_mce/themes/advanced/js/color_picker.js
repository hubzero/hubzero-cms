...
var templateColors = getTemplateColors();
...
function getTemplateColors(){
	var ed 	= tinyMCEPopup.editor, clr = [], c, x, i, hex;		
	var doc = ed.getDoc();
	var styles = doc.styleSheets;
	var re = tinymce.isIE ? new RegExp('#[0-9a-f]{3,6}', 'gi') : new RegExp('rgb\\s*\\(\\s*([0-9]+).*,\\s*([0-9]+).*,\\s*([0-9]+).*\\)', 'gi');
	...
	for (x=0; x<styles.length; x++) {
		...
		if(csses){
			for (i=0; i<csses.length; i++) {
				c = tinymce.isIE ? csses[i].style.cssText : csses[i].cssText;
				if(!c.match(/mce(Item|Editable|NonEditable)/gi) || c.indexOf(ed.settings.visual_table_class) == 0){
					c = c.match(re);
					if(c){
						c = c[0];
						if(/#/.test(c)){
							if(c.length == 4) c = c + c.substring(1, 4);
						}else{
							c = convertRGBToHex(c);
						}
						if(tinymce.inArray(clr, c) == -1){
							clr.push(c);
						}
					}
				}
...
function generateTemplateColors() {
	var el = document.getElementById('templatecolors'), h = '', i;	
	var customColors = tinyMCEPopup.editor.settings.custom_colors || '';
	if (el.className == 'generated')
		return;
	...
	h += '<table border="0" cellspacing="1" cellpadding="0">'
		+ '<tr>';
	if(templateColors){
		for (i=0; i<templateColors.length; i++) {
			h += '<td bgcolor="' + templateColors[i] + '" width="10" height="10">'
				+ '<a href="javascript:insertAction();" onfocus="showColor(\'' + templateColors[i] +  '\');" onmouseover="showColor(\'' + templateColors[i] +  '\');" style="display:block;width:10px;height:10px;overflow:hidden;">'
				+ '</a></td>';
			if ((i+1) % 18 == 0){
				h += '</tr><tr>';
			}
	...
	if(customColors){
		...
		for (i=0; i<customColors.length; i++) {
			h += '<td bgcolor="' + customColors[i] + '" width="10" height="10">'
				+ '<a href="javascript:insertAction();" onfocus="showColor(\'' + customColors[i] +  '\');" onmouseover="showColor(\'' + customColors[i] +  '\');" style="display:block;width:10px;height:10px;overflow:hidden;">'
				+ '</a></td>';
			if ((i+1) % 18 == 0){
				h += '</tr><tr>';
			}
