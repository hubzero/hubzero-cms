...
(function() {
	...
	tinymce.create('tinymce.plugins.SpellcheckerPlugin', {
		...
		_showMenu : function(ed, e) {
			...
			if (dom.hasClass(e.target, 'mceItemHiddenSpellWord')) {
				...
				t._sendRPC('getSuggestions', [t.selectedLang, dom.decode(e.target.innerHTML)], function(r) {
					...
					if(ed.getParam('spellchecker_engine') == 'pspell') {
						m.add({
		...
		_sendRPC : function(m, p, cb) {
			// JCE plugin modification
			var t = this, url = t.editor.getParam("site_url") + 'index.php?option=com_jce&task=plugin&plugin=spellchecker&file=rpc&cid=' + t.editor.getParam('component_id');
