...
<div class="list-editing">
 <p><?php echo ucfirst(JText::_('COM_PROJECTS_SHOWING')); ?> <?php if ($this->total <= count($this->team)) { echo JText::_('COM_PROJECTS_ALL'); }?> <span class="prominent"><?php echo count($this->team); ?></span>  <?php if ($this->total > count($this->team)) { echo JText::_('COM_PROJECTS_OUT_OF').' '.$this->total; }?> <?php echo JText::_('COM_PROJECTS_TEAM_MEMBERS'); ?>
	...
	<span class="editlink"><a href="<?php echo Route::url('index.php?option=' . $this->option . '&task=edit' . $goto . '&active=team'); ?>"><?php echo JText::_('COM_PROJECTS_EDIT_TEAM'); ?></a></span>
	<?php } ?></p>
</div>
...
<?php } ?>
			...
			<div class="nav_pager">
				...
				<?php	} else {  ?>
					<a href="<?php echo Route::url('index.php?option=' . $this->option . $goto . '&active=team') . '/?action=view&amp;t_sortby=' . $this->filters['sortby'] . '&amp;t_limitstart=' . $prev_start . '&amp;t_sortdir=' . $this->filters['sortdir']; ?>" class="ajax_action">&laquo; <?php echo JText::_('COM_PROJECTS_PREVIOUS'); ?></a>
				<?php } ?><span>&nbsp; | &nbsp;</span>
				<?php
				if ( $whatsleft <= 0 or $this->filters['limit'] == 0 ) { ?>
					<span><?php echo JText::_('COM_PROJECTS_NEXT'); ?> &raquo;</span>
				<?php	} else { ?>
					<a href="<?php echo Route::url('index.php?option=' . $this->option . $goto . '&active=team') . '/?action=view&amp;t_sortby=' . $this->filters['sortby'] . '&amp;t_limitstart=' . $next_start . '&amp;t_sortdir=' . $this->filters['sortdir']; ?>" class="ajax_action"><?php echo JText::_('COM_PROJECTS_NEXT'); ?> &raquo;</a>
				<?php } ?>
				</p>
			</div>
			<?php if (($this->project->role != 1 || $this->managers_count > 1) && !$this->setup && $this->project->owner_group == 0) { ?>
			<p class="extras"><a href="<?php echo Route::url('index.php?option=' . $this->option . $goto . '&active=team&action=quit'); ?>"><?php echo JText::_('COM_PROJECTS_LEAVE_PROJECT'); ?></a></p>
			<?php } ?>
</form>
