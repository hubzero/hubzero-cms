...
?>
<div id="plg-header">
	<h3 class="files"><a href="<?php echo Route::url('index.php?option='.$this->option . '&alias=' . $this->model->get('alias') . '&active=files'); ?>"><?php echo $this->title; ?></a> &raquo; <span class="subheader"><?php echo JText::_('PLG_PROJECTS_FILES_CONNECT'); ?></span></h3>
</div>
...
<div id="connections">
	...
	<div class="connect-service <?php echo !$service['on'] ? 'inactive' : ''; ?> <?php echo $servicename; ?>">
		...
		<div class="connect-info">
			...
			<?php } else { ?>
				<p class="connect-action"><a href="<?php echo Route::url('index.php?option=' . $this->option . '&alias=' . $this->model->get('alias') . '&active=files') . '?action=connect&amp;service=' . $servicename; ?>"><?php echo JText::_('PLG_PROJECTS_FILES_CONNECT'); ?></a></p>
			<?php } ?>
		</div>
		<?php } ?>
		<div class="service-info">
			...
			<p>
				<span class=" <?php echo $creator ? ' creator' : ''; ?>">
					<a href="<?php echo Route::url('index.php?option=' . $this->option . '&alias=' . $this->model->get('alias') . '&active=files') . '?action=disconnect&amp;service=' . $servicename . $removeData; ?>" id="disconnect"><?php echo ucfirst(JText::_('PLG_PROJECTS_FILES_CONNECT_DISCONNECT')); ?> &raquo;</a>
				</span>
				&nbsp; &nbsp;
				<span>
					<a href="<?php echo Route::url('index.php?option=' . $this->option . '&alias=' . $this->model->get('alias') . '&active=files') . '?action=connect&amp;reauth=1&amp;service=' . $servicename; ?>"><?php echo JText::_('PLG_PROJECTS_FILES_CONNECT_REAUTH'); ?> &raquo;</a>
				</span>
			</p>
			<?php } ?>
			<?php }
