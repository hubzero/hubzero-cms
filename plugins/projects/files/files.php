...
{	
		...
		$areas = null, $case = 'files')
	{
		...
		if ($returnhtml) 
		{
			...
			{					
				...
				// Sync Manager
				require_once( JPATH_ROOT . DS . 'plugins' . DS . 'projects' . DS . 'files' . DS . 'extensions' . DS . 'sync.php');					
			}
			...
			{				
				...
				case 'connect':
				case 'disconnect': 
					$arr['html'] 	= plgProjectsFilesSync::connect(); 
					break;
				
				case 'sync': 
					$arr['html'] 	= plgProjectsFilesSync::iniSync(); 
					break;
				case 'sync_status':
					$arr['html'] 	= plgProjectsFilesSync::syncStatus();
					break;
	 ...
	 * @return     array   Return array of html
	 */
	public function onProjectExternal ( $identifier, $action, $uid, $case = 'files')
	{
		// Sync Manager
		require_once( JPATH_ROOT . DS . 'plugins' . DS . 'projects' . DS . 'files' . DS . 'extensions' . DS . 'external.php');
		
		return plgProjectsFilesExternal::onProjectExternal($identifier, $action, $uid, $case);
	...
	{				
		...
		if (!$file) 
		{
			...
			elseif ($render == 'download')
			{
				// Multi-file download
				$multifile = 1;
				$archive = $this->_archiveFiles($items, $this->prefix . $path, $subdir);
