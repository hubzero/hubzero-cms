/*
 * Modified to add Dataview specific features.
 * Get the original from: http://www.datatables.net 
 *
 *
...
(function($, window, document) {
	...
	_oExt.oJUIClasses = {
		...
		"sSortColumn": "sorting_", /* Note that an int is postfixed for the sorting order */
		"sSortJUIAsc": "css_left ui-icon ui-icon-triangle-1-n",
		"sSortJUIDesc": "css_left ui-icon ui-icon-triangle-1-s",
		"sSortJUI": "css_left ui-icon ui-icon-carat-2-n-s",
		"sSortJUIAscAllowed": "css_left ui-icon ui-icon-carat-1-n",
		"sSortJUIDescAllowed": "css_left ui-icon ui-icon-carat-1-s",
	...
	$.fn.dataTable = function( oInit )
	{
		...
		function _fnFilterColumn ( oSettings, sInput, iColumn, bRegex, bSmart )
		{
			...
			if (sInput === "" || sInput === ">" || sInput === "<" || sInput === ">=" || sInput === "<=" || sInput === "=" || sInput === "!") {
				return;
			}
			...
			if (sInput.indexOf("=") == 0) {
				sInput = sInput.split('=');
				var sInput = sInput[1].toUpperCase();
				for (var i=oSettings.aiDisplay.length-1 ; i>=0 ; i--) {
					var sData = _fnDataToSearch(oSettings.aoData[ oSettings.aiDisplay[i] ]._aData[iColumn], 'html');
					sData = sData.trim().toUpperCase();
					if (sInput != sData) {
			...
			if (oSettings.aoColumns[iColumn].sType === 'datetime') {
				sInput = sInput.replace(/-/g, '/');

				if (sInput.indexOf("to") !== -1) {
					var minmax = sInput.split('to');
					var mi, ma;
					
					mi = new Date(minmax[0].trim()).getTime();
					ma = new Date(minmax[1].trim()).getTime();
					
					for (var i=oSettings.aiDisplay.length-1 ; i>=0 ; i--) {
						var d_date = new Date(_fnDataToSearch(oSettings.aoData[ oSettings.aiDisplay[i] ]._aData[iColumn], oSettings.aoColumns[iColumn].sType).replace(/-/g, '/')).getTime();
						if (!(mi <= d_date && d_date <= ma)) {
							oSettings.aiDisplay.splice( i, 1 );
							iIndexCorrector++;
						}
					}
					...
					sInput = sInput.split('<=');
					var s_date = new Date(sInput[1].trim()).getTime();
					if (isNaN(s_date)) { return; }
					...
					var s_date = new Date(sInput).getTime();
					if (isNaN(s_date)) { return; }
				
					for (var i=oSettings.aiDisplay.length-1; i>=0; i--) {
						var d_date = new Date(_fnDataToSearch(oSettings.aoData[ oSettings.aiDisplay[i] ]._aData[iColumn], oSettings.aoColumns[iColumn].sType).replace(/-/g, '/')).getTime();
						if (isNaN(d_date) || s_date !== d_date){
							oSettings.aiDisplay.splice(i, 1);
							iIndexCorrector++;
						}
					}
		...
		function _fnFilterCreateSearch( sSearch, bRegex, bSmart )
		{
			...
			if ( bSmart )
			{
				...
				return new RegExp( sRegExpString, "im" );
			}
			else
			{
