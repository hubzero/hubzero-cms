	...
	<table class="listing" id="todo-table">
		...
		<?php if (count($rows) > 0) {  $order = 1; ?>
			...
			<tr class="<?php echo $class; ?>" id="todo-<?php echo $row->get('id'); ?>">
				...
				<td><?php if (!$row->isComplete()) { ?><a href="<?php echo JRoute::_($url . '&action=changestate') . '/?todoid=' . $row->get('id') . '&amp;state=1&' . JSession::getFormToken() . '=1'; ?>" title="<?php echo JText::_('PLG_PROJECTS_TODO_CHECK_OFF'); ?>" class="unchecked confirm-checkoff">&nbsp;</a><?php } ?></td>
			</tr>
			<?php $order++; } ?>
		<?php } else { ?>
			<tr>
