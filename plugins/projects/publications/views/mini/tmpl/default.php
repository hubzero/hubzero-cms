...
?>
<div class="sidebox">
		<h4 class="publications"><a href="<?php echo Route::url($this->model->link('publications')); ?>" class="hlink" title="<?php echo Lang::txt('COM_PROJECTS_VIEW') . ' ' . strtolower(Lang::txt('COM_PROJECTS_PROJECT')) . ' ' . strtolower(Lang::txt('COM_PROJECTS_TAB_PUBLICATIONS')); ?>"><?php echo ucfirst(Lang::txt('COM_PROJECTS_TAB_PUBLICATIONS')); ?></a>
...
<?php if (count($this->items) == 0) { ?>
	<div class="noitems">
		...
		<p class="addnew"><a href="<?php echo Route::url($this->model->link('publications') . '&action=start'); ?>"><?php echo Lang::txt('COM_PROJECTS_ADD_PUBLICATION'); ?></a></p>
	</div>
<?php } else { ?>
	<ul>
