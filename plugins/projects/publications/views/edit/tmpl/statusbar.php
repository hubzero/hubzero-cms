...
<?php } ?>	
	...
	<ul id="status-bar" <?php if($this->move) { echo 'class="moving"'; } ?>>
		...
		<?php for($i=0; $i < count($this->panels); $i++) { 
			$panel = $this->panels[$i];
			//$passed = (($this->checked[$panel] == 1 || $this->last_idx > $i) or $this->version != 'dev') ? 1 : 0;
			$passed = ($this->checked[$panel] == 1 || $this->last_idx > $i) ? 1 : 0;
			if($this->lastpane == 'review') {
				$passed = ($this->checked[$panel] == 1) ? 1 : 0;
		...
		?>
			...
			else { ?><a href="<?php echo $this->url.'/?section=description'.$version.$move.$review; ?>" class="<?php echo $passed ? 'c_passed': 'c_failed'; ?>"><?php echo JText::_('PLG_PROJECTS_PUBLICATIONS_PUB_TITLE_ABSTRACT'); ?></a><?php } ?> <span class="proceedto">&nbsp;</span> <?php if($this->step == 'abstract') { echo $passed ? '<a href="'. $this->url.'/?section=description'.$version.$move.$review.a.'step=metadata" >'.JText::_('PLG_PROJECTS_PUBLICATIONS_PUB_METADATA').'</a>' : '<span>'.JText::_('PLG_PROJECTS_PUBLICATIONS_PUB_METADATA').'</span>'; } else { echo '<span class="active">'.JText::_('PLG_PROJECTS_PUBLICATIONS_PUB_METADATA').'</span>'; } ?></li>
		<?php } ?>
			
		<li <?php if($this->active == $panel) { echo 'class="active"'; } ?>>
		<?php if($this->move) { ?>
			<?php if($i < $this->current_idx) { ?><a href="<?php echo $this->url.'/?section='.$panel.$version.$move.$review; ?>"  class="<?php if($passed) { echo 'c_passed'; } else { echo 'c_failed'; } ?>"><?php } ?><?php echo JText::_('PLG_PROJECTS_PUBLICATIONS_PANEL_'.strtoupper($panel)); ?><?php if($i < $this->current_idx) { ?></a><?php } ?>
		<?php } else { ?>	
			<?php if($i != $this->current_idx or $this->active == 'version' or $review) { ?><a href="<?php echo $this->url.'/?section='.$panel.$version.$move.$review; ?>" class="<?php if($passed) { echo 'c_passed'; } else { echo 'c_failed'; }?>"><?php } ?><?php echo JText::_('PLG_PROJECTS_PUBLICATIONS_PANEL_'.strtoupper($panel)); ?><?php if($i != $this->current_idx or $this->active == 'version' or $review) { ?></a><?php } ?>
		<?php } ?>			
		</li>
		<?php } ?>
	</ul>
