			...
			<table class="tbl-panel">
				<tbody>
					...
					<tr>
						...
						<td class="tbl-input"><?php echo JHTML::_('date', $this->row->created, 'M d, Y').' ('.\Components\Projects\Helpers\Html::timeAgo($this->row->created).' '.JText::_('PLG_PROJECTS_PUBLICATIONS_AGO').')'; ?></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="tbl-input"><?php echo JHTML::_('date', $this->row->published_up, 'M d, Y').' ('.\Components\Projects\Helpers\Html::timeAgo($this->row->published_up).' '.JText::_('PLG_PROJECTS_PUBLICATIONS_AGO').')'; ?></td>
					</tr>
					<?php } ?>
					<?php if ($this->row->accepted != '0000-00-00 00:00:00') { ?>
					<tr>
						<td class="tbl-lbl"><?php echo JText::_('PLG_PROJECTS_PUBLICATIONS_ACCEPTED'); ?>:</td>
						<td class="tbl-input"><?php echo JHTML::_('date', $this->row->accepted, 'M d, Y').' ('.\Components\Projects\Helpers\Html::timeAgo($this->row->accepted).' '.JText::_('PLG_PROJECTS_PUBLICATIONS_AGO').')'; ?></td>
					</tr>
					<?php } ?>
					...
					<tr>
						...
						<td class="tbl-input"><?php echo JHTML::_('date', $date, 'M d, Y').' ('.\Components\Projects\Helpers\Html::timeAgo($date).' '.JText::_('PLG_PROJECTS_PUBLICATIONS_AGO').')'; ?></td>
					</tr>
					<?php } ?>
					<?php if ($this->pub->state == 0) { ?>
					<tr>
						<td class="tbl-lbl"><?php echo ucfirst(JText::_('PLG_PROJECTS_PUBLICATIONS_UNPUBLISHED')); ?>:</td>
						<td class="tbl-input"><?php echo JHTML::_('date', $this->row->published_down, 'M d, Y').' ('.\Components\Projects\Helpers\Html::timeAgo($this->row->published_down).' '.JText::_('PLG_PROJECTS_PUBLICATIONS_AGO').')'; ?></td>
					</tr>
					<?php } ?>
					<tr>
		 ...
		 <div class="c-inner">
			...
			<?php if ($this->version == 'dev' || $this->row->state == 4) { // draft (initial or final) ?>
				...
				<li id="next-ready"><p><?php echo JText::_('PLG_PROJECTS_PUBLICATIONS_WHATS_NEXT_SAVE');  ?></p>
					<p class="centeralign"><span class="<?php echo $this->publication_allowed ? 'btn' : 'btn disabled'; ?>"><?php if ($this->publication_allowed) {  ?><a href="<?php echo $this->url . '/?action=post&amp;version='.$this->version; ?>"><?php } ?><?php echo JText::_('PLG_PROJECTS_PUBLICATIONS_PUB_SAVE_REVIEW'); ?><?php if ($this->publication_allowed) {  ?></a><?php } ?></span></p>
				</li>
				<?php } ?>
				<li id="next-cancel"><p><?php echo JText::_('PLG_PROJECTS_PUBLICATIONS_WHATS_NEXT_NEED_TO_CANCEL') .' <a href="' . $this->url . '/?action=cancel&amp;version=' . $this->version . '">' . JText::_('PLG_PROJECTS_PUBLICATIONS_WHATS_NEXT_CANCEL') . '</a> ' . JText::_('PLG_PROJECTS_PUBLICATIONS_WHATS_NEXT_CANCEL_BEFORE');  ?></p></li>
			<?php } ?>
			...
			<?php if ($this->row->state == 1) { // published ?>
				...
				.JText::_('PLG_PROJECTS_PUBLICATIONS_WHATS_NEXT_FOLLOW_FEEDBACK');  ?>
					<span class="block italic"><a href="<?php echo $this->url . '/?action=stats' . '&amp;version='.$this->version; ?>"><?php echo JText::_('PLG_PROJECTS_PUBLICATIONS_VIEW_USAGE'); ?> &raquo;</a></span></p></li>
				<?php if ($showCitations) { ?>
				<li id="next-citation"><p><?php echo JText::_('PLG_PROJECTS_PUBLICATIONS_WHATS_NEXT_WATCH_ADD_CITATIONS');  ?>
					<span class="block italic"><a href="<?php echo $this->url . '/?section=citations&amp;version=' . $this->version; ?>"><?php echo JText::_('PLG_PROJECTS_PUBLICATIONS_ADD_CITATIONS'); ?> &raquo;</a></span></p></li>
				<?php } ?>
			<?php } ?>
