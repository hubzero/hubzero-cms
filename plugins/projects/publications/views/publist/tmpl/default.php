...
<div class="public-list-wrap">
	<ul class="public-list">
		...
		?>
		<li><a href="<?php echo Route::url($route . '&id=' . $item->id);  ?>"><span class="pub-image"><img src="<?php echo Route::url('index.php?option=com_publications&id=' . $item->id . '&v=' . $item->version_id) . '/Image:thumb'; ?>" alt="" /></span> <?php echo $item->title; ?></a> <span class="public-list-info"> - <?php echo JText::_('COM_PROJECTS_PUBLISHED') . ' ' . JHTML::_('date', $item->published_up, 'M d, Y') . ' ' . JText::_('COM_PROJECTS_IN') . ' <a href="' . Route::url('index.php?option=com_publications&category=' . $item->cat_url) . '">' . $item->cat_name . '</a>'; ?></span></li>
		<?php
		} ?>
	</ul>
</div>
