...
JPlugin::loadLanguage( 'plg_groups_projects' );

//-----------

class plgGroupsProjects extends JPlugin
{
	
	public function plgGroupsProjects(&$subject, $config) 
	{
		...
		if ($activities && count($activities) > 0 ) {
			foreach($activities as $a) { 
				...
				if(!in_array($a->id, $shown)) {
					...
					$prep[] = array('activity' => $a, 'eid' => $eid, 'etbl' => $etbl, 'body' => $ebody, 'deletable' => $deletable, 'comments' => $comments, 'class' => $class, 'timeclass' => $timeclass, 'projectid' => $a->projectid, 'recorded' => $a->recorded);
				}
			}
		}	
	...
	public function onGroupUsage( $group ) 
	{
