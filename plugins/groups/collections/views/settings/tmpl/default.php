...
<?php } ?>
	<form action="<?php echo JRoute::_('index.php?option=' . $this->option . '&cn=' . $this->group->get('cn') .'&active=collections&action=savesettings'); ?>" method="post" id="hubForm" class="full">
		...
		<fieldset class="settings">
			<legend><?php echo JText::_('Collections'); ?></legend>

			<label for="param-posting">
				Who can create collections?
				<select name="params[create_collection]" id="param-create_collection">
					<option value="0"<?php if (!$this->params->get('create_collection', 1)) { echo ' selected="selected"'; }?>>All group members</option>
					<option value="1"<?php if ($this->params->get('create_collection', 1) == 1) { echo ' selected="selected"'; }?>>Group managers only</option>
				</select>
			</label>
			
			<p class="info">
				<strong>Default settings:</strong> Group managers can add/edit/delete any collection. Standard group members cannot create collections.
			</p>
		</fieldset>
		<fieldset class="settings">
			...
			<label for="param-posting">
				...
				<select name="params[create_post]" id="param-create_post">
					<option value="0"<?php if (!$this->params->get('create_post', 0)) { echo ' selected="selected"'; }?>>All group members</option>
					<option value="1"<?php if ($this->params->get('create_post', 0) == 1) { echo ' selected="selected"'; }?>>Group managers only</option>
				</select>
			</label>
			
			<p class="info">
				<strong>Default settings:</strong> All group member can create posts.
			</p>
		...
		<p class="submit">
			...
			<a href="<?php echo JRoute::_('index.php?option='.$this->option.'&cn='.$this->group->get('cn').'&active=collections'); ?>">Cancel</a>
		</p>
	</form>
