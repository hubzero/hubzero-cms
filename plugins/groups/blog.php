...
class plgGroupsBlog extends JPlugin
{
	public function plgGroupsBlog(&$subject, $config)
	{
		...
		$this->_plugin = JPluginHelper::getPlugin( 'groups', 'blog' );
	...
	private function _feed() 
	{
		...
		if (count($rows) > 0) {
			...
			foreach ($rows as $row)
			{
				...
				// URL link to article
				$link = JRoute::_('index.php?option='.$this->option.'&gid='.$this->group->cn.'&active=blog&scope='.JHTML::_('date',$row->publish_up, '%Y', 0).'/'.JHTML::_('date',$row->publish_up, '%m', 0).'/'.$row->alias);
