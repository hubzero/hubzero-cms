...
class plgUsageOverview extends JPlugin
{
	...
	private function getSparkline($db, $id, $period, $datetime) 
	{
		...
		if ($results) {
			...
			foreach ($results as $result) 
			{
				...
				$sparkline .= "\t".'<span class="index"><span class="count" style="height: '.$height.'%;" title="'.JHTML::_('date', $result->datetime, '%d %b. %Y').': '.trim($this->fmt_result($result->value,$result->valfmt)).'">'.trim($this->fmt_result($result->value,$result->valfmt)).'</span> </span>'."\n";
			}
			$sparkline .= '</span>'."\n";
		}
	...
	public function onUsageDisplay( $option, $task, $db, $months, $monthsReverse, $enddate ) 
	{
		...
		if (!$selectedPeriod) {
	        $sql = "SELECT MAX(datetime) FROM summary_user_vals";
			$db->setQuery( $sql );
		...
		if ($results) {
			...
			foreach ($results as $row)
			{
				...
				$html .= "\t"."\t".'<tr class="'.$cls.'">'."\n";
				$html .= "\t"."\t"."\t".'<th scope="row">'.trim($label).'</th>'."\n";
				$html .= "\t"."\t"."\t".'<td>'. $this->fmt_result($row->value,$row->valfmt) .'</td>'."\n";
				if ($row->plot == '1') {
...
/*		
		...
		*/
		...
		$html .= "\t".'<caption><a name="tot"></a>'.JText::_('Table 4: User statistics by registered/unregistered').'</caption>'."\n";
		$html .= "\t".'<thead>'."\n";
		$html .= "\t"."\t".'<tr>'."\n";
		$html .= "\t"."\t"."\t".'<th scope="col" rowspan="2" colspan="2">'.JText::_('Users').'</th>'."\n";
		$html .= "\t"."\t"."\t".'<th scope="col" rowspan="2" class="numerical-data">'.JText::_('Totals').'</th>'."\n";
		...
		$html .= "\t"."\t".'<li id="fn4"><a name="fn4"></a>Number of Unregistered users, identified by unique hosts/IPs, that had an active session of less than 15 minutes without logging in and downloaded a non-interactive resource such as PDF or podcas"\t". Does not include web bots/crawlers.</li>'."\n";	
		...
		$html .= "\t"."\t".'<li id="fn7"><a name="fn7"></a>All Unregistered users, identified by unique hosts/IPs that downloaded a non-interactive resource such as PDF or podcas"\t". Does not include known web bots/crawlers.</li>'."\n";					
		...
		$html .= "\t"."\t".'<li id="fn11"><a name="fn11"></a>Based on MIT OCW metric of Visitors: Visitors are identified as Unique IP addresses</li>'."\n";
