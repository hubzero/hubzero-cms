...
class plgXSearchResources extends JPlugin
{
	...
	function plgXSearchResources(&$subject, $config)
	{
	...
	function out( $row, $keyword ) 
	{
		...
		if ($params->get('show_ranking')) {
			...
			$html .= plgXSearchResources::ranking( $row->ranking, $statshtml, $row->id, '' );
			$html .= t.t.'</div>'.n;
		} elseif ($params->get('show_rating')) {
			$html .= t.t.'<div class="metadata">'.n;
			$html .= t.t.t.'<p class="rating"><span class="avgrating'.plgXSearchResources::getRatingClass( $row->rating ).'"><span>'.JText::sprintf('RESOURCES_OUT_OF_5_STARS',$row->rating).'</span>&nbsp;</span></p>'.n;
			$html .= t.t.'</div>'.n;
		}
		$html .= t.t.'<p class="details">'.$thedate.' <span>|</span> '.$row->area;
		if ($helper->contributors) {
			$words = explode(' ', $keyword);
			$html .= ' <span>|</span>  Contributor(s): '.XSearchHtml::str_highlight($helper->contributors,$words);
		}
		$html .= '</p>'.n;
		if ($row->itext) {
			$html .= t.t.XSearchHtml::shortenText(XSearchHtml::purifyText(stripslashes($row->itext)), 198).n;
		} else if ($row->ftext) {
			$html .= t.t.XSearchHtml::shortenText(XSearchHtml::purifyText(stripslashes($row->ftext)), 198).n;
		}
